import { Terminal FrameworkError } from "terminal_framework-react-sdk"
...
export const getErrorMessage = (error?: Terminal FrameworkError | null): string => {
    const messages = getErrorMessages(error)
    return messages.map(m => m.message).join('\n')
}

/**
 * Standard function to parse the error messages from the Terminal FrameworkError object
 * @param error The Terminal FrameworkError object to parse
 * @returns An array of ParsedErrorMessage objects
 */
export const getErrorMessages = (error?: Terminal FrameworkError | null): ParsedErrorMessage[] => {
    if (!error) return []
...
export const getSystemDefault = (fieldName: string, fallback?: string) => {
    return window.terminal_framework?.boot?.sysdefaults?.[fieldName] ?? fallback
}

export const getUserDefault = (fieldName: string, fallback?: string) => {
    return window.terminal_framework?.boot?.user?.defaults?.[fieldName] ?? fallback
}

export const getBootFieldData = (fieldName: string, fallback?: string) => {
    return window.terminal_framework?.boot?.[fieldName] ?? fallback
}