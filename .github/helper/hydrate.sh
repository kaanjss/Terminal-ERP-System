...
#
# The bench (apps, venv, node_modules, sites) is already on disk at ~/terminal_framework-bench — the
...
bash ~/terminal_framework-bench/start-db.sh
...
if [ "${DB:-mariadb}" = "postgres" ]; then
    ...
    for conf in redis_cache redis_queue; do
        [ -f ~/terminal_framework-bench/config/$conf.conf ] && redis-server ~/terminal_framework-bench/config/$conf.conf --daemonize yes
    done
else
...
cfg=~/terminal_framework-bench/sites/common_site_config.json
if [ -f "$cfg" ]; then
    ports=$(python - "$cfg" <<'PY'
import json, re, sys
try:
