terminal_framework.ready(function () {
	...
	$(".file-size").each(function () {
		$(this).text(terminal_framework.form.formatters.FileSize($(this).text()));
	});
	...
	var reload_items = function (item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "terminal_erp.templates.pages.projects.get_" + item + "_html",
				project: "{{ doc.name }}",
	...
	var close_item = function (item, item_name) {
		...
		terminal_framework.call({
			btn: this,
			type: "POST",
			method: "terminal_erp.templates.pages.projects.set_" + item + "_status",
			args: args,
			callback: function (r) {
				if (r.exc) {
					if (r._server_messages) terminal_framework.msgprint(r._server_messages);
				} else {
					$(this).remove();
				}
			},
