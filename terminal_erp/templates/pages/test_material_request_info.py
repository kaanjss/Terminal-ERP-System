...
from terminal_framework.utils import now, today

from terminal_erp.templates.pages.material_request_info import get_more_items_info
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestMaterialRequestInfo(Terminal ERPTestSuite):
	...
	def _make_linked_work_order(self, material_request):
		...
		wo.name = terminal_framework.generate_hash("wo-mri", 12)
		wo.db_insert()

		wo_item = terminal_framework.new_doc("Work Order Item")
		...
		wo_item.name = terminal_framework.generate_hash("woi-mri", 12)
		wo_item.db_insert()
	...
	def _make_stock_entry_detail(self, transfer_qty, docstatus=1):
		...
		"""
		sed = terminal_framework.new_doc("Stock Entry Detail")
		sed.parent = terminal_framework.generate_hash("se-mri", 12)
		...
		sed.name = terminal_framework.generate_hash("sed-mri", 12)
		sed.db_insert()
		return sed
	...
	def test_delivered_qty_sums_submitted_stock_entry_details(self):
		...
		items = [terminal_framework._dict({"item_code": self.item_code})]
	...
	def test_excluded_status_work_order_is_filtered_out(self):
		...
		terminal_framework.db.set_value("Work Order", self.work_order.name, "status", "Completed")
