...
from terminal_framework.core.utils import html2text
from terminal_framework.utils import sanitize_html
from terminal_framework.utils.global_search import search
from jinja2 import utils


def get_context(context):
	context.no_cache = 1
	if terminal_framework.form_dict.q:
		query = str(utils.escape(sanitize_html(terminal_framework.form_dict.q)))
		context.title = _("Help Results for")
		context.query = query

		context.route = "/search_help"
		d = terminal_framework._dict()
...
def get_help_results_sections(text: str):
	out = []
	settings = terminal_framework.get_doc("Support Settings", "Support Settings")
