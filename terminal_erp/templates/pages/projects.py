...
def get_context(context):
	project_user = terminal_framework.db.get_value(
		"Project User",
		{"parent": terminal_framework.form_dict.project, "user": terminal_framework.session.user},
		["user", "view_attachments", "hide_timesheets"],
		as_dict=True,
	)
	if terminal_framework.session.user != "Administrator" and (not project_user or terminal_framework.session.user == "Guest"):
		raise terminal_framework.PermissionError

	context.no_cache = 1
	context.show_sidebar = True
	project = terminal_framework.get_doc("Project", terminal_framework.form_dict.project)

	project.has_permission("read")

	project.tasks = get_tasks(
		project.name, start=0, item_status="open", search=terminal_framework.form_dict.get("search")
	)

	if project_user and not project_user.hide_timesheets:
		project.timesheets = get_timesheets(project.name, start=0, search=terminal_framework.form_dict.get("search"))
...
def get_timesheets(project, start=0, search=None):
	...
	timesheets = terminal_framework.get_all(
		"Timesheet Detail",
		filters=filters,
	...
	for timesheet in timesheets:
		info = terminal_framework.get_all(
			"Timesheet",
