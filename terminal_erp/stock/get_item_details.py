...
from terminal_framework.model import child_table_fields, default_fields
from terminal_framework.model.document import Document
from terminal_framework.model.meta import get_field_precision
from terminal_framework.model.utils import get_fetch_values
from terminal_framework.query_builder.functions import IfNull, NullIf, Sum
from terminal_framework.utils import add_days, add_months, cint, cstr, flt, get_link_to_form, getdate, parse_json

import terminal_erp
from terminal_erp import get_company_currency
from terminal_erp.accounts.doctype.pricing_rule.pricing_rule import (
	get_pricing_rule_for_item,
	set_transaction_type,
)
from terminal_erp.setup.doctype.brand.brand import get_brand_defaults
from terminal_erp.setup.doctype.item_group.item_group import get_item_group_defaults
from terminal_erp.setup.utils import get_exchange_rate
from terminal_erp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from terminal_erp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from terminal_erp.stock.doctype.price_list.price_list import get_price_list_details

ItemDetails = terminal_framework._dict
ItemDetailsCtx = terminal_framework._dict
ItemPriceCtx = terminal_framework._dict
...
@terminal_erp.normalize_ctx_input(ItemDetailsCtx)
def get_item_details(
	...
	item = terminal_framework.get_cached_doc("Item", ctx.item_code)
	...
	if (
		...
		and terminal_framework.get_single_value("Selling Settings", "fallback_to_default_price_list")
	):
		fallback_args = ctx.copy()
		fallback_args.price_list = terminal_framework.get_single_value("Selling Settings", "selling_price_list")
	...
	if (
		terminal_framework.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward")
...
def set_valuation_rate(out: ItemDetails | dict, ctx: ItemDetailsCtx):
	...
	if active_bundle:
		valuation_rate = 0.0
		bundled_items = terminal_framework.get_doc("Product Bundle", active_bundle)
...
def update_stock(ctx, out, doc=None):
	from terminal_erp.stock.doctype.batch.batch import get_available_batches
	from terminal_erp.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
	...
	if (
		...
		if doc and isinstance(doc, dict):
			doc = terminal_framework._dict(doc)

		kwargs = terminal_framework._dict(
			{
				"item_code": ctx.item_code,
				"warehouse": ctx.warehouse,
				"based_on": terminal_framework.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
...
def get_item_code(barcode=None, serial_no=None):
	if barcode:
		item_code = terminal_framework.db.get_value("Item Barcode", {"barcode": barcode}, fieldname=["parent"])
		if not item_code:
			terminal_framework.throw(_("No Item with Barcode {0}").format(barcode))
	elif serial_no:
		item_code = terminal_framework.db.get_value("Serial No", serial_no, "item_code")
		if not item_code:
			terminal_framework.throw(_("No Item with Serial No {0}").format(serial_no))
...
def validate_item_details(ctx: ItemDetailsCtx, item):
	...
	from terminal_erp.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: ItemDetailsCtx, item, overwrite_warehouse=True) -> ItemDetails:
	...
	if item.variant_of and not item.taxes and terminal_framework.db.exists("Item Tax", {"parent": item.variant_of}):
	...
	defaults = terminal_framework._dict(
		{
	...
	if ctx.doctype == "Material Request" and not ctx.material_request_type:
		ctx["material_request_type"] = terminal_framework.db.get_value(
	...
	if item.is_fixed_asset:
		from terminal_erp.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
		...
		if is_cwip_accounting_enabled(item.asset_category):
			...
			from terminal_erp.assets.doctype.asset_category.asset_category import get_asset_category_account

			expense_account = get_asset_category_account(
				fieldname="fixed_asset_account", item=ctx.item_code, company=ctx.company
			)

	company_values = terminal_framework.get_cached_value(
		"Company",
		ctx.company,
		[
	...
	if ctx.batch_no and item.name != terminal_framework.get_cached_value("Batch", ctx.batch_no, "item"):
		ctx.batch_no = ""
	...
	if not item.is_stock_item and not out.expense_account:
		out.expense_account = terminal_framework.get_cached_value("Company", ctx.company, "service_expense_account")
	...
	if ctx.doctype in purchase_doctypes and not terminal_framework.db.get_single_value(
		"Buying Settings", "disable_last_purchase_rate"
	):
		from terminal_erp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	if (
		item.is_stock_item
		and terminal_erp.is_perpetual_inventory_enabled(ctx.company)
		and (
	...
	for d in [
		...
		if not out[d[1]]:
			out[d[1]] = terminal_framework.get_cached_value("Company", ctx.company, d[2]) if d[2] else None
	...
	meta = terminal_framework.get_meta(child_doctype)
...
def get_item_warehouse_(ctx: ItemDetailsCtx, item, overwrite_warehouse, defaults=None):
	...
	if not warehouse:
		default_warehouse = terminal_framework.get_single_value("Stock Settings", "default_warehouse")
		if (
			default_warehouse
			and terminal_framework.get_cached_value("Warehouse", default_warehouse, "company") == ctx.company
		):
...
def get_barcode_data(items_list=None, item_code=None):
	...
	if not items_list and item_code:
		_dict_item_code = terminal_framework._dict(
			{
				"item_code": item_code,
			}
		)

		items_list = [terminal_framework._dict(_dict_item_code)]

	for item in items_list:
		barcodes = terminal_framework.db.get_all("Item Barcode", filters={"parent": item.item_code}, fields="barcode")
...
def get_item_tax_info(
	...
	for item_code in item_codes:
		...
		item = terminal_framework.get_cached_doc("Item", item_code[0])
		ctx: ItemDetailsCtx = {
			"company": doc.company,
...
def get_item_tax_template(
	...
	if not item:
		if not ctx.get("item_code"):
			terminal_framework.throw(_("Item/Item Code required to get Item Tax Template."))
		else:
			item = terminal_framework.get_cached_doc("Item", ctx.item_code)
...
def _get_item_tax_template_from_item_group(ctx, item_group, out=None):
	from terminal_framework.utils.nestedset import get_ancestors_of

	ancestors = get_ancestors_of("Item Group", item_group)
	for group in [item_group, *ancestors]:
		group_doc = terminal_framework.get_cached_doc("Item Group", group)
...
def _get_item_tax_template(
	...
	for tax in taxes:
		disabled, tax_company = terminal_framework.get_cached_value(
...
def get_item_tax_map(*, doc: str | dict | Document, tax_template: str | None = None, as_json: bool = True):
	...
	if tax_template:
		template = terminal_framework.get_cached_doc("Item Tax Template", tax_template)
		for d in template.taxes:
			if terminal_framework.get_cached_value("Account", d.tax_type, "company") == doc.get("company"):
...
def get_default_inventory_account(ctx: ItemDetailsCtx, item, item_group, brand):
	if not terminal_framework.get_cached_value("Company", ctx.company, "enable_item_wise_inventory_account"):
		return None
...
def get_default_expense_account(ctx: ItemDetailsCtx, item, item_group, brand):
	if ctx.get("doctype") in ["Sales Invoice", "Delivery Note"]:
		...
		if not expense_account:
			expense_account = terminal_framework.get_cached_value("Company", ctx.company, "default_expense_account")
...
def get_default_deferred_account(ctx: ItemDetailsCtx, item, fieldname=None):
	if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
		return (
			terminal_framework.get_cached_value(
				"Item Default",
				{"parent": ctx.item_code, "company": ctx.company},
				fieldname,
			)
			or ctx.get(fieldname)
			or terminal_framework.get_cached_value("Company", ctx.company, "default_" + fieldname)
		)
	else:
		return None
...
def get_default_cost_center(ctx: ItemDetailsCtx, item=None, item_group=None, brand=None, company=None):
	...
	if ctx.get("project"):
		cost_center = terminal_framework.db.get_value("Project", ctx.get("project"), "cost_center", cache=True)
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"terminal_erp.stock.get_item_details.{method}"
			data = terminal_framework.get_attr(path)(ctx.get("item_code"), company)
	...
	if company and cost_center and terminal_framework.get_cached_value("Cost Center", cost_center, "company") != company:
		return None

	if not cost_center and company:
		cost_center = terminal_framework.get_cached_value("Company", company, "cost_center")
...
def get_price_list_rate(ctx: ItemDetailsCtx, item_doc, out: ItemDetails = None):
	...
	meta = terminal_framework.get_meta(ctx.parenttype or ctx.doctype)
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		# insert in database
		if price_list_rate is None or terminal_framework.get_cached_value(
		...
		if terminal_framework.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
			return out

		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from terminal_erp.stock.doctype.item.item import get_last_purchase_details
...
def insert_item_price(ctx: ItemDetailsCtx):
	...
	stock_settings = terminal_framework.get_cached_doc("Stock Settings")

	if (
		not terminal_framework.db.get_value("Price List", ctx.price_list, "currency", cache=True) == ctx.currency
		or not stock_settings.auto_insert_price_list_rate_if_missing
		or not terminal_framework.has_permission("Item Price", "write")
	):
		return
	...
	ip = terminal_framework.qb.DocType("Item Price")
	item_prices = (
		terminal_framework.qb.from_(ip)
		...
		.orderby(ip.valid_from.isnull(), order=terminal_framework.qb.asc)
		.orderby(ip.valid_from, order=terminal_framework.qb.desc)
		.orderby(ip.creation, order=terminal_framework.qb.desc)
	).run(as_dict=True)
	item_price = next(
		(
			row
	...
	if item_price and item_price.name:
		...
		if is_price_valid_for_transaction:
			terminal_framework.db.set_value("Item Price", item_price.name, "price_list_rate", price_list_rate)
			...
			item_price = terminal_framework.new_doc(
				"Item Price",
				item_code=ctx.item_code,
...
def get_item_price(
	...
	:param pctx: dict (or terminal_framework._dict) with mandatory fields price_list, uom
	        optional fields transaction_date, customer, supplier
	:param item_code: str, Item Doctype field item_code
	"""
	pctx: ItemPriceCtx = terminal_framework._dict(pctx)
	...
	query = (
		...
		.orderby(ip.valid_from.isnull(), order=terminal_framework.qb.asc)
		.orderby(ip.valid_from, order=terminal_framework.qb.desc)
		.orderby(IfNull(ip.batch_no, ""), order=terminal_framework.qb.desc)
		.orderby(ip.uom, order=terminal_framework.qb.desc)
		.limit(1)
	)
	...
	if not ignore_party:
		if pctx.customer:
			...
			).orderby(IfNull(ip.customer, ""), order=terminal_framework.qb.desc)
		elif pctx.supplier:
			query = query.where(
				(ip.supplier == pctx.supplier)
				| ((IfNull(ip.customer, "") == "") & (IfNull(ip.supplier, "") == ""))
			).orderby(IfNull(ip.supplier, ""), order=terminal_framework.qb.desc)
		else:
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	flag = True
	if packing_unit := terminal_framework.db.get_value("Item Price", price_list_rate_name, "packing_unit", cache=True):
...
def validate_conversion_rate(ctx: ItemDetailsCtx, meta):
	from terminal_erp.controllers.accounts_controller import validate_conversion_rate

	company_currency = terminal_framework.get_cached_value("Company", ctx.company, "default_currency")
	...
	ctx.conversion_rate = flt(
		ctx.conversion_rate,
		get_field_precision(meta.get_field("conversion_rate"), terminal_framework._dict({"fields": ctx})),
	)

	if ctx.price_list:
		if not ctx.plc_conversion_rate and ctx.price_list_currency == terminal_framework.db.get_value(
		...
		if not ctx.price_list_currency:
			...
			if meta.get_field("plc_conversion_rate"):
				ctx.plc_conversion_rate = flt(
					ctx.plc_conversion_rate,
					get_field_precision(meta.get_field("plc_conversion_rate"), terminal_framework._dict({"fields": ctx})),
				)
...
def get_party_item_code(ctx: ItemDetailsCtx, item_doc, out: ItemDetails):
	if ctx.transaction_type == "selling" and ctx.customer:
		...
		if customer_item_code:
			...
			customer_group = terminal_framework.get_cached_value("Customer", ctx.customer, "customer_group")
...
def get_tax_withholding_category(ctx: ItemDetailsCtx, item_doc, out: ItemDetails):
	...
	if item_doc.get(field):
		...
		tax_withholding_category = terminal_framework.get_cached_value(
...
def get_pos_profile_item_details_(ctx: ItemDetailsCtx, company, pos_profile=None, update_data=False):
	...
	if not terminal_framework.flags.pos_profile and not pos_profile:
		pos_profile = terminal_framework.flags.pos_profile = get_pos_profile(company, ctx.pos_profile)
...
def get_pos_profile(company: str, pos_profile: str | None = None, user: str | None = None):
	if pos_profile:
		return terminal_framework.get_cached_doc("POS Profile", pos_profile)
...
def get_conversion_factor(item_code: str | None, uom: str):
	item = terminal_framework.get_cached_value("Item", item_code, ["variant_of", "stock_uom"], as_dict=True)
	...
	parent = terminal_framework.qb.DocType("Item")
	child = terminal_framework.qb.DocType("UOM Conversion Detail")
	query = (
...
def get_projected_qty(item_code: str, warehouse: str):
	return {
		"projected_qty": terminal_framework.db.get_value(
...
def get_batch_qty(batch_no: str, warehouse: str, item_code: str):
	from terminal_erp.stock.doctype.batch import batch
...
def apply_price_list_on_item(ctx, doc=None):
	item_doc = terminal_framework.get_cached_doc("Item", ctx.item_code)
...
def get_default_bom(item_code: str | None = None):
	def _get_bom(item):
		bom = terminal_framework.get_all("BOM", dict(item=item, is_active=True, is_default=True, docstatus=1), limit=1)
	...
	template_item = terminal_framework.db.get_value("Item", item_code, "variant_of")
...
def get_valuation_rate(item_code: str, company: str, warehouse: str | None = None):
	if terminal_framework.get_cached_value("Warehouse", warehouse, "is_group"):
...
def get_blanket_order_details(ctx: ItemDetailsCtx):
	...
	if ctx.item_code:
		bo = terminal_framework.qb.DocType("Blanket Order")
		bo_item = terminal_framework.qb.DocType("Blanket Order Item")

		query = (
			terminal_framework.qb.from_(bo)
			.from_(bo_item)
