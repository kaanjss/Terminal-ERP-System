...
from terminal_framework.query_builder.functions import Abs, IfNull, Sum
from terminal_framework.utils import cstr, flt, get_link_to_form, get_time, getdate, nowdate, nowtime
...
from terminal_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_available_serial_nos
...
from terminal_erp.stock.valuation import FIFOValuation, LIFOValuation

BarcodeScanResult = dict[str, str | None]


class InvalidWarehouseCompany(terminal_framework.ValidationError):
	pass


class PendingRepostingError(terminal_framework.ValidationError):
	pass
...
def get_stock_value_from_bin(warehouse=None, item_code=None):
	...
	if warehouse:
		w1 = terminal_framework.qb.DocType("Warehouse").as_("w1")
		w2 = terminal_framework.qb.DocType("Warehouse").as_("w2")
		descendants = (
			terminal_framework.qb.from_(w1)
			.join(w2)
...
def get_stock_value_on(
	...
	if warehouses:
		...
		for wh in list(warehouses):
			if terminal_framework.db.get_value("Warehouse", wh, "is_group"):
...
def get_stock_balance(
	...
	terminal_framework.has_permission("Item", "read", throw=True)
			...
			if field not in inventory_dimensions_fieldname:
				terminal_framework.throw(
					_("{0} is not a valid {1} fieldname.").format(
						terminal_framework.bold(field), terminal_framework.bold("Inventory Dimension")
					)
				)
			args[field] = value
...
def get_latest_stock_qty(item_code: str, warehouse: str | None = None):
	bin_dt = terminal_framework.qb.DocType("Bin")
	query = terminal_framework.qb.from_(bin_dt).select(Sum(bin_dt.actual_qty)).where(bin_dt.item_code == item_code)

	if warehouse:
		lft, rgt, is_group = terminal_framework.db.get_value("Warehouse", warehouse, ["lft", "rgt", "is_group"])

		if is_group:
			wh = terminal_framework.qb.DocType("Warehouse")
			query = query.where(
				bin_dt.warehouse.isin(
					terminal_framework.qb.from_(wh).select(wh.name).where((wh.lft >= lft) & (wh.rgt <= rgt))
				)
			)
		else:
...
def get_latest_stock_balance():
	bin_map = {}
	for d in terminal_framework.get_all("Bin", fields=["item_code", "warehouse", "stock_value"]):
...
def get_bin(item_code, warehouse):
	bin = terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
	if not bin:
		bin_obj = _create_bin(item_code, warehouse)
	else:
		bin_obj = terminal_framework.get_doc("Bin", bin)
	bin_obj.flags.ignore_permissions = True
	return bin_obj


def get_or_make_bin(item_code: str, warehouse: str) -> str:
	bin_record = terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def _create_bin(item_code, warehouse):
	...
	try:
		terminal_framework.db.savepoint(bin_creation_savepoint)
		bin_obj = terminal_framework.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		bin_obj.flags.ignore_permissions = 1
		bin_obj.insert()
	except terminal_framework.UniqueValidationError:
		terminal_framework.db.rollback(save_point=bin_creation_savepoint)  # preserve transaction in postgres
		bin_obj = terminal_framework.get_last_doc("Bin", {"item_code": item_code, "warehouse": warehouse})
...
def get_incoming_rate(args: dict | str, raise_error_if_no_rate: bool = True, fallbacks: bool = True):
	...
	use_moving_avg_for_batch = terminal_framework.get_single_value("Stock Settings", "do_not_use_batchwise_valuation")
	...
	if item_details and item_details.has_serial_no and args.get("serial_and_batch_bundle"):
		...
		args.actual_qty = args.qty
		args.batch_nos = terminal_framework._dict({args.batch_no: args})
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=terminal_erp.get_company_currency(args.get("company")),
...
def get_valuation_method(item_code, company=None):
	...
	val_method = terminal_framework.get_cached_value("Item", item_code, "valuation_method")
	if not val_method:
		val_method = (
			terminal_framework.get_cached_value("Company", company, "valuation_method")
			if company
			else terminal_framework.get_single_value("Stock Settings", "valuation_method") or "FIFO"
		)
	return val_method
	...
	for val in serial_nos:
		if val:
			...
			if val in valid_serial_nos:
				terminal_framework.throw(_("Serial number {0} entered more than once").format(val))
			else:
				valid_serial_nos.append(val)

	if qty and len(valid_serial_nos) != abs(qty):
		terminal_framework.throw(_("{0} valid serial nos for Item {1}").format(abs(qty), item_code))

	return valid_serial_nos


def validate_warehouse_company(warehouse, company):
	warehouse_company = terminal_framework.db.get_value("Warehouse", warehouse, "company", cache=True)
...
def is_group_warehouse(warehouse):
	if terminal_framework.db.get_value("Warehouse", warehouse, "is_group", cache=True):
		terminal_framework.throw(_("Group node warehouse is not allowed to select for transactions"))


def validate_disabled_warehouse(warehouse):
	if terminal_framework.db.get_value("Warehouse", warehouse, "disabled", cache=True):
		terminal_framework.throw(
		...
		if d.get("convertible"):
			...
			columns.insert(
				idx + 1,
				{
					...
					"fieldname": "{}_{}".format(d.get("fieldname"), terminal_framework.scrub(include_uom)),
	...
	for row_idx, row in enumerate(result):
		...
		for key, value in data:
			...
			if not is_dict_obj:
				...
				new_key = f"{key}_{terminal_framework.scrub(include_uom)}"
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
	...
	for col_idx in list(reversed(range(0, len(columns)))):
		...
		if isinstance(col, dict) and col.get("convertible") in ["rate", "qty"]:
			...
			convertible_column_map[col.get("fieldname")] = terminal_framework._dict(
...
def is_reposting_item_valuation_in_progress():
	reposting_in_progress = terminal_framework.db.exists(
...
def check_pending_reposting(posting_date: str, company: str | None = None, throw_error: bool = True) -> bool:
	...
	reposting_pending = terminal_framework.db.exists("Repost Item Valuation", filters)
	if reposting_pending and throw_error:
		...
		terminal_framework.msgprint(
			...
			primary_action={
				...
				"client_action": "terminal_erp.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str, ctx: dict | str | None = None) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		terminal_framework.cache().set_value(f"terminal_erp:barcode_scan:{search_value}", data, expires_in_sec=120)
		_update_item_info(data, ctx)

	def get_cache() -> BarcodeScanResult | None:
		data = terminal_framework.cache().get_value(f"terminal_erp:barcode_scan:{search_value}")
		if not data:
			return
	...
	if ctx is None:
		ctx = terminal_framework._dict()

	if scan_data := get_cache():
		return scan_data

	# search barcode no
	barcode_data = terminal_framework.db.get_value(
		"Item Barcode",
	...
	# search serial no
	serial_no_data = terminal_framework.db.get_value(
		"Serial No",
		search_value,
	...
	# search batch no
	batch_no_data = terminal_framework.db.get_value(
		"Batch",
		search_value,
	...
	if batch_no_data:
		if terminal_framework.get_cached_value("Item", batch_no_data.item_code, "has_serial_no"):
			terminal_framework.throw(
				_(
	...
	warehouse = terminal_framework.get_cached_value("Warehouse", search_value, ("name", "disabled"), as_dict=True)
...
def _update_item_info(scan_result: dict[str, str | None], ctx: dict | None = None) -> dict[str, str | None]:
	from terminal_erp.stock.get_item_details import get_item_warehouse_

	item_code = scan_result.get("item_code")
	if not item_code:
		return scan_result

	if item_info := terminal_framework.get_cached_value(
		"Item",
		item_code,
	...
	if ctx and (
		warehouse := get_item_warehouse_(ctx, terminal_framework._dict(name=item_code), overwrite_warehouse=True)
	):
...
def get_default_stock_uom() -> str | None:
	if default_uom := terminal_framework.get_cached_value("Stock Settings", None, "stock_uom"):
		return default_uom
	...
	acceptable_default_uoms = dict.fromkeys(
		(
			...
			_("Nos", terminal_framework.get_system_settings("language")),
			# or the current user's language
			_("Nos"),
		)
	)

	available_default_uoms = terminal_framework.db.get_values(
