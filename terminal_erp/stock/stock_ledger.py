...
from terminal_framework import _, bold, scrub
from terminal_framework.model.meta import get_field_precision
from terminal_framework.query_builder import Order
from terminal_framework.query_builder.functions import Lower, NullIf, Sum
...
import terminal_erp
from terminal_erp.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from terminal_erp.stock.utils import (
...
from terminal_erp.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero


class NegativeStockError(terminal_framework.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(terminal_framework.ValidationError):
	pass
	...
	for sle in sl_entries:
		...
		if key not in checked:
			...
			if get_valuation_method(item_code, company) == "Standard Cost":
				latest_isc = terminal_framework.db.get_value(
					"Item Standard Cost",
		...
		if latest_isc and getdate(posting_date) < getdate(latest_isc.effective_date):
			effective_date = terminal_framework.bold(terminal_framework.format(latest_isc.effective_date, "Date"))
			terminal_framework.throw(
				_(
					...
					terminal_framework.bold(terminal_framework.format(posting_date, "Date")),
					effective_date,
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = terminal_framework.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
				bin_name = get_or_make_bin(args.get("item_code"), args.get("warehouse"))
				args.reserved_stock = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_stock"))
...
def validate_cancellation(kargs):
	if kargs[0].get("is_cancelled"):
		repost_entry = terminal_framework.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = terminal_framework.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def set_as_cancel(voucher_type, voucher_no):
	...
	(
		terminal_framework.qb.update(sle)
		.set(sle.is_cancelled, 1)
		.set(sle.modified, now())
		.set(sle.modified_by, terminal_framework.session.user)
...
def make_entry(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	sle = terminal_framework.get_doc(args)
...
def update_args_in_repost_item_valuation(
	...
	if doc.reposting_data_file:
		...
		# terminal_framework.delete_doc("File", file_name, ignore_permissions=True, delete_permanently=True)
	...
	doc.db_set(
		{
			"current_index": index,
			"items_to_be_repost": terminal_framework.as_json(items_to_be_repost),
...
def create_json_gz_file(data, doc, file_name=None) -> str:
	encoded_content = terminal_framework.safe_encode(terminal_framework.as_json(data))
	compressed_content = gzip.compress(encoded_content)

	if not file_name:
		return create_file(doc, compressed_content)
	else:
		file_doc = terminal_framework.get_doc("File", file_name)
		if "/terminal_framework_s3_attachment." in file_doc.file_url:
			file_doc.delete()
			return create_file(doc, compressed_content)
...
def create_file(doc, compressed_content):
	...
	_file = terminal_framework.get_doc(
		{
			"doctype": "File",
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if args.get(field) in [None, ""]:
			terminal_framework.throw(_("The field {0} is required for reposting").format(terminal_framework.unscrub(field)))
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None, reposting_data=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = terminal_framework.db.get_all(
			"Stock Ledger Entry",
...
def get_item_wh_wise_last_posted_sle_from_reposting_data(doc, reposting_data=None):
	...
	if reposting_data and reposting_data.item_wh_wise_last_posted_sle:
		return terminal_framework._dict(reposting_data.item_wh_wise_last_posted_sle)

	return terminal_framework._dict()


def get_reposting_data(file_path) -> dict:
	file_name = terminal_framework.db.get_value(
		"File",
		{
			"file_url": file_path,
			"attached_to_field": "reposting_data_file",
	...
	attached_file = terminal_framework.get_doc("File", file_name)
...
class update_entries_after:
	...
	def __init__(
		...
		self.args = terminal_framework._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.prev_sle_dict = terminal_framework._dict({})
		self.company = terminal_framework.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
		...
		self.data = terminal_framework._dict()
	...
	def set_precision(self):
		self.flt_precision = cint(terminal_framework.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			terminal_framework.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, terminal_framework._dict())
		...
		if previous_sle:
			...
			self.prev_sle_dict[(args.get("item_code"), args.get("warehouse"))] = terminal_framework._dict(
				{
	...
	def get_item_wh_wise_last_posted_sle(self):
		if self.args and self.args.get("item_wh_wise_last_posted_sle"):
			...
			for key, sle in self.args.get("item_wh_wise_last_posted_sle").items():
				_sles[terminal_framework.safe_eval(key)] = terminal_framework._dict(sle)

			return _sles

		return {
			(self.args.item_code, self.args.warehouse): terminal_framework._dict(
				{
	...
	def repost_stock_ledger_entry(self, sle):
		if isinstance(sle, dict):
			sle = terminal_framework._dict(sle)
	...
	def update_item_wh_wise_last_posted_sle(self, sle):
		if not self._sles:
			self.item_wh_wise_last_posted_sle = terminal_framework._dict()
			return

		self.item_wh_wise_last_posted_sle[(sle.item_code, sle.warehouse)] = terminal_framework._dict(
			{
	...
	def update_data_in_repost(self, total_sles=None, index=None):
		...
		if not terminal_framework.in_test:
			...
			terminal_framework.db.commit()  # nosemgrep
	...
	def has_stock_reco_with_serial_batch(self, sle):
		if (
			...
			and terminal_framework.db.get_value(sle.voucher_type, sle.voucher_no, "set_posting_time") == 1
		):
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or terminal_framework._dict(
	...
	def process_standard_cost(self, sle):
		from terminal_erp.stock.doctype.item_standard_cost.item_standard_cost import get_item_standard_rate
	...
	def process_sle(self, sle):
		...
		if not self.prev_sle_dict.get(key):
			self.prev_sle_dict[key] = terminal_framework._dict(
				{
					"qty_after_transaction": 0.0,
					"valuation_rate": 0.0,
					"stock_value": 0.0,
		...
		if self.valuation_method == "Standard Cost":
			...
			sle.batch_no
			and terminal_framework.db.get_value("Batch", sle.batch_no, "use_batchwise_valuation", cache=True)
			...
			if (
				...
				if (
					...
					source_rate = terminal_framework.get_cached_value(
		...
		sle.modified = now()
		terminal_framework.get_doc(sle).db_update()
		...
		if not cint(terminal_erp.is_perpetual_inventory_enabled(sle.company)):
			return
	...
	def get_serialized_values(self, sle):
		from terminal_erp.stock.serial_batch_bundle import SerialNoValuation
	...
	def calculate_valuation_for_serial_batch_bundle(self, sle):
		if not terminal_framework.db.exists("Serial and Batch Bundle", sle.serial_and_batch_bundle):
			return

		if sle.actual_qty < 0 and (
			sle.voucher_type in ["Stock Reconciliation", "Asset Capitalization"]
			or not terminal_framework.db.get_value(sle.voucher_type, sle.voucher_no, "is_return")
		):
			doc = terminal_framework._dict({})
			self.update_serial_batch_no_valuation(sle, doc, prev_sle=self.wh_data)
		else:
			doc = terminal_framework.get_doc("Serial and Batch Bundle", sle.serial_and_batch_bundle)
			doc.set_incoming_rate(
				save=True, allow_negative_stock=self.allow_negative_stock, prev_sle=self.wh_data
			)
			doc.calculate_qty_and_amount(save=True)

		if stock_queue := terminal_framework.get_all(
	...
	def update_serial_batch_no_valuation(self, sle, doc, prev_sle=None):
		from terminal_erp.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
	...
	def has_landed_cost_based_on_pi(self, sle):
		if sle.voucher_type == "Purchase Receipt" and terminal_framework.db.get_single_value(
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = terminal_framework.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if terminal_framework.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from terminal_erp.controllers.sales_and_purchase_return import (
				...
				# check in item table
				item_code, incoming_rate = terminal_framework.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		terminal_framework.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def is_manufacture_entry_with_sabb(self, sle):
		if (
			...
			purpose = terminal_framework.get_cached_value("Stock Entry", sle.voucher_no, "purpose")
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no, voucher_detail_no):
		stock_entry = terminal_framework.get_lazy_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = terminal_framework.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			terminal_framework.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if terminal_framework.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			if sle.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
				details = terminal_framework.get_cached_value(
					sle.voucher_type,
					sle.voucher_no,
		...
		if terminal_framework.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = terminal_framework.get_lazy_doc(sle.voucher_type, sle.voucher_no)
			doc.update_valuation_rate(reset_outgoing_rate=False)
			for d in doc.items + doc.supplied_items:
				d.db_update()

	def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
		if terminal_framework.db.exists("Subcontracting Receipt Item", sle.voucher_detail_no):
			terminal_framework.db.set_value("Subcontracting Receipt Item", sle.voucher_detail_no, "rate", outgoing_rate)
		else:
			terminal_framework.db.set_value(
		...
		scr = terminal_framework.get_lazy_doc("Subcontracting Receipt", sle.voucher_no, for_update=True)
	...
	def update_rate_on_stock_reconciliation(self, sle):
		...
		terminal_framework.get_lazy_doc(
	...
	def get_incoming_value_for_serial_nos(sle, serial_nos):
		...
		all_serial_nos = terminal_framework.get_all(
		...
		sle_entry = terminal_framework.qb.DocType("Stock Ledger Entry")
		for serial_no in invalid_serial_nos:
			incoming_rate = (
				terminal_framework.qb.from_(sle_entry)
				...
				.orderby(sle_entry.posting_date, order=terminal_framework.qb.desc)
				.orderby(sle_entry.creation, order=terminal_framework.qb.desc)
				.orderby(sle_entry.name, order=terminal_framework.qb.desc)
				.limit(1)
			).run()
	...
	def is_return_purchase_entry(self, sle):
		if sle.voucher_type in ["Purchase Invoice", "Purchase Receipt"]:
			return terminal_framework.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return")

		return False

	def update_batched_values(self, sle):
		from terminal_erp.stock.serial_batch_bundle import BatchNoValuation
		...
		if actual_qty > 0:
			...
			new_sle.qty = new_sle.actual_qty
			new_sle.batch_nos = terminal_framework._dict({new_sle.batch_no: new_sle})
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return terminal_framework.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=terminal_erp.get_company_currency(sle.company),
			company=sle.company,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else terminal_framework._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			) in terminal_framework.local.flags.currently_saving:
				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					terminal_framework.bold(abs(deficiency)),
					terminal_framework.get_desk_link("Item", exceptions[0]["item_code"], show_title_with_name=True),
					terminal_framework.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					terminal_framework.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
				...
				if self.reserved_stock:
					...
					if allowed_qty > 0:
						msg = "{} As {} units are reserved for other sales orders, you are allowed to consume only {} units.".format(
							msg, terminal_framework.bold(self.reserved_stock), terminal_framework.bold(allowed_qty)
						)
					else:
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				terminal_framework.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin_data(self, sle):
		...
		terminal_framework.db.set_value("Bin", bin_name, values_to_update)
	...
	def update_bin(self):
		...
		for (item_code, warehouse), data in self.prev_sle_dict.items():
			...
			terminal_framework.db.set_value("Bin", bin_name, updated_values, update_modified=True)
...
def get_sle_against_current_voucher(kwargs):
	...
	doctype = terminal_framework.qb.DocType("Stock Ledger Entry")
...
def get_previous_sle_of_current_voucher(args, operator="<", exclude_current_voucher=False):
	...
	sle = terminal_framework.db.sql(  # nosemgrep
		f"""
	...
	return sle[0] if sle else terminal_framework._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(terminal_framework.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
		...
		"""
			...
			terminal_framework.db.escape(serial_no),
			terminal_framework.db.escape(f"{serial_no}\n%"),
			terminal_framework.db.escape(f"%\n{serial_no}"),
			terminal_framework.db.escape(f"%\n{serial_no}\n%"),
		)
...
def get_valuation_rate(
	...
	if warehouse and batch_no and terminal_framework.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
		table = terminal_framework.qb.DocType("Stock Ledger Entry")
		query = (
			...
			sle=terminal_framework._dict(
				{
					...
					"posting_datetime": terminal_framework.get_value(
	...
	if last_valuation_rate := (
		...
		.orderby(sle_entry.posting_datetime, order=terminal_framework.qb.desc)
		.orderby(sle_entry.creation, order=terminal_framework.qb.desc)
		.limit(1)
	).run():
		return flt(last_valuation_rate[0][0])
	...
	if fallbacks:
		...
		if rate := (
			terminal_framework.db.get_value("Item", item_code, "valuation_rate")
			or terminal_framework.db.get_value("Item", item_code, "standard_rate")
			or terminal_framework.db.get_value(
	...
	if (
		...
		and cint(terminal_erp.is_perpetual_inventory_enabled(company))
	):
		...
		solutions += (
			...
			+ " {} ".format(terminal_framework.bold(_("after")))
		...
		terminal_framework.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def update_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if get_valuation_method(args.get("item_code"), args.get("company")) == "Standard Cost":
		...
		terminal_framework.db.sql(  # nosemgrep
			f"""
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if (
		...
		and terminal_framework.db.get_value("Stock Reconciliation Item", args.voucher_detail_no, "qty") > 0
	):
		return
	...
	if is_negative_with_precision(neg_sle):
		message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
			...
			terminal_framework.get_desk_link("Item", args.item_code, show_title_with_name=True),
			terminal_framework.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			terminal_framework.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if args.batch_no:
		...
		if is_negative_with_precision(neg_batch_sle, is_batch=True):
			message = _(
				...
				terminal_framework.get_desk_link("Batch", args.batch_no),
				terminal_framework.get_desk_link("Warehouse", args.warehouse),
				neg_batch_sle[0]["posting_date"],
				neg_batch_sle[0]["posting_time"],
				terminal_framework.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
			)
			terminal_framework.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
...
def is_negative_with_precision(neg_sle, is_batch=False):
	...
	precision = cint(terminal_framework.db.get_default("float_precision")) or 2
...
def validate_reserved_stock(kwargs):
	...
	if diff < 0 and abs(diff) > 0.0001:
		msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
			abs(diff),
			terminal_framework.get_desk_link("Item", kwargs.item_code, show_title_with_name=True),
			terminal_framework.get_desk_link("Warehouse", kwargs.warehouse),
			nowdate(),
			nowtime(),
		)
		terminal_framework.throw(msg, title=_("Reserved Stock"))
	...
	if reserved_serial_nos_details := get_sre_reserved_serial_nos_details(item_code, warehouse, serial_nos):
		if common_serial_nos := list(set(serial_nos).intersection(set(reserved_serial_nos_details.keys()))):
			...
			msg += _("Example: Serial No {0} reserved in {1}.").format(
				terminal_framework.bold(common_serial_nos[0]),
				terminal_framework.get_desk_link(
					"Stock Reservation Entry", reserved_serial_nos_details[common_serial_nos[0]]
				),
			)
			terminal_framework.throw(msg, title=_("Reserved Serial No."))
	...
	if reserved_batches_map := get_sre_reserved_batch_nos_details(item_code, warehouse, batch_nos):
		...
		for batch_no in batch_nos:
			...
			if diff < 0 and abs(diff) > 0.0001:
				msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
					abs(diff),
					terminal_framework.get_desk_link("Batch", batch_no),
					terminal_framework.get_desk_link("Warehouse", warehouse),
					nowdate(),
					nowtime(),
				)
				terminal_framework.throw(msg, title=_("Reserved Stock for Batch"))


def is_negative_stock_allowed(*, item_code: str | None = None) -> bool:
	if terminal_framework.get_cached_doc("Stock Settings").allow_negative_stock:
		return True
	if item_code and cint(terminal_framework.get_cached_value("Item", item_code, "allow_negative_stock")):
		return True
	return False
...
def get_incoming_rate_for_inter_company_transfer(sle) -> float:
	...
	reference_name = terminal_framework.get_cached_value(sle.voucher_type + " Item", sle.voucher_detail_no, field)

	if reference_name:
		rate = terminal_framework.get_cached_value(
			doctype,
			reference_name,
			"incoming_rate",
		)
	...
	lcv_amount = terminal_framework.db.get_value(
...
def is_transfer_stock_entry(voucher_no):
	purpose = terminal_framework.get_cached_value("Stock Entry", voucher_no, "purpose")
...
def is_repack_entry(stock_entry_id):
	return terminal_framework.get_cached_value("Stock Entry", stock_entry_id, "purpose") == "Repack"
...
def has_correct_data(sle):
	...
	if previous_qty and not terminal_framework.db.get_value(
		"Stock Ledger Entry",
