...
from terminal_framework.model.mapper import map_child_doc
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		terminal_framework.throw(_("Add items in the Item Locations table"))
...
def create_delivery(source_name: str, target_doc: str | Document | None = None, target: str | None = None):
	pick_list = terminal_framework.get_doc("Pick List", source_name)
	target = target or (terminal_framework.flags.args or {}).get("target") or "Delivery Note"
	...
	if len(documents) == 1:
		...
		from terminal_framework.utils import comma_and
...
def create_delivery_wo_so(pick_list, target, target_doc=None):
	if not target_doc:
		target_doc = terminal_framework.new_doc(target)
...
def create_dn_for_pick_lists(
	...
	if not sales_order_arg and not all(item.sales_order for item in pick_list.locations):
		if isinstance(delivery_note, str):
			delivery_note = terminal_framework.get_doc(terminal_framework.parse_json(delivery_note))
...
def map_pl_locations(pick_list, item_mapper, target_doc, sales_order=None):
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = terminal_framework.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	target_doc.company = pick_list.company
	if sales_order:
		target_doc.customer = terminal_framework.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_target(pick_list, target_doc, item_mapper, sales_order=None) -> None:
	...
	for so_row, value in product_bundles.items():
		sales_order_item = terminal_framework.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list: str | dict):
	pick_list = terminal_framework.get_doc(terminal_framework.parse_json(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return terminal_framework.msgprint(_("Stock Entry has already been created against this Pick List"))
...
def update_child_item(source, target, target_doc):
	cost_center = terminal_framework.db.get_value("Project", target_doc.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return terminal_framework.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = terminal_framework.get_doc("Work Order", pick_list.get("work_order"))
	...
	if work_order.bom_no:
		stock_entry.inspection_required = terminal_framework.db.get_value("BOM", work_order.bom_no, "inspection_required")

	is_wip_warehouse_group = terminal_framework.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = terminal_framework.get_value(
