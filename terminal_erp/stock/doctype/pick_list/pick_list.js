...
terminal_framework.ui.form.on("Pick List", {
	after_save(frm) {
		setTimeout(() => {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "terminal_erp.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
	...
	set_item_locations: (frm, save) => {
		if (!(frm.doc.locations && frm.doc.locations.length)) {
			terminal_framework.msgprint(__("Add items in the Item Locations table"));
		} else {
				...
				terminal_framework.prompt(
					...
					(data) => {
						...
						terminal_erp.utils.map_current_doc({
							method: "terminal_erp.manufacturing.doctype.work_order.mapper.create_pick_list",
							target: frm,
	...
	create_delivery(frm, doctype) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.pick_list.mapper.create_delivery",
			args: {
				target: doctype,
			},
			frm: frm,
	...
	create_stock_entry: (frm) => {
		terminal_framework
			.xcall("terminal_erp.stock.doctype.pick_list.mapper.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
				terminal_framework.model.sync(stock_entry);
				terminal_framework.set_route("Form", "Stock Entry", stock_entry.name);
			});
	},
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new terminal_erp.utils.BarcodeScanner(opts);
...
terminal_framework.ui.form.on("Pick List Item", {
	item_code: (frm, cdt, cdn) => {
		...
		if (row.item_code) {
			get_item_details(row.item_code, row.uom, row.warehouse, frm.doc.company).then((data) => {
				terminal_framework.model.set_value(cdt, cdn, "uom", data.stock_uom);
				terminal_framework.model.set_value(cdt, cdn, "stock_uom", data.stock_uom);
				terminal_framework.model.set_value(cdt, cdn, "conversion_factor", 1);
				terminal_framework.model.set_value(cdt, cdn, "actual_qty", data.actual_qty);
				terminal_framework.model.set_value(cdt, cdn, "company_total_stock", data.company_total_stock);
			});
		}
	},
	...
	uom: (frm, cdt, cdn) => {
		...
		if (row.uom) {
			get_item_details(row.item_code, row.uom).then((data) => {
				terminal_framework.model.set_value(cdt, cdn, "conversion_factor", data.conversion_factor);
			});
		}
	},
	...
	qty: (frm, cdt, cdn) => {
		let row = terminal_framework.get_doc(cdt, cdn);
		terminal_framework.model.set_value(cdt, cdn, "stock_qty", row.qty * row.conversion_factor);
	},
...
function get_item_details(item_code, uom = null, warehouse = null, company = null) {
	if (item_code) {
		return terminal_framework.xcall("terminal_erp.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
			warehouse,
			company,
