...
from terminal_framework.query_builder.functions import Coalesce, GroupConcat, Locate, Lower, Max, Replace, Sum
from terminal_framework.utils import cint, floor, flt, get_link_to_form
...
from terminal_erp.stock.get_item_details import get_company_total_stock, get_conversion_factor
...
class MissingWarehouseValidationError(terminal_framework.ValidationError):
	pass


class IncorrectWarehouseValidationError(terminal_framework.ValidationError):
	pass
...
class PickList(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.pick_list_item.pick_list_item import PickListItem
	...
	def onload(self) -> None:
		if terminal_framework.get_cached_value("Stock Settings", None, "enable_stock_reservation"):
	...
	def validate_warehouses(self):
		for location in self.locations:
			...
			company = terminal_framework.get_cached_value("Warehouse", location.warehouse, "company")
			...
			if company != self.company:
				terminal_framework.throw(
					_(
						...
						location.idx,
						terminal_framework.bold(location.warehouse),
	...
	def check_serial_no_status(self):
		...
		for row in self.get("locations"):
			...
			validated_serial_nos = terminal_framework.get_all(
				"Serial No",
	...
	def validate_sales_order_percentage(self):
		...
		for location in self.get("locations"):
			if (
				...
				and terminal_framework.db.get_value("Sales Order", location.sales_order, "per_picked", cache=True) == 100
			):
				terminal_framework.throw(
	...
	def validate_sales_order(self):
		...
		if so_list:
			for so in so_list:
				so_doc = terminal_framework.get_doc("Sales Order", so)
				for item in so_doc.items:
					if item.stock_reserved_qty > 0:
						terminal_framework.throw(
							...
							).format(terminal_framework.bold(so))
						)
	...
	def validate_expired_batches(self):
		...
		if batches:
			...
			query = (
				terminal_framework.qb.from_(batch)
				.select(batch.name)
				.where(
					(batch.name.isin(batches))
					& (batch.expiry_date <= terminal_framework.utils.nowdate())
	...
	def make_bundle_using_old_serial_batch_fields(self):
		...
		for row in self.locations:
			...
			if not row.use_serial_batch_fields and (row.serial_no or row.batch_no):
				terminal_framework.throw(_("Please enable Use Old Serial / Batch Fields to make_bundle"))
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": terminal_framework._dict({row.batch_no: row.stock_qty}) if row.batch_no else None,
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.locations:
			if (
				...
				and terminal_framework.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "docstatus")
				== 1
			):
	...
	def remove_serial_and_batch_bundle(self):
		for row in self.locations:
			if row.serial_and_batch_bundle:
				terminal_framework.delete_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def update_packed_items_qty(self, packed_items):
		...
		if doc_updates:
			terminal_framework.db.bulk_update("Packed Item", doc_updates, update_modified=False)
	...
	def update_sales_order_item_qty(self, so_items):
		...
		if doc_updates:
			terminal_framework.db.bulk_update("Sales Order Item", doc_updates, update_modified=False)
	...
	def update_sales_order_picking_status(self) -> None:
		...
		for sales_order in sales_orders:
			terminal_framework.get_doc("Sales Order", sales_order, for_update=True).update_picking_status()
	...
	def validate_picked_qty(self, data):
		over_delivery_receipt_allowance = 100 + flt(
			terminal_framework.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
		)
	...
	def set_item_locations(self, save: bool = False):
		...
		self.item_location_map = terminal_framework._dict()
		...
		if self.parent_warehouse:
			...
			root_warehouse = terminal_framework.db.get_value(
		...
		item_warehouse_dict = terminal_framework._dict()
		if self.work_order:
			item_warehouse_list = terminal_framework.get_all(
				"Work Order Item",
		...
		updated_locations = terminal_framework._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			...
			if not item.item_code:
				terminal_framework.throw(_("Row #{0}: Item Code is Mandatory").format(item.idx))
			if not cint(
				terminal_framework.get_cached_value("Item", item.item_code, "is_stock_item")
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			terminal_framework.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, value in product_bundles.items():
			...
			already_picked = terminal_framework.db.get_value(item_table, so_row, "picked_qty", for_update=True)
			terminal_framework.db.set_value(
				item_table,
				so_row,
				"picked_qty",
	...
	def get_picked_items_details(self, items):
		picked_items = terminal_framework._dict()
		...
		for item_data in items_data:
			...
			if key not in picked_items[item_data.item_code]:
				picked_items[item_data.item_code][key] = terminal_framework._dict(
					{
						"picked_qty": 0,
						"serial_no": [],
		...
		for row in self.locations:
			if flt(row.picked_qty) > 0:
				...
				if key not in picked_items[row.item_code]:
					picked_items[row.item_code][key] = terminal_framework._dict(
						{
							"picked_qty": 0,
							"serial_no": [],
	...
	def _get_pick_list_items(self, items):
		pi = terminal_framework.qb.DocType("Pick List")
		pi_item = terminal_framework.qb.DocType("Pick List Item")
		query = (
			terminal_framework.qb.from_(pi)
			.inner_join(pi_item)
	...
	def _get_product_bundles(self) -> dict[str, str]:
		...
		for item in self.locations:
			...
			product_bundles[item.sales_order_item] = terminal_framework._dict(
				{
	...
	def _get_product_bundle_qty_map(self, bundles) -> dict[str, dict[str, float]]:
		...
		for data in bundles:
			bundle = terminal_framework.get_doc("Product Bundle", get_active_product_bundle(data.item_code))
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = terminal_framework.get_precision("Stock Ledger Entry", "qty_after_transaction")
		possible_bundles = {}
		for item in self.locations:
...
def update_pick_list_status(pick_list):
	if pick_list:
		doc = terminal_framework.get_doc("Pick List", pick_list)
...
def get_picked_items_qty(items, contains_packed_items=False) -> list[dict]:
	...
	if terminal_framework.db.db_type == "postgres":
		terminal_framework.qb.from_(pi_item).select(pi_item.name).where(conditions).for_update().run()
	else:
		query = query.for_update()
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	precision = terminal_framework.get_precision("Pick List Item", "qty")
	while flt(remaining_stock_qty, precision) > 0 and available_locations:
		item_location = available_locations.pop(0)
		item_location = terminal_framework._dict(item_location)

		stock_qty = remaining_stock_qty if item_location.qty >= remaining_stock_qty else item_location.qty
		qty = stock_qty / (item_doc.conversion_factor or 1)

		uom_must_be_whole_number = terminal_framework.get_cached_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	has_serial_no = terminal_framework.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = terminal_framework.get_cached_value("Item", item_code, "has_batch_no")
...
def get_available_item_locations_for_serial_and_batched_item(
	...
	if locations:
		sn = terminal_framework.qb.DocType("Serial No")
		...
		for location in locations:
			...
			serial_nos = (
				terminal_framework.qb.from_(sn)
				.select(sn.name)
				.where(
...
def get_available_item_locations_for_serialized_item(
	...
	warehouse_serial_nos_map = terminal_framework._dict()
...
def get_available_item_locations_for_batched_item(
	...
	warehouse_wise_batches = terminal_framework._dict()
...
def get_item_details(
	...
	details = terminal_framework.db.get_value("Item", item_code, "stock_uom", as_dict=1)
...
def get_rejected_warehouses():
	if not hasattr(terminal_framework.local, "rejected_warehouses"):
		terminal_framework.local.rejected_warehouses = []

	if not terminal_framework.local.rejected_warehouses:
		terminal_framework.local.rejected_warehouses = terminal_framework.get_all(
			"Warehouse", filters={"is_rejected_warehouse": 1}, pluck="name"
		)

	return terminal_framework.local.rejected_warehouses


@terminal_framework.whitelist()
def get_pick_list_query(doctype: Any, txt: str, searchfield: Any, start: int, page_len: int, filters: dict):
	terminal_framework.has_permission("Pick List", throw=True)

	if not filters.get("company"):
		terminal_framework.throw(_("Please select a Company"))

	PICK_LIST = terminal_framework.qb.DocType("Pick List")
	PICK_LIST_ITEM = terminal_framework.qb.DocType("Pick List Item")
	SALES_ORDER = terminal_framework.qb.DocType("Sales Order")

	query = (
		terminal_framework.qb.from_(PICK_LIST)
		.join(PICK_LIST_ITEM)
	...
	if txt:
		meta = terminal_framework.get_meta("Pick List")
