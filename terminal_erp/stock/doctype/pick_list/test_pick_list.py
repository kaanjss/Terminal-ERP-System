...
from terminal_erp.selling.doctype.sales_order.mapper import create_pick_list
...
from terminal_erp.stock.doctype.packed_item.test_packed_item import create_product_bundle
from terminal_erp.stock.doctype.pick_list.mapper import (
	create_delivery,
...
class TestPickList(Terminal ERPTestSuite):
	def test_pick_list_picks_warehouse_for_each_item(self):
		...
		pick_list = terminal_framework.get_doc(
			{
				"doctype": "Pick List",
				"company": "_Test Company",
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = terminal_framework.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = terminal_framework.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = terminal_framework.get_doc("Item", {"item_name": "Batched Item"})
	...
	def test_pick_list_warehouse_for_batched_item(self):
		...
		batch_company = terminal_framework.get_doc(
			{
				"doctype": "Company",
		...
		batch_warehouse = terminal_framework.get_doc(
			{
				"doctype": "Warehouse",
		...
		batch_item = terminal_framework.db.exists("Item", "Batch Warehouse Item")
		if not batch_item:
			...
			batch_item = terminal_framework.get_doc("Item", "Batch Warehouse Item")
	...
	def test_pick_list_warehouse_validation(self):
		...
		temp_warehouse = terminal_framework.get_doc(
	...
	def test_pick_list_for_items_from_multiple_sales_orders(self):
		...
		sales_order = terminal_framework.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
				"items": [
					{
						"item_code": item_code,
						"qty": 10,
						"delivery_date": terminal_framework.utils.today(),
						"warehouse": "_Test Warehouse - _TC",
					}
				],
			}
	...
	def test_multiple_dn_creation(self):
		sales_order_1 = terminal_framework.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
		...
		sales_order_2 = terminal_framework.get_doc(
			{
				"doctype": "Sales Order",
		...
		for dn in terminal_framework.get_all(
			"Delivery Note",
			filters={"against_pick_list": pick_list.name, "customer": "_Test Customer"},
			fields=["name"],
		):
			for dn_item in terminal_framework.get_doc("Delivery Note", dn.name).get("items"):
		...
		pick_list_1 = terminal_framework.get_doc(
			{
				"doctype": "Pick List",
				"company": "_Test Company",
				"purpose": "Delivery",
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"use_serial_batch_fields": 1})
	def test_sales_invoice_from_pick_list_copies_old_batch_serial_fields(self):
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			f"_Test PLSI Old Fields {terminal_framework.generate_hash(length=8)}",
			properties={
				"is_stock_item": 1,
				"has_batch_no": 1,
				"create_new_batch": 1,
				"has_serial_no": 1,
				"batch_number_series": f"PLSI-OLD-B-{terminal_framework.generate_hash(length=6)}-.#####",
				"serial_no_series": f"PLSI-OLD-S-{terminal_framework.generate_hash(length=6)}-.#####",
			},
		).name
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"use_serial_batch_fields": 0})
	def test_sales_invoice_from_pick_list_copies_serial_and_batch_bundle(self):
		...
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			f"_Test PLSI Bundle {terminal_framework.generate_hash(length=8)}",
			properties={
				"is_stock_item": 1,
				"has_batch_no": 1,
				"create_new_batch": 1,
				"has_serial_no": 1,
				"batch_number_series": f"PLSI-BND-B-{terminal_framework.generate_hash(length=6)}-.#####",
				"serial_no_series": f"PLSI-BND-S-{terminal_framework.generate_hash(length=6)}-.#####",
			},
		).name
			...
			SerialBatchCreation(
				{
					...
					"qty": -2,
					"batches": terminal_framework._dict({batch_no: 2}),
					"serial_nos": serial_nos,
	...
	def test_picklist_reserved_qty_validation(self):
		...
		if not terminal_framework.db.exists("Item", test_stock_item):
			create_item(
	...
	def test_picklist_for_batch_item(self):
		...
		make_stock_entry(
			...
			qty=50,
			basic_rate=100,
			batches=terminal_framework._dict({"PICKLT-000001": 30, "PICKLT-000002": 20}),
		)
	...
	def test_pick_list_warehouse_for_work_order(self):
		...
		from terminal_erp.manufacturing.doctype.work_order.mapper import create_pick_list
	...
	def test_validate_picked_qty_with_manual_option(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pl.save)
	...
	def test_over_allowance_picking(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pl_doc.submit)

		terminal_framework.db.set_single_value("Stock Settings", "over_picking_allowance", 50)

		pl_doc.reload()
		pl_doc.submit()

		terminal_framework.db.set_single_value("Stock Settings", "over_picking_allowance", 0)

	@Terminal ERPTestSuite.change_settings("Selling Settings", {"allow_multiple_items": 1})
	def test_ignore_pricing_rule_in_pick_list(self):
		terminal_framework.flags.print_stmt = False
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			properties={
				"is_stock_item": 1,
		...
		pricing_rule = terminal_framework.get_doc(
			{
		...
		pricing_rule.save()
		terminal_framework.flags.print_stmt = True
	...
	def test_multiple_pick_lists_delivery_note(self):
		from terminal_erp.stock.doctype.pick_list.mapper import create_dn_for_pick_lists
	...
	def test_packed_item_multiple_times_in_so(self):
		terminal_framework.db.delete("Item Price")
		warehouse_1 = "_Test Warehouse - _TC"
		warehouse_2 = "_Test Warehouse 2 - _TC"
		warehouse = "_Test Warehouse - _TC"
		item_1 = make_item(properties={"is_stock_item": 0}).name
	...
	def test_creating_dn_from_so_with_different_addresses(self):
		...
		customer_shipping_address_1 = terminal_framework.get_doc(
			{
				"doctype": "Address",
		...
		customer_shipping_address_2 = terminal_framework.get_doc(
			{
				"doctype": "Address",
				"address_title": "Customer Shipping Address",
				"address_type": "Shipping",
		...
		pick_list = terminal_framework.new_doc("Pick List")
		map_docs(
			"terminal_erp.selling.doctype.sales_order.mapper.create_pick_list",
		...
		create_delivery_note(pick_list.name)
		delivery_notes = terminal_framework.get_all(
		...
		for delivery_note in delivery_notes:
			doc = terminal_framework.get_doc("Delivery Note", delivery_note)
			if len(doc.items) == 1:
	...
	def test_get_pick_list_query_postgres_valid(self):
		...
		from terminal_erp.stock.doctype.pick_list.pick_list import get_pick_list_query
