...
class StockEntryDetail(Document):
	...
	def validate_batch(self):
		...
		disabled = terminal_framework.db.get_value("Batch", self.batch_no, "disabled")
		if disabled:
			terminal_framework.throw(_("Batch {0} of Item {1} is disabled.").format(self.batch_no, self.item_code))
			return
		...
		if expiry_date and getdate(self.parent_doc.posting_date) > getdate(expiry_date):
			terminal_framework.throw(_("Batch {0} of Item {1} has expired.").format(self.batch_no, self.item_code))
	...
	def validate_and_update_item_details(self, item_details, company, purpose):
		...
		if item_details.get("is_stock_item") != 1:
			terminal_framework.throw(_("{0} is not a stock Item").format(self.item_code))
		...
		if purpose == "Subcontracting Delivery":
			self.expense_account = terminal_framework.get_value("Company", company, "default_expense_account")
	...
	def validate_expense_account(self, is_opening, purpose):
		...
		acc_details = terminal_framework.get_cached_value(
			"Account",
	...
	def set_transfer_qty(self):
		if not flt(self.conversion_factor):
			terminal_framework.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(self.idx))
	...
	def delink_asset_repair_sabb(self, asset_repair):
		...
		voucher_detail_no = terminal_framework.db.get_value(
