...
from terminal_framework.utils import cint, cstr, floor, flt, nowdate
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = terminal_framework.db.exists(
	...
	def validate_priority(self):
		if self.priority < 1:
			terminal_framework.throw(_("Priority cannot be less than 1."), title=_("Invalid Priority"))

	def validate_warehouse_and_company(self):
		company = terminal_framework.db.get_value("Warehouse", self.warehouse, "company")
		if company != self.company:
			terminal_framework.throw(
				_("Warehouse {0} does not belong to Company {1}.").format(
					terminal_framework.bold(self.warehouse), terminal_framework.bold(self.company)
				),
				title=_("Invalid Warehouse"),
			)

	def validate_capacity(self):
		stock_uom = terminal_framework.db.get_value("Item", self.item_code, "stock_uom")
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			terminal_framework.throw(
				...
				).format(self.item_code, terminal_framework.bold(balance_qty), stock_uom),
				title=_("Insufficient Capacity"),
			)

		if not self.capacity:
			terminal_framework.throw(_("Capacity must be greater than 0"), title=_("Invalid"))
...
def get_available_putaway_capacity(rule: str):
	stock_capacity, item_code, warehouse = terminal_framework.db.get_value(
...
def apply_putaway_rule(
	...
	for item in items:
		...
		uom_must_be_whole_number = terminal_framework.db.get_value("UOM", item.uom, "must_be_whole_number")
	...
	if updated_table and _items_changed(items, updated_table, doctype):
		terminal_framework.msgprint(_("Applied putaway rules."), alert=True)
		return updated_table

	if sync and terminal_framework.parse_json(sync):  # sync with client side
		return items
...
def _items_changed(old, new, doctype: str) -> bool:
	...
	old = [terminal_framework._dict(item) if isinstance(item, dict) else item for item in old]
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		item_link = terminal_framework.utils.get_link_to_form("Item", entry[0])
		formatted_item_rows += f"""
			<td>{item_link}</td>
			<td>{terminal_framework.bold(entry[1])}</td>
		</tr>"""
	...
	terminal_framework.msgprint(msg, title=_("Insufficient Capacity"), is_minimizable=True, wide=True)
...
def validate_putaway_capacity(doc):
	...
	if not terminal_framework.get_all("Putaway Rule", limit=1):
		return
	...
	if valid_doctype:
		...
		for item in doc.get("items"):
			...
			rule = terminal_framework.db.get_value(
				"Putaway Rule",
		...
		for rule, values in rule_map.items():
			if flt(values["qty_put"]) > flt(values["capacity"]):
				...
				terminal_framework.throw(msg=message, title=_("Over Receipt"))
...
def _prepare_over_receipt_message(rule, values):
	message = _("{0} qty of Item {1} is being received into Warehouse {2} with capacity {3}.").format(
		terminal_framework.bold(values["qty_put"]),
		terminal_framework.bold(values["item"]),
		terminal_framework.bold(values["warehouse"]),
		terminal_framework.bold(values["capacity"]),
	)
	message += "<br><br>"
	rule_link = terminal_framework.utils.get_link_to_form("Putaway Rule", rule)
