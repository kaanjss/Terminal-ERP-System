...
class SendToSubcontractorStockEntry(BaseStockEntry):
	...
	def validate_subcontract_order(self):
		...
		backflush_raw_materials_based_on = terminal_framework.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		if backflush_raw_materials_based_on == "BOM":
			subcontract_order = terminal_framework.get_doc(
	...
	def validate_subcontracting_order_for_bom(self, child_row, subcontract_order):
		...
		qty_allowance = flt(terminal_framework.db.get_single_value("Buying Settings", "over_transfer_allowance"))
	...
	def _get_required_qty_for_bom(self, item_code, child_row, subcontract_order):
		...
		if not required_qty and child_row.allow_alternative_item:
			original_item_code = terminal_framework.get_value(
	...
	def get_order_rm_detail(self, child_row):
		...
		return terminal_framework.db.get_value(self.doc.subcontract_data.order_supplied_items_field, filters, "name")
	...
	def _update_supplied_items_in_order(self, order_supplied_items, supplied_items):
		for row in order_supplied_items:
			...
			terminal_framework.db.set_value(self.doc.subcontract_data.order_supplied_items_field, row.name, item)
...
def get_supplied_items(
	...
	for row in terminal_framework.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, terminal_framework._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
...
def get_items_from_subcontract_order(source_name: str, target_doc: str | Document | None = None):
	...
	target_doc = terminal_framework.get_doc(terminal_framework.parse_json(target_doc))
