...
from terminal_framework.utils import cint, cstr, flt, nowdate

from terminal_erp.manufacturing.doctype.bom.bom import get_backflush_based_on
from terminal_erp.stock.serial_batch_bundle import SerialBatchCreation, get_serial_or_batch_items
...
class StockEntrySABB(BaseStockEntry):
	...
	def _create_or_update_bundle_for_row(self, row, serial_nos, batch_nos, already_picked_serial_nos):
		if row.serial_and_batch_bundle and abs(row.transfer_qty) != abs(
			terminal_framework.get_cached_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "total_qty")
		):
		...
		if not row.serial_and_batch_bundle and terminal_framework.get_single_value(
	...
	def get_serial_batch_fields_for_subcontracting_inward(self):
		serial_nos, batch_nos = terminal_framework._dict(), terminal_framework._dict()
		for row in self.doc.items:
	...
	def get_available_reserved_materials(self):
		...
		itemwise_serial_batch_qty = terminal_framework._dict()

		for d in reserved_entries:
			key = (d.item_code, d.warehouse)
			if key not in itemwise_serial_batch_qty:
				itemwise_serial_batch_qty[key] = terminal_framework._dict(
					{
						"serial_no": [],
	...
	def set_serial_batch_based_on_reservation(self):
		if self.doc.work_order and terminal_framework.get_cached_value(
			"Work Order", self.doc.work_order, "reserve_stock"
		):
			skip_transfer = terminal_framework.get_cached_value("Work Order", self.doc.work_order, "skip_transfer")
	...
	def _make_overflow_batch_row(self, d, batches, details, new_items_to_add, batch_no, qty, original_qty):
		new_row = terminal_framework.copy_doc(d)
		new_row.name = None
...
def _enrich_expired_batches_with_stock(expired_batches):
	...
	expired_batches_stock = get_auto_batch_nos(
		terminal_framework._dict({"batch_no": list(expired_batches.keys()), "for_stock_levels": True})
	)
...
def get_expired_batches():
	...
	expired_batches = terminal_framework._dict()
	for row in data:
