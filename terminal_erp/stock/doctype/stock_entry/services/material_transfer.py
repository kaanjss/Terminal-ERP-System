...
class BaseMaterialTransferStockEntry(BaseStockEntry):
	...
	def validate_warehouse(self):
		for row in self.doc.items:
			if not row.t_warehouse:
				terminal_framework.throw(_("Target Warehouse is required for item {0}").format(row.item_code))
			if not row.s_warehouse:
				terminal_framework.throw(_("Source Warehouse is required for item {0}").format(row.item_code))

	def validate_same_source_target_warehouse(self):
		"""
		Raises: terminal_framework.ValidationError: If warehouses are same and no inventory dimensions differ
		"""

		if not terminal_framework.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
			return
	...
	def _validate_item_transferred_qty(self, item, transferred_qty):
		...
		transfer_qty = terminal_framework.get_value("Stock Entry Detail", item.ste_detail, "transfer_qty")
	...
	def _bulk_update_transferred_qty(self, stock_entries, child_list):
		...
		(
			terminal_framework.qb.update(sed)
...
class MaterialTransferForManufactureStockEntry(BaseMaterialTransferStockEntry):
	...
	def validate_component_and_quantities(self):
		if self.doc.fg_completed_qty:
			if terminal_framework.db.get_single_value("Manufacturing Settings", "validate_components_quantities_per_bom"):
	...
	def _validate_no_excess_transfer(self):
		...
		if (
			terminal_framework.db.get_single_value("Manufacturing Settings", "backflush_raw_materials_based_on")
	...
	def get_pending_raw_materials(self):
		...
		if not item_dict:
			terminal_framework.msgprint(_("All items have already been transferred for this Work Order."))
	...
	def _resolve_wip_warehouse(self, work_order):
		if not terminal_framework.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
	...
	def _build_required_item_row(self, item_dict, d, consider_job_card, wip_warehouse, work_order):
		...
		if d.source_warehouse and not terminal_framework.db.get_value("Warehouse", d.source_warehouse, "is_group"):
	...
	def _get_job_card_item(self, item_code):
		return (
			terminal_framework.db.get_value("Job Card Item", {"item_code": item_code, "parent": self.doc.get("job_card")})
			or None
		)
...
class MaterialRequestStockEntry(BaseMaterialTransferStockEntry):
	...
	def get_material_request(self, item_row):
		...
		if self.doc.outgoing_stock_entry:
			parent_se = terminal_framework.get_value(
				"Stock Entry Detail",
				item_row.ste_detail,
	...
	def validate_material_request(self):
		for row in self.doc.items:
			...
			mreq_item = terminal_framework.db.get_value(
			...
			if mreq_item.item_code != row.item_code:
				terminal_framework.throw(
					...
					terminal_framework.MappingMismatchError,
				)
	...
	def set_material_request_transfer_status(self, status):
		material_requests = []
		parent_se = (
			terminal_framework.get_value("Stock Entry", self.doc.outgoing_stock_entry, "add_to_transit")
			if self.doc.outgoing_stock_entry
			else None
		)
		for item in self.doc.items:
			...
			if mr not in material_requests and self.doc.outgoing_stock_entry and parent_se:
				mr = terminal_framework.get_value("Stock Entry Detail", item.ste_detail, "material_request")
	...
	def _update_mr_transfer_status(self, material_request, status, material_requests):
		...
		if status == "Completed":
			...
			if qty.get("transfer_qty") > qty.get("transferred_qty"):
				status = "In Transit"
		terminal_framework.db.set_value("Material Request", material_request, "transfer_status", status)
		return status
