...
from terminal_framework.utils import ceil, cint, flt, get_link_to_form

from terminal_erp.manufacturing.doctype.bom.bom import add_additional_cost
...
class BaseManufactureStockEntry(BaseStockEntry):
	...
	def validate_warehouse(self):
		for row in self.doc.items:
			if not row.s_warehouse and not row.t_warehouse:
				terminal_framework.throw(_("Source or Target Warehouse is required for item {0}").format(row.item_code))

	def validate_raw_materials_exists(self):
		if terminal_framework.db.get_single_value("Manufacturing Settings", "material_consumption"):
			return
	...
	def add_secondary_items(self):
		...
		for row in secondary_items:
			...
			if not item_args.get("t_warehouse"):
				item_args["t_warehouse"] = terminal_framework.get_cached_value(
	...
	def set_process_loss_qty(self):
		...
		if not self.doc.process_loss_percentage and not self.doc.process_loss_qty:
			self.doc.process_loss_percentage = terminal_framework.get_cached_value(
		...
		item_details.update(
			{
				...
				or terminal_framework.get_cached_value("BOM", self.doc.bom_no, "default_target_warehouse"),
				"s_warehouse": None,
				"is_finished_item": 1,
			}
		)
	...
	def set_serial_nos_for_finished_good(self, item_details, existing_row=None):
		...
		row = terminal_framework._dict({"serial_nos": serial_nos[0 : cint(item_details.qty)]})
	...
	def add_batchwise_finished_good(self, batches, item_details, existing_row=None):
		...
		row = terminal_framework._dict({"batches_to_be_consume": defaultdict(float)})
...
class ManufactureStockEntry(BaseManufactureStockEntry):
		...
		for row in self.doc.items:
			...
			if self.check_invalid_serial_batch_nos_for_finished_good_item(row):
				...
				terminal_framework.msgprint(
					...
					).format(row.idx, terminal_framework.bold(self.doc.work_order))
				)
	...
	def check_invalid_serial_batch_nos_for_finished_good_item(self, row) -> bool:
		if self.wo_doc.has_serial_no:
			...
			if serial_nos:
				valid_serial_nos = terminal_framework.get_all(
					"Serial No",
		...
		if self.wo_doc.has_batch_no:
			...
			if batch_nos:
				valid_batch_nos = terminal_framework.get_all(
					"Batch",
	...
	def validate_component_and_quantities(self):
		if not terminal_framework.db.get_single_value("Manufacturing Settings", "validate_components_quantities_per_bom"):
			return
	...
	def validate_work_order(self):
		if not self.doc.work_order:
			terminal_framework.throw(_("Work Order is mandatory"))
	...
	def add_raw_materials(self):
		material_consumption = terminal_framework.db.get_single_value("Manufacturing Settings", "material_consumption")
	...
	def _resolve_rm_warehouse(self, row):
		...
		if s_warehouse := terminal_framework.get_cached_value("BOM", self.doc.bom_no, "default_source_warehouse"):
			return s_warehouse
	...
	def _query_alternative_items(self, item_codes_in_bom):
		doctype = terminal_framework.qb.DocType("Stock Entry")
		child_doc = terminal_framework.qb.DocType("Stock Entry Detail")
		query = (
			terminal_framework.qb.from_(child_doc)
			.inner_join(doctype)
	...
	def _index_alternative_items(self, data):
		alternative_items = terminal_framework._dict()
		for row in data:
	...
	def get_transfer_entries(self):
		...
		stock_entry_detail = terminal_framework.qb.DocType("Stock Entry Detail")
	...
	def add_materials_from_transfer(self):
		for row in self._transfer_entries:
			...
			if key not in self.available_materials:
				self.available_materials[key] = terminal_framework._dict(row)
			else:
	...
	def get_sabb_details(self, sabb):
		sabb_entries = terminal_framework.get_all(
		...
		return terminal_framework._dict({"serial_nos": serial_nos, "batches": batches})
	...
	def _update_job_card_on_manufacture(self):
		job_doc = terminal_framework.get_doc("Job Card", self.doc.job_card)
...
def get_production_item_details(work_order=None, bom_no=None):
	production_item = (
		terminal_framework.get_cached_value("Work Order", work_order, "production_item")
		if work_order
		else terminal_framework.get_cached_value("BOM", bom_no, "item")
	)
...
def get_bom_items(bom_no, use_multi_level_bom=None, qty=None, fetch_secondary_items=False):
	if use_multi_level_bom is None:
		use_multi_level_bom = terminal_framework.get_cached_value("BOM", bom_no, "use_multi_level_bom")
	qty = qty or 1
...
def _run_bom_items_query(bom_no, table_name, qty):
	...
	doctype = terminal_framework.qb.DocType(table_name)
	query = (
...
def get_secondary_items(bom_no, work_order=None):
	if (
		...
		and work_order
		and terminal_framework.get_cached_value("Work Order", work_order, "use_multi_level_bom")
	):
...
def get_secondary_items_from_job_card(work_order, jc_name=None):
	...
	job_card_secondary_item = terminal_framework.qb.DocType("Job Card Secondary Item")

	secondary_items = (
		terminal_framework.qb.from_(job_card)
		.select(
...
def ceil_qty_if_uom_has_whole_number(qty, stock_uom):
	if cint(terminal_framework.get_cached_value("UOM", stock_uom, "must_be_whole_number")):
		qty = ceil(qty)
...
def move_sample_to_retention_warehouse(company: str, items: str | list):
	...
	retention_warehouse = terminal_framework.get_single_value("Stock Settings", "sample_retention_warehouse")
	stock_entry = terminal_framework.new_doc("Stock Entry")
...
def _apply_sample_quantity(sabb, sabe, warehouse, batch_no, sample_quantity):
	if sabb.has_serial_no:
		entries = [
			...
			and terminal_framework.db.exists("Serial No", {"name": e.serial_no, "warehouse": warehouse})
		][: int(sample_quantity)]
...
def _adjust_sample_quantity(item_code, sample_quantity, batch_no, get_batch_qty):
	...
	max_retain_qty = terminal_framework.get_value("Item", item_code, "sample_quantity")
