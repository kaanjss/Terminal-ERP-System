...
from terminal_framework.query_builder.functions import Max, Min, NullIf, Sum
...
class DisassembleStockEntry(BaseStockEntry):
	...
	def validate_fg_completed_qty(self):
		...
		from terminal_erp.manufacturing.doctype.work_order.work_order import get_disassembly_available_qty
	...
	def validate_finished_good_consumption(self):
		...
		precision = terminal_framework.get_precision("Stock Entry Detail", "transfer_qty")
			...
			if abs(flt(row.transfer_qty, precision) - flt(expected, precision)) > tolerance:
				terminal_framework.throw(
					_(
						...
						row.idx,
						terminal_framework.bold(row.item_code),
	...
	def _get_disassembly_scale_factor(self) -> float:
		...
		if self.doc.source_stock_entry:
			source_fg_qty = flt(
				terminal_framework.db.get_value("Stock Entry", self.doc.source_stock_entry, "fg_completed_qty")
			)
		elif self.doc.work_order:
			source_fg_qty = flt(terminal_framework.db.get_value("Work Order", self.doc.work_order, "produced_qty"))
		else:
			return 0.0
	...
	def _add_items_for_disassembly_from_stock_entry(self):
		source_fg_qty = terminal_framework.db.get_value("Stock Entry", self.doc.source_stock_entry, "fg_completed_qty")
		if not source_fg_qty:
			terminal_framework.throw(
	...
	def _add_items_for_disassembly_from_work_order(self):
		wo_produced_qty = terminal_framework.db.get_value("Work Order", self.doc.work_order, "produced_qty")

		wo_produced_qty = flt(wo_produced_qty)
		if wo_produced_qty <= 0:
			terminal_framework.throw(_("Work Order {0} has no produced qty").format(self.doc.work_order))

		disassemble_qty = flt(self.doc.fg_completed_qty)
		if disassemble_qty <= 0:
			terminal_framework.throw(_("Disassemble Qty cannot be less than or equal to 0."))
	...
	def _add_items_for_disassembly_from_bom(self):
		if not self.doc.bom_no or not self.doc.fg_completed_qty:
			terminal_framework.throw(_("BOM and Finished Good Quantity is mandatory for Disassembly"))
	...
	def get_items_from_manufacture_stock_entry(self):
		...
		query = terminal_framework.qb.from_(SED).join(SE).on(SED.parent == SE.name).where(SE.docstatus == 1)
	...
	def update_disassembled_order(self):
		...
		if self.doc.fg_completed_qty:
			pro_doc = terminal_framework.get_doc("Work Order", self.doc.work_order)
			pro_doc.run_method(
...
def get_available_materials(work_order, stock_entry_doc=None) -> dict:
	...
	for row in data:
		...
		if key not in available_materials:
			available_materials[key] = terminal_framework._dict(
