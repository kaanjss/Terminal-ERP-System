// Copyright (c) 2015, Terminal Framework Technologies Pvt. Ltd. and Contributors // License: GNU General Public License v3. See license.txt

terminal_framework.provide("terminal_erp.stock");
terminal_framework.provide("terminal_erp.accounts.dimensions");

terminal_erp.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");

terminal_framework.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("source_warehouse_address", function () {
			return {
				query: "terminal_erp.controllers.queries.get_warehouse_address",
				filters: {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				terminal_framework.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (
					[
					...
					filters = {
						...
						posting_date: frm.doc.posting_date || terminal_framework.datetime.nowdate(),
					};
				} else {
					filters = {
						item_code: item.item_code,
		...
		frm.set_query("project", "items", function (doc) {
			return {
				query: "terminal_erp.controllers.queries.get_project_name",
				filters: {
					company: doc.company,
				},
			};
		...
		terminal_framework.db.get_single_value("Stock Settings", "disable_serial_no_and_batch_selector").then((value) => {
			if (value) {
		...
		if (frm.doc.purpose == "Receive from Customer") {
			frm.set_query("against_fg", "items", function () {
				return {
					query: "terminal_erp.controllers.subcontracting_inward_controller.get_fg_reference_names",
					filters: {
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new terminal_erp.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "terminal_erp.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	source_stock_entry: async function (frm) {
		...
		let available_qty = await terminal_framework.xcall(
			"terminal_erp.manufacturing.doctype.work_order.work_order.get_disassembly_available_qty",
	...
	refresh: function (frm) {
		...
		terminal_erp.toggle_serial_batch_fields(frm);
		...
		if (!frm.doc.docstatus && !frm.doc.subcontracting_inward_order) {
			...
			frm.add_custom_button(
				__("Material Request"),
				function () {
					terminal_framework.model.with_doctype("Material Request", function () {
						var mr = terminal_framework.model.get_new_doc("Material Request");
						...
						items.forEach(function (item) {
							var mr_item = terminal_framework.model.add_child(mr, "items");
							...
							mr_item.required_date = terminal_framework.datetime.nowdate();
						});
						terminal_framework.set_route("Form", "Material Request", mr.name);
					});
				},
				__("Create")
			);
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					terminal_erp.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					terminal_framework.model.open_mapped_doc({
						method: "terminal_erp.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(
					...
					function () {
						...
						terminal_framework.set_route("List", "Stock Entry");
					},
					__("View")
				);
			}
						...
						terminal_framework.prompt(
							...
							async (data) => {
								if (frm.doc.work_order) {
									let stock_entry = await terminal_framework.xcall(
										"terminal_erp.manufacturing.doctype.work_order.mapper.make_stock_entry",
										{
									...
									if (stock_entry) {
										terminal_framework.model.sync(stock_entry);
										terminal_framework.set_route("Form", stock_entry.doctype, stock_entry.name);
									}
								} else {
									let se = terminal_framework.model.get_new_doc("Stock Entry");
									...
									terminal_framework.set_route("Form", "Stock Entry", se.name);
								}
							},
							__("Disassemble"),
							__("Create")
		...
		if (frm.doc.docstatus === 0 && !frm.doc.subcontracting_inward_order) {
			frm.add_custom_button(
				...
				function () {
					terminal_erp.utils.map_current_doc({
						method: "terminal_erp.accounts.doctype.purchase_invoice.mapper.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = terminal_erp.utils.map_current_doc({
						method: "terminal_erp.stock.doctype.material_request.mapper.make_stock_entry",
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.call({
						method: "terminal_erp.stock.doctype.stock_entry.stock_entry_handler.serial_batch.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	cost_center(frm, cdt, cdn) {
		terminal_erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	validate_purpose_consumption: function (frm) {
		terminal_framework
			.call({
				method: "terminal_erp.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
				if (cint(r.message) == 0 && frm.doc.purpose == "Material Consumption for Manufacture") {
					frm.set_value("purpose", "Manufacture");
					terminal_framework.throw(__("Material Consumption is not set in Manufacturing Settings."));
				}
			});
	},
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			terminal_framework.call({
				...
				callback: function (r) {
					terminal_framework.model.set_value(cdt, cdn, "basic_rate", r.message || 0.0);
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			terminal_framework.call({
				method: "terminal_erp.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
				...
				callback: function (r) {
					if (!r.exc) {
						...
						fields.forEach((field) => {
							terminal_framework.model.set_value(cdt, cdn, field, r.message[field] || 0.0);
						});
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			if (!frm.doc.company) terminal_framework.throw(__("Company field is required"));
			terminal_framework.call({
				method: "terminal_erp.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						terminal_framework.throw(__("BOM does not contain any stock item"));
					} else {
						terminal_erp.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function (i, item) {
							let d = terminal_framework.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function (frm) {
		const total_additional_costs = terminal_framework.utils.sum(
	...
	source_warehouse_address: function (frm) {
		terminal_erp.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		terminal_erp.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	set_transit_warehouse: function (frm) {
		if (
			...
			terminal_framework.db.get_value(dt, dn, "default_in_transit_warehouse", (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) terminal_erp.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			terminal_erp.utils.map_current_doc({
				method: "terminal_erp.stock.doctype.stock_entry.stock_entry_handler.subcontracting.get_items_from_subcontract_order",
...
terminal_framework.ui.form.on("Stock Entry Detail", {
	...
	s_warehouse(frm, cdt, cdn) {
		...
		let item = terminal_framework.get_doc(cdt, cdn);
		if (item.s_warehouse) {
			terminal_framework.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return terminal_framework.call({
				method: "terminal_erp.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						terminal_framework.model.set_value(cdt, cdn, r.message);
					}
				},
			});
		}
					...
					if (r.message) {
						...
						$.each(r.message, function (k, v) {
							if (v) {
								...
								if (k != "barcode") {
									terminal_framework.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
								} else {
									d.barcode = v;
								}
							}
						...
						if (no_batch_serial_number_value && !terminal_framework.flags.hide_serial_batch_dialog) {
							if (!terminal_framework.flags.dialog_set) {
								terminal_framework.flags.dialog_set = true;
							}
							terminal_erp.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		terminal_erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		terminal_erp.stock.select_batch_and_serial_no(frm, child);
	},
});
...
terminal_framework.ui.form.on("Landed Cost Taxes and Charges", {
...
terminal_erp.stock.StockEntry = class StockEntry extends terminal_erp.stock.StockController {
	setup() {
		var me = this;

		this.barcode_scanner = new terminal_erp.utils.BarcodeScanner({
			frm: this.frm,
			warehouse_field: (doc) => {
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return terminal_erp.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (terminal_erp.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		terminal_framework.dynamic_link = { doc: this.frm.doc, fieldname: "supplier", doctype: "Supplier" };
		this.frm.set_query("supplier_address", terminal_erp.queries.address_query);
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 1 && terminal_erp.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		terminal_erp.hide_company(this.frm);
		terminal_erp.utils.add_item(this.frm);
	...
	company() {
		if (this.frm.doc.company) {
			var company_doc = terminal_framework.get_doc(":Company", this.frm.doc.company);
			if (company_doc.default_letter_head) {
				this.frm.set_value("letter_head", company_doc.default_letter_head);
			}
			this.frm.trigger("toggle_display_account_head");

			terminal_erp.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "terminal_erp.accounts.utils.get_company_default",
			args: {
	...
	clean_up() {
		...
		if (
			...
			terminal_framework.model.remove_from_locals("Work Order", this.frm.doc.work_order);
		}
	}
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						terminal_erp.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return terminal_framework.call({
			method: "terminal_erp.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button() {
		if (terminal_framework.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(
				__("Excise Invoice"),
				function () {
					var excise = terminal_framework.model.make_new_doc_and_get_name("Journal Entry");
					excise = locals["Journal Entry"][excise];
					excise.voucher_type = "Excise Entry";
					terminal_framework.set_route("Form", "Journal Entry", excise.name);
				},
				__("Create")
			);
	}
	...
	items_add(doc, cdt, cdn) {
		var row = terminal_framework.get_doc(cdt, cdn);
		...
		if (cint(terminal_framework.user_defaults?.use_serial_batch_fields)) {
			terminal_framework.model.set_value(row.doctype, row.name, "use_serial_batch_fields", 1);
		}
	}
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new terminal_erp.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		terminal_erp.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (terminal_erp.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		terminal_erp.utils.get_party_details(this.frm, null, null, null);
	}
};

terminal_erp.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/terminal_erp/js/utils/serial_no_batch_selector.js";

	terminal_framework.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			item.has_serial_no = r.message.has_serial_no;
			item.has_batch_no = r.message.has_batch_no;
			item.type_of_transaction = item.s_warehouse ? "Outward" : "Inward";

			new terminal_erp.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
					terminal_framework.model.set_value(item.doctype, item.name, {
...
terminal_erp.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	terminal_framework.db.get_doc("BOM", bom_no).then((bom) => {
		const { name, items } = bom;
		terminal_erp.stock.bom = { name, items: {} };
		items.forEach((item) => {
			terminal_erp.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (terminal_erp.stock.bom && terminal_erp.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new terminal_erp.stock.StockEntry({ frm: cur_frm }));
