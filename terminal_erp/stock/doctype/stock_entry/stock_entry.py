...
import terminal_erp
from terminal_erp.buying.utils import check_on_hold_or_closed_status
from terminal_erp.controllers.taxes_and_totals import init_landed_taxes_and_totals
from terminal_erp.manufacturing.doctype.bom.bom import (
...
from terminal_erp.stock.stock_ledger import get_previous_sle, get_valuation_rate
from terminal_erp.stock.utils import get_incoming_rate
...
class FinishedGoodError(terminal_framework.ValidationError):
	pass


class IncorrectValuationRateError(terminal_framework.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(terminal_framework.ValidationError):
	pass


from terminal_erp.controllers.stock_controller import StockController
from terminal_erp.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from terminal_erp.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def __init__(self, *args, **kwargs):
		...
		if self.subcontracting_inward_order:
			self.subcontract_data = terminal_framework._dict(
				{
	...
	def transfer_for_material_request(self):
		if self.outgoing_stock_entry and terminal_framework.get_all(
			"Stock Entry Detail",
	...
	def before_insert(self):
		if self.subcontracting_order and terminal_framework.get_cached_value(
			"Subcontracting Order", self.subcontracting_order, "reserve_stock"
		):
			self.set_serial_batch_from_reserved_entry()

	def before_validate(self):
		from terminal_erp.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		from terminal_erp.stock.doctype.putaway_rule.putaway_rule import validate_putaway_capacity
		from terminal_erp.stock.services.serial_batch_bundle_service import SerialBatchBundleService
	...
	def validate_job_card_fg_item(self):
		...
		job_card = terminal_framework.db.get_value(
		...
		for row in self.items:
			if row.is_finished_item and row.item_code != job_card.finished_good:
				terminal_framework.throw(_("Row #{0}: Finished Good must be {1}").format(row.idx, job_card.finished_good))

	def validate_job_card_item(self):
		if not self.job_card or self.purpose == "Manufacture":
			return

		if cint(terminal_framework.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")):
			return
	...
	def validate_work_order_status(self):
		pro_doc = terminal_framework.get_doc("Work Order", self.work_order)
		if pro_doc.status == "Completed":
			terminal_framework.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in terminal_framework.get_all(
				...
				terminal_framework.delete_doc("Stock Entry", d.name)
	...
	def update_cost_in_project(self):
		if self.work_order and not terminal_framework.db.get_value(
		...
		for project in projects:
			project_doc = terminal_framework.get_doc("Project", project)
	...
	def _check_process_loss_qty(self, fg_qty):
		precision = terminal_framework.get_precision("Stock Entry Detail", "qty")
	...
	def _validate_fg_qty_with_process_loss(self, d, fg_item_qty, fg_completed_qty, precision):
		...
		if fg_completed_qty != (flt(fg_item_qty, precision) + flt(self.process_loss_qty, precision)):
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(self.process_loss_qty), terminal_framework.bold(d.item_code))
			)

	def validate_difference_account(self):
		if not cint(terminal_erp.is_perpetual_inventory_enabled(self.company)):
			return
	...
	def validate_source_stock_entry(self):
		...
		if self.work_order:
			source_wo = terminal_framework.db.get_value("Stock Entry", self.source_stock_entry, "work_order")
	...
	def _set_incoming_item_rate(self, d, outgoing_items_cost, raise_error_if_no_rate, zero_valuation_items):
		if d.allow_zero_valuation_rate and d.basic_rate and self.purpose != "Receive from Customer":
			...
			if self.bom_no:
				d.basic_rate *= terminal_framework.get_value("BOM", self.bom_no, "cost_allocation_per") / 100
		elif d.secondary_item_type and d.bom_secondary_item:
			cost_allocation_per = terminal_framework.get_value(
		...
		if not d.basic_rate and not d.allow_zero_valuation_rate:
			d.basic_rate = get_valuation_rate(
				...
				currency=terminal_erp.get_company_currency(self.company),
				company=self.company,
	...
	def _notify_zero_valuation_rate(self, items):
		if len(items) > 1:
			...
			).format(", ".join(terminal_framework.bold(item) for item in items))
		else:
			message = _(
				"Item rate has been updated to zero as Allow Zero Valuation Rate is checked for item {0}"
			).format(terminal_framework.bold(items[0]))

		terminal_framework.msgprint(message, alert=True)
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0) -> float:
		settings = terminal_framework.get_single("Manufacturing Settings")
	...
	def _validate_no_raw_materials_in_manufacture_entry(self, settings):
		for item in self.items:
			if not item.is_finished_item and not item.secondary_item_type and not item.is_legacy_scrap_item:
				label = terminal_framework.get_meta(settings.doctype).get_label("get_rm_cost_from_consumption_entry")
				terminal_framework.throw(
					_(
						"Row {0}: As {1} is enabled, raw materials cannot be added to {2} entry. Use {3} entry to consume raw materials."
					).format(
						item.idx,
						terminal_framework.bold(label),
						terminal_framework.bold(_("Manufacture")),
						terminal_framework.bold(_("Material Consumption for Manufacture")),
					)
				)
		...
		if terminal_framework.db.exists(
			...
			terminal_framework.throw(
				_("Only one {0} entry can be created against the Work Order {1}").format(
					terminal_framework.bold(_("Manufacture")), terminal_framework.bold(self.work_order)
				)
			)

	def _fetch_consumption_entry_cost(self):
		SE = terminal_framework.qb.DocType("Stock Entry")
		SE_ITEM = terminal_framework.qb.DocType("Stock Entry Detail")

		return (
			terminal_framework.qb.from_(SE)
			.left_join(SE_ITEM)
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = terminal_framework.get_cached_value(
	...
	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = terminal_framework.get_cached_value("Stock Entry Type", self.stock_entry_type, "purpose")
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = terminal_framework.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = terminal_framework.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		if self.work_order:
			wo_details = terminal_framework.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
			if wo_details:
	...
	def validate_serial_batch_bundle_type(self, serial_and_batch_bundle):
		if (
			terminal_framework.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "type_of_transaction")
			!= "Outward"
		):
			terminal_framework.throw(
				_(
		...
		for d in self.get("items"):
			if cstr(d.s_warehouse):
				...
				if sle.serial_and_batch_bundle and self.docstatus == 2:
					bundle_id = terminal_framework.get_cached_value(
						"Serial and Batch Bundle",
						{
	...
	def get_gl_entries(self, inventory_account_map):
		from terminal_erp.stock.doctype.stock_entry.services.gl_composer import StockEntryGLComposer
	...
	def pro_doc(self):
		if not getattr(self, "_wo_doc", None):
			if self.work_order:
				self._wo_doc = terminal_framework.get_doc("Work Order", self.work_order)
		return getattr(self, "_wo_doc", None)

	def make_stock_reserve_for_wip_and_fg(self):
		from terminal_erp.manufacturing.doctype.work_order.services.reservation import (
	...
	def reserve_stock_for_subcontracting(self):
		if self.purpose == "Send to Subcontractor" and terminal_framework.get_value(
			"Subcontracting Order", self.subcontracting_order, "reserve_stock"
		):
			items = {}
			for item in self.items:
				if item.sco_rm_detail in items:
					...
					items[item.sco_rm_detail] = terminal_framework._dict(
						{
			...
			terminal_framework.get_doc("Subcontracting Order", self.subcontracting_order).reserve_raw_materials(
	...
	def is_stock_reserve_for_work_order(self):
		if (
			...
			and terminal_framework.get_cached_value("Work Order", self.work_order, "reserve_stock")
		):
			return True
	...
	def update_wo_reservation_for_subcontracting(self):
		...
		for wo_name in self.get_reserved_work_orders_for_subcontracting():
			pro_doc = terminal_framework.get_doc("Work Order", wo_name)
	...
	def get_reserved_work_orders_for_subcontracting(self):
		...
		for work_order in set(work_orders):
			...
			if terminal_framework.get_cached_value("Work Order", work_order, "reserve_stock"):
	...
	def _fetch_item_data(self, args):
		item_dt = terminal_framework.qb.DocType("Item")
		item_default = terminal_framework.qb.DocType("Item Default")

		result = (
			terminal_framework.qb.from_(item_dt)
	...
	def _apply_account_defaults(self, ret):
		if not ret.get("expense_account"):
			ret["expense_account"] = terminal_framework.get_cached_value(
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = terminal_framework.get_cached_value("Company", self.company, company_field)

	def _resolve_subcontract_item(self, args, ret):
		if not (self.get(self.subcontract_data.order_field) and args.get("item_code")):
			return

		subcontract_items = terminal_framework.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = terminal_framework.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				terminal_framework.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def set_process_loss_qty(self):
		...
		if not self.process_loss_percentage and not self.process_loss_qty:
			self.process_loss_percentage = terminal_framework.get_cached_value(
	...
	def get_bom_raw_materials(self, qty):
		from terminal_erp.manufacturing.doctype.bom.bom import get_bom_items_as_dict
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = terminal_framework.db.get_value(
					"Work Order", self.work_order, "allow_alternative_item"
				)

			skip_transfer, from_wip_warehouse = (
				terminal_framework.get_value("Work Order", self.work_order, ["skip_transfer", "from_wip_warehouse"])
				if self.work_order
				else [None, None]
			)

			item.from_warehouse = (
				terminal_framework.get_value(
					"Work Order Item",
	...
	def update_pick_list_status(self):
		from terminal_erp.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def make_stock_in_entry(source_name: str, target_doc: str | Document | None = None):
	def set_missing_values(source, target):
		...
		if not terminal_framework.get_single_value("Stock Settings", "use_serial_batch_fields"):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = terminal_framework.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
...
def get_work_order_details(work_order: str, company: str):
	work_order = terminal_framework.get_doc("Work Order", work_order)
...
def get_consumed_operating_cost(wo_name, bom_no, operation_id):
	...
	child_table = terminal_framework.qb.DocType("Landed Cost Taxes and Charges")
	query = (
		terminal_framework.qb.from_(child_table)
		.join(table)
...
def get_remaining_operating_cost(work_order=None, bom_no=None):
	...
	if work_order:
		if (
			bom_no
			and terminal_framework.db.get_single_value(
				"Manufacturing Settings", "set_op_cost_and_secondary_items_from_sub_assemblies"
			)
			and terminal_framework.get_cached_value("Work Order", work_order.name, "use_multi_level_bom")
		):
	...
	if not remaining_operating_cost and bom_no:
		bom = terminal_framework.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_used_alternative_items(
	...
	ste = terminal_framework.qb.DocType("Stock Entry")
	sted = terminal_framework.qb.DocType("Stock Entry Detail")

	query = (
		terminal_framework.qb.from_(ste)
		.inner_join(sted)
	...
	if subcontract_order:
		...
		if subcontract_order_field not in ("subcontracting_order", "subcontracting_inward_order"):
			terminal_framework.throw(_("Invalid subcontract order field: {0}").format(subcontract_order_field))
		query = query.where(
...
def get_uom_details(item_code: str, uom: str, qty: float | None):
	...
	if not conversion_factor:
		terminal_framework.msgprint(_("UOM conversion factor required for UOM: {0} in Item: {1}").format(uom, item_code))
		ret = {"uom": ""}
	else:
		ret = {
