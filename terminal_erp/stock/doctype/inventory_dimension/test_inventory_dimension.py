...
from terminal_erp.stock.doctype.inventory_dimension.inventory_dimension import (
	CanNotBeChildDoc,
...
from terminal_erp.stock.doctype.stock_ledger_entry.stock_ledger_entry import InventoryDimensionNegativeStockError
from terminal_erp.stock.doctype.warehouse.test_warehouse import create_warehouse
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestInventoryDimension(Terminal ERPTestSuite):
	...
	def test_delete_inventory_dimension(self):
		...
		custom_field = terminal_framework.db.get_value(
	...
	def test_inventory_dimension(self):
		...
		sle_data = terminal_framework.db.get_value(
		...
		sle_shelf = terminal_framework.db.get_value("Stock Ledger Entry", {"voucher_no": outward.name}, "shelf")
	...
	def test_inventory_dimension_for_purchase_receipt_and_delivery_note(self):
		...
		sle_rack = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_check_standard_dimensions(self):
		...
		self.assertFalse(
			terminal_framework.db.get_value("Custom Field", {"fieldname": "project", "dt": "Stock Ledger Entry"}, "name")
		)
	...
	def test_for_purchase_sales_and_stock_transaction(self):
		...
		if not terminal_framework.db.exists("Store", "Rejected Store"):
			terminal_framework.get_doc({"doctype": "Store", "store_name": "Rejected Store"}).insert(
	...
	def test_validate_negative_stock_for_inventory_dimension(self):
		...
		site_name = terminal_framework.get_all(
		...
		terminal_framework.clear_cache(doctype="Inventory Dimension")
...
def create_inventory_dimension(**args):
	...
	if terminal_framework.db.exists("Inventory Dimension", args.dimension_name):
		return terminal_framework.get_doc("Inventory Dimension", args.dimension_name)

	doc = terminal_framework.new_doc("Inventory Dimension")
	doc.update(args)
...
def prepare_data_for_internal_transfer():
	...
	for store in ["Inter Transfer Store 1", "Inter Transfer Store 2", "Inter Transfer Store 3"]:
		if not terminal_framework.db.exists("Store", store):
			terminal_framework.get_doc({"doctype": "Store", "store_name": store}).insert(ignore_permissions=True)
	...
	cost_center = terminal_framework.db.get_value("Company", company, "cost_center") or terminal_framework.db.get_value(
		"Cost Center", {"company": company}, "name"
	)

	expene_account = terminal_framework.db.get_value(
		"Company", company, "stock_adjustment_account"
	) or terminal_framework.db.get_value("Account", {"company": company, "account_type": "Expense Account"}, "name")
	...
	return terminal_framework._dict(
		{
			...
			"store_warehouse": terminal_framework.db.get_value(
