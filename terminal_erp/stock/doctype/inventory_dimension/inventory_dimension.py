...
class DoNotChangeError(terminal_framework.ValidationError):
	pass


class CanNotBeChildDoc(terminal_framework.ValidationError):
	pass


class CanNotBeDefaultDimension(terminal_framework.ValidationError):
	pass
...
class InventoryDimension(Document):
	...
	def onload(self):
		if not self.is_new() and terminal_framework.db.has_column("Stock Ledger Entry", self.target_fieldname):
	...
	def do_not_update_document(self):
		...
		for field in terminal_framework.get_meta("Inventory Dimension").fields:
			if field.fieldname not in allow_to_edit_fields and old_doc.get(field.fieldname) != self.get(
				...
				terminal_framework.throw(_(msg), DoNotChangeError)
	...
	def delete_custom_fields(self):
		...
		for field in terminal_framework.get_all("Custom Field", filters=filters):
			terminal_framework.delete_doc("Custom Field", field.name)

		msg = f"Deleted custom fields related to the dimension {self.name}"
		terminal_framework.msgprint(_(msg))
	...
	def validate_reference_document(self):
		if terminal_framework.get_cached_value("DocType", self.reference_document, "istable") == 1:
			msg = f"The reference document {self.reference_document} can not be child table."
			terminal_framework.throw(_(msg), CanNotBeChildDoc)
		...
		if self.reference_document in ["Batch", "Serial No", "Warehouse", "Item"]:
			...
			terminal_framework.throw(_(msg), CanNotBeDefaultDimension)
	...
	def add_custom_fields(self):
		...
		for dt in ["Stock Ledger Entry", "Stock Closing Balance"]:
			if (
				dimension_fields
				and not terminal_framework.db.get_value("Custom Field", {"dt": dt, "fieldname": self.target_fieldname})
...
def field_exists(doctype, fieldname) -> str | None:
	return terminal_framework.db.get_value("DocField", {"parent": doctype, "fieldname": fieldname}, "name")
...
def get_evaluated_inventory_dimension(doc, sl_dict, parent_doc=None):
	...
	for row in dimensions:
		...
		if row.condition and terminal_framework.safe_eval(row.condition, evals):
...
def delete_dimension(dimension: str):
	doc = terminal_framework.get_doc("Inventory Dimension", dimension)
	doc.delete()


@terminal_framework.whitelist()
def get_parent_fields(child_doctype: str, dimension_name: str):
	parent_doctypes = terminal_framework.get_all("DocField", fields=["parent"], filters={"options": child_doctype})
