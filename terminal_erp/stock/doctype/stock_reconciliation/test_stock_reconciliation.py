...
from terminal_framework.utils import add_days, cstr, flt, nowdate, nowtime

from terminal_erp.accounts.utils import get_stock_and_account_balance
...
from terminal_erp.stock.stock_ledger import get_previous_sle, update_entries_after
...
class TestStockReconciliation(Terminal ERPTestSuite, StockTestMixin):
	def setUp(self):
		terminal_framework.db.set_single_value("Stock Settings", "allow_negative_stock", 1)
		terminal_framework.local.future_sle = {}
		terminal_framework.flags.pop("dont_execute_stock_reposts", None)
	...
	def test_stock_reco_for_serialized_item(self):
		...
		serial_nos = terminal_framework.get_doc(
		...
		sn_doc = terminal_framework.get_doc("Serial and Batch Bundle", sr.items[0].serial_and_batch_bundle)
		...
		for d in to_delete_records:
			stock_doc = terminal_framework.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no)

		sr.cancel()

		self.assertEqual(terminal_framework.db.get_value("Serial No", serial_nos[0], "warehouse"), None)
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no)
		self.assertTrue(terminal_framework.db.get_value("Serial No", reco_serial_no, "warehouse"))

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertFalse(terminal_framework.db.get_value("Serial No", serial_no_2, "warehouse"))
	...
	def test_backdated_stock_reco_qty_reposting(self):
		...
		terminal_framework.flags.dont_execute_stock_reposts = True

		def assertBalance(doc, qty_after_transaction):
			sle_balance = terminal_framework.db.get_value(
	...
	def test_backdated_stock_reco_future_negative_stock(self):
		...
		pr1_balance = terminal_framework.db.get_value(
			"Stock Ledger Entry", {"voucher_no": pr1.name, "is_cancelled": 0}, "qty_after_transaction"
		)
		dn2_balance = terminal_framework.db.get_value(
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = terminal_framework.db.get_value(
		...
		repost_exists = bool(
			terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": sr.name, "status": "Queued"})
		)
	...
	def test_intermediate_sr_bin_update(self):
		...
		old_bin_qty = terminal_framework.db.get_value(
		...
		new_bin_qty = terminal_framework.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_valid_batch(self):
		...
		self.assertRaises(terminal_framework.ValidationError, doc.save)
	...
	def test_serial_no_cancellation(self):
		...
		active_sr_no = terminal_framework.get_all(
	...
	def test_serial_no_creation_and_inactivation(self):
		...
		if not terminal_framework.db.exists("Serial No", "SR-CREATED-SR-NO"):
	...
	def test_serial_no_batch_no_item(self):
		...
		sl_entry = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_stock_reco_entry(self):
		...
		posting_datetime = terminal_framework.db.get_value("Serial and Batch Bundle", stock_reco_sabb, "posting_datetime")
		self.assertEqual(
		...
		self.assertFalse(terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": stock_reco.name}))

		self.assertRaises(terminal_framework.ValidationError, stock_reco.cancel)
	...
	def assert_reco_difference_matches_gl(self, reco_name):
		...
		reco = terminal_framework.get_doc("Stock Reconciliation", reco_name)
	...
	def test_make_stock_zero_for_serial_batch_item(self):
		...
		for item_code in [serial_item, batch_item, serial_batch_item]:
			...
			serial_batch_bundle = terminal_framework.get_all(
				"Stock Ledger Entry",
	...
	def test_not_reconcile_all_batch(self):
		...
		for qty in [10, 20, 30]:
			...
			batches.append(terminal_framework._dict({"batch_no": batch_no, "qty": qty}))
		...
		doc = terminal_framework.get_doc("Serial and Batch Bundle", current_sabb)
		...
		for row in terminal_framework.get_all("Repost Item Valuation", filters={"voucher_no": sr.name}):
			rdoc = terminal_framework.get_doc("Repost Item Valuation", row.name)
			rdoc.cancel()
			rdoc.delete()
	...
	def test_not_reconcile_all_serial_nos(self):
		...
		doc = terminal_framework.get_doc("Serial and Batch Bundle", sabb)
		for row in doc.entries:
	...
	def test_stock_reco_with_legacy_batch(self):
		...
		if not terminal_framework.db.exists("Batch", batch_id):
			batch_doc = terminal_framework.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_id,
		...
		batch_doc = terminal_framework.get_doc("Batch", batch_id)
	...
	def test_stock_reco_for_negative_batch(self):
		...
		doc = terminal_framework.get_doc("Stock Ledger Entry", sles[0].name)
		doc.db_set(
			{
				"actual_qty": -20,
		...
		sabb_doc = terminal_framework.get_doc("Serial and Batch Bundle", doc.serial_and_batch_bundle)
	...
	def test_stock_reco_recalculate_qty_for_backdated_entry(self):
		...
		batch_no = get_batch_from_bundle(sr.items[0].serial_and_batch_bundle)
		stock_ledgers = terminal_framework.get_all(
			"Stock Ledger Entry",
		...
		stock_ledger = terminal_framework.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se.name, "is_cancelled": 0},
	...
	def test_serial_no_backdated_stock_reco(self):
		...
		status = terminal_framework.get_value(
			"Serial No",
			serial_no,
			"status",
		)
	...
	def test_sabb_cancel_on_stock_reco_cancellation(self):
		...
		for sabb in [serial_and_batch_bundle, current_serial_and_batch_bundle]:
			docstatus = terminal_framework.db.get_value(
	...
	def test_stock_reco_with_opening_stock_with_diff_inventory(self):
		from terminal_erp.stock.doctype.inventory_dimension.test_inventory_dimension import (
			create_inventory_dimension,
		)

		if terminal_framework.db.exists("DocType", "Plant"):
			return

		doctype = terminal_framework.get_doc(
			{
				"doctype": "DocType",
				"name": "Plant",
				"module": "Stock",
		...
		plant_a = terminal_framework.get_doc(
			{
				"doctype": "Plant",
				"plant_name": "Plant A",
			}
		...
		plant_b = terminal_framework.get_doc(
			{
				"doctype": "Plant",
				"plant_name": "Plant B",
			}
		...
		sr = terminal_framework.new_doc("Stock Reconciliation")
		...
		sle_count = terminal_framework.db.count(
			"Stock Ledger Entry",
	...
	def test_serial_no_status_with_backdated_stock_reco(self):
		...
		self.assertEqual(terminal_framework.get_value("Serial No", serial_no, "status"), "Delivered")
...
def create_batch_item_with_batch(item_name, batch_id):
	...
	if not terminal_framework.db.exists("Batch", batch_id):
		b = terminal_framework.new_doc("Batch")
		b.item = item_name
		b.batch_id = batch_id
		b.save()
...
def create_stock_reconciliation(**args):
	...
	sr.expense_account = args.expense_account or (
		(
			terminal_framework.get_cached_value("Company", sr.company, "stock_adjustment_account")
			or terminal_framework.get_cached_value(
				"Account", {"account_type": "Stock Adjustment", "company": sr.company}, "name"
			)
		)
		if terminal_framework.get_all("Stock Ledger Entry", {"company": sr.company})
		else terminal_framework.get_cached_value("Account", {"account_type": "Temporary", "company": sr.company}, "name")
	)
	sr.cost_center = (
		args.cost_center
		or terminal_framework.get_cached_value("Company", sr.company, "cost_center")
		or terminal_framework.get_cached_value("Cost Center", filters={"is_group": 0, "company": sr.company})
	)
...
def set_valuation_method(item_code, valuation_method):
	...
	terminal_framework.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in terminal_framework.get_all(
