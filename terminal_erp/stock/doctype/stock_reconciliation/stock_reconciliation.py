...
from terminal_framework import _, bold, json, msgprint
from terminal_framework.query_builder.functions import Sum
from terminal_framework.utils import add_to_date, cint, cstr, flt, get_link_to_form, now
...
from terminal_erp.stock.doctype.batch.batch import get_available_batches, get_batch_qty
...
from terminal_erp.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import StockReconciliationItem
from terminal_erp.stock.utils import get_incoming_rate, get_stock_balance, get_valuation_method


class OpeningEntryAccountError(terminal_framework.ValidationError):
	pass


class EmptyStockReconciliationItemsError(terminal_framework.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import (
	...
	def validate(self):
		...
		if not self.expense_account:
			self.expense_account = terminal_framework.get_cached_value(
				"Company", self.company, "stock_adjustment_account"
			)
		if not self.cost_center:
			self.cost_center = terminal_framework.get_cached_value("Company", self.company, "cost_center")
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": terminal_framework._dict({row.batch_no: row.current_qty}) if row.batch_no else None,
	...
	def set_current_serial_and_batch_bundle(self, voucher_detail_no=None, save=False) -> None:
		...
		for item in self.items:
			...
			if not item.reconcile_all_serial_batch and item.serial_and_batch_bundle:
				...
				if save:
					...
					if (
						...
						sabb_doc = terminal_framework.get_doc(
			...
			if not item.current_serial_and_batch_bundle:
				serial_and_batch_bundle = terminal_framework.get_doc(
					{
			...
			if save:
				sle_creation = terminal_framework.db.get_value(
	...
	def has_change_in_serial_batch(self, row) -> bool:
		...
		if diff:
			...
			terminal_framework.delete_doc("Serial and Batch Bundle", bundle)
	...
	def set_new_serial_and_batch_bundle(self):
		for item in self.items:
			...
			if item.current_serial_and_batch_bundle and not item.serial_and_batch_bundle:
				current_doc = terminal_framework.get_doc("Serial and Batch Bundle", item.current_serial_and_batch_bundle)

				item.qty = abs(current_doc.total_qty)
				item.valuation_rate = abs(current_doc.avg_rate)

				bundle_doc = terminal_framework.copy_doc(current_doc)
				...
				bundle_doc = terminal_framework.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def update_existing_serial_and_batch_bundle(self, item):
		batch_details = terminal_framework.get_all(
			"Serial and Batch Entry",
		...
		if batch_details and len(batch_details) == 1:
			...
			terminal_framework.db.set_value("Serial and Batch Entry", batch.name, update_values)

	def remove_items_with_no_change(self):
		from terminal_erp.stock.stock_ledger import get_stock_value_difference
		...
		if not items:
			...
			terminal_framework.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		default_currency = terminal_framework.db.get_default("currency")
		...
		for row in self.items:
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = terminal_framework.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = terminal_framework.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise terminal_framework.ValidationError(self.validation_messages)
	...
	def validate_item(self, item_code, row):
		...
		def validate_serial_batch_items():
			has_batch_no, has_serial_no = terminal_framework.get_value(
				"Item", item_code, ["has_batch_no", "has_serial_no"]
			)
			if row.use_serial_batch_fields and self.purpose == "Stock Reconciliation":
				if has_batch_no and not row.batch_no:
					raise terminal_framework.ValidationError(_("Please enter Batch No"))
				if has_serial_no and not row.serial_no:
					raise terminal_framework.ValidationError(_("Please enter Serial No"))
			...
			if (item.has_serial_no or item.has_batch_no) and not is_standard_cost_item(
				...
				if row.serial_and_batch_bundle:
					terminal_framework.throw(
						...
						).format(row.idx, terminal_framework.bold(row.item_code))
					)
		...
		if sl_entries:
			if not allow_negative_stock:
				allow_negative_stock = cint(
					terminal_framework.db.get_single_value("Stock Settings", "allow_negative_stock")
				)
	...
	def get_sle_for_items(self, row, serial_nos=None, current_bundle=True):
		...
		data = terminal_framework._dict(
			{
				...
				"actual_qty": 0,
				"company": self.company,
				"stock_uom": terminal_framework.db.get_value("Item", row.item_code, "stock_uom"),
		...
		from terminal_erp.stock.services.stock_ledger_service import StockLedgerService
	...
	def make_sle_on_cancel(self):
		...
		if sl_entries:
			...
			sl_entries.reverse()
			allow_negative_stock = cint(terminal_framework.db.get_single_value("Stock Settings", "allow_negative_stock"))
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)
	...
	def get_gl_entries(self, inventory_account_map=None):
		from terminal_erp.stock.doctype.stock_reconciliation.services.gl_composer import (
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			terminal_framework.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not terminal_framework.db.get_all("Stock Ledger Entry", limit=1):
			if terminal_framework.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				terminal_framework.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = terminal_framework.get_cached_value("Item", d.item_code, "is_customer_provided_item")
	...
	def get_current_qty_from_ledger(self, row: StockReconciliationItem):
		...
		reco_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": self.doctype,
				"voucher_no": self.name,
...
def get_item_and_warehouses(item_code, warehouse):
	...
	if terminal_framework.get_cached_value("Warehouse", warehouse, "is_group"):
		...
		for ch_warehouse in childrens:
			items.append(terminal_framework._dict({"item_code": item_code, "warehouse": ch_warehouse}))
	else:
		items = [terminal_framework._dict({"item_code": item_code, "warehouse": warehouse})]

	return items


def get_items_for_stock_reco(warehouse, company):
	lft, rgt = terminal_framework.db.get_value("Warehouse", warehouse, ["lft", "rgt"])
	...
	warehouses_in_tree = (
		terminal_framework.qb.from_(wh).select(wh.name).where((wh.lft >= lft) & (wh.rgt <= rgt) & (wh.is_group == 0))
	)

	items = (
		terminal_framework.qb.from_(bin_dt)
		.inner_join(item)
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from terminal_erp.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	terminal_framework.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = terminal_framework.get_cached_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)

	row = terminal_framework.parse_json(row)
	...
	if not item_dict:
		...
		terminal_framework.throw(msg, title=_("Missing"))

	serial_nos = None
	has_serial_no = bool(item_dict.get("has_serial_no"))
	has_batch_no = bool(item_dict.get("has_batch_no"))

	use_serial_batch_fields = terminal_framework.db.get_single_value("Stock Settings", "use_serial_batch_fields")
