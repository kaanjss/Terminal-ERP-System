...
from terminal_framework.utils import add_to_date, nowdate
...
def create_product_bundle(
	...
	bundle_doc = terminal_framework.get_doc({"doctype": "Product Bundle", "new_item_code": bundle})
...
class TestPackedItem(Terminal ERPTestSuite):
	...
	def test_backfill_patch_stamps_existing_rows(self):
		...
		from terminal_erp.patches.v16_0.submit_existing_product_bundles import (
		...
		terminal_framework.db.set_value("Sales Order Item", so.items[0].name, "product_bundle", None)
		terminal_framework.db.set_value("Sales Order Item", so.items[0].name, "is_product_bundle", 0)
		for pi in so.packed_items:
			terminal_framework.db.set_value("Packed Item", pi.name, "product_bundle", None)

		stamp_versions()

		self.assertEqual(terminal_framework.db.get_value("Sales Order Item", so.items[0].name, "product_bundle"), version)
		self.assertEqual(terminal_framework.db.get_value("Sales Order Item", so.items[0].name, "is_product_bundle"), 1)
		for pi in so.packed_items:
			self.assertEqual(terminal_framework.db.get_value("Packed Item", pi.name, "product_bundle"), version)
	...
	def test_disabled_bundle_blocks_transaction(self):
		...
		terminal_framework.db.set_value("Product Bundle", version, "disabled", 1)
	...
	def test_get_items_from_product_bundle_endpoint(self):
		...
		from terminal_erp.selling.doctype.product_bundle.product_bundle import get_active_product_bundle
		from terminal_erp.stock.doctype.packed_item.packed_item import get_items_from_product_bundle
		...
		draft = terminal_framework.get_doc(
			{
		...
		terminal_framework.db.set_value("Product Bundle", version, "disabled", 1)
		self.addCleanup(terminal_framework.db.set_value, "Product Bundle", version, "disabled", 0)
		self.assertRaises(
			terminal_framework.ValidationError,
			get_items_from_product_bundle,
