...
from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_item_details, get_price_list_rate
...
class PackedItem(Document):
	...
	def set_actual_and_projected_qty(self):
		...
		_bin = terminal_framework.db.get_value(
			"Bin",
...
def make_packing_list(doc):
	...
	set_price_from_children = terminal_framework.get_single_value("Selling Settings", "editable_bundle_item_rates")
...
def get_product_bundle_items_by_name(bundle_name):
	...
	product_bundle_item = terminal_framework.qb.DocType("Product Bundle Item")
	return (
		terminal_framework.qb.from_(product_bundle_item)
		.select(
			...
			if bundle.disabled:
				terminal_framework.throw(
					_("Row #{0}: Product Bundle {1} is disabled and cannot be used in transactions.").format(
						item_row.idx, terminal_framework.bold(chosen)
					),
...
def update_packed_item_stock_data(main_item_row, pi_row, packing_item, item_data, doc):
	...
	if main_item_row.get("so_detail"):
		pi_row.warehouse = terminal_framework.get_value(
			"Packed Item",
			{
	...
	pi_row.use_serial_batch_fields = terminal_framework.get_single_value("Stock Settings", "use_serial_batch_fields")


def update_packed_item_with_pick_list_info(main_item_row, pi_row):
	pl_row = terminal_framework.db.get_value(
		"Pick List Item",
		{
...
def update_packed_item_price_data(pi_row, item_data, doc):
	...
	item_doc = terminal_framework.get_cached_doc("Item", pi_row.item_code)
...
def get_packed_item_bin_qty(item, warehouse):
	bin_data = terminal_framework.db.get_values(
		"Bin",
...
def on_doctype_update():
	terminal_framework.db.add_index("Packed Item", ["item_code", "warehouse"])
...
def get_items_from_product_bundle(row: str | dict):
	...
	row, items = ItemDetailsCtx(terminal_framework.parse_json(row)), []

	if bundle_name := row.get("product_bundle"):
		terminal_framework.has_permission("Product Bundle", "read", bundle_name, throw=True)
		bundle = terminal_framework.db.get_value("Product Bundle", bundle_name, ["docstatus", "disabled"], as_dict=True)
		if not bundle or bundle.docstatus != 1:
			terminal_framework.throw(_("Product Bundle {0} is not submitted").format(terminal_framework.bold(bundle_name)))
		if bundle.disabled:
			terminal_framework.throw(
				_("Product Bundle {0} is disabled and cannot be used in transactions.").format(
					terminal_framework.bold(bundle_name)
				)
			)
		...
		row.update(
			{
				...
				"conversion_rate": 1,
				"currency": terminal_framework.defaults.get_defaults().currency,
			}
		)
