...
from terminal_framework.utils import add_days, cint, flt, formatdate, get_datetime, getdate

from terminal_erp.accounts.utils import get_fiscal_year
from terminal_erp.controllers.item_variant import ItemTemplateCannotHaveStock
from terminal_erp.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
from terminal_erp.stock.serial_batch_bundle import SerialBatchBundle


class StockFreezeError(terminal_framework.ValidationError):
	pass


class BackDatedStockTransaction(terminal_framework.ValidationError):
	pass


class InventoryDimensionNegativeStockError(terminal_framework.ValidationError):
	pass
...
class StockLedgerEntry(Document):
	...
	def validate_inventory_dimension_negative_stock(self):
		...
		flt_precision = cint(terminal_framework.db.get_default("float_precision")) or 2
	...
	def throw_validation_error(self, diff, dimensions):
		msg = _(
			...
			abs(diff),
			terminal_framework.get_desk_link("Item", self.item_code),
			terminal_framework.get_desk_link("Warehouse", self.warehouse),
			terminal_framework.bold(
				", ".join([f"{dimension}: {values.get('value')}" for dimension, values in dimensions.items()])
			),
			self.posting_date,
			self.posting_time,
			terminal_framework.get_desk_link(self.voucher_type, self.voucher_no),
		)
	...
	def on_submit(self):
		...
		if terminal_framework.in_test and terminal_framework.flags.ignore_serial_batch_bundle_validation:
			return
	...
	def validate_mandatory(self):
		...
		if self.voucher_type != "Stock Reconciliation" and not self.actual_qty:
			terminal_framework.throw(_("Actual Qty is mandatory"))

	def validate_serial_batch_no_bundle(self):
		if self.is_cancelled == 1:
			return

		item_detail = terminal_framework.get_cached_value(
			"Item",
			self.item_code,
	...
	def is_standard_cost_revaluation(self):
		...
		from terminal_erp.stock.doctype.stock_reconciliation.stock_reconciliation import is_standard_cost_item
	...
	def throw_error_message(self, message, exception=terminal_framework.ValidationError):
		terminal_framework.throw(_(message), exception)
	...
	def check_stock_frozen_date(self):
		...
		if stock_settings.stock_frozen_upto:
			if (
				...
				and stock_settings.stock_auth_role not in terminal_framework.get_roles()
			):
				terminal_framework.throw(
		...
		if stock_frozen_upto_days:
			...
			if older_than_x_days_ago and stock_settings.stock_auth_role not in terminal_framework.get_roles():
				terminal_framework.throw(
	...
	def validate_batch(self):
		if self.batch_no and self.voucher_type != "Stock Entry":
			...
			expiry_date = terminal_framework.db.get_value("Batch", self.batch_no, "expiry_date")
			if expiry_date:
	...
	def validate_and_set_fiscal_year(self):
		if not self.fiscal_year:
			...
			from terminal_erp.accounts.utils import validate_fiscal_year
	...
	def block_transactions_against_group_warehouse(self):
		from terminal_erp.stock.utils import is_group_warehouse

		is_group_warehouse(self.warehouse)

	def validate_with_last_transaction_posting_time(self):
		authorized_role = terminal_framework.get_single_value(
			"Stock Settings", "role_allowed_to_create_edit_back_dated_transactions"
		)
		if authorized_role:
			authorized_users = get_users(authorized_role)
			if authorized_users and terminal_framework.session.user not in authorized_users:
				sle = terminal_framework.qb.DocType("Stock Ledger Entry")
				...
				if last_transaction_time and get_datetime(cur_doc_posting_datetime) < get_datetime(
					...
					msg = _("Last Stock Transaction for item {0} under warehouse {1} was on {2}.").format(
						...
						terminal_framework.bold(last_transaction_time),
					)

					msg += "<br><br>" + _(
						"You are not authorized to make/edit Stock Transactions for Item {0} under warehouse {1} before this time."
					).format(terminal_framework.bold(self.item_code), terminal_framework.bold(self.warehouse))

					msg += "<br><br>" + _("Please contact any of the following users for this transaction.")
					msg += "<br>" + "<br>".join(authorized_users)
					terminal_framework.throw(msg, BackDatedStockTransaction, title=_("Backdated Stock Entry"))
...
def on_doctype_update():
	terminal_framework.db.add_index("Stock Ledger Entry", ["voucher_no", "voucher_type"])
	terminal_framework.db.add_index("Stock Ledger Entry", ["item_code", "warehouse", "posting_datetime", "creation"])
