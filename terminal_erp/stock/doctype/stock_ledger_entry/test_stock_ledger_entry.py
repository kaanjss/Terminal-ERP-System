...
from terminal_framework.core.page.permission_manager.permission_manager import reset
from terminal_framework.query_builder.functions import CombineDatetime
from terminal_framework.utils import add_days, add_to_date, flt, today
...
from terminal_erp.stock.doctype.stock_ledger_entry.stock_ledger_entry import BackDatedStockTransaction
...
class TestStockLedgerEntry(Terminal ERPTestSuite, StockTestMixin):
	def setUp(self):
		create_items()
		reset("Stock Entry")
	...
	def test_incoming_value_for_transferred_serial_no_is_deterministic(self):
		...
		from terminal_erp.stock.stock_ledger import update_entries_after

		item = "_Test Serialized Item"
		serial = "_Test SN Tie 9"
		company_a, company_b = "_Test Company", "_Test Company 1"
		if terminal_framework.db.exists("Serial No", serial):
			terminal_framework.delete_doc("Serial No", serial, force=1)
		terminal_framework.get_doc(
			{"doctype": "Serial No", "serial_no": serial, "item_code": item, "company": company_b}
		).insert(ignore_permissions=True)

		def mk_sle(name, rate):
			if terminal_framework.db.exists("Stock Ledger Entry", name):
				terminal_framework.delete_doc("Stock Ledger Entry", name, force=1)
		...
		value = update_entries_after.get_incoming_value_for_serial_nos(
			terminal_framework._dict(company=company_a), [serial]
		)
	...
	def test_item_cost_reposting(self):
		...
		target_wh_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": "_Test Item for Reposting",
				"warehouse": "Finished Goods - _TC",
		...
		finished_item_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_back_dated_entry_not_allowed(self):
		...
		try:
			user = terminal_framework.get_doc("User", "test@example.com")
	...
	def test_fifo_dependent_consumption(self):
		...
		for rate in rates[1:]:
			row = terminal_framework.copy_doc(receipt.items[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		for _ in rates[1:]:
			row = terminal_framework.copy_doc(transfer.items[0], ignore_no_copy=False)
	...
	def test_fifo_multi_item_repack_consumption(self):
		...
		for _ in rates[1:]:
			row = terminal_framework.copy_doc(repack.items[0], ignore_no_copy=False)
	...
	def test_tie_breaking(self):
		from terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation import repost_entries

		terminal_framework.flags.dont_execute_stock_reposts = True
		self.addCleanup(terminal_framework.flags.pop, "dont_execute_stock_reposts")
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"float_precision": 3, "currency_precision": 2})
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"float_precision": 4})
...
def create_repack_entry(**args):
	...
	repack = terminal_framework.new_doc("Stock Entry")
...
def create_product_bundle_item(new_item_code, packed_items):
	...
	if not get_active_product_bundle(new_item_code):
		item = terminal_framework.new_doc("Product Bundle")
