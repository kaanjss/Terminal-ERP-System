...
from terminal_framework.desk.notifications import clear_doctype_notifications
from terminal_framework.model.document import Document
from terminal_framework.utils import cint, flt, getdate, nowdate
...
from terminal_erp.stock.doctype.purchase_receipt.services.billing_status import BillingStatusService
from terminal_erp.stock.doctype.purchase_receipt.services.provisional_accounting import (
	ProvisionalAccountingService,
)
from terminal_erp.stock.doctype.purchase_receipt.services.reservation import (
...
class PurchaseReceipt(BuyingController):
		...
		if (
			...
			for d in self.get("items"):
				if not d.purchase_order:
					terminal_framework.throw(_("Purchase Order number required for Item {0}").format(d.item_code))

	def validate_items_quality_inspection(self):
		for item in self.get("items"):
			if item.quality_inspection:
				qi = terminal_framework.db.get_value(
					"Quality Inspection",
				...
				if qi.reference_type != self.doctype or qi.reference_name != self.name:
					terminal_framework.throw(
						...
						).format(item.idx, terminal_framework.bold(self.doctype), terminal_framework.bold(self.name))
					)
	...
	def get_already_received_qty(self, po, po_detail):
		qty = terminal_framework.get_all(
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = terminal_framework.db.get_value("Purchase Order Item", po_detail, ["qty", "warehouse"])
	...
	def update_received_qty_if_from_pp(self):
		from terminal_framework.query_builder.functions import Coalesce, NullIf, Sum

		items_from_po = [item.purchase_order_item for item in self.items if item.purchase_order_item]
		if items_from_po:
			table = terminal_framework.qb.DocType("Purchase Order Item")
			subquery = (
	...
	def check_next_docstatus(self):
		submit_rv = terminal_framework.get_all(
		...
		if submit_rv:
			terminal_framework.throw(_("Purchase Invoice {0} is already submitted").format(submit_rv[0][0]))
	...
	def on_cancel(self):
		...
		submitted = terminal_framework.get_all(
			"Purchase Invoice Item",
			filters={"purchase_receipt": self.name, "docstatus": 1},
			fields=["parent"],
			as_list=True,
		...
		if submitted:
			terminal_framework.throw(_("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
	...
	def get_gl_entries(self, inventory_account_map=None, via_landed_cost_voucher=False):
		from terminal_erp.stock.doctype.purchase_receipt.services.gl_composer import (
	...
	def update_assets(self, item, valuation_rate):
		assets = terminal_framework.db.get_all(
			"Asset",
			filters={
	...
	def enable_recalculate_rate_in_sles(self):
		rejected_warehouses = terminal_framework.get_all(
...
def update_purchase_receipt_status(docname: str, status: str):
	pr = terminal_framework.get_lazy_doc("Purchase Receipt", docname, check_permission="submit")
...
def make_lcv(doctype: str, docname: str):
	landed_cost_voucher = terminal_framework.new_doc("Landed Cost Voucher")

	details = terminal_framework.db.get_value(doctype, docname, ["supplier", "company", "base_grand_total"], as_dict=1)
