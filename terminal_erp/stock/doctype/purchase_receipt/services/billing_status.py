...
class BillingStatusService:
	...
	def update_billing_status(self, update_modified: bool = True) -> None:
		...
		for pr in set(updated_pr):
			pr_doc = doc if (pr == doc.name) else terminal_framework.get_lazy_doc("Purchase Receipt", pr)
...
def get_purchase_receipts_against_po_details(po_details: list) -> list[dict]:
	...
	purchase_receipt = terminal_framework.qb.DocType("Purchase Receipt")
	purchase_receipt_item = terminal_framework.qb.DocType("Purchase Receipt Item")

	query = (
		terminal_framework.qb.from_(purchase_receipt)
...
def update_billing_percentage(
	...
	over_billing_allowance, role_allowed_to_over_bill = terminal_framework.get_single_value(
	...
	billed_qty_amt = terminal_framework._dict()
	...
	for item in pr_doc.items:
		...
		if adjust_incoming_rate:
			...
			if (
				...
				and role_allowed_to_over_bill not in terminal_framework.get_roles()
			):
				terminal_framework.throw(
					_("Over Billing Allowance exceeded for Purchase Receipt Item {0} ({1}) by {2}%").format(
						item.name, terminal_framework.bold(item.item_code), per_over_billed - over_billing_allowance
					)
				)
...
def get_billed_qty_amount_against_purchase_receipt(pr_doc) -> dict:
	...
	parent_table = terminal_framework.qb.DocType("Purchase Invoice")
	table = terminal_framework.qb.DocType("Purchase Invoice Item")
	query = (
		terminal_framework.qb.from_(parent_table)
		.inner_join(table)
...
def get_billed_qty_amount_against_purchase_order(pr_doc) -> dict:
	...
	invoice_data_po_based = terminal_framework._dict()
	if po_names:
		parent_table = terminal_framework.qb.DocType("Purchase Invoice")
		table = terminal_framework.qb.DocType("Purchase Invoice Item")
