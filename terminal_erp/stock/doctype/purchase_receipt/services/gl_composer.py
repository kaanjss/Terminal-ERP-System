...
class PurchaseReceiptGLComposer(BaseStockGLComposer):
	...
	def compose(
		...
		from terminal_erp.stock.doctype.purchase_receipt.purchase_receipt import update_regional_gl_entries

		update_regional_gl_entries(gl_entries, self.doc)

		return process_gl_map(gl_entries, from_repost=terminal_framework.flags.through_repost_item_valuation)
	...
	def _make_item_gl_entries(self, gl_entries: list, inventory_account_map: dict | None) -> None:
		...
		from terminal_erp.stock.doctype.purchase_receipt.purchase_receipt import get_stock_value_difference

		doc = self.doc
		provisional_accounting_for_non_stock_items = cint(
			terminal_framework.db.get_value("Company", doc.company, "enable_provisional_accounting_for_non_stock_items")
		)
		...
		def validate_account(account_type):
			terminal_framework.throw(_("{0} account not found while submitting purchase receipt").format(account_type))
		...
		def make_stock_received_but_not_billed_entry(item):
			if (
				...
				and not terminal_framework.db.get_single_value(
			...
			if item.get("rejected_qty") and terminal_framework.db.get_single_value(
				"Buying Settings", "set_valuation_rate_for_rejected_materials"
			):
		...
		def make_divisional_loss_gl_entry(item, outgoing_amount):
			...
			if divisional_loss:
				...
				cost_center = item.cost_center or terminal_framework.get_cached_value(
		...
		for d in doc.get("items"):
			...
			if (
				...
				if not (
					(terminal_erp.is_perpetual_inventory_enabled(doc.company) and d.item_code in stock_items)
				...
				not terminal_framework.db.get_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials")
			...
			if d.rejected_qty and terminal_framework.db.get_single_value(
				"Buying Settings", "set_valuation_rate_for_rejected_materials"
			):
				stock_asset_rbnb = (
					doc.get_company_default("asset_received_but_not_billed")
