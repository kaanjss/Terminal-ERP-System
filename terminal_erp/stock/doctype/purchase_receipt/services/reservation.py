...
from terminal_framework.utils import get_datetime
...
class PurchaseReceiptStockReservation:
	...
	def reserve_stock_for_sales_order(self) -> None:
		...
		if (
			doc.is_return
			or not terminal_framework.get_single_value("Stock Settings", "enable_stock_reservation")
			or not terminal_framework.get_single_value("Stock Settings", "auto_reserve_stock_for_sales_order_on_purchase")
		):
			return
		...
		if so_items_details_map:
			...
			for so, items_details in so_items_details_map.items():
				so_doc = terminal_framework.get_lazy_doc("Sales Order", so)
	...
	def reserve_stock_for_production_plan(self) -> None:
		doc = self.doc
		if doc.is_return or not terminal_framework.get_single_value("Stock Settings", "enable_stock_reservation"):
			return
	...
	def get_production_plan_references(self) -> terminal_framework._dict:
		production_plan_references = terminal_framework._dict()
