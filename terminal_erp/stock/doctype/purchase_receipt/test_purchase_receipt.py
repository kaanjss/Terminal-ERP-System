...
from terminal_framework.utils import add_days, cint, cstr, flt, get_datetime, getdate, nowtime, today
from pypika import functions as fn

import terminal_erp
import terminal_erp.controllers
import terminal_erp.controllers.status_updater
...
from terminal_erp.controllers.buying_controller import QtyMismatchError
from terminal_erp.stock import get_warehouse_account_map
from terminal_erp.stock.doctype.item.test_item import create_item, make_item
from terminal_erp.stock.doctype.material_request.mapper import make_purchase_order
from terminal_erp.stock.doctype.purchase_receipt.mapper import make_purchase_invoice
...
class TestPurchaseReceipt(Terminal ERPTestSuite):
	def setUp(self):
		terminal_framework.local.future_sle = {}
	...
	def test_purchase_receipt_skips_validation(self):
		...
		mr = terminal_framework.new_doc("Material Request")
		...
		mr.insert()
		mr.submit()
		terminal_framework.db.set_single_value("Buying Settings", "over_order_allowance", 200)
		po = make_purchase_order(mr.name)
		po.supplier = "_Test Supplier"
		po.items[0].qty = 300
		po.save()
		po.submit()
		terminal_framework.db.set_single_value("Buying Settings", "over_order_allowance", 0)
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry = terminal_framework.db.get_all(
			"Stock Ledger Entry",
		...
		sl_entry_cancelled = terminal_framework.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import create_payment_term
		...
		template = terminal_framework.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		terminal_framework.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(terminal_framework.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(terminal_framework.ValidationError, terminal_framework.get_doc(pi).submit)
	...
	def test_purchase_receipt_no_gl_entry(self):
		from terminal_erp.stock.doctype.stock_entry.test_stock_entry import make_stock_entry

		existing_bin_qty, existing_bin_stock_value = terminal_framework.db.get_value(
			"Bin",
		...
		stock_value_difference = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = terminal_framework.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = terminal_framework.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = terminal_framework.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(terminal_framework.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
	...
	def test_duplicate_serial_nos(self):
		...
		item = terminal_framework.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = terminal_framework.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		bundle_id = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(terminal_erp.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_serial_no_warehouse(self):
		...
		self.assertEqual(
			terminal_framework.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"), pr.get("items")[0].warehouse
		)

		pr.cancel()
		self.assertFalse(terminal_framework.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))
	...
	def test_rejected_warehouse_filter(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pr.save)
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				terminal_framework.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
		...
		for serial_no in rejected_serial_nos:
			self.assertEqual(
				terminal_framework.db.get_value("Serial No", serial_no, "warehouse"),
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		returned = terminal_framework.get_doc("Purchase Receipt", return_pr.name)
	...
	def test_purchase_return_for_rejected_qty(self):
		from terminal_erp.stock.doctype.warehouse.test_warehouse import get_warehouse

		rejected_warehouse = "_Test Rejected Warehouse - TCP1"
		if not terminal_framework.db.exists("Warehouse", rejected_warehouse):
			get_warehouse(
				company="_Test Company with perpetual inventory",
				abbr=" - TCP1",
		...
		actual_qty = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = terminal_framework.get_doc("Serial No", serial_no)
	...
	def test_closed_purchase_receipt(self):
		from terminal_erp.stock.doctype.purchase_receipt.purchase_receipt import (
		...
		self.assertEqual(terminal_framework.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_pr_billing_status(self):
		...
		3. PO -> PR2.
		"""
		from terminal_erp.buying.doctype.purchase_order.mapper import (
			make_purchase_invoice as make_purchase_invoice_from_po,
		)
		from terminal_erp.buying.doctype.purchase_order.mapper import make_purchase_receipt
		from terminal_erp.buying.doctype.purchase_order.test_purchase_order import create_purchase_order

		terminal_framework.flags.print_test_messages = False
		# Qty: 10, Rate: 500
		...
		terminal_framework.flags.print_test_messages = True
	...
	def test_serial_no_against_purchase_receipt(self):
		...
		if not terminal_framework.db.exists("Serial No", serial_no[0]):
			terminal_framework.get_doc(
				{
					"doctype": "Serial No",
					"item_code": item_code,
		...
		voucher_no = terminal_framework.db.get_value("Serial and Batch Bundle", bundle_id, "voucher_no")
	...
	def test_auto_asset_creation(self):
		...
		if not terminal_framework.db.exists("Item", asset_item):
			asset_category = terminal_framework.get_all("Asset Category")
		...
		assets = terminal_framework.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = terminal_framework.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from terminal_erp.stock.doctype.purchase_receipt.mapper import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = terminal_framework.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = terminal_framework.utils.nowdate()
		...
		self.assertRaises(terminal_framework.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not terminal_framework.db.exists("Location", "Test Location"):
			terminal_framework.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		from terminal_erp.buying.doctype.purchase_order.test_purchase_order import (
			create_pr_against_po,
			create_purchase_order,
		)

		terminal_framework.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 0)
	...
	def test_stock_transfer_from_purchase_receipt(self):
		from terminal_erp.stock.doctype.delivery_note.mapper import make_inter_company_purchase_receipt
		from terminal_erp.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
	...
	def test_inter_company_purchase_receipt_does_not_inherit_party_fields(self):
		...
		supplier = terminal_framework.get_doc("Supplier", "_Test Internal Supplier 2")
	...
	def test_lcv_for_internal_transfer(self):
		from terminal_erp.stock.doctype.delivery_note.mapper import make_inter_company_purchase_receipt
		from terminal_erp.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from terminal_erp.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import (
		...
		sle_entries = terminal_framework.get_all(
			"Stock Ledger Entry",
		...
		inward_sabb = terminal_framework.get_all(
		...
		original_cost = terminal_framework.db.get_value("Serial and Batch Bundle", inward_sabb[0], "total_amount")
		...
		new_inward_sabb = terminal_framework.get_all(
			"Serial and Batch Bundle",
			filters={
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		new_cost = terminal_framework.db.get_value("Serial and Batch Bundle", new_inward_sabb[0], "total_amount")
	...
	def test_stock_transfer_from_purchase_receipt_with_valuation(self):
		...
		from terminal_erp.stock.get_item_details import get_valuation_rate
		from terminal_erp.stock.utils import get_stock_balance
	...
	def test_valuation_tax_distribution_with_non_stock_item(self):
		...
		pr = terminal_framework.new_doc("Purchase Receipt")
		pr.company = company
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		from terminal_erp.buying.doctype.purchase_order import test_purchase_order
	...
	def test_purchase_receipt_with_exchange_rate_difference(self):
		from terminal_erp.accounts.doctype.purchase_invoice.mapper import (
		...
		from terminal_erp.accounts.party import add_party_account
		...
		exchange_gain_loss_account = terminal_framework.db.get_value("Company", pr.company, "exchange_gain_loss_account")

		# fetching the latest GL Entry with exchange gain and loss account account
		amount = terminal_framework.db.get_value(
	...
	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"automatically_fetch_payment_terms": 1})
	def test_payment_terms_are_fetched_when_creating_purchase_invoice(self):
		from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import (
		...
		from terminal_erp.selling.doctype.sales_order.test_sales_order import (
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"allow_negative_stock": 1})
	...
	def test_backdated_transaction_for_internal_transfer(self):
		...
		dn_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		pr_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_receipt(
		...
		stk_ledger = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_invoice(
		...
		from terminal_erp.accounts.doctype.sales_invoice.mapper import (
		...
		si_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		pi_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_disable_last_purchase_rate(self):
		from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_item_details
		...
		terminal_framework.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 1)
		...
		terminal_framework.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 0)
	...
	def test_validate_received_qty_for_internal_pr(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pr.submit)

		# Step 5: Test Over Receipt Allowance
		terminal_framework.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 50)
		...
		terminal_framework.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 0)
	...
	def test_internal_pr_gl_entries(self):
		...
		warehouse_account = get_warehouse_account_map(company)
		stock_account_value = terminal_framework.db.get_value(
			"GL Entry",
			{
		...
		stock_diff = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
	...
	def test_return_from_rejected_warehouse(self):
		from terminal_erp.stock.doctype.purchase_receipt.mapper import (
	...
	def test_purchase_receipt_with_backdated_landed_cost_voucher(self):
		...
		available_qty = terminal_framework.db.get_value(
			"Bin",
		...
		sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		qty_after_transaction = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": new_pr.name, "is_cancelled": 0},
	...
	def test_purchase_receipt_provisional_accounting(self):
		...
		amount = terminal_framework.db.get_value("GL Entry", {"docstatus": 1, "voucher_no": pr.name}, [{"SUM": "debit"}])
	...
	def test_use_serial_batch_fields_for_serial_nos(self):
		...
		sbb_doc = terminal_framework.get_doc("Serial and Batch Bundle", pr.items[0].serial_and_batch_bundle)
		...
		serial_no_status = terminal_framework.db.get_value("Serial No", "SNU-TSFISI-000015", "status")
		...
		doc = terminal_framework.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		for row in doc.entries:
			self.assertIn(row.serial_no, new_serial_nos)

		for sn in new_serial_nos:
			serial_no_status = terminal_framework.db.get_value("Serial No", sn, "status")
			self.assertNotEqual(serial_no_status, "Active")
	...
	def test_pr_billed_amount_against_return_entry(self):
		from terminal_erp.accounts.doctype.purchase_invoice.mapper import make_debit_note
		from terminal_erp.stock.doctype.purchase_receipt.mapper import (
	...
	def test_valuation_taxes_lcv_repost_after_billing(self):
		...
		old_perpetual_inventory = terminal_erp.is_perpetual_inventory_enabled("_Test Company")
		terminal_framework.local.enable_perpetual_inventory["_Test Company"] = 1
		...
		terminal_framework.local.enable_perpetual_inventory["_Test Company"] = old_perpetual_inventory
	...
	def test_purchase_receipt_with_use_serial_batch_field_for_rejected_qty(self):
		...
		if not terminal_framework.db.exists("Batch", batch_no):
			terminal_framework.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_no,
		...
		for row in inter_transfer_pr_return.items:
			...
			if row.item_code == serial_item_doc.name:
				...
				for sn in serial_nos:
					serial_no_details = terminal_framework.db.get_value(
	...
	def test_internal_transfer_with_serial_batch_items_without_use_serial_batch_fields(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "use_serial_batch_fields", 0)
	...
	def test_purchase_receipt_bill_for_rejected_quantity_in_purchase_invoice(self):
		...
		return_pr.items[0].purchase_receipt_item = pr.items[0].name
		return_pr.submit()
		old_value = terminal_framework.db.get_single_value(
		...
		terminal_framework.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 1)
		pi = make_purchase_invoice(pr.name)
		pi.submit()
	...
	def test_zero_valuation_rate_for_batched_item(self):
		...
		sabb_doc = terminal_framework.get_doc("Serial and Batch Bundle", se.items[0].serial_and_batch_bundle)
	...
	def test_manufacturing_and_expiry_date_for_batch(self):
		...
		batch_no = get_batch_from_bundle(pr.items[0].serial_and_batch_bundle)
		batch = terminal_framework.get_doc("Batch", batch_no)
	...
	def test_tax_account_heads_on_lcv_and_item_repost(self):
		...
		company_doc = terminal_framework.get_doc("Company", test_company)
		...
		repost_doc = terminal_framework.get_doc(
			{
	...
	def create_lcv(self, receipt_document_type, receipt_document, company, expense_account, charges=50):
		ref_doc = terminal_framework.get_doc(receipt_document_type, receipt_document)

		lcv = terminal_framework.new_doc("Landed Cost Voucher")
		lcv.company = company
	...
	def test_do_not_use_batchwise_valuation_rate(self):
		...
		valuation_rate = terminal_framework.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
	...
	def test_internal_transfer_for_batch_items_with_cancel_use_serial_batch_fields(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		terminal_framework.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 0)
	...
	def test_sles_with_same_posting_datetime_and_creation(self):
		from terminal_erp.stock.doctype.stock_entry.test_stock_entry import make_stock_entry
		from terminal_erp.stock.report.stock_balance.stock_balance import execute
		...
		sle_doc = terminal_framework.get_doc("Stock Ledger Entry", sle)
	...
	def test_purchase_receipt_return_valuation_without_use_serial_batch_field(self):
		...
		for row in return_entry.items:
			if row.item_code == batch_item:
				bundle_data = terminal_framework.get_all(
	...
	def test_do_not_allow_to_inward_same_serial_no_multiple_times(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "allow_existing_serial_no", 0)
		...
		status = terminal_framework.db.get_value("Serial No", serial_no, "status")
		...
		self.assertRaises(terminal_framework.exceptions.ValidationError, pr.submit)
	...
	def test_seral_no_return_validation(self):
		...
		sn_return.save()
		self.assertRaises(terminal_framework.ValidationError, sn_return.submit)
	...
	def test_batch_no_return_validation(self):
		...
		batch_return.save()
		self.assertRaises(terminal_framework.ValidationError, batch_return.submit)
	...
	def test_recreate_stock_ledgers(self):
		...
		for row in sles:
			terminal_framework.db.delete("Stock Ledger Entry", {"name": row})
	...
	def test_validate_recreate_stock_ledgers_for_sn_item(self):
		...
		self.assertRaises(terminal_framework.ValidationError, repost_doc.save)
	...
	def test_internal_pr_qty_change_only_single_batch(self):
		...
		def get_sabb_qty(sabb):
			return terminal_framework.get_value("Serial and Batch Bundle", sabb, "total_qty")
		...
		self.assertRaises(terminal_erp.controllers.status_updater.OverAllowanceError, pr.submit)
	...
	def test_valuation_rate_for_rejected_materials(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials", 1)
		...
		stock_received_but_not_billed_account = terminal_framework.get_value(
			"Company",
			company,
			"stock_received_but_not_billed",
		)

		rejected_item_cost = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		srbnb_cost = terminal_framework.db.get_value(
			"GL Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		terminal_framework.db.set_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials", 0)
	...
	def test_valuation_rate_for_rejected_materials_withoout_accepted_materials(self):
		...
		gl_entry = terminal_framework.get_all(
			"GL Entry", filters={"debit": (">", 0), "voucher_no": pr.name}, pluck="name"
		)

		stock_value_diff = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_expense_account(self):
		...
		if not terminal_framework.db.exists("Account", expense_account):
			terminal_framework.get_doc(
				{
					"doctype": "Account",
		...
		if not terminal_framework.db.exists("Account", expense_contra_account):
			terminal_framework.get_doc(
				{
					"doctype": "Account",
	...
	def test_repost_gl_entries(self):
		...
		if not terminal_framework.db.exists("Account", account):
			terminal_framework.get_doc(
				{
					"doctype": "Account",
	...
	def test_multiple_transactions_with_same_posting_datetime(self):
		from terminal_erp.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from terminal_erp.stock.stock_ledger import NegativeStockError
	...
	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"over_billing_allowance": 100})
	def test_set_lcv_from_pi_created_against_po(self):
		...
		pr.items[0].qty = 5
		item = terminal_framework.copy_doc(pr.items[0])
		item.qty = 2
	...
	def test_purchase_return_with_and_without_return_against_rejected_qty(self):
		...
		company = terminal_framework.db.get_value("Warehouse", warehouse, "company")
	...
	def test_do_not_use_batchwise_valuation_with_fifo(self):
		...
		doc = terminal_framework.new_doc("Batch")
		doc.update(
			{
		...
		stock_queue = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": item_code,
				"warehouse": warehouse,
		...
		stock_queue = terminal_framework.parse_json(stock_queue)
		...
		ste_details = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": item_code,
				"warehouse": warehouse,
		...
		stock_queue = terminal_framework.parse_json(ste_details.stock_queue)
		...
		self.assertEqual(terminal_framework.parse_json(stock_queue), [[20, 0.0]])
	...
	def test_negative_stock_error_for_purchase_return(self):
		...
		self.assertRaises(terminal_framework.ValidationError, return_pr.submit)
	...
	def test_internal_purchase_receipt_incoming_rate_with_lcv(self):
		...
		repost_doc = terminal_framework.new_doc("Repost Item Valuation")
		repost_doc.update(
			{
				"based_on": "Transaction",
	...
	def test_purchase_return_from_different_warehouse(self):
		...
		sabb = terminal_framework.get_doc("Serial and Batch Bundle", make_purchase_entry.items[0].serial_and_batch_bundle)
	...
	def test_different_exchange_rate_in_pr_and_pi(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 1)
		...
		pr = make_purchase_receipt(
			...
			rate=100,
			company="_Test Company with perpetual inventory",
			warehouse=terminal_framework.get_value(
	...
	def test_purchase_receipt_gl_entries_for_asset_item(self):
		from terminal_erp.assets.doctype.asset.test_asset import create_fixed_asset_item

		# Create a Company without Stock Accounts Linked.
		company = terminal_framework.get_doc(
			{
				"doctype": "Company",
		...
		for account in stock_accounts:
			terminal_framework.db.delete("Account", account)
		...
		arnb_account = terminal_framework.db.get_value("Company", company.name, "asset_received_but_not_billed")
		...
		for entry in gl_entries:
			account_type = terminal_framework.db.get_value("Account", entry.account, "account_type")
	...
	def test_purchase_receipt_gl_entries_with_mixed_asset_and_stock_items(self):
		...
		pr.append(
			"items",
			{
				...
				"stock_uom": "_Test UOM",
				"conversion_factor": 1.0,
				"cost_center": terminal_framework.get_cached_value("Company", pr.company, "cost_center"),
			},
		)
		pr.insert()
		pr.submit()
	...
	def test_check_next_docstatus_blocks_with_submitted_invoice(self):
		...
		with self.assertRaises(terminal_framework.ValidationError) as cm:
...
def prepare_data_for_internal_transfer():
	from terminal_erp.accounts.doctype.sales_invoice.test_sales_invoice import create_internal_supplier
	...
	if not terminal_framework.db.get_value("Company", company, "unrealized_profit_loss_account"):
		...
		terminal_framework.db.set_value("Company", company, "unrealized_profit_loss_account", account)
...
def get_gl_entries(voucher_type, voucher_no, skip_cancelled=False, as_dict=True):
	...
	gl_query = (
		...
		.orderby(gl.account, order=terminal_framework.qb.desc)
	)
	if skip_cancelled:
...
def make_purchase_receipt(**args):
	...
	terminal_framework.db.set_single_value("Buying Settings", "allow_multiple_items", 1)
	pr = terminal_framework.new_doc("Purchase Receipt")
	args = terminal_framework._dict(args)
	...
	uom = args.uom or terminal_framework.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"
	...
	pr.append(
		"items",
		{
			...
			"uom": uom,
			"cost_center": args.cost_center or terminal_framework.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = terminal_framework.get_cached_value("Company", pr.company, "cost_center")
