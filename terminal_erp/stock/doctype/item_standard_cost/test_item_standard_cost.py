...
from terminal_framework.utils import add_days, flt, today
...
def create_item_standard_cost(item_code, rate, company=TEST_COMPANY, effective_date=None, submit=True):
	doc = terminal_framework.new_doc("Item Standard Cost")
...
def ensure_ppv_account(company):
	...
	expense_account = terminal_framework.get_cached_value("Company", company, "default_expense_account")
	parent_account = terminal_framework.db.get_value("Account", expense_account, "parent_account")
	...
	terminal_framework.db.set_value("Company", company, "default_purchase_price_variance_account", account)
	return account


class TestItemStandardCost(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_only_for_standard_cost_items(self):
		...
		isc = terminal_framework.new_doc("Item Standard Cost")
		isc.item_code = item.name
		isc.company = TEST_COMPANY
		isc.standard_rate = 100
		self.assertRaises(terminal_framework.ValidationError, isc.insert)

	def test_item_link_query_lists_only_standard_cost_items(self):
		from terminal_erp.stock.doctype.item_standard_cost.item_standard_cost import get_standard_cost_items
	...
	def test_first_record_requires_no_stock_ledger_entry(self):
		...
		terminal_framework.db.set_value("Item", item.name, "valuation_method", "Standard Cost")
		terminal_framework.local.request_cache.clear()
	...
	def test_rate_change_revalues_on_hand_stock(self):
		...
		reco_status = terminal_framework.db.get_value("Stock Reconciliation", isc.revaluation_entry, "docstatus")
		self.assertEqual(reco_status, 1)

		stock_value = terminal_framework.db.get_value(
	...
	def test_backdated_entry_fast_qty_repost(self):
		...
		bin_data = terminal_framework.db.get_value(
			"Bin",
		...
		self.assertFalse(terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": se0.name}))

	def test_cannot_cancel(self):
		item = create_standard_cost_item()
		isc = create_item_standard_cost(item.name, rate=100)
		self.assertRaises(terminal_framework.ValidationError, isc.cancel)
	...
	def test_manufacturing_variance_books_to_stock_adjustment(self):
		...
		fg_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		stock_adj = terminal_framework.get_cached_value("Company", PI_COMPANY, "stock_adjustment_account")
		net = terminal_framework.db.sql(
	...
	def test_pr_stock_value_excludes_rejected_warehouse(self):
		...
		def pr_value(stock_qty):
			mock_item = terminal_framework._dict(
	...
	def test_pr_books_variance_to_ppv_account(self):
		...
		cogs_account = terminal_framework.get_cached_value("Company", PI_COMPANY, "default_expense_account")
	...
	def test_pr_throws_without_ppv_account(self):
		...
		previous = terminal_framework.get_cached_value("Company", PI_COMPANY, "default_purchase_price_variance_account")
		terminal_framework.db.set_value("Company", PI_COMPANY, "default_purchase_price_variance_account", None)
		terminal_framework.clear_cache(doctype="Company")
		try:
			item = create_standard_cost_item()
			create_item_standard_cost(item.name, rate=130, company=PI_COMPANY)
			self.assertRaises(
			...
			terminal_framework.db.set_value("Company", PI_COMPANY, "default_purchase_price_variance_account", previous)
			terminal_framework.clear_cache(doctype="Company")
	...
	def test_revaluation_posted_after_same_day_movement(self):
		...
		reco_time = terminal_framework.db.get_value("Stock Reconciliation", isc.revaluation_entry, "posting_time")
		se_time = terminal_framework.db.get_value(
