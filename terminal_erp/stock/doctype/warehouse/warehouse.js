...
terminal_framework.ui.form.on("Warehouse", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			...
			if ("Stock Balance" in terminal_framework.boot.user.all_reports) {
				frm.add_custom_button(
					__("Stock Balance"),
					function () {
		...
		if (
			...
			"General Ledger" in terminal_framework.boot.user.all_reports
		) {
			frm.add_custom_button(
...
function convert_to_group_or_ledger(frm) {
	terminal_framework.call({
		method: "terminal_erp.stock.doctype.warehouse.warehouse.convert_to_group_or_ledger",
		args: {
