...
from terminal_erp.stock import get_warehouse_account
...
class Warehouse(NestedSet):
	...
	def autoname(self):
		if self.company:
			suffix = " - " + terminal_framework.get_cached_value("Company", self.company, "abbr")
	...
	def onload(self):
		if self.company and cint(terminal_framework.db.get_value("Company", self.company, "enable_perpetual_inventory")):
	...
	def on_trash(self):
		# delete bin
		bins = terminal_framework.get_all("Bin", fields="*", filters={"warehouse": self.name})
		for d in bins:
			if (
				d["actual_qty"]
				or d["reserved_qty"]
		...
		terminal_framework.db.delete("Bin", filters={"warehouse": self.name})
		self.update_nsm_model()
	...
	def warn_about_multiple_warehouse_account(self):
		...
		if not terminal_framework.db.count("Stock Ledger Entry", {"warehouse": self.name}):
			return
	...
	def check_if_sle_exists(self, non_cancelled_only=False):
		...
		return terminal_framework.db.exists("Stock Ledger Entry", filters)

	def check_if_child_exists(self):
		return terminal_framework.db.exists("Warehouse", {"parent_warehouse": self.name})
	...
	def convert_to_ledger(self):
		if self.check_if_child_exists():
			terminal_framework.throw(_("Warehouses with child nodes cannot be converted to ledger"))
	...
	def unlink_from_items(self):
		terminal_framework.db.set_value("Item Default", {"default_warehouse": self.name}, "default_warehouse", None)
...
def get_children(
	...
	return terminal_framework.get_list(doctype, fields=fields, filters=filters, order_by="name")
...
def add_node():
	...
	args = make_tree_args(**terminal_framework.form_dict)
...
def convert_to_group_or_ledger(docname: str | None = None):
	if not docname:
		docname = terminal_framework.form_dict.docname
	return terminal_framework.get_doc("Warehouse", docname).convert_to_group_or_ledger()
...
def get_warehouses_based_on_account(account, company=None):
	...
	if (
		not warehouses
		and company
		and terminal_framework.get_cached_value("Company", company, "default_inventory_account") == account
	):
		warehouses = [d.name for d in terminal_framework.get_all("Warehouse", filters={"is_group": 0})]

	if not warehouses:
		terminal_framework.throw(_("Warehouse not found against the account {0}").format(account))

	return warehouses


# Will be use for terminal_framework.qb
def apply_warehouse_filter(query, sle, filters):
	if not (warehouses := filters.get("warehouse")):
		return query

	warehouse_table = terminal_framework.qb.DocType("Warehouse")

	if isinstance(warehouses, str):
		warehouses = [warehouses]

	warehouse_range = terminal_framework.get_all(
		"Warehouse",
		filters={
			"name": ("in", warehouses),
		},
	...
	child_query = terminal_framework.qb.from_(warehouse_table).select(warehouse_table.name)
...
def get_warehouses_for_reorder(
	...
	filters = terminal_framework._dict(filters or {})

	if filters.warehouse and not terminal_framework.db.exists("Warehouse", filters.warehouse):
		terminal_framework.throw(_("Warehouse {0} does not exist").format(filters.warehouse))

	doctype = terminal_framework.qb.DocType("Warehouse")
