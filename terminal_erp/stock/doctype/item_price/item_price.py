...
from terminal_framework.query_builder.functions import Cast_


class ItemPriceDuplicateItem(terminal_framework.ValidationError):
	pass
...
class ItemPrice(Document):
	...
	def validate_item(self):
		if not terminal_framework.db.exists("Item", self.item_code):
			terminal_framework.throw(_("Item {0} not found.").format(self.item_code))

		if self.uom and not terminal_framework.db.exists(
			"UOM Conversion Detail", {"parenttype": "Item", "parent": self.item_code, "uom": self.uom}
		):
			terminal_framework.throw(_("UOM {0} not found in Item {1}").format(self.uom, self.item_code))

	def update_price_list_details(self):
		if self.price_list:
			price_list_details = terminal_framework.db.get_value(
				"Price List", {"name": self.price_list, "enabled": 1}, ["buying", "selling", "currency"]
			)

			if not price_list_details:
				link = terminal_framework.utils.get_link_to_form("Price List", self.price_list)
				terminal_framework.throw(_("The price list {0} does not exist or is disabled").format(link))

			self.buying, self.selling, self.currency = price_list_details

	def update_item_details(self):
		if self.item_code:
			self.item_name, self.item_description = terminal_framework.db.get_value(
				"Item", self.item_code, ["item_name", "description"]
			)

	def validate_item_template(self):
		if terminal_framework.get_cached_value("Item", self.item_code, "has_variants"):
	...
	def check_duplicates(self):
		...
		query = (
			terminal_framework.qb.from_(item_price)
