...
from terminal_erp.stock.doctype.item_price.item_price import ItemPriceDuplicateItem
from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_price_list_rate_for
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestItemPrice(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_duplicate_item(self):
		doc = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][0])
		self.assertRaises(ItemPriceDuplicateItem, doc.save)

	def test_addition_of_new_fields(self):
		# Based on https://github.com/terminal_framework/terminal_erp/issues/8456
		...
		doc_fields = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][1]).__dict__.keys()

		for test_field in test_fields_existance:
			self.assertIn(test_field, doc_fields)

	def test_dates_validation_error(self):
		doc = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][1])
	...
	def test_price_in_a_qty(self):
		...
		doc = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][2])
	...
	def test_prices_at_invalid_date(self):
		...
		doc = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][3])
	...
	def test_prices_outside_of_date(self):
		...
		doc = terminal_framework.copy_doc(self.globalTestRecords["Item Price"][4])
