...
terminal_framework.ui.form.on("Shipment", {
	...
	onload: function (frm) {
		frm.set_query("delivery_address_name", () => {
			let delivery_to = `delivery_${terminal_framework.model.scrub(frm.doc.delivery_to_type)}`;
		...
		frm.set_query("pickup_address_name", () => {
			let pickup_from = `pickup_${terminal_framework.model.scrub(frm.doc.pickup_from_type)}`;
			return frm.events.address_query(
				frm,
	...
	pickup_from_type: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			frm.set_value("pickup_company", terminal_framework.defaults.get_default("company"));
	...
	delivery_to_type: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			frm.set_value("delivery_company", terminal_framework.defaults.get_default("company"));
	...
	delivery_address_name: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			terminal_erp.utils.get_address_display(frm, "delivery_address_name", "delivery_address", true);
		} else {
			terminal_erp.utils.get_address_display(frm, "delivery_address_name", "delivery_address", false);
		}
	},
	pickup_address_name: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			terminal_erp.utils.get_address_display(frm, "pickup_address_name", "pickup_address", true);
		} else {
			terminal_erp.utils.get_address_display(frm, "pickup_address_name", "pickup_address", false);
		}
	},
	get_contact_display: function (frm, contact_name, contact_type) {
		terminal_framework.call({
			method: "terminal_framework.contacts.doctype.contact.contact.get_contact_details",
	...
	pickup_contact_person: function (frm) {
		if (frm.doc.pickup_contact_person) {
			terminal_framework.call({
				method: "terminal_erp.stock.doctype.shipment.shipment.get_company_contact",
			...
			if (frm.doc.pickup_from_type === "Company") {
				terminal_framework.call({
					method: "terminal_erp.stock.doctype.shipment.shipment.get_company_contact",
					args: { user: terminal_framework.session.user },
					callback: function ({ message }) {
						const r = message;
						let contact_display = `${r.first_name} ${r.last_name}`;
						if (r.email) {
	...
	set_company_contact: function (frm, delivery_type) {
		terminal_framework.db.get_value(
			"User",
			{ name: terminal_framework.session.user },
				...
				if (!(r.full_name && r.email && (r.phone || r.mobile_no))) {
					...
					terminal_framework.throw(
						__("Full Name, Email or Phone/Mobile of the user are mandatory to continue.") +
							...
							` <a href="/app/user/${terminal_framework.session.user}">${terminal_framework.session.user}</a>`
					);
				}
		...
		frm.set_value("pickup_contact_person", terminal_framework.session.user);
	},
	...
	set_address_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.shipment.shipment.get_address_name",
			args: {
	...
	set_contact_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.shipment.shipment.get_contact_name",
			args: {
				ref_doctype: ref_doctype,
				docname: ref_docname,
			},
	...
	add_template: function (frm) {
		if (frm.doc.parcel_template) {
			terminal_framework.model.with_doc("Shipment Parcel Template", frm.doc.parcel_template, () => {
				let parcel_template = terminal_framework.model.get_doc(
					"Shipment Parcel Template",
					frm.doc.parcel_template
				);
				let row = terminal_framework.model.add_child(frm.doc, "Shipment Parcel", "shipment_parcel");
	...
	pickup_date: function (frm) {
		if (frm.doc.pickup_date < terminal_framework.datetime.get_today()) {
			terminal_framework.throw(__("Pickup Date cannot be before this day"));
		}
	},
...
terminal_framework.ui.form.on("Shipment Delivery Note", {
