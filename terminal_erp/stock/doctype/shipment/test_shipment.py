...
from terminal_erp.stock.doctype.delivery_note.mapper import make_shipment
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestShipment(Terminal ERPTestSuite):
	...
	def test_get_total_weight(self):
		shipment = terminal_framework.new_doc("Shipment")
		shipment.extend(
			"shipment_parcel",
			[
...
def create_test_delivery_note():
	...
	delivery_note = terminal_framework.new_doc("Delivery Note")
...
def get_shipment_customer_contact(customer_name):
	...
	contacts = terminal_framework.get_all("Contact", fields=["name"], filters={"name": customer_name})
	if len(contacts):
		return contacts[0]
	else:
...
def get_shipment_customer_address(customer_name):
	...
	customer_address = terminal_framework.get_all("Address", fields=["name"], filters={"address_title": address_title})
...
def get_shipment_customer():
	...
	customer = terminal_framework.get_all("Customer", fields=["name"], filters={"name": customer_name})
	if len(customer):
		return customer[0]
	else:
...
def get_shipment_company_address(company_name):
	...
	addresses = terminal_framework.get_all("Address", fields=["name"], filters={"address_title": address_title})
	if len(addresses):
		return addresses[0]
	else:
...
def get_shipment_company():
	return terminal_framework.get_doc("Company", "_Test Company")
...
def create_shipment_address(address_title, company_name, postal_code):
	address = terminal_framework.new_doc("Address")
...
def create_customer_contact(fname, lname):
	customer = terminal_framework.new_doc("Contact")
...
def create_material_receipt(item, company):
	...
	stock = terminal_framework.new_doc("Stock Entry")
