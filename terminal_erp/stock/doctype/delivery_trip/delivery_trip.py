...
from terminal_framework.utils import cint, get_datetime, get_link_to_form
...
class DeliveryTrip(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.delivery_stop.delivery_stop import DeliveryStop
	...
	def __init__(self, *args, **kwargs):
		...
		self.default_distance_uom = (
			terminal_framework.db.get_single_value("Global Defaults", "default_distance_unit") or "Meter"
		)
		self.uom_conversion_factor = terminal_framework.db.get_value(
	...
	def validate(self):
		if self._action == "submit" and not self.driver:
			terminal_framework.throw(_("A driver must be set to submit."))
	...
	def validate_stop_addresses(self):
		for stop in self.delivery_stops:
			if not stop.customer_address:
				stop.customer_address = get_address_display(terminal_framework.get_doc("Address", stop.address).as_dict())

	def validate_delivery_note_not_draft(self):
		delivery_notes = list(set(stop.delivery_note for stop in self.delivery_stops if stop.delivery_note))
		draft_delivery_notes = terminal_framework.get_all(
			"Delivery Note",
	...
	def update_delivery_notes(self, delete=False):
		...
		for delivery_note in delivery_notes:
			note_doc = terminal_framework.get_doc("Delivery Note", delivery_note)
		...
		terminal_framework.msgprint(_("Delivery Notes {0} updated").format(", ".join(delivery_notes_updated)))
		...
		for route in route_list:
			...
			if directions:
				...
				for leg in legs:
					...
					stop_delay = terminal_framework.db.get_single_value("Delivery Settings", "stop_delay")
	...
	def form_route_list(self, optimize):
		...
		if not self.driver_address:
			terminal_framework.throw(_("Cannot calculate arrival time as the driver address is missing."))

		home_address = get_address_display(terminal_framework.get_doc("Address", self.driver_address).as_dict())
	...
	def get_directions(self, route, optimize):
		...
		"""
		if not terminal_framework.db.get_single_value("Google Settings", "api_key"):
			terminal_framework.throw(_("Enter API key in Google Settings."))

		import googlemaps

		try:
			maps_client = googlemaps.Client(key=terminal_framework.db.get_single_value("Google Settings", "api_key"))
		except Exception as e:
			terminal_framework.throw(e)
		...
		try:
			...
			terminal_framework.throw(_(str(e)))
...
def get_contact_display(contact: str):
	terminal_framework.has_permission("Contact", "read", doc=contact, throw=True)

	contact_info = terminal_framework.db.get_value(
...
def notify_customers(delivery_trip: str):
	delivery_trip = terminal_framework.get_doc("Delivery Trip", delivery_trip)
	delivery_trip.check_permission()

	context = delivery_trip.as_dict()

	if delivery_trip.driver:
		context.update(terminal_framework.db.get_value("Driver", delivery_trip.driver, "cell_number", as_dict=1))
	...
	for stop in delivery_trip.delivery_stops:
		...
		if contact_info and contact_info.email_id:
			...
			dispatch_template_name = terminal_framework.db.get_single_value("Delivery Settings", "dispatch_template")
			dispatch_template = terminal_framework.get_doc("Email Template", dispatch_template_name)

			terminal_framework.sendmail(
				recipients=contact_info.email_id,
				subject=dispatch_template.subject,
				message=terminal_framework.render_template(dispatch_template.response, context),
	...
	if email_recipients:
		terminal_framework.msgprint(_("Email sent to {0}").format(", ".join(email_recipients)))
		delivery_trip.db_set("email_notification_sent", 1)
	else:
		terminal_framework.msgprint(_("No contacts with email IDs found."))


def get_attachments(delivery_stop):
	if not (
		terminal_framework.db.get_single_value("Delivery Settings", "send_with_attachment")
		and delivery_stop.delivery_note
	):
		return []

	dispatch_attachment = terminal_framework.db.get_single_value("Delivery Settings", "dispatch_attachment")
	attachments = terminal_framework.attach_print(
		"Delivery Note",
...
def get_driver_email(driver: str):
	terminal_framework.has_permission("Driver", "read", doc=driver, throw=True)

	employee = terminal_framework.db.get_value("Driver", driver, "employee")
	email = terminal_framework.db.get_value("Employee", employee, "prefered_email")
	return {"email": email}
