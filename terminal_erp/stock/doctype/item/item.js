...
terminal_framework.provide("terminal_erp.item");
...
terminal_framework.ui.form.on("Item", {
	...
	onload: function (frm) {
		terminal_erp.item.setup_queries(frm);
	...
	toggle_has_serial_batch_fields(frm) {
		let hide_fields = cint(terminal_framework.user_defaults?.enable_serial_and_batch_no_for_item) === 0 ? 1 : 0;
		...
		if (hide_fields) {
			...
			wrapper.find(".go-to-settings").on("click", function () {
				terminal_framework.set_route("Form", "Stock Settings");
			});
		}
	},
	...
	refresh: function (frm) {
		...
		if (terminal_framework.defaults.get_default("item_naming_by") != "Naming Series" || frm.doc.variant_of) {
			frm.toggle_display("naming_series", false);
		} else {
			terminal_erp.toggle_naming_series();
		}

		frm.toggle_display(["standard_rate"], terminal_framework.model.can_create("Item Price"));
		...
		if (frm.doc.is_stock_item) {
			frm.add_custom_button(
				...
				function () {
					...
					terminal_framework.set_route("query-report", "Stock Balance");
				},
				__("View")
			);
			frm.add_custom_button(
			...
			frm.add_custom_button(
				...
				function () {
					...
					terminal_framework.set_route("query-report", "Stock Projected Qty");
				},
				__("View")
			);

			const can_create_stock_entry =
				terminal_framework.model.can_create("Stock Entry") && terminal_framework.model.can_write("Stock Entry");
			...
			if (can_create_stock_entry && !has_existing_stock) {
				frm.add_custom_button(
					...
					() => terminal_erp.item.show_opening_stock_dialog(frm),
					__("Actions")
				);
			}
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(
				__("Show Variants"),
				function () {
					terminal_framework.set_route("List", "Item", { variant_of: frm.doc.name });
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.set_route("Form", "Item Variant Settings");
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.set_route("query-report", "Item Variant Details", { item: frm.doc.name });
				},
				__("View")
			);
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						terminal_erp.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						terminal_erp.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						terminal_erp.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		terminal_erp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			terminal_erp.item.make_dashboard(frm);
		}

		terminal_erp.item.render_item_prices(frm);

		frm.add_custom_button(__("Duplicate"), function () {
			var new_item = terminal_framework.model.copy_doc(frm.doc);
			...
			terminal_framework.set_route("Form", "Item", new_item.name);
		});
	...
	validate: function (frm) {
		terminal_erp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
	...
	page_name: terminal_framework.utils.warn_page_name_change,
...
terminal_framework.ui.form.on("Item Default", {
	form_render: function (frm, cdt, cdn) {
		if (!frm.fields_dict["item_defaults"]) return;

		const row = locals[cdt][cdn];
		if (!row || !row.company) {
			Object.values(virtual_field_map).forEach((vf) => terminal_framework.model.set_value(cdt, cdn, vf, ""));
			return;
		}
		...
		terminal_erp.item.populate_virtual_fields(frm, cdt, cdn, row);
	},
...
terminal_framework.ui.form.on("Item Reorder", {
	...
	warehouse_group(frm, cdt, cdn) {
		...
		if (!row.warehouse_group) {
			terminal_framework.throw(__("Please select the Warehouse first"));
		}
	},
});

terminal_framework.ui.form.on("Item Customer Detail", {
	customer_items_add: function (frm, cdt, cdn) {
		terminal_framework.model.set_value(cdt, cdn, "customer_group", "");
	},
	...
	customer_group: function (frm, cdt, cdn) {
		if (set_customer_group(frm, cdt, cdn)) {
			terminal_framework.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	},
});
...
var set_customer_group = function (frm, cdt, cdn) {
	...
	terminal_framework.model.with_doc("Customer", row.customer_name, function () {
		var customer = terminal_framework.model.get_doc("Customer", row.customer_name);
	...
	let banner_html = `
		<div class="custom-serial-batch-banner ${hiddenClass}">
			<div class="banner-content">
				<span class="banner-icon">${terminal_framework.utils.icon("solid-warning", "lg", "", "padding-bottom:2px")}</span>
...
$.extend(terminal_erp.item, {
			...
			callback: function (r) {
				...
				Object.entries(virtual_field_map).forEach(([real_field, vf_field]) => {
					if (group_defaults[real_field]) {
						terminal_framework.model.set_value(cdt, cdn, vf_field, group_defaults[real_field]);
					} else {
						terminal_framework.model.set_value(cdt, cdn, vf_field, "");
						needs_company_fallback.push(real_field);
					}
				});

				if (!needs_company_fallback.length) {
					setTimeout(() => terminal_erp.item.update_vf_labels(frm, cdn, {}), 50);
					return;
				}
				terminal_framework.call({
					method: "terminal_erp.setup.doctype.item_group.item_group.get_company_resolved_defaults",
					...
					callback: function (cr) {
						...
						needs_company_fallback.forEach((real_field) => {
							...
							terminal_framework.model.set_value(cdt, cdn, virtual_field_map[real_field], val || "—");
						});

						setTimeout(() => terminal_erp.item.update_vf_labels(frm, cdn, from_company), 50);
					},
				});
			},
		});
	...
	setup_queries: function (frm) {
		...
		frm.fields_dict["item_defaults"].grid.get_field("income_account").get_query = function (
			...
			const row = locals[cdt][cdn];
			return {
				query: "terminal_erp.controllers.queries.get_income_account",
				filters: { company: row.company },
			};
		};
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "terminal_erp.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			terminal_framework.require("item-dashboard.bundle.js", function () {
				const section = frm.fields_dict["stock_levels_html"].$wrapper;

				terminal_erp.item.item_dashboard = new terminal_erp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "terminal_erp.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				terminal_erp.item.item_dashboard.refresh();
			});
		}
	},
	...
	render_item_prices: function (frm) {
		...
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.item.item.get_item_prices",
			...
			callback: function (r) {
				...
				const html = terminal_framework.render_template("item_prices", {
					prices,
					has_more,
				...
				container.find(".price-row").on("click", function (e) {
					...
					terminal_framework.set_route("Form", "Item Price", $(this).data("name"));
				});
			},
		});
	},
	...
	show_opening_stock_dialog: function (frm) {
		...
		if (has_serial || has_batch) {
			const default_company = terminal_framework.defaults.get_default("company");
			...
			terminal_framework.new_doc("Stock Reconciliation", null, (doc) => {
				const child = doc.items[0];
				terminal_framework.model.set_value(child.doctype, child.name, "item_code", frm.doc.name);
				if (default_warehouse) {
					terminal_framework.model.set_value(child.doctype, child.name, "warehouse", default_warehouse);
				}
			});
			return;
		}
		...
		const dialog = new terminal_framework.ui.Dialog({
			...
			primary_action: function (values) {
				terminal_framework.call({
					method: "terminal_erp.stock.doctype.item.item.make_opening_stock_entry",
					args: {
						item_code: frm.doc.name,
		...
		dialog.add_custom_action(__("Edit Full Form"), function () {
			...
			terminal_framework.new_doc("Stock Reconciliation", null, (doc) => {
				...
				if (row && row.default_warehouse) {
					terminal_framework.model.set_value(child.doctype, child.name, "warehouse", row.default_warehouse);
				}
			});
	...
	show_modal_for_manufacturers: function (frm) {
		var dialog = new terminal_framework.ui.Dialog({
			fields: [
				{
					fieldtype: "Link",
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			terminal_framework.call({
				method: "terminal_erp.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_fields_from_attribute_values(attr_dict) {
			...
			att_key.forEach((name, i) => {
				...
				fields.push({
					fieldtype: "Data",
					placeholder: "Search",
					fieldname: `search_${terminal_framework.scrub(name)}`,
					onchange: function (e) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new terminal_framework.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				terminal_framework.call({
					method: "terminal_erp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find(".form-column")).find(".terminal_framework-control")).css(
				"margin-bottom",
				"0px"
			);
	...
	show_single_variant_dialog: function (frm) {
			...
			if (!row.disabled) {
				if (row.numeric_values) {
					...
					desc = __("Min Value: {0}, Max Value: {1}, in Increments of: {2}", [
						terminal_framework.format(row.from_range, df, options),
						terminal_framework.format(row.to_range, df, options),
						terminal_framework.format(row.increment, df, options),
					]);
				} else {
					fieldtype = "Data";
					desc = "";
		...
		var d = new terminal_framework.ui.Dialog({
			...
			terminal_framework.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						terminal_framework.msgprint_dialog = terminal_framework.msgprint(
						...
						terminal_framework.msgprint_dialog.hide_on_page_refresh = true;
						terminal_framework.msgprint_dialog.$wrapper.find(".variant-click").on("click", function () {
							d.hide();
						});
					} else {
						d.hide();
						terminal_framework.call({
							method: "terminal_erp.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					terminal_framework.call({
						method: "terminal_erp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
terminal_framework.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			terminal_framework.call({
				method: "terminal_erp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
					stock_uom: frm.doc.stock_uom,
				},
				callback: function (r) {
					if (!r.exc && r.message) {
						terminal_framework.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				},
			});
		}
...
terminal_framework.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, Terminal ERP will make a stock ledger entry for each transaction of this item."
		),
	},
	{
...
function open_form(frm, doctype, child_doctype, parentfield) {
	terminal_framework.model.with_doctype(doctype, () => {
		let new_doc = terminal_framework.model.get_new_doc(doctype);

		let new_child_doc = terminal_framework.model.add_child(new_doc, child_doctype, parentfield);
		...
		terminal_framework.run_serially([
			() => terminal_framework.ui.form.make_quick_entry(doctype, null, null, new_doc),
			() => {
				terminal_framework.flags.ignore_company_party_validation = true;
				terminal_framework.model.trigger("item_code", frm.doc.name, new_child_doc);
			},
		]);
	});
}
