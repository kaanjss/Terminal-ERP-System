...
from terminal_erp.stock.doctype.item.item import (
	DataValidationError,
	InvalidBarcode,
...
def make_item(item_code=None, properties=None, uoms=None, barcode=None):
	if not item_code:
		item_code = terminal_framework.generate_hash(length=16)

	if terminal_framework.db.exists("Item", item_code):
		return terminal_framework.get_doc("Item", item_code)
...
class TestItem(Terminal ERPTestSuite):
	def setUp(self):
		super().setUp()
	...
	def test_merge_rename_runs_duplicate_stock_reco_check(self):
		...
		terminal_framework.rename_doc("Item", "_Test Item Merge Source", "_Test Item Merge Target", merge=True)

		self.assertTrue(terminal_framework.db.exists("Item", "_Test Item Merge Target"))
		self.assertFalse(terminal_framework.db.exists("Item", "_Test Item Merge Source"))

	def get_item(self, idx):
		item_code = self.globalTestRecords["Item"][idx].get("item_code")
		if not terminal_framework.db.exists("Item", item_code):
			item = terminal_framework.copy_doc(self.globalTestRecords["Item"][idx])
			item.insert()
		else:
	...
	def make_bin(self, records):
		for x in records:
			...
			if not terminal_framework.db.exists("Bin", filters):
				qb.from_(bin).delete().where(
					bin.item_code.eq(x.item_code) & bin.warehouse.eq(x.warehouse)
				).run()
				terminal_framework.get_doc(x).insert()
	...
	def test_get_item_details(self):
		...
		currency = terminal_framework.get_cached_value("Company", company, "default_currency")
	...
	def test_get_asset_item_details(self):
		...
		terminal_framework.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		...
		terminal_framework.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", "1")
		details = get_item_details(
			ItemDetailsCtx(
				{
					"item_code": "Macbook Pro",
	...
	def test_item_defaults(self):
		terminal_framework.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		with self.assertRaises(terminal_framework.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_rename_attribute_value_updates_variants(self):
		...
		def restore_test_size_large():
			doc = terminal_framework.get_doc("Item Attribute", "Test Size")
	...
	def test_swapped_attribute_value_renames_update_variants(self):
		terminal_framework.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)
		terminal_framework.delete_doc_if_exists("Item", "_Test Variant Item-S", force=1)
	...
	def test_copy_fields_from_template_to_variants(self):
		terminal_framework.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not terminal_framework.db.get_value(
			...
			item_attribute = terminal_framework.get_doc("Item Attribute", "Test Size")
		...
		variant = terminal_framework.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in terminal_framework.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			terminal_framework.delete_doc_if_exists("Item", d.name)

		terminal_framework.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		terminal_framework.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(terminal_framework.generate_hash(length=20)).name
		new = create_item(terminal_framework.generate_hash(length=20)).name
		...
		terminal_framework.rename_doc("Item", old, new, merge=True)

		self.assertFalse(terminal_framework.db.exists("Item", old))

		self.assertTrue(terminal_framework.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"}))
		self.assertTrue(
			terminal_framework.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			terminal_framework.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.cancel()
		bundle1.delete()
		terminal_framework.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(terminal_framework.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if terminal_framework.db.exists("Item", "Test Item UOM"):
			terminal_framework.delete_doc("Item", "Test Item UOM")
	...
	def test_item_variant_by_manufacturer(self):
		...
		for manufacturer in ["DFSS", "DASA", "ASAAS"]:
			if not terminal_framework.db.exists("Manufacturer", manufacturer):
				m_doc = terminal_framework.new_doc("Manufacturer")
				m_doc.short_name = manufacturer
				m_doc.insert()

		self.assertFalse(terminal_framework.db.exists("Item Manufacturer", {"manufacturer": "DFSS"}))
		variant = get_variant(template, manufacturer="DFSS", manufacturer_part_no="DFSS-123")

		item_manufacturer = terminal_framework.db.exists(
			"Item Manufacturer", {"manufacturer": "DFSS", "item_code": variant.name}
		)
		self.assertTrue(item_manufacturer)

		terminal_framework.delete_doc("Item Manufacturer", item_manufacturer)

	def test_stock_exists_against_template_item(self):
		stock_item = terminal_framework.get_all("Stock Ledger Entry", fields=["item_code"], limit=1)
		if stock_item:
	...
	def test_add_item_barcode(self):
		...
		if terminal_framework.db.exists("Item", item_code):
			terminal_framework.delete_doc("Item", item_code)
		...
		barcodes = terminal_framework.get_all(
		...
		terminal_framework.db.savepoint("dup_barcode")
		self.assertRaises(terminal_framework.UniqueValidationError, item_doc.save)
		terminal_framework.db.rollback(save_point="dup_barcode")  # preserve transaction in postgres
	...
	def test_index_creation(self):
		...
		for column in list(expected_columns):
			if terminal_framework.db.get_column_index("tabItem", column, unique=False) or terminal_framework.db.get_column_index(
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = terminal_framework.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(terminal_framework.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from terminal_erp.stock.stock_balance import update_bin_qty
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", terminal_framework._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(terminal_framework.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"item_naming_by": "Naming Series"})
	def test_autoname_series(self):
		item = terminal_framework.new_doc("Item")
	...
	def test_item_dashboard(self):
		from terminal_erp.stock.dashboard.item_dashboard import get_data
	...
	def test_customer_codes_length(self):
		...
		for _row in range(3):
			item.append("customer_items", {"ref_code": terminal_framework.generate_hash("", 120)})
		item.save()
	...
	def test_update_is_stock_item(self):
		...
		pb = terminal_framework.new_doc("Product Bundle")
	...
	def test_search_fields_for_item(self):
		from terminal_erp.controllers.queries import item_query
	...
	def test_group_warehouse_for_reorder_item(self):
		...
		warehouse_doc = terminal_framework.get_doc("Warehouse", warehouse)
		...
		self.assertRaises(terminal_framework.ValidationError, item_doc.save)

	def test_variant_uom_mismatch_throws_error(self):
		terminal_framework.db.set_single_value("Item Variant Settings", "allow_different_uom", 0)

		template_item = terminal_framework.get_doc(
			{
				"doctype": "Item",
	...
	def test_opening_stock_for_serial_batch(self):
		...
		for item_code, properties in items.items():
			...
			sabb_qty = terminal_framework.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "total_qty")
			self.assertEqual(abs(sabb_qty), properties["opening_stock"])


def set_item_variant_settings(fields):
	doc = terminal_framework.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not terminal_framework.db.exists("Item", "_Test Variant Item-S"):
