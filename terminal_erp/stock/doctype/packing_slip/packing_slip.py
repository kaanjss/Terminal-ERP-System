...
class PackingSlip(StatusUpdater):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.packing_slip_item.packing_slip_item import PackingSlipItem
	...
	def validate_delivery_note(self):
		...
		if cint(terminal_framework.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
			terminal_framework.throw(_("A Packing Slip can only be created for a Draft Delivery Note."))

	def validate_case_nos(self):
		"""Validate if case nos overlap. If they do, recommend next case no."""

		if cint(self.from_case_no) <= 0:
			terminal_framework.throw(_("The 'From Package No.' field must not be empty or have a value less than 1."))
		elif not self.to_case_no:
			self.to_case_no = self.from_case_no
		elif cint(self.to_case_no) < cint(self.from_case_no):
			terminal_framework.throw(_("'To Package No.' cannot be less than 'From Package No.'"))
		else:
			ps = terminal_framework.qb.DocType("Packing Slip")
			res = (
	...
	def validate_items(self):
		for item in self.items:
			if item.qty <= 0:
				terminal_framework.throw(_("Row {0}: Qty must be greater than 0.").format(item.idx))
			...
			DocType = terminal_framework.qb.DocType("Delivery Note Item" if item.dn_detail else "Packed Item")
			remaining_qty = terminal_framework.db.get_value(
			...
			if remaining_qty is None:
				...
				terminal_framework.throw(
					_("Row {0}: Qty cannot be greater than {1} for the Item {2}.").format(
						item.idx, terminal_framework.bold(remaining_qty), terminal_framework.bold(item.item_code)
					)
				)
	...
	def set_missing_values(self):
		...
		for item in self.items:
			weight_per_unit, weight_uom = terminal_framework.db.get_value(
...
def item_details(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	return (
		query.where(
			item.name.isin(
				terminal_framework.qb.from_(dn_item).select(dn_item.item_code).where(dn_item.parent == delivery_note)
			)
