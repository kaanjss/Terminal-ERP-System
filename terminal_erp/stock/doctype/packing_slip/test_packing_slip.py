...
class TestPackingSlip(Terminal ERPTestSuite):
	def test_packing_slip(self):
		...
		for item in dn.items:
			if not terminal_framework.db.exists("Product Bundle", {"new_item_code": item.item_code}):
		...
		self.assertRaises(terminal_framework.exceptions.ValidationError, ps3.save)
		...
		dn.load_from_db()
		self.assertRaises(terminal_framework.exceptions.ValidationError, dn.submit)
