...
from terminal_framework.utils import flt, nowtime, today
...
class TestSerialandBatchBundle(Terminal ERPTestSuite):
	...
	def test_old_serial_no_valuation(self):
		...
		if not terminal_framework.db.exists("Serial No", serial_no_id):
	...
	def test_batch_not_belong_to_serial_no(self):
		...
		self.assertRaises(terminal_framework.exceptions.ValidationError, doc.save)
	...
	def test_auto_delete_draft_serial_and_batch_bundle(self):
		...
		ste.delete()
		self.assertFalse(terminal_framework.db.exists("Serial and Batch Bundle", bundle_doc.name))
	...
	def test_serial_and_batch_bundle_company(self):
		...
		for serial_no in ["TT-SER-VAL-00001", "TT-SER-VAL-00002", "TT-SER-VAL-00003"]:
			entries.append(terminal_framework._dict({"serial_no": serial_no, "qty": 1}))
	...
	def test_auto_cancel_serial_and_batch(self):
		...
		docstatus = terminal_framework.db.get_value("Serial and Batch Bundle", bundle, "docstatus")
	...
	def test_submitted_bundle_entries_cannot_be_mutated(self):
		...
		bundle = se.items[0].serial_and_batch_bundle
		self.assertEqual(terminal_framework.db.get_value("Serial and Batch Bundle", bundle, "docstatus"), 1)

		original = terminal_framework.db.get_value(
		...
		child_row = terminal_framework._dict({"name": se.items[0].name})
		parent_doc = terminal_framework._dict({"posting_date": today(), "posting_time": nowtime()})
		...
		self.assertEqual(
			terminal_framework.db.get_value("Serial and Batch Entry", original.name, "qty"),
			10,
		)
	...
	def test_batch_duplicate_entry(self):
		...
		self.assertTrue(terminal_framework.db.exists("Batch", batch_id))
		use_batchwise_valuation = terminal_framework.db.get_value("Batch", batch_id, "use_batchwise_valuation")
	...
	def test_serial_no_duplicate_entry(self):
		...
		self.assertTrue(terminal_framework.db.exists("Serial No", serial_no_id))
	...
	def test_duplicate_serial_and_batch_bundle(self):
		...
		self.assertRaises(terminal_framework.exceptions.ValidationError, pr2.save)
	...
	def test_auto_create_serial_and_batch_bundle_for_outward_for_batch_item(self):
		...
		if not terminal_framework.db.exists("Batch", "ACSBBO-TACSB-00001"):
			terminal_framework.get_doc(
				{
					"doctype": "Batch",
		...
		self.assertRaises(terminal_framework.ValidationError, dispatch.submit)
	...
	def test_voucher_detail_no(self):
		...
		if not terminal_framework.db.exists("Batch", "TST-ACSBBO-TACSB-00001"):
			terminal_framework.get_doc(
				{
					"doctype": "Batch",
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"posting_time": nowtime(),
				"qty": 10,
				"batches": terminal_framework._dict({"TST-ACSBBO-TACSB-00001": 10}),
				"type_of_transaction": "Inward",
				"do_not_submit": True,
			}
		)
		...
		bundle_doc = terminal_framework.get_doc("Serial and Batch Bundle", bundle_doc.name)
		...
		self.assertRaises(terminal_framework.ValidationError, bundle_doc.submit)
	...
	def test_reference_voucher_on_cancel(self):
		...
		serial_no = get_serial_nos_from_bundle(se.items[0].serial_and_batch_bundle)[0]
		self.assertEqual(terminal_framework.get_value("Serial No", serial_no, "reference_name"), se.name)

		se.cancel()
		self.assertIsNone(terminal_framework.get_value("Serial No", serial_no, "reference_name"))

		se1 = terminal_framework.copy_doc(se, ignore_no_copy=False)
		se1.items[0].serial_no = serial_no
		se1.submit()

		self.assertEqual(terminal_framework.get_value("Serial No", serial_no, "reference_name"), se1.name)
	...
	def test_stock_queue_for_return_entry_with_non_batchwise_valuation(self):
		...
		return_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
	...
	def test_get_picked_batches_runs(self):
		from terminal_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_picked_batches
		...
		result = get_picked_batches(terminal_framework._dict())
	...
	def _assert_legacy_return_valuation(self, item_code, props, batch_no=None):
		...
		if batch_no and not terminal_framework.db.exists("Batch", batch_no):
			terminal_framework.get_doc({"doctype": "Batch", "batch_id": batch_no, "item": item_code}).insert()
		...
		for row in pr.items:
			if row.serial_and_batch_bundle:
				...
				terminal_framework.db.delete("Serial and Batch Bundle", {"name": row.serial_and_batch_bundle})
				terminal_framework.db.set_value("Purchase Receipt Item", row.name, "serial_and_batch_bundle", None)
		...
		if serial_nos:
			legacy["serial_no"] = "\n".join(serial_nos)
		for sle in terminal_framework.get_all("Stock Ledger Entry", filters={"voucher_no": pr.name}, pluck="name"):
			terminal_framework.db.set_value("Stock Ledger Entry", sle, legacy)
		...
		difference_in_stock_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
...
def get_batch_from_bundle(bundle):
	from terminal_erp.stock.serial_batch_bundle import get_batch_nos
