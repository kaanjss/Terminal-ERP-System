...
import terminal_framework.query_builder
from terminal_framework import _, _dict, bold
from terminal_framework.model.document import Document
from terminal_framework.model.naming import make_autoname
from terminal_framework.query_builder.functions import Concat_ws, Max, Sum
...
from terminal_framework.utils.csvutils import build_csv_response

from terminal_erp.stock.doctype.purchase_receipt_item.purchase_receipt_item import PurchaseReceiptItem
from terminal_erp.stock.serial_batch_bundle import (
	BatchNoValuation,
	SerialNoValuation,
	get_batches_from_bundle,
)
from terminal_erp.stock.serial_batch_bundle import get_serial_nos as get_serial_nos_from_bundle
from terminal_erp.stock.valuation import FIFOValuation


class SerialNoExistsInFutureTransactionError(terminal_framework.ValidationError):
	pass


class BatchNegativeStockError(terminal_framework.ValidationError):
	pass


class SerialNoDuplicateError(terminal_framework.ValidationError):
	pass


class SerialNoWarehouseError(terminal_framework.ValidationError):
	pass
...
class SerialandBatchBundle(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.serial_and_batch_entry.serial_and_batch_entry import SerialandBatchEntry
	...
	def autoname(self):
		if terminal_framework.get_single_value(
			"Stock Settings", "set_serial_and_batch_bundle_naming_based_on_naming_series"
		):
			if not self.naming_series:
				terminal_framework.throw(_("Naming Series is mandatory"))
			...
			try:
				self.name = terminal_framework.generate_hash(length=20)
	...
	def validate(self):
		...
		if self.docstatus == 1:
			if not self.flags.ignore_validate_serial_batch or terminal_framework.in_test:
	...
	def validate_allow_to_set_serial_batch(self):
		if not terminal_framework.db.get_single_value("Stock Settings", "enable_serial_and_batch_no_for_item"):
	...
	def validate_serial_no_status(self):
		...
		invalid_serial_nos = terminal_framework.get_all(
			"Serial No",
			filters={
				"name": ("in", serial_nos),
	...
	def validate_voucher_detail_no(self):
		...
		if self.voucher_type == "POS Invoice":
			if not terminal_framework.db.exists("POS Invoice Item", self.voucher_detail_no):
				terminal_framework.throw(
		...
		elif not terminal_framework.db.exists("Stock Ledger Entry", {"voucher_detail_no": self.voucher_detail_no}):
			if self.voucher_type in ["Delivery Note", "Sales Invoice"] and terminal_framework.db.exists(
	...
	def allow_existing_serial_nos(self):
		...
		if terminal_framework.get_single_value("Stock Settings", "allow_existing_serial_no"):
			return

		if self.voucher_type not in ["Purchase Receipt", "Purchase Invoice", "Stock Entry"]:
			return

		if self.voucher_type == "Stock Entry" and terminal_framework.get_cached_value(
	...
	def set_batch_no(self):
		if self.has_serial_no and self.has_batch_no:
			...
			serial_no_batch = terminal_framework._dict(
	...
	def validate_serial_nos_inventory(self):
		...
		available_serial_nos = get_available_serial_nos(terminal_framework._dict(kwargs))
	...
	def validate_serial_nos_duplicate(self):
		...
		serial_nos = [d.serial_no for d in self.entries if d.serial_no]
		kwargs = terminal_framework._dict(
			{
				"item_code": self.item_code,
				"posting_datetime": self.posting_datetime,
				"serial_nos": serial_nos,
		...
		if (
			...
			and terminal_framework.get_cached_value("Stock Entry", self.voucher_no, "purpose")
			in ["Manufacture", "Repack"]
		):
			serial_nos = terminal_framework.get_all(
	...
	def throw_error_message(self, message, exception=terminal_framework.ValidationError):
		terminal_framework.throw(_(message), exception, title=_("Error"))
	...
	def set_valuation_rate_for_return_entry(self, return_against, row, save=False, prev_sle=None):
		if valuation_details := self.get_valuation_rate_for_return_entry(return_against):
			...
			if not self.has_serial_no and valuation_method == "FIFO":
				non_batchwise_batches = terminal_framework.get_all(
					"Batch",
					filters={
			...
			for row in self.entries:
				...
				if terminal_framework.flags.through_repost_item_valuation and not valuation_rate:
	...
	def validate_returned_serial_batch_no(self, return_against, row, original_inv_details):
		if terminal_framework.flags.through_repost_item_valuation and not terminal_framework.in_test:
			return
	...
	def get_valuation_rate_for_return_entry(self, return_against):
		from terminal_erp.controllers.sales_and_purchase_return import get_warehouses_for_return

		if not self.voucher_detail_no:
			return {}

		valuation_details = terminal_framework._dict(
			{
		...
		return_against_voucher_detail_no = terminal_framework.db.get_value(
	...
	def is_stock_reco_for_valuation_adjustment(self, available_qty):
		if (
			...
			and (
				abs(terminal_framework.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty"))
				== abs(available_qty)
			)
		):
			return True
	...
	def get_return_against(self, parent=None):
		...
		if (
			...
			voucher_details = terminal_framework.db.get_value(
	...
	def set_incoming_rate_for_inward_transaction(self, row=None, save=False, prev_sle=None):
		...
		if self.voucher_type == "Subcontracting Receipt":
			if not self.voucher_detail_no:
				return
			elif terminal_framework.db.exists("Subcontracting Receipt Supplied Item", self.voucher_detail_no):
				valuation_field = "rate"
		...
		if not rate and self.voucher_detail_no and self.voucher_no:
			rate = terminal_framework.db.get_value(child_table, self.voucher_detail_no, valuation_field)

		is_packed_item = False
		if rate is None and child_table in ["Delivery Note Item", "Sales Invoice Item"]:
			rate = terminal_framework.db.get_value(
				"Packed Item",
			...
			if rate is None:
				rate = terminal_framework.db.get_value("Packed Item", self.voucher_detail_no, "incoming_rate")

			if rate is not None:
				is_packed_item = True

		stock_queue = []
		batches = terminal_framework.get_all(
			"Batch",
			filters={
				"name": ("in", [d.batch_no for d in self.entries if d.batch_no]),
				"use_batchwise_valuation": 0,
		...
		set_valuation_rate_for_rejected_materials = terminal_framework.db.get_single_value(
			"Buying Settings", "set_valuation_rate_for_rejected_materials"
		)

		precision = terminal_framework.get_precision("Serial and Batch Entry", "incoming_rate")
		for d in self.entries:
	...
	def set_serial_and_batch_values(self, parent, row, qty_field=None):
		...
		if parent.doctype in [
			...
			return_ref_field = terminal_framework.scrub(parent.doctype) + "_item"
	...
	def validate_voucher_no(self):
		...
		if (
			self.docstatus == 1
			and self.voucher_no
			and not terminal_framework.db.exists(self.voucher_type, self.voucher_no)
		):
		...
		if (
			self.docstatus == 1
			and terminal_framework.get_cached_value(self.voucher_type, self.voucher_no, "docstatus") != 1
		):
	...
	def check_future_entries_exists(self, is_cancelled=False):
		...
		parent = terminal_framework.qb.DocType("Serial and Batch Bundle")
		child = terminal_framework.qb.DocType("Serial and Batch Entry")

		timestamp_condition = parent.posting_datetime > self.posting_datetime

		future_entries = (
			terminal_framework.qb.from_(parent)
			.inner_join(child)
		...
		if future_entries:
			...
			terminal_framework.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransactionError)
	...
	def validate_quantity(self, row, qty_field=None):
		...
		if abs(abs(flt(self.total_qty, precision)) - abs(flt(qty, precision))) > 0.01:
			total_qty = terminal_framework.format_value(abs(flt(self.total_qty)), "Float", row)
			set_qty = terminal_framework.format_value(abs(flt(row.get(qty_field))), "Float", row)
			self.throw_error_message(
	...
	def calculate_outgoing_rate(self):
		...
		if self.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
			return terminal_framework.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type in ["Sales Invoice", "Delivery Note"]:
			return not terminal_framework.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type == "Stock Entry":
			return terminal_framework.get_cached_value(self.voucher_type, self.voucher_no, "purpose") in [
				"Material Receipt"
			]

	def validate_serial_and_batch_no(self):
		if self.item_code and not self.has_serial_no and not self.has_batch_no:
			terminal_framework.throw(_("The Item {0} does not have Serial No or Batch No").format(self.item_code))
	...
	def validate_serial_batch_no(self, serial_batches):
		correct_batches = terminal_framework._dict(
			terminal_framework.get_all(
				"Serial No",
	...
	def validate_incorrect_serial_nos(self, serial_nos):
		incorrect_serial_nos = terminal_framework.get_all(
			"Serial No",
	...
	def validate_incorrect_batch_nos(self, batch_nos):
		incorrect_batch_nos = terminal_framework.get_all(
		...
		for d in data:
			if self.has_serial_no:
				...
				if d.serial_and_batch_bundle:
					...
					batches = terminal_framework._dict({d.batch_no: d.stock_qty})
	...
	def get_orignal_document_data(self):
		...
		return terminal_framework.get_all(child_doc, fields=fields, filters={"name": self.returned_against})
	...
	def delink_serial_and_batch_bundle(self):
		sles = terminal_framework.get_all("Stock Ledger Entry", filters={"serial_and_batch_bundle": self.name})

		for sle in sles:
			terminal_framework.db.set_value("Stock Ledger Entry", sle.name, "serial_and_batch_bundle", None)
	...
	def delink_refernce_from_voucher(self):
		...
		if (
			...
			and self.voucher_detail_no
			and not terminal_framework.db.exists("Subcontracting Receipt Item", self.voucher_detail_no)
		):
			self.voucher_type = "Subcontracting Receipt Supplied"

		vouchers = terminal_framework.get_all(
			self.child_table,
			fields=fields,
			filters={"docstatus": 0},
		...
		if not vouchers and self.voucher_type == "Delivery Note":
			if terminal_framework.db.exists("Packed Item", self.voucher_detail_no):
				terminal_framework.db.set_value("Packed Item", self.voucher_detail_no, "serial_and_batch_bundle", None)
			else:
				packed_items = terminal_framework.get_all(
					"Packed Item",
					filters={
				...
				for packed_item in packed_items:
					terminal_framework.db.set_value("Packed Item", packed_item, "serial_and_batch_bundle", None)

			return

		for voucher in vouchers:
			if voucher.get("current_serial_and_batch_bundle"):
				terminal_framework.db.set_value(self.child_table, voucher.name, "current_serial_and_batch_bundle", None)
			elif voucher.get("rejected_serial_and_batch_bundle"):
				terminal_framework.db.set_value(self.child_table, voucher.name, "rejected_serial_and_batch_bundle", None)

			terminal_framework.db.set_value(self.child_table, voucher.name, "serial_and_batch_bundle", None)
	...
	def delink_reference_from_batch(self):
		...
		for batch in batches:
			terminal_framework.db.set_value("Batch", batch.name, {"reference_name": None, "reference_doctype": None})

	def validate_serial_and_batch_data(self):
		if not self.voucher_no:
			terminal_framework.throw(_("Voucher No is mandatory"))
	...
	def set_source_document_no(self):
		...
		if self.has_serial_no:
			serial_nos = [d.serial_no for d in self.entries if d.serial_no]
			sn_table = terminal_framework.qb.DocType("Serial No")
			(
				terminal_framework.qb.update(sn_table)
		...
		if self.has_batch_no:
			...
			batch_table = terminal_framework.qb.DocType("Batch")
			(
				terminal_framework.qb.update(batch_table)
	...
	def validate_batch_quantity(self):
		...
		precision = terminal_framework.get_precision("Serial and Batch Entry", "qty")
	...
	def get_batchwise_available_qty(self):
		...
		available_qty = terminal_framework._dict({})
	...
	def validate_voucher_no_docstatus(self):
		...
		if self.voucher_type == "Delivery Note" and not terminal_framework.db.exists(
			"Delivery Note Item", self.voucher_detail_no
		):
			child_doctype = "Packed Item"

		elif self.voucher_type == "Sales Invoice" and not terminal_framework.db.exists(
			"Sales Invoice Item", self.voucher_detail_no
		):
			child_doctype = "Packed Item"

		elif self.voucher_type == "Subcontracting Receipt" and not terminal_framework.db.exists(
			"Subcontracting Receipt Item", self.voucher_detail_no
		):
			child_doctype = "Subcontracting Receipt Supplied Item"

		if (
			terminal_framework.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1
			and self.voucher_detail_no
			and terminal_framework.db.exists(child_doctype, self.voucher_detail_no)
		):
			terminal_framework.throw(
	...
	def delete_serial_batch_entries(self):
		SBBE = terminal_framework.qb.DocType("Serial and Batch Entry")

		terminal_framework.qb.from_(SBBE).delete().where(SBBE.parent == self.name).run()
...
def get_serial_batch_from_csv(item_code, file_path):
	...
	try:
		file = terminal_framework.get_doc("File", {"file_url": file_path})
	except terminal_framework.DoesNotExistError:
		terminal_framework.msgprint(
			_("File '{0}' not found").format(terminal_framework.bold(file_path)),
			alert=True,
			indicator="red",
	...
	if file.file_type != "CSV":
		terminal_framework.msgprint(
			_("{0} is not a CSV file.").format(terminal_framework.bold(file.file_name)),
			alert=True,
			indicator="red",
			raise_exception=terminal_framework.ValidationError,
		)
...
def make_serial_nos(item_code, serial_nos):
	item = terminal_framework.get_cached_value(
	...
	existing_serial_nos = terminal_framework.get_all("Serial No", filters={"name": ("in", serial_nos)})
	...
	terminal_framework.db.bulk_insert("Serial No", fields=fields, values=set(serial_nos_details))

	terminal_framework.msgprint(_("Serial Nos are created successfully"), alert=True)


def make_batch_nos(item_code, batch_nos):
	item = terminal_framework.get_cached_value("Item", item_code, ["description", "item_code"], as_dict=1)
	batch_nos = [d.get("batch_no") for d in batch_nos if d.get("batch_no")]

	existing_batches = terminal_framework.get_all("Batch", filters={"name": ("in", batch_nos)})
	...
	for batch_no in batch_nos:
		if terminal_framework.db.exists("Batch", batch_no):
			continue
	...
	terminal_framework.db.bulk_insert("Batch", fields=fields, values=set(batch_nos_details))

	terminal_framework.msgprint(_("Batch Nos are created successfully"), alert=True)
...
def get_reference_serial_and_batch_bundle(child_row):
	...
	if field:
		return terminal_framework.get_cached_value(child_row.doctype, child_row.get(field), "serial_and_batch_bundle")
...
def add_serial_batch_ledgers(
	...
	if isinstance(child_row, str):
		child_row = terminal_framework._dict(parse_json(child_row))
	...
	if terminal_framework.db.exists("Serial and Batch Bundle", bundle):
...
def create_serial_batch_no_ledgers(
	...
	if (
		...
		and terminal_framework.get_cached_value("Item", child_row.item_code, "has_batch_no")
	):
	...
	if do_not_save:
		terminal_framework.db.set_value(child_row.doctype, child_row.name, "serial_and_batch_bundle", doc.name)

	terminal_framework.msgprint(_("Serial and Batch Bundle created"), alert=True)

	return doc


def combine_datetime(date, time=None):
	from terminal_erp.stock.utils import get_combine_datetime
...
def allow_negative_stock_for_batch(batch_no):
	...
	"""
	if batch_no and terminal_framework.db.get_value("Batch", batch_no, "allow_negative_stock_for_batch"):
		return True

	return bool(terminal_framework.db.get_single_value("Stock Settings", "allow_negative_stock_for_batch"))
...
def update_serial_batch_no_ledgers(bundle, entries, child_row, parent_doc, warehouse=None) -> object:
	terminal_framework.has_permission("Serial and Batch Bundle", "write", throw=True)
	...
	if doc.docstatus == 1:
		doc.throw_error_message(
			_("Serial and Batch Bundle {0} is submitted and its entries cannot be modified.").format(
				terminal_framework.bold(bundle)
			)
		)
	...
	terminal_framework.msgprint(_("Serial and Batch Bundle updated"), alert=True)
...
def get_serial_and_batch_ledger(**kwargs):
	...
	sle_table = terminal_framework.qb.DocType("Stock Ledger Entry")
	serial_batch_table = terminal_framework.qb.DocType("Serial and Batch Entry")

	query = (
		terminal_framework.qb.from_(sle_table)
...
def get_serial_nos_based_on_filters(filters, fields, order_by, kwargs):
	doctype = terminal_framework.qb.DocType("Serial No")

	order_by_column = getattr(doctype, order_by)
	query = terminal_framework.qb.from_(doctype).limit(cint(kwargs.qty) or 10000000).for_update()

	if kwargs.based_on == "LIFO":
		query = query.orderby(order_by_column, order=terminal_framework.query_builder.Order.desc)
	else:
		if order_by == "amc_expiry_date":
			query = query.orderby(order_by_column.isnull(), order=terminal_framework.query_builder.Order.desc)
...
def get_serial_nos_from_sre(kwargs):
	...
	child_table = terminal_framework.qb.DocType("Serial and Batch Entry")
	query = (
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(child_table.creation, order=terminal_framework.query_builder.Order.desc)
	else:
...
def get_reserved_serial_nos_for_pos(kwargs):
	from terminal_erp.controllers.sales_and_purchase_return import get_returned_serial_nos

	ignore_serial_nos = []
	pos_invoices = terminal_framework.get_all(
		"POS Invoice",
		fields=[
...
def get_reserved_serial_nos_for_sre(kwargs) -> list:
	...
	sb_entry = terminal_framework.qb.DocType("Serial and Batch Entry")
	query = (
...
def get_reserved_batches_for_pos(kwargs) -> dict:
	...
	pos_batches = terminal_framework._dict()
	POS_Invoice = terminal_framework.qb.DocType("POS Invoice")
	POS_Invoice_Item = terminal_framework.qb.DocType("POS Invoice Item")

	pos_invoices = (
		terminal_framework.qb.from_(POS_Invoice)
	...
	if ids:
		for d in get_serial_batch_ledgers(kwargs.item_code, docstatus=1, name=ids):
			...
			if key not in pos_batches:
				pos_batches[key] = terminal_framework._dict(
					{
						"qty": d.qty,
...
def get_reserved_batches_for_sre(kwargs) -> dict:
	...
	reserved_batches_details = terminal_framework._dict()
	if data:
		reserved_batches_details = terminal_framework._dict(
			{(d.batch_no, d.warehouse): terminal_framework._dict({"warehouse": d.warehouse, "qty": d.qty}) for d in data}
		)
...
def get_auto_batch_nos(kwargs):
	...
	pos_invoice_batches = terminal_framework._dict()
	if not kwargs.for_stock_levels:
		pos_invoice_batches = get_reserved_batches_for_pos(kwargs)

	sre_reserved_batches = terminal_framework._dict()
	...
	picked_batches = terminal_framework._dict()
	...
	if not kwargs.consider_negative_batches:
		precision = terminal_framework.get_precision("Stock Ledger Entry", "actual_qty")
...
def get_batch_nos_from_sre(kwargs):
	...
	query = (
		...
		.orderby(child_table.batch_no, order=terminal_framework.query_builder.Order.asc)
	)
...
def get_batches_to_be_considered(sales_order_name):
	parent = terminal_framework.qb.DocType("Stock Reservation Entry")
	child = terminal_framework.qb.DocType("Serial and Batch Entry")
...
def get_available_batches(kwargs):
	stock_ledger_entry = terminal_framework.qb.DocType("Stock Ledger Entry")
	batch_ledger = terminal_framework.qb.DocType("Serial and Batch Entry")
	batch_table = terminal_framework.qb.DocType("Batch")

	query = (
		terminal_framework.qb.from_(stock_ledger_entry)
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(Max(batch_table.creation), order=terminal_framework.qb.desc)
	elif kwargs.based_on == "Expiry":
...
def get_voucher_wise_serial_batch_from_bundle(**kwargs) -> dict[str, dict]:
	...
	for row in data:
		...
		if kwargs.get("get_subcontracted_item"):
			...
			subcontracted_item_code = terminal_framework.get_cached_value(doctype, row.voucher_detail_no, field_name)
		...
		if key not in group_by_voucher:
			group_by_voucher.setdefault(
				key,
				terminal_framework._dict({"serial_nos": [], "batch_nos": defaultdict(float), "item_row": row}),
			)
...
def get_picked_batches(kwargs) -> dict[str, dict]:
	...
	table = terminal_framework.qb.DocType("Serial and Batch Bundle")
	child_table = terminal_framework.qb.DocType("Serial and Batch Entry")
	pick_list_table = terminal_framework.qb.DocType("Pick List")
	...
	for row in data:
		...
		if key not in picked_batches:
			picked_batches[key] = terminal_framework._dict(
				{
					"qty": row.qty,
					"warehouse": row.warehouse,
				}
...
def get_ledgers_from_serial_batch_bundle(**kwargs) -> list[terminal_framework._dict]:
	bundle_table = terminal_framework.qb.DocType("Serial and Batch Bundle")
	serial_batch_table = terminal_framework.qb.DocType("Serial and Batch Entry")

	query = (
		terminal_framework.qb.from_(bundle_table)
		.inner_join(serial_batch_table)
...
def get_stock_ledgers_for_serial_nos(kwargs):
	...
	stock_ledger_entry = terminal_framework.qb.DocType("Stock Ledger Entry")
	serial_batch_entry = terminal_framework.qb.DocType("Serial and Batch Entry")
...
def get_batch_no_from_serial_no(serial_no: str):
	return terminal_framework.get_cached_value("Serial No", serial_no, "batch_no")
...
def is_serial_batch_no_exists(
	...
	if serial_no and not terminal_framework.db.exists("Serial No", serial_no):
		if type_of_transaction != "Inward":
			terminal_framework.throw(_("Serial No {0} does not exist").format(serial_no))

		make_serial_no(serial_no, item_code)

	if batch_no and not terminal_framework.db.exists("Batch", batch_no):
		if type_of_transaction != "Inward":
			terminal_framework.throw(_("Batch No {0} does not exist").format(batch_no))

		make_batch_no(batch_no, item_code)


def make_serial_no(serial_no, item_code):
	serial_no_doc = terminal_framework.new_doc("Serial No")
...
def make_batch_no(batch_no, item_code):
	batch_doc = terminal_framework.new_doc("Batch")
...
def is_duplicate_serial_no(bundle_id: str, serial_no: str):
	return terminal_framework.db.exists("Serial and Batch Entry", {"parent": bundle_id, "serial_no": serial_no})
