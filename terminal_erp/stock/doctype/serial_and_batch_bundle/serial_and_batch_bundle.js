...
terminal_framework.ui.form.on("Serial and Batch Bundle", {
	setup(frm) {
		frm.trigger("set_queries");
	},

	before_submit(frm) {
		terminal_framework.throw(__("The user cannot submit the Serial and Batch Bundle manually"));
	},
	...
	validate_prompt_data(frm, prompt_data) {
		if (prompt_data.using_csv_file && !prompt_data.csv_file) {
			terminal_framework.throw(__("Please attach CSV file"));
		}
	...
	toggle_fields(frm) {
		let show_naming_series_field =
			terminal_framework.user_defaults.set_serial_and_batch_bundle_naming_based_on_naming_series;
		...
		if (frm.doc.has_serial_no) {
			frm.doc.entries.forEach((row) => {
				if (Math.abs(row.qty) !== 1) {
					terminal_framework.model.set_value(row.doctype, row.name, "qty", 1);
				}
			});
		}
	...
	set_queries(frm) {
		frm.set_query("item_code", () => {
			return {
				query: "terminal_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.item_query",
			};
		});
...
terminal_framework.ui.form.on("Serial and Batch Entry", {
	entries_add(frm, cdt, cdn) {
		if (frm.doc.warehouse) {
			terminal_framework.model.set_value(cdt, cdn, "warehouse", frm.doc.warehouse);
		}
	},
});
