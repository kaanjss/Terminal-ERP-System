...
from terminal_erp.stock.doctype.item_alternative.item_alternative import get_alternative_items
...
class TestItemAlternative(Terminal ERPTestSuite):
	def setUp(self):
		super().setUp()
		make_items()
	...
	def test_alternative_item_for_subcontract_rm(self):
		...
		reserved_qty_for_sub_contract = terminal_framework.db.get_value(
			"Bin",
		...
		doc = terminal_framework.get_doc("Stock Entry", se.name)
		...
		doc.save()
		doc.submit()
		after_transfer_reserved_qty_for_sub_contract = terminal_framework.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_sub_contract",
		)
		...
		scr = terminal_framework.get_doc("Subcontracting Receipt", scr.name)
		status = False
	...
	def test_alternative_item_for_production_rm(self):
		...
		reserved_qty_for_production = terminal_framework.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)

		ste = terminal_framework.get_doc(make_stock_entry(pro_order.name, "Material Transfer for Manufacture", 5))
		ste.insert()
		...
		ste.submit()
		reserved_qty_for_production_after_transfer = terminal_framework.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)
		...
		ste1 = terminal_framework.get_doc(make_stock_entry(pro_order.name, "Manufacture", 5))
	...
	def test_get_used_alternative_items_returns_substitution(self):
		# get_used_alternative_items (raw SQL -> terminal_framework.qb) returns the alternative items substituted
		# into a work order's transfer entries, keyed by the original item. Exercises the converted
		# query on both engines.
		from terminal_erp.stock.doctype.stock_entry.stock_entry import get_used_alternative_items
	...
	def test_get_alternative_items_case_insensitive_match(self):
		...
		case-insensitive by default, and terminal_framework compiles the `like` filter to ILIKE on
	...
	def test_get_alternative_items_pagination(self):
		...
		create_item(base)
		base_item = terminal_framework.get_doc("Item", base)
		...
		for alt in alts:
			create_item(alt)
			alt_item = terminal_framework.get_doc("Item", alt)
...
def make_items():
	...
	if terminal_framework.db.exists("Item", "Test FG A RW 1"):
		doc = terminal_framework.get_doc("Item", "Test FG A RW 1")
		doc.allow_alternative_item = 1
		doc.save()

	if terminal_framework.db.exists("Item", "Test Finished Goods - A"):
		doc = terminal_framework.get_doc("Item", "Test Finished Goods - A")
		doc.is_sub_contracted_item = 1
		doc.save()

	if not terminal_framework.db.get_value("BOM", {"item": "Test Finished Goods - A", "docstatus": 1}):
		make_bom(item="Test Finished Goods - A", raw_materials=["Test FG A RW 1", "Test FG A RW 2"])

	if not terminal_framework.db.get_value("Warehouse", {"warehouse_name": "Test Supplier Warehouse"}):
