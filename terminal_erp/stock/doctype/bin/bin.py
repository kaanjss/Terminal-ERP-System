...
class Bin(Document):
	...
	def recalculate_qty(self):
		from terminal_erp.manufacturing.doctype.work_order.work_order import get_reserved_qty_for_production
	...
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = terminal_framework.get_cached_value("Item", self.item_code, "stock_uom")
		self.set_projected_qty()
	...
	def update_reserved_qty_for_sub_contracting(
		...
		subcontract_order = terminal_framework.qb.DocType(subcontract_doctype)
		supplied_item = terminal_framework.qb.DocType("Subcontracting Order Supplied Item")
		...
		reserved_qty_for_sub_contract = (
			terminal_framework.qb.from_(subcontract_order)
			.from_(supplied_item)
		...
		if terminal_framework.db.field_exists("Stock Entry", "is_return"):
...
def on_doctype_update():
	terminal_framework.db.add_unique("Bin", ["item_code", "warehouse"], constraint_name="unique_item_warehouse")
...
def update_qty(bin_name, args):
	...
	terminal_framework.db.set_value("Bin", bin_name, bin_values, update_modified=True)
