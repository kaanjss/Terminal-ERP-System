...
from terminal_framework.utils import add_days, add_to_date, now, nowdate, today

from terminal_erp.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from terminal_erp.accounts.utils import repost_gle_for_stock_vouchers
from terminal_erp.controllers.stock_controller import create_item_wise_repost_entries
...
from terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation import (
...
from terminal_erp.stock.utils import PendingRepostingError
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestRepostItemValuation(Terminal ERPTestSuite, StockTestMixin):
	def test_repost_time_slot(self):
		repost_settings = terminal_framework.get_doc("Stock Reposting Settings")
	...
	def test_clear_old_logs(self):
		...
		logs = terminal_framework.get_all("Repost Item Valuation", filters={"status": "Skipped"})
	...
	def test_deduplication(self):
		...
		riv_args = terminal_framework._dict(
			doctype="Repost Item Valuation",
			item_code="_Test Item",
			warehouse="_Test Warehouse - _TC",
			based_on="Item and Warehouse",
		...
		riv1 = terminal_framework.get_doc(riv_args)
		riv1.flags.dont_run_in_test = True
		riv1.submit()
		_assert_status(riv1, "Queued")

		# newer than existing duplicate - riv1
		riv2 = terminal_framework.get_doc(riv_args.update({"posting_date": "2021-01-03"}))
		...
		riv3 = terminal_framework.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = terminal_framework.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_stock_freeze_validation(self):
		...
		riv = terminal_framework.get_doc(
			doctype="Repost Item Valuation",
			item_code="_Test Item",
			warehouse="_Test Warehouse - _TC",
			based_on="Item and Warehouse",
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = terminal_framework.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(terminal_framework.ValidationError, riv.cancel)
	...
	def test_queue_progress_serialization(self):
		...
		self.assertEqual(
			[["a", "b"], ["c", "d"]],
			sorted(terminal_framework.parse_json(terminal_framework.as_json(set([("a", "b"), ("c", "d")])))),
		)

	def test_gl_repost_progress(self):
		from terminal_erp.accounts import utils
	...
	def test_duplicate_ple_on_repost(self):
		...
		ple_entries = terminal_framework.db.get_list(
	...
	def test_account_freeze_validation(self):
		...
		self.assertRaises(terminal_framework.ValidationError, riv.save)
	...
	def test_create_repost_entry_for_cancelled_document(self):
		...
		self.assertFalse(terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

		pr.load_from_db()

		pr.cancel()
		self.assertEqual(pr.docstatus, 2)
		self.assertTrue(terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

	def test_repost_item_valuation_for_closing_stock_balance(self):
		from terminal_erp.stock.doctype.stock_closing_entry.stock_closing_entry import (
			prepare_closing_stock_balance,
		)

		doc = terminal_framework.new_doc("Stock Closing Entry")
		...
		riv = terminal_framework.new_doc("Repost Item Valuation")
		riv.update(
			{
				"item_code": "_Test Item",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_remove_attached_file(self):
		...
		if docname := terminal_framework.db.exists("Repost Item Valuation", {"voucher_no": pr1.name}):
			self.assertFalse(
