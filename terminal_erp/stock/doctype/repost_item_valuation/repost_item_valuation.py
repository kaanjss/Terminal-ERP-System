...
from terminal_framework.exceptions import QueryDeadlockError, QueryTimeoutError
from terminal_framework.model.document import Document
from terminal_framework.query_builder import DocType, Interval
from terminal_framework.query_builder.functions import CombineDatetime, Max, Now
from terminal_framework.utils import cint, get_link_to_form, get_weekday, getdate, now, nowtime
from terminal_framework.utils.user import get_users_with_role
from rq.timeouts import JobTimeoutException

import terminal_erp
from terminal_erp.accounts.services.gl_validator import validate_accounting_period
from terminal_erp.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from terminal_erp.stock.stock_ledger import (
...
class RepostItemValuation(Document):
	...
	def set_default_posting_time(self):
		...
		if not self.posting_date:
			terminal_framework.throw(_("Posting date is required"))
	...
	def validate_update_stock(self):
		if (
			...
			update_stock = terminal_framework.get_value(self.voucher_type, self.voucher_no, "update_stock")
			if not update_stock:
				msg = _(
	...
	def validate_recreate_stock_ledgers(self):
		...
		if serial_batch_items := terminal_framework.get_all(
	...
	def validate_period_closing_voucher(self):
		...
		if year_end_date and getdate(self.posting_date) <= getdate(year_end_date):
			date = terminal_framework.format(year_end_date, "Date")
		...
		if closing_stock and closing_stock[0].name:
			...
			to_date = terminal_framework.format(closing_stock[0].posting_date, "Date")
	...
	def get_max_period_closing_date(company):
		table = terminal_framework.qb.DocType("Period Closing Voucher")
	...
	def validate_accounts_freeze(self):
		acc_frozen_till_date = terminal_framework.db.get_value("Company", self.company, "accounts_frozen_till_date")
		frozen_accounts_modifier = terminal_framework.db.get_value(
		...
		if getdate(self.posting_date) <= getdate(acc_frozen_till_date):
			if frozen_accounts_modifier and terminal_framework.session.user in get_users_with_role(
				frozen_accounts_modifier
			):
				terminal_framework.msgprint(_("Caution: This might alter frozen accounts."))
				return
			terminal_framework.throw(_("You cannot repost item valuation before {0}").format(acc_frozen_till_date))
	...
	def set_company(self):
		if self.based_on == "Transaction":
			self.company = terminal_framework.get_cached_value(self.voucher_type, self.voucher_no, "company")
		elif self.warehouse:
			self.company = terminal_framework.get_cached_value("Warehouse", self.warehouse, "company")
	...
	def clear_attachment(self):
		if attachments := get_attachments(self.doctype, self.name):
			...
			terminal_framework.delete_doc("File", attachment.name, ignore_permissions=True)
	...
	def on_submit(self):
		...
		Exceptions:
		        ...
		        2. global flag terminal_framework.flags.dont_execute_stock_reposts is set
		...
		if not terminal_framework.in_test:
			return
		if self.flags.dont_run_in_test or terminal_framework.flags.dont_execute_stock_reposts:
			return
	...
	def check_pending_repost_against_cancelled_transaction(self):
		...
		terminal_framework.throw(msg, title=_("Pending processing"))
	...
	def skipped_similar_reposts(self):
		repost_entries = terminal_framework.get_all(
		...
		for entry in repost_entries:
			if (
				terminal_framework.db.get_value("Repost Item Valuation", entry.reposting_reference, "status")
				== "Completed"
			):
				terminal_framework.db.set_value("Repost Item Valuation", entry.name, "status", "Skipped")
	...
	def deduplicate_similar_repost(self):
		...
		riv = terminal_framework.qb.DocType("Repost Item Valuation")
		(
			terminal_framework.qb.update(riv)
	...
	def _recalculate_valuation_rate(self):
		doc = terminal_framework.get_doc(self.voucher_type, self.voucher_no)
...
def bulk_restart_reposting(names: str | list):
	...
	for name in names:
		doc = terminal_framework.get_doc("Repost Item Valuation", name)
		if doc.status != "Failed":
			continue

		doc.restart_reposting()

	terminal_framework.msgprint(_("Repost Item Valuation restarted for selected failed records."))


def on_doctype_update():
	terminal_framework.db.add_index("Repost Item Valuation", ["warehouse", "item_code"], "item_warehouse")


def repost(doc):
	try:
		terminal_framework.flags.through_repost_item_valuation = True
		if not terminal_framework.db.exists("Repost Item Valuation", doc.name):
			return

		# This is to avoid TooManyWritesError in case of large reposts
		terminal_framework.db.MAX_WRITES_PER_TRANSACTION *= 4
		...
		message = terminal_framework.message_log.pop() if terminal_framework.message_log else ""
		...
		if status == "Failed":
			outgoing_email_account = terminal_framework.get_cached_value(
...
def remove_attached_file(docname):
	if file_name := terminal_framework.db.get_value(
		"File", {"attached_to_name": docname, "attached_to_doctype": "Repost Item Valuation"}, "name"
	):
		terminal_framework.delete_doc("File", file_name, ignore_permissions=True, delete_permanently=True, force=True)
...
def repost_gl_entries(doc):
	if not cint(terminal_erp.is_perpetual_inventory_enabled(doc.company)):
		return
	...
	if terminal_framework.get_cached_value("Company", doc.company, "stock_delivered_but_not_billed"):
		_update_post_delivery_billed_vouchers(transactions)

	enable_separate_reposting_for_gl = terminal_framework.db.get_single_value(
...
def _get_directly_dependent_vouchers(doc):
	...
	if doc.based_on == "Transaction":
		ref_doc = terminal_framework.get_lazy_doc(doc.voucher_type, doc.voucher_no)
...
def notify_error_to_stock_managers(doc, traceback):
	...
	terminal_framework.sendmail(recipients=recipients, subject=subject, message=message)


def get_recipients():
	role = (
		terminal_framework.db.get_single_value("Stock Reposting Settings", "notify_reposting_error_to_role")
		or "Stock Manager"
	)
...
def run_parallel_reposting():
	...
	if not terminal_framework.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		return
	...
	no_of_parallel_reposting = (
		terminal_framework.db.get_single_value("Stock Reposting Settings", "no_of_parallel_reposting") or 4
	)

	riv_entries = get_repost_item_valuation_entries()

	rq_jobs = terminal_framework.get_all(
		"RQ Job",
		fields=["arguments"],
		filters={
			"status": ("like", "%started%"),
			"job_name": "terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation.execute_reposting_entry",
		},
	)
...
def repost_entries():
	...
	if terminal_framework.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		return
...
def execute_reposting_entry(name):
	...
	if (
		...
		and terminal_framework.db.get_value("Repost Item Valuation", doc.reposting_reference, "status")
...
def get_repost_item_valuation_entries():
	doctype = terminal_framework.qb.DocType("Repost Item Valuation")
	...
	query = (
		...
		.orderby(CombineDatetime(doctype.posting_date, doctype.posting_time), order=terminal_framework.qb.asc)
		.orderby(doctype.creation, order=terminal_framework.qb.asc)
		.orderby(doctype.status, order=terminal_framework.qb.asc)
	)
...
def in_configured_timeslot(repost_settings=None, current_time=None):
	...
	if repost_settings is None:
		repost_settings = terminal_framework.get_cached_doc("Stock Reposting Settings")
...
def execute_repost_item_valuation():
	...
	method = "terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation.repost_entries"
	if terminal_framework.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		method = "terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation.run_parallel_reposting"

	if name := terminal_framework.db.get_value(
		"Scheduled Job Type",
		{"method": method},
		"name",
	):
		terminal_framework.get_doc("Scheduled Job Type", name).enqueue(force=True)
...
def make_reposting_for_accounting_ledgers(transactions, company, repost_doc):
	...
	for voucher_type, voucher_no in transactions:
		...
		terminal_framework.db.savepoint("repost_accounting_ledger")
		try:
			new_repost_doc = terminal_framework.new_doc("Repost Item Valuation")
			...
			terminal_framework.db.rollback(save_point="repost_accounting_ledger")


def get_existing_reposting_only_gl_entries(reposting_reference):
	existing_reposting = terminal_framework.get_all(
		"Repost Item Valuation",
		filters={
