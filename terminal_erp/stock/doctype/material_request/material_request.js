...
terminal_framework.ui.form.on("Material Request", {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		terminal_erp.utils.add_item(frm);
		...
		if (!frm.doc.buying_price_list) {
			frm.doc.buying_price_list = terminal_framework.defaults.get_default("buying_price_list");
		}
	},
	...
	set_from_warehouse: function (frm) {
		if (frm.doc.material_request_type == "Material Transfer" && frm.doc.set_from_warehouse) {
			frm.doc.items.forEach((d) => {
				terminal_framework.model.set_value(d.doctype, d.name, "from_warehouse", frm.doc.set_from_warehouse);
			});
		}
	},
	...
	make_custom_buttons: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status != "Stopped") {
			let precision = terminal_framework.defaults.get_default("float_precision");
	...
	update_status: function (frm, stop_status) {
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.material_request.material_request.update_status",
			...
			args: {
				ctx: {
					...
					buying_price_list: frm.doc.buying_price_list
						...
						: terminal_framework.defaults.get_default("buying_price_list"),
					currency: terminal_framework.defaults.get_default("Currency"),
					name: frm.doc.name,
					qty: item.qty || 1,
			...
			callback: function (r) {
				...
				if (!r.exc) {
					...
					if (d.price_list_rate != r.message.price_list_rate) {
						...
						terminal_framework.model.set_value(d.doctype, d.name, "rate", d.price_list_rate);
					}
					...
					callback: function (r) {
						if (!r.message) {
							...
							$.each(r.message, function (i, item) {
								var d = terminal_framework.model.add_child(cur_frm.doc, "Material Request Item", "items");
	...
	make_request_for_quotation: function (frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.material_request.mapper.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	make_supplier_quotation: function (frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.material_request.mapper.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_in_transit_stock_entry(frm) {
		terminal_framework.prompt(
			...
			(values) => {
				terminal_framework.call({
					method: "terminal_erp.stock.doctype.material_request.mapper.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.material_request.mapper.create_pick_list",
			frm: frm,
		});
	},
	...
	raise_work_orders: function (frm) {
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
terminal_framework.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		const item = locals[doctype][name];
		if (flt(item.qty) < flt(item.min_order_qty)) {
			terminal_framework.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
	...
	rate(frm, doctype, name) {
		...
		terminal_framework.model.set_value(doctype, name, "amount", item.amount);
...
terminal_erp.buying.MaterialRequestController = class MaterialRequestController extends (
	...
	qty(doc, cdt, cdn) {
		...
		terminal_framework.model.set_value(cdt, cdn, "amount", row.amount);
		refresh_field("amount", row.name, row.parentfield);
	}
};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new terminal_erp.buying.MaterialRequestController({ frm: cur_frm }));
