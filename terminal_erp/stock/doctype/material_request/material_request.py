...
from terminal_framework.query_builder.functions import Min, Sum
from terminal_framework.utils import cint, flt, get_datetime, get_link_to_form, getdate, new_line_sep, nowdate

from terminal_erp.buying.utils import check_on_hold_or_closed_status, validate_for_items
from terminal_erp.controllers.buying_controller import BuyingController
from terminal_erp.manufacturing.doctype.work_order.work_order import get_item_details
from terminal_erp.stock.stock_balance import get_indented_qty, update_bin_qty
...
class MaterialRequest(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.material_request_item.material_request_item import MaterialRequestItem
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = terminal_framework.get_all(
				...
				actual_so_qty = terminal_framework.get_all(
					"Sales Order Item",
	...
	def validate(self):
		...
		if not self.buying_price_list:
			self.buying_price_list = terminal_framework.defaults.get_defaults().buying_price_list
	...
	def validate_pp_qty(self):
		...
		if items_from_pp:
			...
			table = terminal_framework.qb.DocType("Material Request Plan Item")
			query = (
	...
	def on_submit(self):
		...
		if self.material_request_type == "Purchase":
			...
			if terminal_framework.db.exists("Budget", {"applicable_on_material_request": 1, "docstatus": 1}):
	...
	def check_modified_date(self):
		mod_db = terminal_framework.db.get_value("Material Request", self.name, "modified")

		if mod_db and get_datetime(mod_db) != get_datetime(self.modified):
			terminal_framework.throw(_("{0} {1} has been modified. Please refresh.").format(_(self.doctype), self.name))
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if self.material_request_type in ("Material Issue", "Material Transfer", "Customer Provided"):
			doctype = terminal_framework.qb.DocType("Stock Entry Detail")
		...
		if doctype and qty_field:
			...
			mr_items_ordered_qty = terminal_framework._dict(query.run())
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		...
		mr_qty_allowance = terminal_framework.db.get_single_value("Stock Settings", "mr_qty_allowance")
		...
		for d in self.get("items"):
			...
			if d.name in mr_items:
				...
				terminal_framework.db.set_value(d.doctype, d.name, "ordered_qty", d.ordered_qty)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (
				...
				and d.warehouse
				and terminal_framework.db.get_value("Item", d.item_code, "is_stock_item") == 1
			):
	...
	def update_requested_qty_in_production_plan(self, cancel=False):
		...
		for d in self.get("items"):
			if d.production_plan and d.material_request_plan_item:
				requested_qty = terminal_framework.get_value(
		...
		for production_plan in production_plans:
			doc = terminal_framework.get_doc("Production Plan", production_plan)
			doc.set_status()
...
def update_status(name: str, status: str):
	material_request = terminal_framework.get_doc("Material Request", name)
...
def get_material_requests_based_on_supplier(
	...
	if not supplier_items:
		terminal_framework.throw(_("{0} is not the default supplier for any items.").format(supplier))

	mr = terminal_framework.qb.DocType("Material Request")
	mr_item = terminal_framework.qb.DocType("Material Request Item")

	query = (
		terminal_framework.qb.from_(mr)
		.from_(mr_item)
...
def raise_work_orders(material_request: str, company: str):
	...
	errors = []
	work_orders = []
	default_wip_warehouse = terminal_framework.get_cached_value("Company", company, "default_wip_warehouse")

	for d in mr.items:
		if (d.stock_qty - d.ordered_qty) > 0:
			if terminal_framework.db.exists("BOM", {"item": d.item_code, "is_default": 1, "is_active": 1}) or (
				(variant_of := terminal_framework.get_value("Item", d.item_code, "variant_of"))
				and terminal_framework.db.exists("BOM", {"item": variant_of, "is_default": 1, "is_active": 1})
			):
		...
		if len(work_orders) > 1:
			msgprint(
				_("The following {0} were created: {1}").format(
					terminal_framework.bold(_("Work Orders")), "<br>" + ", ".join(work_orders_list)
				)
			)
		else:
			msgprint(
				_("The {0} {1} created successfully").format(
					terminal_framework.bold(_("Work Order")), work_orders_list[0]
				)
			)
