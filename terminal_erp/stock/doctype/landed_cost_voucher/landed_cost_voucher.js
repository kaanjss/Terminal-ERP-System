...
terminal_erp.landed_cost_taxes_and_charges.setup_triggers("Landed Cost Voucher");
terminal_erp.stock.LandedCostVoucher = class LandedCostVoucher extends terminal_erp.stock.StockController {
	refresh() {
		...
		if (this.frm.doc.company) {
			let company_currency = terminal_framework.get_doc(":Company", this.frm.doc.company).default_currency;
	...
	get_items_from_purchase_receipts() {
		...
		if (!this.frm.doc.purchase_receipts.length) {
			terminal_framework.msgprint(__("Please enter Purchase Receipt first"));
		} else {
...
cur_frm.script_manager.make(terminal_erp.stock.LandedCostVoucher);
...
terminal_framework.ui.form.on("Landed Cost Voucher", {
	setup(frm) {
	...
	setup_queries(frm) {
		...
		frm.set_query("vendor_invoice", "vendor_invoices", (doc, cdt, cdn) => {
			return {
				query: "terminal_erp.stock.doctype.landed_cost_voucher.landed_cost_voucher.get_vendor_invoices",
				filters: {
					company: doc.company,
				},
			};
...
terminal_framework.ui.form.on("Landed Cost Purchase Receipt", {
...
terminal_framework.ui.form.on("Landed Cost Vendor Invoice", {
