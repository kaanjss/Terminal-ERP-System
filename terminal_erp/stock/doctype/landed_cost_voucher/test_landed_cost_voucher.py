...
from terminal_framework.utils import add_days, add_to_date, flt, now, nowtime, today
...
from terminal_erp.accounts.utils import update_gl_entries_after
...
class TestLandedCostVoucher(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_get_vendor_invoices_runs(self):
		...
		from terminal_erp.stock.doctype.landed_cost_voucher.landed_cost_voucher import get_vendor_invoices
	...
	def _cancel_and_delete_pi(name):
		if not terminal_framework.db.exists("Purchase Invoice", name):
			return
		doc = terminal_framework.get_doc("Purchase Invoice", name)
		if doc.docstatus == 1:
			doc.cancel()
		terminal_framework.delete_doc("Purchase Invoice", name, force=1)
	...
	def test_landed_cost_voucher(self):
		...
		last_sle = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
		...
		pr_lc_value = terminal_framework.db.get_value(
			"Purchase Receipt Item", {"parent": pr.name}, "landed_cost_voucher_amount"
		)
		self.assertEqual(pr_lc_value, 25.0)

		last_sle_after_landed_cost = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": pr.doctype,
				"voucher_no": pr.name,
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		pr1 = make_purchase_receipt(
			...
			qty=500,
			rate=80,
			posting_date=add_days(terminal_framework.utils.nowdate(), -2),
		)
		...
		last_sle = terminal_framework.db.get_value(  # SLE of second PR
			"Stock Ledger Entry",
			{
		...
		last_sle_after_landed_cost = terminal_framework.db.get_value(  # SLE of second PR after LCV's effect
			"Stock Ledger Entry",
			{
				"voucher_type": pr2.doctype,
				"voucher_no": pr2.name,
	...
	def test_lcv_validates_company(self):
		from terminal_erp import is_perpetual_inventory_enabled
		...
		company_doc = terminal_framework.get_doc("Company", company_a)
		...
		terminal_framework.db.set_value("Company", company_a, "enable_perpetual_inventory", epi)
		terminal_framework.local.enable_perpetual_inventory = {}
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = terminal_framework.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		terminal_framework.db.set_value("Item", "_Test Serialized Item", "serial_no_series", "SNJJ.###")
	...
	def test_multi_currency_lcv(self):
		from terminal_erp.setup.doctype.currency_exchange.test_currency_exchange import save_new_records
	...
	def test_asset_lcv(self):
		...
		if not terminal_framework.db.exists("Item", "Macbook Pro"):
		...
		self.assertEqual(terminal_framework.db.get_value("Asset", assets[0].name, "net_purchase_amount"), 50080)
	...
	def test_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not terminal_framework.db.exists("Batch", "BATCH-TLCVSNO-0001"):
		...
		for row in pr.items:
			...
			self.assertEqual(
				row.valuation_rate,
				terminal_framework.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "avg_rate"),
			)
	...
	def test_do_not_validate_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		from terminal_erp.stock.doctype.item.test_item import make_item
		from terminal_erp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
		...
		if not terminal_framework.db.exists("Batch", "BATCH-TDVLCVSNO-0001"):
			batch_doc = terminal_framework.get_doc(
				{
					"doctype": "Batch",
					"item": batch_item,
		...
		stock_ledger_entries = terminal_framework.get_all("Stock Ledger Entry", filters={"voucher_no": pr.name})
		for sle in stock_ledger_entries:
			doc = terminal_framework.get_doc("Stock Ledger Entry", sle.name)
		...
		stock_ledger_entries = terminal_framework.get_all("Stock Ledger Entry", filters={"voucher_no": dn.name})
		for sle in stock_ledger_entries:
			doc = terminal_framework.get_doc("Stock Ledger Entry", sle.name)
			if doc.item_code == sn_item:
				doc.db_set("serial_no", ", ".join(serial_nos))
	...
	def test_lcv_for_work_order_scr(self):
		...
		company = "_Test Company with perpetual inventory"
		warehouse = terminal_framework.get_value("Warehouse", {"company": company}, "name")
		wip_warehouse = terminal_framework.get_value(
			"Warehouse",
		...
		se = terminal_framework.get_doc(make_stock_entry_for_wo(wo_order.name, "Material Transfer for Manufacture", 10))
		se.submit()

		se = terminal_framework.get_doc(make_stock_entry_for_wo(wo_order.name, "Manufacture", 10))
		se.submit()
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = terminal_framework.get_doc(args.receipt_document_type, args.receipt_document)
