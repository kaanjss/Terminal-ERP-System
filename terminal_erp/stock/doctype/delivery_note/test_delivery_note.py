...
from terminal_framework.utils import add_days, cstr, flt, getdate, nowdate, nowtime, today
...
from terminal_erp.accounts.utils import get_balance_on
...
from terminal_erp.stock.doctype.delivery_note.mapper import (
	make_delivery_trip,
	make_sales_invoice,
)
from terminal_erp.stock.doctype.delivery_trip.test_delivery_trip import create_driver
...
from terminal_erp.stock.stock_ledger import get_previous_sle
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestDeliveryNote(Terminal ERPTestSuite):
	def setUp(self):
		self.load_test_records("Stock Entry")

	def get_perpetual_defaults(self):
		company = terminal_framework.get_doc("Company", "_Test Company with perpetual inventory")
	...
	def test_over_billing_against_dn(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(terminal_framework.ValidationError, terminal_framework.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		terminal_framework.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
		...
		sle = terminal_framework.get_doc("Stock Ledger Entry", {"voucher_type": "Delivery Note", "voucher_no": dn.name})
	...
	def test_delivery_note_gl_entry_packing_item(self):
		terminal_framework.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_serialize_status(self):
		...
		serial_no = terminal_framework.get_doc(
			{
				"doctype": "Serial No",
	...
	def test_delivery_note_return_against_denormalized_serial_no(self):
		...
		terminal_framework.flags.ignore_serial_batch_bundle_validation = True
		...
		for sn in serial_nos:
			if not terminal_framework.db.exists("Serial No", sn):
				sn_doc = terminal_framework.get_doc(
					{
						"doctype": "Serial No",
						"item_code": sn_item,
						"serial_no": sn,
		...
		terminal_framework.flags.ignore_serial_batch_bundle_validation = False
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = terminal_framework.get_doc("Delivery Note", dn1.name)
	...
	def test_sales_return_against_serial_batch_bundle(self):
		...
		for row in return_dn.items:
			total_qty = terminal_framework.db.get_value(
				"Serial and Batch Bundle", row.serial_and_batch_bundle, "total_qty"
			)

			self.assertEqual(total_qty, 2)

			doc = terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def test_delivery_note_internal_transfer_serial_no_status(self):
		...
		for serial_no in serial_nos:
			sn = terminal_framework.db.get_value("Serial No", serial_no, ["status", "warehouse"], as_dict=1)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from terminal_erp.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(terminal_framework.db.get_value("Delivery Note", dn.name, "status"), "Closed")
	...
	def test_sales_order_reference_validation(self):
		...
		self.assertRaises(terminal_framework.ValidationError, dn.save)
		dn.reload()
	...
	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"delete_linked_ledger_entries": True})
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from terminal_erp.accounts.doctype.sales_invoice.mapper import make_delivery_note
	...
	def test_delivery_trip(self):
		...
		self.assertRaisesRegex(
			terminal_framework.exceptions.ValidationError,
	...
	def test_delivery_note_bundle_with_batched_item(self):
		...
		reposting_entries = terminal_framework.get_all("Repost Item Valuation", filters={"docstatus": 1})
		for entry in reposting_entries:
			doc = terminal_framework.get_doc("Repost Item Valuation", entry.name)
			doc.cancel()
			doc.delete()

		terminal_framework.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 1)

		dn.reload()
		dn.delete()

		bundle = terminal_framework.db.get_value("Serial and Batch Bundle", {"voucher_detail_no": packed_name}, "name")
		self.assertFalse(bundle)

		terminal_framework.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		terminal_framework.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 0)
		...
		terminal_framework.get_doc(
			{
				...
				"title": terminal_framework.generate_hash(),
				"apply_on": "Item Code",
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(terminal_framework.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype}))
	...
	def test_reserve_qty_on_sales_return(self):
		terminal_framework.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 0)
		self.reserved_qty_check()

	def test_dont_reserve_qty_on_sales_return(self):
		terminal_framework.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 1)
		self.reserved_qty_check()

	def reserved_qty_check(self):
		from terminal_erp.controllers.sales_and_purchase_return import make_return_doc
		from terminal_erp.selling.doctype.sales_order.mapper import make_delivery_note
		from terminal_erp.stock.stock_balance import get_reserved_qty

		dont_reserve_qty = terminal_framework.db.get_single_value(
		...
		returned = terminal_framework.get_doc("Delivery Note", dn_return.name)
		returned.update_prevdoc_status()
	...
	def test_serial_no_status(self):
		...
		dn.reload()
		self.assertEqual(terminal_framework.db.get_value("Serial No", serial_no, "status"), "Delivered")
	...
	def test_sales_return_valuation_for_moving_average_case2(self):
		...
		terminal_framework.flags.print_debug_messages = False
		item_code = make_item(
	...
	def test_batch_with_non_stock_uom(self):
		...
		if not terminal_framework.db.exists("UOM Conversion Detail", {"parent": item.name, "uom": "Kg"}):
		...
		delivered_batch_qty = terminal_framework.db.get_value(
	...
	def test_delivery_note_legacy_serial_no_valuation(self):
		...
		sle_data = terminal_framework.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se_doc.name, "voucher_type": "Stock Entry"},
		)[0]

		sle_doc = terminal_framework.get_doc("Stock Ledger Entry", sle_data.name)
		...
		for sn in serial_nos:
			sn_doc = terminal_framework.get_doc("Serial No", sn)
			sn_doc.db_set(
				{
					"status": "Active",
					"warehouse": warehouse,
	...
	def test_partial_sales_return_batch_no_for_batched_item_in_dn(self):
		...
		returned_batch_no = get_batch_from_bundle(dn_return.items[0].serial_and_batch_bundle)
		self.assertEqual(batch_no, returned_batch_no)
		sabb_qty = terminal_framework.db.get_value(
	...
	def test_same_posting_date_and_posting_time(self):
		...
		self.assertRaises(terminal_framework.ValidationError, dn5.submit)
	...
	def test_warranty_expiry_date_for_serial_item(self):
		...
		for row in serial_nos:
			sn = terminal_framework.get_doc("Serial No", row)
	...
	def test_auto_set_serial_batch_for_draft_dn(self):
		terminal_framework.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)
		terminal_framework.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", "FIFO")
	...
	def test_packed_item_serial_no_status(self):
		...
		for row in dn.packed_items:
			...
			for row in doc.entries:
				status = terminal_framework.db.get_value("Serial No", row.serial_no, "status")
	...
	def test_sales_return_for_product_bundle(self):
		...
		for row in dn.packed_items:
			...
			if row.item_code not in serial_batch_map:
				serial_batch_map[row.item_code] = terminal_framework._dict(
					{
						"serial_nos": [],
	...
	def test_negative_stock_with_higher_precision(self):
		original_flt_precision = terminal_framework.db.get_default("float_precision")
		terminal_framework.db.set_single_value("System Settings", "float_precision", 7)
		...
		self.assertRaises(terminal_framework.ValidationError, dn.submit)

		terminal_framework.db.set_single_value("System Settings", "float_precision", original_flt_precision)
	...
	def test_different_rate_for_same_serial_nos(self):
		...
		sabb = terminal_framework.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		...
		sabb = terminal_framework.get_doc("Serial and Batch Bundle", dn1.items[0].serial_and_batch_bundle)
		for entry in sabb.entries:
	...
	def test_sdbnb_disabled_for_sales_return(self):
		...
		self.get_perpetual_defaults()
		terminal_framework.db.set_value("Company", self.perpetual_company, "disable_sdbnb_in_sr", 1)
		...
		try:
			...
			terminal_framework.db.set_value("Company", self.perpetual_company, "disable_sdbnb_in_sr", 0)
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"validate_selling_price": 1})
	...
	def test_validate_proj_cust_matches_project_customer(self):
		...
		mismatch_project = terminal_framework.get_doc(
			{
				"doctype": "Project",
		...
		open_project = terminal_framework.get_doc(
			{
				"doctype": "Project",
	...
	def test_cancel_packing_slips_cancels_submitted_slips(self):
		...
		from terminal_erp.stock.doctype.delivery_note.mapper import make_packing_slip
		from terminal_erp.stock.doctype.delivery_note.services.packing import PackingService
		...
		ps.submit()
		dn.submit()
		self.assertEqual(terminal_framework.db.get_value("Packing Slip", ps.name, "docstatus"), 1)

		PackingService(dn).cancel_packing_slips()

		self.assertEqual(terminal_framework.db.get_value("Packing Slip", ps.name, "docstatus"), 2)


def create_delivery_note(**args):
	dn = terminal_framework.new_doc("Delivery Note")
	...
	if not args.use_serial_batch_fields and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batches"):
			batches = terminal_framework._dict(args.batches)
