...
from terminal_erp.stock.doctype.delivery_note.services.billing_status import BillingStatusService
...
class DeliveryNote(SellingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.delivery_note_item.delivery_note_item import DeliveryNoteItem
	...
	def before_print(self, settings=None):
		...
		item_meta = terminal_framework.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def so_required(self):
		...
		if terminal_framework.get_single_value("Selling Settings", "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					terminal_framework.throw(_("Sales Order required for Item {0}").format(d.item_code))
			...
			if missing_label and missing_label != "No Label":
				errors.append(
					_("The field {0} in row {1} is not set").format(
						terminal_framework.bold(_(missing_label)), terminal_framework.bold(item.idx)
					)
				)

		if errors:
			terminal_framework.throw("<br>".join(errors), title=error_title)
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if not d["warehouse"] and terminal_framework.get_cached_value("Item", d["item_code"], "is_stock_item") == 1:
				terminal_framework.throw(_("Warehouse required for stock Item {0}").format(d["item_code"]))

	def update_current_stock(self):
		if self.get("_action") and self._action != "update_after_submit":
			for d in self.get("items"):
				d.actual_qty = terminal_framework.db.get_value(
					"Bin", {"item_code": d.item_code, "warehouse": d.warehouse}, "actual_qty"
				)

			for d in self.get("packed_items"):
				bin_qty = terminal_framework.db.get_value(
					"Bin",
	...
	def validate_expense_account(self):
		...
		for item in self.items:
			if item.get("against_sales_invoice"):
				...
				is_stock_item = terminal_framework.get_cached_value("Item", item.item_code, "is_stock_item")
				# Only stock items
			...
			if not item.warehouse:
				...
				if item.warehouse not in reserved_warehouses:
					msg = _("Row #{0}: Stock is reserved for item {1} in warehouse {2}.").format(
						...
						terminal_framework.bold(reserved_warehouses[0])
						if len(reserved_warehouses) == 1
						else _("{0} and {1}").format(
							terminal_framework.bold(", ".join(reserved_warehouses[:-1])),
							terminal_framework.bold(reserved_warehouses[-1]),
						),
					)
					terminal_framework.throw(msg, title=_("Stock Reservation Warehouse Mismatch"))
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			terminal_framework.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = terminal_framework.get_all(
		...
		if submit_in:
			terminal_framework.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
...
def update_delivery_note_status(docname: str, status: str):
	dn = terminal_framework.get_lazy_doc("Delivery Note", docname)
