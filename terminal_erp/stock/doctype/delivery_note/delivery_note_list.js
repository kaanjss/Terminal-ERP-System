terminal_framework.listview_settings["Delivery Note"] = {
	add_fields: [
		"customer",
		"customer_name",
		"base_grand_total",
	...
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				terminal_framework.new_doc("Delivery Trip").then(() => {
		...
		if (terminal_framework.model.can_create("Delivery Trip")) {
		...
		if (terminal_framework.model.can_create("Sales Invoice")) {
			doclist.page.add_action_item(__("Sales Invoice"), () => {
				terminal_erp.bulk_transaction_processing.create(doclist, "Delivery Note", "Sales Invoice");
			});
		}

		if (terminal_framework.model.can_create("Packing Slip")) {
			doclist.page.add_action_item(__("Packaging Slip From Delivery Note"), () => {
				terminal_erp.bulk_transaction_processing.create(doclist, "Delivery Note", "Packing Slip");
			});
		}
	},
};
