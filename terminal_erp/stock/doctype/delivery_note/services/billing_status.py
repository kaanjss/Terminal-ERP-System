...
class BillingStatusService:
	...
	def update_billing_status(self, update_modified: bool = True) -> None:
		...
		for dn in set(updated_delivery_notes):
			dn_doc = doc if (dn == doc.name) else terminal_framework.get_lazy_doc("Delivery Note", dn)
	...
	def make_return_invoice(self) -> None:
		...
		try:
			...
			credit_note_link = terminal_framework.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			terminal_framework.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
...
def update_billed_amount_based_on_so(so_detail: str, update_modified: bool = True) -> list[str]:
	...
	si = terminal_framework.qb.DocType("Sales Invoice").as_("si")
	si_item = terminal_framework.qb.DocType("Sales Invoice Item").as_("si_item")
	...
	dn = terminal_framework.qb.DocType("Delivery Note").as_("dn")
	dn_item = terminal_framework.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_against_dn = terminal_framework.get_all(
				"Sales Invoice Item",
