...
terminal_framework.provide("terminal_erp.stock.delivery_note");
...
terminal_erp.accounts.taxes.setup_tax_validations("Delivery Note");
terminal_erp.sales_common.setup_selling_controller();

terminal_framework.ui.form.on("Delivery Note", {
	setup: function (frm) {
		...
		terminal_erp.queries.setup_warehouse_query(frm);
		...
		frm.set_query("expense_account", "items", function (doc, cdt, cdn) {
			if (terminal_erp.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function (frm) {
		terminal_erp.stock.delivery_note.set_print_hide(frm.doc);
	},
	...
	refresh: function (frm) {
		if (
			...
			terminal_framework.model.can_create("Sales Invoice")
		) {
		...
		if (
			...
			terminal_framework.model.can_create("Purchase Receipt")
		) {
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						terminal_framework.model.open_mapped_doc({
							method: "terminal_erp.stock.doctype.delivery_note.mapper.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
terminal_framework.ui.form.on("Delivery Note Item", {
...
terminal_erp.stock.DeliveryNoteController = class DeliveryNoteController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			terminal_framework.model.can_read("Sales Order") &&
		...
		if (
			...
			doc.status != "Closed" &&
			this.frm.has_perm("write") &&
			terminal_framework.model.can_read("Pick List") &&
			this.frm.doc.docstatus === 0
		) {
			this.frm.add_custom_button(
				__("Pick List"),
				function () {
					...
					terminal_erp.utils.map_current_doc({
						method: "terminal_erp.stock.doctype.pick_list.mapper.create_dn_for_pick_lists",
						...
						get_query_method: "terminal_erp.stock.doctype.pick_list.pick_list.get_pick_list_query",
						size: "extra-large",
						allow_child_item_selection: true,
		...
		if (!doc.is_return && doc.status != "Closed") {
			if (doc.docstatus == 1 && terminal_framework.model.can_create("Shipment")) {
				this.frm.add_custom_button(
					__("Shipment"),
					function () {
						me.make_shipment();
			...
			if (
				...
				doc.docstatus == 1 &&
				terminal_framework.model.can_create("Installation Note")
			) {
				this.frm.add_custom_button(
			...
			if (doc.docstatus == 1 && doc.status != "Completed" && terminal_framework.model.can_create("Delivery Trip")) {
				this.frm.add_custom_button(
					__("Delivery Trip"),
					function () {
			...
			if (
				...
				terminal_framework.model.can_create("Packing Slip")
			) {
				this.frm.add_custom_button(
					__("Packing Slip"),
					function () {
		...
		terminal_erp.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.delivery_note.mapper.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.delivery_note.mapper.make_sales_return",
			frm: this.frm,
		});
	}
	...
	make_delivery_trip() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.stock.doctype.delivery_note.mapper.make_delivery_trip",
			frm: cur_frm,
		});
	}
	...
	update_status(status) {
		...
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new terminal_erp.stock.DeliveryNoteController({ frm: cur_frm }));
...
terminal_framework.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = terminal_erp.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

terminal_erp.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
	var dn_fields = terminal_framework.meta.docfield_map["Delivery Note"];
	var dn_item_fields = terminal_framework.meta.docfield_map["Delivery Note Item"];
...
terminal_framework.tour["Delivery Note"] = [
	{
		fieldname: "customer",
		title: __("Customer"),
