...
from terminal_erp.accounts.party import CROSS_PARTY_FIELD_NO_MAP, get_due_date
from terminal_erp.controllers.accounts_controller import get_taxes_and_charges, merge_taxes
from terminal_erp.stock.doctype.packed_item.packed_item import is_product_bundle
...
def get_invoiced_qty_map(delivery_note: str) -> dict:
	...
	invoiced_qty_map = terminal_framework._dict(
		(
...
def get_returned_qty_map(delivery_note: str) -> dict:
	...
	returned_qty_map = terminal_framework._dict(
		(
			terminal_framework.qb.from_(dni)
			.join(dn)
...
def make_sales_invoice(
	...
	doc = terminal_framework.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			terminal_framework.throw(_("All these items have already been invoiced/returned"))
	...
	if not doc.is_return:
		...
		if (
			...
			payment_terms_template = terminal_framework.db.get_value(doctype, so, "payment_terms_template")
...
def make_delivery_trip(
	...
	if not target_doc:
		target_doc = terminal_framework.new_doc("Delivery Trip")
...
def make_shipment(source_name: str, target_doc: str | Document | None = None):
	def postprocess(source, target):
		user = terminal_framework.db.get_value(
			"User", terminal_framework.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = terminal_framework.session.user
...
def make_inter_company_transaction(doctype: str, source_name: str, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if not target.get("items"):
			terminal_framework.throw(_("All items have already been received"))
