...
class StockSettings(Document):
	...
	def validate(self):
		for key in [
			...
			terminal_framework.db.set_default(key, self.get(key, ""))
	...
	def validate_do_not_use_batchwise_valuation(self):
		...
		if not terminal_framework.db.exists("Serial and Batch Bundle", {"docstatus": 1}):
			return
		...
		if doc_before_save.do_not_use_batchwise_valuation and not self.do_not_use_batchwise_valuation:
			terminal_framework.throw(
				_("Cannot disable {0} as it may lead to incorrect stock valuation.").format(
					terminal_framework.bold(_("Do Not Use Batchwise Valuation"))
				)
			)
	...
	def validate_serial_and_batch_no_settings(self):
		...
		if (
			...
			if terminal_framework.db.exists("Serial and Batch Bundle", {"docstatus": 1}):
				terminal_framework.throw(
					_(
	...
	def validate_warehouses(self):
		...
		for field in warehouse_fields:
			if terminal_framework.db.get_value("Warehouse", self.get(field), "is_group"):
				terminal_framework.throw(
					_(
						"Group Warehouses cannot be used in transactions. Please change the value of {0}"
					).format(terminal_framework.bold(self.meta.get_field(field).label)),
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) and not int(self.db_get("clean_description_html") or 0):
			...
			terminal_framework.enqueue(
				"terminal_erp.stock.doctype.stock_settings.stock_settings.clean_all_descriptions",
			...
			if self.allow_negative_stock and self.enable_stock_reservation:
				terminal_framework.throw(
					_("As {0} is enabled, you can not enable {1}.").format(
						terminal_framework.bold(_("Stock Reservation")), terminal_framework.bold(_("Allow Negative Stock"))
					)
				)
		...
		if self.has_value_changed("enable_stock_reservation"):
			...
			if self.enable_stock_reservation:
				...
				if self.allow_negative_stock:
					terminal_framework.throw(
						_("As {0} is enabled, you can not enable {1}.").format(
							terminal_framework.bold(_("Allow Negative Stock")), terminal_framework.bold(_("Stock Reservation"))
						)
					)
			...
			# Enable -> Disable
				...
				has_reserved_stock = terminal_framework.db.exists(
	...
	def validate_auto_insert_price_list_rate_if_missing(self):
		if (
			...
			selling_meta = terminal_framework.get_meta("Selling Settings")
			terminal_framework.msgprint(
				_(
					...
					terminal_framework.bold(_("Selling Settings")),
				)
			)
	...
	def make_property_setter_for_precision(doctypes):
		for doctype in doctypes:
			if property_name := terminal_framework.db.exists(
				"Property Setter",
				{"doc_type": doctype, "field_name": "conversion_factor", "property": "precision"},
			):
				terminal_framework.db.set_value("Property Setter", property_name, "value", 9)
				continue
...
def clean_all_descriptions():
	for item in terminal_framework.get_all("Item", ["name", "description"]):
		if item.description:
			clean_description = clean_html(item.description)
			if item.description != clean_description:
				terminal_framework.db.set_value("Item", item.name, "description", clean_description)


@terminal_framework.whitelist()
def get_enable_stock_uom_editing():
	return terminal_framework.get_single_value(
		"Stock Settings",
		[
