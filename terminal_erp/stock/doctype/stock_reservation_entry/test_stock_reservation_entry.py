...
from terminal_erp.selling.doctype.sales_order.mapper import create_pick_list, make_delivery_note
...
from terminal_erp.stock.doctype.stock_entry.stock_entry import StockEntry
...
class TestStockReservationEntry(Terminal ERPTestSuite):
	def setUp(self) -> None:
	...
	def test_validate_stock_reservation_settings(self) -> None:
		...
		voucher = terminal_framework._dict(
			{
				"doctype": "Sales Order",
			}
		)
		...
		with self.change_settings("Stock Settings", {"enable_stock_reservation": 0}):
			self.assertRaises(terminal_framework.ValidationError, validate_stock_reservation_settings, voucher)
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"enable_stock_reservation": 1})
	...
	def test_serial_and_batch_reservation_can_be_unreserved(self) -> None:
		...
		for item in so.items:
			...
			pinned = terminal_framework.get_all(
	...
	def test_auto_reserve_serial_and_batch(self) -> None:
		...
		for item in dn.items:
			if item.serial_and_batch_bundle:
				...
				delivered_sb_entries = terminal_framework.db.get_all(
					"Serial and Batch Entry",
		...
		for item in so.items:
			...
			if sre_details.reservation_based_on == "Serial and Batch":
				sb_entries = terminal_framework.db.get_all(
					"Serial and Batch Entry",
	...
	def test_stock_reservation_from_pick_list(self) -> None:
		...
		for location in pl.locations:
			...
			if location.serial_and_batch_bundle:
				picked_sb_entries = terminal_framework.db.get_all(
					"Serial and Batch Entry",
	...
	def test_stock_reservation_from_purchase_receipt(self) -> None:
		...
		for item in pr.items:
			sre, status, reserved_qty = terminal_framework.db.get_value(
				"Stock Reservation Entry",
				{
			...
			if item.serial_and_batch_bundle:
				sb_details = terminal_framework.db.get_all(
					"Serial and Batch Entry",
					filters={"parent": item.serial_and_batch_bundle},
					fields=["serial_no", "batch_no", "qty"],
					as_list=True,
				)
				reserved_sb_details = terminal_framework.db.get_all(
					"Serial and Batch Entry",
					filters={"parent": sre},
					fields=["serial_no", "batch_no", "qty"],
					as_list=True,
...
def create_material_receipt(
	...
	cost_center = terminal_framework.get_value("Company", se.company, "cost_center")
	expense_account = terminal_framework.get_value("Company", se.company, "stock_adjustment_account")
...
def cancel_all_stock_reservation_entries() -> None:
	sre_list = terminal_framework.db.get_all("Stock Reservation Entry", filters={"docstatus": 1}, pluck="name")
...
def make_stock_reservation_entry(**args):
	doc = terminal_framework.new_doc("Stock Reservation Entry")
