...
from terminal_framework.query_builder.functions import Max, Min, Sum
from terminal_framework.utils import cint, flt, nowdate, nowtime, parse_json

from terminal_erp.stock.utils import get_or_make_bin, get_stock_balance
...
class StockReservationEntry(Document):
	...
	def validate(self) -> None:
		from terminal_erp.stock.utils import validate_disabled_warehouse, validate_warehouse_company
	...
	def validate_reserved_entries(self):
		entries = terminal_framework.get_all(
		...
		if entries:
			...
			terminal_framework.throw(
				_(
					...
					", ".join([terminal_framework.bold(entry.name) for entry in entries]),
					", ".join([terminal_framework.bold(wo.name) for wo in work_orders]),
				)
			)
	...
	def validate_amended_doc(self) -> None:
		...
		if self.amended_from:
			msg = _("Cannot amend {0} {1}, please create a new one instead.").format(
				self.doctype, terminal_framework.bold(self.amended_from)
			)
	...
	def validate_group_warehouse(self) -> None:
		...
		if terminal_framework.get_cached_value("Warehouse", self.warehouse, "is_group"):
			msg = _("Stock cannot be reserved in group warehouse {0}.").format(terminal_framework.bold(self.warehouse))
			terminal_framework.throw(msg, title=_("Invalid Warehouse"))

	def validate_uom_is_integer(self) -> None:
		"""Validates `Reserved Qty` with Stock UOM."""

		if cint(terminal_framework.db.get_value("UOM", self.stock_uom, "must_be_whole_number", cache=True)):
			if cint(self.reserved_qty) != flt(self.reserved_qty, self.precision("reserved_qty")):
				msg = _(
					...
					terminal_framework.bold(_("Must be Whole Number")),
					terminal_framework.bold(self.stock_uom),
				)
				terminal_framework.throw(msg)
	...
	def auto_reserve_serial_and_batch(self, based_on: str | None = None) -> None:
		...
		if (
			...
			and terminal_framework.get_single_value("Stock Settings", "auto_reserve_serial_and_batch")
			...
			from terminal_erp.stock.serial_batch_bundle import get_serial_nos_batch
			...
			kwargs = terminal_framework._dict(
				{
					...
					"based_on": based_on
					or terminal_framework.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
				}
			)
			...
			if serial_nos:
				serial_no_wise_batch = terminal_framework._dict({})
	...
	def validate_reservation_based_on_serial_and_batch(self) -> None:
		...
		if self.reservation_based_on == "Serial and Batch":
			allow_partial_reservation = terminal_framework.get_single_value("Stock Settings", "allow_partial_reservation")
			...
			if self.has_serial_no:
				...
				if not available_serial_nos:
					msg = _("Stock not available for Item {0} in Warehouse {1}.").format(
						terminal_framework.bold(self.item_code), terminal_framework.bold(self.warehouse)
					)
					terminal_framework.throw(msg)
				...
				if self.has_serial_no:
					...
					if key not in available_serial_nos:
						msg = _(
							...
							entry.idx,
							terminal_framework.bold(entry.serial_no),
							terminal_framework.bold(self.item_code),
							_("Batch {0} and Warehouse").format(terminal_framework.bold(entry.batch_no))
							if self.has_batch_no
							else _("Warehouse"),
							terminal_framework.bold(self.warehouse),
							terminal_framework.bold(_("Stock Reservation Entry")),
						)
					...
					if entry.serial_no in selected_serial_nos:
						msg = _("Row #{0}: Serial No {1} is already selected.").format(
							entry.idx, terminal_framework.bold(entry.serial_no)
						)
						terminal_framework.throw(msg)
					else:
					...
					if cint(terminal_framework.db.get_value("Batch", entry.batch_no, "disabled")):
						msg = _(
							"Row #{0}: Stock cannot be reserved for Item {1} against a disabled Batch {2}."
						).format(entry.idx, terminal_framework.bold(self.item_code), terminal_framework.bold(entry.batch_no))
						terminal_framework.throw(msg)
					...
					if available_qty_to_reserve <= 0:
						msg = _(
							...
							entry.idx,
							terminal_framework.bold(self.item_code),
							terminal_framework.bold(entry.batch_no),
							terminal_framework.bold(self.warehouse),
						)
						terminal_framework.throw(msg)
					...
					if entry.qty > available_qty_to_reserve:
						if allow_partial_reservation:
							...
							msg = _(
								...
								entry.idx,
								terminal_framework.bold(available_qty_to_reserve),
								terminal_framework.bold(self.item_code),
								terminal_framework.bold(entry.batch_no),
								terminal_framework.bold(self.warehouse),
							)
					...
					if entry.batch_no in selected_batch_nos:
						msg = _("Row #{0}: Batch No {1} is already selected.").format(
							entry.idx, terminal_framework.bold(entry.batch_no)
						)
						terminal_framework.throw(msg)
					else:
	...
	def update_reserved_qty_in_voucher(
		...
		if item_doctype:
			...
			if self.voucher_type == "Production Plan" and terminal_framework.db.exists(
	...
	def update_reserved_stock_in_bin(self) -> None:
		...
		bin_doc = terminal_framework.get_cached_doc("Bin", bin_name)
	...
	def update_status(self, status: str | None = None, update_modified: bool = True) -> None:
		...
		terminal_framework.db.set_value(self.doctype, self.name, "status", status, update_modified=update_modified)
	...
	def validate_with_allowed_qty(self, qty_to_be_reserved: float) -> None:
		...
		if self.voucher_type == "Sales Order":
			...
			if not terminal_framework.db.exists("Sales Order Item", self.voucher_detail_no):
				voucher_detail_no = terminal_framework.get_value(
					"Packed Item", self.voucher_detail_no, "parent_detail_docname"
				)

			delivered_qty, conversion_factor = terminal_framework.db.get_value(
		...
		if self.get("_action") != "submit" and self.voucher_type == "Sales Order" and allowed_qty <= 0:
			msg = _("Item {0} is already reserved/delivered against Sales Order {1}.").format(
				terminal_framework.bold(self.item_code), terminal_framework.bold(self.voucher_no)
			)

			if self.docstatus == 1:
				self.cancel()
				return terminal_framework.msgprint(msg)
			else:
				terminal_framework.throw(msg)
		...
		if qty_to_be_reserved > allowed_qty:
			...
			msg = _(
				...
				terminal_framework.bold(allowed_qty),
				self.stock_uom,
				terminal_framework.bold(self.item_code),
				self.voucher_type,
				terminal_framework.bold(self.voucher_no),
				actual_qty,
...
def validate_stock_reservation_settings(voucher: object) -> None:
	...
	if not terminal_framework.get_single_value("Stock Settings", "enable_stock_reservation"):
		msg = _("Please enable {0} in the {1}.").format(
			terminal_framework.bold(_("Stock Reservation")),
			terminal_framework.bold(_("Stock Settings")),
		)
		terminal_framework.throw(msg)
...
def get_available_qty_to_reserve(
	...
	if available_qty:
		...
		if terminal_framework.db.db_type == "postgres":
			terminal_framework.qb.from_(sre).select(sre.name).where(conditions).for_update().run()
...
def get_sre_reserved_qty_details_for_voucher(voucher_type: str, voucher_no: str) -> dict:
	...
	return terminal_framework._dict(data)
...
def get_sre_reserved_serial_nos_details(
	...
	return terminal_framework._dict(query.run())
...
def get_ssb_bundle_for_voucher(sre_list) -> object:
	...
	if sb_entries:
		bundle = terminal_framework.new_doc("Serial and Batch Bundle")
		...
		if terminal_framework.flags.in_test:
...
class StockReservation:
	def __init__(self, doc, items=None, kwargs=None):
		if isinstance(doc, str):
			doc = parse_json(doc)
			doc = terminal_framework.get_doc("Work Order", doc.get("name"))
	...
	def cancel_stock_reservation_entries(self, names=None) -> None:
		...
		if sre_names:
			for sre_name in sre_names:
				sre_doc = terminal_framework.get_doc("Stock Reservation Entry", sre_name.name)
				sre_doc.cancel()

		if sre_names and names:
			terminal_framework.msgprint(
				_("Stock has been unreserved for work order {0}.").format(terminal_framework.bold(self.doc.name)),
				alert=True,
			)
	...
	def make_stock_reservation_entries(self):
		...
		child_doctype = terminal_framework.scrub(self.doc.doctype + " Item")

		is_sre_created = False
		for item in items:
			sre = terminal_framework.new_doc("Stock Reservation Entry")
			if isinstance(item, dict):
				item = terminal_framework._dict(item)
			...
			if (
				...
				terminal_framework.throw(
					_("Source Warehouse is mandatory for the Item {0}.").format(terminal_framework.bold(item.item_code))
				)
	...
	def set_serial_batch(self, sre, serial_batch_bundles):
		bundle_details = terminal_framework.get_all(
	...
	def throw_stock_not_exists_error(self, idx, item_code, warehouse):
		terminal_framework.msgprint(
			_("Row #{0}: Stock not available to reserve for the Item {1} in Warehouse {2}.").format(
				idx, terminal_framework.bold(item_code), terminal_framework.bold(warehouse)
			),
	...
	def transfer_reservation_entries_to(
		...
		entries_to_reserve = terminal_framework._dict()
		...
		for entry in [
			...
			if not available_qty:
				terminal_framework.msgprint(
					_("No available quantity to reserve for item {0} in warehouse {1}").format(
						terminal_framework.bold(entry.item_code), terminal_framework.bold(entry.warehouse)
					),
					alert=True,
				)
				continue
			...
			sre.has_serial_no = terminal_framework.get_value("Item", sre.item_code, "has_serial_no")
			sre.has_batch_no = terminal_framework.get_value("Item", sre.item_code, "has_batch_no")
			sre.insert()
			sre.submit()
	...
	def make_stock_reservation_entry(self, row):
		...
		if row.serial_nos:
			for serial_no in row.serial_nos:
				batch_no = None
				if row.batches:
					batch_no = terminal_framework.db.get_value("Serial No", serial_no, "batch_no")
	...
	def get_reserved_entries(self, doctype, docnames, against_fg_item=None):
		...
		sre = terminal_framework.qb.DocType("Stock Reservation Entry")
		sabb_entry = terminal_framework.qb.DocType("Serial and Batch Entry")
	...
	def get_items_to_reserve(self, docnames, from_doctype, to_doctype):
		field = terminal_framework.scrub(from_doctype)
		...
		doctype = terminal_framework.qb.DocType(to_doctype)
		child_doctype = terminal_framework.qb.DocType(to_doctype + child_table_suffix)
...
def create_stock_reservation_entries_for_so_items(
	...
	from terminal_erp.selling.doctype.sales_order.services.reservation import get_unreserved_qty

	if not from_voucher_type and (
		sales_order.get("_action") == "submit"
		and sales_order.set_warehouse
		and cint(terminal_framework.get_cached_value("Warehouse", sales_order.set_warehouse, "is_group"))
	):
		return terminal_framework.msgprint(
			_("Stock cannot be reserved in the group warehouse {0}.").format(
				terminal_framework.bold(sales_order.set_warehouse)
			)
		)
	...
	if items_details:
		for item in items_details:
			so_item = terminal_framework.get_doc("Sales Order Item", item.get("sales_order_item"))
	...
	for item in items if items_details else sales_order.get("items"):
		...
		is_stock_item, has_serial_no, has_batch_no = terminal_framework.get_cached_value(
		...
		if terminal_framework.get_cached_value("Warehouse", item.warehouse, "is_group"):
			terminal_framework.msgprint(
				_("Row #{0}: Stock cannot be reserved in group warehouse {1}.").format(
					item.idx, terminal_framework.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="yellow",
			)
		...
		if available_qty_to_reserve <= 0:
			terminal_framework.msgprint(
				_("Row #{0}: Stock not available to reserve for the Item {1} in Warehouse {2}.").format(
					item.idx, terminal_framework.bold(item.item_code), terminal_framework.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="orange",
			)
		...
		if qty_to_be_reserved < unreserved_qty:
			if not from_voucher_type and (
				...
				msg = _("Row #{0}: Only {1} available to reserve for the Item {2}").format(
					item.idx,
					terminal_framework.bold(str(qty_to_be_reserved / item.conversion_factor) + " " + item.uom),
					terminal_framework.bold(item.item_code),
				)
				terminal_framework.msgprint(msg, title=_("Stock Reservation"), indicator="orange")
			...
			if not allow_partial_reservation:
				if qty_to_be_reserved == flt(item.get("qty_to_reserve")):
					...
					terminal_framework.msgprint(msg, title=_("Partial Stock Reservation"), indicator="yellow")
		...
		if item.get("serial_and_batch_bundle"):
			sbb = terminal_framework.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
			sre.reservation_based_on = "Serial and Batch"
...
def cancel_stock_reservation_entries(
	...
	if sre_list:
		for sre in sre_list:
			terminal_framework.get_doc("Stock Reservation Entry", sre).cancel()

		if notify:
			msg = _("Stock Reservation Entries Cancelled")
			terminal_framework.msgprint(msg, alert=True, indicator="red")
...
def update_serial_batch_delivered_qty(row, name, is_cancelled=False):
	if row.serial_nos:
		doctype = terminal_framework.qb.DocType("Serial and Batch Entry")
		query = (
			terminal_framework.qb.update(doctype)
...
def get_reserved_materials(voucher_no):
	doctype = terminal_framework.qb.DocType("Stock Reservation Entry")
	serial_batch_doc = terminal_framework.qb.DocType("Serial and Batch Entry")
