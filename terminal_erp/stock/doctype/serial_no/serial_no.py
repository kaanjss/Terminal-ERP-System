...
from terminal_framework import ValidationError, _
from terminal_framework.model.naming import make_autoname
from terminal_framework.query_builder.functions import Coalesce
from terminal_framework.utils import cint, cstr, getdate, nowdate, safe_json_loads
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = terminal_framework.db.get_value("Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				terminal_framework.throw(_("Item Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				terminal_framework.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
	...
	def on_trash(self):
		sl_entries = terminal_framework.get_all(
			"Stock Ledger Entry",
...
def get_new_serial_number(series):
	...
	if terminal_framework.db.exists("Serial No", sr_no):
...
def get_serial_nos_from_sle_list(bundles):
	...
	query = terminal_framework.qb.from_(table).select(table.parent, table.serial_no).where(table.parent.isin(bundles))
...
def update_maintenance_status():
	...
	for serial_no in serial_nos:
		doc = terminal_framework.get_doc("Serial No", serial_no)
		doc.set_maintenance_status()
		terminal_framework.db.set_value("Serial No", doc.name, "maintenance_status", doc.maintenance_status)
...
def auto_fetch_serial_number(
	...
	filters = terminal_framework._dict({"item_code": item_code, "warehouse": warehouse})
...
def get_pos_reserved_serial_nos(filters: str | dict):
	...
	POSInvoice = terminal_framework.qb.DocType("POS Invoice")
	POSInvoiceItem = terminal_framework.qb.DocType("POS Invoice Item")
	query = (
		terminal_framework.qb.from_(POSInvoice)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	query = (
		terminal_framework.qb.from_(serial_no)
...
def on_doctype_update():
	terminal_framework.db.add_index("Serial No", ["item_code", "warehouse"])
