...
from terminal_framework.utils import cint, flt, get_link_to_form, get_number_format_info

from terminal_erp.stock.doctype.quality_inspection_template.quality_inspection_template import (
...
class QualityInspection(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.stock.doctype.quality_inspection_reading.quality_inspection_reading import (
	...
	def validate(self):
		...
		if self.inspection_type == "In Process" and self.reference_type == "Job Card":
			item_qi_template = terminal_framework.db.get_value("Item", self.item_code, "quality_inspection_template")
	...
	def set_company(self):
		if self.reference_type and self.reference_name:
			company = terminal_framework.get_cached_value(self.reference_type, self.reference_name, "company")
	...
	def set_child_row_reference(self):
		...
		child_doc = terminal_framework.qb.DocType(doctype)
		qi_doc = terminal_framework.qb.DocType("Quality Inspection")
	...
	def validate_inspection_required(self):
		if terminal_framework.db.get_single_value(
			"Stock Settings", "allow_to_make_quality_inspection_after_purchase_or_delivery"
		):
			return

		if self.reference_type in ["Purchase Receipt", "Purchase Invoice"] and not terminal_framework.get_cached_value(
		...
		if self.reference_type in ["Delivery Note", "Sales Invoice"] and not terminal_framework.get_cached_value(
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = terminal_framework.db.get_value(
		...
		for d in parameters:
			...
			child.parameter_group = terminal_framework.get_value(
	...
	def get_quality_inspection_template(self):
		template = ""
		if self.bom_no:
			template = terminal_framework.db.get_value("BOM", self.bom_no, "quality_inspection_template")

		if not template:
			template = terminal_framework.db.get_value("BOM", self.item_code, "quality_inspection_template")

		self.quality_inspection_template = template
		self.get_item_specification_details()

	def on_update(self):
		action_if_qi_in_draft = terminal_framework.db.get_single_value(
	...
	def on_submit(self):
		if (
			terminal_framework.db.get_single_value("Stock Settings", "action_if_quality_inspection_is_not_submitted")
			== "Stop"
		):
			self.update_qc_reference()
	...
	def validate_readings_status_mandatory(self):
		for reading in self.readings:
			if not reading.status:
				terminal_framework.throw(_("Row #{0}: Status is mandatory").format(reading.idx))
	...
	def update_qc_reference(self, remove_reference=False):
		...
		if self.reference_type == "Job Card":
			if self.reference_name:
				ref = terminal_framework.qb.DocType(self.reference_type)
				(
					terminal_framework.qb.update(ref)
			...
			if doctype and self.reference_name:
				...
				query = (
					terminal_framework.qb.update(child_doc)
	...
	def set_status_based_on_acceptance_formula(self, reading):
		...
		try:
			result = terminal_framework.safe_eval(condition, None, data)
			reading.status = "Accepted" if result else "Rejected"
		except NameError as e:
			field = terminal_framework.bold(e.args[0].split()[1])
			terminal_framework.throw(
				_(
...
def item_query(doctype: Any, txt: str | None, searchfield: Any, start: int, page_len: int, filters: dict):
	...
	if not reference_doctype:
		...
		production_item, item_name = terminal_framework.get_value(
		...
		query = terminal_framework.get_query(
			reference_doctype,
		...
		# terminal_framework's db_query drops ORDER BY for a distinct query on Postgres, which (with offset/limit)
		...
		items_field = terminal_framework.get_meta(reference_doctype).get_field("items")
		if items_field:
			query = query.orderby(terminal_framework.qb.DocType(items_field.options).item_code)
		return query.run()
...
def make_quality_inspection(source_name: str, target_doc: Document | str | None = None):
	def postprocess(source, doc):
		doc.inspected_by = terminal_framework.session.user
