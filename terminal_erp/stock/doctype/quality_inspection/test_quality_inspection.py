...
class TestQualityInspection(Terminal ERPTestSuite):
	def setUp(self):
		super().setUp()
		create_item("_Test Item with QA")
		terminal_framework.db.set_value("Item", "_Test Item with QA", "inspection_required_before_delivery", 1)
	...
	def test_qa_for_delivery(self):
		...
		terminal_framework.db.set_value("Quality Inspection", qa.name, "status", "Accepted")
		dn.reload()
		dn.submit()
	...
	def test_make_quality_inspections_from_linked_document(self):
		...
		for qi in quality_inspections:
			terminal_framework.delete_doc("Quality Inspection", qi)
		dn.delete()
	...
	def test_rejected_qi_validation(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Stop")
		se.reload()
		self.assertRaises(
			QualityInspectionRejectedError, se.submit
		)  # when blocked in Stock settings, block rejected QI

		terminal_framework.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Warn")
		se.reload()
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"number_format": "#.###,##"})
	def test_diff_number_format(self):
		self.assertEqual(terminal_framework.db.get_default("number_format"), "#.###,##")  # sanity check
	...
	def test_qi_updates_job_card_reference(self):
		...
		# keep it untouched.
		other_item = create_item("_Test Item for QC " + terminal_framework.utils.random_string(6)).name
		...
		self.assertEqual(
			terminal_framework.db.get_value("Job Card", matching_jc, "quality_inspection"),
			qa.name,
		)
		# The production_item filter excluded the Job Card with a different item.
		self.assertFalse(terminal_framework.db.get_value("Job Card", non_matching_jc, "quality_inspection"))
	...
	def test_qi_job_card_reference_respects_production_item(self):
		...
		create_item("_Test Item")
		mismatch_item = create_item("_Test Item Mismatch QC " + terminal_framework.utils.random_string(6)).name
		...
		self.assertFalse(terminal_framework.db.get_value("Job Card", jc, "quality_inspection"))
...
def make_minimal_job_card(production_item):
	...
	jc.name = "_T-Job Card-" + terminal_framework.utils.random_string(10)
...
def create_quality_inspection(**args):
	...
	qa = terminal_framework.new_doc("Quality Inspection")
	...
	qa.sample_size = 1
	qa.inspected_by = terminal_framework.session.user
...
def create_quality_inspection_parameter(parameter):
	if not terminal_framework.db.exists("Quality Inspection Parameter", parameter):
