...
class TestFIFOValuation(Terminal ERPTestSuite):
	def setUp(self):
...
class TestLIFOValuation(Terminal ERPTestSuite):
	def setUp(self):
...
class TestLIFOValuationSLE(Terminal ERPTestSuite):
	...
	def assertStockQueue(self, se, expected_queue):
		sle_name = terminal_framework.db.get_value(
			"Stock Ledger Entry", {"voucher_no": se.name, "is_cancelled": 0, "voucher_type": "Stock Entry"}
		)
		sle = terminal_framework.get_doc("Stock Ledger Entry", sle_name)
