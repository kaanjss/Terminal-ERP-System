...
class StockTestMixin:
	...
	def assertGLEs(self, doc, expected_gles, gle_filters=None, order_by=None):
		...
		actual_gles = terminal_framework.get_all(
			"GL Entry",
			fields=["*"],
			filters=filters,
...
class TestStockUtilities(Terminal ERPTestSuite, StockTestMixin):
	def test_barcode_scanning(self):
		...
		batch = terminal_framework.get_doc(doctype="Batch", item=batch_item.name).insert()
		...
		serial = terminal_framework.get_doc(
			doctype="Serial No",
			item_code=serial_item.name,
			serial_no=terminal_framework.generate_hash(),
	...
	def test_barcode_scanning_of_warehouse(self):
		...
		warehouse_2 = terminal_framework.get_doc(
			{
				"doctype": "Warehouse",
	...
	def test_get_latest_stock_qty(self):
		...
		from terminal_erp.stock.utils import get_latest_stock_qty
	...
	def test_get_stock_value_from_bin(self):
		...
		from terminal_erp.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
		from terminal_erp.stock.utils import get_stock_value_from_bin
	...
	def test_get_avg_purchase_rate(self):
		...
		field (terminal_framework compiles `[{"AVG": "purchase_rate"}]` to `AVG(purchase_rate)` on both engines)."""
		from terminal_framework.utils import flt, random_string

		from terminal_erp.stock.utils import get_avg_purchase_rate
