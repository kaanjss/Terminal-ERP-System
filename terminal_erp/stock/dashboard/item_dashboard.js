...
terminal_erp.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(terminal_framework.render_template("item_dashboard")).appendTo(this.parent);
		...
		this.content.on("click", ".btn-edit", function () {
			...
			terminal_framework.db.get_value(
				...
				"name",
				(r) => {
					terminal_framework.set_route("Form", "Putaway Rule", r.name);
				}
			);
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					terminal_erp.stock.move_item(item, null, warehouse, actual_qty, rate, stock_uom, function () {
						me.refresh();
					});
				} else {
					terminal_erp.stock.move_item(item, warehouse, null, actual_qty, null, stock_uom, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type, stock_uom) {
			terminal_framework.model.with_doctype("Stock Entry", function () {
				var doc = terminal_framework.model.get_new_doc("Stock Entry");
				if (entry_type) {
					doc.stock_entry_type = entry_type;
				}

				var row = terminal_framework.model.add_child(doc, "items");
				row.item_code = item;
				row.uom = stock_uom;
	...
	render(data) {
		...
		if (context.data.length > 0) {
			...
			$(terminal_framework.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data(data, max_count, show_item) {
		...
		let can_write = 0;
		if (terminal_framework.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data(data) {
		...
		let can_write = 0;
		if (terminal_framework.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
terminal_erp.stock.move_item = function (item, source, target, actual_qty, rate, stock_uom, callback) {
	...
	dialog.set_primary_action(__("Create Stock Entry"), function () {
		if (flt(dialog.get_value("qty")) <= 0) {
			terminal_framework.msgprint(__("Quantity must be greater than zero"));
			return;
		}

		if (source && dialog.get_value("qty") > actual_qty) {
			terminal_framework.msgprint(__("Quantity must be less than or equal to {0}", [actual_qty]));
			return;
		}

		if (dialog.get_value("source") === dialog.get_value("target")) {
			terminal_framework.msgprint(__("Source and target warehouse must be different"));
			return;
		}

		terminal_framework.model.with_doctype("Stock Entry", function () {
			let doc = terminal_framework.model.get_new_doc("Stock Entry");
			doc.from_warehouse = dialog.get_value("source");
			doc.to_warehouse = dialog.get_value("target");
			doc.stock_entry_type = doc.from_warehouse ? "Material Transfer" : "Material Receipt";
			let row = terminal_framework.model.add_child(doc, "items");
