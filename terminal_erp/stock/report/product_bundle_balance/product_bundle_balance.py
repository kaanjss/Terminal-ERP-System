...
from terminal_framework.query_builder.functions import IfNull, Max
...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		for child_item in required_items:
			child_item_balance = stock_balance.get(child_item.item_code, terminal_framework._dict())
		...
		for warehouse, company in warehouse_company_map.items():
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, terminal_framework._dict()).get(
					warehouse, terminal_framework._dict()
				)
				child_row = {
					"indent": 1,
...
def get_items(filters):
	pb_details = terminal_framework._dict()
	item_details = terminal_framework._dict()

	item = terminal_framework.qb.DocType("Item")
	pb = terminal_framework.qb.DocType("Product Bundle")
	...
	if parent_items:
		...
		child_item_details = (
			terminal_framework.qb.from_(pbi)
			.inner_join(pb)
...
def get_stock_balance(filters, items):
	...
	stock_balance = terminal_framework._dict()
	for d in sle:
		stock_balance.setdefault(d.item_code, terminal_framework._dict())[d.warehouse] = d
