...
def fix_sabb_entries(selected_rows: str | list):
	...
	if isinstance(selected_rows, str):
		selected_rows = terminal_framework.parse_json(selected_rows)

	for row in selected_rows:
		doc = terminal_framework.get_doc("Serial and Batch Bundle", row.get("name"))
		doc.check_permission("write")
		if doc.is_cancelled == 0 and not terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		elif doc.is_cancelled == 1 and terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{"serial_and_batch_bundle": doc.name, "is_cancelled": 0},
			"name",
		):
	...
	terminal_framework.msgprint(_("Selected Serial and Batch Bundle entries have been fixed."))
