...
from terminal_framework.utils import add_to_date, cint, flt, get_datetime, getdate
...
def execute(filters=None):
	...
	sle_count = terminal_framework.db.estimate_count("Stock Ledger Entry")
...
def get_stock_ledger_entries_for_batch_no(filters):
	if not filters.get("from_date"):
		terminal_framework.throw(_("'From Date' is required"))
	if not filters.get("to_date"):
		terminal_framework.throw(_("'To Date' is required"))
...
def get_stock_ledger_entries_for_batch_bundle(filters):
	sle = terminal_framework.qb.DocType("Stock Ledger Entry")
	batch_package = terminal_framework.qb.DocType("Serial and Batch Entry")
...
def get_item_details(filters):
	item_map = {}
	for d in (terminal_framework.qb.from_("Item").select("name", "item_name", "description", "stock_uom")).run(as_dict=1):
