...
class ReportData:
	...
	def get_data(self):
		...
		if self.filters.get("traceability_direction") in ["Forward", "Both"]:
			data = self.get_serial_no_batches()
			batch_details = terminal_framework._dict({})
			for row in data:
	...
	def parse_batch_details(self, sabb_data_details, data, direction, indent=0):
		for key in sabb_data_details:
			...
			if sabb.reference_doctype == "Purchase Receipt":
				row["supplier"] = terminal_framework.db.get_value(
					"Purchase Receipt",
					sabb.reference_name,
					"supplier",
				)
			elif sabb.reference_doctype == "Stock Entry":
				row["work_order"] = terminal_framework.db.get_value(
					"Stock Entry",
					sabb.reference_name,
					"work_order",
				)
			elif sabb.reference_doctype == "Delivery Note":
				row["customer"] = terminal_framework.db.get_value(
					"Delivery Note",
					sabb.reference_name,
					"customer",
				)
	...
	def prepare_source_data(self, data):
		source_data = terminal_framework._dict({})
		for row in data:
			...
			if key not in source_data:
				row["raw_materials"] = terminal_framework._dict({})
				source_data[key] = row

		return source_data

	def get_data_from_sabb(self, row):
		sabb = terminal_framework.qb.DocType("Serial and Batch Bundle")
		sabb_entry = terminal_framework.qb.DocType("Serial and Batch Entry")

		query = (
			terminal_framework.qb.from_(sabb)
	...
	def set_backward_data(self, sabb_data, qty=None):
		...
		if "raw_materials" not in sabb_data:
			sabb_data.raw_materials = terminal_framework._dict({})
		...
		for material in materials:
			...
			if material.serial_no or material.batch_no:
				...
				if sub_key not in sabb_data.raw_materials:
					sabb_data.raw_materials[sub_key] = terminal_framework._dict(
						{
	...
	def get_serial_no_batches(self, name=None):
		...
		doctype = terminal_framework.qb.DocType(self.doctype_name)
	...
	def get_sabb_entries(self, value, type_of_transaction=None):
		...
		SABB = terminal_framework.qb.DocType("Serial and Batch Bundle")
		SABE = terminal_framework.qb.DocType("Serial and Batch Entry")
	...
	def process_manufacture_or_repack_entry(self, row, batch_details):
		ste = terminal_framework.db.get_value("Stock Entry", row.reference_name, ["purpose", "work_order"], as_dict=True)
				...
				fg_item.update(
					{
						...
						"raw_materials": terminal_framework._dict(
