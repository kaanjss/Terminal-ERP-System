...
from terminal_framework.utils import cint, flt, get_link_to_form, parse_json
...
def add_invariant_check_fields(sles, filters):
	...
	for idx, sle in enumerate(sles):
		...
		if (
			...
			balance_qty = terminal_framework.db.get_value("Stock Reconciliation Item", sle.voucher_detail_no, "qty")
		...
		if sle.batch_no:
			sle.use_batchwise_valuation = terminal_framework.db.get_value(
...
def create_reposting_entries(rows: str | list, item_code: str | None = None, warehouse: str | None = None):
	...
	for row in rows:
		...
		terminal_framework.db.savepoint("repost_invariant_check")
		try:
			...
			terminal_framework.db.rollback(save_point="repost_invariant_check")
			continue
