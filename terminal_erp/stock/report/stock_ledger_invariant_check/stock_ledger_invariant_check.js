...
terminal_framework.query_reports["Stock Ledger Invariant Check"] = {
	filters: [
		{
			fieldname: "item_code",
			fieldtype: "Link",
	...
	onload(report) {
		report.page.add_inner_button(__("Create Reposting Entry"), () => {
			...
			if (!selected_rows.length) {
				...
				terminal_framework.throw(__("Please select only one row to create a Reposting Entry"));
			} else {
				terminal_framework.confirm(message, () => {
					terminal_framework.call({
						method: "terminal_erp.stock.report.stock_ledger_invariant_check.stock_ledger_invariant_check.create_reposting_entries",
						args: {
							rows: selected_rows,
							item_code: terminal_framework.query_report.get_filter_values().item_code,
							warehouse: terminal_framework.query_report.get_filter_values().warehouse,
						},
					});
				});
			}
