...
from terminal_erp.stock.report.stock_ageing.stock_ageing import FIFOSlots, format_report_data, get_average_age
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestStockAgeing(Terminal ERPTestSuite):
	def setUp(self) -> None:
		self.filters = terminal_framework._dict(company="_Test Company", to_date="2021-12-10", ranges=["30", "60", "90"])
	...
	def test_item_filter_supports_multi_select_values(self):
		...
		query = terminal_framework.qb.from_(bundle).select(bundle.name)

		filtered_query = FIFOSlots(terminal_framework._dict(item_code=["Item A"]), [])._apply_filter(
	...
	def test_non_serial_stock_reco_decrease_preserves_ageing(self):
		...
		filters = terminal_framework._dict(company="_Test Company", to_date="2026-02-15", ranges=["30", "60", "90"])
		sle = [
		...
		def prepare_stock_reco_voucher_wise_count():
			fifo_slots.stock_reco_voucher_wise_count = terminal_framework._dict({"SRI-DECREASE": 100, "SRI-INCREASE": 60})
	...
	def test_repack_entry_same_item_split_rows(self):
		...
		"""
		sle = [
			terminal_framework._dict(  # stock up item
				name="Flask Item",
				actual_qty=500,
	...
	def test_batchwise_valuation(self):
		...
		def make_batch(batch_id, use_batchwise_valuation):
			...
			terminal_framework.db.set_value("Batch", batch_id, "use_batchwise_valuation", use_batchwise_valuation)
	...
	def test_batchwise_valuation_same_voucher_transfer(self):
		...
		def make_batch(batch_id):
			...
			terminal_framework.db.set_value("Batch", batch_id, "use_batchwise_valuation", 1)
	...
	def test_batchwise_valuation_negative_stock_same_voucher(self):
		...
		terminal_framework.db.set_value("Batch", batch_no, "use_batchwise_valuation", 1)
	...
	def test_batchwise_valuation_stock_reconciliation_with_bundle(self):
		...
		suffix = terminal_framework.generate_hash(length=8).upper()
		item_code = make_item(
	...
	def test_legacy_batch_no_sle_with_streaming_cursor(self):
		...
		with patch("terminal_framework.database.database.SQL_ITERATOR_BATCH_SIZE", 1):
			slots = fifo_slots.generate()
