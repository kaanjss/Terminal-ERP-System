...
from terminal_framework.query_builder.functions import Abs, Count
from terminal_framework.utils import cint, date_diff, flt, get_datetime
...
def get_float_precision() -> int:
	return cint(terminal_framework.db.get_single_value("System Settings", "float_precision", cache=True))
...
class FIFOSlots:
	...
	def _get_bundle_wise_details(self, stock_ledger_entries: list | None) -> tuple[dict, dict]:
		if stock_ledger_entries is not None:
			return terminal_framework._dict({}), terminal_framework._dict({})
	...
	def _get_serial_and_batch_nos(
		...
		from terminal_erp.stock.serial_batch_bundle import get_serial_nos_from_bundle
	...
	def _get_batchwise_valuation(self, batch_no: str):
		if batch_no not in self.batchwise_valuation_by_batch:
			...
			self.batchwise_valuation_by_batch[batch_no] = terminal_framework.db.get_value(
	...
	def _get_stock_ledger_entries(self) -> Iterator[dict]:
		...
		return sle_query.run(as_dict=True, as_iterator=terminal_framework.db.db_type != "postgres")

	def _get_bundle_wise_serial_nos(self) -> dict:
		bundle = terminal_framework.qb.DocType("Serial and Batch Bundle")
		entry = terminal_framework.qb.DocType("Serial and Batch Entry")
		...
		bundle_wise_serial_nos = terminal_framework._dict({})
	...
	def _get_bundle_wise_batch_nos(self, sabb_name=None) -> dict:
		...
		bundle_wise_batch_nos = terminal_framework._dict({})
	...
	def _get_item_query(self) -> str:
		...
		item = terminal_framework.qb.from_("Item").select(
			"name",
			"item_name",
			"description",
			"stock_uom",
