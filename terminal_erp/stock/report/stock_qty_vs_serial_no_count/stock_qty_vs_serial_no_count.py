...
def validate_warehouse(filters):
	...
	if not terminal_framework.db.exists("Warehouse", {"name": warehouse, "company": company}):
		terminal_framework.throw(_("Warehouse: {0} does not belong to {1}").format(warehouse, company))
...
def get_data(warehouse, show_disabled_items):
	...
	serial_item_list = terminal_framework.get_all(
		"Item",
		filters=filters,
	...
	for item in serial_item_list:
		total_serial_no = terminal_framework.db.count(
			"Serial No",
		...
		# terminal_framework.db.get_value returns null if no record exist.
		if not actual_qty:
			actual_qty = 0
