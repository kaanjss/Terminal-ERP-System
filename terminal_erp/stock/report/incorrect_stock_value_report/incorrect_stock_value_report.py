...
from terminal_framework.query_builder.functions import Min
...
from terminal_erp.stock.utils import get_stock_value_on
...
def get_unsync_date(filters):
	...
	if not date:
		date = (terminal_framework.qb.from_("Stock Ledger Entry").select(Min(Field("posting_date")))).run()
		date = date[0][0]
