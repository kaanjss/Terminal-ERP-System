...
from terminal_erp.stock.report.stock_balance.stock_balance import execute, get_stock_ageing_data
...
class TestStockBalance(Terminal ERPTestSuite):
	# ----------- utils
	...
	def setUp(self):
		self.item = terminal_framework.get_doc("Item", "_Test Item")
		self.filters = _dict(
			{
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"float_precision": 3, "currency_precision": 3})
	...
	def test_alt_uom_balance_uses_first_alternate_uom(self):
		...
		terminal_framework.get_doc({"doctype": "UOM", "uom_name": "Carton"}).insert(ignore_if_duplicate=True)
		item = self.make_alt_uom_item(
			uoms=[
