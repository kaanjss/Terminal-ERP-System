...
from terminal_framework.query_builder.functions import Coalesce, Count
from terminal_framework.utils import add_days, cint, date_diff, flt, getdate
...
from terminal_erp.stock.doctype.stock_closing_entry.stock_closing_entry import StockClosing
from terminal_erp.stock.doctype.warehouse.warehouse import apply_warehouse_filter
from terminal_erp.stock.report.stock_ageing.stock_ageing import (
	FIFOSlots,
	get_average_age,
	normalize_fifo_queue,
)
from terminal_erp.stock.utils import add_additional_uom_columns
...
class StockBalanceReport:
	...
	def set_company_currency(self) -> None:
		if self.filters.get("company"):
			self.company_currency = terminal_erp.get_company_currency(self.filters.get("company"))
		else:
			self.company_currency = terminal_framework.db.get_single_value("Global Defaults", "default_currency")

	def run(self):
		self.float_precision = cint(terminal_framework.db.get_default("float_precision")) or 3

		self.item_warehouse_map = terminal_framework._dict({})
	...
	def prepare_opening_stock(self) -> None:
		...
		for entry in opening_entries:
			...
			self.item_warehouse_map[key] = terminal_framework._dict(
				{
	...
	def prepare_item_warehouse_map_for_current_period(self):
		...
		_system_settings = terminal_framework.get_cached_doc("System Settings")
	...
	def prepare_stock_reco_voucher_wise_count(self):
		self.stock_reco_voucher_wise_count = terminal_framework._dict()
		...
		for row in data:
			...
			sr_item = terminal_framework.db.get_value(
	...
	def initialize_data(self, group_by_key, entry):
		self.item_warehouse_map[group_by_key] = terminal_framework._dict(
			{
				"item_code": entry.item_code,
				"warehouse": entry.warehouse,
				"item_group": entry.item_group,
	...
	def get_item_alt_uom_map(self) -> dict:
		...
		uom_detail = terminal_framework.qb.DocType("UOM Conversion Detail")
		item_table = terminal_framework.qb.DocType("Item")

		rows = (
			terminal_framework.qb.from_(uom_detail)
			.join(item_table)
	...
	def get_itemwise_conversion_factor(self):
		...
		table = terminal_framework.qb.DocType("UOM Conversion Detail")
		query = (
	...
	def get_variant_values_for(self):
		...
		attribute_info = terminal_framework.get_all(
	...
	def get_opening_vouchers(self):
		...
		sr = terminal_framework.qb.DocType("Stock Reconciliation")

		vouchers_data = (
			terminal_framework.qb.from_(
				(
				...
				+ (
					terminal_framework.qb.from_(sr)
...
def get_variants_attributes() -> list[str]:
	...
	return terminal_framework.get_all("Item Attribute", pluck="name")
