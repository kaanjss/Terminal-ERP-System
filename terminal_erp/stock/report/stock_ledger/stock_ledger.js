...
terminal_framework.query_reports["Stock Ledger"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			get_data: async function (txt) {
				let { message: data } = await terminal_framework.call({
					method: "terminal_erp.controllers.queries.item_query",
					args: {
						doctype: "Item",
						txt: txt,
						searchfield: "name",
		...
		},
		{
			...
			options: "Batch",
			on_change() {
				const batch_no = terminal_framework.query_report.get_filter_value("batch_no");
				if (batch_no) {
					terminal_framework.query_report.set_filter_value("segregate_serial_batch_bundle", 1);
				} else {
					terminal_framework.query_report.set_filter_value("segregate_serial_batch_bundle", 0);
				}
			},
		},
		{
	...
	onload: function (report) {
		report.page.add_inner_button(__("View Stock Balance"), function () {
			...
			terminal_framework.set_route("query-report", "Stock Balance", filters);
		});
	},
};

terminal_erp.utils.add_inventory_dimensions("Stock Ledger", 10);
