...
def update_batch_qty(selected_batches: str | list | None = None):
	terminal_framework.has_permission("Batch", "write", throw=True, ignore_share_permissions=True)
	if not selected_batches:
		return

	selected_batches = terminal_framework.parse_json(selected_batches)
	for row in selected_batches:
		...
		terminal_framework.db.set_value("Batch", batch_name, "batch_qty", batch_qty)

	terminal_framework.msgprint(_("Batch Qty updated successfully"), alert=True)
