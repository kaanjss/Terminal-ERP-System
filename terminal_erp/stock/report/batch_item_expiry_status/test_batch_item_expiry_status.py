...
from terminal_erp.stock.report.batch_item_expiry_status.batch_item_expiry_status import execute
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestBatchItemExpiryStatus(Terminal ERPTestSuite):
	...
	def test_batch_listed_with_balance(self):
		...
		batch_no = terminal_framework.db.get_value("Batch", {"item": item}, "name")
		...
		batch_expiry = terminal_framework.db.get_value("Batch", batch_no, "expiry_date")
		self.assertIsNotNone(row[5], "Expiry date should be set for a batch with shelf life")
		self.assertEqual(terminal_framework.utils.getdate(row[5]), terminal_framework.utils.getdate(batch_expiry))
		# Expiry (In Days) column = days until expiry
		expected_days = max((terminal_framework.utils.getdate(batch_expiry) - terminal_framework.utils.datetime.date.today()).days, 0)
