...
class TestStockAndAccountValueComparison(Terminal ERPTestSuite):
	def test_purchase_voucher_reposted_transaction_based(self):
		...
		terminal_framework.db.delete("GL Entry", {"voucher_type": "Purchase Receipt", "voucher_no": pr.name})

		# The receipt now shows up in the comparison report (stock value 500 vs account value 0).
		filters = terminal_framework._dict(company=PI_COMPANY, as_on_date=today())
		...
		transaction_rivs = terminal_framework.get_all(
		...
		item_wh_rivs = terminal_framework.get_all(
			"Repost Item Valuation",
