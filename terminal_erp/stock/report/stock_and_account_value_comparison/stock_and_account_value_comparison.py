...
from terminal_framework.utils import get_datetime, get_link_to_form, parse_json

import terminal_erp
from terminal_erp.accounts.utils import get_currency_precision, get_stock_accounts
from terminal_erp.stock.doctype.stock_reposting_settings.stock_reposting_settings import get_stock_ledgers
from terminal_erp.stock.doctype.warehouse.warehouse import get_warehouses_based_on_account


def execute(filters=None):
	if not terminal_erp.is_perpetual_inventory_enabled(filters.company):
...
def create_reposting_entries(rows: str | list, company: str):
	...
	item_wh = terminal_framework._dict()
	vouchers = [
		row.get("voucher_no")
		for row in rows
	...
	for key, sle in item_wh.items():
		...
		terminal_framework.db.savepoint("repost_value_comparison")
		try:
			...
			terminal_framework.db.rollback(save_point="repost_value_comparison")

	if entries:
		entries = ", ".join(entries)
		terminal_framework.msgprint(_("Reposting entries created: {0}").format(entries))
...
def repost_based_on_transaction(rows, company=None, entries=None):
	...
	for row in rows:
		if (
			...
			and terminal_framework.get_cached_value("Purchase Invoice", row.get("voucher_no"), "update_stock") == 0
		):
			continue
