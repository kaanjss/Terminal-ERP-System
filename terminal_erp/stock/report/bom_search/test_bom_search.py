...
from terminal_erp.stock.report.bom_search.bom_search import execute
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestBomSearch(Terminal ERPTestSuite):
	def run_report(self, **extra):
		filters = terminal_framework._dict({"search_sub_assemblies": 0})
	...
	def test_bom_found_by_contained_item(self):
		...
		bom = terminal_framework.get_doc(doctype="BOM", item=finished_good, company="_Test Company", currency="INR")
	...
	def test_search_sub_assemblies_finds_top_level_bom(self):
		...
		top_bom = terminal_framework.get_doc(
