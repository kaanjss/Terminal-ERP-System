...
from terminal_erp.stock.report.stock_projected_qty.stock_projected_qty import execute
...
class TestStockProjectedQty(Terminal ERPTestSuite):
	"""Correctness tests for the Stock Projected Qty report (a current-Bin snapshot)."""

	def run_report(self, item_code, warehouse=None):
		filters = terminal_framework._dict(company="_Test Company", item_code=item_code)
		if warehouse:
	...
	def test_projected_qty_includes_all_quantity_components(self):
		...
		bin_doc = terminal_framework.get_doc("Bin", {"item_code": item, "warehouse": WAREHOUSE})
		bin_doc.update(
			{
				"actual_qty": 100,
				"ordered_qty": 50,
	...
	def test_shortage_qty_from_reorder_level(self):
		item = "_Test Item"
		doc = terminal_framework.get_doc("Item", item)
		doc.append(
			"reorder_levels",
			{
				"warehouse": WAREHOUSE,
