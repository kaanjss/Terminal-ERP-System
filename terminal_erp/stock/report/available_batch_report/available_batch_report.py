...
from terminal_framework.utils import flt, get_datetime, today
...
def get_batchwise_data_from_stock_ledger(filters):
	batchwise_data = terminal_framework._dict({})
...
def get_batchwise_data_from_serial_batch_bundle(batchwise_data, filters):
	table = terminal_framework.qb.DocType("Stock Ledger Entry")
	ch_table = terminal_framework.qb.DocType("Serial and Batch Entry")
	batch = terminal_framework.qb.DocType("Batch")
...
def get_query_based_on_filters(query, batch, table, filters):
	...
	if filters.warehouse:
		lft, rgt = terminal_framework.db.get_value("Warehouse", filters.warehouse, ["lft", "rgt"])
