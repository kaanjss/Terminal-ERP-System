...
class StockLedgerService:
	...
	def get_sl_entries(self, d, args):
		sl_dict = terminal_framework._dict(
			{
				...
				"stock_uom": terminal_framework.get_cached_value(
			...
			if not sl_dict.get(dimension.target_fieldname) and dimension.fetch_from_parent:
				...
				if not sl_dict.get(dimension.target_fieldname):
					fieldname = next(
						(
							field.fieldname
							for field in terminal_framework.get_meta(self.doc.doctype).fields
	...
	def make_sl_entries(self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
		from terminal_erp.stock.serial_batch_bundle import update_batch_qty
		from terminal_erp.stock.services.serial_batch_bundle_service import SerialBatchBundleService
		from terminal_erp.stock.stock_ledger import make_sl_entries
	...
	def repost_future_sle_and_gle(self, force=False, via_landed_cost_voucher=False):
		...
		if force or future_sle_exists(args) or repost_required_for_queue(self.doc):
			item_based_reposting = terminal_framework.get_single_value("Stock Reposting Settings", "item_based_reposting")
