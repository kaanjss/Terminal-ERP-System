	...
	def compose(
		...
		for item_row in voucher_details:
			...
			if abs(sle_rounding_diff) > (1.0 / (10**precision)) and doc.is_internal_transfer():
				...
				expense_account = terminal_framework.get_cached_value("Company", doc.company, "default_expense_account")
				if not expense_account:
					terminal_framework.throw(
						...
						).format(terminal_framework.bold(doc.company))
					)
		...
		if warehouse_with_no_account:
			for wh in warehouse_with_no_account:
				if terminal_framework.get_cached_value("Warehouse", wh, "company"):
					terminal_framework.throw(
						_(
		...
		return process_gl_map(
			gl_list, precision=precision, from_repost=terminal_framework.flags.through_repost_item_valuation
		)

	def get_debit_field_precision(self):
		if not terminal_framework.flags.debit_field_precision:
			terminal_framework.flags.debit_field_precision = terminal_framework.get_precision("GL Entry", "debit_in_account_currency")

		return terminal_framework.flags.debit_field_precision

	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		doc = self.doc
		if doc.doctype == "Stock Reconciliation":
			reconciliation_purpose = terminal_framework.db.get_value(doc.doctype, doc.name, "purpose")
		...
		if not item.get("expense_account"):
			...
			terminal_framework.throw(
				_("Row #{0}: Expense Account not set for the Item {1}. {2}").format(
					item.idx, terminal_framework.bold(item.item_code), msg
				),
			...
			is_expense_account = (
				terminal_framework.get_cached_value("Account", item.get("expense_account"), "report_type")
				== "Profit and Loss"
			)
			if (
				self.doc.doctype
