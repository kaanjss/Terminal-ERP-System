...
class StockInternalTransferService:
	...
	def validate_multi_currency(self):
		if self.doc.currency != self.doc.company_currency:
			terminal_framework.throw(_("Internal transfers can only be done in company's default currency"))

	def validate_packed_items(self):
		if self.doc.doctype in ("Sales Invoice", "Delivery Note Item") and self.doc.get("packed_items"):
			terminal_framework.throw(_("Packed Items cannot be transferred internally"))
	...
	def validate_internal_transfer_qty(self):
		...
		precision = terminal_framework.get_precision(self.doc.doctype + " Item", "qty")

		over_receipt_allowance = terminal_framework.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
	...
	def get_item_wise_inter_transfer_qty(self, inter_company_reference):
		...
		parent_tab = terminal_framework.qb.DocType(parent_doctype)
		child_tab = terminal_framework.qb.DocType(child_doctype)
	...
	def get_item_wise_inter_received_qty(self):
		...
		parent_tab = terminal_framework.qb.DocType(self.doc.doctype)
		child_tab = terminal_framework.qb.DocType(child_doctype)

		query = (
			terminal_framework.qb.from_(self.doc.doctype)
			.inner_join(child_tab)
			.on(child_tab.parent == parent_tab.name)
			.select(
				child_tab.item_code,
