...
from terminal_framework.utils import cstr, flt, get_link_to_form, getdate
...
class SerialBatchBundleService:
	...
	def validate_warehouse_of_sabb(self):
		...
		for row in self.doc.items:
			...
			sabb_details = terminal_framework.db.get_value(
			...
			if sabb_details.has_serial_no and doc_before_save and doc_before_save.get("items"):
				...
				if prev_row and prev_row[0].serial_and_batch_bundle != row.serial_and_batch_bundle:
					sabb_doc = terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
			...
			if data:
				...
				terminal_framework.throw(
					_("Serial and Batch Bundle {0} is already used in {1} {2}.").format(
						terminal_framework.bold(data.serial_and_batch_bundle), data.voucher_type, data.voucher_no
					)
				)

	def validate_serialized_batch(self):
		from terminal_erp.exceptions import BatchExpiredError
		...
		for d in self.doc.get("items"):
			...
			if (
				...
				expiry_date = terminal_framework.get_cached_value("Batch", d.get("batch_no"), "expiry_date")
	...
	def clean_serial_nos(self):
		from terminal_erp.stock.doctype.serial_no.serial_no import clean_serial_no_string
	...
	def make_bundle_using_old_serial_batch_fields(self, table_name=None, via_landed_cost_voucher=False):
		...
		if terminal_framework.in_test and terminal_framework.flags.use_serial_and_batch_fields:
			return
		...
		parent_details = terminal_framework._dict()
				...
				if not data.get("qty"):
					terminal_framework.throw(
						_("For the {0}, no stock is available for the return in the warehouse {1}.").format(
							terminal_framework.bold(row.item_code), row.get(warehouse_field)
						)
					)
		...
		for row in self.doc.get(table_name):
			...
			if data := available_dict.get(value):
				...
				if self.doc.doctype in ["Sales Invoice", "Delivery Note"]:
					row.db_set(
						"incoming_rate", terminal_framework.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
					)
	...
	def is_serial_batch_item(self, item_code) -> bool:
		...
		if not item_details:
			terminal_framework.throw(_("Item {0} does not exist.").format(bold(item_code)))
	...
	def update_bundle_details(self, bundle_details, table_name, row, is_rejected=False, parent_details=None):
		...
		if is_rejected:
			...
			qty = flt(
				...
				terminal_framework.get_precision("Serial and Batch Entry", "qty"),
			)
		...
		bundle_details.update(
			{
				...
				"warehouse": warehouse,
				"batches": terminal_framework._dict({row.batch_no: qty}) if row.batch_no else None,
	...
	def set_use_serial_batch_fields(self):
		if terminal_framework.get_single_value("Stock Settings", "use_serial_batch_fields"):
			for row in self.doc.items:
				row.use_serial_batch_fields = 1
	...
	def set_serial_and_batch_bundle(self, table_name=None, ignore_validate=False):
		...
		for row in self.doc.get(table_name):
			for field in QTY_FIELD.keys():
				if row.get(field):
					terminal_framework.get_doc("Serial and Batch Bundle", row.get(field)).set_serial_and_batch_values(
	...
	def validate_reserved_batches(self):
		if not terminal_framework.db.get_single_value("Stock Settings", "enable_stock_reservation"):
			return
				...
				for row in reserved_batches_data:
					...
					terminal_framework.throw(
						_(
							...
							terminal_framework.bold(row.batch_no),
							terminal_framework.bold(row.voucher_type),
							terminal_framework.bold(row.voucher_no),
							terminal_framework.bold(self.doc.doctype),
							terminal_framework.bold(self.doc.name),
							terminal_framework.bold(field[0]),
							terminal_framework.bold(value),
						),
	...
	def get_reserved_batches(self, batches):
		...
		child_doc = terminal_framework.qb.DocType("Serial and Batch Entry")
