...
from terminal_framework.utils import cint, cstr, flt, getdate

from terminal_erp.stock.doctype.item.item import get_last_purchase_details, validate_end_of_life
	...
	for d in doc.get("items"):
		...
		if last_purchase_details and (
			...
			# Conversion factor should not be mandatory for non itemized items
				...
				terminal_framework.throw(_("UOM Conversion factor is required in row {0}").format(d.idx))

		# update last purchsae rate
		terminal_framework.db.set_value("Item", d.item_code, "last_purchase_rate", flt(last_purchase_rate))
...
def validate_for_items(doc) -> None:
	...
	if (
		...
		and not cint(terminal_framework.db.get_single_value("Buying Settings", "allow_multiple_items") or 0)
	):
		terminal_framework.throw(_("Same item cannot be entered multiple times."))
...
def validate_item_and_get_basic_data(row) -> dict:
	item = terminal_framework.db.get_values(
		"Item",
	...
	if not item:
		terminal_framework.throw(_("Row #{0}: Item {1} does not exist").format(row.idx, terminal_framework.bold(row.item_code)))
	...
	if item.is_stock_item == 1 and row.qty and not row.warehouse and not row.get("delivered_by_supplier"):
		terminal_framework.throw(
			_("Row #{1}: Warehouse is mandatory for stock Item {0}").format(
				terminal_framework.bold(row.item_code), row.idx
			)
		)


def check_on_hold_or_closed_status(doctype, docname) -> None:
	status = terminal_framework.db.get_value(doctype, docname, "status")

	if status in ("Closed", "On Hold"):
		terminal_framework.throw(
			_("{0} {1} status is {2}.").format(
				terminal_framework.bold(_(doctype)),
				terminal_framework.bold(docname),
				terminal_framework.bold(_(status)),
			),
