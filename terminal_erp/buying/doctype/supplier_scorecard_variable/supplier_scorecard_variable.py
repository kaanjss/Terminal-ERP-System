...
from terminal_framework.query_builder.functions import DateDiff, Sum
from terminal_framework.utils import flt, getdate


class VariablePathNotFound(terminal_framework.ValidationError):
	pass
...
class SupplierScorecardVariable(Document):
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				from terminal_erp.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
					import_string_path,
				)

				import_string_path(self.path)
			except AttributeError:
				terminal_framework.throw(_("Could not find path for {0}").format(self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	...
	PO_Item = terminal_framework.qb.DocType("Purchase Order Item")

	query = (
		terminal_framework.qb.from_(PO_Item)
		.join(PO)
...
def get_total_received(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Count(pr_item.base_amount))
...
def get_total_received_amount(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.received_qty * pr_item.base_rate))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
	...
	result = query.run()
	return terminal_framework.utils.flt(result[0][0]) if result else 0.0
...
def get_total_received_items(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.received_qty))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
...
def get_total_rejected_amount(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.rejected_qty * pr_item.base_rate))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
...
def get_total_rejected_items(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.rejected_qty))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
...
def get_total_accepted_amount(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.qty * pr_item.base_rate))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
...
def get_total_accepted_items(scorecard):
	...
	query = (
		...
		.join(pr_item)
		.on(pr_item.parent == pr.name)
		.select(terminal_framework.qb.fn.Sum(pr_item.qty))
		.where(pr.supplier == scorecard.supplier)
		.where(pr.posting_date[scorecard.start_date : scorecard.end_date])
		.where(pr_item.docstatus == 1)
	)
...
def get_total_shipments(scorecard):
	...
	query = (
		...
		.join(po_item)
		.on(po_item.parent == po.name)
		.select(terminal_framework.qb.fn.Count(po_item.base_amount))
		.where(po.supplier == scorecard.supplier)
		.where(po_item.schedule_date[scorecard.start_date : scorecard.end_date])
		.where(po_item.docstatus == 1)
	)
	...
	result = query.run()
	return terminal_framework.utils.cint(result[0][0]) if result else 0
...
def get_rfq_total_number(scorecard):
	...
	rfq_item = terminal_framework.qb.DocType("Request for Quotation Item")
	rfq_sup = terminal_framework.qb.DocType("Request for Quotation Supplier")
	...
	query = (
		...
		.select(terminal_framework.qb.fn.Count(rfq.name))
...
def get_rfq_total_items(scorecard):
	...
	query = (
		...
		.join(rfq_sup)
		.on(rfq_sup.parent == rfq.name)
		.select(terminal_framework.qb.fn.Count(rfq_item.name))
		.where(rfq_sup.supplier == scorecard.supplier)
		.where(rfq.transaction_date[scorecard.start_date : scorecard.end_date])
		.where(rfq_item.docstatus == 1)
	)
...
def get_sq_total_number(scorecard):
	...
	query = (
		...
		.select(terminal_framework.qb.fn.Count(sq.name))
		.where(rfq_sup.supplier == scorecard.supplier)
...
def get_sq_total_items(scorecard):
	...
	query = (
		...
		.join(sq)
		.on(sq_item.parent == sq.name)
		.select(terminal_framework.qb.fn.Count(sq_item.name))
		.where(rfq_sup.supplier == scorecard.supplier)
		.where(sq.supplier == scorecard.supplier)
		.where(rfq.transaction_date[scorecard.start_date : scorecard.end_date])
		.where(rfq_item.docstatus == 1)
...
def get_rfq_response_days(scorecard):
	...
	query = (
		...
		.join(sq)
		.on(sq_item.parent == sq.name)
		.select(terminal_framework.qb.fn.Sum(DateDiff(sq.transaction_date, rfq.transaction_date)))
		.where(rfq_sup.supplier == scorecard.supplier)
		.where(sq.supplier == scorecard.supplier)
		.where(rfq.transaction_date[scorecard.start_date : scorecard.end_date])
		.where(rfq_item.docstatus == 1)
