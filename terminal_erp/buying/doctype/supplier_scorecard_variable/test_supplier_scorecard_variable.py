...
from terminal_erp.buying.doctype.purchase_order.mapper import make_purchase_receipt as make_pr_from_po
from terminal_erp.buying.doctype.purchase_order.test_purchase_order import create_purchase_order
from terminal_erp.buying.doctype.supplier_scorecard_variable.supplier_scorecard_variable import (
...
class TestSupplierScorecardVariable(Terminal ERPTestSuite):
	def test_variable_exist(self):
		for d in test_existing_variables:
			my_doc = terminal_framework.get_doc("Supplier Scorecard Variable", d.get("name"))
	...
	def test_path_exists(self):
		for d in test_good_variables:
			if terminal_framework.db.exists(d):
				terminal_framework.delete_doc(d.get("doctype"), d.get("name"))
			terminal_framework.get_doc(d).insert()

		for d in test_bad_variables:
			self.assertRaises(VariablePathNotFound, terminal_framework.get_doc(d).insert)
