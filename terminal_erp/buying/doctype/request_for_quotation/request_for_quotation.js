...
terminal_framework.ui.form.on("Request for Quotation", {
	setup: function (frm) {
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.call({
						method: "terminal_erp.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
					...
					terminal_framework.prompt(
						[
							...
							},
							{
								...
								default: terminal_framework.get_meta("Request for Quotation").default_print_format || "",
								get_query: () => {
									return {
										filters: {
							...
							},
							{
								...
								fieldname: "language",
								options: "Language",
								default: terminal_framework.boot.lang,
							},
							{
								fieldtype: "Link",
								label: "Letter Head",
						...
						(data) => {
							var w = window.open(
								terminal_framework.urllib.get_full_url(
									"/api/method/terminal_erp.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
											supplier: data.supplier,
											print_format: data.print_format || "Standard",
											language: data.language || terminal_framework.boot.lang,
	...
	show_supplier_quotation_comparison(frm) {
		...
		terminal_framework.set_route("query-report", "Supplier Quotation Comparison");
	},
		...
		var dialog = new terminal_framework.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return terminal_framework.call({
					type: "GET",
					method: "terminal_erp.buying.doctype.request_for_quotation.mapper.make_supplier_quotation_from_rfq",
					args: {
...
terminal_framework.ui.form.on("Request for Quotation Item", {
	items_add(frm, cdt, cdn) {
		if (frm.doc.schedule_date) {
			terminal_framework.model.set_value(cdt, cdn, "schedule_date", frm.doc.schedule_date);
		}
	},
});
terminal_framework.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		terminal_framework.call({
			method: "terminal_erp.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
			callback: function (r) {
				if (r.message) {
					terminal_framework.model.set_value(cdt, cdn, "contact", r.message.contact_person);
					terminal_framework.model.set_value(cdt, cdn, "email_id", r.message.contact_email);
				}
			},
		});
	},
...
terminal_erp.buying.RequestforQuotationController = class RequestforQuotationController extends (
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
				...
				function () {
					...
					var dialog = new terminal_framework.ui.Dialog({
						...
						primary_action: (args) => {
							...
							terminal_erp.utils.map_current_doc({
								method: "terminal_erp.buying.doctype.request_for_quotation.mapper.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(
				...
				function () {
					terminal_erp.buying.link_to_mrs(me.frm);
				},
				__("Tools")
			);
		...
		var dialog = new terminal_framework.ui.Dialog({
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							terminal_framework
								.call({
									method: "terminal_erp.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
			...
			primary_action: (args) => {
				...
				if (args.search_type === "Tag" && args.tag) {
					return terminal_framework.call({
						type: "GET",
						method: "terminal_framework.desk.doctype.tag.tag.get_tagged_docs",
						args: {
							doctype: "Supplier",
							tag: "%" + args.tag + "%",
						},
...
extend_cscript(cur_frm.cscript, new terminal_erp.buying.RequestforQuotationController({ frm: cur_frm }));
