...
from terminal_erp.buying.doctype.request_for_quotation.mapper import (
	create_supplier_quotation,
	make_supplier_quotation_from_rfq,
)
from terminal_erp.buying.doctype.request_for_quotation.request_for_quotation import (
	get_pdf,
)
from terminal_erp.controllers.accounts_controller import InvalidQtyError
from terminal_erp.crm.doctype.opportunity.mapper import make_request_for_quotation as make_rfq
from terminal_erp.crm.doctype.opportunity.test_opportunity import make_opportunity
...
from terminal_erp.templates.pages.rfq import check_supplier_has_docname_access
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestRequestforQuotation(Terminal ERPTestSuite):
	...
	def test_duplicate_supplier_rejected(self):
		rfq = terminal_framework.new_doc("Request for Quotation")
		...
		self.assertRaises(terminal_framework.ValidationError, rfq.insert)

	def test_rfq_blocked_for_supplier_with_prevent_rfqs(self):
		terminal_framework.db.set_value("Supplier", "_Test Supplier", "prevent_rfqs", 1)
		rfq = make_request_for_quotation(
			supplier_data=[{"supplier": "_Test Supplier", "supplier_name": "_Test Supplier"}],
			do_not_save=True,
		)
		self.assertRaises(terminal_framework.ValidationError, rfq.save)
	...
	def test_make_supplier_quotation_with_taxes(self):
		...
		tax_template = terminal_framework.new_doc("Purchase Taxes and Charges Template")
		...
		tax_rule = terminal_framework.new_doc("Tax Rule")
	...
	def test_make_supplier_quotation_with_special_characters(self):
		terminal_framework.delete_doc_if_exists("Supplier", "_Test Supplier '1", force=1)
	...
	def test_make_supplier_quotation_from_portal(self):
		...
		supplier_quotation_doc = terminal_framework.get_doc("Supplier Quotation", supplier_quotation_name)
	...
	def test_make_multi_uom_supplier_quotation(self):
		...
		supplier_quotation_name = create_supplier_quotation(rfq)
		supplier_quotation = terminal_framework.get_doc("Supplier Quotation", supplier_quotation_name)
	...
	def test_get_pdf(self):
		...
		self.assertEqual(terminal_framework.local.response.type, "pdf")

	def test_portal_user_with_new_supplier(self):
		supplier_doc = terminal_framework.get_doc(
			{
				"doctype": "Supplier",
	...
	@Terminal ERPTestSuite.change_settings("Buying Settings", {"allow_zero_qty_in_request_for_quotation": 1})
	...
	def test_supplier_quotation_from_zero_qty_rfq_in_portal(self):
		...
		sq = terminal_framework.get_doc("Supplier Quotation", sq_name)
		self.assertEqual(len(sq.items), 1)
		self.assertEqual(sq.items[0].qty, 0)
		self.assertEqual(sq.items[0].item_code, rfq.items[0].item_code)
