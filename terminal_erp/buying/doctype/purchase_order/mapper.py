...
def make_purchase_receipt(
	...
	has_unit_price_items = terminal_framework.db.get_value("Purchase Order", source_name, "has_unit_price_items")
...
def make_purchase_invoice_from_portal(purchase_order_name: str):
	...
	if terminal_framework.session.user not in terminal_framework.get_all("Portal User", {"parent": doc.supplier}, pluck="user"):
	...
	terminal_framework.response["type"] = "redirect"
	terminal_framework.response.location = "/purchase-invoices/" + doc.name
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False, args=None):
	...
	def update_item(obj, target, source_parent):
		...
		target.cost_center = (
			obj.cost_center
			or terminal_framework.db.get_value("Project", obj.project, "cost_center")
...
def make_subcontracting_order(
	...
	if not is_po_fully_subcontracted(source_name):
		...
		if (save or submit) and terminal_framework.has_permission(target_doc.doctype, "create"):
			target_doc.save()

			if submit and terminal_framework.has_permission(target_doc.doctype, "submit", target_doc):
				terminal_framework.db.savepoint("submit_subcontracting_order")
				try:
					target_doc.submit()
				except Exception as e:
					terminal_framework.db.rollback(save_point="submit_subcontracting_order")
		...
		terminal_framework.throw(_("This Purchase Order has been fully subcontracted."))
	...
	def post_process(source_doc, target_doc):
		...
		for idx, item in enumerate(target_doc.items):
			...
			if not target_doc.supplier_warehouse:
				...
				target_doc.supplier_warehouse = terminal_framework.get_cached_value(
		...
		target_doc.reserve_stock = terminal_framework.get_single_value(
			"Stock Settings", "auto_reserve_stock"
		) or terminal_framework.get_value("Production Plan", target_doc.production_plan, "reserve_stock")
