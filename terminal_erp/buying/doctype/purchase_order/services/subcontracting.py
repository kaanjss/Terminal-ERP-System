...
class SubcontractingService:
		...
		if doc.is_subcontracted:
			for item in doc.items:
				if not item.fg_item:
					...
					elif not item.bom and not terminal_framework.get_value("Item", item.fg_item, "default_bom"):
						terminal_framework.throw(
				...
				if not item.fg_item_qty:
					terminal_framework.throw(_("Row #{0}: Finished Good Item Qty cannot be zero").format(item.idx))
		else:
			for item in doc.items:
	...
	def auto_create_subcontracting_order(self) -> None:
		...
		if self.doc.is_subcontracted:
			if terminal_framework.db.get_single_value("Buying Settings", "auto_create_subcontracting_order"):
	...
	def update_subcontracting_order_status(self) -> None:
		...
		doc = self.doc
		if doc.is_subcontracted:
			sco = terminal_framework.db.get_value("Subcontracting Order", {"purchase_order": doc.name, "docstatus": 1})
