...
from terminal_erp.buying.doctype.purchase_order.services.drop_ship import DropShipService
from terminal_erp.buying.doctype.purchase_order.services.status import StatusService
from terminal_erp.buying.doctype.purchase_order.services.subcontracting import SubcontractingService
...
from terminal_erp.controllers.status_updater import get_allowance_for
...
class PurchaseOrder(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.buying.doctype.purchase_order_item.purchase_order_item import PurchaseOrderItem
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not terminal_framework.db.get_single_value("Buying Settings", "allow_zero_qty_in_purchase_order"):
			return
	...
	def validate_with_previous_doc(self):
		...
		if cint(terminal_framework.db.get_single_value("Buying Settings", "maintain_same_rate")):
			self.validate_rate_with_reference_doc(
				[["Supplier Quotation", "supplier_quotation", "supplier_quotation_item"]]
			)

	def validate_supplier(self):
		prevent_po = terminal_framework.db.get_value("Supplier", self.supplier, "prevent_pos")
		if prevent_po:
			standing = terminal_framework.db.get_value("Supplier Scorecard", self.supplier, "status")
			if standing:
		...
		warn_po = terminal_framework.db.get_value("Supplier", self.supplier, "warn_pos")
		if warn_po:
			standing = terminal_framework.db.get_value("Supplier Scorecard", self.supplier, "status")
	...
	def get_schedule_dates(self):
		for d in self.get("items"):
			if d.material_request_item and not d.schedule_date:
				d.schedule_date = terminal_framework.db.get_value(
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = terminal_framework.get_cached_value(
	...
	def update_ordered_qty_in_so_for_removed_items(self, removed_items):
		...
		for item in removed_items:
			...
			prev_ordered_qty = flt(
				terminal_framework.get_cached_value("Sales Order Item", sales_order_item, "ordered_qty")
			)
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
	...
	if last_purchase_details:
		...
		item_last_purchase_rate = terminal_framework.get_cached_value("Item", item_code, "last_purchase_rate")
		if item_last_purchase_rate:
...
def update_status(status: str, name: str):
	po = terminal_framework.get_lazy_doc("Purchase Order", name, check_permission="submit")
	po.update_status(status)
