...
from terminal_framework.utils import add_days, flt, getdate, nowdate
from terminal_framework.utils.data import today

from terminal_erp.accounts.doctype.payment_entry.payment_entry import get_payment_entry
from terminal_erp.accounts.party import get_due_date_from_template
...
class TestPurchaseOrder(Terminal ERPTestSuite):
	@Terminal ERPTestSuite.change_settings("Buying Settings", {"allow_multiple_items": 1})
	def test_purchase_order_qty(self):
		...
		self.assertRaises(terminal_framework.NonNegativeError, po.save)
	...
	def test_make_purchase_receipt(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_purchase_receipt, po.name)
		po.submit()
	...
	def test_make_purchase_receipt_respects_over_receipt_allowance(self):
		...
		try:
			...
			terminal_framework.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 0)
	...
	def test_ordered_qty_against_pi_with_update_stock(self):
		...
		terminal_framework.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 0)
		terminal_framework.db.set_value("Item", "_Test Item", "over_billing_allowance", 0)
		terminal_framework.db.set_single_value("Accounts Settings", "over_billing_allowance", 0)
	...
	def test_over_order_allowance_against_material_request(self) -> None:
		...
		terminal_framework.db.set_single_value("Buying Settings", "over_order_allowance", 10)
		po.reload()
		po.items[0].qty = 110
		po.submit()
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			terminal_framework.ValidationError, update_child_qty_rate, "Purchase Order", trans_item, po.name
		)
	...
	def test_purchase_order_invoice_receipt_workflow(self):
		from terminal_erp.accounts.doctype.purchase_invoice.mapper import make_purchase_receipt
	...
	def test_make_purchase_invoice(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_pi_from_po, po.name)
		...
		terminal_framework.get_doc(
			{
				...
				"currency": terminal_framework.get_cached_value("Company", "_Test Company", "default_currency"),
				"conversion_factor": 1,
	...
	def test_po_for_blocked_supplier_all(self):
		...
		self.assertRaises(terminal_framework.ValidationError, create_purchase_order)
	...
	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"automatically_fetch_payment_terms": 0})
	def test_terms_are_not_copied_if_automatically_fetch_payment_terms_is_unchecked(self):
		...
		terminal_framework.db.set_value("Company", "_Test Company", "payment_terms", "_Test Payment Term Template 1")
		pi = make_pi_from_po(po.name)
		pi.save()

		self.assertEqual(pi.get("payment_terms_template"), "_Test Payment Term Template 1")
		terminal_framework.db.set_value("Company", "_Test Company", "payment_terms", "")
	...
	def test_advance_payment_entry_unlink_against_purchase_order(self):
		...
		po_doc = terminal_framework.get_doc("Purchase Order", po_doc.name)
		po_doc.cancel()

		pe_doc = terminal_framework.get_doc("Payment Entry", pe.name)
		pe_doc.cancel()

	def create_account(self, account_name, company, currency, parent):
		if not terminal_framework.db.get_value("Account", filters={"account_name": account_name, "company": company}):
			account = terminal_framework.get_doc(
				{
					"doctype": "Account",
			...
			account = terminal_framework.get_doc("Account", {"account_name": account_name, "company": company})
	...
	def test_po_optional_blanket_order(self):
		...
		po_doc = terminal_framework.get_doc("Purchase Order", po.get("name"))
	...
	def test_internal_transfer_flow(self):
		...
		terminal_framework.db.set_single_value("Selling Settings", "maintain_same_sales_rate", 1)
	...
	def test_variant_item_po(self):
		...
		self.assertRaises(terminal_framework.ValidationError, po.save)
	...
	def test_update_items_for_subcontracting_purchase_order(self):
		...
		self.assertRaises(terminal_framework.ValidationError, update_items, po=po, qty=30)
	...
	def test_new_sc_flow(self):
		...
		self.assertRaises(terminal_framework.ValidationError, sco.save)
		...
		self.assertRaises(terminal_framework.ValidationError, make_subcontracting_order, po.name)

	@Terminal ERPTestSuite.change_settings("Buying Settings", {"auto_create_subcontracting_order": 1})
	def test_auto_create_subcontracting_order(self):
		...
		self.assertTrue(terminal_framework.db.get_value("Subcontracting Order", {"purchase_order": po.name}))
	...
	def test_purchase_order_advance_payment_status(self):
		...
		po = create_purchase_order()
		self.assertEqual(terminal_framework.db.get_value(po.doctype, po.name, "advance_payment_status"), "Not Initiated")
		...
		po.reload()
		self.assertEqual(terminal_framework.db.get_value(po.doctype, po.name, "advance_payment_status"), "Initiated")

		pe = get_payment_entry(po.doctype, po.name).save().submit()

		pr.reload()
		self.assertEqual(pr.status, "Paid")
		self.assertEqual(terminal_framework.db.get_value(po.doctype, po.name, "advance_payment_status"), "Fully Paid")
	...
	@Terminal ERPTestSuite.change_settings("Buying Settings", {"allow_zero_qty_in_purchase_order": 1})
	...
	@Terminal ERPTestSuite.change_settings("Buying Settings", {"maintain_same_rate": 0})
	...
	def test_get_item_details_propagates_drop_ship_flag_to_po(self):
		...
		details = get_item_details(ctx, terminal_framework.new_doc("Purchase Order"))
	...
	def test_drop_ship_po_allows_non_company_shipping_address_without_so(self):
		...
		Regression test for https://github.com/terminal_framework/terminal_erp/issues/51629.
		"""
		from terminal_erp.crm.doctype.prospect.test_prospect import make_address
	...
	def test_remove_unlinked_item_from_mixed_po_does_not_crash(self):
		...
		po.update_ordered_qty_in_so_for_removed_items([terminal_framework._dict({"sales_order_item": None, "qty": 1})])
...
def create_purchase_order(**args):
	po = terminal_framework.new_doc("Purchase Order")
	...
	po.currency = args.currency or terminal_framework.get_cached_value("Company", po.company, "default_currency")
...
def get_ordered_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "ordered_qty"))


def get_requested_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty"))
