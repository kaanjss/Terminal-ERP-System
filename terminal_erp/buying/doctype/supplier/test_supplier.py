...
from terminal_erp.exceptions import PartyDisabled
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSupplier(Terminal ERPTestSuite):
	def test_get_supplier_group_details(self):
		doc = terminal_framework.new_doc("Supplier Group")
		...
		doc.save()
		s_doc = terminal_framework.new_doc("Supplier")
	...
	def test_supplier_default_payment_terms(self):
		...
		terminal_framework.db.set_value("Supplier", "_Test Supplier With Template 1", "payment_terms", "")
		...
		terminal_framework.db.set_value("Supplier Group", "_Test Supplier Group", "payment_terms", "")
		terminal_framework.db.set_value("Supplier", "_Test Supplier", "payment_terms", "")
	...
	def test_supplier_disabled(self):
		terminal_framework.db.set_value("Supplier", "_Test Supplier", "disabled", 1)
		...
		terminal_framework.db.set_value("Supplier", "_Test Supplier", "disabled", 0)

		po.save()

	def test_supplier_country(self):
		# Test that country field exists in Supplier DocType
		supplier = terminal_framework.get_doc("Supplier", "_Test Supplier with Country")
	...
	def test_party_details_tax_category(self):
		...
		terminal_framework.delete_doc_if_exists("Address", "_Test Address With Tax Category-Billing")
...
def create_supplier(**args):
	...
	if not args.supplier_name:
		args.supplier_name = terminal_framework.generate_hash()

	if terminal_framework.db.exists("Supplier", args.supplier_name):
		return terminal_framework.get_doc("Supplier", args.supplier_name)
	...
	if args.get("party_account"):
		doc.append(
			"accounts",
			{
				"company": terminal_framework.db.get_value("Account", args.get("party_account"), "company"),
...
class TestSupplierPortal(Terminal ERPTestSuite):
	def test_portal_user_can_access_supplier_data(self):
		supplier = create_supplier()

		user = terminal_framework.generate_hash() + "@example.com"
