...
class TestSupplierScorecard(Terminal ERPTestSuite):
	...
	def test_criteria_weight(self):
		...
		for d in my_doc.criteria:
			d.weight = 0
		self.assertRaises(terminal_framework.ValidationError, my_doc.insert)
	...
	def test_overlapping_standings_are_rejected(self):
		...
		self.assertRaises(terminal_framework.ValidationError, doc.validate_standings)
	...
	def test_update_standing_propagates_blocking_flags_to_supplier(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Supplier", supplier, "prevent_pos"), 1)
		self.assertEqual(terminal_framework.db.get_value("Supplier", supplier, "prevent_rfqs"), 1)
	...
	def test_make_all_scorecards_is_idempotent(self):
		...
		terminal_framework.db.set_value("Supplier", supplier, "creation", add_days(nowdate(), -75))

		terminal_framework.delete_doc_if_exists("Supplier Scorecard", supplier)
		doc = make_supplier_scorecard()
		doc.supplier = supplier
		doc.name = supplier
		doc.insert()  # on_update generates the period scorecards

		created = terminal_framework.db.count("Supplier Scorecard Period", {"scorecard": doc.name, "docstatus": 1})
		self.assertGreater(created, 0)
		self.assertEqual(make_all_scorecards(doc.name), 0)


def make_supplier_scorecard():
	my_doc = terminal_framework.get_doc(valid_scorecard[0])

	# Make sure the criteria exist (making them)
	for d in valid_scorecard[0].get("criteria"):
		if not terminal_framework.db.exists("Supplier Scorecard Criteria", d.get("criteria_name")):
			d["doctype"] = "Supplier Scorecard Criteria"
			d["name"] = d.get("criteria_name")
			my_criteria = terminal_framework.get_doc(d)
			my_criteria.insert()
	return my_doc
