...
from terminal_framework.utils import add_days, add_years, get_last_day, getdate, nowdate
...
class SupplierScorecard(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.buying.doctype.supplier_scorecard_scoring_criteria.supplier_scorecard_scoring_criteria import (
			SupplierScorecardScoringCriteria,
		)
		from terminal_erp.buying.doctype.supplier_scorecard_scoring_standing.supplier_scorecard_scoring_standing import (
	...
	def calculate_total_score(self):
		scorecards = terminal_framework.get_all(
		...
		for scp in scorecards:
			my_sc = terminal_framework.get_doc("Supplier Scorecard Period", scp.name)
	...
	def apply_standing(self, standing):
		...
		for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
			...
			terminal_framework.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))
...
def refresh_scorecards():
	...
	"""
	scorecards = terminal_framework.get_list("Supplier Scorecard", fields=["name"], pluck="name", limit_page_length=0)
	for sc_name in scorecards:
		...
		if make_all_scorecards(sc_name) > 0:
			...
			terminal_framework.get_doc("Supplier Scorecard", sc_name).save()


@terminal_framework.whitelist()
def make_all_scorecards(docname: str):
	sc = terminal_framework.get_doc("Supplier Scorecard", docname)
	supplier = terminal_framework.get_doc("Supplier", sc.supplier)
...
def make_default_records():
	...
	for d in install_variable_docs:
		...
		terminal_framework.get_doc(d).insert(ignore_if_duplicate=True)
