...
class InvalidFormulaVariable(terminal_framework.ValidationError):
	pass
...
class SupplierScorecardCriteria(Document):
	...
	def validate_formula(self):
		...
		try:
			terminal_framework.safe_eval(test_formula, None, {"max": max, "min": min})
		except Exception:
			terminal_framework.throw(_("Error evaluating the criteria formula"))
...
def get_criteria_list():
	...
	"""
	return terminal_framework.get_list("Supplier Scorecard Criteria", fields=["name"])


def get_variables(criteria_name):
	criteria = terminal_framework.get_doc("Supplier Scorecard Criteria", criteria_name)
...
def _get_variables(criteria):
	...
	for _dummy1, match in enumerate(mylist):
		for _dummy2 in range(0, len(match.groups())):
			...
			variables = terminal_framework.get_all(
			...
			if not variables:
				terminal_framework.throw(_("Unable to find variable: {0}").format(param_name), InvalidFormulaVariable)
