...
import terminal_erp.buying.doctype.supplier_scorecard_variable.supplier_scorecard_variable as variable_functions
from terminal_erp.buying.doctype.supplier_scorecard_criteria.supplier_scorecard_criteria import (
	get_variables,
)
...
class SupplierScorecardPeriod(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.buying.doctype.supplier_scorecard_scoring_variable.supplier_scorecard_scoring_variable import (
	...
	def calculate_weighted_score(self, weighing_function):
		try:
			weighed_score = terminal_framework.safe_eval(
...
def make_supplier_scorecard(source_name, target_doc=None):
	def update_criteria_fields(obj, target, source_parent):
		target.max_score, target.formula = terminal_framework.db.get_value(
