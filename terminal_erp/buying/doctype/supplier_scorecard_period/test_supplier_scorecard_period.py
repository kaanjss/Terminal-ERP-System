...
class TestSupplierScorecardPeriod(Terminal ERPTestSuite):
	...
	def test_invalid_criteria_formula_raises(self):
		...
		self.assertRaises(terminal_framework.ValidationError, period.calculate_criteria)
	...
	def test_criteria_weights_must_total_100(self):
		...
		self.assertRaises(terminal_framework.ValidationError, period.validate_criteria_weights)


def make_period(variables=None, criteria=None):
	period = terminal_framework.new_doc("Supplier Scorecard Period")
