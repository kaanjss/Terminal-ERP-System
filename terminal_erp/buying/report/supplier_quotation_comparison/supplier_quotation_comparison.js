...
terminal_framework.query_reports["Supplier Quotation Comparison"] = {
	filters: [
		{
			fieldtype: "Link",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Link",
			get_query: () => {
				let quote = terminal_framework.query_report.get_filter_value("supplier_quotation");
				if (quote != "") {
					return {
						query: "terminal_erp.stock.doctype.quality_inspection.quality_inspection.item_query",
						filters: {
		...
		},
		{
			...
			get_data: function (txt) {
				return terminal_framework.db.get_link_options("Supplier", txt);
			},
		},
		{
			fieldtype: "MultiSelectList",
			...
			default: "",
			get_data: function (txt) {
				return terminal_framework.db.get_link_options("Supplier Quotation", txt, { docstatus: ["<", 2] });
			},
		},
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter) => {
		...
		if (column.fieldname === "valid_till" && data.valid_till) {
			if (terminal_framework.datetime.get_diff(data.valid_till, terminal_framework.datetime.nowdate()) <= 1) {
				value = `<div style="color:red">${value}</div>`;
			} else if (terminal_framework.datetime.get_diff(data.valid_till, terminal_framework.datetime.nowdate()) <= 7) {
	...
	onload: (report) => {
		...
		report.page.add_inner_button(
			...
			() => {
				let reporter = terminal_framework.query_reports["Supplier Quotation Comparison"];
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if (values) {
				...
				terminal_framework.call({
					method: "terminal_erp.buying.report.supplier_quotation_comparison.supplier_quotation_comparison.set_default_supplier",
					args: {
					...
					freeze: true,
					callback: (r) => {
						terminal_framework.msgprint(__("Successfully Set Supplier"));
						dialog.hide();
					},
				});
			}
