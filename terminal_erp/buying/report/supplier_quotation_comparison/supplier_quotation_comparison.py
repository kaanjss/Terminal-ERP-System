...
def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map):
	...
	dataset = []
	currency_symbol = terminal_framework.db.get_value("Currency", terminal_framework.db.get_default("currency"), "symbol")
	for qty in qty_list:
		datapoints = {
...
def get_columns(filters):
	currency = terminal_framework.get_cached_value("Company", filters.get("company"), "default_currency")
...
def set_default_supplier(item_code: str, supplier: str, company: str):
	terminal_framework.has_permission("Item", "write", doc=item_code, throw=True)
