...
def get_subcontracted_orders(report_filters):
	...
	return terminal_framework.get_all("Subcontracting Order", fields=fields, filters=filters) or []
...
def get_supplied_items(orders, report_filters):
	...
	supplied_items = {}
	for row in terminal_framework.get_all("Subcontracting Order Supplied Item", fields=fields, filters=filters):
...
def prepare_subcontracted_data(orders, supplied_items):
	...
	for row in orders:
		...
		if key not in order_details:
			order_details.setdefault(key, terminal_framework._dict({"order_item": row, "supplied_items": []}))
