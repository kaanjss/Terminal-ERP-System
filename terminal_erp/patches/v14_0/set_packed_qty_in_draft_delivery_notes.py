...
def execute():
	...
	ps_item = terminal_framework.qb.DocType("Packing Slip Item")
	...
	if ps_details:
		...
		dn_details = terminal_framework._dict()
		for r in dn_item_query.run(as_dict=True):
			dn_details.setdefault((r.delivery_note, r.item_code), terminal_framework._dict()).setdefault(r.name, r.qty)
			...
			if dn_items:
				...
				for name, qty in dn_items.items():
					if remaining_qty > 0:
						...
						terminal_framework.db.set_value("Delivery Note Item", name, "packed_qty", row_packed_qty)
