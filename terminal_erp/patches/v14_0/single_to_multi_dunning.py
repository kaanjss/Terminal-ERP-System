...
def execute():
	terminal_framework.reload_doc("accounts", "doctype", "overdue_payment")
	terminal_framework.reload_doc("accounts", "doctype", "dunning")
	...
	all_dunnings = terminal_framework.get_all("Dunning", filters=filters, pluck="name")

	for dunning_name in all_dunnings:
		dunning = terminal_framework.get_doc("Dunning", dunning_name)
		...
		payment_schedules = terminal_framework.get_all(
			"Payment Schedule",
...
def get_accounts_closing_date():
	...
	accounts_frozen_till = terminal_framework.db.get_single_value(
		"Accounts Settings", "acc_frozen_upto"
	)  # always returns datetime.date

	period_closing_date = terminal_framework.db.get_value(
