...
from terminal_framework.utils import add_months, flt, get_first_day, get_last_day
...
def execute():
	...
	budget_names = terminal_framework.db.get_list(
		"Budget",
...
def remove_old_property_setter():
	old_property_setter = terminal_framework.db.get_value(
		"Property Setter",
		{
			"doc_type": "Budget",
	...
	if old_property_setter:
		terminal_framework.delete_doc("Property Setter", old_property_setter, force=1)


def migrate_single_budget(budget_name):
	budget_doc = terminal_framework.get_doc("Budget", budget_name)

	account_rows = terminal_framework.get_all(
		"Budget Account",
	...
	terminal_framework.db.delete("Budget Account", filters={"parent": budget_doc.name})
	...
	if budget_doc.docstatus == 1:
		...
		terminal_framework.delete_doc("Budget", budget_name)


def get_percentage_allocations(budget_doc):
	if budget_doc.monthly_distribution:
		distribution_doc = terminal_framework.get_cached_doc("Monthly Distribution", budget_doc.monthly_distribution)
