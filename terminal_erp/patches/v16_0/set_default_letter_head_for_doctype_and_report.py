...
def execute():
	...
	update_letter_head_for_query = (
		terminal_framework.qb.update(LH).set(LH.letter_head_for, "DocType").where(LH.letter_head_for.isnull())
	)

	update_letter_head_for_query.run()

	for letter_head_for in ("DocType", "Report"):
		default_exists = terminal_framework.db.exists(
			"Letter Head",
			{
				"is_default": 1,
				"disabled": 0,
		...
		standard_letter_head = terminal_framework.db.get_value(
			"Letter Head",
			{
				"standard": "Yes",
				"disabled": 0,
