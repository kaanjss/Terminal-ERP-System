...
from terminal_framework.query_builder.functions import IfNull, Max, Sum
from terminal_framework.utils import flt, now
...
def execute():
	...
	terminal_framework.db.delete("Tax Withholding Entry", filters={"created_by_migration": 1})
...
def get_column_cache():
	return {
		"Supplier": {
			"pan": terminal_framework.db.has_column("Supplier", "pan"),
			"tax_id": terminal_framework.db.has_column("Supplier", "tax_id"),
		},
		"Customer": {
			"pan": terminal_framework.db.has_column("Customer", "pan"),
			"tax_id": terminal_framework.db.has_column("Customer", "tax_id"),
		},
		"Purchase Invoice": {
			"tax_withholding_category": terminal_framework.db.has_column("Purchase Invoice", "tax_withholding_category"),
		},
		"Purchase Invoice Item": {
			"tax_withholding_category": terminal_framework.db.has_column(
				"Purchase Invoice Item", "tax_withholding_category"
			),
		},
		"Sales Invoice": {
			"tax_withholding_category": terminal_framework.db.has_column("Sales Invoice", "tax_withholding_category"),
		},
		"Sales Invoice Item": {
			"tax_withholding_category": terminal_framework.db.has_column(
...
def get_tds_accounts():
	twa = terminal_framework.qb.DocType("Tax Withholding Account")

	result = (
		terminal_framework.qb.from_(twa).select(twa.account, twa.company, twa.parent.as_("category")).run(as_dict=True)
	)
...
def get_tax_rate_map():
	twr = terminal_framework.qb.DocType("Tax Withholding Rate")
	twc = terminal_framework.qb.DocType("Tax Withholding Category")

	rates = (
		terminal_framework.qb.from_(twr)
		.join(twc)
...
def get_party_tax_id(party_type, party, column_cache, party_tax_id_cache):
	...
	if doctype_cols.get("pan"):
		tax_id = terminal_framework.db.get_value(party_type, party, "pan")
	elif doctype_cols.get("tax_id"):
		tax_id = terminal_framework.db.get_value(party_type, party, "tax_id")
...
def bulk_insert_entries(all_entries):
	...
	existing_names = set(terminal_framework.get_all("Tax Withholding Entry", pluck="name"))
	...
	current_time = now()
	current_user = terminal_framework.session.user
	...
	if values:
		terminal_framework.db.bulk_insert("Tax Withholding Entry", fields, values, ignore_duplicates=True)
...
class PurchaseInvoiceMigrator:
	...
	def _fetch_data(self):
		...
		ptc = terminal_framework.qb.DocType("Purchase Taxes and Charges")
		twv = terminal_framework.qb.DocType("Tax Withheld Vouchers")
		at = terminal_framework.qb.DocType("Advance Tax")
		pe = terminal_framework.qb.DocType("Payment Entry")
		atc = terminal_framework.qb.DocType("Advance Taxes and Charges")
		...
		self._all_withheld_vouchers = (
			terminal_framework.qb.from_(twv)
		...
		self._all_advance_taxes = (
			terminal_framework.qb.from_(at)
			.join(pi)
...
def migrate_sales_invoices(tds_accounts, tax_rate_map, column_cache, party_tax_id_cache):
	...
	cust = terminal_framework.qb.DocType("Customer")
	...
	for category, invoices in category_wise_invoices.items():
		terminal_framework.db.set_value("Sales Invoice", {"name": ("in", invoices)}, {"apply_tds": 1})
...
def copy_category_to_items_for_purchase(column_cache):
	...
	parent = terminal_framework.qb.DocType(parent_doctype)
	item = terminal_framework.qb.DocType(item_doctype, alias="item")
...
def copy_category_to_items_for_sales(column_cache):
	...
	parent = terminal_framework.qb.DocType(parent_doctype)
	item = terminal_framework.qb.DocType(item_doctype, alias="item")
	customer = terminal_framework.qb.DocType("Customer", alias="customer")
