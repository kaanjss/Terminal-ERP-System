...
from terminal_erp.selling.doctype.product_bundle.product_bundle import NAME_PREFIX, build_bundle_name
...
def submit_existing_bundles():
	legacy_bundles = terminal_framework.get_all(
		"Product Bundle",
		filters={"docstatus": 0},
	...
	for bundle in legacy_bundles:
		...
		if not bundle.name.startswith(f"{NAME_PREFIX}-"):
			...
			if not terminal_framework.db.exists("Product Bundle", new_name):
...
def stamp_versions_on_transactions():
	...
	for doctype, item_field in SELLING_ITEM_TABLES.items():
		if not terminal_framework.db.has_column(doctype, "product_bundle"):
			continue
		table = terminal_framework.qb.DocType(doctype)
		item_column = getattr(table, item_field)
		flag_bundle_rows = terminal_framework.db.has_column(doctype, "is_product_bundle")
		for item_code, version in version_by_item.items():
			(
				terminal_framework.qb.update(table)
			...
			if flag_bundle_rows:
				...
				(
					terminal_framework.qb.update(table).set(table.is_product_bundle, 1).where(item_column == item_code)
				).run()
...
def _next_index(item_code: str) -> int:
	...
	existing = terminal_framework.get_all(
		"Product Bundle",
		filters={"new_item_code": item_code, "name": ("like", f"{NAME_PREFIX}-%")},
		pluck="name",
	)
	from terminal_erp.selling.doctype.product_bundle.product_bundle import get_next_version_index
...
def _enforce_single_active_version():
	...
	"""
	active = terminal_framework.get_all(
		"Product Bundle",
	...
	for bundle in active:
		if bundle.new_item_code in seen:
			...
			terminal_framework.db.set_value("Product Bundle", bundle.name, "is_active", 0, update_modified=False)
		else:
