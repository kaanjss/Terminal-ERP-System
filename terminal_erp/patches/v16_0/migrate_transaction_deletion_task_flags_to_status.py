...
def execute():
	...
	"""
	if not terminal_framework.db.table_exists("tabTransaction Deletion Record"):
		return
	...
	records = terminal_framework.db.get_all("Transaction Deletion Record", pluck="name")
	...
	for name in records or []:
		...
		for old_field, new_field in field_mapping.items():
			...
			current_value = terminal_framework.db.get_value("Transaction Deletion Record", name, old_field)
		...
		if updates:
			terminal_framework.db.set_value("Transaction Deletion Record", name, updates, update_modified=False)
