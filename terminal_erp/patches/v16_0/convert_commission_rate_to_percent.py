...
def execute():
	...
	for doctype in ("Sales Person", "Sales Team"):
		if not terminal_framework.db.has_column(doctype, "commission_rate"):
			continue
		...
		rows = terminal_framework.db.get_all(doctype, fields=["name", "commission_rate"])
		for row in rows:
			if _is_plain_number(row.commission_rate):
				continue
			cleaned = flt(_strip_percent_sign(row.commission_rate))
			terminal_framework.db.set_value(doctype, row.name, "commission_rate", cleaned, update_modified=False)
