...
from terminal_framework.utils import date_diff, getdate, today


def execute():
	steps = terminal_framework.get_all(
		"Onboarding Step",
	...
	company_creation = terminal_framework.get_all("Company", fields=["creation"], order_by="creation asc", limit=1)
...
def complete_all_onboarding_steps(steps):
	for step in steps:
		terminal_framework.db.set_value("Onboarding Step", step.name, "is_complete", 1, update_modified=False)
...
def complete_onboarding_steps_if_record_exists(steps):
	for step in steps:
		if (
			...
			and terminal_framework.get_all(step.reference_document, limit=1)
		):
			terminal_framework.db.set_value("Onboarding Step", step.name, "is_complete", 1, update_modified=False)
