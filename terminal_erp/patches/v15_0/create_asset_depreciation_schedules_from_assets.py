...
def get_asset_finance_books_map():
	...
	records = (
		terminal_framework.qb.from_(afb)
		.join(asset)
	...
	asset_finance_books_map = terminal_framework._dict()
	for d in records:
...
def get_asset_depreciation_schedules_map():
	...
	records = (
		terminal_framework.qb.from_(ds)
		.join(asset)
	...
	asset_depreciation_schedules_map = terminal_framework._dict()
	for d in records:
...
def update_depreciation_schedules(
	...
	for idx, depr_schedule in enumerate(depreciation_schedules, start=1):
		(
			terminal_framework.qb.update(ds)
			.set(ds.idx, idx)
