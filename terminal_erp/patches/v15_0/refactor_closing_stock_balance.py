...
def execute():
	if not terminal_framework.db.has_table("Closing Stock Balance"):
		return
...
def add_inventory_dimensions_to_stock_closing_balance():
	...
	for inv_dim in inventory_dimensions:
		if not terminal_framework.db.get_value(
			...
			dimension_field = terminal_framework._dict()
...
def create_stock_closing_entries():
	for row in terminal_framework.get_all(
		...
		new_entry = terminal_framework.new_doc("Stock Closing Entry")
