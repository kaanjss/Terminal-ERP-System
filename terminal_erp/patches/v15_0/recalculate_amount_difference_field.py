...
from terminal_erp.stock.doctype.purchase_receipt.services.billing_status import adjust_incoming_rate_for_pr


def execute():
	if not terminal_framework.db.get_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate"):
		return

	for company in terminal_framework.get_all("Company", pluck="name"):
		table = terminal_framework.qb.DocType("Purchase Receipt Item")
		parent = terminal_framework.qb.DocType("Purchase Receipt")
		query = (
		...
		accounting_period = terminal_framework.get_all(
		...
		period_closing_voucher = terminal_framework.get_all(
		...
		stock_frozen_upto = terminal_framework.db.get_single_value("Stock Settings", "stock_frozen_upto")
		if stock_frozen_upto and getdate(stock_frozen_upto) > getdate(posting_date):
			posting_date = stock_frozen_upto

		fiscal_year = get_fiscal_year(terminal_framework.utils.datetime.date.today(), raise_on_missing=False)
		...
		if result := query.run(as_dict=True):
			...
			precision = terminal_framework.get_precision("Purchase Receipt Item", "amount")
			for item in result:
				adjusted_amt = 0.0
			...
			for pr in purchase_receipts:
				adjust_incoming_rate_for_pr(terminal_framework.get_doc("Purchase Receipt", pr))
...
def get_billed_qty_against_purchase_receipt(pr_names):
	...
	return terminal_framework._dict(invoice_data)
