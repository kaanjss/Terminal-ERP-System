...
logger = terminal_framework.logger("patch", allow_site=True, file_count=50)


def execute():
	affected_purchase_receipts = terminal_framework.db.sql(
	...
	terminal_framework.reload_doc("stock", "doctype", "Purchase Receipt")
	terminal_framework.reload_doc("stock", "doctype", "Purchase Receipt Item")

	for pr in affected_purchase_receipts:
		pr_name = pr[0]
		logger.info(f"purchase_receipt_status: patching PR - {pr_name}")

		pr_doc = terminal_framework.get_doc("Purchase Receipt", pr_name)
