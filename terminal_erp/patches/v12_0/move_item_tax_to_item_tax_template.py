...
def execute():
	if "tax_type" not in terminal_framework.db.get_table_columns("Item Tax"):
		return
	old_item_taxes = {}
	item_tax_templates = {}

	terminal_framework.reload_doc("accounts", "doctype", "item_tax_template_detail", force=1)
	terminal_framework.reload_doc("accounts", "doctype", "item_tax_template", force=1)
	existing_templates = terminal_framework.db.sql(
	...
	terminal_framework.reload_doc("stock", "doctype", "item", force=1)
	terminal_framework.reload_doc("stock", "doctype", "item_tax", force=1)
	terminal_framework.reload_doc("selling", "doctype", "quotation_item", force=1)
	terminal_framework.reload_doc("selling", "doctype", "sales_order_item", force=1)
	terminal_framework.reload_doc("stock", "doctype", "delivery_note_item", force=1)
	terminal_framework.reload_doc("accounts", "doctype", "sales_invoice_item", force=1)
	terminal_framework.reload_doc("buying", "doctype", "supplier_quotation_item", force=1)
	terminal_framework.reload_doc("buying", "doctype", "purchase_order_item", force=1)
	terminal_framework.reload_doc("stock", "doctype", "purchase_receipt_item", force=1)
	terminal_framework.reload_doc("accounts", "doctype", "purchase_invoice_item", force=1)
	terminal_framework.reload_doc("accounts", "doctype", "accounts_settings", force=1)
	...
	for item_code in old_item_taxes.keys():
		...
		terminal_framework.db.sql("delete from `tabItem Tax` where parent=%s and parenttype='Item'", item_code)
	...
	for dt in doctypes:
		for d in terminal_framework.db.sql(
			...
			terminal_framework.db.set_value(dt + " Item", d.name, "item_tax_template", item_tax_template_name)
...
def get_item_tax_template(
	...
	item_tax_template = terminal_framework.new_doc("Item Tax Template")
	...
	for tax_type, tax_rate in item_tax_map.items():
		account_details = terminal_framework.db.get_value(
		...
		if account_details:
			...
			if account_details.account_type not in (
				...
				terminal_framework.db.set_value("Account", account_details.name, "account_type", "Chargeable")
		else:
			...
			if not account_name:
				...
				parent_account = terminal_framework.get_value(
				...
				tax_type = terminal_framework.db.get_value("Account", filters)
				if not tax_type:
					account = terminal_framework.new_doc("Account")
					account.update(filters)
					try:
						account.insert()
						tax_type = account.name
					except terminal_framework.DuplicateEntryError:
						tax_type = terminal_framework.db.get_value(
							"Account", {"account_name": account_name, "company": company}, "name"
						)

		account_type = terminal_framework.get_cached_value("Account", tax_type, "account_type")
...
def get_company(company_abbr, parenttype=None, parent=None):
	if parenttype and parent:
		company = terminal_framework.get_cached_value(parenttype, parent, "company")
	else:
		company = terminal_framework.db.get_value("Company", filters={"abbr": company_abbr})
