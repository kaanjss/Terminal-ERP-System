...
def execute():
	...
	company = terminal_framework.db.get_value("Company", {"country": "India"}, "name")
	if company:
...
def create_stock_entry_types():
	...
	for purpose in [
		...
		ste_type = terminal_framework.get_doc({"doctype": "Stock Entry Type", "name": purpose, "purpose": purpose})
	...
	terminal_framework.db.sql(" UPDATE `tabStock Entry` set stock_entry_type = purpose ")
...
def add_gst_hsn_code_field():
	...
	create_custom_fields(custom_fields, ignore_validate=terminal_framework.flags.in_patch, update=True)
