...
def delete_broken_bins():
	# delete useless bins
	terminal_framework.db.sql("delete from `tabBin` where item_code is null or warehouse is null")


def delete_and_patch_duplicate_bins():
	duplicate_bins = terminal_framework.db.sql(
		"""
		SELECT
	...
	for duplicate_bin in duplicate_bins:
		...
		existing_bins = terminal_framework.get_list(
			"Bin",
		...
		for broken_bin in existing_bins:
			terminal_framework.delete_doc("Bin", broken_bin.name)
		...
		if terminal_framework.db.count(
