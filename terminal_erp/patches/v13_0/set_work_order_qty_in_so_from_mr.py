...
def execute():
	...
	for row in results:
		so_item = terminal_framework.get_value("Material Request Item", row.material_request_item, "sales_order_item")
		terminal_framework.db.set_value("Work Order", row.name, "sales_order_item", so_item)

		if so_item:
			wo = terminal_framework.get_doc("Work Order", row.name)
