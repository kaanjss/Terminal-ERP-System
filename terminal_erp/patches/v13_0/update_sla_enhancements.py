...
def execute():
	...
	if terminal_framework.db.exists("DocType", "Service Level Agreement"):
		sla_details = terminal_framework.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = terminal_framework.db.get_all(
		...
		terminal_framework.reload_doc("support", "doctype", "pause_sla_on_status")
		terminal_framework.reload_doc("support", "doctype", "service_level_priority")
		terminal_framework.reload_doc("support", "doctype", "service_day")

		for entry in sla_details:
			values = terminal_framework.db.get_value(
		...
		if terminal_framework.db.exists("DocType", "Service Level"):
			service_levels = terminal_framework.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = terminal_framework.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = terminal_framework.get_doc("Service Level Agreement", sla.name)
	...
	terminal_framework.delete_doc_if_exists("DocType", "Service Level")
