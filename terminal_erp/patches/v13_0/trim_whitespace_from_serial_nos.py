...
def execute():
	broken_sles = terminal_framework.db.sql(
		"""
	...
	terminal_framework.db.MAX_WRITES_PER_TRANSACTION += len(broken_sles)
	...
	for sle in broken_sles:
		...
		terminal_framework.db.set_value("Stock Ledger Entry", sle.name, "serial_no", correct_sr_no, update_modified=False)
	...
	broken_sr_no_records = terminal_framework.get_list(
		"Serial No",
		filters={
			"status": "Active",
	...
	terminal_framework.db.MAX_WRITES_PER_TRANSACTION += len(broken_sr_no_records)

	patch_savepoint = "serial_no_patch"
	for serial_no in broken_sr_no_records:
		try:
			terminal_framework.db.savepoint(patch_savepoint)
			sn = terminal_framework.get_doc("Serial No", serial_no)
			sn.update_serial_no_reference()
			sn.db_update()
		except Exception:
			terminal_framework.db.rollback(save_point=patch_savepoint)
