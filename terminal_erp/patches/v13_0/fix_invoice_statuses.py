...
def execute():
	...
	if not terminal_framework.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = terminal_framework.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = terminal_framework.get_all(
			"Payment Schedule",
		...
		for invoice in invoices_to_update.values():
			...
			doc = terminal_framework.get_doc(invoice)
		...
		for status, docs in status_map.items():
			terminal_framework.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
