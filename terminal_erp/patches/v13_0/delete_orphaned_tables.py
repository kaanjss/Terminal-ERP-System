...
def execute():
	terminal_framework.reload_doc("setup", "doctype", "transaction_deletion_record")

	if has_deleted_company_transactions():
		child_doctypes = get_child_doctypes_whose_parent_doctypes_were_affected()

		for doctype in child_doctypes:
			docs = terminal_framework.get_all(doctype, fields=["name", "parent", "parenttype", "creation"])

			for doc in docs:
				if not terminal_framework.db.exists(doc["parenttype"], doc["parent"]):
					terminal_framework.db.delete(doctype, {"name": doc["name"]})
...
def has_deleted_company_transactions():
	return terminal_framework.get_all("Transaction Deletion Record")


def get_child_doctypes_whose_parent_doctypes_were_affected():
	parent_doctypes = get_affected_doctypes()
	child_doctypes = terminal_framework.get_all(
...
def get_affected_doctypes():
	affected_doctypes = []
	tdr_docs = terminal_framework.get_all("Transaction Deletion Record", pluck="name")

	for tdr in tdr_docs:
		tdr_doc = terminal_framework.get_doc("Transaction Deletion Record", tdr)
...
def is_not_child_table(doctype):
	return not bool(terminal_framework.get_value("DocType", doctype, "istable"))
...
def check_for_new_doc_with_same_name_as_deleted_parent(doc):
	...
	parent_creation_time = terminal_framework.db.get_value(doc["parenttype"], doc["parent"], "creation")
