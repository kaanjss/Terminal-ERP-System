...
def execute():
	terminal_framework.reload_doc("stock", "doctype", "quality_inspection_parameter")
	params = set()

	# get all parameters from QI readings table
	for (p,) in terminal_framework.db.get_all("Quality Inspection Reading", fields=["specification"], as_list=True):
		params.add(p.strip())

	# get all parameters from QI Template as some may be unused in QI
	for (p,) in terminal_framework.db.get_all(
	...
	for parameter in params:
		if terminal_framework.db.exists("Quality Inspection Parameter", parameter):
			continue
