...
from terminal_framework.utils import cint, cstr, flt
...
def execute():
	if not terminal_framework.db.has_column("Work Order", "has_batch_no"):
		return

	terminal_framework.reload_doc("manufacturing", "doctype", "manufacturing_settings")
	if cint(terminal_framework.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")):
		return
	...
	work_orders = [d.name for d in terminal_framework.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = terminal_framework.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = terminal_framework.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = terminal_framework.get_cached_value("Company", doc.company, "stock_adjustment_account")
