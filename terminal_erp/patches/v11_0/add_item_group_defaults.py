...
def execute():
	...
	terminal_framework.reload_doc("setup", "doctype", "item_group")

	companies = terminal_framework.get_all("Company")
	item_groups = terminal_framework.db.sql(
	...
	if len(companies) == 1:
		for item_group in item_groups:
			doc = terminal_framework.get_doc("Item Group", item_group.get("name"))
			item_group_defaults = []
		...
		for item_group in item_groups:
			...
			for d in [
				...
				if item_group.get(d[0]):
					company = terminal_framework.get_value(d[1], item_group.get(d[0]), "company", cache=True)
