...
def execute():
	...
	"""
	if not terminal_framework.db.has_column("Item", "default_warehouse"):
		return
	...
	if len(companies) == 1 and not terminal_framework.get_all("Item Default", limit=1):
		try:
		...
		item_details = terminal_framework.db.sql(
		...
		for item_data in item_details:
			for d in [
				...
				if item_data.get(d[0]):
					company = terminal_framework.get_value(d[1], item_data.get(d[0]), "company", cache=True)
