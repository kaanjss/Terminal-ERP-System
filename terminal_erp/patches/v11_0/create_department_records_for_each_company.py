...
def execute():
	...
	for doctype in ["department", "leave_period", "staffing_plan", "job_opening"]:
		terminal_framework.reload_doc("hr", "doctype", doctype)
	terminal_framework.reload_doc("Payroll", "doctype", "payroll_entry")

	companies = terminal_framework.db.get_all("Company", fields=["name", "abbr"])
	departments = terminal_framework.db.get_all("Department")
	comp_dict = {}
	...
	for department in departments:
		...
		department_doc = terminal_framework.get_doc("Department", department)
		for company in companies:
			copy_doc = terminal_framework.copy_doc(department_doc)
	...
	terminal_framework.local.lang = "en"
...
def update_instructors(comp_dict):
	when_then = []
	emp_details = terminal_framework.get_all("Employee", fields=["name", "company"])
