...
def execute():
	terminal_framework.reload_doc("automation", "doctype", "auto_repeat")
	...
	for module, doctypes in doctypes_to_rename.items():
		for doctype in doctypes:
			...
			if terminal_framework.db.has_column(doctype, "subscription"):
				rename_field(doctype, "subscription", "auto_repeat")

	subscriptions = terminal_framework.db.sql("select * from `tabSubscription`", as_dict=1)

	for doc in subscriptions:
		doc["doctype"] = "Auto Repeat"
		auto_repeat = terminal_framework.get_doc(doc)
		auto_repeat.db_insert()

	terminal_framework.db.sql("delete from `tabSubscription`")
...
def drop_columns_from_subscription():
	...
	for field in [
		...
		if field in terminal_framework.db.get_table_columns("Subscription"):
			fields_to_drop["Subscription"].append(field)

	terminal_framework.model.delete_fields(fields_to_drop, delete=1)
