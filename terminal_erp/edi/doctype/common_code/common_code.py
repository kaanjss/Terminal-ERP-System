...
from terminal_framework.utils.data import get_link_to_form
from lxml import etree

from terminal_erp.edi.doctype.code_list.code_list_import import parse_genericode_content
...
class CommonCode(Document):
	...
	def validate_distinct_references(self):
		...
		for link in self.applies_to:
			existing_links = terminal_framework.get_all(
				"Common Code",
				filters=[
					["name", "!=", self.name],
...
def import_genericode(code_list: str, file_name: str, column_map: dict, filters: dict | None = None):
	...
	for i, xml_element in enumerate(elements, start=1):
		common_code: CommonCode = terminal_framework.new_doc("Common Code")
		common_code.code_list = code_list
		common_code.from_genericode(column_map, xml_element)
		common_code.save()
		terminal_framework.publish_progress(i / total_elements * 100, title=_("Importing Common Codes"))

	return total_elements


def on_doctype_update():
	terminal_framework.db.add_index("Common Code", ["code_list", "common_code"])
