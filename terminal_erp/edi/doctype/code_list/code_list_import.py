...
from terminal_framework.utils.file_manager import save_file
...
def import_genericode():
	try:
		...
		terminal_framework.throw(_("The uploaded file does not match the selected Code List."))
...
def get_uploaded_genericode_file() -> tuple[bytes, str | None]:
	uploaded_data = terminal_framework.local.uploaded_file
	file_name = terminal_framework.local.uploaded_filename
	if uploaded_data and file_name:
		return uploaded_data, file_name

	file_url = terminal_framework.local.uploaded_file_url
	if not file_url:
		raise terminal_framework.ValidationError(_("No file uploaded or URL provided."))
...
def import_genericode_content(
	...
	if terminal_framework.db.exists(doctype, docname):
		code_list = terminal_framework.get_doc(doctype, docname)
		if code_list.name != name:
			raise CodeListSelectionMismatchError
	else:
		# Create a new Code List document with the extracted name
		code_list = terminal_framework.new_doc(doctype)
		code_list.name = name
...
def process_genericode_import(
	...
	from terminal_erp.edi.doctype.common_code.common_code import import_genericode

	column_map = {"code": code_column, "title": title_column, "description": description_column}

	return import_genericode(
		code_list_name, file_name, column_map, terminal_framework.parse_json(filters) if filters else None
	)
