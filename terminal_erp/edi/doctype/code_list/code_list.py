...
class CodeList(Document):
	...
	def __delete_linked_docs(self):
		...
		for doc in linked_docs:
			terminal_framework.delete_doc("Common Code", doc.name)
	...
	def get_default_code(self) -> str | None:
		...
		return (
			terminal_framework.db.get_value("Common Code", self.default_common_code, "common_code")
...
def get_codes_for(code_list: str, doctype: str, name: str) -> tuple[str]:
	...
	CommonCode = terminal_framework.qb.DocType("Common Code")
	DynamicLink = terminal_framework.qb.DocType("Dynamic Link")

	codes = (
		terminal_framework.qb.from_(CommonCode)
		.join(DynamicLink)
...
def get_default_code(code_list: str) -> str | None:
	...
	code_id = terminal_framework.db.get_value("Code List", code_list, "default_common_code")
	return terminal_framework.db.get_value("Common Code", code_id, "common_code") if code_id else None
