terminal_framework.provide("terminal_erp.edi");

terminal_erp.edi.import_genericode = function (listview_or_form) {
	...
	new terminal_framework.ui.FileUploader({
		method: "terminal_erp.edi.doctype.code_list.code_list_import.import_genericode",
		doctype: doctype,
		docname: docname,
...
function show_column_selection_dialog(context) {
	...
	let fields = [
		{
			...
			)} ${terminal_framework.utils.get_form_link(
				"Code List",
				context.code_list,
				true,
	...
	let d = new terminal_framework.ui.Dialog({
		...
		primary_action(values) {
			...
			terminal_framework
				.xcall("terminal_erp.edi.doctype.code_list.code_list_import.process_genericode_import", {
				...
				.then((count) => {
					terminal_framework.msgprint(__("Import completed. {0} common codes created.", [count]));
				});
			d.hide();
		},
	});
