...
from terminal_erp.edi.doctype.code_list import code_list_import
...
class TestCodeListImport(Terminal ERPTestSuite):
	def test_import_genericode_rejects_remote_file_url(self):
		...
		with patch("terminal_erp.edi.doctype.code_list.code_list_import.requests.get") as mock_get:
			with self.assertRaisesRegex(
				terminal_framework.ValidationError, "Importing Code Lists from remote URLs is not allowed."
			):
	...
	def test_import_genericode_from_trusted_url(self):
		...
		with patch(
			"terminal_erp.edi.doctype.code_list.code_list_import.requests.get",
		...
		file_doc = terminal_framework.get_doc("File", import_result["file"])
	...
	def test_process_genericode_import_reads_file_doc_content(self):
		...
		self.assertEqual(terminal_framework.db.count("Common Code", {"code_list": import_result["code_list"]}), 3)
		self.assertEqual(
	...
	def test_import_genericode_from_local_file_url(self):
		source_file = terminal_framework.get_doc(
			{
				"doctype": "File",
	...
	def set_upload_context(
		...
		originals = {attr: getattr(terminal_framework.local, attr, None) for attr in attrs}

		terminal_framework.local.form_dict = terminal_framework._dict(doctype="Code List", docname=docname)
		terminal_framework.local.uploaded_file = content
		terminal_framework.local.uploaded_file_url = file_url
		terminal_framework.local.uploaded_filename = file_name

		def restore():
			for attr, value in originals.items():
				setattr(terminal_framework.local, attr, value)
	...
	def assert_import_response(self, import_result):
		...
		self.assertTrue(terminal_framework.db.exists("Code List", import_result["code_list"]))
		self.assertTrue(terminal_framework.db.exists("File", import_result["file"]))
