...
from terminal_framework.share import add_docshare
from terminal_framework.utils import get_url, getdate, now
from terminal_framework.utils.verified_command import get_signed_params
...
class Appointment(Document):
	...
	def find_lead_by_email(self):
		lead_list = terminal_framework.get_list(
	...
	def find_customer_by_email(self):
		customer_list = terminal_framework.get_list(
	...
	def before_insert(self):
		number_of_appointments_in_same_slot = terminal_framework.db.count(
			"Appointment", filters={"scheduled_time": self.scheduled_time}
		)
		number_of_agents = terminal_framework.db.get_single_value("Appointment Booking Settings", "number_of_agents")
		if number_of_agents != 0:
			if number_of_appointments_in_same_slot >= number_of_agents:
				terminal_framework.throw(_("Time slot is not available"))
		# Link lead
		if not self.party:
	...
	def send_confirmation_email(self):
		...
		args = {
			"link": verify_url,
			"site_url": terminal_framework.utils.get_url(),
		...
		if terminal_framework.session.user == "Guest":
			terminal_framework.msgprint(_("Please check your email to confirm the appointment"))
		else:
	...
	def on_change(self):
		...
		if not self.calendar_event:
			return
		cal_event = terminal_framework.get_doc("Event", self.calendar_event)
		cal_event.starts_on = self.scheduled_time
		cal_event.save(ignore_permissions=True)

	def set_verified(self, email):
		if email != self.customer_email:
			terminal_framework.throw(_("Email verification failed."))
		# Create new lead
		...
		if self.customer_details:
			lead.append(
				"notes",
				{
					...
					"added_by": terminal_framework.session.user,
					"added_on": now(),
				},
			)
	...
	def get_assignee_from_latest_opportunity(self):
		if not self.party:
			return None
		if not terminal_framework.db.exists("Lead", self.party):
			return None
		opporutnities = terminal_framework.get_list(
			"Opportunity",
			filters={
		...
		if not opporutnities:
			return None
		latest_opportunity = terminal_framework.get_doc("Opportunity", opporutnities[0].name)
		assignee = latest_opportunity._assign
		if not assignee:
			return None
		assignee = terminal_framework.parse_json(assignee)[0]
		return assignee

	def create_calendar_event(self):
		if self.calendar_event:
			return
		appointment_event = terminal_framework.get_doc(
			{
				"doctype": "Event",
				...
				"status": "Open",
				"type": "Public",
				"send_reminder": terminal_framework.db.get_single_value(
	...
	def assign_agent(self, agent):
		if not terminal_framework.has_permission(doc=self, user=agent):
...
def _get_agents_sorted_by_asc_workload(date):
	appointments = terminal_framework.get_all("Appointment", fields="*")
	...
	for appointment in appointments:
		...
		if not assign_data:
			continue
		assigned_to = terminal_framework.parse_json(assign_data)
...
def _get_agent_list_as_strings():
	...
	agent_list = terminal_framework.get_doc("Appointment Booking Settings").agent_list
...
def _check_agent_availability(agent_email, scheduled_time):
	appointemnts_at_scheduled_time = terminal_framework.get_all("Appointment", filters={"scheduled_time": scheduled_time})
...
def _get_employee_from_user(user):
	employee_docname = terminal_framework.db.get_value("Employee", {"user_id": user})
	if employee_docname:
		return terminal_framework.get_doc("Employee", employee_docname)
	return None
