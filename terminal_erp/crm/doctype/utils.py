...
def get_last_interaction(contact: str | None = None, lead: str | None = None):
	...
	last_issue = None
	if contact:
		communication = terminal_framework.qb.DocType("Communication")
		link_conditions = []
		...
		if link_conditions:
			last_communication = (
				terminal_framework.qb.from_(communication)
	...
	if lead:
		last_communication = terminal_framework.get_all(
			"Communication",
...
def get_last_issue_from_customer(customer_name):
	issues = terminal_framework.get_all(
		"Issue",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = terminal_framework.utils.nowtime()
	weekday = terminal_framework.utils.get_weekday()

	available_employee_groups = terminal_framework.get_all(
	...
	employees = terminal_framework.get_all(
