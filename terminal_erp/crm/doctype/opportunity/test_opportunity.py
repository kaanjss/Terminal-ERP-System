...
from terminal_framework.utils import add_days, now_datetime, random_string, today

from terminal_erp.crm.doctype.lead.mapper import make_customer
from terminal_erp.crm.doctype.lead.test_lead import make_lead
from terminal_erp.crm.doctype.opportunity.mapper import make_quotation
from terminal_erp.crm.doctype.opportunity.opportunity import auto_close_opportunity, get_item_details
from terminal_erp.crm.utils import get_linked_communication_list
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestOpportunity(Terminal ERPTestSuite):
	@classmethod
	def make_opportunities(cls):
		...
		cls.opportunities = []
		for x in records:
			if not terminal_framework.db.exists("Opportunity", {"name": x.get("name")}):
				cls.opportunities.append(terminal_framework.get_doc(x).insert())
			else:
				cls.opportunities.append(terminal_framework.get_doc("Opportunity", {"party_name": x.get("party_name")}))
	...
	def test_opportunity_status(self):
		...
		doc = terminal_framework.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email)
	...
	def test_carry_forward_of_email_and_comments(self):
		terminal_framework.db.set_single_value("CRM Settings", "carry_forward_communication_and_comments", 1)
		lead_doc = make_lead()
		...
		opportunity_comment_count = terminal_framework.db.count(
	...
	def test_get_notification_email(self):
		...
		opp = terminal_framework.new_doc("Opportunity")
	...
	def test_declare_lost_blocked_when_quotation_active(self):
		...
		opp.reload()
		self.assertRaises(terminal_framework.ValidationError, opp.declare_enquiry_lost, [], [], "x")
		self.assertNotEqual(opp.status, "Lost")

	def test_get_item_details(self):
		details = get_item_details("_Test Item")
		self.assertEqual(details["item_name"], terminal_framework.db.get_value("Item", "_Test Item", "item_name"))
		self.assertEqual(details["uom"], terminal_framework.db.get_value("Item", "_Test Item", "stock_uom"))
	...
	def test_auto_close_replied_opportunity(self):
		days = terminal_framework.db.get_single_value("CRM Settings", "close_opportunity_after_days") or 15

		stale = make_opportunity(with_items=0)
		fresh = make_opportunity(with_items=0)
		for opp in (stale, fresh):
			terminal_framework.db.set_value("Opportunity", opp.name, "status", "Replied", update_modified=False)
		...
		self.assertEqual(terminal_framework.db.get_value("Opportunity", stale.name, "status"), "Closed")
		self.assertEqual(terminal_framework.db.get_value("Opportunity", fresh.name, "status"), "Replied")

	def test_opportunity_synced_to_prospect(self):
		prospect_name = "_Test Prospect For Opportunity"
		if not terminal_framework.db.exists("Prospect", prospect_name):
			terminal_framework.get_doc(
				{"doctype": "Prospect", "company_name": prospect_name, "company": "_Test Company"}
			).insert(ignore_permissions=True)

		opp = terminal_framework.get_doc(
			{
				"doctype": "Opportunity",
...
def _ensure_master(doctype, fieldname, value):
	if not terminal_framework.db.exists(doctype, value):
		terminal_framework.get_doc({"doctype": doctype, fieldname: value}).insert(ignore_permissions=True)
	return value
...
def make_opportunity_from_lead(company):
	...
	opp_doc = terminal_framework.get_doc(args).insert(ignore_permissions=True)
...
def make_opportunity(**args):
	...
	opp_doc = terminal_framework.get_doc(
		{
			"doctype": "Opportunity",
