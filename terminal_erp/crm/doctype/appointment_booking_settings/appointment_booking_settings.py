...
class AppointmentBookingSettings(Document):
	...
	if TYPE_CHECKING:
		from terminal_framework.automation.doctype.assignment_rule_user.assignment_rule_user import AssignmentRuleUser
		from terminal_framework.types import DF

		from terminal_erp.crm.doctype.appointment_booking_slots.appointment_booking_slots import (
	...
	def validate_from_and_to_time(self, from_time, to_time, record):
		if from_time > to_time:
			...
			terminal_framework.throw(err_msg)

	def duration_is_divisible(self, from_time, to_time):
		timedelta = to_time - from_time
		if timedelta.total_seconds() % (self.appointment_duration * 60):
			terminal_framework.throw(_("The difference between from time and To Time must be a multiple of Appointment"))
