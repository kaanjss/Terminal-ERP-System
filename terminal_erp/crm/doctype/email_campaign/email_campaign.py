...
class EmailCampaign(Document):
	...
	def set_date(self):
		if getdate(self.start_date) < getdate(today()):
			terminal_framework.throw(_("Start Date cannot be before the current date"))

		# set the end date as start date + max(send after days) in campaign schedule
		campaign = terminal_framework.get_cached_doc("Campaign", self.campaign_name)
	...
	def validate_lead(self):
		lead_email_id = terminal_framework.db.get_value("Lead", self.recipient, "email_id")
		if not lead_email_id:
			lead_name = terminal_framework.db.get_value("Lead", self.recipient, "lead_name")
			terminal_framework.throw(_("Please set an email id for the Lead {0}").format(lead_name))

	def validate_email_campaign_already_exists(self):
		email_campaign_exists = terminal_framework.db.exists(
			"Email Campaign",
			{
...
def send_email_to_leads_or_contacts():
	...
	email_campaigns = terminal_framework.get_all(
		"Email Campaign",
	...
	for email_campaign in email_campaigns:
		try:
			campaign = terminal_framework.get_cached_doc("Campaign", email_campaign.campaign_name)
...
def send_mail(entry, email_campaign):
	...
	if campaign_for == "Email Group":
		recipient_list = terminal_framework.get_all(
			"Email Group Member",
			filters={"email_group": recipient, "unsubscribed": 0},
			pluck="email",
		)
	else:
		email_id = terminal_framework.db.get_value(campaign_for, recipient, "email_id")
		if not email_id:
			terminal_framework.log_error(
				title=_("Email Campaign Error"),
	...
	email_template = terminal_framework.get_cached_doc("Email Template", entry.get("email_template"))
	sender = terminal_framework.db.get_value("User", sender_user, "email") if sender_user else None

	# Build context for template rendering
	if campaign_for != "Email Group":
		context = {"doc": terminal_framework.get_doc(campaign_for, recipient)}
	else:
		# For email groups, use the email group document as context
		context = {"doc": terminal_framework.get_doc("Email Group", recipient)}

	# Render template
	subject = terminal_framework.render_template(email_template.get("subject"), context)
	content = terminal_framework.render_template(email_template.response_, context)

	terminal_framework.db.savepoint("email_campaign_send")
	try:
		comm = make(
		...
		terminal_framework.db.rollback(save_point="email_campaign_send")
		terminal_framework.log_error(title="Email Campaign Failed.")
...
def unsubscribe_recipient(unsubscribe, method):
	...
	email_campaign = terminal_framework.get_doc("Email Campaign", unsubscribe.reference_name)
	...
	if email_campaign.email_campaign_for == "Email Group":
		...
		# For Lead or Contact
		terminal_framework.db.set_value("Email Campaign", email_campaign.name, "status", "Unsubscribed")
...
def set_email_campaign_status():
	...
	for name in email_campaigns:
		email_campaign = terminal_framework.get_doc("Email Campaign", name)
