...
class Contract(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.crm.doctype.contract_fulfilment_checklist.contract_fulfilment_checklist import (
	...
	def set_missing_values(self):
		if not self.party_full_name:
			...
			if res := terminal_framework.db.get_value(self.party_type, self.party_name, field):
				self.party_full_name = res

	def before_submit(self):
		self.signed_by_company = terminal_framework.session.user
	...
	def validate_dates(self):
		if self.end_date and self.end_date < self.start_date:
			terminal_framework.throw(_("End Date cannot be before Start Date."))
...
def update_status_for_contracts():
	...
	contracts = terminal_framework.get_all(
		"Contract",
	...
	for contract in contracts:
		...
		terminal_framework.db.set_value("Contract", contract.get("name"), "status", status)
