...
terminal_erp.LeadController = class LeadController extends terminal_framework.ui.form.Controller {
	setup() {
	...
	refresh() {
		...
		if (!this.frm.is_new()) {
			terminal_framework.contacts.render_address_and_contact(this.frm);
		} else {
			terminal_framework.contacts.clear_address_and_contact(this.frm);
		}
		...
		terminal_framework.prompt(
			...
			function (data) {
				terminal_framework.call({
					method: "terminal_erp.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.crm.doctype.lead.mapper.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.crm.doctype.lead.mapper.make_quotation",
			frm: this.frm,
		});
	}
	...
	async make_opportunity() {
		...
		let existing_prospect = (
			await terminal_framework.db.get_value(
				"Prospect Lead",
				{
					lead: frm.doc.name,
				},
		...
		if (fields.length) {
			const d = new terminal_framework.ui.Dialog({
	...
	make_prospect() {
		const me = this;
		terminal_framework.model.with_doctype("Prospect", function () {
			let prospect = terminal_framework.model.get_new_doc("Prospect");
			...
			let leads_row = terminal_framework.model.add_child(prospect, "leads");
			leads_row.lead = me.frm.doc.name;

			terminal_framework.set_route("Form", "Prospect", prospect.name);
		});
	}
...
if (cur_frm) {
	extend_cscript(cur_frm.cscript, new terminal_erp.LeadController({ frm: cur_frm }));
}
