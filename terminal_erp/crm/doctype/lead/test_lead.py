...
from terminal_framework.utils import random_string, today

from terminal_erp.crm.doctype.lead.mapper import make_opportunity
from terminal_erp.crm.utils import get_linked_prospect
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestLead(Terminal ERPTestSuite):
	def test_make_customer(self):
		from terminal_erp.crm.doctype.lead.mapper import make_customer

		lead = terminal_framework.db.get_all("Lead", {"lead_name": "_Test Lead"})[0].name

		terminal_framework.delete_doc_if_exists("Customer", "_Test Lead")
	...
	def test_make_customer_from_organization(self):
		...
		lead = terminal_framework.db.get_all("Lead", {"lead_name": "_Test Lead 1"})[0].name
		customer = make_customer(lead)
		self.assertEqual(customer.doctype, "Customer")
		self.assertEqual(customer.lead_name, lead)
	...
	def test_create_lead_and_unlinking_dynamic_links(self):
		...
		address_1 = terminal_framework.get_doc(
			{
				"doctype": "Address",
				"address_type": "Billing",
				"address_line1": "Baner",
		...
		lead_doc.delete()
		address_1.reload()
		self.assertEqual(terminal_framework.db.exists("Lead", lead_doc.name), None)
	...
	def test_opportunity_from_lead(self):
		...
		self.assertTrue(
			terminal_framework.db.get_value("ToDo", {"reference_type": "Opportunity", "reference_name": opportunity.name})
		)
	...
	def test_get_notification_email(self):
		...
		lead = terminal_framework.new_doc("Lead")
	...
	def test_lead_prospect_sync_and_unlink(self):
		...
		lead.save()
		self.assertEqual(terminal_framework.db.get_value("Prospect Lead", {"lead": lead.name}, "mobile_no"), "9999999999")

		# deleting the only lead of a prospect removes the prospect
		lead.delete()
		self.assertFalse(terminal_framework.db.exists("Prospect", prospect_name))
	...
	def test_set_lead_name_fallbacks(self):
		...
		lead = terminal_framework.new_doc("Lead")
		self.assertRaises(terminal_framework.ValidationError, lead.set_lead_name)


def create_event(subject, starts_on, reference_type, reference_name):
	event = terminal_framework.new_doc("Event")
...
def create_todo(description, reference_type, reference_name):
	todo = terminal_framework.new_doc("ToDo")
...
def make_lead(**args):
	...
	lead_doc = terminal_framework.get_doc(
		{
			"doctype": "Lead",
