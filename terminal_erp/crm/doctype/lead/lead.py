...
from terminal_framework.utils import comma_and, get_link_to_form, validate_email_address
from terminal_framework.utils.data import DateTimeLikeObject

from terminal_erp.accounts.party import set_taxes
from terminal_erp.controllers.selling_controller import SellingController
from terminal_erp.crm.utils import CRMNote, copy_comments, link_communications, link_open_events
from terminal_erp.selling.doctype.customer.mapper import parse_full_name
...
class Lead(SellingController, CRMNote):
	...
	def onload(self):
		customer = terminal_framework.db.get_value("Customer", {"lead_name": self.name})
	...
	def before_insert(self):
		...
		if terminal_framework.db.get_single_value("CRM Settings", "auto_creation_of_contact"):
			if self.utm_source == "Existing Customer" and self.customer:
				...
				if contact:
					self.contact_doc = terminal_framework.get_doc("Contact", contact)
					return
	...
	def on_trash(self):
		terminal_framework.db.set_value("Issue", {"lead": self.name}, "lead", None)
	...
	def set_lead_name(self):
		if not self.lead_name:
			...
			if not self.company_name and not self.email_id and not self.flags.ignore_mandatory:
				terminal_framework.throw(_("A Lead requires either a person's name or an organization's name"))
	...
	def check_email_id_is_unique(self):
		if self.email_id:
			...
			if not terminal_framework.db.get_single_value("CRM Settings", "allow_lead_duplication_based_on_emails"):
				duplicate_leads = terminal_framework.get_all(
					"Lead", filters={"email_id": self.email_id, "name": ["!=", self.name]}
				)
				duplicate_leads = [
					terminal_framework.bold(get_link_to_form("Lead", lead.name)) for lead in duplicate_leads
				]
	...
	def validate_email_id(self):
		if self.email_id:
			...
			if self.email_id == self.lead_owner:
				terminal_framework.throw(_("Lead Owner cannot be same as the Lead Email Address"))
	...
	def update_prospect(self):
		lead_row_name = terminal_framework.db.get_value("Prospect Lead", filters={"lead": self.name}, fieldname="name")
		if lead_row_name:
			lead_row = terminal_framework.get_doc("Prospect Lead", lead_row_name)
			lead_row.update(
				{
	...
	def remove_link_from_prospect(self):
		...
		for linked_prospect in linked_prospects:
			prospect = terminal_framework.get_doc("Prospect", linked_prospect.parent)
	...
	def has_customer(self):
		return terminal_framework.db.get_value("Customer", {"lead_name": self.name})

	def has_opportunity(self):
		return terminal_framework.db.get_value("Opportunity", {"party_name": self.name, "status": ["!=", "Lost"]})
	...
	def has_lost_quotation(self):
		return terminal_framework.db.get_value("Quotation", {"party_name": self.name, "docstatus": 1, "status": "Lost"})
	...
	def create_prospect(self, company_name):
		try:
			...
			terminal_framework.throw(_("Prospect {0} already exists").format(company_name or self.company_name))

	def get_notification_email(self):
		"""Hook to return the target email address for notifications."""
		if self.lead_owner:
			return terminal_framework.db.get_value("User", self.lead_owner, "email")
...
def get_lead_details(
	...
	from terminal_erp.accounts.party import set_address_details

	out = terminal_framework._dict()

	lead_doc = terminal_framework.get_doc("Lead", lead)
...
def get_lead_with_phone_number(number):
	...
	leads = terminal_framework.get_all(
		"Lead",
		or_filters={
			"phone": ["like", f"%{number}"],
...
def add_lead_to_prospect(lead: str, prospect: str):
	prospect = terminal_framework.get_doc("Prospect", prospect)
	prospect.append("leads", {"lead": lead})
	prospect.save()

	carry_forward_communication_and_comments = terminal_framework.db.get_single_value(
	...
	terminal_framework.msgprint(
		_("Lead {0} has been added to prospect {1}.").format(terminal_framework.bold(lead), terminal_framework.bold(prospect.name)),
