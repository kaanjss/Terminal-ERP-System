...
class OpportunitySummaryBySalesStage:
	...
	def set_sales_stage_columns(self):
		self.sales_stage_list = terminal_framework.db.get_list("Sales Stage", pluck="name")
	...
	def get_data_query(self, based_on, data_based_on):
		if self.filters.get("data_based_on") == "Number":
			...
			self.query_result = terminal_framework.db.get_list(
				"Opportunity",
	...
	def get_formatted_data(self):
		self.formatted_data = terminal_framework._dict()
	...
	def set_formatted_data_based_on_sales_stage(self, based_on, sales_stage, count):
		self.formatted_data.setdefault(based_on, terminal_framework._dict()).setdefault(sales_stage, 0)
