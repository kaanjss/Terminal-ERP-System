...
from terminal_framework.utils import add_days, flt
...
def get_columns(based_on):
	return [
		{"fieldname": terminal_framework.scrub(based_on), "label": _(based_on), "fieldtype": "Data", "width": 150},
...
def get_lead_data(filters, based_on):
	based_on_field = terminal_framework.scrub(based_on)
	...
	lead_details = terminal_framework.get_all("Lead", filters=lead_filters, fields=[based_on_field, "name"])

	lead_map = terminal_framework._dict()
...
def get_lead_quotation_count(leads):
	return terminal_framework.db.count("Quotation", {"quotation_to": "Lead", "party_name": ["in", leads]})


def get_lead_opp_count(leads):
	return terminal_framework.db.count("Opportunity", {"opportunity_from": "Lead", "party_name": ["in", leads]})
