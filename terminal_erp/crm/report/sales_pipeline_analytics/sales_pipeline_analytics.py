...
from terminal_framework.query_builder.custom import Month, MonthName, Quarter
...
class SalesPipelineAnalytics:
	...
	def validate_filters(self):
		if not self.filters.from_date:
			terminal_framework.throw(_("From Date is mandatory"))

		if not self.filters.to_date:
			terminal_framework.throw(_("To Date is mandatory"))
	...
	def get_fields(self):
		...
		opp = terminal_framework.qb.DocType("Opportunity")
			...
			self.query_result = (
				terminal_framework.qb.get_query(
					...
					fields=[
						...
						terminal_framework.query_builder.functions.Count("*").as_("count"),
						self.duration,
					],
	...
	def get_periodic_data(self):
		self.periodic_data = terminal_framework._dict()
				...
				if self.filters.get("assigned_to"):
					for user in assigned_to:
						if self.filters.get("assigned_to") == user:
							value = user
							self.periodic_data.setdefault(value, terminal_framework._dict()).setdefault(period, 0)
	...
	def get_default_currency(self):
		...
		return terminal_framework.db.get_value("Company", company, ["default_currency"])

	def get_currency_rate(self, from_currency, to_currency):
		cacheobj = terminal_framework.cache()
