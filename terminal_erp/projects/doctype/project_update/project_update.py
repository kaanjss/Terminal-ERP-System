...
def daily_reminder():
	...
	terminal_framework.only_for("Projects Manager")

	# Same for every project this run, so check once instead of once per project.
	holiday_today = terminal_framework.db.exists("Holiday", {"holiday_date": today()})

	projects = terminal_framework.get_all(
		"Project",
		fields=[
			"name",
			"project_name",
	...
	for project in projects:
		...
		number_of_drafts = terminal_framework.db.count("Project Update", {"project": project_id, "docstatus": 0})
		...
		update = terminal_framework.get_all(
			"Project Update",
...
def email_sending(
	...
	if not holiday_today:
		recipients = terminal_framework.get_all(
			"Project User",
		...
		for user in recipients:
			terminal_framework.sendmail(recipients=[user], subject=terminal_framework._(project_name + " " + "Summary"), message=msg)
	else:
		pass
