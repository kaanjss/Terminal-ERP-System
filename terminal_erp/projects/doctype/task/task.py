...
from terminal_framework.desk.form.assign_to import clear, close_all_assignments
...
from terminal_framework.utils import add_days, add_to_date, cstr, date_diff, flt, get_link_to_form, getdate, today
from terminal_framework.utils.data import format_date
from terminal_framework.utils.nestedset import NestedSet


class CircularReferenceError(terminal_framework.ValidationError):
	pass


class ParentIsGroupError(terminal_framework.ValidationError):
	pass
...
class Task(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.projects.doctype.task_depends_on.task_depends_on import TaskDependsOn
	...
	def get_customer_details(self):
		customer_name = terminal_framework.db.get_value("Customer", self.customer, "customer_name")
		if customer_name:
	...
	def validate_parent_expected_end_date(self):
		...
		parent_exp_end_date = terminal_framework.db.get_value("Task", self.parent_task, "exp_end_date")
	...
	def validate_parent_project_dates(self):
		if not self.project or terminal_framework.in_test:
			return

		if project_end_date := terminal_framework.db.get_value("Project", self.project, "expected_end_date"):
				...
				if task_date and date_diff(project_end_date, getdate(task_date)) < 0:
					terminal_framework.throw(
						_("{0}'s {1} cannot be after {2}'s Expected End Date.").format(
							terminal_framework.bold(terminal_framework.get_desk_link("Task", self.name)),
							_(self.meta.get_label(fieldname)),
							terminal_framework.bold(terminal_framework.get_desk_link("Project", self.project)),
						),
						terminal_framework.exceptions.InvalidDates,
					)
	...
	def validate_status(self):
		...
		if self.status != self.get_db_value("status") and self.status == "Completed":
			for d in self.depends_on:
				if terminal_framework.db.get_value("Task", d.task, "status") not in ("Completed", "Cancelled"):
					terminal_framework.throw(
						_(
							"Cannot complete task {0} as its dependent task {1} is not completed / cancelled."
						).format(terminal_framework.bold(self.name), terminal_framework.bold(d.task))
					)
	...
	def validate_progress(self):
		if flt(self.progress or 0) > 100:
			terminal_framework.throw(_("Progress % for a task cannot be more than 100."))
	...
	def validate_parent_template_task(self):
		if self.parent_task:
			if not terminal_framework.db.get_value("Task", self.parent_task, "is_template"):
				terminal_framework.throw(
	...
	def validate_depends_on_tasks(self):
		if self.depends_on:
			for task in self.depends_on:
				if not terminal_framework.db.get_value("Task", task.task, "is_template"):
					terminal_framework.throw(
	...
	def validate_completed_on(self):
		if self.completed_on and getdate(self.completed_on) > getdate():
			terminal_framework.throw(_("Completed On cannot be greater than Today"))

	def validate_parent_is_group(self):
		if self.parent_task:
			if not terminal_framework.db.get_value("Task", self.parent_task, "is_group"):
				terminal_framework.throw(
	...
	def update_project(self):
		if self.project and not self.flags.from_project:
			terminal_framework.get_cached_doc("Project", self.project).update_project()
	...
	def check_recursion(self):
		...
		for d in check_list:
			...
			while len(task_list) > count:
				tasks = terminal_framework.get_all(
					"Task Depends On",
				...
				for b in tasks:
					if b[0] == self.name:
						terminal_framework.throw(_("Circular Reference Error"), CircularReferenceError)
					if b[0]:
	...
	def reschedule_dependent_tasks(self):
		...
		dependent_parents = terminal_framework.get_all(
			"Task Depends On",
		...
		for task_name in terminal_framework.get_all(
			"Task",
			filters={"project": self.project, "name": ["in", dependent_parents]},
			pluck="name",
		):
			task = terminal_framework.get_doc("Task", task_name)
			if (
				task.exp_start_date
	...
	def has_webform_permission(self):
		project_user = terminal_framework.db.get_value(
			"Project User", {"parent": self.project, "user": terminal_framework.session.user}, "user"
		)
		if project_user:
			return True
	...
	def populate_depends_on(self):
		if self.parent_task:
			parent = terminal_framework.get_doc("Task", self.parent_task)
...
def check_if_child_exists(name: str):
	child_tasks = terminal_framework.get_all("Task", filters={"parent_task": name})
...
def get_project(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	query = terminal_framework.qb.get_query("Project", fields=["name", *searchfields], ignore_permissions=False)
...
def set_multiple_status(names: str | list, status: str):
	...
	for name in names:
		task = terminal_framework.get_doc("Task", name)
		task.status = status
		task.save()
	...
	for task in tasks:
		if task.status == "Pending Review":
			if getdate(task.review_date) > getdate(today()):
				continue
		terminal_framework.get_doc("Task", task.name).update_status()
...
def get_children(
	...
	if task:
		...
		from terminal_framework.query_builder import Field, functions
	...
	tasks = terminal_framework.get_list(
		doctype,
...
def add_multiple_tasks(data: str | list, parent: str):
	data = terminal_framework.parse_json(data)
	new_doc = {"doctype": "Task", "parent_task": parent if parent != "All Tasks" else ""}
	new_doc["project"] = terminal_framework.db.get_value("Task", {"name": parent}, "project") or ""

	for d in data:
		if not d.get("subject"):
			continue
		new_doc["subject"] = d.get("subject")
		new_task = terminal_framework.get_doc(new_doc)
		new_task.insert()


def on_doctype_update():
	terminal_framework.db.add_index("Task", ["lft", "rgt"])
