# Copyright (c) 2015, Terminal Framework Technologies Pvt. Ltd. and Contributors
# License: GNU General Public License v3. See license.txt

import terminal_framework
from terminal_framework.utils import add_days, getdate, nowdate

from terminal_erp.projects.doctype.task.task import CircularReferenceError, ParentIsGroupError
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestTask(Terminal ERPTestSuite):
	def test_task_total_costing_and_billing_amount(self):
		from terminal_erp.projects.doctype.project.test_project import make_project
		from terminal_erp.projects.doctype.timesheet.test_timesheet import make_timesheet
		from terminal_erp.setup.doctype.employee.test_employee import make_employee

		project_name = "Test Project Costing"
		employee = make_employee("employee@terminal_framework.io", company="_Test Company")
		project = make_project({"project_name": project_name})
		task = create_task("_Test Task 1")
		task.project = project.name
		task.save()
		timesheet = make_timesheet(
			employee=employee,
			is_billable=1,
			currency="USD",
			project=project.name,
			simulate=True,
			exchange_rate=80,
			task=task.name,
		)
		timesheet.reload()
		project.reload()
		task.reload()
		self.assertEqual(task.total_costing_amount, 3200)
		self.assertEqual(task.total_billing_amount, 8000)

	def test_circular_reference(self):
		task1 = create_task("_Test Task 1", add_days(nowdate(), -15), add_days(nowdate(), -10))
		task2 = create_task("_Test Task 2", add_days(nowdate(), 11), add_days(nowdate(), 15), task1.name)
		task3 = create_task("_Test Task 3", add_days(nowdate(), 11), add_days(nowdate(), 15), task2.name)

		task1.reload()
		task1.append("depends_on", {"task": task3.name})

		self.assertRaises(CircularReferenceError, task1.save)

		task1.set("depends_on", [])
		task1.save()

		task4 = create_task("_Test Task 4", nowdate(), add_days(nowdate(), 15), task1.name)

		task3.append("depends_on", {"task": task4.name})

	def test_reschedule_dependent_task(self):
		project = terminal_framework.get_value("Project", {"project_name": "_Test Project"})

		task1 = create_task("_Test Task 1", nowdate(), add_days(nowdate(), 10))

		task2 = create_task("_Test Task 2", add_days(nowdate(), 11), add_days(nowdate(), 15), task1.name)
		task2.get("depends_on")[0].project = project
		task2.save()

		task3 = create_task("_Test Task 3", add_days(nowdate(), 11), add_days(nowdate(), 15), task2.name)
		task3.get("depends_on")[0].project = project
		task3.save()

		task1.update({"exp_end_date": add_days(nowdate(), 20)})
		task1.save()

		self.assertEqual(
			getdate(terminal_framework.db.get_value("Task", task2.name, "exp_start_date")),
			getdate(add_days(nowdate(), 21)),
		)

		self.assertEqual(
			getdate(terminal_framework.db.get_value("Task", task2.name, "exp_end_date")), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			getdate(terminal_framework.db.get_value("Task", task3.name, "exp_start_date")),
			getdate(add_days(nowdate(), 26)),
		)

		self.assertEqual(
			getdate(terminal_framework.db.get_value("Task", task3.name, "exp_end_date")), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not terminal_framework.db.exists("Task", "Test Close Assignment"):
			task = terminal_framework.new_doc("Task")
			task.subject = "Test Close Assignment"
			task.insert()

		def assign():
			from terminal_framework.desk.form import assign_to

			assign_to.add(
				{
					"assign_to": ["test@example.com"],
					"doctype": task.doctype,
					"name": task.name,
					"description": "Close this task",
				}
			)

		def get_owner_and_status():
			return terminal_framework.db.get_value(
				"ToDo",
				filters={
					"reference_type": task.doctype,
					"reference_name": task.name,
					"description": "Close this task",
				},
				fieldname=("allocated_to", "status"),
				as_dict=True,
			)

		assign()
		todo = get_owner_and_status()
		self.assertEqual(todo.allocated_to, "test@example.com")
		self.assertEqual(todo.status, "Open")

		# assignment should be
		task.load_from_db()
		task.status = "Completed"
		task.save()
		todo = get_owner_and_status()
		self.assertEqual(todo.allocated_to, "test@example.com")
		self.assertEqual(todo.status, "Closed")

	def test_overdue(self):
		task = create_task("Testing Overdue", add_days(nowdate(), -10), add_days(nowdate(), -5))

		from terminal_erp.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(terminal_framework.db.get_value("Task", task.name, "status"), "Overdue")

	def test_parent_task_must_be_group(self):
		parent_task = create_task(
			subject="_Test Parent Task Non Group",
			is_group=0,
		)

		child_task = create_task(
			subject="_Test Child Task",
			parent_task=parent_task.name,
			save=False,
		)

		self.assertRaises(ParentIsGroupError, child_task.save)

	def test_expected_end_date(self):
		task = create_task("Testing End Date", add_days(nowdate(), 1), add_days(nowdate(), 5))
		task.expected_time = 72
		task.save()
		self.assertEqual(getdate(task.exp_end_date), getdate(add_days(nowdate(), 5)))

	def test_set_multiple_status(self):
		from terminal_erp.projects.doctype.task.task import set_multiple_status

		task1 = create_task("_Test Bulk Status 1")
		task2 = create_task("_Test Bulk Status 2")

		set_multiple_status(terminal_framework.as_json([task1.name, task2.name]), "Completed")

		self.assertEqual(terminal_framework.db.get_value("Task", task1.name, "status"), "Completed")
		self.assertEqual(terminal_framework.db.get_value("Task", task2.name, "status"), "Completed")

	def test_add_multiple_tasks_under_parent(self):
		from terminal_erp.projects.doctype.task.task import add_multiple_tasks

		parent = create_task("_Test Bulk Parent", is_group=1)
		rows = [{"subject": "_Test Bulk Child A"}, {"subject": ""}, {"subject": "_Test Bulk Child B"}]

		add_multiple_tasks(terminal_framework.as_json(rows), parent.name)

		children = terminal_framework.get_all("Task", filters={"parent_task": parent.name}, pluck="subject")
		# the row with a blank subject is skipped
		self.assertEqual(sorted(children), ["_Test Bulk Child A", "_Test Bulk Child B"])

	def test_template_task_dependency_must_be_template(self):
		normal_task = create_task("_Test Non Template Dependency")
		template_task = create_task("_Test Template With Dependency", is_template=1, save=False)
		template_task.append("depends_on", {"task": normal_task.name})

		self.assertRaises(terminal_framework.ValidationError, template_task.save)

	def test_cannot_delete_task_with_children(self):
		parent = create_task("_Test Parent With Child", is_group=1)
		create_task("_Test Child Blocking Delete", parent_task=parent.name)

		self.assertRaises(terminal_framework.ValidationError, parent.delete)

	def test_child_task_registers_in_parent_depends_on(self):
		parent = create_task("_Test Parent Depends On", is_group=1)
		child = create_task("_Test Child Depends On", parent_task=parent.name)

		parent.reload()
		self.assertIn(child.name, [row.task for row in parent.depends_on])


def create_task(
	subject,
	start=None,
	end=None,
	depends_on=None,
	project=None,
	parent_task=None,
	is_group=0,
	is_template=0,
	begin=0,
	duration=0,
	save=True,
	priority=None,
):
	if not terminal_framework.db.exists("Task", subject):
		task = terminal_framework.new_doc("Task")
		task.status = "Open"
		task.subject = subject
		task.exp_start_date = start or nowdate()
		task.exp_end_date = end or nowdate()
		task.project = (
			project or None if is_template else terminal_framework.get_value("Project", {"project_name": "_Test Project"})
		)
		task.is_template = is_template
		task.start = begin
		task.duration = duration
		task.is_group = is_group
		task.parent_task = parent_task
		task.priority = priority
		if save:
			task.save()
	else:
		task = terminal_framework.get_doc("Task", subject)

	if depends_on:
		task.append("depends_on", {"task": depends_on})
		if save:
			task.save()
	return task
