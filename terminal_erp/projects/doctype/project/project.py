...
from terminal_framework.query_builder import Case, Interval
from terminal_framework.query_builder.functions import Count, CurDate, Date, Locate, Lower, Sum, UnixTimestamp
from terminal_framework.utils import add_days, flt, get_datetime, get_link_to_form, get_time, nowtime, today
...
from terminal_erp.setup.doctype.holiday_list.holiday_list import is_holiday
...
class Project(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.projects.doctype.project_user.project_user import ProjectUser
	...
	def onload(self):
		timesheet_detail = terminal_framework.qb.DocType("Timesheet Detail")
		self.set_onload(
			"activity_summary",
			terminal_framework.qb.from_(timesheet_detail)
			...
			.orderby("total_hours", order=terminal_framework.qb.desc)
			.run(as_dict=True),
		)
	...
	def copy_from_template(self, trigger=None):
		...
		"""
		if self.project_template and not terminal_framework.db.exists("Task", {"project": self.name}):
			...
			template = terminal_framework.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = terminal_framework.get_doc("Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for project_task in project_tasks:
			template_task = terminal_framework.get_doc("Task", project_task.template_task)

			self.check_depends_on_value(template_task, project_task, project_tasks)
			self.check_for_parent_tasks(template_task, project_task, project_tasks)

	def set_consumed_material_cost(self):
		parent_doc = terminal_framework.qb.DocType("Stock Entry")
		child_doc = terminal_framework.qb.DocType("Stock Entry Detail")
		lcv_doc = terminal_framework.qb.DocType("Landed Cost Taxes and Charges")
	...
	def link_with_sales_order(self) -> None:
		...
		existing_project = terminal_framework.db.get_value("Sales Order", self.sales_order, "project")
		...
		terminal_framework.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		terminal_framework.db.set_value("Sales Order", {"project": self.name}, "project", "")

	def update_percent_complete(self):
		if self.status == "Completed":
			if (
				len(terminal_framework.get_all("Task", dict(project=self.name))) == 0
		...
		total = terminal_framework.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if (self.percent_complete_method == "Task Completion" and total > 0) or (
				...
				completed = terminal_framework.db.count(
			...
			if self.percent_complete_method == "Task Progress" and total > 0:
				...
				progress = (
					terminal_framework.qb.from_(task).select(Sum(task.progress)).where(task.project == self.name).run()
				)[0][0]
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				task = terminal_framework.qb.DocType("Task")
				weight_sum = (
					terminal_framework.qb.from_(task).select(Sum(task.task_weight)).where(task.project == self.name).run()
				)[0][0]
				weighted_progress = terminal_framework.get_all(
					"Task", filters={"project": self.name}, fields=["progress", "task_weight"]
				)
				pct_complete = 0
				for row in weighted_progress:
					pct_complete += row["progress"] * terminal_framework.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_costing(self):
		...
		TimesheetDetail = terminal_framework.qb.DocType("Timesheet Detail")
		from_time_sheet = (
			terminal_framework.qb.from_(TimesheetDetail)
			.select(
	...
	def update_sales_amount(self):
		...
		total_sales_amount = (
			terminal_framework.qb.from_(so)
	...
	def get_billed_amount_from_parent(self):
		...
		si_item = terminal_framework.qb.DocType("Sales Invoice Item")
		total_billed_amount = (
	...
	def get_billed_amount_from_child(self):
		si_item = terminal_framework.qb.DocType("Sales Invoice Item")
		total_billed_amount = (
			terminal_framework.qb.from_(si_item)
			.select(Sum(si_item.base_net_amount))
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			terminal_framework.db.set_value("Project", new_name, "copied_from", new_name)
	...
	def send_welcome_email(self):
		...
		for user in self.users:
			...
			if user.welcome_email_sent == 0:
				...
				user_info = terminal_framework.db.get_value("User", user.user, ["enabled", "email"], as_dict=True)
				# send email only if user is enabled and has a valid email
				if user_info and user_info.enabled and user_info.email:
					terminal_framework.sendmail(
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="creation"):
	customers, suppliers = get_customers_suppliers("Project", terminal_framework.session.user)

	ignore_permissions = False
	if is_website_user() and terminal_framework.session.user != "Guest":
		if not filters:
			filters = []
	...
	# terminal_framework drop ORDER BY on Postgres, leaving the portal list unordered there.
...
def get_users_for_project(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	User = terminal_framework.qb.DocType("User")
	search_str = f"%{txt}%"
	txt_no_percent = txt.replace("%", "")

	query = terminal_framework.qb.get_query(
		"User",
...
def get_cost_center_name(project: str):
	return terminal_framework.db.get_value("Project", project, "cost_center")
...
def allow_to_make_project_update(project, time, frequency):
	data = terminal_framework.get_all("Project Update", filters={"project": project, "date": today()}, pluck="name")
...
def create_duplicate_project(prev_doc: str | dict, project_name: str):
	...
	prev_doc = terminal_framework.parse_json(prev_doc)

	if project_name == prev_doc.get("name"):
		terminal_framework.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = terminal_framework.copy_doc(prev_doc)
	...
	task_list = terminal_framework.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = terminal_framework.get_doc("Task", task)
		new_task = terminal_framework.copy_doc(task)
...
def send_project_update_email_to_users(project):
	doc = terminal_framework.get_doc("Project", project)

	if is_holiday(doc.holiday_list) or not doc.users:
		return

	project_update = terminal_framework.get_doc(
		{
	...
	incoming_email_account = terminal_framework.db.get_value(
...
def collect_project_status():
	for data in terminal_framework.get_all("Project Update", {"date": today(), "sent": 0}):
		replies = terminal_framework.get_all(
			"Communication",
		...
		for d in replies:
			doc = terminal_framework.get_doc("Project Update", data.name)
			user_data = terminal_framework.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": terminal_framework.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in terminal_framework.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = terminal_framework.get_doc("Project Update", d.name)

		project_doc = terminal_framework.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = terminal_framework.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and terminal_framework.utils.now_datetime().day != 1:
		return

	# Else simply fallback to Daily
	for project in terminal_framework.get_all("Project", filters={"status": ["!=", "Cancelled"]}):
		terminal_framework.get_doc("Project", project.name).save()


@terminal_framework.whitelist()
def create_kanban_board_if_not_exists(project: str):
	from terminal_framework.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = terminal_framework.get_doc("Project", project)
	if not terminal_framework.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project: str, status: str):
	...
	if status not in ("Completed", "Cancelled"):
		terminal_framework.throw(_("Status must be Cancelled or Completed"))

	project = terminal_framework.get_doc("Project", project)
	project.check_permission("write")

	for task in terminal_framework.get_all("Task", dict(project=project.name)):
		terminal_framework.db.set_value("Task", task.name, "status", status)
...
def get_holiday_list(company: str | None = None) -> str:
	if not company:
		company = get_default_company() or terminal_framework.get_all("Company")[0].name

	holiday_list = terminal_framework.get_cached_value("Company", company, "default_holiday_list")
	if not holiday_list:
		terminal_framework.throw(
			_("Please set a default Holiday List for Company {0}").format(terminal_framework.bold(get_default_company()))
		)
	return holiday_list


def get_users_email(doc):
	return [d.email for d in doc.users if terminal_framework.db.get_value("User", d.user, "enabled")]
