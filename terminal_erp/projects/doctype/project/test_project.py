...
from terminal_erp.projects.doctype.project_template.test_project_template import make_project_template
from terminal_erp.projects.doctype.task.test_task import create_task
from terminal_erp.selling.doctype.sales_order.mapper import make_project as make_project_from_so
from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestProject(Terminal ERPTestSuite):
	def test_get_timeline_data_runs(self):
		...
		from terminal_erp.projects.doctype.project.project import get_timeline_data
		from terminal_erp.projects.doctype.timesheet.test_timesheet import make_timesheet
	...
	def test_project_total_costing_and_billing_amount(self):
		...
		employee = make_employee("employee@terminal_framework.io", company="_Test Company")
	...
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		...
		terminal_framework.db.delete("Task", {"project": project_name})
		terminal_framework.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if terminal_framework.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = terminal_framework.db.get_value("Project", {"project_name": project_name}, "name")
	...
	def test_sales_order_link_is_not_overwritten_by_second_project(self):
		...
		second_project = terminal_framework.get_doc(
			doctype="Project",
	...
	def test_project_with_template_tasks_having_common_name(self):
		...
		project_tasks = terminal_framework.get_all(
	...
	def test_project_having_no_tasks_complete(self):
		...
		project = terminal_framework.get_doc(
			{
				"doctype": "Project",
	...
	def _project_with_tasks(self, method, count):
		name = f"_Test PercentComplete {terminal_framework.generate_hash(length=8)}"
		project = terminal_framework.get_doc(
			{
				"doctype": "Project",
				"project_name": name,
		...
		for i in range(count):
			task = terminal_framework.get_doc(
				{
					"doctype": "Task",
	...
	def test_percent_complete_by_task_completion(self):
		...
		terminal_framework.db.set_value("Task", tasks[0], "status", "Completed")
		project.update_percent_complete()
		self.assertEqual(project.percent_complete, 25)  # 1 of 4

		for task in tasks:
			terminal_framework.db.set_value("Task", task, "status", "Completed")
		project.update_percent_complete()
		...
		terminal_framework.db.set_value("Task", tasks[0], "status", "Open")
		project.update_percent_complete()
	...
	def test_percent_complete_by_task_progress(self):
		...
		terminal_framework.db.set_value("Task", tasks[0], "progress", 50)
		terminal_framework.db.set_value("Task", tasks[1], "progress", 100)
		project.update_percent_complete()
	...
	def test_percent_complete_by_task_weight(self):
		...
		terminal_framework.db.set_value("Task", tasks[0], {"progress": 100, "task_weight": 3})
		terminal_framework.db.set_value("Task", tasks[1], {"progress": 0, "task_weight": 1})
		project.update_percent_complete()
		self.assertEqual(project.percent_complete, 75)  # 100 * 3/4 + 0 * 1/4

	def test_create_duplicate_project_copies_tasks(self):
		from terminal_erp.projects.doctype.project.project import create_duplicate_project

		source, tasks = self._project_with_tasks("Task Completion", 2)
		new_name = f"{source.project_name} Copy"

		create_duplicate_project(terminal_framework.as_json(source.as_dict()), new_name)

		# Project is named by series, so look the copy up by its project_name
		new_project = terminal_framework.db.get_value("Project", {"project_name": new_name})
		self.assertTrue(new_project)
		copied_tasks = terminal_framework.get_all("Task", filters={"project": new_project})
	...
	def test_create_duplicate_project_rejects_same_name(self):
		...
		self.assertRaises(
			...
			terminal_framework.as_json(source.as_dict()),
			source.name,
		)

	def test_set_project_status_updates_project_and_tasks(self):
		from terminal_erp.projects.doctype.project.project import set_project_status

		project, tasks = self._project_with_tasks("Task Completion", 2)

		set_project_status(project.name, "Cancelled")

		self.assertEqual(terminal_framework.db.get_value("Project", project.name, "status"), "Cancelled")
		for task in tasks:
			self.assertEqual(terminal_framework.db.get_value("Task", task, "status"), "Cancelled")

	def test_set_project_status_rejects_invalid_status(self):
		from terminal_erp.projects.doctype.project.project import set_project_status

		project, _ = self._project_with_tasks("Task Completion", 1)
		self.assertRaises(terminal_framework.ValidationError, set_project_status, project.name, "Open")
	...
	def test_costing_rollup_from_sales_documents(self):
		...
		from terminal_erp.projects.doctype.project.project import update_costing_and_billing
		from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order

		project = make_project({"project_name": f"_Test Costing Rollup {terminal_framework.generate_hash(length=6)}"})
	...
	def test_consumed_material_cost_from_stock_entry(self):
		...
		project = make_project({"project_name": f"_Test Consumed Cost {terminal_framework.generate_hash(length=6)}"})
...
def make_project(args):
	...
	if args.project_name and terminal_framework.db.exists("Project", {"project_name": args.project_name}):
		return terminal_framework.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = terminal_framework.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return terminal_framework.get_doc("Task", result[0].name)
