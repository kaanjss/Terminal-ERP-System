...
from terminal_framework.utils import add_to_date, now_datetime, nowdate
...
from terminal_erp.projects.doctype.timesheet.timesheet import OverlapError, make_sales_invoice
from terminal_erp.setup.doctype.employee.test_employee import make_employee
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestTimesheet(Terminal ERPTestSuite):
	def test_timesheet_post_update(self):
		...
		timesheet = terminal_framework.new_doc("Timesheet")
		timesheet.append(
			"time_logs",
			{
				"task": task.name,
	...
	def test_sales_invoice_from_timesheet(self):
		...
		timesheet = terminal_framework.get_doc("Timesheet", timesheet.name)
	...
	@Terminal ERPTestSuite.change_settings("Projects Settings", {"fetch_timesheet_in_sales_invoice": 1})
	def test_timesheet_billing_based_on_project(self):
		...
		ts = terminal_framework.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_billing_validations(self):
		...
		with self.subTest("unsubmitted timesheet is rejected"):
			...
			self.assertRaises(terminal_framework.ValidationError, sales_invoice.save)
	...
	def test_timesheet_time_overlap(self):
		...
		self.assertRaises(terminal_framework.ValidationError, timesheet.save)
		...
		self.assertRaises(terminal_framework.ValidationError, timesheet.submit)
	...
	def test_per_billed_hours(self):
		...
		ts = terminal_framework.new_doc("Timesheet")
	...
	def test_partial_billing_and_return(self):
		...
		timesheet_status = terminal_framework.get_value("Timesheet", timesheet.name, "status")
	...
	def test_get_timesheets_list_portal_sales_invoice(self):
		...
		from terminal_erp.projects.doctype.timesheet.timesheet import get_timesheets_list
		...
		terminal_framework.db.set_value("Timesheet", timesheet.name, "sales_invoice", si.name)
	...
	def test_get_activity_cost_falls_back_to_activity_type(self):
		from terminal_erp.projects.doctype.timesheet.timesheet import get_activity_cost
	...
	def test_billing_helpers_for_timesheet_detail(self):
		from terminal_erp.projects.doctype.timesheet.timesheet import (
			get_timesheet_data,
	...
	def _delete_if_exists(doctype, name):
		if terminal_framework.db.exists(doctype, name):
			terminal_framework.delete_doc(doctype, name, force=True)
...
def update_activity_type(activity_type):
	activity_type = terminal_framework.get_doc("Activity Type", activity_type)
