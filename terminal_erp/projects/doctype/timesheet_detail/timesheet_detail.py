...
from terminal_framework.utils import add_to_date, flt, get_datetime, time_diff_in_hours, time_diff_in_seconds
...
class TimesheetDetail(Document):
	...
	def set_project(self):
		...
		if self.task and not self.project:
			self.project = terminal_framework.db.get_value("Task", self.task, "project")
	...
	def update_cost(self, employee: str):
		...
		exchange_rate = flt(terminal_framework.get_value("Timesheet", self.parent, "exchange_rate")) or 1.0
	...
	def validate_dates(self):
		...
		if self.from_time and self.to_time and time_diff_in_hours(self.to_time, self.from_time) < 0:
			terminal_framework.throw(_("To Time cannot be before From Time"))
	...
	def validate_task_project(self):
		...
		if self.task and self.project:
			task_project = terminal_framework.db.get_value("Task", self.task, "project")
			if task_project and task_project != self.project:
				terminal_framework.throw(
					_("Row {0}: Task {1} does not belong to Project {2}").format(
						self.idx, terminal_framework.bold(self.task), terminal_framework.bold(self.project)
					)
				)
