...
from terminal_framework.utils import flt, random_string, today

from terminal_erp.projects.report.project_wise_stock_tracking.project_wise_stock_tracking import execute
from terminal_erp.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestProjectWiseStockTracking(Terminal ERPTestSuite):
	def test_project_wise_stock_tracking(self):
		...
		terminal_framework.db.set_value("Delivery Note", dn, "project", project.name)
	...
	def make_parent_row(self, doctype, **fields):
		doc = terminal_framework.new_doc(doctype)
		for key, value in fields.items():
			doc.set(key, value)
		doc.posting_date = today()
		doc.docstatus = 1
		doc.flags.name_set = True
		doc.name = terminal_framework.generate_hash("pwst", 12)
		doc.db_insert()
		return doc.name

	def make_child_row(self, doctype, parenttype, base_net_amount, project=None, parent=None):
		row = terminal_framework.new_doc(doctype)
		row.parenttype = parenttype
		row.parentfield = "items"
		row.parent = parent or terminal_framework.generate_hash("pwst", 12)
		row.idx = 1
		...
		row.name = terminal_framework.generate_hash("pwst", 12)
		row.db_insert()
		return row.name
