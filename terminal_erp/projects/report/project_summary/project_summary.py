...
def execute(filters=None):
	...
	data = terminal_framework.db.get_all(
		"Project",
		filters=filters,
		fields=[
			"name",
	...
	for project in data:
		project["total_tasks"] = terminal_framework.db.count("Task", filters={"project": project.name})
		project["completed_tasks"] = terminal_framework.db.count(
			"Task", filters={"project": project.name, "status": "Completed"}
		)
		project["overdue_tasks"] = terminal_framework.db.count(
