...
from terminal_erp.accounts.report.trial_balance.trial_balance import validate_filters
...
def get_data(filters, dimension_list):
	company_currency = terminal_erp.get_company_currency(filters.company)

	acc = terminal_framework.get_all(
		"Account",
	...
	lft_rgt = terminal_framework.get_all(
		"Account",
		filters={"company": filters.company},
	...
	format_gl_entries(
		gl_entries_by_account, accounts_by_name, dimension_list, terminal_framework.scrub(filters.get("dimension"))
	)
...
def set_gl_entries_by_account(dimension_list, filters, account, gl_entries_by_account):
	dimension_field = terminal_framework.scrub(filters.get("dimension"))
		...
		for entry in entries:
			...
			for dimension in dimension_list:
				if dimension == entry.get(dimension_type):
					d[terminal_framework.scrub(dimension)] = (
						d.get(terminal_framework.scrub(dimension), 0.0) + flt(entry.debit) - flt(entry.credit)
					)
...
def prepare_data(accounts, filters, company_currency, dimension_list):
	...
	for d in accounts:
		...
		for dimension in dimension_list:
			row[terminal_framework.scrub(dimension)] = flt(d.get(terminal_framework.scrub(dimension), 0.0), 3)

			if abs(row[terminal_framework.scrub(dimension)]) >= get_zero_cutoff(company_currency):
				# ignore zero values
				has_value = True
				total += flt(d.get(terminal_framework.scrub(dimension), 0.0), 3)
...
def accumulate_values_into_parents(accounts, accounts_by_name, dimension_list):
	...
	for d in reversed(accounts):
		if d.parent_account:
			for dimension in dimension_list:
				accounts_by_name[d.parent_account][terminal_framework.scrub(dimension)] = accounts_by_name[
					d.parent_account
				].get(terminal_framework.scrub(dimension), 0.0) + d.get(terminal_framework.scrub(dimension), 0.0)


def get_dimensions(filters):
	meta = terminal_framework.get_meta(filters.get("dimension"), cached=False)
	query_filters = {}

	if meta.has_field("company"):
		query_filters = {"company": filters.get("company")}

	return terminal_framework.get_all(filters.get("dimension"), filters=query_filters, pluck="name")
...
def get_columns(dimension_list):
	...
	for dimension in dimension_list:
		columns.append(
			{
				"fieldname": terminal_framework.scrub(dimension),
				"label": dimension,
				"fieldtype": "Currency",
				"options": "currency",
				"width": 150,
