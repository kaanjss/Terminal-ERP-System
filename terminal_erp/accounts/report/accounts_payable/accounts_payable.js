...
terminal_framework.query_reports["Accounts Payable"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			on_change: function () {
				terminal_framework.query_report.set_filter_value("party", "");
				terminal_framework.query_report.toggle_filter_display(
					"supplier_group",
					terminal_framework.query_report.get_filter_value("party_type") !== "Supplier"
				);
			},
		},
		{
			...
			get_data: function (txt) {
				...
				return terminal_framework.db.get_link_options(party_type, txt);
			},
		},
		{
	...
	onload: function (report) {
		report.page.add_inner_button(__("Accounts Payable Summary"), function () {
			...
			terminal_framework.set_route("query-report", "Accounts Payable Summary", { company: filters.company });
		});

		if (terminal_framework.boot.sysdefaults.default_ageing_range) {
			report.set_filter_value("range", terminal_framework.boot.sysdefaults.default_ageing_range);
		}
		...
		if (terminal_framework.model.can_create("Payment Entry")) {
			report.page.add_inner_button(
				...
				function () {
					terminal_erp.accounts.create_payment_entries_from_payable_report(report);
				},
				__("Actions")
			);
		}
...
terminal_erp.accounts.create_payment_entries_from_payable_report = function (report) {
	...
	if (!rows.length) {
		terminal_framework.msgprint(__("Select one or more Purchase Invoice rows"));
		return;
	}
	...
	const dialog = new terminal_framework.ui.Dialog({
		...
		primary_action() {
			...
			terminal_framework
				.call({
					method: "terminal_erp.accounts.bulk_payment.create_payment_entries",
...
terminal_erp.utils.add_dimensions("Accounts Payable", 10);
