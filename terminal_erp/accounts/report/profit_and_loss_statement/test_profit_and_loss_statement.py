...
from terminal_framework.desk.query_report import export_query
...
from terminal_erp.accounts.report.profit_and_loss_statement.profit_and_loss_statement import execute
from terminal_erp.accounts.test.accounts_mixin import AccountsTestMixin
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestProfitAndLossStatement(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def get_fiscal_year(self):
		active_fy = terminal_framework.db.get_all(
			"Fiscal Year",
			filters={"disabled": 0, "year_start_date": ("<=", today()), "year_end_date": (">=", today())},
		)[0]
		return terminal_framework.get_doc("Fiscal Year", active_fy.name)
	...
	def test_p_and_l_export(self):
		...
		filters = self.get_report_filters()
		terminal_framework.local.form_dict = terminal_framework._dict(
			{
		...
		export_query()
		contents = terminal_framework.response["filecontent"].decode()
		sales_account = terminal_framework.db.get_value("Company", self.company, "default_income_account")
	...
	def test_accumulate_filter(self):
		...
		_x = terminal_framework.db.get_all(
			"Fiscal Year",
			filters={"disabled": 0, "year_start_date": ("<=", find_for), "year_end_date": (">=", find_for)},
		)[0]
		prev_fy = terminal_framework.get_doc("Fiscal Year", _x.name)
