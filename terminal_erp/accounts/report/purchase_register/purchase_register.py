...
def _execute(filters=None, additional_table_columns=None):
	...
	if filters.get("include_payments") and not filters.get("supplier"):
		terminal_framework.throw(_("Please select a supplier for fetching payments."))
	...
	company_currency = terminal_framework.get_cached_value("Company", filters.company, "default_currency")
	...
	for inv in invoice_list:
		...
		for expense_acc in expense_accounts:
			...
			row.update({terminal_framework.scrub(expense_acc): expense_amount})
		...
		outstanding_precision = (
			get_field_precision(
				terminal_framework.get_meta("Purchase Invoice").get_field("outstanding_amount"),
				currency=company_currency,
			)
			or 2
		)
...
def get_account_columns(invoice_list, include_payments):
	...
	if invoice_list:
		# terminal_framework drops ORDER BY for distinct queries on postgres (db_query), so sort in python with
...
def get_invoice_expense_map(invoice_list):
	expense_details = terminal_framework.get_all(
		"Purchase Invoice Item",
	...
	for d in expense_details:
		invoice_expense_map.setdefault(d.parent, terminal_framework._dict()).setdefault(d.expense_account, [])
...
def get_invoice_tax_map(invoice_list, invoice_expense_map, expense_accounts, include_payments=False):
	...
	tax_details = (
		terminal_framework.qb.from_(ptc)
...
def get_invoice_po_pr_map(invoice_list):
	pi_items = terminal_framework.get_all(
		"Purchase Invoice Item",
		filters={"parent": ["in", [inv.name for inv in invoice_list]], "parenttype": "Purchase Invoice"},
	...
	for d in pi_items:
		if d.purchase_order:
			invoice_po_pr_map.setdefault(d.parent, terminal_framework._dict()).setdefault("purchase_order", []).append(
				d.purchase_order
			)
		...
		if d.purchase_receipt:
			...
			pr_list = terminal_framework.get_all(
		...
		if pr_list:
			invoice_po_pr_map.setdefault(d.parent, terminal_framework._dict()).setdefault("purchase_receipt", pr_list)

		if d.project:
			invoice_po_pr_map.setdefault(d.parent, terminal_framework._dict()).setdefault("project", []).append(d.project)
...
def get_account_details(invoice_list):
	...
	for acc in terminal_framework.get_all(
