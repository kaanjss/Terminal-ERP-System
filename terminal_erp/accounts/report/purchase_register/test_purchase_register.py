...
from terminal_framework.utils import add_months, flt, today

from terminal_erp.accounts.report.purchase_register.purchase_register import execute
from terminal_erp.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestPurchaseRegister(Terminal ERPTestSuite):
	def test_purchase_register(self):
		filters = terminal_framework._dict(company="_Test Company 6", from_date=add_months(today(), -1), to_date=today())

		pi = make_purchase_invoice()

		report_results = execute(filters)
		first_row = terminal_framework._dict(report_results[1][0])
	...
	def test_expense_account_columns_sorted_case_insensitively(self):
		...
		# be identical on both engines. terminal_framework drops ORDER BY for distinct queries on postgres, so the
		...
		filters = terminal_framework._dict(company=company, from_date=add_months(today(), -1), to_date=today())
	...
	def test_purchase_register_ignores_tax_rows_from_other_doctype(self):
		...
		terminal_framework.rename_doc("Purchase Receipt", pr.name, pi.name, force=True)
	...
	def test_purchase_currency_conversion(self):
		usd_creditors = terminal_framework.get_doc(
			{
				"doctype": "Account",
		...
		columns, data, *_ = execute(terminal_framework._dict({"company": foreign_invoice.company}))
	...
	def test_purchase_register_ledger_view(self):
		...
		report_results = execute(filters)
		first_row = terminal_framework._dict(report_results[1][2])
