...
from terminal_framework import _, qb, scrub
from terminal_framework.query_builder import Case, Order
from terminal_framework.query_builder.functions import Coalesce
from terminal_framework.utils import cint, flt, formatdate
...
from terminal_erp.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	filters.currency = terminal_framework.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = terminal_framework._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
		...
		terminal_framework._dict(
			{
				...
				"gross_profit_%": flt(
					...
					cint(terminal_framework.db.get_default("currency_precision")) or 3,
				)
...
def get_data_when_not_grouped_by_invoice(gross_profit_data, filters, group_wise_columns, data):
	...
	currency_precision = cint(terminal_framework.db.get_default("currency_precision")) or 3
...
def get_columns(group_wise_columns, filters):
	...
	column_map = terminal_framework._dict(
		{
			"parent": {
...
class GrossProfitGenerator:
	def __init__(self, filters=None):
		...
		self.filters = terminal_framework._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(terminal_framework.db.get_default("currency_precision")) or 3
		self.float_precision = cint(terminal_framework.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, terminal_framework._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, terminal_framework._dict()
				)
	...
	def get_returned_invoice_items(self):
		...
		self.returned_invoices = terminal_framework._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, terminal_framework._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_buying_amount(self, row, item_code):
		...
		if (
			...
			and row.so_detail
			and (
				po_details := terminal_framework.get_all(
					"Purchase Order Item",
			...
			table = terminal_framework.qb.DocType("Purchase Invoice Item")
			query = (
	...
	def get_buying_amount_from_so_dn(self, sales_order, so_detail, item_code):
		from terminal_framework.query_builder.functions import Avg

		delivery_note_item = terminal_framework.qb.DocType("Delivery Note Item")

		query = (
			terminal_framework.qb.from_(delivery_note_item)
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		purchase_invoice_item = terminal_framework.qb.DocType("Purchase Invoice Item")

		query = (
			terminal_framework.qb.from_(purchase_invoice_item)
		...
		query = (
			query.orderby(purchase_invoice.posting_date, order=terminal_framework.qb.desc)
			.orderby(purchase_invoice.name, order=terminal_framework.qb.desc)
			.limit(1)
		)
	...
	def prepare_invoice_query(self):
		...
		SalesTeam = terminal_framework.qb.DocType("Sales Team")
		PaymentSchedule = terminal_framework.qb.DocType("Payment Schedule")
	...
	def apply_common_filters(self, query, SalesInvoice, SalesInvoiceItem, SalesTeam, Item):
		...
		if self.filters.sales_person:
			query = query.where(
				ExistsCriterion(
					terminal_framework.qb.from_(SalesTeam)
					.select(1)
					.where(
		...
		if self.filters.cost_center:
			self.filters.cost_center = terminal_framework.parse_json(self.filters.get("cost_center"))
			self.filters.cost_center = get_cost_centers_with_children(self.filters.cost_center)
			query = query.where(SalesInvoiceItem.cost_center.isin(self.filters.cost_center))

		if self.filters.project:
			self.filters.project = terminal_framework.parse_json(self.filters.get("project"))
		...
		for dim in get_accounting_dimensions(as_list=False) or []:
			if self.filters.get(dim.fieldname):
				if terminal_framework.get_cached_value("DocType", dim.document_type, "is_tree"):
		...
		if self.filters.warehouse:
			lft, rgt = terminal_framework.db.get_value("Warehouse", self.filters.warehouse, ["lft", "rgt"])
	...
	def get_delivery_notes(self):
		self.delivery_notes = terminal_framework._dict({})
		if self.si_list:
			from terminal_framework.query_builder.functions import Sum
	...
	def load_product_bundle(self):
		...
		pki_query = (
			terminal_framework.qb.from_(pki)
			.select(
				pki.parenttype,
				pki.parent,
				pki.parent_item,
		...
		for d in pki_query.run(as_dict=True):
			self.product_bundles.setdefault(d.parenttype, terminal_framework._dict()).setdefault(
				d.parent, terminal_framework._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = terminal_framework.get_all("Item", filters={"is_stock_item": 0}, pluck="name")
