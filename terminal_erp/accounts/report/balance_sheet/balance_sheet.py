...
from terminal_framework.utils import add_days, cint, flt
...
def execute(filters=None):
	...
	currency = filters.presentation_currency or terminal_framework.get_cached_value(
...
def get_provisional_profit_loss(
	...
	if asset:
		...
		currency = currency or terminal_framework.get_cached_value("Company", company, "default_currency")
		total_row = {
...
def check_opening_balance(asset, liability, equity):
	...
	opening_balance = 0
	float_precision = cint(terminal_framework.db.get_default("float_precision")) or 2
	if asset:
...
def execute_synced_report(filters):
	...
	if not (conn := get_latest_sync("GL Entry")):
		terminal_framework.throw(_("Balance Sheet requires {0} to be synced to DuckDB").format(terminal_framework.bold("GL Entry")))
...
def _load_gl_entries_duckdb(conn, filters, period_list, accounts, gl_entries_by_account, root_type):
	...
	if not ignore_closing_balances:
		last_pcv_list = terminal_framework.db.get_all(
