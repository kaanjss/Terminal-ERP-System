...
terminal_framework.query_reports["Customer Ledger Summary"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			on_change: () => {
				...
				if (party) {
					terminal_framework.db.get_value("Customer", party, ["tax_id", "customer_name"], function (value) {
						terminal_framework.query_report.set_filter_value("tax_id", value["tax_id"]);
						terminal_framework.query_report.set_filter_value("customer_name", value["customer_name"]);
					});
				} else {
					terminal_framework.query_report.set_filter_value("tax_id", "");
					terminal_framework.query_report.set_filter_value("customer_name", "");
				}
			},
		},
		{
...
terminal_erp.utils.add_dimensions("Customer Ledger Summary", 14);
