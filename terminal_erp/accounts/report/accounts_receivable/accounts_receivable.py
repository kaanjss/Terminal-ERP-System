# Copyright (c) 2015, Terminal Framework Technologies Pvt. Ltd.
...
from terminal_framework import _, qb, query_builder, scrub
from terminal_framework.query_builder import Criterion
from terminal_framework.query_builder.functions import Date, Substring, Sum
from terminal_framework.utils import cint, cstr, flt, getdate, nowdate
...
class ReceivablePayableReport:
	def __init__(self, filters=None):
		self.filters = terminal_framework._dict(filters or {})
		...
		self.ple_fetch_method = (
			terminal_framework.get_single_value("Accounts Settings", "receivable_payable_fetch_method")
			or "Buffered Cursor"
		)  # Fail Safe
		self.advance_payment_doctypes = terminal_framework.get_hooks(
			"advance_payment_receivable_doctypes"
		) + terminal_framework.get_hooks("advance_payment_payable_doctypes")

	def run(self, args):
		self.filters.update(args)
		self.set_defaults()
		self.party_naming_by = terminal_framework.db.get_single_value(args.get("naming_by")[0], args.get("naming_by")[1])
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = terminal_framework.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = terminal_framework.get_cached_value(
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			...
			si_against_dn = terminal_framework.get_all(
				"Sales Invoice Item",
			...
			dn_against_si = terminal_framework.get_all(
				"Delivery Note Item",
	...
	def get_invoice_details(self):
		self.invoice_details = terminal_framework._dict()
		if self.account_type == "Receivable":
			# nosemgrep
			si_list = terminal_framework.get_list(
				"Sales Invoice",
				filters={
			...
			if self.filters.show_sales_person:
				sales_team = terminal_framework.get_all(
					"Sales Team",
		...
		if self.account_type == "Payable":
			# nosemgrep
			invoices = terminal_framework.get_list(
				"Purchase Invoice",
				filters={
					"posting_date": ("<=", self.filters.report_date),
					"company": self.filters.company,
		...
		# nosemgrep
		journal_entries = terminal_framework.get_list(
			"Journal Entry",
			filters={
				"posting_date": ("<=", self.filters.report_date),
				"company": self.filters.company,
	...
	def get_payment_terms(self, row):
		...
		si = terminal_framework.qb.DocType(row.voucher_type)
		...
		payment_terms_details = (
			...
			.orderby(ps.paid_amount, order=terminal_framework.qb.desc)
			.orderby(ps.due_date)
			.run(as_dict=1)
		)

		original_row = terminal_framework._dict(row)
		row.payment_terms = []
		...
		company_currency = terminal_framework.get_value("Company", self.filters.get("company"), "default_currency")
		...
		for d in payment_terms_details:
			term = terminal_framework._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = terminal_framework._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = terminal_framework._dict()
	...
	def get_future_payments_from_payment_entry(self):
		...
		pe_ref = terminal_framework.qb.DocType("Payment Entry Reference")
	...
	def get_return_entries(self):
		...
		self.return_entries = terminal_framework._dict(
	...
	def prepare_ple_query(self):
		...
		if self.filters.get("show_remarks"):
			if remarks_length := terminal_framework.get_single_value(
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = terminal_framework.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = terminal_framework._dict()
			for d in records:
	...
	def add_user_permission_filters(self):
		...
		from terminal_framework.core.doctype.user_permission.user_permission import get_user_permissions
		from terminal_framework.permissions import get_allowed_docs_for_doctype
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = terminal_framework.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if party not in self.party_details:
			if self.account_type == "Receivable":
				...
				self.party_details[party] = terminal_framework.db.get_value(
					"Customer",
					party,
					fields,
					as_dict=True,
...
def get_party_group_with_children(party, party_groups):
	...
	for d in party_groups:
		if terminal_framework.db.exists(group_dtype, d):
			lft, rgt = terminal_framework.db.get_value(group_dtype, d, ["lft", "rgt"])
			children = terminal_framework.get_all(
				group_dtype, filters={"lft": [">=", lft], "rgt": ["<=", rgt]}, pluck="name"
			)
			all_party_groups += children
		else:
			terminal_framework.throw(_("{0}: {1} does not exist").format(group_dtype, d))
