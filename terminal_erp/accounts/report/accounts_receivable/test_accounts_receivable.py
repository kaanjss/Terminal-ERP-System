...
from terminal_framework.utils import add_days, flt, getdate, today
...
from terminal_erp.accounts.report.accounts_receivable.accounts_receivable import execute
...
class TestAccountsReceivable(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def test_exchange_revaluation_for_party(self):
		...
		company_doc = terminal_framework.get_doc("Company", self.company)
		...
		je = terminal_framework.get_doc("Journal Entry", err_journals.get("revaluation_jv"))
		je = je.submit()
	...
	def test_sales_person(self):
		sales_person = terminal_framework.get_doc(
	...
	def test_customer_group_filter(self):
		self.create_sales_invoice()
		cus_group = terminal_framework.db.get_value("Customer", self.customer, "customer_group")
		filters = {
			"company": self.company,
			"report_date": today(),
			"range": "30, 60, 90, 120",
	...
	def test_party_account_filter(self):
		si1 = self.create_sales_invoice()
		jane = terminal_framework.get_doc(
			{
				"doctype": "Customer",
	...
	def test_report_output_if_party_is_missing(self):
		...
		if not terminal_framework.db.get_value("Account", filters={"account_name": acc_name, "company": self.company}):
			additional_receivable_acc = terminal_framework.get_doc(
				{
					"doctype": "Account",
	...
	def test_cost_center_on_report_output(self):
		...
		new_cc = terminal_framework.get_doc(
			{
				"doctype": "Cost Center",
	...
	def test_payment_terms_template_filters(self):
		from terminal_erp.controllers.accounts_controller import get_payment_terms

		payment_term1 = terminal_framework.get_doc(
			{"doctype": "Payment Term", "payment_term_name": "_Test 50% on 15 Days"}
		).insert()
		payment_term2 = terminal_framework.get_doc(
	...
	def test_accounts_receivable_respects_user_permissions(self):
		...
		if not terminal_framework.db.exists("User", test_user):
		...
		terminal_framework.permissions.add_user_permission("Customer", original_customer, test_user)
		...
		terminal_framework.set_user(test_user)
		try:
			report = execute(filters)
		finally:
			terminal_framework.set_user("Administrator")
