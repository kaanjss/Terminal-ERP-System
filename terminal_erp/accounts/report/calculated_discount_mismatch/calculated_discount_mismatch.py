...
from terminal_framework.query_builder import Order, Tuple
from terminal_framework.utils.formatters import format_value
...
def get_data():
	...
	VERSION = terminal_framework.qb.DocType("Version")

	versions = (
		terminal_framework.qb.from_(VERSION)
	...
	discount_amount_field_map = {
		doctype: terminal_framework.get_meta(doctype).get_field("discount_amount") for doctype in AFFECTED_DOCTYPES
	}
...
def get_transactions_with_discount_percentage(doctype):
	transactions = terminal_framework.get_all(
		doctype,
		fields=[
			"name",
			"currency",
