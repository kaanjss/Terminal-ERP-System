...
from terminal_framework.query_builder.custom import MonthName
from terminal_framework.utils import add_months, flt, formatdate
...
from terminal_erp.controllers.trends import get_period_date_ranges
...
def get_budget_records(filters, dimensions):
	budget_against_field = terminal_framework.scrub(filters["budget_against"])
...
def get_actual_transactions(dimension_name, filters):
	budget_against = terminal_framework.scrub(filters.get("budget_against"))
	...
	for dimension, accounts in budget_map.items():
		for account, fiscal_year_map in accounts.items():
			...
			for period in periods:
				...
				row[terminal_framework.scrub(budget_label)] = display_budget
				row[terminal_framework.scrub(actual_label)] = display_actual
				row[terminal_framework.scrub(variance_label)] = display_budget - display_actual
	...
	for year in fiscal_year:
		for from_date, to_date in get_period_date_ranges(filters["period"], year[0]):
			if filters["period"] == "Yearly":
				...
				for label in labels:
					columns.append(
						{"label": label, "fieldtype": "Float", "fieldname": terminal_framework.scrub(label), "width": 150}
					)
			else:
				for label in [
...
def get_cost_center_with_children(cost_centers):
	...
	cc = terminal_framework.qb.DocType("Cost Center")
	all_cost_centers = set()
	for cost_center in cost_centers:
		result = terminal_framework.db.get_value("Cost Center", cost_center, ["lft", "rgt"])
		if not result:
			continue
		lft, rgt = result
		children = (
			terminal_framework.qb.from_(cc).select(cc.name).where((cc.lft >= lft) & (cc.rgt <= rgt)).run(pluck="name")
		)
...
def get_budget_dimensions(filters):
	...
	dimension = terminal_framework.qb.DocType(budget_against)

	if budget_against in ["Cost Center", "Project"]:
		query = (
			terminal_framework.qb.from_(dimension)
		...
		return terminal_framework.qb.from_(dimension).select(dimension.name).run(pluck="name")
	...
	if filters.get("budget_against") and filters.get("budget_against") not in dimensions:
		terminal_framework.throw(
			...
			msg=_("{0} is not a valid Accounting Dimension.").format(
				terminal_framework.bold(filters.get("budget_against"))
			),
		)
