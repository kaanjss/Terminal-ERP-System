...
from terminal_framework.query_builder.functions import CurDate, DateDiff
...
def get_sales_details(filters):
	...
	if filters["based_on"] not in ("Sales Order", "Sales Invoice"):
		terminal_framework.throw(_("Invalid value {0} for 'Based On'").format(filters["based_on"]))

	parent = terminal_framework.qb.DocType(filters["based_on"])
	child_doctype = "Sales Order Item" if filters["based_on"] == "Sales Order" else "Sales Invoice Item"
	child = terminal_framework.qb.DocType(child_doctype)
...
def get_territories(filters):
	...
	territories = terminal_framework.get_all("Territory", fields=["name"], filters=filter_dict)
