...
def get_data(filters):
	data = []
	depreciation_accounts = terminal_framework.get_all("Account", filters={"account_type": "Depreciation"}, pluck="name")
	...
	company_fb = terminal_framework.get_cached_value("Company", filters.get("company"), "default_finance_book")
	...
	for d in gl_entries:
		...
		if asset_data:
			if not asset_data.get("accumulated_depreciation_amount"):
				...
				query = (
					terminal_framework.qb.from_(DepreciationSchedule)
			...
			row = terminal_framework._dict(asset_data)
			row.update(
				{
...
def get_assets_details(assets):
	...
	for d in terminal_framework.get_all("Asset", fields=fields, filters={"name": ("in", assets)}):
