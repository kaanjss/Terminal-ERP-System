...
from terminal_framework.query_builder.functions import IfNull, Round
...
def get_ordered_to_be_billed_data(args, filters=None):
	...
	precision = (
		get_field_precision(
			terminal_framework.get_meta(child_tab).get_field("billed_amt"), currency=get_default_currency()
		)
		or 2
	)

	doctype = terminal_framework.qb.DocType(doctype)
	child_doctype = terminal_framework.qb.DocType(child_tab)
