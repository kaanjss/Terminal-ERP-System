...
terminal_framework.query_reports["General Ledger"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Data",
			on_change: function () {
				terminal_framework.query_report.set_filter_value("categorize_by", "Categorize by Voucher (Consolidated)");
			},
		},
		{
		...
		},
		{
			...
			options: Object.keys(terminal_framework.boot.party_account_types),
			on_change: function () {
		...
		},
		{
			...
			on_change: function () {
				...
				var parties = terminal_framework.query_report.get_filter_value("party");

				if (!party_type || parties.length === 0 || parties.length > 1) {
					terminal_framework.query_report.set_filter_value("party_name", "");
					terminal_framework.query_report.set_filter_value("tax_id", "");
					return;
				} else {
					var party = parties[0];
					var fieldname = terminal_erp.utils.get_party_name(party_type) || "name";
					terminal_framework.db.get_value(party_type, party, fieldname, function (value) {
						terminal_framework.query_report.set_filter_value("party_name", value[fieldname]);
					});

					if (party_type === "Customer" || party_type === "Supplier") {
						terminal_framework.db.get_value(party_type, party, "tax_id", function (value) {
...
terminal_erp.utils.add_dimensions("General Ledger", 15);
