...
from terminal_framework import _, _dict
from terminal_framework.query_builder import Criterion
from terminal_framework.utils import cstr, getdate

from terminal_erp import get_company_currency, get_default_company
from terminal_erp.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from terminal_erp.accounts.report.financial_statements import get_cost_centers_with_children
from terminal_erp.accounts.report.utils import convert_to_presentation_currency, get_currency
from terminal_erp.accounts.utils import get_account_currency
...
def execute(filters=None):
	...
	if filters and filters.get("print_in_account_currency") and not filters.get("account"):
		terminal_framework.throw(_("Select an account to print in account currency"))

	for acc in terminal_framework.get_all("Account", fields=["name", "is_group"]):
		account_details.setdefault(acc.name, acc)

	if filters.get("party"):
		filters.party = terminal_framework.parse_json(filters.get("party"))
...
def validate_filters(filters, account_details):
	if not filters.get("company"):
		terminal_framework.throw(_("{0} is mandatory").format(_("Company")))

	if not filters.get("from_date") and not filters.get("to_date"):
		terminal_framework.throw(
			_("{0} and {1} are mandatory").format(terminal_framework.bold(_("From Date")), terminal_framework.bold(_("To Date")))
		)

	if filters.get("account"):
		filters.account = terminal_framework.parse_json(filters.get("account"))
		for account in filters.account:
			if not account_details.get(account):
				terminal_framework.throw(_("Account {0} does not exists").format(account))
	...
	if filters.get("account") and filters.get("categorize_by") == "Categorize by Account":
		...
		for account in filters.account:
			if account_details[account].is_group == 0:
				terminal_framework.throw(_("Can not filter based on Child Account, if grouped by Account"))

	if filters.get("voucher_no") and filters.get("categorize_by") in ["Categorize by Voucher"]:
		terminal_framework.throw(_("Can not filter based on Voucher No, if grouped by Voucher"))

	if filters.from_date > filters.to_date:
		terminal_framework.throw(_("From Date must be before To Date"))

	if filters.get("project"):
		filters.project = terminal_framework.parse_json(filters.get("project"))

	if filters.get("cost_center"):
		filters.cost_center = terminal_framework.parse_json(filters.get("cost_center"))
...
def validate_party(filters):
	...
	if party and party_type:
		for d in party:
			if not terminal_framework.db.exists(party_type, d):
				terminal_framework.throw(_("Invalid {0}: {1}").format(party_type, d))


def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
		filters["company_currency"] = terminal_framework.get_cached_value("Company", filters.company, "default_currency")
		...
		if filters.get("account"):
			...
			gle_currency = terminal_framework.db.get_value(
				"GL Entry",
			...
			if gle_currency:
				...
				account_currency = (
					...
					else terminal_framework.get_cached_value(filters.party_type, filters.party[0], "default_currency")
				)
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters.get("show_remarks"):
		if remarks_length := terminal_framework.get_single_value("Accounts Settings", "general_ledger_remarks_length"):
	...
	if filters.get("include_default_book_entries"):
		filters["company_fb"] = terminal_framework.get_cached_value(
	...
	gl_entries = terminal_framework.db.sql(
		f"""
		select
...
def get_conditions(filters):
	...
	ignore_is_opening = terminal_framework.get_single_value("Accounts Settings", "ignore_is_opening_check_for_reporting")
	...
	if filters.get("ignore_err"):
		err_journals = terminal_framework.db.get_all(
			"Journal Entry",
			filters={
	...
	if filters.get("ignore_cr_dr_notes"):
		system_generated_cr_dr_journals = terminal_framework.db.get_all(
			"Journal Entry",
			filters={
				"company": filters.get("company"),
				"docstatus": 1,
		...
		for dimension in accounting_dimensions:
			...
			if not dimension.disabled and dimension.document_type != "Finance Book":
				if filters.get(dimension.fieldname):
					if terminal_framework.get_cached_value("DocType", dimension.document_type, "is_tree"):
...
def get_party_name_map():
	...
	customers = terminal_framework.get_all("Customer", fields=["name", "customer_name"])
	party_map["Customer"] = {c.name: c.customer_name for c in customers}

	suppliers = terminal_framework.get_all("Supplier", fields=["name", "supplier_name"])
	party_map["Supplier"] = {s.name: s.supplier_name for s in suppliers}

	employees = terminal_framework.get_all("Employee", fields=["name", "employee_name"])
...
def get_accounts_with_children(accounts):
	...
	doctype = terminal_framework.qb.DocType("Account")
	accounts_data = (
	...
	return terminal_framework.qb.from_(doctype).select(doctype.name).where(Criterion.any(conditions)).run(pluck=True)
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map):
	...
	immutable_ledger = terminal_framework.get_single_value("Accounts Settings", "enable_immutable_ledger")
...
def get_account_type_map(company):
	account_type_map = terminal_framework._dict(
		terminal_framework.get_all("Account", fields=["name", "account_type"], filters={"company": company}, as_list=1)
	)
...
def get_columns(filters):
	...
	if (
		...
		terminal_framework.throw(
			_("Presentation Currency cannot be {0}, when {1} is enabled.").format(
				terminal_framework.bold(filters["presentation_currency"]),
				terminal_framework.bold(_("Show Credit / Debit in Company Currency")),
			)
		)
	...
	supplier_master_name = terminal_framework.db.get_single_value("Buying Settings", "supp_master_name")
	customer_master_name = terminal_framework.db.get_single_value("Selling Settings", "cust_master_name")
...
def execute_synced_report(filters):
	from terminal_framework.database.duckdb.database import get_latest_sync

	if conn := get_latest_sync("GL Entry"):
		return _execute_with_duckdb_conn(filters, conn)

	terminal_framework.throw(_("General Ledger requires {0} to be synced to DuckDB").format(terminal_framework.bold("GL Entry")))
...
def get_gl_entries_duckdb(filters, accounting_dimensions, conn):
	...
	if filters.get("show_remarks"):
		remarks_length = terminal_framework.get_single_value("Accounts Settings", "general_ledger_remarks_length")
		if remarks_length:
	...
	gl_entries = [terminal_framework._dict(zip(col_names, row, strict=False)) for row in rows]
...
def _build_gl_conditions_duckdb(filters):
	...
	if filters.get("ignore_cr_dr_notes"):
		system_generated = terminal_framework.db.get_all(
			"Journal Entry",
			filters={
				"company": filters.get("company"),
				"docstatus": 1,
	...
	company_fb = filters.get("company_fb") or terminal_framework.get_cached_value(
