...
from terminal_erp.accounts.doctype.tax_withholding_category.test_tax_withholding_category import (
...
from terminal_erp.accounts.report.tax_withholding_details.tax_withholding_details import execute
...
class TestTaxWithholdingDetails(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def test_tax_withholding_for_customers(self):
		...
		terminal_framework.db.set_value("Customer", "Test TCS Customer", "tax_withholding_category", "TCS")
	...
	def test_single_account_for_multiple_categories(self):
		...
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier", "tax_withholding_category", "TDS - 1")
		...
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier", "tax_withholding_category", "TDS - 2")
		inv_2 = create_purchase_invoice(supplier="Test TDS Supplier", rate=5000)
		inv_2.submit()
		result = execute(
			terminal_framework._dict(company="_Test Company", party_type="Supplier", from_date=today(), to_date=today())
		)[1]
		expected_values = [
	...
	def test_date_filters_in_multiple_tax_withholding_rules(self):
		...
		tds_doc = terminal_framework.get_doc("Tax Withholding Category", "TDS - 3")
		...
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier", "tax_withholding_category", tds_doc.name)
	...
	def check_expected_values(self, result, expected_values):
		...
		for i in range(len(result)):
			voucher = terminal_framework._dict(result[i])
