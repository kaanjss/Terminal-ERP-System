...
class TaxWithholdingDetailsReport:
	...
	def validate_filters(self):
		if not self.filters.from_date or not self.filters.to_date:
			terminal_framework.throw(_("From Date and To Date are required"))
	...
	def get_party_query(self, party_type, party_set):
		doctype = terminal_framework.qb.DocType(party_type)
		fields = [doctype.name]
		...
		return terminal_framework.qb.from_(doctype).select(*fields).where(doctype.name.isin(party_set))
	...
	def get_doc_info_query(self, doctype_name, voucher_set):
		...
		doctype = terminal_framework.qb.DocType(doctype_name)
		fields = [doctype.name, *get_doc_fields(doctype)]
		return terminal_framework.qb.from_(doctype).select(*fields).where(doctype.name.isin(voucher_set))
