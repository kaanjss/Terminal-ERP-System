...
from terminal_erp.accounts.report.sales_register.sales_register import execute
from terminal_erp.accounts.test.accounts_mixin import AccountsTestMixin
from terminal_erp.selling.doctype.customer.test_customer import make_customer
from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestItemWiseSalesRegister(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def create_child_cost_center(self):
		cc_name = "South Wing"
		if terminal_framework.db.exists("Cost Center", cc_name):
			cc = terminal_framework.get_doc("Cost Center", cc_name)
		else:
			parent = terminal_framework.db.get_value("Cost Center", self.cost_center, "parent_cost_center")
			cc = terminal_framework.get_doc(
				{
					"doctype": "Cost Center",
	...
	def _ensure_income_account(self, account_name):
		...
		if not terminal_framework.db.exists("Account", name):
	...
	def test_income_account_columns_sorted_case_insensitively(self):
		...
		filters = terminal_framework._dict({"from_date": today(), "to_date": today(), "company": self.company})
	...
	def test_sales_register_ignores_tax_rows_from_other_doctype(self):
		...
		terminal_framework.rename_doc("Sales Order", so.name, si.name, force=True)

		filters = terminal_framework._dict({"from_date": today(), "to_date": today(), "company": self.company})
		report = execute(filters)

		self.assertEqual(len(report[1]), 1)
		result = terminal_framework._dict(report[1][0])
	...
	def test_journal_with_cost_center_filter(self):
		je1 = terminal_framework.get_doc(
			{
		...
		je2 = terminal_framework.get_doc(
			{
				"doctype": "Journal Entry",
				"voucher_type": "Journal Entry",
				"company": self.company,
