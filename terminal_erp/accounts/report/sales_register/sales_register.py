...
from terminal_erp.accounts.report.utils import (
...
def _execute(filters, additional_table_columns=None):
	...
	if filters.get("include_payments") and not filters.get("customer"):
		terminal_framework.throw(_("Please select a customer for fetching payments."))
	...
	company_currency = terminal_framework.get_cached_value("Company", filters.get("company"), "default_currency")
	...
	for inv in invoice_list:
		...
		for income_acc in income_accounts:
			...
			row.update({terminal_framework.scrub(income_acc): income_amount})

		# Add amount in unrealized account
		for account in unrealized_profit_loss_accounts:
			row.update(
				{terminal_framework.scrub(account + "_unrealized"): flt(internal_invoice_map.get((inv.name, account)))}
			)
		...
		for tax_acc in tax_accounts:
			if tax_acc not in income_accounts:
				tax_amount_precision = (
					get_field_precision(
						terminal_framework.get_meta("Sales Taxes and Charges").get_field("tax_amount"),
						currency=company_currency,
					)
					or 2
				)
				...
				row.update({terminal_framework.scrub(tax_acc): tax_amount})

		# total tax, grand total, outstanding amount & rounded total

		outstanding_precision = (
			get_field_precision(
				terminal_framework.get_meta("Sales Invoice").get_field("outstanding_amount"),
				currency=company_currency,
			)
			or 2
		)
...
def get_account_columns(invoice_list, include_payments):
	...
	if invoice_list:
		# terminal_framework drops ORDER BY for distinct queries on postgres (db_query), so sort in python to keep
	...
	for account in income_accounts:
		income_columns.append(
			{
				"label": account,
				"fieldname": terminal_framework.scrub(account),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
	...
	for account in unrealized_profit_loss_accounts:
		unrealized_profit_loss_account_columns.append(
			{
				"label": account,
				"fieldname": terminal_framework.scrub(account + "_unrealized"),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
...
def get_conditions(filters, query, doctype):
	...
	if filters.get("mode_of_payment"):
		payment_doc = terminal_framework.qb.DocType("Sales Invoice Payment")
...
def get_invoice_income_map(invoice_list):
	income_details = terminal_framework.get_all(
		"Sales Invoice Item",
	...
	for d in income_details:
		invoice_income_map.setdefault(d.parent, terminal_framework._dict()).setdefault(d.income_account, [])
...
def get_invoice_tax_map(invoice_list, invoice_income_map, income_accounts, include_payments=False):
	tax_details = terminal_framework.get_all(
	...
	for d in tax_details:
		if d.account_head in income_accounts:
			...
			invoice_tax_map.setdefault(d.parent, terminal_framework._dict()).setdefault(d.account_head, [])
...
def get_invoice_so_dn_map(invoice_list):
	si_items = terminal_framework.get_all(
		"Sales Invoice Item",
		filters={"parent": ["in", [inv.name for inv in invoice_list]]},
	...
	for d in si_items:
		if d.sales_order:
			invoice_so_dn_map.setdefault(d.parent, terminal_framework._dict()).setdefault("sales_order", []).append(
				d.sales_order
			)
		...
		if d.delivery_note:
			...
			delivery_note_list = terminal_framework.get_all(
				"Delivery Note Item",
		...
		if delivery_note_list:
			invoice_so_dn_map.setdefault(d.parent, terminal_framework._dict()).setdefault(
...
def get_invoice_cc_wh_map(invoice_list):
	...
	for d in si_items:
		if d.cost_center:
			invoice_cc_wh_map.setdefault(d.parent, terminal_framework._dict()).setdefault("cost_center", []).append(
				d.cost_center
			)

		if d.warehouse:
			invoice_cc_wh_map.setdefault(d.parent, terminal_framework._dict()).setdefault("warehouse", []).append(
				d.warehouse
			)
...
def get_mode_of_payments(invoice_list):
	mode_of_payments = {}
	if invoice_list:
		inv_mop = terminal_framework.get_all(
