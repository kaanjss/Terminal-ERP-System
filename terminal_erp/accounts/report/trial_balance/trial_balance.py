...
from terminal_framework.utils import add_days, cstr, flt, formatdate, getdate
...
from terminal_erp.accounts.utils import get_zero_cutoff
...
def validate_filters(filters):
	if not filters.fiscal_year:
		terminal_framework.throw(_("Fiscal Year {0} is required").format(filters.fiscal_year))

	fiscal_year = terminal_framework.get_cached_value(
		"Fiscal Year", filters.fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	if not fiscal_year:
		terminal_framework.throw(_("Fiscal Year {0} does not exist").format(filters.fiscal_year))
	else:
	...
	if filters.from_date > filters.to_date:
		terminal_framework.throw(_("From Date cannot be greater than To Date"))
...
def get_data(filters):
	accounts = terminal_framework.get_all(
		"Account",
	...
	company_currency = filters.presentation_currency or terminal_erp.get_company_currency(filters.company)
...
def get_rootwise_opening_balances(
	...
	opening = terminal_framework._dict()
	for d in gle:
		opening_dr_cr = {
			"account": d.account,
			"opening_debit": 0.0,
...
def get_opening_balance(
	...
	closing_balance = terminal_framework.qb.DocType(doctype)
	accounts = terminal_framework.db.get_all("Account", filters={"report_type": report_type}, pluck="name")

	opening_balance = (
		terminal_framework.qb.from_(closing_balance)
		.select(
	...
	if terminal_framework.db.count("Finance Book"):
...
def execute_synced_report(filters):
	...
	if conn := get_latest_sync("GL Entry"):
		...
		terminal_framework.throw(_("Trial Balance requires {0} to be synced to DuckDB").format(terminal_framework.bold("GL Entry")))


def get_data_duckdb(filters, conn):
	# accounts and all metadata via terminal_framework.db — only GL Entry comes from DuckDB
	accounts = terminal_framework.get_all(
		"Account",
		filters={"company": filters.company},
		fields=[
...
def _fetch_gl_rows_duckdb(conn, conditions, params):
	...
	return [terminal_framework._dict(zip(cols, row, strict=False)) for row in conn.execute(sql, params).fetchall()]
...
def _get_rootwise_opening_duckdb(conn, filters, report_type, ignore_is_opening):
	...
	if not ignore_closing_balances:
		last_pcv = terminal_framework.db.get_all(
			"Period Closing Voucher",
			filters={"docstatus": 1, "company": filters.company, "period_end_date": ("<", filters.from_date)},
			fields=["period_end_date", "name"],
			order_by="period_end_date desc",
	...
	if last_pcv:
		# Account Closing Balance fetched via terminal_framework (not GL Entry)
		gle = get_opening_balance(
			"Account Closing Balance",
			filters,
			report_type,
