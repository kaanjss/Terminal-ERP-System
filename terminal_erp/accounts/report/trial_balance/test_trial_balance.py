...
from terminal_erp.accounts.report.trial_balance.trial_balance import execute
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestTrialBalance(Terminal ERPTestSuite):
	def setUp(self):
...
class TestTrialBalanceReport(Terminal ERPTestSuite):
	...
	def rows_by_account(self, **filters):
		...
		data = execute(terminal_framework._dict(filters))[1]
	...
	def test_opening_balance_respects_ignore_account_closing_balance(self):
		...
		def cash_opening(ignore_closing_balance):
			...
			data = execute(terminal_framework._dict(company="Test PCV Company", fiscal_year="_Test Fiscal Year 2022"))[1]
	...
	def test_show_unclosed_fy_pl_balances_controls_pl_opening(self):
		...
		from terminal_erp.accounts.doctype.journal_entry.test_journal_entry import make_journal_entry
		from terminal_erp.accounts.doctype.period_closing_voucher.test_period_closing_voucher import (
			create_cost_center,
		)
	...
	def test_include_default_book_entries_controls_default_fb_opening(self):
		...
		finance_book = (
			terminal_framework.get_doc({"doctype": "Finance Book", "finance_book_name": "_Test TB Finance Book"})
			.insert(ignore_if_duplicate=True)
			.name
		)
		terminal_framework.db.set_value("Company", "_Test Company", "default_finance_book", finance_book)
