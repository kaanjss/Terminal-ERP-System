...
from terminal_framework.utils import getdate, today

from terminal_erp.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from terminal_erp.accounts.report.item_wise_sales_register.item_wise_sales_register import execute
...
class TestItemWiseSalesRegister(Terminal ERPTestSuite, AccountsTestMixin):
	...
	def test_grouped_report_handles_different_tax_descriptions(self):
		...
		first_tax_amount_field = f"{terminal_framework.scrub(first_tax_description)}_amount"
		second_tax_amount_field = f"{terminal_framework.scrub(second_tax_description)}_amount"
