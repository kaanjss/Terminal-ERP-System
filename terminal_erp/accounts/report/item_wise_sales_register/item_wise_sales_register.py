...
from terminal_framework.utils.xlsxutils import handle_html
from pypika.terms import Bracket, LiteralValue, Order

from terminal_erp.accounts.report.sales_register.sales_register import get_mode_of_payments
from terminal_erp.accounts.report.utils import get_values_for_columns
from terminal_erp.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def apply_conditions(query, si, sii, sip, filters, additional_conditions=None):
	...
	if filters.get("warehouse"):
		if terminal_framework.db.get_value("Warehouse", filters.get("warehouse"), "is_group"):
			lft, rgt = terminal_framework.db.get_all(
				"Warehouse", filters={"name": filters.get("warehouse")}, fields=["lft", "rgt"], as_list=True
			)[0]
			warehouses = terminal_framework.db.get_all("Warehouse", {"lft": (">", lft), "rgt": ("<", rgt)}, pluck="name")
	...
	if filters.get("item_group"):
		if terminal_framework.db.get_value("Item Group", filters.get("item_group"), "is_group"):
...
def apply_order_by_conditions(doctype, query, filters):
	...
	invoice_item = terminal_framework.qb.DocType(f"{doctype} Item")
	...
	if not filters.get("group_by"):
		...
		filter_field = terminal_framework.scrub(filters.get("group_by"))
	...
	if additional_query_columns:
		for column in additional_query_columns:
			if column.get("_doctype"):
				table = terminal_framework.qb.DocType(column.get("_doctype"))
...
def get_delivery_notes_against_sales_order(item_list):
	so_dn_map = terminal_framework._dict()
...
def get_tax_accounts(
	...
	tax = terminal_framework.qb.DocType("Item Wise Tax Detail")
	taxes_and_charges = terminal_framework.qb.DocType(tax_doctype)
	...
	precision = terminal_framework.get_precision(tax_doctype, "tax_amount", currency=company_currency) or 2
	tax_columns = {}
	itemised_tax = {}
	...
	for row in tax_details:
		...
		if not scrubbed_description:
			scrubbed_description = terminal_framework.scrub(description)
...
def get_tax_details_query(doctype, tax_doctype):
	...
	query = (
		terminal_framework.qb.from_(tax)
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = terminal_framework.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = terminal_framework.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
