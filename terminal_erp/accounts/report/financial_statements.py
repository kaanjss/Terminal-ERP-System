...
from terminal_framework.utils import add_days, add_months, cint, cstr, flt, formatdate, get_first_day, getdate
...
from terminal_erp.accounts.utils import get_fiscal_year, get_zero_cutoff
...
def get_period_list(
	...
	for i in range(cint(math.ceil(months / months_to_add))):
		period = terminal_framework._dict({"from_date": start_date})
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
	from_year_start_date = terminal_framework.get_cached_value("Fiscal Year", from_fiscal_year, "year_start_date")
	to_year_end_date = terminal_framework.get_cached_value("Fiscal Year", to_fiscal_year, "year_end_date")
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
	if not fiscal_year.get("year_start_date") or not fiscal_year.get("year_end_date"):
		terminal_framework.throw(_("Start Year and End Year are mandatory"))

	if getdate(fiscal_year.get("year_end_date")) < getdate(fiscal_year.get("year_start_date")):
		terminal_framework.throw(_("End Year cannot be before Start Year"))


def validate_dates(from_date, to_date):
	if not from_date or not to_date:
		terminal_framework.throw(_("From Date and To Date are mandatory"))

	if to_date < from_date:
		terminal_framework.throw(_("To Date cannot be less than From Date"))
...
def get_data(
	...
	for root in terminal_framework.get_all(
		"Account",
...
def set_gl_entries_by_account(
	...
	# For balance sheet
	ignore_closing_balances = terminal_framework.get_single_value("Accounts Settings", "ignore_account_closing_balance")
	if not from_date and not ignore_closing_balances:
		last_period_closing_voucher = terminal_framework.db.get_all(
...
def get_accounting_entries(
	...
	gl_entry = terminal_framework.qb.DocType(doctype)
	query = (
...
def get_account_filter_query(root_lft, root_rgt, root_type, gl_entry):
	...
	exists_query = (
		terminal_framework.qb.from_(acc).select(acc.name).where(acc.name == gl_entry.account).where(acc.is_group == 0)
	)
...
def apply_additional_conditions(doctype, query, from_date, ignore_closing_entries, filters):
	...
	if filters:
		...
		if filters.get("include_default_book_entries"):
			company_fb = terminal_framework.get_cached_value("Company", filters.company, "default_finance_book")
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		if terminal_framework.db.exists("Cost Center", d):
			lft, rgt = terminal_framework.db.get_value("Cost Center", d, ["lft", "rgt"])
			children = terminal_framework.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_cost_centers += [c.name for c in children]
		else:
			terminal_framework.throw(_("Cost Center: {0} does not exist").format(d))
