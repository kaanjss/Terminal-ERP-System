...
def validate_service_stop_date(doc):
	...
	old_stop_dates = {}
	old_doc = terminal_framework.db.get_all(f"{doc.doctype} Item", {"parent": doc.name}, ["name", "service_stop_date"])
	...
	for item in doc.items:
		...
		if item.service_stop_date:
			if date_diff(item.service_stop_date, item.service_start_date) < 0:
				terminal_framework.throw(_("Service Stop Date cannot be before Service Start Date"))

			if date_diff(item.service_stop_date, item.service_end_date) > 0:
				terminal_framework.throw(_("Service Stop Date cannot be after Service End Date"))
		...
		if (
			...
			terminal_framework.throw(_("Cannot change Service Stop Date for item in row {0}").format(item.idx))


def build_conditions(process_type, account, company):
	if process_type == "Income":
		item = terminal_framework.qb.DocType("Sales Invoice Item")
		parent = terminal_framework.qb.DocType("Sales Invoice")
		deferred_account = item.deferred_revenue_account
	else:
		item = terminal_framework.qb.DocType("Purchase Invoice Item")
		parent = terminal_framework.qb.DocType("Purchase Invoice")
...
def convert_deferred_expense_to_expense(deferred_process, start_date=None, end_date=None, conditions=None):
	...
	for invoice in invoices:
		doc = terminal_framework.get_doc("Purchase Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)

	if terminal_framework.flags.deferred_accounting_error:
...
def convert_deferred_revenue_to_income(deferred_process, start_date=None, end_date=None, conditions=None):
	...
	for invoice in invoices:
		doc = terminal_framework.get_doc("Sales Invoice", invoice)
		book_deferred_income_or_expense(doc, deferred_process, end_date)
...
def get_booking_dates(doc, item, posting_date=None, prev_posting_date=None):
	...
	if not prev_posting_date:
		prev_gl_entry = terminal_framework.get_all(
			"GL Entry",
			filters={
				"company": doc.company,
		...
		prev_gl_via_je = (
			...
			.orderby(je.posting_date, order=terminal_framework.qb.desc)
			.limit(1)
			.run(as_dict=True)
		)
...
def book_deferred_income_or_expense(doc, deferred_process, posting_date=None):
	...
	accounts_frozen_upto = terminal_framework.db.get_value("Company", doc.company, "accounts_frozen_till_date")
	...
	via_journal_entry = cint(
		terminal_framework.db.get_singles_value("Accounts Settings", "book_deferred_entries_via_journal_entry")
	)
	submit_journal_entry = cint(terminal_framework.db.get_singles_value("Accounts Settings", "submit_journal_entries"))
	book_deferred_entries_based_on = terminal_framework.db.get_singles_value(
...
def process_deferred_accounting(posting_date=None):
	...
	if not cint(
		terminal_framework.db.get_singles_value("Accounts Settings", "automatically_process_deferred_accounting_entry")
	):
		return
	...
	if gl_entries:
		try:
			...
			if terminal_framework.in_test:
				...
				terminal_framework.db.rollback()
				doc.log_error(f"Error while processing deferred accounting for Invoice {doc.name}")
				terminal_framework.flags.deferred_accounting_error = True
...
def get_deferred_booking_accounts(doctype, voucher_detail_no, dr_or_cr):
	if doctype == "Sales Invoice":
		credit_account, debit_account = terminal_framework.db.get_value(
			"Sales Invoice Item",
