...
from terminal_erp.accounts.utils import create_gain_loss_journal, get_currency_precision
...
def gain_loss_journal_already_booked(
	...
	if res := terminal_framework.db.get_all(
		...
		if exc_vouchers := terminal_framework.db.get_all(
			"Journal Entry",
			...
			for arg in args:
				if (
					...
					if not gain_loss_journal_already_booked(
						...
						posting_date = arg.get("difference_posting_date") or terminal_framework.db.get_value(
	...
	if doc.get("doctype") == "Payment Entry":
		...
		for d in gain_loss_to_book:
			if d.exchange_gain_loss and ((d.reference_doctype, d.reference_name, str(d.idx)) not in booked):
				...
				gain_loss_account = terminal_framework.get_cached_value(
...
def is_payable_account(reference_doctype: str, account: str) -> bool:
	if reference_doctype == "Purchase Invoice" or (
		...
		and terminal_framework.get_cached_value("Account", account, "account_type") == "Payable"
	):
		return True
	return False
