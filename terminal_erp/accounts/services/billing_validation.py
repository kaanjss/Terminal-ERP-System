...
from terminal_framework.utils import cint, flt, fmt_money
...
class BillingValidationService:
	...
	def validate_multiple_billing(self, ref_dt: str, item_ref_dn: str, based_on: str) -> None:
		...
		role_allowed_to_overbill = terminal_framework.get_single_value("Accounts Settings", "role_allowed_to_over_bill")
		is_overbilling_allowed = role_allowed_to_overbill in terminal_framework.get_roles()
	...
	def get_reference_wise_billed_amt(self, ref_dt: str, item_ref_dn: str, based_on: str) -> dict | None:
		...
		for item in self.doc.items:
			...
			ref_wise_billed_amount.setdefault(
				key,
				terminal_framework._dict(item_code=item.item_code, billed_amt=0.0, ref_amt=ref_amt, rows=[]),
			)
	...
	def get_already_billed_amount(
		...
		item_doctype = terminal_framework.qb.DocType(self.doc.items[0].doctype)
		based_on_field = terminal_framework.qb.Field(based_on)
		join_field = terminal_framework.qb.Field(item_ref_dn)
