...
from terminal_framework.utils import cint, flt, parse_json
...
class TaxService:
	...
	def set_taxes(self) -> None:
		...
		if (doc.is_new() or self.is_pos_profile_changed()) and not doc.get("taxes"):
			if doc.company and not doc.get("taxes_and_charges"):
				doc.taxes_and_charges = terminal_framework.db.get_value(
	...
	def is_pos_profile_changed(self) -> bool:
		...
		if (
			...
			and doc.is_pos
			and doc.pos_profile != terminal_framework.db.get_value("Sales Invoice", doc.name, "pos_profile")
		):
			return True
	...
	def set_taxes_and_charges(self) -> None:
		...
		if terminal_framework.get_single_value("Accounts Settings", "add_taxes_from_item_tax_template"):
	...
	def append_taxes_from_item_tax_template(self) -> None:
		doc = self.doc
		if not terminal_framework.get_single_value("Accounts Settings", "add_taxes_from_item_tax_template"):
			return
	...
	def validate_enabled_taxes_and_charges(self) -> None:
		...
		if doc.taxes_and_charges and terminal_framework.get_cached_value(
			taxes_and_charges_doctype, doc.taxes_and_charges, "disabled"
		):
			terminal_framework.throw(_("{0} '{1}' is disabled").format(taxes_and_charges_doctype, doc.taxes_and_charges))

	def validate_tax_account_company(self) -> None:
		doc = self.doc
		for d in doc.get("taxes"):
			if d.account_head:
				tax_account_company = terminal_framework.get_cached_value("Account", d.account_head, "company")
...
def get_tax_rate(account_head: str) -> dict:
	return terminal_framework.get_cached_value("Account", account_head, ["tax_rate", "account_name"], as_dict=True)
...
def get_default_taxes_and_charges(
	...
	if tax_template and company:
		tax_template_company = terminal_framework.get_cached_value(master_doctype, tax_template, "company")
		if tax_template_company == company:
			return

	default_tax = terminal_framework.db.get_value(master_doctype, {"is_default": 1, "company": company})
...
def get_taxes_and_charges(master_doctype: str, master_name: str | None = None) -> list | None:
	...
	tax_master = terminal_framework.get_doc(master_doctype, master_name)
...
def validate_account_head(idx: int, account: str, company: str, context: str | None = None) -> None:
	...
	if company != terminal_framework.get_cached_value("Account", account, "company"):
		terminal_framework.throw(
			_("Row {0}: The {3} Account {1} does not belong to the company {2}").format(
				idx, terminal_framework.bold(account), terminal_framework.bold(company), context or ""
			),
			title=_("Invalid Account"),
		)

	if terminal_framework.get_cached_value("Account", account, "is_group"):
		terminal_framework.throw(
			_(
				"You selected the account group {1} as {2} Account in row {0}. Please select a single account."
			).format(idx, terminal_framework.bold(account), context or ""),
			title=_("Invalid Account"),
		)
...
def validate_cost_center(tax, doc) -> None:
	...
	company = terminal_framework.get_cached_value("Cost Center", tax.cost_center, "company")

	if company != doc.company:
		terminal_framework.throw(
			_("Row {0}: Cost Center {1} does not belong to Company {2}").format(
				tax.idx, terminal_framework.bold(tax.cost_center), terminal_framework.bold(doc.company)
			),
...
def add_taxes_from_tax_template(child_item, parent_doc, db_insert: bool = True) -> None:
	add_taxes_from_item_tax_template = terminal_framework.get_single_value(
