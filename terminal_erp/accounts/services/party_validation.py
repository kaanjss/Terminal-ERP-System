...
class PartyValidator:
			...
			if item.get(item_field) == self.doc.get(party_account_field):
				terminal_framework.throw(
					_("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
						item.idx,
						terminal_framework.bold(terminal_framework.unscrub(item_field)),
						item.get(item_field),
						terminal_framework.bold(terminal_framework.unscrub(party_account_field)),
	...
	def validate_party_account_currency(self) -> None:
		...
		allow_multi_currency = terminal_framework.db.get_singles_value(
		...
		if (
			...
			terminal_framework.throw(
				_("Party Account {0} currency ({1}) and document currency ({2}) should be same").format(
					terminal_framework.bold(party_account), party_account_currency, self.doc.currency
				)
			)
	...
	def validate_company_linked_addresses(self) -> None:
		...
		for field in address_fields:
			...
			if field in ("dispatch_address_name", "shipping_address") and is_drop_ship:
				continue
			if address and not terminal_framework.db.exists(
				"Dynamic Link",
				{
					"parent": address,
					"parenttype": "Address",
				...
				terminal_framework.throw(
					_("{0} does not belong to the Company {1}.").format(
						_(doc.meta.get_label(field)), terminal_framework.bold(doc.company)
					)
				)
	...
	def _validate_address(
		...
		party_addresses = terminal_framework.get_all(
			"Dynamic Link",
			{"link_doctype": party_type, "link_name": party, "parenttype": "Address"},
			pluck="parent",
		)
		if billing_address and billing_address not in party_addresses:
			terminal_framework.throw(_("Billing Address does not belong to the {0}").format(party))
		elif shipping_address and shipping_address not in party_addresses:
			terminal_framework.throw(_("Shipping Address does not belong to the {0}").format(party))
	...
	def _validate_contact(self, party: str, party_type: str) -> None:
		...
		if self.doc.contact_person not in contacts:
			terminal_framework.throw(_("Contact Person does not belong to the {0}").format(party))
