...
from terminal_framework.utils import flt, get_link_to_form, getdate

from terminal_erp.accounts.doctype.accounting_dimension.accounting_dimension import get_accounting_dimensions
from terminal_erp.buying.utils import update_last_purchase_rate
...
class ChildItemUpdater:
	...
	def __init__(self, parent_doctype: str, parent_doctype_name: str, child_docname: str = "items"):
		...
		self.parent = terminal_framework.get_doc(parent_doctype, parent_doctype_name)
	...
	def update(self, trans_items: str | list) -> None:
		...
		from terminal_erp.buying.doctype.supplier_quotation.supplier_quotation import get_purchased_items
		from terminal_erp.selling.doctype.quotation.mapper import get_ordered_items

		data = terminal_framework.parse_json(trans_items)
		...
		for d in data:
			...
			if not d.get("docname"):
				...
				self._check_permissions("write")
				child_item = terminal_framework.get_doc(self.parent_doctype + " Item", d.get("docname"))
		...
		if self.parent_doctype == "Purchase Order":
			...
			if parent.is_subcontracted and not parent.can_update_items():
				terminal_framework.throw(
					...
					).format(terminal_framework.bold(parent.name))
				)
	...
	def _validate_workflow(self) -> None:
		...
		workflow_doc = terminal_framework.get_doc("Workflow", workflow)
		current_state = self.parent.get(workflow_doc.workflow_state_field)
		roles = terminal_framework.get_roles()
	...
	def _get_new_child_item(self, item_row) -> "terminal_framework.model.document.Document":
	...
	def _validate_quantity_and_rate(self, child_item, new_data: dict, rate_unchanged: bool | None) -> None:
		if not flt(new_data.get("qty")) and not self.allow_zero_qty:
			terminal_framework.throw(
				_("Row #{0}:Quantity for Item {1} cannot be zero.").format(
					new_data.get("idx"), terminal_framework.bold(new_data.get("item_code"))
				),
				title=_("Invalid Qty"),
			)
		...
		if not rate_unchanged:
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(new_data.get("item_code")))
			)

		if flt(new_data.get("qty")) < qty_to_check:
			terminal_framework.throw(_("Cannot reduce quantity than ordered or purchased quantity"))
	...
	def _validate_fg_item_for_subcontracting(self, new_data: dict, is_new: bool) -> None:
		if is_new:
			...
			is_sub_contracted_item, default_bom = terminal_framework.db.get_value(
			...
			if not is_sub_contracted_item:
				...
				terminal_framework.throw(_("Default BOM not found for FG Item {0}").format(new_data["fg_item"]))

		if not new_data.get("fg_item_qty"):
			terminal_framework.throw(_("Finished Good Item {0} Qty can not be zero").format(new_data["fg_item"]))
...
def set_order_defaults(
	...
	child_docname: str,
	trans_item: dict,
) -> "terminal_framework.model.document.Document":
	"""Return a new child item populated with item master defaults."""
	from terminal_erp.accounts.services.taxes import add_taxes_from_tax_template, set_child_tax_template_and_map

	p_doc = terminal_framework.get_doc(parent_doctype, parent_doctype_name)
	child_item = terminal_framework.new_doc(child_doctype, parent_doc=p_doc, parentfield=child_docname)
	item = terminal_framework.get_doc("Item", trans_item.get("item_code"))
...
def validate_child_on_delete(row, parent, ordered_item=None) -> None:
	...
	if parent.doctype == "Quotation" and ordered_item and ordered_item.get(row.name):
		terminal_framework.throw(_("Cannot delete an item which has been ordered"))


def update_bin_on_delete(row, doctype: str) -> None:
	"""Update bin quantities after a child item row is deleted."""
	from terminal_erp.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def get_allow_zero_qty(parent_doctype: str) -> bool:
	if parent_doctype == "Sales Order":
		return terminal_framework.db.get_single_value("Selling Settings", "allow_zero_qty_in_sales_order") or False
	if parent_doctype == "Purchase Order":
		return terminal_framework.db.get_single_value("Buying Settings", "allow_zero_qty_in_purchase_order") or False
	return False


def get_child_item_change_state(parent_doctype: str, child_item, new_data) -> terminal_framework._dict:
...
def is_child_item_unchanged(change_state: terminal_framework._dict) -> bool:
	return (
...
def update_child_item_rate_and_discount(
	...
	if not rate_unchanged and not child_item.get("qty") and allow_zero_qty:
		terminal_framework.throw(_("Rate of '{0}' items cannot be changed").format(terminal_framework.bold(_("Unit Price"))))
