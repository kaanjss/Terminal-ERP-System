...
class InternalTransferService:
	...
	def set_account(self) -> None:
		...
		unrealized_profit_loss_account = terminal_framework.get_cached_value(
		...
		if not unrealized_profit_loss_account:
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(self.doc.company))
			)
	...
	def process_common_party_accounting(self) -> None:
		...
		if terminal_framework.get_single_value("Accounts Settings", "enable_common_party_accounting"):
	...
	def validate_reference(self) -> None:
		...
		if not (
			...
			terminal_framework.throw(msg, title=_("Internal Sales Reference Missing"))

		label = "Delivery Note Item" if self.doc.doctype == "Purchase Receipt" else "Sales Invoice Item"
		field = terminal_framework.scrub(label)
	...
	def validate_transaction(self) -> None:
		if not cint(terminal_framework.get_single_value("Accounts Settings", "maintain_same_internal_transaction_rate")):
			return
	...
	def _validate_line_items(self, ref_dt: str, ref_dn_field: str, ref_link_field: str) -> None:
		...
		for d in self.doc.get("items"):
			...
			if action == "Stop":
				user_roles = terminal_framework.get_all(
					"Has Role", filters={"parent": terminal_framework.session.user}, fields=["role"], pluck="role"
				)
				if role_allowed_to_override not in user_roles:
					stop_actions.append(msg)
			else:
				terminal_framework.msgprint(msg, title=_("Warning"), indicator="orange")
