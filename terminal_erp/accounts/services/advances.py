...
from terminal_framework.query_builder.functions import Abs, IfNull, Max, Sum
...
from terminal_erp.accounts.utils import get_account_currency, get_advance_payment_doctypes
...
def set_advances(doc) -> None:
	...
	res = get_advance_entries(
		doc, include_unallocated=not terminal_framework.utils.cint(doc.get("only_include_allocated_payments"))
	)
...
def calculate_total_advance_from_ledger(doc) -> list:
	...
	adv = terminal_framework.qb.DocType("Advance Payment Ledger Entry")
	return (
		terminal_framework.qb.from_(adv)
...
def set_total_advance_paid(doc) -> None:
	...
	if advance:
		...
		if advance.account_currency:
			terminal_framework.db.set_value(doc.doctype, doc.name, "party_account_currency", advance.account_currency)
...
def set_advance_payment_status(doc) -> None:
	...
	PaymentRequest = terminal_framework.qb.DocType("Payment Request")
	paid_amount = terminal_framework.get_value(
...
def delink_advance_entries(doc, linked_doc_name: str) -> None:
	...
	for adv in doc.advances:
		...
		if adv.reference_name == linked_doc_name:
			doctype = terminal_framework.qb.DocType(doc.doctype + " Advance")
			terminal_framework.qb.from_(doctype).delete().where(doctype.name == adv.name).run()
	...
	terminal_framework.db.set_value(doc.doctype, doc.name, "total_advance", total_allocated_amount, update_modified=False)
...
def create_advance_and_reconcile(doc, party_link) -> None:
	...
	default_currency = terminal_erp.get_company_currency(doc.company)
	...
	reconcilation_entry = terminal_framework._dict()
	advance_entry = terminal_framework._dict()
	...
	reconcilation_entry.cost_center = doc.cost_center or terminal_erp.get_default_cost_center(doc.company)

	advance_entry.account = primary_account
	advance_entry.party_type = primary_party_type
	advance_entry.party = primary_party
	advance_entry.cost_center = doc.cost_center or terminal_erp.get_default_cost_center(doc.company)
...
def get_common_query(
	...
	account_type = terminal_framework.db.get_value("Party Type", party_type, "account_type")
