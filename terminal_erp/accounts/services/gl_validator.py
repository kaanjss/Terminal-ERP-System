...
posting sink in ``terminal_erp.accounts.general_ledger``.
"""

import terminal_framework
from terminal_framework import _
from terminal_framework.utils import cint, formatdate, getdate

from terminal_erp.accounts.doctype.accounting_period.accounting_period import ClosedAccountingPeriod
...
def validate_disabled_accounts(gl_map):
	...
	disabled_accounts = terminal_framework.get_all(
		"Account",
	...
	if used_disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([terminal_framework.bold(d) for d in used_disabled_accounts])
...
def validate_accounting_period(gl_map):
	...
	if accounting_periods:
		if accounting_periods[0].exempted_role:
			...
			if exempted_roles in terminal_framework.get_roles():
				return
		terminal_framework.throw(
			_(
				"You cannot create or cancel any accounting entries within the closed Accounting Period {0}"
			).format(terminal_framework.bold(accounting_periods[0].name)),
...
def validate_cwip_accounts(gl_map):
	...
	cwip_enabled = any(
		...
		for ac in terminal_framework.db.get_all("Asset Category", "enable_cwip_accounting")
	)
	if cwip_enabled:
		cwip_accounts = terminal_framework.get_all(
			"Account",
...
def check_freezing_date(posting_date, company, adv_adj=False):
	...
	"""
	if not adv_adj:
		acc_frozen_till_date = terminal_framework.db.get_value("Company", company, "accounts_frozen_till_date")
		if acc_frozen_till_date:
			...
			if getdate(posting_date) <= getdate(acc_frozen_till_date) and (
				frozen_accounts_modifier not in terminal_framework.get_roles() or terminal_framework.session.user == "Administrator"
			):
				terminal_framework.throw(
...
def validate_opening_entry_against_pcv(company):
	if terminal_framework.db.exists("Period Closing Voucher", {"docstatus": 1, "company": company}):
		terminal_framework.throw(
			_(
				"A Period Closing Voucher is already submitted and an Opening Entry can no longer be created. {0} to learn more."
			).format(
				'<a href="https://docs.terminal_framework.io/terminal_erp/period-closing-voucher#14-pcv-and-opening-entries" target="_blank" rel="noopener">'
				+ _("Read the docs")
				+ "</a>"
			),
...
def validate_against_pcv(is_opening, posting_date, company):
	...
	last_pcv_date = terminal_framework.db.get_value(
	...
	if last_pcv_date and getdate(posting_date) <= getdate(last_pcv_date):
		...
		terminal_framework.throw(message, title=_("Period Closed"))
			...
			if value["is_mandatory"] and not gl_entry.get(dimension):
				terminal_framework.throw(
					_("{0} is mandatory for account {1}").format(
						terminal_framework.bold(terminal_framework.unscrub(dimension)), terminal_framework.bold(gl_entry.account)
					),
				...
				if gl_entry.get(dimension) and gl_entry.get(dimension) not in value["allowed_dimensions"]:
					terminal_framework.throw(
						_("Invalid value {0} for {1} against account {2}").format(
							terminal_framework.bold(gl_entry.get(dimension)),
							terminal_framework.bold(terminal_framework.unscrub(dimension)),
							terminal_framework.bold(gl_entry.account),
						),
