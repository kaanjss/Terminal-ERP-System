...
from terminal_framework.utils import flt, formatdate

import terminal_erp
from terminal_erp.accounts.doctype.accounting_dimension.accounting_dimension import get_accounting_dimensions
from terminal_erp.accounts.services.taxes import set_balance_in_account_currency
from terminal_erp.accounts.utils import get_account_currency, get_fiscal_years
...
def get_gl_dict(doc, args: dict, account_currency: str | None = None, item=None) -> dict:
	...
	gl_dict = terminal_framework._dict(
		{
			"company": doc.company,
	...
	dimension_dict = terminal_framework._dict()
...
def get_voucher_subtype(doc) -> str:
	...
	for method_name in terminal_framework.get_hooks("voucher_subtypes"):
		voucher_subtype = terminal_framework.get_attr(method_name)(doc)
		if voucher_subtype:
...
def update_gl_dict_with_app_based_fields(doc, gl_dict):
	for method in terminal_framework.get_hooks("update_gl_dict_with_app_based_fields", default=[]):
		terminal_framework.get_attr(method)(doc, gl_dict)
