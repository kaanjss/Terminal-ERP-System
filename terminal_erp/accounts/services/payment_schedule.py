...
from terminal_framework.utils import DateTimeLikeObject, add_days, add_months, cint, flt, get_last_day, getdate
...
class PaymentScheduleService:
	...
	def fetch_payment_terms_from_order(
		...
		doc = self.doc
		po_or_so = terminal_framework.get_cached_doc(po_or_so_doctype, po_or_so)
...
def linked_order_has_payment_terms_template(po_or_so, doctype) -> str | None:
	return terminal_framework.get_value(doctype, po_or_so, "payment_terms_template")


def linked_order_has_payment_schedule(po_or_so) -> list:
	return terminal_framework.get_all("Payment Schedule", filters={"parent": po_or_so})
...
def get_payment_terms(
	...
	terms_doc = terminal_framework.get_doc("Payment Terms Template", terms_template)
	schedule = []
	for d in terms_doc.get("terms"):
		d = terminal_framework._dict(d.as_dict())
...
def get_payment_term_details(
	term: str | terminal_framework._dict,
	posting_date: DateTimeLikeObject | None = None,
	grand_total: float | None = None,
	base_grand_total: float | None = None,
	bill_date: DateTimeLikeObject | None = None,
) -> terminal_framework._dict:
	term_details = terminal_framework._dict()
	if isinstance(term, str):
		term = terminal_framework.get_doc("Payment Term", term)
	else:
