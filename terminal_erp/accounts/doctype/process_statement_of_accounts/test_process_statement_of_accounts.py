...
from terminal_erp.accounts.doctype.process_statement_of_accounts.process_statement_of_accounts import (
	get_statement_dict,
	send_emails,
)
...
class TestProcessStatementOfAccounts(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def test_process_soa_for_ar(self):
		...
		expected_summary = terminal_framework._dict(
			range1=100,
			range2=0,
			range3=0,
			range4=0,
...
def create_process_soa(**args):
	...
	terminal_framework.delete_doc_if_exists("Process Statement Of Accounts", args.name)
	process_soa = terminal_framework.new_doc("Process Statement Of Accounts")
	soa_dict = terminal_framework._dict(
		name=args.name,
