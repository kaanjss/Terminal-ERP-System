...
from terminal_erp.accounts.doctype.account_category.account_category import import_account_categories
from terminal_erp.accounts.doctype.financial_report_template.financial_report_validation import TemplateValidator
...
class FinancialReportTemplate(Document):
	...
	def _export_template(self):
		from terminal_framework.modules.utils import export_module_json
	...
	def _delete_template(self):
		if not self.module or not terminal_framework.conf.developer_mode:
			return

		module_path = terminal_framework.get_module_path(self.module)
		dir_path = os.path.join(module_path, "financial_report_template", terminal_framework.scrub(self.name))
	...
	def _export_account_categories(self):
		...
		if not self.module or not terminal_framework.conf.developer_mode or terminal_framework.flags.in_import:
			return
		...
		if category_names:
			db_categories = terminal_framework.get_all(
				"Account Category",
...
def sync_financial_report_templates(chart_of_accounts=None, existing_company=None):
	from terminal_erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts import get_chart
	...
	# Allow regional templates to completely override Terminal ERP
	...
	installed_apps = terminal_framework.get_installed_apps()

	for app in installed_apps:
		if disable_default_financial_report_template and app == "terminal_erp":
			continue
...
def _sync_templates_for(app_name):
	...
	for module_name in terminal_framework.local.app_modules.get(app_name) or []:
		module_path = terminal_framework.get_module_path(module_name)
	...
	for template_path in templates:
		with open(template_path) as f:
			template_data = terminal_framework._dict(terminal_framework.parse_json(f.read()))

		template_name = template_data.get("name")

		if not terminal_framework.db.exists("Financial Report Template", template_name):
			doc = terminal_framework.get_doc(template_data)
