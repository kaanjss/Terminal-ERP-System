...
from terminal_erp.accounts.doctype.financial_report_template.test_financial_report_template import (
...
from terminal_erp.accounts.utils import get_currency_precision, get_fiscal_year
from terminal_erp.tests.utils import change_settings
...
class TestFilterExpressionParser(FinancialReportTemplateTestCase):
	...
	def test_build_condition_accepts_document_instance(self):
		...
		row_obj = terminal_framework._dict(
			{
		...
		row_doc = terminal_framework.get_doc(row_obj)
		cond = parser.build_condition(row_doc, account_table)
		self.assertIsNotNone(cond)

		# Also accepts plain terminal_framework._dict object
...
class TestFinancialQueryBuilder(FinancialReportTemplateTestCase):
	def test_fetch_balances_with_journal_entries(self):
		...
		try:
			...
			accounts = [
				terminal_framework._dict({"name": cash_account, "account_name": "Cash", "account_number": "1001"}),
				terminal_framework._dict({"name": bank_account, "account_name": "Bank", "account_number": "1002"}),
			]
	...
	def test_opening_balance_from_previous_period_closing(self):
		...
		pcv = None
		jv_2024 = None
		original_pcv_setting = terminal_framework.db.get_single_value(
		...
		try:
			...
			closing_account = terminal_framework.db.get_value(
				"Account",
				{
					"company": company,
					"root_type": "Liability",
	...
	def test_pl_pcv_exclusion_and_growth_view_year_over_year(self):
		...
		try:
			...
			pcv_2024 = terminal_framework.get_doc(
				{
					"doctype": "Period Closing Voucher",
			...
			pcv_2025 = terminal_framework.get_doc(
				{
					"doctype": "Period Closing Voucher",
			...
			template_name = f"Test Growth Template {terminal_framework.generate_hash()[:8]}"
			...
			if template and terminal_framework.db.exists("Financial Report Template", template.name):
				terminal_framework.delete_doc("Financial Report Template", template.name, force=1)
