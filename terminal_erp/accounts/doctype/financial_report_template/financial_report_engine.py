...
from terminal_framework.database.operator_map import OPERATOR_MAP
from terminal_framework.model import numeric_fieldtypes
from terminal_framework.query_builder import Case
from terminal_framework.query_builder.functions import Cast_, Sum
from terminal_framework.utils import cstr, date_diff, flt, getdate
from terminal_framework.utils.xlsxutils import XLSXMetadata, XLSXStyleBuilder
...
from terminal_erp.accounts.doctype.financial_report_row.financial_report_row import FinancialReportRow
from terminal_erp.accounts.doctype.financial_report_template.financial_report_template import (
	FinancialReportTemplate,
)
from terminal_erp.accounts.doctype.financial_report_template.financial_report_validation import (
	AccountFilterValidator,
	CalculationFormulaValidator,
	DependencyValidator,
)
from terminal_erp.accounts.report.financial_statements import (
	get_columns,
	get_cost_centers_with_children,
	get_period_list,
)
from terminal_erp.accounts.utils import get_children, get_currency_precision
...
class FinancialReportEngine:
	...
	def _validate_filters(self, filters: dict[str, Any]) -> None:
			...
			if not filters.get(filter_key):
				terminal_framework.throw(
					...
					msg=_("Missing required filter: {0}").format(
						terminal_framework.bold(filter_labels.get(filter_key, filter_key))
					),
				)
		...
		if (view := filters.get("selected_view")) and view not in supported_views:
			terminal_framework.msgprint(_("{0} view is currently unsupported in Custom Financial Report.").format(view))

	def _initialize_context(self, filters: dict[str, Any]) -> ReportContext:
		template_name = filters.get("report_template")
		template = terminal_framework.get_doc("Financial Report Template", template_name)

		if not template:
			terminal_framework.throw(_("Financial Report Template {0} not found").format(template_name))

		if template.disabled:
			terminal_framework.throw(_("Financial Report Template {0} is disabled").format(template_name))
...
class DataCollector:
	...
	def __init__(self, filters: dict[str, Any], periods: list[dict]):
		...
		self.account_fields = {field.fieldname for field in terminal_framework.get_meta("Account").fields}
...
class FinancialQueryBuilder:
	...
	def _get_opening_balances(self, accounts: list[str]) -> dict[str, dict[str, dict[str, float]]]:
		...
		if terminal_framework.get_single_value("Accounts Settings", "ignore_account_closing_balance"):
			return self._get_opening_balances_from_gl(accounts)

		first_period_start = getdate(self.periods[0]["from_date"])
		last_closing_voucher = terminal_framework.db.get_all(
	...
	def _get_closing_balances(self, account_names: list[str], closing_voucher: str) -> dict[str, float]:
		...
		acb_table = terminal_framework.qb.DocType("Account Closing Balance")

		query = (
			terminal_framework.qb.from_(acb_table)
			.select(
				acb_table.account,
	...
	def _get_gap_movements(self, account_names: list[str], from_date: str, to_date: str) -> dict[str, float]:
		gl_table = terminal_framework.qb.DocType("GL Entry")

		query = (
			terminal_framework.qb.from_(gl_table)
	...
	def _get_gl_movements(self, account_names: list[str]) -> list[dict]:
		...
		ignore_is_opening = terminal_framework.get_single_value(
	...
	def _apply_standard_filters(self, query, table, doctype: str = "GL Entry"):
		...
		pcv = terminal_framework.qb.DocType("Period Closing Voucher")
		closing_heads = terminal_framework.qb.from_(pcv).select(pcv.closing_account_head).where(pcv.docstatus == 1)
		...
		if self.filters.get("include_default_book_entries"):
			default_book = terminal_framework.get_cached_value("Company", self.filters.company, "default_finance_book")
...
class FilterExpressionParser:
	...
	def build_condition(self, report_row, table):
		...
		if not errors.is_valid:
			...
			terminal_framework.log_error(f"Filter validation errors found:\n{'<br><br>'.join(error_messages)}")
			return None
		...
		try:
			...
			terminal_framework.log_error(f"Invalid filter formula syntax: {filter_formula} - {e}")
			return None
		except Exception as e:
			terminal_framework.log_error(f"Failed to build condition from formula: {filter_formula} - {e}")
			return None
	...
	def _build_simple_condition(self, condition_list: list[str, str, str | float], table):
		...
		if "like" in operator.casefold():
			...
			meta_field = terminal_framework.get_meta("Account").get_field(field_name)
...
class FormulaFieldUpdater:
	...
	def update_in_rows(self, rows: list) -> dict[str, dict[str, str]]:
		...
		if updated_rows:
			terminal_framework.db.bulk_update("Financial Report Row", updated_rows, update_modified=False)
...
def get_filtered_accounts(company: str, account_rows: str | list):
	terminal_framework.has_permission("Financial Report Template", ptype="read", throw=True)

	account_rows = [terminal_framework._dict(row) for row in terminal_framework.parse_json(account_rows)]
...
def get_children_accounts(
	...
	```
	"""
	terminal_framework.has_permission(doctype, ptype="read", throw=True)
	...
	if isinstance(filtered_accounts, str):
		filtered_accounts = terminal_framework.parse_json(filtered_accounts)
...
class RowProcessor:
	...
	def _process_api_row(self, row) -> RowData:
		...
		try:
			values = terminal_framework.call(api_path, filters=self.context.filters, periods=self.period_list, row=row)
			...
			terminal_framework.log_error(f"Custom API Error: {api_path} - {e!s}")
...
class FormulaCalculator:
	...
	def evaluate_formula(self, report_row: dict[str, Any]) -> list[float]:
		...
		if validation_result.issues:
			...
			terminal_framework.log_error(f"Formula validation errors found:\n{messages}")
	...
	def _evaluate_for_period(self, formula: str, period_index: int, negation_factor: int) -> float:
		...
		try:
			...
			result = terminal_framework.safe_eval(formula, context)
			return flt(result * negation_factor, self.precision)

		except ZeroDivisionError:
			terminal_framework.log_error(f"Division by zero in formula: {formula}")
			return 0.0
		except Exception as e:
			terminal_framework.log_error(f"Formula evaluation error: {formula} - {e!s}")
			return 0.0
		...
		for row_data in rows:
			if row_data.row.data_source == "Column Break":
				...
				if section_header and current_rows:
					...
					section_header = RowData(row=terminal_framework._dict({"data_source": "Blank Line"}))
	...
	for row_idx, row in metadata.row_map.items():
		...
		for col_idx, col in metadata.column_map.items():
			...
			if (
				...
				and cell_fieldtype in terminal_framework.model.numeric_fieldtypes
