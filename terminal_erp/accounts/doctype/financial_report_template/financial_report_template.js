...
terminal_framework.ui.form.on("Financial Report Template", {
	refresh(frm) {
		...
		frm.add_custom_button(__("View Report"), function () {
			terminal_framework.set_route("query-report", frm.doc.report_type, {
	...
	after_save(frm) {
		if (!frm.doc.rows || frm.doc.rows.length === 0) {
			terminal_framework.msgprint(__("At least one row is required for a financial report template"));
		}
	},
});

terminal_framework.ui.form.on("Financial Report Row", {
...
function set_up_filters_editor(frm, cdt, cdn) {
	...
	terminal_framework.model.with_doctype(ACCOUNT, () => {
		const filter_group = new terminal_framework.ui.FilterGroup({
			parent: wrapper,
			doctype: ACCOUNT,
			on_change: () => {
				...
				terminal_framework.model.set_value(cdt, cdn, "calculation_formula", JSON.stringify(current));
			},
		});
...
function show_accounts_tree(template_rows, has_selection) {
	...
	if (account_rows.length === 0) {
		terminal_framework.show_alert(__("No <strong>Account Data</strong> row found"));
		return;
	}
...
async function refresh_tree_view(dialog, account_rows) {
	...
	const { message: filtered_accounts } = await terminal_framework.call({
		method: "terminal_erp.accounts.doctype.financial_report_template.financial_report_engine.get_filtered_accounts",
	...
	const tree = new FilteredTree({
		...
		label: company,
		root_value: company,
		method: "terminal_erp.accounts.doctype.financial_report_template.financial_report_engine.get_children_accounts",
...
class FilteredTree extends terminal_framework.ui.Tree {
	...
	if (data_source === "Account Data") {
		...
		description_html = `
			<div ${container_style}>
				...
				<ul ${list_style}>
					<li><code>terminal_erp.custom.financial_apis.get_custom_revenue</code></li>
