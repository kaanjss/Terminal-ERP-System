...
class FinancialReportTemplateTestCase(Terminal ERPTestSuite):
	...
	def create_test_template(cls):
		...
		if not terminal_framework.db.exists("Financial Report Template", "Test P&L Template"):
		...
		cls.test_template = terminal_framework.get_doc("Financial Report Template", "Test P&L Template")

	@staticmethod
	def create_test_template_with_rows(rows_data):
		"""Helper method to create test template with specific rows"""
		template_name = f"Test Template {terminal_framework.generate_hash()[:8]}"
		template = terminal_framework.get_doc(
