...
from terminal_erp.accounts.utils import run_ledger_health_checks
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestLedgerHealth(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def configure_monitoring_tool(self):
		monitor_settings = terminal_framework.get_doc("Ledger Health Monitor")
	...
	def test_debit_credit_mismatch(self):
		...
		gle = terminal_framework.db.get_all(
			"GL Entry", filters={"voucher_no": self.je.name, "account": self.income_account}
		)[0]
		terminal_framework.db.set_value("GL Entry", gle.name, "credit", 8000)
	...
	def test_gl_and_pl_mismatch(self):
		...
		ple = terminal_framework.db.get_all("Payment Ledger Entry", filters={"voucher_no": self.je.name})[0]
		terminal_framework.db.set_value("Payment Ledger Entry", ple.name, "amount", 11000)
