...
from terminal_framework.utils import add_to_date, cint, cstr, pretty_date
from terminal_framework.utils.nestedset import NestedSet, get_ancestors_of, get_descendants_of

import terminal_erp


class RootNotEditable(terminal_framework.ValidationError):
	pass


class BalanceMismatchError(terminal_framework.ValidationError):
	pass


class InvalidAccountMergeError(terminal_framework.ValidationError):
	pass
...
class Account(NestedSet):
	...
	def on_update(self):
		if terminal_framework.local.flags.ignore_update_nsm:
			return
		else:
			super().on_update()

	def onload(self):
		role_allowed_for_frozen_entries = terminal_framework.db.get_value(
			"Company", self.company, "role_allowed_for_frozen_entries"
		)
		if not role_allowed_for_frozen_entries or role_allowed_for_frozen_entries in terminal_framework.get_roles():
	...
	def validate(self):
		if terminal_framework.local.flags.allow_unverified_charts:
			return
	...
	def validate_parent_child_account_type(self):
		if self.parent_account:
			if self.account_type in [
				...
				parent_account_type = terminal_framework.db.get_value("Account", self.parent_account, ["account_type"])
	...
	def validate_parent(self):
		...
		if self.parent_account:
			par = terminal_framework.get_cached_value(
	...
	def set_root_and_report_type(self):
		...
		if cint(self.is_group):
			...
			if db_value:
				Account = terminal_framework.qb.DocType("Account")
				query = terminal_framework.qb.update(Account).where((Account.lft > self.lft) & (Account.rgt < self.rgt))
	...
	def validate_receivable_payable_account_type(self):
		...
		if (
			...
			if terminal_framework.db.get_all("GL Entry", filters={"account": self.name, "is_cancelled": 0}, limit=1):
				msg = _(
					"There are ledger entries against this account. Changing {0} to non-{1} in live system will cause incorrect output in 'Accounts {2}' report"
				).format(
					terminal_framework.bold(_("Account Type")), doc_before_save.account_type, doc_before_save.account_type
				)
	...
	def validate_root_details(self):
		...
		if not self.parent_account and not cint(self.is_group):
			throw(_("The root account {0} must be a group").format(terminal_framework.bold(self.name)))

	def validate_root_company_and_sync_account_to_children(self):
		# ignore validation while creating new compnay or while syncing to child companies
		if terminal_framework.local.flags.ignore_root_company_validation or self.flags.ignore_root_company_validation:
			return
		ancestors = get_root_company(self.company)
		if ancestors:
			if terminal_framework.get_cached_value(
			...
			if not terminal_framework.db.get_value(
				...
				terminal_framework.throw(_("Please add the account to root level Company - {0}").format(ancestors[0]))
		elif self.parent_account:
			descendants = get_descendants_of("Company", self.company)
			if not descendants:
				return
			parent_acc_name_map = {}
			parent_acc_name, parent_acc_number = terminal_framework.get_cached_value(
			...
			for d in terminal_framework.db.get_values(
	...
	def validate_default_accounts_in_company(self):
		...
		company_default_accounts = terminal_framework.db.get_value(
			...
			if company_default_accounts.get(d) == self.name:
				throw(
					msg.format(
						...
						terminal_framework.bold(default_account_fields.get(d)),
	...
	def validate_frozen_accounts_modifier(self):
		...
		if not role_allowed_for_frozen_entries or role_allowed_for_frozen_entries not in terminal_framework.get_roles():
	...
	def validate_account_currency(self):
		...
		if not self.account_currency:
			self.account_currency = terminal_framework.get_cached_value("Company", self.company, "default_currency")
			self.currency_explicitly_specified = False

		gl_currency = terminal_framework.db.get_value(
			"GL Entry", {"account": self.name, "is_cancelled": 0}, "account_currency"
		)

		if gl_currency and self.account_currency != gl_currency:
			if terminal_framework.db.get_value("GL Entry", {"account": self.name}):
				terminal_framework.throw(_("Currency can not be changed after making entries using some other currency"))
	...
	def validate_account_number(self, account_number=None):
		...
		if account_number:
			account_with_same_number = terminal_framework.db.get_value(
				"Account",
	...
	def create_account_for_child_company(self, parent_acc_name_map, descendants, parent_acc_name):
		for company in descendants:
			company_bold = terminal_framework.bold(company)
			parent_acc_name_bold = terminal_framework.bold(parent_acc_name)
			...
			if terminal_framework.get_cached_value(
			...
			) and not terminal_framework.get_cached_value("Account", parent_acc_name_map[company], "is_group"):
				msg = _(
				...
				terminal_framework.throw(msg, title=_("Invalid Parent Account"))
			...
			child_account = terminal_framework.db.get_value("Account", filters, "name")
			if not child_account:
				doc = terminal_framework.copy_doc(self)
				...
				doc.update(
					{
						...
						else terminal_erp.get_company_currency(company),
						"parent_account": parent_acc_name_map[company],
					}
				)

				doc.save()
				terminal_framework.msgprint(_("Account {0} is added in the child company {1}").format(doc.name, company))
			elif child_account:
				# update the parent company's value in child companies
				doc = terminal_framework.get_doc("Account", child_account)
	...
	def check_gle_exists(self):
		return terminal_framework.db.get_value("GL Entry", {"account": self.name})

	def check_if_child_exists(self):
		return terminal_framework.db.exists("Account", {"parent_account": self.name, "docstatus": ["!=", 2]})
...
def get_account_currency(account):
	...
	def generator():
		account_currency, company = terminal_framework.get_cached_value(
			"Account", account, ["account_currency", "company"]
		)
		if not account_currency:
			account_currency = terminal_framework.get_cached_value("Company", company, "default_currency")

		return account_currency

	return terminal_framework.local_cache("account_currency", account, generator)


def on_doctype_update():
	terminal_framework.db.add_index("Account", ["lft", "rgt"])


def get_account_autoname(account_number, account_name, company):
	# first validate if company exists
	company = terminal_framework.get_cached_value("Company", company, ["abbr", "name"], as_dict=True)
	if not company:
		terminal_framework.throw(_("Company {0} does not exist").format(company))
...
def update_account_number(
	...
	_ensure_idle_system()
	account = terminal_framework.get_cached_doc("Account", name)
	...
	allow_independent_account_creation = terminal_framework.get_cached_value(
		"Company", account.company, "allow_account_creation_against_child_company"
	)

	if ancestors and not allow_independent_account_creation:
		for ancestor in ancestors:
			old_name = terminal_framework.db.get_value(
				"Account",
			...
			if old_name and not from_descendant:
				...
				message = _("Account {0} exists in parent company {1}.").format(
					terminal_framework.bold(old_acc_name), terminal_framework.bold(ancestor)
				)
				message += "<br>"
				message += _("Renaming it is only allowed via parent company {0}, to avoid mismatch.").format(
					terminal_framework.bold(ancestor)
				)
				message += "<br><br>"
				message += _("To overrule this, enable '{0}' in company {1}").format(
					allow_child_account_creation, terminal_framework.bold(account.company)
				)

				terminal_framework.throw(message, title=_("Rename Not Allowed"))

	account.validate_account_number(account_number)
	if account_number:
		terminal_framework.db.set_value("Account", name, "account_number", account_number.strip())
	else:
		terminal_framework.db.set_value("Account", name, "account_number", "")
	terminal_framework.db.set_value("Account", name, "account_name", account_name.strip())
	...
	if name != new_name:
		terminal_framework.rename_doc("Account", name, new_name, force=1)
		return new_name
...
def merge_account(old: str, new: str):
	_ensure_idle_system()
	new_account = terminal_framework.get_cached_doc("Account", new)
	old_account = terminal_framework.get_cached_doc("Account", old)
	...
	if old_account.is_group and new_account.parent_account == old:
		new_account.db_set("parent_account", terminal_framework.get_cached_value("Account", old, "parent_account"))

	terminal_framework.rename_doc("Account", old, new, merge=1, force=1)
...
def sync_update_account_number_in_child(
	...
	for d in terminal_framework.db.get_values("Account", filters=filters, fieldname=["company", "name"], as_dict=True):
...
def _ensure_idle_system():
	...
	try:
		...
		last_gl_update = terminal_framework.db.get_value("GL Entry", {}, "modified", for_update=True, wait=False)
	except terminal_framework.QueryTimeoutError:
