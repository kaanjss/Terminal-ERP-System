...
from terminal_framework.utils import add_months, flt
...
class MonthlyDistribution(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.monthly_distribution_percentage.monthly_distribution_percentage import (
	...
	def validate(self):
		...
		if flt(total, 2) != 100.0:
			terminal_framework.throw(_("Percentage Allocation should be equal to 100%") + f" ({flt(total, 2)!s}%)")


def get_periodwise_distribution_data(distribution_id, period_list, periodicity):
	doc = terminal_framework.get_doc("Monthly Distribution", distribution_id)
