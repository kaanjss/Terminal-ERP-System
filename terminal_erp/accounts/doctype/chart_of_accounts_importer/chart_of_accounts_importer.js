terminal_framework.ui.form.on("Chart of Accounts Importer", {
	onload: function (frm) {
		...
		var d = new terminal_framework.ui.Dialog({
			...
			primary_action: function () {
				...
				if (!data.template_type) {
					terminal_framework.throw(__("Please select <b>Template Type</b> to download template"));
				}

				open_url_post(
					"/api/method/terminal_erp.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.download_template",
					{
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.validate_company",
				args: {
	...
	frm.page
		.set_primary_action(__("Import"), function () {
			return terminal_framework.call({
				method: "terminal_erp.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.import_coa",
				args: {
...
var validate_coa = function (frm) {
	if (frm.doc.import_file) {
		...
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.get_coa",
			args: {
				file_name: frm.doc.import_file,
				parent: parent,
...
var generate_tree_preview = function (frm) {
	...
	return new terminal_framework.ui.Tree({
