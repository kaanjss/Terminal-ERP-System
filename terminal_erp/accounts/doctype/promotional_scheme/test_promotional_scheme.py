...
from terminal_erp.accounts.doctype.promotional_scheme.promotional_scheme import TransactionExists
from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestPromotionalScheme(Terminal ERPTestSuite):
	def setUp(self):
		if terminal_framework.db.exists("Promotional Scheme", "_Test Scheme"):
			terminal_framework.delete_doc("Promotional Scheme", "_Test Scheme")

	def test_promotional_scheme(self):
		ps = make_promotional_scheme(applicable_for="Customer", customer="_Test Customer")
		price_rules = terminal_framework.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name", "creation"],
			filters={"promotional_scheme": ps.name},
		)
		self.assertTrue(len(price_rules), 1)
		price_doc_details = terminal_framework.db.get_value(
		...
		terminal_framework.delete_doc("Promotional Scheme", ps.name)
		price_rules = terminal_framework.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name"],
			filters={"promotional_scheme": ps.name},
	...
	def test_promotional_scheme_without_applicable_for(self):
		...
		price_rules = terminal_framework.get_all("Pricing Rule", filters={"promotional_scheme": ps.name})
	...
	def test_min_max_amount_configuration(self):
		...
		price_rules_data = terminal_framework.db.get_value(
	...
	def test_pricing_rule_for_product_discount_slabs(self):
		...
		ps.save()
		pr = terminal_framework.get_doc("Pricing Rule", {"promotional_scheme_id": ps.product_discount_slabs[0].name})
	...
	def test_validation_on_recurse_with_mixed_condition(self):
		...
		self.assertRaises(terminal_framework.ValidationError, ps.save)


def make_promotional_scheme(**args):
	args = terminal_framework._dict(args)

	ps = terminal_framework.new_doc("Promotional Scheme")
	...
	if args.applicable_for:
		...
		ps.append(
			terminal_framework.scrub(args.applicable_for),
			{terminal_framework.scrub(args.applicable_for): args.get(terminal_framework.scrub(args.applicable_for))},
		)
