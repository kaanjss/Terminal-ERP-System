...
terminal_erp.accounts.PaymentReconciliationController = class PaymentReconciliationController extends (
	terminal_framework.ui.form.Controller
) {
	onload() {
		...
		this.frm.set_query("receivable_payable_account", () => {
			return {
				filters: {
					...
					is_group: 0,
					account_type: terminal_framework.boot.party_account_types[this.frm.doc.party_type],
			...
			this.frm.call({
				...
				callback: (r) => {
					if (r.message) {
						this.frm
							.call({
								method: "terminal_erp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.is_any_doc_running",
								args: {
									for_filter: {
										company: this.frm.doc.company,
							...
							.then((r) => {
								if (r.message) {
									let doc_link = terminal_framework.utils.get_form_link(
	...
	set_query_for_dimension_filters() {
		terminal_framework.call({
			method: "terminal_erp.accounts.doctype.payment_reconciliation.payment_reconciliation.get_queries_for_dimension_filters",
			args: {
				company: this.frm.doc.company,
			},
			callback: (r) => {
		...
		return this.frm.call({
			...
			callback: () => {
				if (!(this.frm.doc.payments.length || this.frm.doc.invoices.length)) {
					...
					terminal_framework.throw({ message: __("No Outstanding Invoices found for this party") });
				} else if (!this.frm.doc.payments.length) {
					terminal_framework.throw({ message: __("No Unreconciled Payments found for this party") });
				}
				this.frm.refresh();
			},
		});
...
terminal_framework.ui.form.on("Payment Reconciliation Allocation", {
...
extend_cscript(cur_frm.cscript, new terminal_erp.accounts.PaymentReconciliationController({ frm: cur_frm }));
