...
from terminal_framework.utils import add_days, add_years, cint, flt, getdate, nowdate, today
...
class TestPaymentReconciliation(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_difference_amount_via_journal_entry(self):
		...
		pr.reconcile()
		total_credit_amount = terminal_framework.db.get_all(
			"Journal Entry Account",
	...
	def test_user_permission_on_accounting_dimension_filters_vouchers(self):
		...
		existing_apply_strict_user_permissions = cint(
			terminal_framework.db.get_single_value("System Settings", "apply_strict_user_permissions")
		)
		self.addCleanup(
		...
		for cc in permitted_ccs:
			terminal_framework.permissions.add_user_permission("Cost Center", cc, test_user)

		# Without strict user permissions
		terminal_framework.db.set_single_value("System Settings", "apply_strict_user_permissions", 0)
		...
		terminal_framework.db.set_single_value("System Settings", "apply_strict_user_permissions", 1)
		with self.set_user(test_user):
			pr = self.create_payment_reconciliation()
			pr.get_unreconciled_entries()
		...
		with self.set_user(test_user):
			...
			self.assertRaises(terminal_framework.PermissionError, pr.get_unreconciled_entries)

		for cc in permitted_ccs:
			terminal_framework.permissions.remove_user_permission("Cost Center", cc, test_user)
	...
	def test_no_difference_amount_for_base_currency_accounts(self):
		...
		exc_gain_loss_journals = terminal_framework.db.get_all(
			"Journal Entry Account",
	...
	def test_reconciliation_purchase_invoice_against_return(self):
		...
		pi_return = terminal_framework.get_doc(pi.as_dict())
		pi_return.name = None
		...
		# Should not raise terminal_framework.exceptions.ValidationError: Total Debit must be equal to Total Credit.
		pr.reconcile()
	...
	def test_rounding_of_unallocated_amount(self):
		...
		# Should not raise terminal_framework.exceptions.ValidationError: Payment Entry has been modified after you pulled it. Please pull it again.
		pr.reconcile()
	...
	def test_advance_reverse_payment_against_payment_for_supplier(self):
		...
		pl_entries = terminal_framework.db.get_all(
	...
	def test_advance_payment_reconciliation_date_for_older_date(self):
		old_settings = terminal_framework.db.get_value(
			"Company",
			self.company,
			[
		...
		terminal_framework.db.set_value("Payment Entry Reference", pay.references[1].name, "reconcile_effect_on", None)
		pay.reload()
		pay.cancel()
		...
		terminal_framework.db.set_value("Company", self.company, old_settings)
	...
	def test_reconciliation_on_closed_period_payment(self):
		...
		first_fy_start_date = terminal_framework.db.get_value(
		...
		je_1 = terminal_framework.new_doc("Journal Entry")
	...
	def test_partial_advance_payment_with_closed_fiscal_year(self):
		...
		"""
		default_settings = terminal_framework.db.get_value(
			"Company",
			self.company,
			[
				"book_advance_payments_in_separate_party_account",
		...
		terminal_framework.db.set_value("Company", self.company, default_settings)
...
def create_fiscal_year(company, year_start_date, year_end_date):
	fy_docname = terminal_framework.db.exists(
		"Fiscal Year", {"year_start_date": year_start_date, "year_end_date": year_end_date}
	)
	if not fy_docname:
		fy_doc = terminal_framework.get_doc(
			{
				"doctype": "Fiscal Year",
		...
		fy_doc = terminal_framework.get_doc("Fiscal Year", fy_docname)
		if not terminal_framework.db.exists("Fiscal Year Company", {"parent": fy_docname, "company": company}):
