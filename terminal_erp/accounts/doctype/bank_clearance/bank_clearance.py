...
from terminal_framework.utils import cint, flt, fmt_money, getdate
...
class BankClearance(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.bank_clearance_detail.bank_clearance_detail import (
			BankClearanceDetail,
		)
	...
	def get_payment_entries(self):
		...
		if not self.account:
			terminal_framework.throw(_("Account is mandatory to get payment entries"))

		entries = []

		# get entries from all the apps
		precision = cint(terminal_framework.db.get_default("currency_precision")) or 2
		for method_name in terminal_framework.get_hooks("get_payment_entries_for_bank_clearance"):
			entries += (
		...
		default_currency = terminal_erp.get_default_currency()
	...
	def update_clearance_date(self):
		...
		for d in entries_to_update:
			if d.payment_document == "Sales Invoice":
				old_clearance_date = terminal_framework.db.get_value(
				...
				if d.clearance_date or old_clearance_date:
					...
					sales_invoice = terminal_framework.get_lazy_doc("Sales Invoice", d.payment_entry)
				...
				payment_entry = terminal_framework.get_lazy_doc(d.payment_document, d.payment_entry)
...
def get_payment_entries_for_bank_clearance(
	...
	journal_entry_account = terminal_framework.qb.DocType("Journal Entry Account")

	journal_entry_query = (
		terminal_framework.qb.from_(journal_entry_account)
	...
	if not include_reconciled_entries:
		journal_entry_query = journal_entry_query.where(
			...
			| (journal_entry.clearance_date == ("0000-00-00" if terminal_framework.db.db_type != "postgres" else None))
		)
	...
	if not include_reconciled_entries:
		payment_entry_query = payment_entry_query.where(
			...
			| (pe.clearance_date == ("0000-00-00" if terminal_framework.db.db_type != "postgres" else None))
		)
	...
	if not include_reconciled_entries:
		paid_purchase_invoices_query = paid_purchase_invoices_query.where(
			...
			| (pi.clearance_date == ("0000-00-00" if terminal_framework.db.db_type != "postgres" else None))
		)
	...
	if include_pos_transactions:
		si_payment = terminal_framework.qb.DocType("Sales Invoice Payment")
		si = terminal_framework.qb.DocType("Sales Invoice")

		pos_sales_invoices_query = (
			terminal_framework.qb.from_(si_payment)
			.inner_join(si)
		...
		if not include_reconciled_entries:
			pos_sales_invoices_query = pos_sales_invoices_query.where(
				...
				| (si_payment.clearance_date == ("0000-00-00" if terminal_framework.db.db_type != "postgres" else None))
			)
