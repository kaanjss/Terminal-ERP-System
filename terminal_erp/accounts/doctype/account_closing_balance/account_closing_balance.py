...
from terminal_erp.exceptions import ReportingCurrencyExchangeNotFoundError
...
def make_closing_entries(closing_entries, voucher_name, company, closing_date):
	...
	for _key, value in merged_entries.items():
		cle = terminal_framework.new_doc("Account Closing Balance")
		cle.update(value)
...
def get_previous_closing_entries(company, closing_date, accounting_dimensions):
	...
	if last_period_closing_voucher:
		account_closing_balance = terminal_framework.qb.DocType("Account Closing Balance")
		query = terminal_framework.qb.from_(account_closing_balance).select(
