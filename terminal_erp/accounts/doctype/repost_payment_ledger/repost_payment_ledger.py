...
from terminal_erp.accounts.utils import _delete_adv_pl_entries, _delete_pl_entries, create_payment_ledger_entry
...
def start_payment_ledger_repost(docname: str | None = None):
	...
	"""
	if docname:
		repost_doc = terminal_framework.get_doc("Repost Payment Ledger", docname)
		if repost_doc.docstatus.is_submitted() and repost_doc.repost_status in ["Queued", "Failed"]:
			try:
				for entry in repost_doc.repost_vouchers:
					doc = terminal_framework.get_doc(entry.voucher_type, entry.voucher_no)
				...
				terminal_framework.db.set_value(repost_doc.doctype, repost_doc.name, "repost_error_log", "")
				terminal_framework.db.set_value(repost_doc.doctype, repost_doc.name, "repost_status", "Completed")
			except Exception:
				...
				if traceback:
					...
					terminal_framework.db.set_value(repost_doc.doctype, repost_doc.name, "repost_error_log", message)

				terminal_framework.db.set_value(repost_doc.doctype, repost_doc.name, "repost_status", "Failed")
...
class RepostPaymentLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.repost_payment_ledger_items.repost_payment_ledger_items import (
	...
	def on_submit(self):
		...
		terminal_framework.msgprint(_("Repost started in the background"))
...
def execute_repost_payment_ledger(docname: str):
	...
	terminal_framework.enqueue(
		method="terminal_erp.accounts.doctype.repost_payment_ledger.repost_payment_ledger.start_payment_ledger_repost",
		docname=docname,
		is_async=True,
		job_name=job_name,
	)
