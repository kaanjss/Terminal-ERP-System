...
from terminal_framework import utils
from terminal_framework.model.docstatus import DocStatus

from terminal_erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool import (
	get_linked_payments,
	reconcile_vouchers,
)
...
class TestBankTransaction(Terminal ERPTestSuite):
	def setUp(self):
		make_pos_profile()

		# generate and use a uniq hash identifier for 'Bank Account' and it's linked GL 'Account' to avoid validation error
		uniq_identifier = terminal_framework.generate_hash(length=10)
	...
	# This test checks if Terminal ERP is able to provide a linked payment for a bank transaction based on the amount of the bank transaction.
	def test_linked_payments(self):
		bank_transaction = terminal_framework.get_doc(
			"Bank Transaction",
	...
	def test_reconcile(self):
		...
		payment = terminal_framework.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1700))
		vouchers = json.dumps(
			[
				{
		...
		unallocated_amount = terminal_framework.db.get_value(
			"Bank Transaction", bank_transaction.name, "unallocated_amount"
		)
		self.assertEqual(unallocated_amount, 0)

		clearance_date = terminal_framework.db.get_value("Payment Entry", payment.name, "clearance_date")
	...
	def test_clearance_date_cleared_on_amend(self):
		...
		self.assertTrue(terminal_framework.db.get_value("Payment Entry", payment.name, "clearance_date"))

		payment.reload()
		payment.cancel()

		amended = terminal_framework.copy_doc(payment)
	...
	# Check if Terminal ERP can correctly filter a linked payments based on the debit/credit amount
	...
	def test_already_reconciled(self):
		...
		payment = terminal_framework.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1200))
		vouchers = json.dumps(
			[
				{
					"payment_doctype": "Payment Entry",
	...
	def test_clear_sales_invoice(self):
		...
		payment = terminal_framework.get_doc("Sales Invoice", dict(customer="Fayva", status=["=", "Paid"]))
		vouchers = json.dumps(
			[
				{
		...
		self.assertEqual(
			terminal_framework.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount"), 0
		)
		self.assertIsNot(
			terminal_framework.db.get_value("Sales Invoice Payment", dict(parent=payment.name), "clearance_date"), None
		)
...
def add_vouchers(gl_account="_Test Bank - _TC"):
	...
	mode_of_payment = terminal_framework.get_doc({"doctype": "Mode of Payment", "name": "Wire Transfer"})
...
def create_loan_and_repayment():
	...
	loan = terminal_framework.get_doc(
		{
			"doctype": "Loan",
