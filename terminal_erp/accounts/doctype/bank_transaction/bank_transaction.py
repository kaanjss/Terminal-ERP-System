...
from terminal_framework.model.docstatus import DocStatus
from terminal_framework.model.document import Document
from terminal_framework.query_builder import Tuple
from terminal_framework.query_builder.functions import Abs, Max, Sum
from terminal_framework.utils import flt, getdate
...
class BankTransaction(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.bank_transaction_payments.bank_transaction_payments import (
	...
	def validate_currency(self):
		...
		if self.currency and self.bank_account:
			if account := terminal_framework.get_cached_value("Bank Account", self.bank_account, "account"):
				...
				if self.currency != account_currency:
					terminal_framework.throw(
						_(
							...
							terminal_framework.bold(self.currency),
							terminal_framework.bold(self.bank_account),
							terminal_framework.bold(account_currency),
						)
					)
	...
	def before_submit(self):
		...
		if terminal_framework.get_single_value("Accounts Settings", "enable_party_matching"):
			self.auto_set_party()
	...
	def add_payment_entries(self, vouchers, is_new_voucher: bool = False):
		...
		if 0.0 >= self.unallocated_amount:
			terminal_framework.throw(_("Bank Transaction {0} is already fully reconciled").format(self.name))
	...
	def allocate_payment_entries(self):
		...
		gl_bank_account = terminal_framework.db.get_value("Bank Account", self.bank_account, "account")
		...
		for payment_entry in list(self.payment_entries):
			...
			if allocable_amount < 0:
				terminal_framework.throw(_("Voucher {0} is over-allocated by {1}").format(allocable_amount))
	...
	def clear_linked_payment_entry(self, payment_entry, clearance_date=None):
		...
		terminal_framework.db.set_value(doctype, docname, "clearance_date", clearance_date)

	def update_linked_bank_transaction(self, bank_transaction_name, allocated_amount=None):
		"""For when a second bank transaction has fixed another, e.g. refund"""

		bt = terminal_framework.get_doc(self.doctype, bank_transaction_name)
		if allocated_amount:
			bt.append(
				"payment_entries",
				{
	...
	def auto_set_party(self):
		from terminal_erp.accounts.doctype.bank_transaction.auto_match_party import AutoMatchParty
		...
		try:
			...
			terminal_framework.log_error(title=_("Error in party matching for Bank Transaction {0}").format(self.name))
	...
	def validate_included_fee(self):
		...
		if self.included_fee and self.withdrawal:
			if self.included_fee > self.withdrawal:
				terminal_framework.throw(_("Included fee is bigger than the withdrawal itself."))
	...
	def handle_excluded_fee(self):
		...
		if flt(self.deposit) > 0 and (flt(self.deposit) - excluded_fee) < 0:
			terminal_framework.throw(_("The Excluded Fee is bigger than the Deposit it is deducted from."))
...
def get_doctypes_for_bank_reconciliation():
	...
	return terminal_framework.get_hooks("bank_reconciliation_doctypes")
...
def unreconcile_transaction(transaction_name: str | int):
	...
	"""
	transaction = terminal_framework.get_doc("Bank Transaction", transaction_name)
	...
	for voucher in vouchers_to_cancel:
		terminal_framework.get_doc(voucher["doctype"], voucher["name"]).cancel()
...
def unreconcile_transaction_entry(bank_transaction_id: str | int, voucher_type: str, voucher_id: str | int):
	...
	bank_transaction = terminal_framework.get_doc("Bank Transaction", bank_transaction_id)
	...
	for entry in bank_transaction.payment_entries:
		if entry.payment_document == voucher_type and entry.payment_entry == voucher_id:
			if entry.reconciliation_type == "Voucher Created":
				terminal_framework.get_doc(voucher_type, voucher_id).cancel()
			else:
...
def get_clearance_details(transaction, payment_entry, bt_allocations, gl_entries, gl_bank_account):
	...
	if payment_entry.payment_document == "Bank Transaction":
		bt = terminal_framework.db.get_value(
			"Bank Transaction",
			payment_entry.payment_entry,
			("unallocated_amount", "bank_account"),
			as_dict=True,
		)
		bt_bank_account = terminal_framework.db.get_value("Bank Account", bt.bank_account, "account")
...
def get_related_bank_gl_entries(docs):
	...
	ac = terminal_framework.qb.DocType("Account")
	result = (
...
def get_total_allocated_amount(docs):
	...
	btp = terminal_framework.qb.DocType("Bank Transaction Payments")
	bt = terminal_framework.qb.DocType("Bank Transaction")
	ba = terminal_framework.qb.DocType("Bank Account")

	result = (
		terminal_framework.qb.from_(btp)
		.left_join(bt)
...
def remove_from_bank_transaction(doctype, docname):
	...
	for bt_name in get_reconciled_bank_transactions(doctype, docname):
		bt = terminal_framework.get_doc("Bank Transaction", bt_name)
