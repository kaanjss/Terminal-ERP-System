...
from terminal_erp.accounts.doctype.bank_transaction.test_bank_transaction import create_bank_account
...
class TestAutoMatchParty(Terminal ERPTestSuite):
	def setUp(self):
		create_bank_account()
		terminal_framework.db.set_single_value("Accounts Settings", "enable_party_matching", 1)
		terminal_framework.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 1)
...
def create_supplier_for_match(supplier_name="John Doe & Co.", iban=None, account_no=None):
	if terminal_framework.db.exists("Supplier", {"supplier_name": supplier_name}):
	...
	if not terminal_framework.db.exists("Bank", "TestBank"):
		bank = terminal_framework.new_doc("Bank")
		bank.bank_name = "TestBank"
		bank.insert(ignore_if_duplicate=True)

	if not terminal_framework.db.exists("Bank Account", supplier.name + " - " + "TestBank"):
		bank_account = terminal_framework.new_doc("Bank Account")
...
def create_bank_transaction(
	...
	doc = terminal_framework.new_doc("Bank Transaction")
	doc.update(
		{
