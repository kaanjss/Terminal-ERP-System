...
class AccountsSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.repost_allowed_types.repost_allowed_types import RepostAllowedTypes
	...
	def validate(self):
		...
		if old_doc.add_taxes_from_item_tax_template != self.add_taxes_from_item_tax_template:
			terminal_framework.db.set_default(
	...
	def validate_and_sync_auto_reconcile_config(self):
		if self.has_value_changed("auto_reconciliation_job_trigger"):
			if (
				...
				terminal_framework.throw(_("Cron Interval should be between 1 and 59 Min"))

		if self.has_value_changed("reconciliation_queue_size"):
			if cint(self.reconciliation_queue_size) < 5 or cint(self.reconciliation_queue_size) > 100:
				terminal_framework.throw(_("Queue Size should be between 5 and 100"))

	def validate_auto_tax_settings(self):
		if self.add_taxes_from_item_tax_template and self.add_taxes_from_taxes_and_charges_template:
			terminal_framework.throw(
				_("You cannot enable both the settings '{0}' and '{1}'.").format(
					terminal_framework.bold(_(self.meta.get_label("add_taxes_from_item_tax_template"))),
					terminal_framework.bold(_(self.meta.get_label("add_taxes_from_taxes_and_charges_template"))),
				),
	...
	def update_property_for_accounting_dimension(self):
		...
		from terminal_erp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import get_child_docs
...
def toggle_accounting_dimension_sections(hide):
	accounting_dimension_doctypes = terminal_framework.get_hooks("accounting_dimension_doctypes")
...
def toggle_subscription_sections(hide):
	subscription_doctypes = terminal_framework.get_hooks("subscription_doctypes")
...
def set_allow_on_submit_for_dimension_fields(doctypes):
	for dt in doctypes:
		meta = terminal_framework.get_meta(dt)
		for dimension in get_accounting_dimensions():
			df = meta.get_field(dimension)
			if df and not df.allow_on_submit:
				terminal_framework.db.set_value("Custom Field", dt + "-" + dimension, "allow_on_submit", 1)
