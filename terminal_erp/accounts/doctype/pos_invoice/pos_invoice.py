...
from terminal_framework.model.mapper import map_child_doc, map_doc
from terminal_framework.query_builder.functions import IfNull, Lower, Sum
from terminal_framework.utils import cint, flt, get_link_to_form, getdate, nowdate
from terminal_framework.utils.nestedset import get_descendants_of

from terminal_erp.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
...
from terminal_erp.accounts.doctype.sales_invoice.services.loyalty import LoyaltyService
from terminal_erp.accounts.party import get_due_date, get_party_account
from terminal_erp.controllers.queries import item_query as _item_query
from terminal_erp.controllers.sales_and_purchase_return import get_sales_invoice_item_from_consolidated_invoice
...
from terminal_erp.stock.stock_ledger import is_negative_stock_allowed


class ProductBundleStockValidationError(terminal_framework.ValidationError):
	pass
...
class POSInvoice(SalesInvoice):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.pos_invoice_item.pos_invoice_item import POSInvoiceItem
		from terminal_erp.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from terminal_erp.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from terminal_erp.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from terminal_erp.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
	...
	def validate(self):
		if not self.customer:
			terminal_framework.throw(_("Please select Customer first"))

		if not cint(self.is_pos):
			terminal_framework.throw(
				_("POS Invoice should have the field {0} checked.").format(terminal_framework.bold(_("Include Payment")))
			)
	...
	def on_submit(self):
		...
		if not self.is_return and self.loyalty_program:
			...
			against_psi_doc = terminal_framework.get_doc("POS Invoice", self.return_against)
	...
	def before_cancel(self):
		if (
			...
			and terminal_framework.db.get_value("Sales Invoice", self.consolidated_invoice, "docstatus") == 1
		):
			pos_closing_entry = terminal_framework.get_all(
	...
	def clear_unallocated_mode_of_payments(self):
		...
		terminal_framework.qb.from_(sip).delete().where(sip.parent == self.name).where(sip.amount == 0).run()
	...
	def create_return_sales_invoice(self):
		return_sales_invoice = terminal_framework.new_doc("Sales Invoice")
		...
		return_sales_invoice.return_against = terminal_framework.db.get_value(
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.items:
			if row.serial_and_batch_bundle:
				...
				terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle).cancel()
				row.db_set("serial_and_batch_bundle", None)

	def submit_serial_batch_bundle(self, table_name):
		for item in self.get(table_name):
			if item.serial_and_batch_bundle:
				doc = terminal_framework.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def check_phone_payments(self):
		for pay in self.payments:
			if pay.type == "Phone" and pay.amount >= 0:
				paid_amt = terminal_framework.db.get_value(
					"Payment Request",
					filters=dict(
				...
				if paid_amt and pay.amount != paid_amt:
					return terminal_framework.throw(
	...
	def validate_stock_availablility(self):
		...
		if self.docstatus.is_draft() and not terminal_framework.db.get_value(
		...
		for d in self.get("items"):
			if not d.serial_and_batch_bundle:
				...
				if isinstance(availability, list):
						...
						if flt(item["available"]) < flt(item["required"]):
							error_msgs.append(
								_("<li>Packed Item {0}: Required {1}, Available {2}</li>").format(
									...
									terminal_framework.bold(flt(item["required"], 2)),
									terminal_framework.bold(flt(item["available"], 2)),
								)
							)
					...
					if error_msgs:
						terminal_framework.throw(
							_(
								...
								d.idx,
								terminal_framework.bold(d.item_code),
								terminal_framework.bold(d.warehouse),
								"<br>".join(error_msgs),
							),
					...
					item_code, warehouse = terminal_framework.bold(d.item_code), terminal_framework.bold(d.warehouse)
					if is_stock_item and flt(availability) <= 0:
						...
						terminal_framework.throw(
							_("Row #{0}: Item {1} in warehouse {2}: Available {3}, Needed {4}.").format(
								...
								item_code,
								warehouse,
								terminal_framework.bold(flt(availability, 2)),
								terminal_framework.bold(flt(d.stock_qty, 2)),
							),
							title=_("Insufficient Stock"),
						)

	def validate_is_pos_using_sales_invoice(self):
		self.invoice_type_in_pos = terminal_framework.db.get_single_value("POS Settings", "invoice_type")
		if self.invoice_type_in_pos == "Sales Invoice" and not self.is_return:
			terminal_framework.throw(_("Sales Invoice mode is activated in POS. Please create Sales Invoice instead."))
	...
	def validate_serialised_or_batched_item(self):
		...
		if error_msg:
			terminal_framework.throw(error_msg, title=_("Serial / Batch Bundle Missing"), as_list=True)
		...
		for d in self.get("items"):
			if d.get("qty") > 0:
				terminal_framework.throw(
					...
					).format(d.idx, terminal_framework.bold(d.item_code)),
					title=_("Invalid Item"),
				)
			if d.get("serial_no"):
				serial_nos = get_serial_nos(d.serial_no)
				for sr in serial_nos:
					POI = terminal_framework.qb.DocType("POS Invoice Item")
					s = sr.lower()
					serial_no_exists = (
						terminal_framework.qb.from_(POI)
						.select(POI.name)
					...
					if not serial_no_exists:
						bold_return_against = terminal_framework.bold(self.return_against)
						bold_serial_no = terminal_framework.bold(sr)
						terminal_framework.throw(
							_(
	...
	def validate_mode_of_payment(self):
		if len(self.payments) == 0:
			terminal_framework.throw(_("At least one mode of payment is required for POS invoice."))

	def validate_change_account(self):
		if (
			self.change_amount
			and self.account_for_change_amount
			and terminal_framework.get_cached_value("Account", self.account_for_change_amount, "company") != self.company
		):
			terminal_framework.throw(
	...
	def validate_change_amount(self):
		...
		if flt(self.change_amount) and not self.account_for_change_amount:
			terminal_framework.msgprint(_("Please enter Account for Change Amount"), raise_exception=1)
	...
	def validate_payment_amount(self):
		...
		for entry in self.payments:
			...
			if not self.is_return and entry.amount < 0:
				terminal_framework.throw(_("Row #{0} (Payment Table): Amount must be positive").format(entry.idx))
			if self.is_return and entry.amount > 0:
				terminal_framework.throw(_("Row #{0} (Payment Table): Amount must be negative").format(entry.idx))
		...
		if self.is_return and self.docstatus != 0:
			...
			if total_amount_in_payments and total_amount_in_payments < invoice_total:
				terminal_framework.throw(_("Total payments amount can't be greater than {0}").format(-invoice_total))

	def validate_company_with_pos_company(self):
		if self.company != terminal_framework.db.get_value("POS Profile", self.pos_profile, "company"):
			terminal_framework.throw(
				_("Company {0} does not match with POS Profile Company {1}").format(
					self.company, terminal_framework.db.get_value("POS Profile", self.pos_profile, "company")
				)
			)
	...
	def validate_loyalty_transaction(self):
		if self.redeem_loyalty_points and (
			...
			expense_account, cost_center = terminal_framework.db.get_value(
		...
		if not status:
			if self.docstatus == 2:
				...
				if self.consolidated_invoice:
					...
					and terminal_framework.db.get_value(
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.pos_profile:
			...
			if not pos_profile:
				terminal_framework.throw(_("No POS Profile found. Please create a New POS Profile first"))
			self.pos_profile = pos_profile.get("name")

		profile = {}
		if self.pos_profile:
			profile = terminal_framework.get_doc("POS Profile", self.pos_profile)
		...
		if profile:
			...
			if self.customer:
				customer_price_list, customer_group, customer_currency = terminal_framework.db.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group", "default_currency"]
				)
				customer_group_price_list = terminal_framework.get_cached_value(
			...
			if self.tc_name and not self.terms:
				self.terms = terminal_framework.db.get_value("Terms and Conditions", self.tc_name, "terms")
		...
		if not self.account_for_change_amount:
			self.account_for_change_amount = terminal_framework.get_cached_value(
	...
	def set_missing_values(self, for_validate: bool | None = False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = terminal_framework.get_cached_value(
		...
		if not print_format and not cint(terminal_framework.db.get_value("Print Format", "POS Invoice", "disabled")):
	...
	def reset_mode_of_payments(self):
		if self.pos_profile:
			pos_profile = terminal_framework.get_cached_doc("POS Profile", self.pos_profile)
	...
	def create_payment_request(self):
		for pay in self.payments:
			if pay.type == "Phone":
				if pay.amount <= 0:
					terminal_framework.throw(_("Payment amount cannot be less than or equal to 0"))

				if not self.contact_mobile:
					terminal_framework.throw(_("Please enter the phone number first"))
	...
	def get_new_payment_request(self, mop):
		payment_gateway_account = terminal_framework.db.get_value(
	...
	def get_existing_payment_request(self, pay):
		...
		pr = terminal_framework.db.get_value("Payment Request", filters=filters)
		if pr:
			return terminal_framework.get_doc("Payment Request", pr)

	@terminal_framework.whitelist()
	def update_payments(self, payments: list):
		if self.status == "Consolidated":
			terminal_framework.throw(_("Create Payment Entry for Consolidated POS Invoices."))

		paid_amount = flt(self.paid_amount)
		total = flt(self.rounded_total) or flt(self.grand_total)

		if paid_amount >= total:
			terminal_framework.throw(title=_("Invoice Paid"), msg=_("This invoice has already been paid."))
		...
		for d in payments:
			idx += 1
			payment = create_payments_on_invoice(self, idx, terminal_framework._dict(d))
		...
		pi = terminal_framework.qb.DocType("POS Invoice")
		query = (
			terminal_framework.qb.update(pi)
...
def get_stock_availability(item_code: str | None, warehouse: str):
	if terminal_framework.db.get_value("Item", item_code, "is_stock_item"):
		is_stock_item = True
...
def get_product_bundle_stock_availability(item_code, warehouse, item_qty):
	is_stock_item = True
	bundle = terminal_framework.get_doc("Product Bundle", get_active_product_bundle(item_code))
	availabilities = []
	for bundle_item in bundle.items:
		if terminal_framework.get_value("Item", bundle_item.item_code, "is_stock_item"):
...
def get_bundle_availability(bundle_item_code, warehouse):
	product_bundle = terminal_framework.get_doc("Product Bundle", get_active_product_bundle(bundle_item_code))
	...
	for item in product_bundle.items:
		...
		if bundle_bin_qty > max_available_bundles and terminal_framework.get_value(
...
def get_bin_qty(item_code, warehouse):
	actual_qty = terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty")
...
def get_pos_reserved_qty_from_table(child_table, item_code, warehouse):
	...
	"""
	p_inv = terminal_framework.qb.DocType("POS Invoice")
	p_item = terminal_framework.qb.DocType(child_table)

	qty_column = "qty" if child_table == "Packed Item" else "stock_qty"

	reserved_qty = (
		terminal_framework.qb.from_(p_inv)
		.from_(p_item)
...
def make_merge_log(invoices: str | list):
	...
	invoices = terminal_framework.parse_json(invoices)

	if len(invoices) == 0:
		terminal_framework.throw(_("At least one invoice has to be selected."))

	merge_log = terminal_framework.new_doc("POS Invoice Merge Log")
	merge_log.posting_date = getdate(nowdate())
	for inv in invoices:
		inv_data = terminal_framework.db.get_values(
...
def create_payments_on_invoice(doc, idx, payment_details):
	from terminal_erp.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account

	payment = terminal_framework.new_doc("Sales Invoice Payment")
	payment.idx = idx
