...
from terminal_erp.accounts.doctype.pos_invoice.pos_invoice import make_sales_return
from terminal_erp.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from terminal_erp.accounts.doctype.sales_invoice.sales_invoice import PartialPaymentValidationError
...
class POSInvoiceTestMixin(Terminal ERPTestSuite):
	def setUp(self):
		from terminal_erp.accounts.doctype.pos_closing_entry.test_pos_closing_entry import init_user_and_profile

		self.load_test_records("Stock Entry")
		self.test_user, self.pos_profile = init_user_and_profile()

		if terminal_framework.session.user != "Administrator":
...
class TestPOSInvoice(POSInvoiceTestMixin):
	...
	def test_change_naming_series(self):
		...
		self.assertRaises(terminal_framework.CannotChangeConstantError, inv.save)
	...
	def test_partial_pos_returns(self):
		...
		bundle_id = terminal_framework.get_doc(
	...
	def test_without_payment(self):
		...
		inv.payments = []
		self.assertRaises(terminal_framework.ValidationError, inv.insert)
	...
	def test_serialized_item_transaction(self):
		...
		pos2.insert()
		self.assertRaises(terminal_framework.ValidationError, pos2.submit)
	...
	def test_value_error_on_serial_no_validation(self):
		...
		terminal_framework.db.set_value("POS Invoice", pos.name, "docstatus", 2)
	...
	def test_loyalty_points(self):
		...
		terminal_framework.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Single Loyalty")
		...
		lpe = terminal_framework.get_doc(
			"Loyalty Point Entry",
	...
	def test_pos_batch_reservation(self):
		...
		batches = get_auto_batch_nos(
			terminal_framework._dict({"item_code": "_BATCH ITEM Test For Reserve", "warehouse": "_Test Warehouse - _TC"})
		)
	...
	def test_pos_batch_reservation_with_return_qty(self):
		...
		batches = get_auto_batch_nos(
			terminal_framework._dict({"item_code": "_Batch Item Reserve Return", "warehouse": "_Test Warehouse - _TC"})
		)
	...
	def test_pos_batch_item_qty_validation(self):
		...
		item = terminal_framework.get_doc("Item", "_BATCH ITEM")
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 2,
				"avg_rate": 300,
				"batches": terminal_framework._dict({"TestBatch 01": 2}),
	...
	def test_ignore_pricing_rule(self):
		from terminal_erp.accounts.doctype.pricing_rule.test_pricing_rule import make_pricing_rule
	...
	def test_delivered_serial_no_case(self):
		from terminal_erp.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
		...
		self.assertRaises(terminal_framework.ValidationError, pos_inv.submit)

	def test_bundle_stock_availability_validation(self):
		from terminal_erp.accounts.doctype.pos_invoice.pos_invoice import ProductBundleStockValidationError
		from terminal_erp.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
			init_user_and_profile,
		)
		...
		if not terminal_framework.db.exists("Item", sub_item_a):
			create_item(
		...
		if not terminal_framework.db.exists("Item", sub_item_b):
			create_item(
		...
		if not terminal_framework.db.exists("Item", bundle_item):
			create_item(
				item_code=bundle_item,
				is_stock_item=0,
			)

		if not terminal_framework.db.exists("Product Bundle", bundle_item):
		...
		terminal_framework.set_user("test@example.com")
...
def create_pos_invoice(**args):
	...
	pos_inv = terminal_framework.new_doc("POS Invoice")
	pos_inv.update(args)
	...
	pos_inv.posting_date = args.posting_date or terminal_framework.utils.nowdate()
