...
terminal_erp.accounts.pos.setup("POS Invoice");
terminal_erp.selling.POSInvoiceController = class POSInvoiceController extends terminal_erp.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		if (doc.__islocal && doc.is_pos && terminal_framework.get_route_str() !== "point-of-sale") {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "terminal_erp.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
		...
		if (this.frm.doc.is_pos) {
			...
			if (!this.frm.doc.company) {
				...
				if (!r.exc) {
					...
					terminal_framework.model.set_default_values(this.frm.doc);
	...
	make_sales_return() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	async fetch_pos_payment_methods() {
		...
		const pos_profile_doc = await terminal_framework.db.get_doc("POS Profile", pos_profile);
		const data = [];
...
extend_cscript(cur_frm.cscript, new terminal_erp.selling.POSInvoiceController({ frm: cur_frm }));

terminal_framework.ui.form.on("POS Invoice", {
	...
	request_for_payment: function (frm) {
		if (!frm.doc.contact_mobile) {
			terminal_framework.throw(__("Please enter mobile number first."));
		}
		frm.dirty();
		frm.save().then(() => {
			terminal_framework.dom.freeze(__("Waiting for payment..."));
			terminal_framework
				.call({
				...
				.fail(() => {
					...
					terminal_framework.msgprint(__("Payment request failed"));
				})
				.then(({ message }) => {
					...
					setTimeout(() => {
						terminal_framework.db
							...
							.then(({ message }) => {
								...
								} else if (terminal_framework.dom.freeze_count != 0) {
									terminal_framework.dom.unfreeze();
									frm.reload_doc();
