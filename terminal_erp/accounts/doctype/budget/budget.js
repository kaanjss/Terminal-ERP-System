...
terminal_framework.ui.form.on("Budget", {
	onload: function (frm) {
		...
		terminal_framework.db.get_single_value("Accounts Settings", "use_legacy_budget_controller").then((value) => {
			if (value) {
		...
		terminal_framework.confirm(
			...
			function () {
				terminal_framework.call({
					method: "terminal_erp.accounts.doctype.budget.budget.revise_budget",
					args: { budget_name: frm.doc.name },
					callback: function (r) {
						if (r.message) {
							terminal_framework.msgprint(__("New revised budget created successfully"));
							terminal_framework.set_route("Form", "Budget", r.message);
						}
					},
				});
			},
			function () {
				terminal_framework.msgprint(__("Revision cancelled"));
			}
		);
	},
});

terminal_framework.ui.form.on("Budget Distribution", {
	amount(frm, cdt, cdn) {
