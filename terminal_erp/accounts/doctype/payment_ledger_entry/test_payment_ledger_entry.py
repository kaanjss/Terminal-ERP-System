...
from terminal_framework.query_builder.functions import Count, Sum
...
class TestPaymentLedgerEntry(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_multi_payment_unlink_on_invoice_cancellation(self):
		...
		entries = terminal_framework.db.get_list(
		...
		si.delete()
		self.assertRaises(terminal_framework.DoesNotExistError, terminal_framework.get_doc, si.doctype, si.name)
	...
	def test_advance_payment_unlink_on_order_cancellation(self):
		...
		# with references removed, deletion should be possible
		so.delete()
		self.assertRaises(terminal_framework.DoesNotExistError, terminal_framework.get_doc, so.doctype, so.name)
