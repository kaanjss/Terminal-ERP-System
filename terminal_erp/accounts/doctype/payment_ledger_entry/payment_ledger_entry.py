...
from terminal_erp.accounts.doctype.gl_entry.gl_entry import (
	validate_balance_type,
	validate_frozen_account,
)
from terminal_erp.accounts.utils import OUTSTANDING_DOCTYPES, update_voucher_outstanding
...
class PaymentLedgerEntry(Document):
	...
	def validate_account(self):
		...
		if account.company != self.company:
			terminal_framework.throw(_("{0} account is not of company {1}").format(self.account, self.company))

		if account.account_type != self.account_type:
			terminal_framework.throw(_("{0} account is not of type {1}").format(self.account, self.account_type))
				...
				if value["is_mandatory"] and not self.get(dimension):
					terminal_framework.throw(
						_("{0} is mandatory for account {1}").format(
							terminal_framework.bold(terminal_framework.unscrub(dimension)), terminal_framework.bold(self.account)
						),
					...
					if self.get(dimension) and self.get(dimension) not in value["allowed_dimensions"]:
						terminal_framework.throw(
							_("Invalid value {0} for {1} against account {2}").format(
								terminal_framework.bold(self.get(dimension)),
...
def on_doctype_update():
	terminal_framework.db.add_index("Payment Ledger Entry", ["against_voucher_no", "against_voucher_type"])
	terminal_framework.db.add_index("Payment Ledger Entry", ["voucher_no", "voucher_type"])
