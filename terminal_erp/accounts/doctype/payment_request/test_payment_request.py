...
from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import create_payment_terms_template
...
class TestPaymentRequest(Terminal ERPTestSuite):
	def setUp(self):
		for payment_gateway in payment_gateways:
			if not terminal_framework.db.get_value("Payment Gateway", payment_gateway["gateway"], "name"):
				terminal_framework.get_doc(payment_gateway).insert(ignore_permissions=True)
		...
		for method in payment_method:
			if not terminal_framework.db.get_value(
				...
				terminal_framework.get_doc(method).insert(ignore_permissions=True)

		send_email = patch(
			"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email",
			return_value=None,
		)
		...
		_get_payment_gateway_controller = patch(
			"terminal_erp.accounts.doctype.payment_request.payment_request._get_payment_gateway_controller",
		)
	...
	def test_payment_request_linkings(self):
		...
		pr = make_payment_request(
			dt="Sales Order",
			dn=so_inr.name,
			recipient_id="saurabh@terminal_erp.com",
	...
	def test_payment_entry(self):
		...
		terminal_framework.db.set_value("Company", "_Test Company", "write_off_account", "_Test Write Off - _TC")
		terminal_framework.db.set_value("Company", "_Test Company", "cost_center", "_Test Cost Center - _TC")
		...
		so_inr = terminal_framework.get_doc("Sales Order", so_inr.name)
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@terminal_erp.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
		...
		pr2.grand_total = 900
		self.assertRaises(terminal_framework.ValidationError, pr2.save)
	...
	def test_conversion_on_foreign_currency_accounts(self):
		...
		pr = make_payment_request(dt=po_doc.doctype, dn=po_doc.name, recipient_id="nabin@terminal_erp.com")
		pr = terminal_framework.get_doc(pr).save().submit()
	...
	def test_partial_paid_invoice_with_payment_request(self):
		...
		pe.reference_date = terminal_framework.utils.nowdate()
		pe.paid_amount = 2500
	...
	def test_payment_request_on_unreconcile(self):
		...
		pe = pr.create_payment_entry()
		unreconcile = terminal_framework.get_doc(
			{
...
class TestPaymentRequestV2Gateway(Terminal ERPTestSuite):
	...
	def test_is_v2_gateway_returns_false_for_none(self):
		...
		from terminal_erp.accounts.doctype.payment_request.payment_request import _is_v2_gateway
	...
	def test_get_party_contact_and_address_returns_whitelisted_fields_only(self):
		...
		if not terminal_framework.db.exists("Contact", contact_name):
			test_contact = terminal_framework.get_doc(
				{
					"doctype": "Contact",
					"first_name": "Test",
					"last_name": "Contact",
		...
		if not terminal_framework.db.exists("Address", {"address_title": address_name}):
			test_address = terminal_framework.get_doc(
				{
					"doctype": "Address",
	...
	def test_v2_gateway_uses_process_v2_gateway(self):
		...
		with patch(
			"terminal_erp.accounts.doctype.payment_request.payment_request._is_v2_gateway",
			return_value=True,
		):
			with patch(
				"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest._process_v2_gateway"
			) as mock_process_v2:
				with patch(
					"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.set_payment_request_url"
				) as mock_set_url:
					with patch(
						"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email"
					):
	...
	def test_is_v2_gateway_returns_false_when_payments_not_installed(self):
		...
		with patch(
			"terminal_erp.accounts.doctype.payment_request.payment_request.payment_app_import_guard",
			side_effect=terminal_framework.ValidationError("Payments app not installed"),
		):
			result = _is_v2_gateway("_Test Gateway")
			self.assertFalse(result)
	...
	def test_process_v2_gateway_handles_initiate_failure(self):
		...
		with patch("terminal_erp.accounts.doctype.payment_request.payment_request.payment_app_import_guard"):
			with patch.dict(
				sys.modules,
				{
					"payments": MagicMock(),
				...
				with self.assertRaises(terminal_framework.ValidationError) as context:
	...
	def test_get_party_contact_and_address_supplier(self):
		...
		supplier = terminal_framework.get_doc("Supplier", supplier_name)
		...
		if terminal_framework.db.exists("Contact", contact_name):
			terminal_framework.delete_doc("Contact", contact_name, force=True)
		...
		test_contact = terminal_framework.get_doc(
			{
				...
				"last_name": "Contact",
				# Email and phone are set via child tables in Terminal Framework
		...
		existing_address = terminal_framework.db.get_value("Address", {"address_title": address_title}, "name")
		if existing_address:
			terminal_framework.delete_doc("Address", existing_address, force=True)
		...
		try:
			...
			terminal_framework.delete_doc("Contact", test_contact.name, force=True)
			terminal_framework.delete_doc("Address", test_address.name, force=True)
	...
	def test_get_party_contact_and_address_unsupported_party_type(self):
		...
		with patch.object(terminal_framework, "get_doc", return_value=MagicMock()):
			contact, address = pr._get_party_contact_and_address()
		...
		with patch(
			...
			with patch(
				...
				with patch(
					...
					with patch(
						"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.make_communication_entry"
					) as mock_make_comm:
						make_payment_request(
							dt="Sales Order",
							dn=so.name,
	...
	def test_v1_phone_payment_skips_email(self):
		...
		with patch(
			...
			with patch(
				"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.request_phone_payment"
			) as mock_phone:
				with patch(
					"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email"
				) as mock_send_email:
	...
	def test_no_payment_gateway_skips_payment_processing(self):
		...
		with patch("terminal_erp.accounts.doctype.payment_request.payment_request._is_v2_gateway") as mock_is_v2:
			with patch(
				"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest._process_v2_gateway"
			) as mock_v2:
				with patch(
		...
		test_contact = terminal_framework.get_doc(
			{
				...
				# Phone numbers in Terminal Framework are set via the phone_nos child table
	...
	def test_address_with_missing_optional_fields(self):
		...
		existing = terminal_framework.db.get_value("Address", {"address_title": address_title}, "name")
		if existing:
			terminal_framework.delete_doc("Address", existing, force=True)
	...
	def test_process_v2_gateway_logs_error_on_failure(self):
		"""_process_v2_gateway logs error with terminal_framework.log_error on initiate failure."""
		so = make_sales_order(currency="INR")
		...
		with patch("terminal_erp.accounts.doctype.payment_request.payment_request.payment_app_import_guard"):
			with patch.dict(
				...
				with patch("terminal_framework.log_error") as mock_log_error:
					with self.assertRaises(terminal_framework.ValidationError):
						pr._process_v2_gateway()
