...
from terminal_framework.utils import flt, nowdate
...
from terminal_erp.accounts.doctype.bank_account.bank_account import get_party_bank_account
from terminal_erp.accounts.doctype.payment_entry.payment_entry import (
	get_payment_entry,
)
from terminal_erp.accounts.doctype.subscription_plan.subscription_plan import get_plan_rate
from terminal_erp.accounts.party import get_party_account
from terminal_erp.accounts.utils import get_account_currency, get_advance_payment_doctypes, get_currency_precision
from terminal_erp.utilities import payment_app_import_guard
...
def _is_v2_gateway(payment_gateway):
	...
	try:
		...
		terminal_framework.logger().warning(f"Error detecting v2 gateway for '{payment_gateway}': {e}", exc_info=True)
		return False
...
class PaymentRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.payment_reference.payment_reference import PaymentReference
		from terminal_erp.accounts.doctype.subscription_plan_detail.subscription_plan_detail import (
	...
	def validate_against_payment_reference(self):
		...
		if flt(expected, self.precision("grand_total")) != flt(self.grand_total):
			terminal_framework.throw(_("Grand Total must match sum of Payment References"))
		...
		for r in self.payment_reference:
			...
			if r.payment_schedule in seen:
				terminal_framework.throw(_("Duplicate Payment Schedule selected"))

			seen.add(r.payment_schedule)

	def validate_reference_document(self):
		if not self.reference_doctype or not self.reference_name:
			terminal_framework.throw(_("To create a Payment Request reference document is required"))
	...
	def validate_payment_request_amount(self):
		...
		ref_doc = terminal_framework.get_doc(self.reference_doctype, self.reference_name)
		if not hasattr(ref_doc, "order_type") or ref_doc.order_type != "Shopping Cart":
			ref_amount = get_amount(ref_doc, self.payment_account)
			if not ref_amount:
				terminal_framework.throw(_("Payment Entry is already created"))
	...
	def validate_currency(self):
		ref_doc = terminal_framework.get_doc(self.reference_doctype, self.reference_name)
		if self.payment_account and ref_doc.currency != terminal_framework.get_cached_value(
			"Account", self.payment_account, "account_currency"
		):
			terminal_framework.throw(_("Transaction currency must be same as Payment Gateway currency"))

	def validate_subscription_details(self):
		if self.is_a_subscription:
			amount = 0
			for subscription_plan in self.subscription_plans:
				payment_gateway = terminal_framework.db.get_value(
	...
	def before_submit(self):
		if (
			...
			invoice = terminal_framework.get_value(
				self.reference_doctype,
				self.reference_name,
	...
	def _get_party_contact_and_address(self):
		...
		party_data = terminal_framework.get_value(
	...
	def _get_contact_fields(self, contact_name):
		...
		contact = terminal_framework.get_value(
			"Contact",
			contact_name,
	...
	def _get_address_fields(self, address_name):
		...
		address = terminal_framework.get_value(
			"Address",
			address_name,
	...
	def get_request_amount(self):
		data_of_completed_requests = terminal_framework.get_all(
			"Integration Request",
			filters={
				"reference_doctype": self.doctype,
				"reference_docname": self.name,
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			data = terminal_framework.db.get_value(
			...
			data.update({"company": terminal_framework.defaults.get_defaults().company})
		...
		return controller.get_payment_url(
			**{
				...
				"reference_doctype": "Payment Request",
				"reference_docname": self.name,
				"payer_email": self.email_to or terminal_framework.session.user,
	...
	def create_payment_entry(self, submit=True):
		"""create entry"""
		terminal_framework.flags.ignore_account_permission = True
	...
	def send_email(self):
		...
		enqueue(
			method=terminal_framework.sendmail,
			queue="short",
			timeout=300,
			is_async=True,
	...
	def get_message(self):
		...
		context = {
			"doc": terminal_framework.get_doc(self.reference_doctype, self.reference_name),
			"payment_url": self.payment_url,
			"payment_request": self,
		}

		if self.message:
			return terminal_framework.render_template(self.message, context)
	...
	def check_if_payment_entry_exists(self):
		if self.status == "Paid":
			if terminal_framework.db.exists(
				...
				terminal_framework.throw(_("Payment Entry already exists"), title=_("Error"))
...
def make_payment_request(**args):
	...
	if args.dt not in ALLOWED_DOCTYPES_FOR_PAYMENT_REQUEST:
		terminal_framework.throw(_("Payment Requests cannot be created against: {0}").format(terminal_framework.bold(args.dt)))

	if args.dn and not isinstance(args.dn, str):
		terminal_framework.throw(_("Invalid parameter. 'dn' should be of type str"))

	terminal_framework.has_permission("Payment Request", "create", throw=True)
	terminal_framework.has_permission(args.dt, "read", args.dn, throw=True)

	ref_doc = args.ref_doc or terminal_framework.get_doc(args.dt, args.dn)
	if not args.get("company"):
		args.company = ref_doc.company

	gateway_account = get_gateway_details(args) or terminal_framework._dict()
	...
	selected_payment_schedules = terminal_framework.parse_json(args.get("schedules")) if args.get("schedules") else []
	...
	def validate_and_calculate_grand_total(grand_total, existing_payment_request_amount):
		...
		if not grand_total:
			terminal_framework.throw(_("Payment Request is already created"))
		return grand_total
	...
	if existing_payment_request_amount:
		if args.order_type == "Shopping Cart":
			...
			grand_total = validate_and_calculate_grand_total(grand_total, existing_payment_request_amount)
	draft_payment_request = terminal_framework.db.get_value(
		"Payment Request",
		{"reference_doctype": ref_doc.doctype, "reference_name": ref_doc.name, "docstatus": 0},
	)

	if draft_payment_request:
		pr = terminal_framework.get_doc("Payment Request", draft_payment_request)
		...
		pr = terminal_framework.new_doc("Payment Request")
		...
		if terminal_framework.db.get_single_value("Accounts Settings", "create_pr_in_draft_status", cache=True):
	...
	if args.order_type == "Shopping Cart":
		...
		terminal_framework.local.response["type"] = "redirect"
		terminal_framework.local.response["location"] = pr.get_payment_url()
...
def set_payment_references(payment_schedules):
	payment_schedules = terminal_framework.parse_json(payment_schedules) if payment_schedules else []
	payment_reference = []
...
def get_existing_payment_entry(ref_docname):
	...
	existing_pe = (
		terminal_framework.qb.from_(pe)
		.join(per)
...
def get_irequest_status(payment_requests: None | list = None) -> list:
	IR = terminal_framework.qb.DocType("Integration Request")
	res = []
	if payment_requests:
		res = (
			terminal_framework.qb.from_(IR)
			.select(IR.name)
...
def cancel_old_payment_requests(ref_dt, ref_dn):
	...
	if res := (
		...
		if get_irequest_status([x.name for x in res]):
			terminal_framework.throw(_("Another Payment Request is already processed"))
		else:
			for x in res:
				doc = terminal_framework.get_doc("Payment Request", x.name)
				doc.flags.ignore_permissions = True
				doc.cancel()

				if ireqs := get_irequests_of_payment_request(doc.name):
					for ireq in ireqs:
						terminal_framework.db.set_value("Integration Request", ireq.name, "status", "Cancelled")
...
def get_print_format_list(ref_doctype: str):
	...
	print_format_list.extend(
		[p.name for p in terminal_framework.get_all("Print Format", filters={"doc_type": ref_doctype})]
	)
...
def resend_payment_email(docname: str):
	return terminal_framework.get_doc("Payment Request", docname).send_email()


@terminal_framework.whitelist()
def make_payment_entry(docname: str):
	doc = terminal_framework.get_doc("Payment Request", docname)
...
def update_payment_requests_as_per_pe_references(references=None, cancel=False):
	...
	precision = terminal_framework.get_precision("Payment Entry Reference", "allocated_amount")
	referenced_payment_requests = terminal_framework.get_all(
		"Payment Request",
	...
	if doc_updates:
		terminal_framework.db.bulk_update("Payment Request", doc_updates)
...
def get_subscription_details(reference_doctype: str, reference_name: str):
	if reference_doctype == "Sales Invoice":
		subscriptions = terminal_framework.get_all(
		...
		for subscription in subscriptions:
			plans = terminal_framework.get_doc("Subscription", subscription.sub_name).plans
			for plan in plans:
...
def validate_payment(doc, method=None):
	if doc.reference_doctype != "Payment Request" or (
		terminal_framework.db.get_value(doc.reference_doctype, doc.reference_docname, "status") != "Paid"
	):
		return
...
def get_open_payment_requests_query(
	...
	filters = terminal_framework._dict(filters)
	...
	open_payment_requests = terminal_framework.get_list(
		"Payment Request",
		filters=filters,
...
def get_available_payment_schedules(reference_doctype: str, reference_name: str):
	ref_doc = terminal_framework.get_doc(reference_doctype, reference_name)
...
def get_existing_payment_references(reference_name):
	PR = terminal_framework.qb.DocType("Payment Request")
	PRF = terminal_framework.qb.DocType("Payment Reference")
