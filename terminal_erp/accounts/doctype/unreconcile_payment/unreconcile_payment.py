...
class UnreconcilePayment(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.unreconcile_payment_entries.unreconcile_payment_entries import (
	...
	def validate(self):
		...
		if self.voucher_type not in self.supported_types:
			terminal_framework.throw(_("Only {0} are supported").format(comma_and(self.supported_types)))
	...
	def on_submit(self):
		...
		for alloc in self.allocations:
			doc = terminal_framework.get_doc(alloc.reference_doctype, alloc.reference_name)
			...
			terminal_framework.db.set_value("Unreconcile Payment Entries", alloc.name, "unlinked", 1)
...
def create_unreconcile_doc_for_selection(selections: str | list | None = None):
	if selections:
		selections = terminal_framework.parse_json(selections)
		# assuming each row is a unique voucher
		for row in selections:
			unrecon = terminal_framework.new_doc("Unreconcile Payment")
