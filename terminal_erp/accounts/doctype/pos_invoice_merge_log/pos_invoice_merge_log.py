...
from terminal_framework.utils import cint, flt, get_time, getdate, nowdate, nowtime
from terminal_framework.utils.background_jobs import enqueue, is_job_enqueued
...
class POSInvoiceMergeLog(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.pos_invoice_reference.pos_invoice_reference import POSInvoiceReference
	...
	def validate_duplicate_pos_invoices(self):
		...
		for key, value in pos_occurences.items():
			if len(value) > 1:
				error_list.append(
					_("{0} is added multiple times on rows: {1}").format(terminal_framework.bold(key), terminal_framework.bold(value))
				)

		if error_list:
			terminal_framework.throw(error_list, title=_("Duplicate POS Invoices found"), as_list=True)
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			status, docstatus, is_return, return_against = terminal_framework.db.get_value(
				"POS Invoice", d.pos_invoice, ["status", "docstatus", "is_return", "return_against"]
			)

			bold_pos_invoice = terminal_framework.bold(d.pos_invoice)
			bold_status = terminal_framework.bold(status)
			if docstatus != 1:
				terminal_framework.throw(
			...
			if (
				...
				bold_return_against = terminal_framework.bold(return_against)
				return_against_status = terminal_framework.db.get_value("POS Invoice", return_against, "status")
	...
	def on_submit(self):
		pos_invoice_docs = [terminal_framework.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices]
	...
	def distinguish_return_pos_invoices(self, data, sales_invoice_doc=None):
		...
		for doc in data:
			sales_invoices_of_return_against = terminal_framework.db.get_value(
	...
	def merge_pos_invoice_into(self, invoice, data):
		...
		for doc in data:
			old_new_item_map = terminal_framework._dict()
			old_new_tax_map = terminal_framework._dict()
		...
		invoice.disable_rounded_total = cint(
			terminal_framework.db.get_value("POS Profile", invoice.pos_profile, "disable_rounded_total")
		)
			...
			if not dimension_value and (dimension.mandatory_for_pl or dimension.mandatory_for_bs):
				terminal_framework.throw(
					_("Please set Accounting Dimension {0} in {1}").format(
						terminal_framework.bold(dimension.label),
						terminal_framework.get_desk_link("POS Profile", invoice.pos_profile),
					)
				)
	...
	def get_new_sales_invoice(self):
		sales_invoice = terminal_framework.new_doc("Sales Invoice")
	...
	def serial_and_batch_bundle_reference_for_pos_invoice(self):
		for d in self.pos_invoices:
			pos_invoice = terminal_framework.get_doc("POS Invoice", d.pos_invoice)
	...
	def delink_serial_and_batch_bundle(self):
		...
		query = (
			terminal_framework.qb.update(sle_table)
	...
	def cancel_linked_invoices(self):
		...
		for si_name in invoices:
			if not si_name:
				continue
			si = terminal_framework.get_doc("Sales Invoice", si_name)
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = terminal_framework.db.get_all(
		"POS Invoice",
		filters=filters,
		fields=[
			"name as pos_invoice",
...
def split_invoices_by_accounting_dimension(pos_invoices):
	...
	for invoice in pos_invoices:
		...
		accounting_dimensions = terminal_framework.db.get_value(
...
def consolidate_pos_invoices(pos_invoices=None, closing_entry=None):
	...
	if terminal_framework.in_test and not invoices:
...
def unconsolidate_pos_invoices(closing_entry):
	merge_logs = terminal_framework.get_all(
...
def split_invoices(invoices):
	...
	special_invoices = []
	pos_return_docs = [
		terminal_framework.get_cached_doc("POS Invoice", d.pos_invoice)
		for d in invoices
	...
	for pos_invoice in pos_return_docs:
		for item in pos_invoice.items:
			...
			return_against_is_consolidated = (
				terminal_framework.db.get_value("POS Invoice", pos_invoice.return_against, "status", cache=True)
				== "Consolidated"
			)
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	...
	if terminal_framework.in_test:
		...
		try:
			...
			message_log = terminal_framework.message_log.pop() if terminal_framework.message_log else str(e)
			...
			terminal_framework.publish_realtime("closing_process_complete", user=terminal_framework.session.user)


def cancel_merge_logs(merge_logs, closing_entry=None):
	def merge_cancel_and_close():
		for log in merge_logs:
			merge_log = terminal_framework.get_doc("POS Invoice Merge Log", log)
...
def enqueue_job(job, **kwargs):
	...
	if not is_job_enqueued(job_id):
		...
		terminal_framework.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not terminal_framework.in_test:
		terminal_framework.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
