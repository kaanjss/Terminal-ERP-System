...
from terminal_erp.setup.doctype.customer_group.customer_group import get_parent_customer_groups
from terminal_erp.setup.doctype.supplier_group.supplier_group import get_parent_supplier_groups


class IncorrectCustomerGroup(terminal_framework.ValidationError):
	pass


class IncorrectSupplierType(terminal_framework.ValidationError):
	pass


class ConflictingTaxRule(terminal_framework.ValidationError):
	pass
...
class TaxRule(Document):
	...
	def validate_tax_template(self):
		...
		if not (self.sales_tax_template or self.purchase_tax_template):
			terminal_framework.throw(_("Tax Template is mandatory."))
	...
	def validate_filters(self):
		...
		query = (
			terminal_framework.qb.from_(TaxRule).select(TaxRule.name, TaxRule.priority).where(TaxRule.name != self.name)
		)
...
def get_party_details(party: str | None, party_type: str, args: dict | None = None):
	...
	if args:
		if args.get("billing_address"):
			billing_address = terminal_framework.get_doc("Address", args.get("billing_address"))
		if args.get("shipping_address"):
			shipping_address = terminal_framework.get_doc("Address", args.get("shipping_address"))
	else:
		...
		if billing_address_name:
			billing_address = terminal_framework.get_doc("Address", billing_address_name)
		if shipping_address_name:
			shipping_address = terminal_framework.get_doc("Address", shipping_address_name)
...
def get_tax_template(posting_date, args):
	...
	TaxRule = DocType("Tax Rule")
	query = terminal_framework.qb.from_(TaxRule).select("*")
	...
	if terminal_framework.db.get_value(doctype, tax_template, "disabled") == 1:
		return None
