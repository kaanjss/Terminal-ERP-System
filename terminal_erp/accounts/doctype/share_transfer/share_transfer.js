...
terminal_framework.provide("terminal_erp.share_transfer");

terminal_framework.ui.form.on("Share Transfer", {
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.equity_or_liability_account && frm.doc.asset_account) {
			frm.add_custom_button(__("Create Journal Entry"), function () {
				terminal_erp.share_transfer.make_jv(frm);
			});
		}
	...
	no_of_shares: (frm) => {
		if (frm.doc.rate != undefined || frm.doc.rate != null) {
			terminal_erp.share_transfer.update_amount(frm);
		}
	},
	rate: (frm) => {
	...
	company: async function (frm) {
		if (frm.doc.company) {
			let currency = (await terminal_framework.db.get_value("Company", frm.doc.company, "default_currency")).message
				.default_currency;
...
terminal_erp.share_transfer.update_amount = function (frm) {
	frm.doc.amount = frm.doc.no_of_shares * frm.doc.rate;
	frm.refresh_field("amount");
};

terminal_erp.share_transfer.make_jv = function (frm) {
	var account,
		payment_account,
	...
	terminal_framework.call({
		...
		method: "terminal_erp.accounts.doctype.share_transfer.share_transfer.make_jv_entry",
		callback: function (r) {
