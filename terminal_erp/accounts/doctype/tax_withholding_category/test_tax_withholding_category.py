...
from terminal_framework.utils import add_days, add_months, getdate, today
...
class TestTaxWithholdingCategory(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_tds_withholding_group_different_rates(self):
		...
		for group_name in ["Individual", "Company"]:
			if not terminal_framework.db.exists("Tax Withholding Group", group_name):
				terminal_framework.get_doc({"doctype": "Tax Withholding Group", "group_name": group_name}).insert()
		...
		if not terminal_framework.db.exists("Tax Withholding Category", "TDS Group Rate Category"):
		...
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier5", "tax_withholding_group", "Individual")
		...
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier6", "tax_withholding_group", "Company")
	...
	def test_null_and_empty_tax_withholding_group_are_equivalent(self):
		...
		same value.
		"""
		category = terminal_framework.get_doc("Tax Withholding Category", "Cumulative Threshold TDS")
	...
	def test_tds_calculation_on_net_total_partial_tds(self):
		...
		item_code = terminal_framework.db.get_value("Item", {"item_name": "TDS Item"}, "name")
	...
	def set_previous_fy_and_tax_category(self):
		...
		if prev_fiscal_year := get_fiscal_year(add_days(fiscal_year[1], -10)):
			self.prev_fy = terminal_framework.get_doc("Fiscal Year", prev_fiscal_year[0])
			...
			self.prev_fy = terminal_framework.get_doc(
				{
					"doctype": "Fiscal Year",
	...
	def test_tds_with_multi_currency_invoice(self):
		...
		self.validate_tax_withholding_entries("Purchase Invoice", pi.name, pi_expected)
		self.cleanup_invoices(invoices)
		terminal_framework.db.set_value("Supplier", "_Test Supplier USD", "tax_withholding_category", "")
	...
	def test_journal_entry_negative_amount_debit_note(self):
		...
		jv2 = terminal_framework.new_doc("Journal Entry")
	...
	def test_tax_withholding_entry_status_determination(self):
		...
		from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import (
			TaxWithholdingEntry,
		)

		# Entry with only taxable fields (Under Withheld)
		entry = terminal_framework._dict(
			docstatus=1,
			withholding_name="",
			under_withheld_reason="",
			taxable_name="PI-001",
		...
		# Cancelled entry
		entry = terminal_framework._dict(docstatus=2, withholding_name="", under_withheld_reason="", taxable_name="")
	...
	def test_invalid_withholding_amount_validation(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, "Withholding Amount.*does not match", pi.save)

	def test_tax_id_is_set_in_all_generated_entries_from_party_doctype(self):
		self.setup_party_with_category("Supplier", "Test TDS Supplier3", "New TDS Category")
		terminal_framework.db.set_value("Supplier", "Test TDS Supplier3", "tax_id", "ABCTY1234D")
	...
	def test_threshold_considers_two_parties_with_same_tax_id_with_overrided_hook(self):
		...
		with patch(
			"terminal_erp.accounts.doctype.tax_withholding_category.tax_withholding_category.get_tax_id_for_party",
...
def create_purchase_invoice(**args):
	...
	item = terminal_framework.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = terminal_framework.db.get_value("Item", {"item_name": "TCS Item"}, "name")

	args = terminal_framework._dict(args)
	si = terminal_framework.get_doc(
		{
...
def create_records():
	...
	for name in [
		...
		if terminal_framework.db.exists("Supplier", name):
			continue
	...
	# create item
	if not terminal_framework.db.exists("Item", "TDS Item"):
		terminal_framework.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not terminal_framework.db.exists("Item", "TCS Item"):
		terminal_framework.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not terminal_framework.db.exists("Account", "TDS - _TC"):
		terminal_framework.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not terminal_framework.db.exists("Account", "TCS - _TC"):
		terminal_framework.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_withholding_category(
	...
	if not terminal_framework.db.exists("Tax Withholding Category", category_name):
		...
		doc = terminal_framework.get_doc("Tax Withholding Category", category_name)
...
def create_lower_deduction_certificate(
	...
	fiscal_year = get_fiscal_year(today(), company="_Test Company")
	if not terminal_framework.db.exists("Lower Deduction Certificate", certificate_no):
		terminal_framework.get_doc(
			{
