...
from terminal_erp import allow_regional
...
class TaxWithholdingCategory(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.tax_withholding_account.tax_withholding_account import (
			TaxWithholdingAccount,
		)
		from terminal_erp.accounts.doctype.tax_withholding_rate.tax_withholding_rate import TaxWithholdingRate
	...
	def validate_dates(self):
		...
		for d in self.get("rates"):
			if getdate(d.from_date) >= getdate(d.to_date):
				terminal_framework.throw(_("Row #{0}: From Date cannot be before To Date").format(d.idx))
	...
	def validate_companies_and_accounts(self):
		...
		for d in self.get("accounts"):
			...
			if d.get("company") in companies:
				terminal_framework.throw(_("Company {0} added multiple times").format(terminal_framework.bold(d.get("company"))))
			companies.add(d.get("company"))

			# validate duplicate account
			if d.get("account") in existing_accounts:
				terminal_framework.throw(_("Account {0} added multiple times").format(terminal_framework.bold(d.get("account"))))
	...
	def get_applicable_tax_row(self, posting_date, tax_withholding_group):
		...
		terminal_framework.throw(_("No Tax Withholding data found for the current posting date."))
	...
	def get_company_account(self, company):
		...
		terminal_framework.throw(
			_("No Tax withholding account set for Company {0} in Tax Withholding Category {1}.").format(
				terminal_framework.bold(company), terminal_framework.bold(self.name)
			)
		)
...
class TaxWithholdingDetails:
	...
	def get(self) -> list:
		...
		"""
		category_details = terminal_framework._dict()
		...
		for category_name in self.tax_withholding_categories:
			doc: TaxWithholdingCategory = terminal_framework.get_cached_doc("Tax Withholding Category", category_name)
			...
			category_detail = terminal_framework._dict(
				name=category_name,
	...
	def get_valid_ldc_records(self, tax_id):
		ldc = terminal_framework.qb.DocType("Lower Deduction Certificate")
		query = (
			terminal_framework.qb.from_(ldc)
			.select(
				ldc.name,
	...
	def get_ldc_utilization_by_category(self, ldc_names, tax_id):
		twe = terminal_framework.qb.DocType("Tax Withholding Entry")
		query = (
			terminal_framework.qb.from_(twe)
...
def get_tax_id_for_party(party_type, party):
	...
	return terminal_framework.db.get_value(party_type, party, "tax_id")
