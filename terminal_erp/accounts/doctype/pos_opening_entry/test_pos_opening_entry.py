...
class TestPOSOpeningEntry(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_multiple_pos_opening_entry_for_multiple_pos_profiles(self):
		...
		terminal_framework.set_user(cashier_user.name)
	...
	def test_cancel_pos_opening_entry_with_invoice(self):
		...
		self.assertRaises(terminal_framework.ValidationError, opening_entry.cancel)


def create_opening_entry(pos_profile, user, get_obj=False):
	entry = terminal_framework.new_doc("POS Opening Entry")
	entry.pos_profile = pos_profile.name
	entry.user = user
	entry.company = pos_profile.company
	entry.period_start_date = terminal_framework.utils.get_datetime()

	balance_details = []
	for d in pos_profile.payments:
		balance_details.append(terminal_framework._dict({"mode_of_payment": d.mode_of_payment}))
