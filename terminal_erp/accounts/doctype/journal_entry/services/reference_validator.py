...
from terminal_framework.utils import cstr, flt, fmt_money

from terminal_erp.accounts.deferred_revenue import get_deferred_booking_accounts
...
class JournalEntryReferenceValidator:
	...
	def _validate_reference_party_and_account(self, row) -> None:
		...
		against_voucher = terminal_framework.db.get_value(
			row.reference_type, row.reference_name, [scrub(f) for f in party_fields]
		)
		if not against_voucher:
			terminal_framework.throw(_("Row {0}: Invalid reference {1}").format(row.idx, row.reference_name))
	...
	def _validate_orders(self) -> None:
		...
		for reference_name, total in self.doc.reference_totals.items():
			...
			order = terminal_framework.get_doc(reference_type, reference_name)
	...
	def _validate_order_status(self, order, reference_type, reference_name) -> None:
		if order.docstatus != 1:
			terminal_framework.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))
		if flt(order.per_billed) >= 100:
			terminal_framework.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))
		if cstr(order.status) == "Closed":
			terminal_framework.throw(_("{0} {1} is closed").format(reference_type, reference_name))
	...
	def _validate_invoices(self) -> None:
		...
		for reference_name, total in self.doc.reference_totals.items():
			...
			if reference_type not in ("Sales Invoice", "Purchase Invoice"):
				continue
			invoice = terminal_framework.get_doc(reference_type, reference_name)
