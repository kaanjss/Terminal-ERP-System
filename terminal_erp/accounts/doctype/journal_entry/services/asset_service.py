...
class AssetService:
	...
	def validate_depr_account_and_depr_entry_voucher_type(self) -> None:
		...
		for d in self.doc.get("accounts"):
			if d.account_type == "Depreciation":
				...
				if terminal_framework.get_cached_value("Account", d.account, "root_type") != "Expense":
					terminal_framework.throw(_("Account {0} should be of type Expense").format(d.account))
	...
	def has_asset_adjustment_entry(self) -> None:
		...
		asset_value_adjustment = terminal_framework.db.get_value(
		...
		if asset_value_adjustment:
			terminal_framework.throw(
				...
				).format(terminal_framework.utils.get_link_to_form("Asset Value Adjustment", asset_value_adjustment))
			)
	...
	def update_asset_on_depreciation(self) -> None:
		...
		for d in self.doc.get("accounts"):
			if (
				...
				and terminal_framework.get_cached_value("Account", d.account, "root_type") == "Expense"
				and d.debit
			):
				asset = terminal_framework.get_cached_doc("Asset", d.reference_name)
	...
	def update_journal_entry_link_on_depr_schedule(self, asset, je_row) -> None:
		...
		for d in depr_schedule or []:
			if (
				...
				terminal_framework.db.set_value("Depreciation Schedule", d.name, "journal_entry", self.doc.name)
		...
		if self.doc.voucher_type == "Asset Disposal":
			...
			for d in self.doc.get("accounts"):
				if (
					...
					asset_doc = terminal_framework.get_doc("Asset", d.reference_name)
	...
	def _reverse_asset_depreciation(self, d) -> None:
		...
		asset = terminal_framework.get_doc("Asset", d.reference_name)
	...
	def _block_scrap_journal_cancel(self, d) -> None:
		...
		journal_entry_for_scrap = terminal_framework.db.get_value("Asset", d.reference_name, "journal_entry_for_scrap")
	...
	def unlink_asset_adjustment_entry(self) -> None:
		...
		(
			terminal_framework.qb.update(AssetValueAdjustment)
