...
from terminal_erp.accounts.doctype.journal_entry.journal_entry import StockAccountInvalidTransaction
from terminal_erp.exceptions import InvalidAccountCurrency
from terminal_erp.selling.doctype.customer.test_customer import make_customer, set_credit_limit
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestJournalEntry(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_journal_entry_with_against_jv(self):
		jv_invoice = terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][2])
		base_jv = terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][0])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = terminal_framework.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def advance_paid_testcase(self, base_jv, test_voucher, dr_or_cr):
		...
		advance_paid = terminal_framework.db.get_value(test_voucher.doctype, test_voucher.name, "advance_paid")
	...
	def cancel_against_voucher_testcase(self, test_voucher):
		if test_voucher.doctype == "Journal Entry":
			...
			submitted_voucher = terminal_framework.get_doc(test_voucher.doctype, test_voucher.name)
			self.assertRaises(terminal_framework.LinkExistsError, submitted_voucher.cancel)
	...
	def test_jv_with_project(self):
		...
		if not terminal_framework.db.exists("Project", {"project_name": "Journal Entry Project"}):
			...
			project_name = terminal_framework.get_value("Project", {"project_name": "_Test Project"})
	...
	def check_gl_entries(self):
		...
		query = terminal_framework.qb.from_(gl)
	...
	def test_negative_debit_and_credit_with_same_account_head(self):
		from terminal_erp.accounts.general_ledger import process_gl_map
	...
	def test_credit_limit_for_customer(self):
		...
		jv.save()
		self.assertRaises(terminal_framework.ValidationError, jv.submit)
	...
	def test_validate_reference_doc_debit_against_sales_order_throws(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, "Debit entry can not be linked", jv.insert)
	...
	def test_validate_reference_doc_credit_against_purchase_order_throws(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, "Credit entry can not be linked", jv.insert)
	...
	def test_validate_reference_doc_nonexistent_reference_rejected(self):
		...
		because Terminal Framework link validation rejects the missing reference before validate_reference_doc.
		"""
		...
		self.assertRaises(terminal_framework.LinkValidationError, jv.insert)
	...
	def test_validate_reference_doc_invoice_party_mismatch_throws(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, "Party / Account does not match", jv.insert)
	...
	def test_validate_reference_doc_order_party_mismatch_throws(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, "does not match", jv.insert)
	...
	def test_get_payment_entry_against_order_builds_advance_je(self):
		...
		from terminal_erp.accounts.doctype.journal_entry.mapper import get_payment_entry_against_order
		from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
	...
	def test_make_inter_company_journal_entry_builds_linked_draft(self):
		...
		from terminal_erp.accounts.doctype.journal_entry.mapper import make_inter_company_journal_entry
