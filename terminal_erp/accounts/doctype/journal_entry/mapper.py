...
from terminal_framework.utils import flt, get_link_to_form, nowdate
...
def get_payment_entry_against_order(
	...
	ref_doc = terminal_framework.get_doc(dt, dn)

	if flt(ref_doc.per_billed, 2) > 0:
		terminal_framework.throw(_("Can only make payment against unbilled {0}").format(dt))
...
def get_payment_entry(ref_doc, args: dict) -> dict | Document:
	...
	cost_center = ref_doc.get("cost_center") or terminal_framework.get_cached_value(
...
def _append_party_row(je, ref_doc, args: dict, cost_center, exchange_rate: float):
	...
	return je.append(
		"accounts",
		{
			...
			"account_type": terminal_framework.get_cached_value("Account", args.get("party_account"), "account_type"),
...
def _append_bank_row(je, ref_doc, args: dict, cost_center, exchange_rate: float):
	...
	from terminal_erp.accounts.doctype.journal_entry.journal_entry import (
...
def make_reverse_journal_entry(source_name: str, target_doc: str | Document | None = None) -> Document:
	...
	existing_reverse = terminal_framework.db.exists("Journal Entry", {"reversal_of": source_name, "docstatus": 1})
	if existing_reverse:
