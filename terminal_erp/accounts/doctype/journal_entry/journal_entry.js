...
terminal_framework.provide("terminal_erp.accounts");
terminal_framework.provide("terminal_erp.journal_entry");

terminal_framework.ui.form.on("Journal Entry", {
	setup(frm) {
	...
	onload(frm) {
		terminal_erp.journal_entry.load_defaults(frm);
		terminal_erp.journal_entry.setup_queries(frm);
	...
	refresh(frm) {
		...
		terminal_erp.journal_entry.add_custom_buttons(frm);
		terminal_erp.journal_entry.toggle_fields_based_on_currency(frm);
		terminal_erp.accounts.unreconcile_payment.add_unreconcile_btn(frm);

		if (frm.doc.voucher_type !== "Exchange Gain Or Loss") {
			(frm.doc.accounts || []).forEach((row) =>
				terminal_erp.journal_entry.set_exchange_rate(frm, row.doctype, row.name)
			);
		}
	},
		...
		if (manual_payment_references.length) {
			...
			terminal_framework.throw(
				__("Rows: {0} have 'Payment Entry' as reference_type. This should not be set manually.", [
					terminal_framework.utils.comma_and(rows),
				])
			);
		}
	},
	...
	company(frm) {
		terminal_framework.db.get_value("Company", frm.doc.company, "cost_center").then(({ message }) => {
			if (!message) return;
			(frm.doc.accounts || []).forEach((row) =>
				terminal_framework.model.set_value(row.doctype, row.name, "cost_center", message.cost_center)
			);
		});
	...
	voucher_type(frm) {
		...
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
			args: {
			...
			callback: ({ message }) => {
				if (message && !$.isEmptyObject(message)) {
					terminal_erp.journal_entry.update_jv_details(frm, [message]);
				}
			},
		});
	},
	...
	from_template(frm) {
		...
		terminal_framework.db.get_doc("Journal Entry Template", frm.doc.from_template).then((template) => {
			terminal_framework.model.clear_table(frm.doc, "accounts");
			frm.set_value({
			...
			terminal_erp.journal_entry.update_jv_details(frm, template.accounts);
		});
	},
	...
	get_balance(frm) {
		terminal_erp.journal_entry.update_totals(frm);
...
terminal_framework.ui.form.on("Journal Entry Account", {
	party(frm, cdt, cdn) {
		const row = terminal_framework.get_doc(cdt, cdn);
		if (row.account || !row.party_type || !row.party) return;

		if (!frm.doc.company) terminal_framework.throw(__("Please select Company"));
		return frm.call({
			method: "terminal_erp.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
			child: row,
	...
	account(frm, cdt, cdn) {
		terminal_erp.journal_entry.set_account_details(frm, cdt, cdn);
	},

	debit_in_account_currency(frm, cdt, cdn) {
		terminal_erp.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate(frm, cdt, cdn) {
		const row = terminal_framework.get_doc(cdt, cdn);
		const company_currency = terminal_erp.get_currency(frm.doc.company);
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			terminal_framework.model.set_value(cdt, cdn, "exchange_rate", 1);
		}
		terminal_erp.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
	...
	reference_name(frm, cdt, cdn) {
		...
		if (row.reference_type === "Purchase Invoice" && !flt(row.debit)) {
			terminal_erp.journal_entry.get_outstanding(frm, "Purchase Invoice", row.reference_name, row);
		} else if (row.reference_type === "Sales Invoice" && !flt(row.credit)) {
			terminal_erp.journal_entry.get_outstanding(frm, "Sales Invoice", row.reference_name, row);
		} else if (row.reference_type === "Journal Entry" && !flt(row.credit) && !flt(row.debit)) {
			terminal_erp.journal_entry.get_outstanding(frm, "Journal Entry", row.reference_name, row);
		}
	},
	...
	accounts_add(frm, cdt, cdn) {
		...
		terminal_erp.journal_entry.set_balancing_amount(row, frm.doc.difference);
		terminal_erp.journal_entry.update_totals(frm);
		terminal_erp.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, "accounts");
	},
...
Object.assign(terminal_erp.journal_entry, {
	load_defaults(frm) {
		if (!(frm.doc.__islocal && frm.doc.company)) return;

		terminal_framework.model.set_default_values(frm.doc);
		(frm.doc.accounts || []).forEach((row) => terminal_framework.model.set_default_values(row));

		if (!frm.doc.amended_from) {
			frm.set_value("posting_date", frm.doc.posting_date || terminal_framework.datetime.get_today());
		}
	},
	...
	add_custom_buttons(frm) {
		if (frm.doc.docstatus > 0) {
			frm.add_custom_button(
				__("Ledger"),
				() => terminal_erp.journal_entry.show_general_ledger(frm),
				__("View")
			);
		}
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				() => terminal_erp.journal_entry.reverse_journal_entry(frm),
				__("Actions")
			);
		}

		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), () => terminal_erp.journal_entry.quick_entry(frm));
		}
		...
		if (
			...
			frm.add_custom_button(
				...
				() => terminal_erp.journal_entry.make_inter_company_journal_entry(frm),
				__("Make")
			);
		}
	},
	...
	make_inter_company_journal_entry(frm) {
		...
		dialog.set_primary_action(__("Create"), () => {
			...
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.journal_entry.mapper.make_inter_company_journal_entry",
				args: {
					name: frm.doc.name,
				...
				callback: ({ message }) => {
					if (message) {
						const doc = terminal_framework.model.sync(message)[0];
	...
	reverse_journal_entry(frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.accounts.doctype.journal_entry.mapper.make_reverse_journal_entry",
			frm: frm,
		});
	},
	...
	quick_entry(frm) {
		...
		const dialog = new terminal_framework.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					reqd: 1,
					options: "Account",
					get_query: () => terminal_erp.journal_entry.account_query(frm),
				},
				{
					fieldtype: "Link",
		...
		dialog.set_primary_action(__("Save"), () => {
			terminal_erp.journal_entry.save_quick_entry(frm, dialog.get_values());
			dialog.hide();
		});
		dialog.show();
	},
	...
	save_quick_entry(frm, values) {
		...
		terminal_erp.journal_entry.add_quick_entry_row(
			frm,
	...
	add_quick_entry_row(frm, account, amount_field, amount) {
		...
		terminal_framework.model.set_value(row.doctype, row.name, "account", account);
		terminal_framework.model.set_value(row.doctype, row.name, amount_field, amount);
	},

	get_outstanding(frm, reference_type, reference_name, child) {
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: {
			...
			callback: ({ message }) => {
				...
				Object.entries(message).forEach(([field, value]) =>
					terminal_framework.model.set_value(child.doctype, child.name, field, value)
				);
			},
		});
	},
	...
	set_account_details(frm, cdt, cdn) {
		const row = terminal_framework.get_doc(cdt, cdn);
		if (!row.account) {
			terminal_erp.journal_entry.clear_fields(frm, cdt, cdn);
			return;
		}
		if (!frm.doc.company) terminal_framework.throw(__("Please select Company first"));
		if (!frm.doc.posting_date) terminal_framework.throw(__("Please select Posting Date first"));

		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
			args: {
				account: row.account,
			...
			callback: ({ message }) => {
				if (!message) return;
				$.extend(row, message);
				terminal_erp.journal_entry.set_amount_on_last_row(frm, cdt, cdn);
				terminal_erp.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	...
	set_amount_on_last_row(frm, cdt, cdn) {
		...
		terminal_erp.journal_entry.set_balancing_amount(row, difference);
	},
	...
	setup_queries(frm) {
		...
		frm.set_query("project", "accounts", (doc, cdt, cdn) => {
			...
			return { query: "terminal_erp.controllers.queries.get_project_name", filters };
		});

		frm.set_query("account", "accounts", () => terminal_erp.journal_entry.account_query(frm));

		frm.set_query("party_type", "accounts", (doc, cdt, cdn) => {
			return {
				query: "terminal_erp.setup.doctype.party_type.party_type.get_party_type",
				filters: { account: terminal_framework.get_doc(cdt, cdn).account },
			};
		});

		frm.set_query("reference_name", "accounts", (doc, cdt, cdn) => {
			return terminal_erp.journal_entry.reference_name_query(terminal_framework.get_doc(cdt, cdn));
		});
	},

	reference_name_query(row) {
		if (row.reference_type === "Journal Entry") {
			terminal_framework.model.validate_missing(row, "account");
			return {
				query: "terminal_erp.accounts.doctype.journal_entry.journal_entry.get_against_jv",
		...
		if (["Sales Order", "Purchase Order"].includes(row.reference_type)) {
			terminal_framework.model.validate_missing(row, "party_type");
			terminal_framework.model.validate_missing(row, "party");
	...
	update_jv_details(frm, rows) {
		rows.forEach((source) => {
			const row = terminal_framework.model.add_child(frm.doc, "Journal Entry Account", "accounts");
			const {
				idx,
				name,
				owner,
			...
			terminal_framework.model.set_value(row.doctype, row.name, fields);
		});
		frm.refresh_field("accounts");
		terminal_erp.journal_entry.update_totals(frm);
	},
