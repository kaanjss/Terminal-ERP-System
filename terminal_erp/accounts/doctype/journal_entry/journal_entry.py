...
from terminal_framework import _, msgprint, scrub
from terminal_framework.core.doctype.submission_queue.submission_queue import queue_submission
from terminal_framework.model.document import Document
from terminal_framework.query_builder.functions import Sum
from terminal_framework.utils import comma_and, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate
...
# terminal_erp.accounts.doctype.journal_entry.journal_entry.<fn> keep working.
from terminal_erp.accounts.doctype.journal_entry.mapper import (
...
from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
from terminal_erp.accounts.party import get_party_account
from terminal_erp.accounts.services.gl_validator import validate_opening_entry_against_pcv
...
from terminal_erp.controllers.accounts_controller import AccountsController
from terminal_erp.setup.utils import get_exchange_rate as _get_exchange_rate


class StockAccountInvalidTransaction(terminal_framework.ValidationError):
	pass
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate(self):
		...
		from terminal_erp.accounts.doctype.journal_entry.services.asset_service import AssetService
		from terminal_erp.accounts.doctype.journal_entry.services.reference_validator import (
		...
		if not terminal_framework.flags.is_reverse_depr_entry:
	...
	def validate_advance_accounts(self):
		...
		advance_accounts.add(
			terminal_framework.get_cached_value("Company", self.company, "default_advance_received_account")
		)
		advance_accounts.add(terminal_framework.get_cached_value("Company", self.company, "default_advance_paid_account"))
		if advance_accounts_used := journal_accounts & advance_accounts:
			terminal_framework.msgprint(
				...
				).format(terminal_framework.bold(comma_and(advance_accounts_used)))
			)
	...
	def before_submit(self):
		...
		if not terminal_framework.flags.in_import:
	...
	def get_balance_for_periodic_accounting(self) -> None:
		...
		for account in self.get_stock_accounts_for_periodic_accounting():
			...
			if difference_value == 0:
				terminal_framework.msgprint(
					_("No difference found for stock account {0}").format(terminal_framework.bold(account)),
					alert=True,
				)
				continue
	...
	def validate_company_for_periodic_accounting(self):
		if terminal_erp.is_perpetual_inventory_enabled(self.company):
		...
		if not self.periodic_entry_difference_account:
			terminal_framework.throw(_("Please select Periodic Accounting Entry Difference Account"))
	...
	def get_stock_accounts_for_periodic_accounting(self):
		...
		if not self.stock_asset_account:
			terminal_framework.throw(_("Please select Stock Asset Account"))
	...
	def validate_inter_company_accounts(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			doc = terminal_framework.db.get_value(
				"Journal Entry",
				self.inter_company_journal_entry_reference,
				["company", "total_debit", "total_credit"],
				as_dict=True,
			)
			account_currency = terminal_framework.get_cached_value("Company", self.company, "default_currency")
			previous_account_currency = terminal_framework.get_cached_value("Company", doc.company, "default_currency")
			if account_currency == previous_account_currency:
				...
				if (flt(self.total_credit, credit_precision) != flt(doc.total_debit, debit_precision)) or (
					...
					terminal_framework.throw(_("Total Credit/ Debit Amount should be same as linked Journal Entry"))

	def validate_stock_accounts(self):
		if (
			not terminal_erp.is_perpetual_inventory_enabled(self.company)
	...
	def update_invoice_discounting(self) -> None:
		...
		for name in discounting_names:
			inv_disc = terminal_framework.get_doc("Invoice Discounting", name)
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = terminal_framework.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			terminal_framework.db.set_value("Journal Entry", self.name, "inter_company_journal_entry_reference", "")

	def validate_party(self):
		for d in self.get("accounts"):
			account_type = terminal_framework.get_cached_value("Account", d.account, "account_type")
			...
			if account_type in ["Receivable", "Payable"]:
				if (
					...
					d.party_type
					and terminal_framework.db.get_value("Party Type", d.party_type, "account_type") != account_type
					and d.party_type
	...
	def check_credit_limit(self):
		...
		if customers:
			...
			customer_details = terminal_framework._dict(
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						terminal_framework.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == "Supplier" and flt(d.credit) > 0:
						terminal_framework.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))
	...
	def _validate_jv_reference(self, row) -> None:
		...
		if row.reference_name == self.name:
			terminal_framework.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))
	...
	def _validate_jv_reference_direction(self, row) -> None:
		...
		account_root_type = terminal_framework.get_cached_value("Account", row.account, "root_type")
	...
	def set_against_account(self):
		...
		if self.voucher_type in ("Deferred Revenue", "Deferred Expense"):
			for d in self.get("accounts"):
				...
				d.against_account = terminal_framework.db.get_value(d.reference_type, d.reference_name, field)
		else:
			for d in self.get("accounts"):
				if flt(d.debit) > 0:
		...
		if not (self.voucher_type == "Exchange Gain Or Loss" and self.multi_currency):
			for d in self.get("accounts"):
				if not flt(d.debit) and not flt(d.credit):
					terminal_framework.throw(_("Row {0}: Both Debit and Credit values cannot be zero").format(d.idx))
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				terminal_framework.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		alternate_currency = []
		for d in self.get("accounts"):
			account = terminal_framework.get_cached_value(
		...
		if alternate_currency:
			if not self.multi_currency:
				terminal_framework.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def set_exchange_rate(self) -> None:
		...
		for row in self.get("accounts"):
			...
			if not row.exchange_rate:
				terminal_framework.throw(_("Row {0}: Exchange Rate is mandatory").format(row.idx))
	...
	def _get_cheque_remark(self) -> str | None:
		...
		if not self.cheque_date:
			msgprint(_("Please enter Reference date"), raise_exception=terminal_framework.MandatoryError)
	...
	def _get_bill_remark(self, row) -> str | None:
		...
		bill_no, bill_date = terminal_framework.db.get_value(
	...
	def set_print_format_fields(self) -> None:
		...
		if amounts.party_type and amounts.pay_to_recd_from:
			self.pay_to_recd_from = terminal_framework.db.get_value(
				amounts.party_type,
	...
	def _get_party_and_bank_amounts(self) -> terminal_framework._dict:
		"""Sum the party and bank/cash amounts, with their currencies, across the account rows."""
		totals = terminal_framework._dict(
			bank_amount=0.0,
			party_amount=0.0,
		...
		for row in self.get("accounts"):
			...
			elif terminal_framework.get_cached_value("Account", row.account, "account_type") in ["Bank", "Cash"]:
	...
	def build_gl_map(self):
		from terminal_erp.accounts.doctype.journal_entry.services.gl_composer import JournalEntryGLComposer

		return JournalEntryGLComposer(self).compose()

	def make_gl_entries(self, cancel: int = 0, adv_adj: int = 0) -> None:
		from terminal_erp.accounts.general_ledger import make_gl_entries
		...
		if gl_map:
			...
			terminal_framework.flags.party_not_required = False
			if cancel:
		...
		if not blank_row:
			blank_row = self.append(
				"accounts",
				{
					...
					"cost_center": terminal_erp.get_default_cost_center(self.company),
				},
			)
	...
	def get_values(self):
		...
		query = (
			terminal_framework.qb.from_(invoice)
			.select(
				invoice.name,
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if terminal_framework.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				terminal_framework.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))
	...
	def validate_empty_accounts_table(self):
		if not self.get("accounts"):
			terminal_framework.throw(_("Accounts table cannot be blank."))
	...
	*,
	...
	if not account:
		...
		if account_type == "Bank":
			account = terminal_framework.get_cached_value("Company", company, "default_bank_account")
			if not account:
				account_list = terminal_framework.get_all(
			...
			account = terminal_framework.get_cached_value("Company", company, "default_cash_account")
			if not account:
				account_list = terminal_framework.get_all(
	...
	if account:
		...
		return terminal_framework._dict(result)
	else:
...
def get_against_jv(
	...
	if not terminal_framework.db.has_column("Journal Entry", searchfield):
		return []

	JournalEntry = terminal_framework.qb.DocType("Journal Entry")
	JournalEntryAccount = terminal_framework.qb.DocType("Journal Entry Account")

	query = (
		terminal_framework.qb.from_(JournalEntry)
		...
		.orderby(JournalEntry.name, order=terminal_framework.qb.desc)
		.limit(page_len)
		.offset(start)
	)
...
def get_outstanding(
	...
	**kwargs,
	...
	"""
	if not terminal_framework.has_permission("Account"):
		terminal_framework.msgprint(_("No Permission"), raise_exception=1)
...
def _get_invoice_outstanding(doctype: str, docname: str, company: str, account_currency: str | None) -> dict:
	...
	invoice = terminal_framework.db.get_value(
		doctype,
		docname,
	...
	company_currency = terminal_erp.get_company_currency(company)
...
def get_party_account_and_currency(company: str, party_type: str, party: str) -> dict:
	...
	return {
		"account": account,
		"account_currency": terminal_framework.get_cached_value("Account", account, "account_currency"),
	}
...
def get_account_details_and_party_type(
	...
	grid_values = {
		...
		"bank_account": (
			terminal_framework.db.get_value("Bank Account", {"account": account, "company": company}) or None
		),
...
def get_exchange_rate(
	...
	if not account_details:
		terminal_framework.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = terminal_framework.db.get_value(reference_type, reference_name, "conversion_rate")
