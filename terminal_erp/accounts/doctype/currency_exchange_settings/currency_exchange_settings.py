...
class CurrencyExchangeSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.currency_exchange_settings_details.currency_exchange_settings_details import (
			CurrencyExchangeSettingsDetails,
		)
		from terminal_erp.accounts.doctype.currency_exchange_settings_result.currency_exchange_settings_result import (
	...
	def validate(self):
		...
		if terminal_framework.in_test or terminal_framework.flags.in_install or terminal_framework.flags.in_setup_wizard:
			return
			...
			if not self.access_key:
				terminal_framework.throw(
					_("Access Key is required for Service Provider: {0}").format(
						terminal_framework.bold(self.service_provider)
					)
				)
	...
	def validate_parameters(self):
		...
		try:
			...
			terminal_framework.throw(_("Error: {0}").format(str(e)))
	...
	def validate_result(self, response, value):
		try:
			...
			terminal_framework.throw(_("Invalid result key. Response:") + " " + response.text)
		if not isinstance(value, int | float):
			terminal_framework.throw(_("Returned exchange rate is neither integer not float."))
