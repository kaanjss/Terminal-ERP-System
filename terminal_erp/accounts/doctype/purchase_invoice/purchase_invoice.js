...
terminal_erp.accounts.payment_triggers.setup("Purchase Invoice");
terminal_erp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
terminal_erp.accounts.taxes.setup_tax_validations("Purchase Invoice");
terminal_erp.buying.setup_buying_controller();

terminal_erp.accounts.PurchaseInvoice = class PurchaseInvoice extends terminal_erp.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if (
			...
			!cint(doc.is_return) &&
			!doc.on_hold &&
			terminal_framework.boot.user.in_create.includes("Payment Request")
		) {
			this.frm.add_custom_button(
				__("Payment Request"),
				function () {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			terminal_framework.model.with_doc("Supplier", me.frm.doc.supplier, function () {
				var supplier = terminal_framework.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice() {
		...
		terminal_framework.call({
			method: "terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	can_change_release_date(date) {
		const diff = terminal_framework.datetime.get_diff(date, terminal_framework.datetime.nowdate());
		if (diff < 0) {
			terminal_framework.throw(__("New release date should be in the future"));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.accounts.doctype.purchase_invoice.mapper.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	on_submit() {
		...
		$.each(this.frm.doc["items"] || [], function (i, row) {
			if (row.purchase_receipt) terminal_framework.model.clear_doc("Purchase Receipt", row.purchase_receipt);
		});
	}

	make_debit_note() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.accounts.doctype.purchase_invoice.mapper.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(terminal_erp.accounts.PurchaseInvoice);
...
terminal_framework.ui.form.on("Purchase Invoice", {
	setup: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.per_received > 0) {
			frm.add_custom_button(
				...
				() => {
					...
					terminal_framework.set_route("List", "Purchase Receipt", "List");
				},
				__("View")
			);
		}
