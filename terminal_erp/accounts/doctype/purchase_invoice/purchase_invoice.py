...
from terminal_framework.utils import cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate

import terminal_erp
from terminal_erp.accounts.deferred_revenue import validate_service_stop_date
from terminal_erp.accounts.doctype.purchase_invoice.services.billing_status import BillingStatusService
from terminal_erp.accounts.doctype.purchase_invoice.services.expense_account import ExpenseAccountService
...
from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import PurchaseTaxWithholding
...
from terminal_erp.assets.doctype.asset.asset import is_cwip_accounting_enabled
from terminal_erp.controllers.buying_controller import BuyingController


class WarehouseMissingError(terminal_framework.ValidationError):
	pass
...
class PurchaseInvoice(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.purchase_invoice_advance.purchase_invoice_advance import (
			PurchaseInvoiceAdvance,
		)
		from terminal_erp.accounts.doctype.purchase_invoice_item.purchase_invoice_item import PurchaseInvoiceItem
		from terminal_erp.accounts.doctype.purchase_taxes_and_charges.purchase_taxes_and_charges import (
			PurchaseTaxesandCharges,
		)
		from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import TaxWithholdingEntry
		from terminal_erp.buying.doctype.purchase_receipt_item_supplied.purchase_receipt_item_supplied import (
	...
	def validate_release_date(self):
		if self.release_date and getdate(nowdate()) >= getdate(self.release_date):
			terminal_framework.throw(_("Release date must be in the future"))

	def validate_cash(self):
		if not self.cash_bank_account and flt(self.paid_amount):
			terminal_framework.throw(_("Cash or Bank Account is mandatory for making payment entry"))
		...
		if flt(self.paid_amount) + flt(self.write_off_amount) - flt(
			...
			terminal_framework.throw(_("""Paid amount + Write Off Amount can not be greater than Grand Total"""))
	...
	def validate_credit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(_("Credit To"))),
		...
		if self.supplier and account.account_type != "Payable":
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(_("Credit To")), terminal_framework.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(terminal_framework.db.get_single_value("Buying Settings", "maintain_same_rate"))
	...
	def validate_item_code(self):
		for d in self.get("items"):
			if not d.item_code:
				terminal_framework.msgprint(_("Item Code required at Row No {0}").format(d.idx), raise_exception=True)
	...
	def po_required(self):
		if (
			terminal_framework.db.get_single_value("Buying Settings", "po_required") == "Yes"
			and not self.is_internal_transfer()
			and not terminal_framework.get_value(
				"Supplier", self.supplier, "allow_purchase_invoice_creation_without_purchase_order"
			)
		):
			for d in self.get("items"):
				if not d.purchase_order:
					msg = _("Purchase Order Required for item {0}").format(terminal_framework.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase order please set {0} as {1} in {2}"
					).format(
						terminal_framework.bold(_("Purchase Order Required")),
						terminal_framework.bold(_("No")),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
					throw(msg, title=_("Mandatory Purchase Order"))

	def pr_required(self):
		if terminal_framework.db.get_single_value("Buying Settings", "pr_required") == "Yes":
			stock_and_asset_items = self.get_stock_items()
			stock_and_asset_items.extend(self.get_asset_items())
			if terminal_framework.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_and_asset_items:
					msg = _("Purchase Receipt Required for item {0}").format(terminal_framework.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase receipt please set {0} as {1} in {2}"
					).format(
						terminal_framework.bold(_("Purchase Receipt Required")),
						terminal_framework.bold(_("No")),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.purchase_order:
				submitted = terminal_framework.db.exists("Purchase Order", {"docstatus": 1, "name": d.purchase_order})
				if not submitted:
					terminal_framework.throw(_("Purchase Order {0} is not submitted").format(d.purchase_order))
			if d.purchase_receipt:
				submitted = terminal_framework.db.exists("Purchase Receipt", {"docstatus": 1, "name": d.purchase_receipt})
				if not submitted:
					terminal_framework.throw(_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt))
	...
	def on_submit(self):
		...
		if terminal_framework.db.get_single_value("Buying Settings", "project_update_frequency") == "Each Transaction":
			self.update_project()
	...
	def get_gl_entries(self, inventory_account_map=None):
		from terminal_erp.accounts.doctype.purchase_invoice.services.gl_composer import (
	...
	def check_asset_cwip_enabled(self):
		...
		for item in self.get("items"):
			if item.item_code and item.is_fixed_asset:
				asset_category = terminal_framework.get_cached_value("Item", item.item_code, "asset_category")
	...
	def update_project(self):
		projects = terminal_framework._dict()
		for d in self.items:
			if d.project:
				if self.docstatus == 1:
		...
		pj = terminal_framework.qb.DocType("Project")
		for proj, value in projects.items():
			res = terminal_framework.qb.from_(pj).select(pj.total_purchase_cost).where(pj.name == proj).for_update().run()
			current_purchase_cost = res and res[0][0] or 0
			# terminal_framework.db.set_value("Project", proj, "total_purchase_cost", current_purchase_cost + value)
			project_doc = terminal_framework.get_lazy_doc("Project", proj)
	...
	def validate_supplier_invoice(self):
		if self.bill_no:
			if cint(terminal_framework.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				fiscal_year = get_fiscal_year(self.posting_date, company=self.company, as_dict=True)

				pi = terminal_framework.get_all(
					"Purchase Invoice",
					filters={
	...
	def set_status(self, update=False, status=None, update_modified=True):
		...
		if not status:
			if self.docstatus == 2:
				...
				elif self.is_return == 0 and terminal_framework.db.get_value(
...
def get_purchase_document_details(doc):
	...
	exchange_rate_map = terminal_framework._dict(
	...
	net_rate_map = terminal_framework._dict(
		terminal_framework.get_all(child_doctype, filters={"name": ("in", items)}, fields=["name", "net_rate"], as_list=1)
	)
...
def change_release_date(name: str, release_date: str | None = None):
	pi = terminal_framework.get_lazy_doc("Purchase Invoice", name)
	pi.check_permission()
...
def unblock_invoice(name: str):
	if terminal_framework.db.exists("Purchase Invoice", name):
		pi = terminal_framework.get_lazy_doc("Purchase Invoice", name)
		pi.unblock_invoice()
