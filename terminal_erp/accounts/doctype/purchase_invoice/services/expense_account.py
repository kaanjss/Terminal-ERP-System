...
class ExpenseAccountService:
	...
	def set_expense_account(self, for_validate: bool = False) -> None:
		doc = self.doc
		auto_accounting_for_stock = terminal_erp.is_perpetual_inventory_enabled(doc.company)
		...
		for item in doc.get("items"):
			...
			if (
				...
				and (
					not item.po_detail
					or not terminal_framework.db.get_value("Purchase Order Item", item.po_detail, "delivered_by_supplier")
				)
			):
				if doc.update_stock and item.warehouse and (not item.from_warehouse):
					...
					if item.purchase_receipt:
						negative_expense_booked_in_pr = terminal_framework.db.exists(
							"GL Entry",
							{
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx,
									terminal_framework.bold(stock_not_billed_account),
									terminal_framework.bold(item.purchase_receipt),
								)
								terminal_framework.msgprint(msg, title=_("Expense Head Changed"))
						...
						if (
							...
							msg = _(
								...
								item.idx, terminal_framework.bold(stock_not_billed_account), terminal_framework.bold(item.item_code)
							)
							msg += "<br>"
							msg += _(
				...
				if not item.pr_detail and item.po_detail:
					receipt_item = terminal_framework.get_cached_value(
				...
				if item.pr_detail:
					...
					arbnb_booked_in_pr = terminal_framework.db.get_value(
						"GL Entry",
						{
							"voucher_type": "Purchase Receipt",
							"voucher_no": item.purchase_receipt,
	...
	def force_set_against_expense_account(self) -> None:
		...
		terminal_framework.db.set_value(doc.doctype, doc.name, "against_expense_account", doc.against_expense_account)
