...
class PurchaseInvoiceGLComposer(BaseGLComposer):
	...
	def compose(self, inventory_account_map=None):
		from terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice import make_regional_gl_entries
		from terminal_erp.accounts.general_ledger import merge_similar_entries

		doc = self.doc
		doc.auto_accounting_for_stock = terminal_erp.is_perpetual_inventory_enabled(doc.company)
	...
	def make_item_gl_entries(self, gl_entries):
		from terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice import (
		...
		if doc.update_stock:
			stock_ledger_entries = terminal_framework.get_all(
				"Stock Ledger Entry",
		...
		for item in doc.get("items"):
			if flt(item.base_net_amount) or (doc.get("update_stock") and item.valuation_rate):
				if item.item_code:
					terminal_framework.get_cached_value("Item", item.item_code, "asset_category")
			...
			if (
				...
				if item.purchase_receipt and valuation_tax_accounts:
					negative_expense_booked_in_pr = terminal_framework.get_all(
						"GL Entry",
						filters={
	...
	def get_provisional_accounts(self):
		doc = self.doc
		self.provisional_accounts = terminal_framework._dict()
		...
		provisional_gl_entries = terminal_framework.get_all(
			"GL Entry",
			filters={
				"voucher_type": "Purchase Receipt",
	...
	def make_provisional_gl_entry(self, gl_entries, item):
		if item.purchase_receipt:
			...
			if pr_item.get("has_provisional_entry"):
				purchase_receipt_doc = terminal_framework.get_cached_doc("Purchase Receipt", item.purchase_receipt)
	...
	def make_standard_cost_srbnb_split(
		...
		can't be resolved."""
		from terminal_erp.stock.doctype.item_standard_cost.item_standard_cost import (
	...
	def get_pr_stock_value(self, item):
		...
		pr_detail = terminal_framework.db.get_value(
						...
						self.get_gl_dict(
							{
								...
								"credit_in_transaction_currency": flt(
									...
									terminal_framework.get_precision("Purchase Invoice Item", "item_tax_amount"),
								),
								"remarks": doc.remarks or _("Accounting Entry for Stock"),
							},
							item=tax,
