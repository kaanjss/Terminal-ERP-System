...
from terminal_framework.utils import add_days, cint, flt, getdate, nowdate, today
...
from terminal_erp.buying.doctype.purchase_order.mapper import get_mapped_purchase_invoice
from terminal_erp.buying.doctype.purchase_order.mapper import make_purchase_invoice as make_pi_from_po
...
from terminal_erp.controllers.accounts_controller import InvalidQtyError, get_payment_terms
from terminal_erp.controllers.buying_controller import QtyMismatchError
from terminal_erp.exceptions import InvalidCurrency
...
from terminal_erp.stock.doctype.material_request.test_material_request import make_material_request
from terminal_erp.stock.doctype.purchase_receipt.mapper import (
	make_purchase_invoice as create_purchase_invoice_from_receipt,
)
from terminal_erp.stock.doctype.purchase_receipt.test_purchase_receipt import (
	get_taxes,
...
from terminal_erp.stock.doctype.stock_entry.test_stock_entry import get_qty_after_transaction
from terminal_erp.stock.tests.test_utils import StockTestMixin
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestPurchaseInvoice(Terminal ERPTestSuite, StockTestMixin):
	def setUp(self):
	...
	def test_update_received_qty_in_material_request(self):
		from terminal_erp.buying.doctype.purchase_order.mapper import make_purchase_invoice
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		pi = terminal_framework.copy_doc(self.globalTestRecords["Purchase Invoice"][0])
		self.assertTrue(not cint(terminal_erp.is_perpetual_inventory_enabled(pi.company)))
		pi.insert()
		pi.submit()
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		self.assertTrue(cint(terminal_erp.is_perpetual_inventory_enabled(pi.company)), 1)

		self.check_gle_for_pi(pi.name)

	def test_terms_added_after_save(self):
		pi = terminal_framework.copy_doc(self.globalTestRecords["Purchase Invoice"][1])
		pi.insert()
	...
	def test_payment_entry_unlink_against_purchase_invoice(self):
		...
		pi_doc = terminal_framework.get_doc("Purchase Invoice", pi_doc.name)
		pi_doc.load_from_db()
		self.assertTrue(pi_doc.status, "Paid")

		self.assertRaises(terminal_framework.LinkExistsError, pi_doc.cancel)
		unlink_payment_on_cancel_of_invoice()
	...
	def test_purchase_invoice_for_blocked_supplier(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_blocked_invoice_must_be_in_future(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pi.save)
		pi.release_date = ""
		pi.save()
	...
	def test_purchase_invoice_temporary_blocked(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pe.save)
	...
	def test_full_actual_charge_capitalized_on_stock_items_only(self):
		...
		from terminal_erp.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
		...
		pi = terminal_framework.new_doc("Purchase Invoice")
		pi.company = company
	...
	def test_purchase_invoice_with_exchange_rate_difference(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
		...
		exchange_gain_loss_account = terminal_framework.db.get_value("Company", pi.company, "exchange_gain_loss_account")
	...
	def test_purchase_invoice_change_naming_series(self):
		...
		self.assertRaises(terminal_framework.CannotChangeConstantError, pi.save)
	...
	def test_purchase_invoice_with_advance(self):
		jv = terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][1])
		jv.insert()
		jv.submit()
	...
	def test_total_purchase_cost_for_project(self):
		if not terminal_framework.db.exists("Project", {"project_name": "_Test Project for Purchase"}):
			project = make_project({"project_name": "_Test Project for Purchase"})
		else:
			project = terminal_framework.get_doc("Project", {"project_name": "_Test Project for Purchase"})

		existing_purchase_cost = terminal_framework.get_all(
		...
		self.assertEqual(
			terminal_framework.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(
			terminal_framework.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost
		)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		pi = make_purchase_invoice(
			update_stock=1,
			posting_date=terminal_framework.utils.nowdate(),
			posting_time=terminal_framework.utils.nowtime(),
	...
	def test_auto_batch(self):
		item_code = terminal_framework.db.get_value("Item", {"has_batch_no": 1, "create_new_batch": 1}, "name")
		...
		self.assertTrue(terminal_framework.db.get_value("Batch", {"item": item_code, "reference_name": pi.name}))

	def test_update_stock_and_purchase_return(self):
		actual_qty_0 = get_qty_after_transaction()

		pi = make_purchase_invoice(
			update_stock=1, posting_date=terminal_framework.utils.nowdate(), posting_time=terminal_framework.utils.nowtime()
		)
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(
			terminal_framework.db.get_value("Serial No", rejected_serial_no, "warehouse"),
	...
	def test_outstanding_amount_after_advance_payment_entry_cancelation(self):
		...
		# added to avoid Document has been modified exception
		pe = terminal_framework.get_doc("Payment Entry", pe.name)
		pe.cancel()
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pi.insert)

	def test_debit_note(self):
		from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from terminal_erp.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
		...
		pi_doc = terminal_framework.get_doc("Purchase Invoice", pi.name)
	...
	def test_gain_loss_with_advance_entry(self):
		unlink_enabled = terminal_framework.db.get_single_value(
			"Accounts Settings", "unlink_payment_on_cancellation_of_invoice"
		)

		terminal_framework.db.set_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice", 1)

		original_account = terminal_framework.db.get_value("Company", "_Test Company", "exchange_gain_loss_account")
		terminal_framework.db.set_value(
			"Company", "_Test Company", "exchange_gain_loss_account", "Exchange Gain/Loss - _TC"
		)

		pay = terminal_framework.get_doc(
			{
				"doctype": "Payment Entry",
				"company": "_Test Company",
				"payment_type": "Pay",
		...
		total_debit_amount = terminal_framework.db.get_all(
			"Journal Entry Account",
		...
		jea_parent = terminal_framework.db.get_all(
			"Journal Entry Account",
			filters={
		...
		self.assertEqual(
			terminal_framework.db.get_value("Journal Entry", jea_parent.parent, "voucher_type"), "Exchange Gain Or Loss"
		)
		...
		jea_parent_2 = terminal_framework.db.get_all(
			"Journal Entry Account",
			filters={
				"account": creditors_account,
				"docstatus": 1,
		...
		self.assertEqual(
			terminal_framework.db.get_value("Journal Entry", jea_parent_2.parent, "voucher_type"),
		...
		self.assertEqual(terminal_framework.db.get_value("Journal Entry", jea_parent.parent, "docstatus"), 2)

		pi_2.reload()
		pi_2.cancel()

		self.assertEqual(terminal_framework.db.get_value("Journal Entry", jea_parent_2.parent, "docstatus"), 2)
		...
		terminal_framework.db.set_value("Company", "_Test Company", "exchange_gain_loss_account", original_account)

	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"unlink_payment_on_cancellation_of_invoice": 1})
	def test_purchase_invoice_advance_taxes(self):
		from terminal_erp.accounts.doctype.payment_entry.payment_entry import get_payment_entry

		terminal_framework.db.set_single_value("Accounts Settings", "merge_similar_account_heads", 1)
		...
		tds_account_args = {
			...
			"account_type": "Tax",
			"parent_account": terminal_framework.db.get_value(
	...
	def test_provisional_accounting_entry_for_over_billing(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "maintain_same_rate", 0)

		# Configure Accounts Settings to allow 300% over billing
		terminal_framework.db.set_single_value("Accounts Settings", "over_billing_allowance", 300)
	...
	def test_adjust_incoming_rate(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "maintain_same_rate", 1)
	...
	def test_batch_expiry_for_purchase_invoice(self):
		...
		terminal_framework.db.set_value("Batch", batch_no, "expiry_date", add_days(nowdate(), -1))
	...
	def test_offsetting_entries_for_accounting_dimensions(self):
		...
		dim = terminal_framework.get_doc("Accounting Dimension", "Branch")
		dim.append(
			"dimension_defaults",
			{
				"company": "_Test Company",
		...
		branch1 = terminal_framework.new_doc("Branch")
		branch1.branch = "Location 1"
		branch1.insert(ignore_if_duplicate=True)
		branch2 = terminal_framework.new_doc("Branch")
	...
	@Terminal ERPTestSuite.change_settings("Buying Settings", {"supplier_group": None})
	def test_purchase_invoice_without_supplier_group(self):
		# Create a Supplier
		test_supplier_name = "_Test Supplier Without Supplier Group"
		if not terminal_framework.db.exists("Supplier", test_supplier_name):
	...
	def test_debit_note_with_account_mismatch(self):
		...
		self.assertRaises(terminal_framework.ValidationError, dr_note.save)
	...
	def test_make_pr_and_pi_from_po(self):
		...
		pr_gl_entries = terminal_framework.get_all(
			"GL Entry",
		...
		pi_gl_entries = terminal_framework.get_all(
			"GL Entry",
			filters={"voucher_type": "Purchase Invoice", "voucher_no": pi.name},
			fields=["account", "debit", "credit"],
			order_by="account asc",
	...
	def test_opening_invoice_rounding_adjustment_validation(self):
		...
		pi.is_opening = "Yes"
		pi.save()
		self.assertRaises(terminal_framework.ValidationError, pi.submit)
	...
	def test_pr_pi_over_billing(self):
		...
		terminal_framework.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
		terminal_framework.db.set_single_value("Accounts Settings", "over_billing_allowance", 20)
		pi.cancel()
		pi = make_purchase_invoice_from_pr(pr.name)
		pi.items[0].rate = 12
...
def toggle_provisional_accounting_setting(**args):
	args = terminal_framework._dict(args)
	company = terminal_framework.get_doc("Company", args.company or "_Test Company")
