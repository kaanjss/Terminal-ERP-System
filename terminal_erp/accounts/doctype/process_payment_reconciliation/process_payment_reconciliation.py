...
class ProcessPaymentReconciliation(Document):
	...
	def validate_receivable_payable_account(self):
		if self.receivable_payable_account:
			if self.company != terminal_framework.db.get_value("Account", self.receivable_payable_account, "company"):
				terminal_framework.throw(
					_("Receivable/Payable Account: {0} doesn't belong to company {1}").format(
						terminal_framework.bold(self.receivable_payable_account), terminal_framework.bold(self.company)
					)
				)

	def validate_bank_cash_account(self):
		if self.bank_cash_account:
			if self.company != terminal_framework.db.get_value("Account", self.bank_cash_account, "company"):
				terminal_framework.throw(
					_("Bank/Cash Account {0} doesn't belong to company {1}").format(
						terminal_framework.bold(self.bank_cash_account), terminal_framework.bold(self.company)
					)
				)
	...
	def on_cancel(self):
		self.db_set("status", "Cancelled")
		log = terminal_framework.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": self.name})
		if log:
			terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "status", "Cancelled")
...
def get_reconciled_count(docname: str | None = None) -> float:
	current_status = {}
	if docname:
		reconcile_log = terminal_framework.db.get_value(
...
def get_pr_instance(doc: str):
	process_payment_reconciliation = terminal_framework.get_doc("Process Payment Reconciliation", doc)
...
def is_job_running(job_name: str) -> bool:
	jobs = terminal_framework.db.get_all("RQ Job", filters={"status": ["in", ["started", "queued"]]})
	for x in jobs:
...
def pause_job_for_doc(docname: str | None = None):
	if docname:
		terminal_framework.has_permission("Process Payment Reconciliation", "write", doc=docname, throw=True)
		terminal_framework.db.set_value("Process Payment Reconciliation", docname, "status", "Paused")
		log = terminal_framework.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": docname})
		if log:
			terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "status", "Paused")
...
def trigger_job_for_doc(docname: str | None = None):
	...
	if not terminal_framework.get_single_value("Accounts Settings", "auto_reconcile_payments"):
		terminal_framework.throw(
	...
	if not is_scheduler_inactive():
		if terminal_framework.db.get_value("Process Payment Reconciliation", docname, "status") == "Queued":
			terminal_framework.db.set_value("Process Payment Reconciliation", docname, "status", "Running")
			job_name = f"start_processing_{docname}"
			if not is_job_running(job_name):
				terminal_framework.enqueue(
					method="terminal_erp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.reconcile_based_on_filters",
					queue="long",
					is_async=True,
					job_name=job_name,
		...
		elif terminal_framework.db.get_value("Process Payment Reconciliation", docname, "status") == "Paused":
			terminal_framework.db.set_value("Process Payment Reconciliation", docname, "status", "Running")
			log = terminal_framework.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": docname})
			if log:
				terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "status", "Running")
		...
		terminal_framework.msgprint(_("Scheduler is Inactive. Can't trigger job now."))
...
def trigger_reconciliation_for_queued_docs():
	...
	if not is_scheduler_inactive():
		...
		all_queued = terminal_framework.db.get_all(
		...
		docs_to_trigger = []
		unique_filters = set()
		queue_size = terminal_framework.get_single_value("Accounts Settings", "reconciliation_queue_size") or 5
		...
		for x in all_queued:
			doc = terminal_framework.get_doc("Process Payment Reconciliation", x)
		...
		terminal_framework.msgprint(_("Scheduler is Inactive. Can't trigger jobs now."))
...
def reconcile_based_on_filters(doc: None | str = None) -> None:
	...
	"""
	if doc:
		log = terminal_framework.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": doc})
		if not log:
			log = terminal_framework.new_doc("Process Payment Reconciliation Log")
			log.process_pr = doc
			log.status = "Running"
			log = log.save()
			...
			if not is_job_running(job_name):
				terminal_framework.enqueue(
					method="terminal_erp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.fetch_and_allocate",
					queue="long",
					timeout="3600",
					is_async=True,
					job_name=job_name,
			...
			if not allocated:
				...
				if not is_job_running(reconcile_job_name):
					terminal_framework.enqueue(
						method="terminal_erp.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.reconcile",
						queue="long",
						timeout="3600",
						is_async=True,
				...
				terminal_framework.db.set_value("Process Payment Reconciliation", doc, "status", "Completed")


def get_next_allocation(log: str) -> list:
	if log:
		allocations = []
		next = terminal_framework.db.get_all(
		...
		if next:
			allocations = terminal_framework.db.get_all(
				"Process Payment Reconciliation Log Allocations",
				filters={
					"parent": log,
					"reconciled": 0,
...
def fetch_and_allocate(doc: str) -> None:
	...
	if doc:
		log = terminal_framework.db.get_value("Process Payment Reconciliation Log", filters={"process_pr": doc})
		if log:
			if not terminal_framework.db.get_value("Process Payment Reconciliation Log", log, "allocated"):
				reconcile_log = terminal_framework.get_doc("Process Payment Reconciliation Log", log)
			...
			if reconciled_entries != total_allocations:
				try:
					...
					if allocations[0].reference_type == "Payment Entry":
						...
						pe = terminal_framework.get_doc(allocations[0].reference_type, allocations[0].reference_name)
					...
					reconciled_count = terminal_framework.db.count(
						"Process Payment Reconciliation Log Allocations",
					...
					if traceback:
						...
						terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "error_log", message)
						terminal_framework.db.set_value(
							"Process Payment Reconciliation",
							doc,
							"error_log",
					...
					if reconciled_entries and total_allocations and reconciled_entries < total_allocations:
						...
						terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "status", "Failed")
						terminal_framework.db.set_value(
							"Process Payment Reconciliation",
							doc,
							"status",
					...
					if reconciled_entries == total_allocations:
						terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "status", "Reconciled")
						terminal_framework.db.set_value("Process Payment Reconciliation Log", log, "reconciled", 1)
						terminal_framework.db.set_value("Process Payment Reconciliation", doc, "status", "Completed")
					else:
						if terminal_framework.db.get_value("Process Payment Reconciliation", doc, "status") != "Paused":
...
def is_any_doc_running(for_filter: str | dict | None = None) -> str | None:
	running_doc = None
	if for_filter:
		for_filter = terminal_framework.parse_json(for_filter)

		running_doc = terminal_framework.db.get_value(
			"Process Payment Reconciliation",
			filters={
				"docstatus": 1,
