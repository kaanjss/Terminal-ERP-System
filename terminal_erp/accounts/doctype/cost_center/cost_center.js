...
terminal_framework.ui.form.on("Cost Center", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			frm.add_custom_button(__("Chart of Cost Centers"), function () {
				terminal_framework.set_route("Tree", "Cost Center");
			});

			frm.add_custom_button(__("Budget"), function () {
				terminal_framework.set_route("List", "Budget", { cost_center: frm.doc.name });
			});
		}
	},
		...
		var d = new terminal_framework.ui.Dialog({
			...
			primary_action: function () {
				...
				terminal_framework.call({
					method: "terminal_erp.accounts.utils.update_cost_center",
					args: {
					...
					callback: function (r) {
						...
						if (!r.exc) {
							if (r.message) {
								terminal_framework.set_route("Form", "Cost Center", r.message);
							} else {
	...
	parent_cost_center(frm) {
		if (!frm.doc.company) {
			terminal_framework.msgprint(__("Please enter company name first"));
		}
	},
