...
from terminal_framework.utils import flt, fmt_money

import terminal_erp


class OverlappingConditionError(terminal_framework.ValidationError):
	pass


class FromGreaterThanToError(terminal_framework.ValidationError):
	pass


class ManyBlankToValuesError(terminal_framework.ValidationError):
	pass
...
class ShippingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.shipping_rule_condition.shipping_rule_condition import (
			ShippingRuleCondition,
		)
		from terminal_erp.accounts.doctype.shipping_rule_country.shipping_rule_country import (
			ShippingRuleCountry,
		)
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if doc.meta.get_field("taxes").options != "Sales Taxes and Charges":
				terminal_framework.throw(_("Shipping rule only applicable for Selling"))
			...
			if doc.meta.get_field("taxes").options != "Purchase Taxes and Charges":
				terminal_framework.throw(_("Shipping rule only applicable for Buying"))
