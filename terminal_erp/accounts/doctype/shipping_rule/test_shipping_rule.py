...
from terminal_erp.accounts.doctype.shipping_rule.shipping_rule import (
...
class TestShippingRule(Terminal ERPTestSuite):
	def setUp(self):
		self.load_test_records("Shipping Rule")

	def test_from_greater_than_to(self):
		shipping_rule = terminal_framework.copy_doc(self.globalTestRecords["Shipping Rule"][0])
...
def create_shipping_rule(shipping_rule_type, shipping_rule_name):
	if terminal_framework.db.exists("Shipping Rule", shipping_rule_name):
		return terminal_framework.get_doc("Shipping Rule", shipping_rule_name)

	sr = terminal_framework.new_doc("Shipping Rule")
