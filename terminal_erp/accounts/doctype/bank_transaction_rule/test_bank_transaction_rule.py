...
class TestBankTransactionRule(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def _unique_rule_name(self, prefix: str) -> str:
		return f"{prefix}-{terminal_framework.generate_hash(length=8)}"
	...
	def _make_rule_doc(self, rule_name, description_rules, **fields):
		...
		return terminal_framework.get_doc(data)
