...
from terminal_framework.desk.form.linked_with import get_child_tables_of_doctypes
...
class RepostAccountingLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.repost_accounting_ledger_items.repost_accounting_ledger_items import (
		...
		if self.vouchers:
			...
			for vtype in self._allowed_types:
				if names := [x.voucher_no for x in self.vouchers if x.voucher_type == vtype]:
					latest_voucher = terminal_framework.db.get_all(
						vtype,
					...
					if latest_voucher and latest_pcv[0] >= latest_voucher:
						terminal_framework.throw(_("Cannot Resubmit Ledger entries for vouchers in Closed fiscal year."))
	...
	def get_existing_ledger_entries(self):
		...
		self.gles = terminal_framework._dict({})

		for gle in existing_gles:
			self.gles.setdefault((gle.voucher_type, gle.voucher_no), terminal_framework._dict({})).setdefault(
				"existing", []
			).append(gle.update({"old": True}))

	def generate_preview_data(self):
		terminal_framework.flags.through_repost_accounting_ledger = True
		self.gl_entries = []
		self.get_existing_ledger_entries()
		for x in self.vouchers:
			doc = terminal_framework.get_doc(x.voucher_type, x.voucher_no)
	...
	def generate_preview(self):
		from terminal_erp.accounts.report.general_ledger.general_ledger import get_columns as get_gl_columns

		if not self.vouchers:
			terminal_framework.msgprint(_("Add vouchers to generate preview."))
			return
		...
		rendered_page = terminal_framework.render_template(
			"terminal_erp/accounts/doctype/repost_accounting_ledger/repost_accounting_ledger.html",
	...
	def on_submit(self):
		if len(self.vouchers) > 5:
			...
			terminal_framework.enqueue(
				method="terminal_erp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger.start_repost",
				account_repost_doc=self.name,
				is_async=True,
				job_name=job_name,
				enqueue_after_commit=True,
			)
			terminal_framework.msgprint(_("Repost has started in the background"))
		else:
...
def start_repost(account_repost_doc: str | None = None) -> None:
	...
	if account_repost_doc:
		repost_doc = terminal_framework.get_doc("Repost Accounting Ledger", account_repost_doc)
		...
		if repost_doc.docstatus == 1:
			...
			for x in repost_doc.vouchers:
				...
				if doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
					...
					doc.make_gl_entries()
				elif doc.doctype in terminal_framework.get_hooks("repost_allowed_doctypes"):
...
def validate_docs_for_deferred_accounting(sales_docs, purchase_docs):
	docs_with_deferred_revenue = terminal_framework.db.get_all(
		"Sales Invoice Item",
	...
	docs_with_deferred_expense = terminal_framework.db.get_all(
	...
	if disallowed_types := voucher_types.difference(allowed_types):
		...
		terminal_framework.throw(
			_(
				...
				terminal_framework.bold(comma_and(list(disallowed_types))),
				message,
				terminal_framework.bold("Allowed Doctype"),
				terminal_framework.utils.get_link_to_form("Accounts Settings"),
			)
		)
...
def get_repost_allowed_types(
	...
	if allowed_types := terminal_framework.db.get_all(
		"Repost Allowed Types",
		filters=filters,
		fields=["document_type"],
		as_list=1,
