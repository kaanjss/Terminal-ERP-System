...
class TaxWithholdingEntry(Document):
	...
	def _adjust_against_old_entries(self, field_type: str) -> set:
		...
		# old entries
		old_entries = terminal_framework.get_all(
			DOCTYPE,
			filters={
		...
		for old_entry_data in old_entries:
			old_entry = terminal_framework.get_doc(DOCTYPE, **old_entry_data)
			...
			if old_amount <= amount_we_can_match:
				# complete adjustment
				terminal_framework.db.set_value(DOCTYPE, old_entry.name, values_to_update)
				...
				terminal_framework.db.set_value(DOCTYPE, old_entry.name, balance_values)
				...
				new_entry = terminal_framework.copy_doc(old_entry)
	...
	def _handle_return_invoice_cancellation(self, filters):
		...
		for entry in old_entries:
			...
			new_entry = terminal_framework.copy_doc(terminal_framework.get_doc(DOCTYPE, **entry))
			values_to_update = {
...
from terminal_erp.accounts.doctype.tax_withholding_category.tax_withholding_category import (
...
class TaxWithholdingController:
	...
	def _check_historical_threshold_status(self, category):
		entry = terminal_framework.qb.DocType(DOCTYPE)
		result = terminal_framework._dict(
	...
	def _base_threshold_query(self, category):
		entry = terminal_framework.qb.DocType(DOCTYPE)
		query = (
			terminal_framework.qb.from_(entry)
	...
	def _get_linked_payments(self):
		references = terminal_framework._dict()
	...
	def _create_tax_row(self, account_head, tax_amount):
		cost_center = self.doc.cost_center or terminal_erp.get_default_cost_center(self.doc.company)
...
class PaymentTaxWithholding(TaxWithholdingController):
	...
	def _get_open_entries_for_category(self, category):
		...
		current_entry = terminal_framework._dict(
			{
				**self._create_default_entry(category),
				"taxable_amount": category.taxable_amount,
				"taxable_doctype": "",
...
class JournalTaxWithholding(TaxWithholdingController):
	...
	def _calculate_net_total(self):
		from terminal_erp.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def update_tax_rows(self):
		...
		if self.has_multiple_parties:
			terminal_framework.throw(_("Cannot apply TDS against multiple parties in one entry"))
	...
	def _create_or_update_tds_row(self, account_head, tax_amount):
		...
		company_currency = terminal_framework.get_cached_value("Company", self.doc.company, "default_currency")
		...
		if not tax_row:
			tax_row = self.doc.append(
				"accounts",
				{
					...
					or terminal_erp.get_default_cost_center(self.doc.company),
					"credit": 0,
					"credit_in_account_currency": 0,
					"debit": 0,
					"debit_in_account_currency": 0,
...
def _reset_idx(docs_to_reset_idx):
	...
	if updates:
		terminal_framework.db.bulk_update(DOCTYPE, updates, update_modified=False)
