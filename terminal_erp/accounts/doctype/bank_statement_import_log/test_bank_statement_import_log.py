...
from terminal_erp.accounts.doctype.bank_statement_import_log.bank_statement_import_log import (
...
class TestBankStatementImportLog(Terminal ERPTestSuite, AccountsTestMixin):
	def setUp(self):
	...
	def test_pdf_password_protected(self):
		...
		self.assertRaises(terminal_framework.ValidationError, extract_pdf_tables, encrypted)
		self.assertRaises(terminal_framework.ValidationError, extract_pdf_tables, encrypted, "wrong")
	...
	def test_pdf_no_tables_detected(self):
		...
		self.assertRaises(terminal_framework.ValidationError, extract_pdf_tables, self._make_pdf(html))

	def _create_pdf_import_log(self, html: str) -> BankStatementImportLog:
		pdf_bytes = self._make_pdf(html)
		file_doc = terminal_framework.get_doc(
			{
				"doctype": "File",
				"file_name": f"test-statement-{terminal_framework.generate_hash(length=8)}.pdf",
				"is_private": 1,
				"content": pdf_bytes,
			}
		...
		doc = terminal_framework.get_doc(
			{
				"doctype": "Bank Statement Import Log",
				"name": f"test-pdf-{terminal_framework.generate_hash(length=8)}",
				"bank_account": self.bank_account,
	...
	def test_pdf_full_lifecycle(self):
		...
		for table in tables:
			...
			attached_to = terminal_framework.db.get_value("File", {"file_url": table["page_image"]}, "attached_to_name")
		...
		created = terminal_framework.get_all(
	...
	def _create_csv_import_log(self, csv_text: str) -> BankStatementImportLog:
		file_doc = terminal_framework.get_doc(
			{
				"doctype": "File",
				"file_name": f"test-statement-{terminal_framework.generate_hash(length=8)}.csv",
				"is_private": 1,
				"content": csv_text,
			}
		).insert(ignore_permissions=True)
