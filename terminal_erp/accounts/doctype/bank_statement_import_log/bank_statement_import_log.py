...
from terminal_erp.accounts.doctype.bank_account.bank_account import set_closing_balance_as_per_statement
...
class BankStatementImportLog(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.bank_statement_import_log_column_map.bank_statement_import_log_column_map import (
	...
	def set_currency(self):
		account = terminal_framework.get_cached_value("Bank Account", self.bank_account, "account")
		self.currency = terminal_framework.get_cached_value("Account", account, "account_currency")

	def validate(self):
		if not terminal_framework.has_permission("Bank Transaction", "write"):
			terminal_framework.throw(
				_("You do not have permission to import bank transactions"), title=_("Permission Denied")
			)
		if not terminal_framework.has_permission("Bank Transaction", "create"):
			terminal_framework.throw(
				_("You do not have permission to import bank transactions"), title=_("Permission Denied")
			)

		if not terminal_framework.has_permission("Bank Transaction", "submit"):
			terminal_framework.throw(
				_("You do not have permission to import and submit bank transactions"),
				title=_("Permission Denied"),
			)

		is_company_account, disabled = terminal_framework.get_value(
	...
	def get_file_doc(self):
		return terminal_framework.get_doc("File", {"file_url": self.file})
	...
	def get_statement_password(self):
		...
		from terminal_framework.utils.password import get_decrypted_password
	...
	def insert_transactions(self):
		...
		company, account, is_company_account, disabled = terminal_framework.get_value(
		...
		currency = terminal_framework.get_value("Account", account, "account_currency")
		...
		for transaction in final_transactions:
			bank_tx = terminal_framework.get_doc(
				{
		...
		if self.closing_balance and self.closing_balance > 0 and self.end_date:
			set_closing_balance_as_per_statement(
				self.bank_account, terminal_framework.utils.getdate(self.end_date), self.closing_balance
			)

		from terminal_erp.accounts.doctype.bank_transaction_rule.bank_transaction_rule import run_rule_evaluation
...
def guess_column_mapping_by_content(rows: list[list]) -> list[dict]:
	...
	for idx in range(num_cols):
		...
		if not count:
			...
			continue
		date_hits = sum(1 for c in cells if isinstance(c, str) and terminal_framework.utils.guess_date_format(c))
...
def extract_transaction_rows(data: list[list], column_mapping: dict[str, int], header_index: int):
	...
	for row_index, row in enumerate(valid_rows):
		...
		row_date_format = terminal_framework.utils.guess_date_format(date)
...
def get_statement_details(statement_import_id: str):
	doc = terminal_framework.get_doc("Bank Statement Import Log", statement_import_id)
...
def update_pdf_tables(statement_import_id: str, tables: list | str):
	...
	if doc.status == "Completed":
		terminal_framework.throw(_("This statement has already been imported."), title=_("Already Imported"))

	tables = terminal_framework.parse_json(tables)
...
def reextract_pdf_table(statement_import_id: str, page: int, table_index: int, bbox: list | str):
	...
	bbox = terminal_framework.parse_json(bbox)
...
def update_column_mapping(statement_import_id: str, column_mapping: list | str):
	...
	column_mapping = terminal_framework.parse_json(column_mapping)
...
def check_for_conflicts(bank_account: str, start_date: str, end_date: str):
	...
	conflicts = terminal_framework.get_all(
		"Bank Transaction",
		filters={
