...
terminal_framework.provide("terminal_erp.cheque_print");

terminal_framework.ui.form.on("Cheque Print Template", {
		...
		if (!frm.doc.__islocal) {
			if (terminal_framework.user.has_role("System Manager")) {
				frm.add_custom_button(
					...
					function () {
						terminal_erp.cheque_print.view_cheque_print(frm);
					}
			...
			var template =
				...
				<div id="cheque_preview" style="width: {{ cheque_width }}cm; \
					...
					<span style="top: {{ date_dist_from_top_edge }}cm;\
						...
						position: absolute;"> {{ terminal_framework.datetime.obj_to_user() }} </span>\
			...
			$(terminal_framework.render(template, frm.doc)).appendTo(frm.fields_dict.cheque_print_preview.wrapper);
...
terminal_erp.cheque_print.view_cheque_print = function (frm) {
	terminal_framework.call({
		method: "terminal_erp.accounts.doctype.cheque_print_template.cheque_print_template.create_or_update_cheque_print_format",
		args: {
		...
		callback: function (r) {
			if (!r.exe && !frm.doc.has_print_format) {
				...
				terminal_framework.msgprint(__("Print settings updated in respective print format"));
			}
		},
	});
};
