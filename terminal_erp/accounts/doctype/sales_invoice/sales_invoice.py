...
from terminal_framework import _, msgprint, throw
from terminal_framework.query_builder import Case
from terminal_framework.utils import cint, flt, formatdate, get_link_to_form
from terminal_framework.utils.data import comma_and

import terminal_erp
from terminal_erp.accounts.deferred_revenue import validate_service_stop_date
...
from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import SalesTaxWithholding
from terminal_erp.accounts.party import get_due_date, get_party_account
from terminal_erp.accounts.utils import refresh_subscription_status, update_voucher_outstanding
...
from terminal_erp.stock.doctype.delivery_note.services.billing_status import (
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
	...
	def onload(self):
		super().onload()
		if self.customer:
			tax_withholding_category, tax_withholding_group = terminal_framework.get_cached_value(
	...
	def validate(self):
		...
		if self.is_return and not self.return_against and self.timesheets:
			terminal_framework.throw(_("Direct return is not allowed for Timesheet."))

		if not self.is_return:
			TimesheetBillingService(self).validate_time_sheets_are_submitted()

		from terminal_erp.accounts.services.billing_validation import BillingValidationService
		...
		for row in self.items:
			if row.get("against_pick_list"):
				terminal_framework.throw(
					...
					).format(row.idx, terminal_framework.bold(row.item_code), terminal_framework.bold(row.against_pick_list))
				)
	...
	def on_submit(self):
		...
		if (
			...
			against_si_doc = terminal_framework.get_doc("Sales Invoice", self.return_against)
	...
	def check_credit_limit(self):
		...
		bypass_credit_limit_check_at_sales_order = terminal_framework.db.get_value(
	...
	def set_missing_values(self, for_validate: bool = False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = terminal_framework.db.get_value(
	...
	def get_company_abbr(self):
		return terminal_framework.db.get_value("Company", self.company, "abbr")
	...
	def validate_debit_to_acc(self):
		...
		if not account:
			terminal_framework.throw(_("Debit To is required"), title=_("Account Missing"))

		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {0} account is a Balance Sheet account.").format(terminal_framework.bold(_("Debit To")))
				+ " "
			)
			msg += _(
				"You can change the parent account to a Balance Sheet account or select a different account."
			)
			terminal_framework.throw(msg, title=_("Invalid Account"))

		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {0} account {1} is a Receivable account.").format(
					terminal_framework.bold(_("Debit To")), terminal_framework.bold(self.debit_to)
				)
				+ " "
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(terminal_framework.get_single_value("Selling Settings", "maintain_same_sales_rate"))
	...
	def force_set_against_income_account(self):
		self.set_against_income_account()
		terminal_framework.db.set_value(self.doctype, self.name, "against_income_account", self.against_income_account)
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.items():
			if terminal_framework.get_single_value("Selling Settings", value[0]) == "Yes":
				if terminal_framework.get_value("Customer", self.customer, value[0]):
					continue
	...
	def validate_proj_cust(self):
		...
		if self.project and self.customer:
			Project = terminal_framework.qb.DocType("Project")

			query = (
				terminal_framework.qb.from_(Project)
				.select(Project.name)
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if (
				not d.warehouse
				and d.item_code
				and terminal_framework.get_cached_value("Item", d.item_code, "is_stock_item")
			):
				terminal_framework.throw(_("Warehouse required for stock Item {0}").format(d.item_code))
	...
	def validate_subcontracted_sales_order(self):
		if self.has_subcontracted:
			...
			if not all(
				...
				terminal_framework.throw(_("All linked Sales Orders must be subcontracted."))
				...
				if item.stock_qty > max_qty:
					terminal_framework.throw(
						_("Row #{0}: Stock quantity {1} ({2}) for item {3} cannot exceed {4}").format(
							...
							terminal_framework.bold(max_qty),
						)
					)

	def validate_write_off_account(self):
		if flt(self.write_off_amount) and not self.write_off_account:
			self.write_off_account = terminal_framework.get_cached_value("Company", self.company, "write_off_account")
	...
	def is_auto_fetch_timesheet_enabled(self):
		return terminal_framework.db.get_single_value("Projects Settings", "fetch_timesheet_in_sales_invoice")
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if (
				d.sales_order
				and terminal_framework.db.get_value("Sales Order", d.sales_order, "docstatus", cache=True) != 1
			):
				terminal_framework.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if (
				d.delivery_note
				and terminal_framework.db.get_value("Delivery Note", d.delivery_note, "docstatus", cache=True) != 1
			):
				throw(_("Delivery Note {0} is not submitted").format(d.delivery_note))

	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from terminal_erp.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = terminal_erp.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
	...
	def enable_discount_accounting(self):
		if not hasattr(self, "_enable_discount_accounting"):
			self._enable_discount_accounting = cint(
				terminal_framework.get_single_value("Selling Settings", "enable_discount_accounting")
			)
	...
	def update_billing_status_in_dn(self, update_modified=True):
		...
		SalesInvoiceItem = terminal_framework.qb.DocType("Sales Invoice Item")
		from terminal_framework.query_builder.functions import Coalesce, Sum

		for d in self.get("items"):
			if d.dn_detail:
				query = (
					terminal_framework.qb.from_(SalesInvoiceItem)
		...
		for dn in set(updated_delivery_notes):
			terminal_framework.get_doc("Delivery Note", dn).update_billing_percentage(update_modified=update_modified)
	...
	def update_project(self):
		...
		for p in unique_projects:
			project = terminal_framework.get_doc("Project", p)
	...
	def update_billed_qty_in_scio(self):
		...
		if data:
			...
			terminal_framework.qb.update(table).set(table.billed_qty, case_expr).where(
...
def check_if_return_invoice_linked_with_payment_entry(self):
	if not terminal_framework.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
		return
	...
	PaymentEntry = terminal_framework.qb.DocType("Payment Entry")
	PaymentEntryReference = terminal_framework.qb.DocType("Payment Entry Reference")

	query = (
		terminal_framework.qb.from_(PaymentEntry)
	...
	if payment_entries:
		for payment in payment_entries:
			payment_entry = terminal_framework.get_doc("Payment Entry", payment)
