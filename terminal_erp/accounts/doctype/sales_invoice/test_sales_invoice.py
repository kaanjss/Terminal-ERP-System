...
from terminal_framework.model.dynamic_links import get_dynamic_link_map
from terminal_framework.utils import add_days, cint, flt, format_date, getdate, nowdate, today

import terminal_erp
from terminal_erp.accounts.doctype.account.test_account import create_account, get_inventory_account
from terminal_erp.accounts.doctype.mode_of_payment.test_mode_of_payment import (
	set_default_account_for_mode_of_payment,
)
from terminal_erp.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from terminal_erp.accounts.doctype.purchase_invoice.purchase_invoice import WarehouseMissingError
from terminal_erp.accounts.doctype.purchase_invoice.test_purchase_invoice import (
	unlink_payment_on_cancel_of_invoice,
)
from terminal_erp.accounts.doctype.sales_invoice.mapper import make_inter_company_transaction
from terminal_erp.accounts.doctype.sales_invoice.services.pos import (
	POSService,
	get_all_mode_of_payments,
	get_mode_of_payment_info,
	get_mode_of_payments_info,
)
from terminal_erp.accounts.utils import PaymentEntryUnlinkError
from terminal_erp.assets.doctype.asset.depreciation import post_depreciation_entries
...
from terminal_erp.controllers.accounts_controller import InvalidQtyError, update_invoice_status
from terminal_erp.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from terminal_erp.exceptions import InvalidAccountCurrency, InvalidCurrency
from terminal_erp.selling.doctype.customer.test_customer import get_customer_dict
from terminal_erp.stock.doctype.delivery_note.mapper import make_sales_invoice
...
from terminal_erp.stock.get_item_details import get_item_tax_map
from terminal_erp.stock.utils import get_incoming_rate, get_stock_balance
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSalesInvoice(Terminal ERPTestSuite):
	def setUp(self):
		...
		from terminal_erp.stock.doctype.stock_ledger_entry.test_stock_ledger_entry import create_items

		create_items(["_Test Internal Transfer Item"], uoms=[{"uom": "Box", "conversion_factor": 10}])
		create_internal_parties()
		setup_accounts()
		mode_of_payment = terminal_framework.get_doc("Mode of Payment", "Bank Draft")
		...
		for company in terminal_framework.get_all("Company", pluck="name"):
			terminal_framework.db.set_value("Company", company, "accounts_frozen_till_date", None)
	...
	def test_invalid_rate_without_override(self):
		from terminal_framework import ValidationError

		from terminal_erp.accounts.doctype.sales_invoice.mapper import make_inter_company_purchase_invoice
	...
	def make(self):
		w = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = terminal_framework.get_doc(w.doctype, w.name)
		...
		time.sleep(1)
		self.assertRaises(terminal_framework.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][2])
		si.insert()
		si.naming_series = "TEST-"

		self.assertRaises(terminal_framework.CannotChangeConstantError, si.save)

		si = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][1])
		si.insert()
		si.naming_series = "TEST-"
	...
	def test_payment_entry_unlink_against_invoice(self):
		from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry

		si = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		si = terminal_framework.get_doc("Sales Invoice", si.name)
		self.assertRaises(terminal_framework.LinkExistsError, si.cancel)
		unlink_payment_on_cancel_of_invoice()

	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"unlink_payment_on_cancellation_of_invoice": 1})
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"number_format": "#,###", "currency_precision": 0})
	...
	def test_sales_invoice_discount_amount(self):
		si = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][3])
	...
	def test_discount_amount_gl_entry(self):
		terminal_framework.db.set_value("Company", "_Test Company", "round_off_account", "Round Off - _TC")
		si = terminal_framework.copy_doc(self.globalTestRecords["Sales Invoice"][3])
		si.discount_amount = 104.94
		si.append(
			"taxes",
		...
		gle = terminal_framework.get_all(
			"GL Entry",
			filters={"voucher_type": "Sales Invoice", "voucher_no": si.name},
			fields=["*"],
		)
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"allow_multiple_items": True})
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(terminal_framework.ValidationError, si.insert)
	...
	def test_sales_invoice_calculation_export_currency_with_tax_inclusive_price(self):
		...
		if terminal_framework.db.get_value("Currency", "USD", "smallest_currency_fraction_value") < 0.01:
	...
	def test_payment(self):
		w = self.make()
		jv = terminal_framework.get_doc(terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(terminal_framework.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(terminal_framework.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)

	def test_outstanding_on_cost_center_allocation(self):
		# setup cost centers
		from terminal_erp.accounts.doctype.cost_center.test_cost_center import create_cost_center
		from terminal_erp.accounts.doctype.cost_center_allocation.test_cost_center_allocation import (
	...
	def test_pos_gl_entry_with_perpetual_inventory(self):
		...
		si = terminal_framework.copy_doc(pos)
		si.insert()
		si.submit()
	...
	def test_pos_returns_with_repayment(self):
		from terminal_erp.accounts.doctype.sales_invoice.mapper import make_sales_return
	...
	def test_set_pos_fields_populates_invoice_from_profile(self):
		terms = terminal_framework.db.exists("Terms and Conditions", "_Test POS Terms")
		if not terms:
			terms = (
	...
	def test_ledger_entries_of_return_pos_invoice(self):
		...
		res = terminal_framework.db.get_all(
			"GL Entry",
	...
	def test_pos_with_no_gl_entry_for_change_amount(self):
		terminal_framework.db.set_single_value("POS Settings", "post_change_gl_entries", 0)
		...
		terminal_framework.db.set_single_value("POS Settings", "post_change_gl_entries", 1)
	...
	def test_get_gle_for_change_amount(self):
		from terminal_erp.accounts.doctype.sales_invoice.services.gl_composer import SalesInvoiceGLComposer
		...
		self.assertRaises(terminal_framework.ValidationError, SalesInvoiceGLComposer(si).get_gle_for_change_amount)
	...
	def validate_pos_gl_entry(self, si, pos, cash_amount, validate_without_change_gle=False):
		...
		terminal_framework.db.delete("POS Profile")

	def test_bin_details_of_packed_item(self):
		from terminal_erp.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
		from terminal_erp.stock.doctype.item.test_item import make_item

		# test Update Items with product bundle
		if not terminal_framework.db.exists("Item", "_Test Product Bundle Item New"):
		...
		bin_details = terminal_framework.db.get_value(
			"Bin",
	...
	def test_pos_si_without_payment(self):
		...
		self.assertRaises(terminal_framework.ValidationError, si.submit)
	...
	def _insert_purchase_receipt(self):
		pr = terminal_framework.copy_doc(self.globalTestRecords["Purchase Receipt"][0])
		pr.naming_series = "_T-Purchase Receipt-"
		pr.insert()
		pr.submit()

	def _insert_delivery_note(self):
		dn = terminal_framework.copy_doc(self.globalTestRecords["Delivery Note"][0])
	...
	def test_sales_invoice_with_advance(self):
		jv = terminal_framework.copy_doc(self.globalTestRecords["Journal Entry"][0])
		jv.insert()
		jv.submit()
	...
	def test_serialized(self):
		...
		self.assertFalse(terminal_framework.db.get_value("Serial No", serial_nos[0], "warehouse"))
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(
			terminal_framework.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_return_sales_invoice(self):
		...
		incoming_rate, stock_value_difference = terminal_framework.db.get_value(
			"Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = terminal_framework.db.get_value(
			"GL Entry",
		...
		party_credited = terminal_framework.db.get_value(
			"GL Entry",
			{
		...
		self.assertEqual(terminal_framework.db.get_value("Sales Invoice", si1.name, "outstanding_amount"), -1000)
		self.assertEqual(terminal_framework.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 2500)
	...
	def test_zero_qty_return_invoice_with_stock_effect(self):
		...
		self.assertRaises(terminal_framework.ValidationError, cr_note.save)
	...
	def test_gle_made_when_asset_is_returned(self):
		...
		disposal_account = terminal_framework.get_cached_value("Company", "_Test Company", "disposal_account")

		# Asset value is 100,000 but it was sold for 90,000, so there should be a loss of 10,000
		loss_for_si = terminal_framework.get_all(
			"GL Entry",
			filters={"voucher_no": si.name, "account": disposal_account},
			fields=["credit", "debit"],
		)[0]

		loss_for_return_si = terminal_framework.get_all(
			"GL Entry",
	...
	def test_incoming_rate_for_stand_alone_credit_note(self):
		...
		debit_amount = terminal_framework.db.get_value(
	...
	def test_outstanding_amount_after_advance_jv_cancellation(self):
		...
		jv = terminal_framework.get_doc("Journal Entry", jv.name)
		jv.cancel()
	...
	def test_multiple_uom_in_selling(self):
		item_price = terminal_framework.new_doc("Item Price")
	...
	def test_item_wise_tax_breakup(self):
		terminal_framework.flags.country = "United States"
		...
		terminal_framework.flags.country = None
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"sales_update_frequency": "Each Transaction"})
	def test_company_monthly_sales(self):
		from terminal_erp.setup.doctype.company.company import update_company_current_month_sales

		company = "_Test Company"

		update_company_current_month_sales(company)
		existing_current_month_sales = terminal_framework.db.get_value("Company", company, "total_monthly_sales")

		si = create_sales_invoice()
		current_month_sales = terminal_framework.db.get_value("Company", company, "total_monthly_sales")
	...
	def test_rounding_adjustment_2(self):
		...
		round_off_account = terminal_framework.get_cached_value("Company", "_Test Company", "round_off_account")
		expected_values = {
	...
	def test_rounding_adjustment_3(self):
		...
		round_off_gle = terminal_framework.db.get_value(
			"GL Entry",
	...
	def test_sales_invoice_with_shipping_rule(self):
		from terminal_erp.accounts.doctype.shipping_rule.test_shipping_rule import create_shipping_rule
	...
	def test_credit_note(self):
		...
		si_doc = terminal_framework.get_doc("Sales Invoice", si.name)
	...
	def test_deferred_revenue(self):
		...
		pda1 = terminal_framework.get_doc(
	...
	def test_deferred_revenue_missing_account(self):
		...
		self.assertRaises(terminal_framework.ValidationError, si.save)
	...
	def test_inter_company_transaction_does_not_inherit_party_fields(self):
		...
		supplier = terminal_framework.get_doc("Supplier", "_Test Internal Supplier")
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		# setup
		old_negative_stock = terminal_framework.db.get_single_value("Stock Settings", "allow_negative_stock")
		terminal_framework.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		old_perpetual_inventory = terminal_erp.is_perpetual_inventory_enabled("_Test Company 1")
		terminal_framework.local.enable_perpetual_inventory["_Test Company 1"] = 1
		...
		# tear down
		terminal_framework.local.enable_perpetual_inventory["_Test Company 1"] = old_perpetual_inventory
		terminal_framework.db.set_single_value("Stock Settings", "allow_negative_stock", old_negative_stock)
	...
	def test_internal_transfer_gl_precision_issues(self):
		...
		if get_stock_balance("_Test Internal Transfer Item", "Stores - TCP1", "2022-04-10"):
			create_stock_reconciliation(
				...
				if terminal_framework.get_all("Stock Ledger Entry")
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"enable_discount_accounting": 1})
	...
	def test_additional_discount_for_sales_invoice_with_discount_accounting_enabled(self):
		from terminal_erp.accounts.doctype.repost_accounting_ledger.test_repost_accounting_ledger import (
	...
	def test_fixed_asset_sale_validations(self):
		from terminal_erp.accounts.doctype.sales_invoice.services.fixed_assets import FixedAssetService
		...
		with self.subTest("item without an asset is rejected"):
			...
			self.assertRaises(terminal_framework.ValidationError, FixedAssetService(si).validate_fixed_asset)
		...
		for bad_status in ("Sold", "Scrapped", "Cancelled", "Capitalized"):
			with self.subTest(f"selling a {bad_status} asset is rejected"):
				terminal_framework.db.set_value("Asset", asset.name, "status", bad_status)
				si = asset_invoice(asset.name)
				self.assertRaises(terminal_framework.ValidationError, FixedAssetService(si).validate_fixed_asset)
				terminal_framework.db.set_value("Asset", asset.name, "status", "Submitted")

	def test_fixed_asset_restore_note_text(self):
		from terminal_erp.accounts.doctype.sales_invoice.services.fixed_assets import FixedAssetService

		asset = terminal_framework._dict(doctype="Asset", name="_Test Asset For Note")
		si = create_sales_invoice(do_not_save=True)
	...
	def test_sales_invoice_against_supplier(self):
		from terminal_erp.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from terminal_erp.accounts.doctype.party_link.party_link import create_party_link
		from terminal_erp.buying.doctype.supplier.test_supplier import create_supplier
		...
		terminal_framework.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 1)
		...
		party_link.delete()
		terminal_framework.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 0)
	...
	def test_sales_invoice_against_supplier_usd_with_dimensions(self):
		...
		cust_doc = terminal_framework.get_doc("Customer", customer)
		cust_doc.default_currency = "USD"
		cust_doc.save()
		# create a supplier
		supplier = create_supplier(supplier_name="_Test Common Supplier USD").name
		supp_doc = terminal_framework.get_doc("Supplier", supplier)
		...
		if not terminal_framework.get_all("Accounting Dimension", filters={"document_type": "Department"}):
			dim = terminal_framework.get_doc(
				{
			...
			dim = terminal_framework.get_doc(
				...
				terminal_framework.get_all("Accounting Dimension", filters={"document_type": "Department"})[0],
			)
			dim.disabled = False
	...
	def test_sales_invoice_cancel_with_common_party_advance_jv(self):
		...
		# check creation of journal entry
		jv = terminal_framework.db.get_value(
			"Journal Entry Account",
			filters={
				"reference_type": si.doctype,
				"reference_name": si.name,
		...
		jv_status = terminal_framework.db.get_value("Journal Entry", jv, "docstatus")
		self.assertEqual(jv_status, 2)

		party_link.delete()
		terminal_framework.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 0)

	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"enable_common_party_accounting": True})
	def test_sales_invoice_return_common_party_je_has_no_negative_amounts(self):
		...
		jv_accounts = terminal_framework.get_all(
			"Journal Entry Account",
	...
	def test_status_indicator(self):
		from terminal_erp.accounts.doctype.sales_invoice.services.status import StatusService
	...
	def test_payment_statuses(self):
		...
		# Test Partly Paid
		pe = terminal_framework.copy_doc(pe)
	...
	def test_sales_commission(self):
		...
		terminal_framework.db.set_value("Item", si.get("items")[0].item_code, "grant_commission", 1)
		terminal_framework.db.set_value("Item", si.get("items")[1].item_code, "grant_commission", 0)
	...
	def test_sales_invoice_submission_post_account_freezing_date(self):
		terminal_framework.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", add_days(getdate(), 1))
		si = create_sales_invoice(do_not_save=True)
		...
		si.submit()
		terminal_framework.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", None)

	def test_sales_invoice_cancellation_post_account_freezing_date(self):
		si = create_sales_invoice()
		terminal_framework.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", add_days(getdate(), 1))
		try:
			self.assertRaises(terminal_framework.ValidationError, si.cancel)
		finally:
			terminal_framework.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", None)

	@Terminal ERPTestSuite.change_settings("Accounts Settings", {"over_billing_allowance": 0})
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"allow_multiple_items": 1})
	def test_over_billing_case_against_delivery_note(self):
		...
		si = make_sales_invoice(dn.name)
		item_copy = terminal_framework.copy_doc(si.items[0])
		si.save()
	...
	def test_non_stock_item_over_billing_against_so_is_blocked(self):
		from terminal_erp.selling.doctype.sales_order.mapper import make_sales_invoice as make_si_from_so
		from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
	...
	def test_non_stock_item_over_billing_against_so_from_quotation_is_blocked(self):
		from terminal_erp.selling.doctype.quotation.mapper import make_sales_order as make_so_from_quotation
		...
		so.delivery_date = terminal_framework.utils.add_days(terminal_framework.utils.today(), 7)
		so.insert()
		so.submit()
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		terminal_framework.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", getdate("2019-01-31"))
	...
	def test_sales_invoice_with_disabled_account(self):
		try:
			...
			account = terminal_framework.get_doc("Account", account_name)
			account.disabled = 1
			account.save()
	...
	def test_gain_loss_with_advance_entry(self):
		...
		si.reload()
		self.assertEqual(si.outstanding_amount, 0)
		journals = terminal_framework.db.get_all(
			"Journal Entry Account",
		...
		je_type = terminal_framework.get_cached_value("Journal Entry", journals[0], "voucher_type")
	...
	def test_batch_expiry_for_sales_invoice_return(self):
		...
		terminal_framework.db.set_value("Batch", batch_no, "expiry_date", add_days(today(), -1))
	...
	def test_advance_entries_as_liability(self):
		from terminal_erp.accounts.doctype.payment_entry.test_payment_entry import create_payment_entry
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"customer_group": None, "territory": None})
	def test_sales_invoice_without_customer_group_and_territory(self):
		# create a customer
		if not terminal_framework.db.exists("Customer", "_Test Simple Customer"):
			customer_dict = get_customer_dict("_Test Simple Customer")
			customer_dict.pop("customer_group")
			customer_dict.pop("territory")
			customer = terminal_framework.get_doc(customer_dict).insert(ignore_permissions=True)
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"allow_negative_rates_for_items": 0})
	...
	def test_partial_allocation_on_advance_as_liability(self):
		...
		pr = terminal_framework.get_doc("Payment Reconciliation")
		pr.company = company
		...
		pr.allocate_entries(terminal_framework._dict({"invoices": invoices, "payments": payments}))
	...
	def test_loyalty_points_redemption_with_shopping_cart(self):
		from terminal_erp.accounts.doctype.loyalty_program.test_loyalty_program import (
		...
		loyalty_customer = terminal_framework.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name
		terminal_framework.db.set_value("Customer", loyalty_customer, "loyalty_program", "Test Single Loyalty")
		...
		from terminal_framework.model.trace import traced_field_context
		...
		loyalty_account = terminal_framework.db.get_value("Loyalty Program", "Test Single Loyalty", "expense_account")
		expected_gl_entries = [
	...
	def test_taxes_merging_from_delivery_note(self):
		...
		from terminal_framework.model.mapper import map_docs

		map_docs(
			method="terminal_erp.stock.doctype.delivery_note.mapper.make_sales_invoice",
	...
	def _create_opening_roundoff_account(self, company_name):
		liability_root = terminal_framework.db.get_all(
			"Account",
		...
		if acc := terminal_framework.db.exists(
			"Account",
			{
			...
			terminal_framework.db.set_value("Company", company_name, "round_off_for_opening", acc)
		else:
			...
			acc.save()
			terminal_framework.db.set_value("Company", company_name, "round_off_for_opening", acc.name)
	...
	def test_ledger_entries_on_opening_invoice_with_rounding_loss_by_inclusive_tax(self):
		...
		si.submit()
		actual = terminal_framework.db.get_all(
			"GL Entry",
	...
	def test_total_billed_amount(self):
		...
		project = terminal_framework.new_doc("Project")
		...
		doc = terminal_framework.get_doc("Project", project.name)
	...
	def test_prevents_fully_returned_invoice_with_zero_quantity(self):
		from terminal_erp.controllers.sales_and_purchase_return import StockOverReturnError, make_return_doc
	...
	def test_pos_sales_invoice_creation_during_pos_invoice_mode(self):
		...
		terminal_framework.db.delete("POS Opening Entry")
		...
		with self.change_settings("POS Settings", {"invoice_type": "POS Invoice"}):
			...
			self.assertRaises(terminal_framework.ValidationError, pos.insert)
	...
	def test_stand_alone_credit_note_valuation(self):
		...
		stock_ledger_entry = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Sales Invoice",
				"voucher_no": si.name,
	...
	def test_non_batchwise_valuation_for_moving_average(self):
		...
		doc = terminal_framework.get_doc("Stock Settings")
		...
		change_in_value = terminal_framework.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Sales Invoice",
				"voucher_no": si.name,
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"set_zero_rate_for_expired_batch": True})
	...
	def test_inter_company_transaction_cost_center(self):
		...
		cost_center = terminal_framework.db.get_value("Company", "_Test Company 1", "cost_center")
		terminal_framework.db.set_value("Company", "_Test Company 1", "cost_center", None)
		...
		terminal_framework.db.set_value("Company", "_Test Company 1", "cost_center", cost_center)
...
def create_sales_invoice(**args):
	si = terminal_framework.new_doc("Sales Invoice")
	...
	if si.update_stock and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batch_no"):
			batches = terminal_framework._dict({args.batch_no: qty})
...
def get_outstanding_amount(against_voucher_type, against_voucher, account, party, party_type):
	balance = terminal_framework.get_all(
		"GL Entry",
		filters={
...
def create_internal_parties():
	from terminal_erp.selling.doctype.customer.test_customer import create_internal_customer
	...
	terminal_framework.db.set_value("Company", "_Test Company", "unrealized_profit_loss_account", account)
...
def create_internal_supplier(supplier_name, represents_company, allowed_to_interact_with):
	if not terminal_framework.db.exists("Supplier", supplier_name):
		supplier = terminal_framework.get_doc(
			{
		...
		supplier_name = terminal_framework.db.exists("Supplier", supplier_name)
