...
class PartialPaymentValidationError(terminal_framework.ValidationError):
	pass
...
class POSService:
	...
	def set_pos_fields(self, for_validate: bool = False) -> terminal_framework.Document | dict | None:
		...
		pos = terminal_framework.get_doc("POS Profile", doc.pos_profile) if doc.pos_profile else {}
		if pos:
	...
	def _set_default_change_amount_account(self) -> None:
		...
		if not doc.account_for_change_amount:
			doc.account_for_change_amount = terminal_framework.get_cached_value(
	...
	def _ensure_pos_profile(self) -> bool:
		...
		from terminal_erp.stock.get_item_details import get_pos_profile
	...
	def _set_selling_price_list(self, pos) -> None:
		doc = self.doc
		if doc.customer:
			customer_price_list, customer_group = terminal_framework.get_value(
				"Customer", doc.customer, ["default_price_list", "customer_group"]
			)
			customer_group_price_list = terminal_framework.get_value(
	...
	def _apply_pos_item_defaults(self, pos, for_validate: bool) -> None:
		from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_pos_profile_item_details_
	...
	def _set_terms_and_taxes(self, pos) -> None:
		...
		if doc.tc_name and not doc.terms:
			doc.terms = terminal_framework.db.get_value("Terms and Conditions", doc.tc_name, "terms")
	...
	def reset_mode_of_payments(self) -> None:
		doc = self.doc
		if doc.pos_profile:
			pos_profile = terminal_framework.get_cached_doc("POS Profile", doc.pos_profile)
	...
	def validate_created_using_pos(self) -> None:
		...
		if doc.is_created_using_pos and not doc.pos_profile:
			terminal_framework.throw(_("POS Profile is mandatory to mark this invoice as POS Transaction."))

		doc.invoice_type_in_pos = terminal_framework.db.get_single_value("POS Settings", "invoice_type")
		if doc.invoice_type_in_pos == "POS Invoice" and not doc.is_return:
			terminal_framework.throw(_("Transactions using Sales Invoice in POS are disabled."))
	...
	def validate_full_payment(self) -> None:
		...
		doc = self.doc
		allow_partial_payment = terminal_framework.db.get_value("POS Profile", doc.pos_profile, "allow_partial_payment")
	...
	def validate_pos_opening_entry(self) -> None:
		...
		doc = self.doc
		opening_entries = terminal_framework.get_all(
			"POS Opening Entry",
		...
		if not opening_entries:
			terminal_framework.throw(
				...
				msg=_("No open POS Opening Entry found for POS Profile {0}.").format(
					terminal_framework.bold(doc.pos_profile)
				),
			)
		...
		if terminal_framework.utils.get_date_str(opening_entries[0].get("period_start_date")) != terminal_framework.utils.today():
			terminal_framework.throw(
		...
		if doc.doctype == "Sales Invoice" and doc.is_consolidated:
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {0} you need to cancel the POS Closing Entry {1}.").format(
					terminal_framework.bold(_("Consolidated Sales Invoice")),
					get_link_to_form("POS Closing Entry", pos_closing_entry[0]),
				)
				terminal_framework.throw(msg, title=_("Not Allowed"))
	...
	def check_if_created_using_pos_and_pos_closing_entry_generated(self) -> None:
		...
		if doc.doctype == "Sales Invoice" and doc.is_created_using_pos and doc.pos_closing_entry:
			pos_closing_entry_docstatus = terminal_framework.db.get_value(
	...
	def cancel_pos_invoice_credit_note_generated_during_sales_invoice_mode(self) -> None:
		...
		for pos_invoice in pos_invoices:
			terminal_framework.get_doc("POS Invoice", pos_invoice).cancel()

	def clear_unallocated_mode_of_payments(self) -> None:
		doc = self.doc
		doc.set("payments", doc.get("payments", {"amount": ["not in", [0, None, ""]]}))
		terminal_framework.db.delete("Sales Invoice Payment", filters={"parent": doc.name, "amount": 0})
...
def _enabled_mode_of_payment_query(company: str):
	...
	mopa = terminal_framework.qb.DocType("Mode of Payment Account")
	mop = terminal_framework.qb.DocType("Mode of Payment")
	query = (
		terminal_framework.qb.from_(mopa)
		.join(mop)
