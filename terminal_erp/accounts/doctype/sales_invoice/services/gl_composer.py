...
from terminal_framework.utils import cint, cstr, flt, get_link_to_form

import terminal_erp
from terminal_erp.accounts.general_ledger import get_round_off_account_and_cost_center
from terminal_erp.accounts.services.base_gl_composer import BaseGLComposer
...
class SalesInvoiceGLComposer(BaseGLComposer):
	...
	def compose(self, inventory_account_map=None):
		from terminal_erp.accounts.doctype.sales_invoice.sales_invoice import make_regional_gl_entries
		from terminal_erp.accounts.general_ledger import merge_similar_entries
		...
		disable_sdbnb_in_sr = terminal_framework.get_cached_value("Company", doc.company, "disable_sdbnb_in_sr")
				...
				doc.get_gl_dict(
					{
						...
						"cost_center": doc.cost_center or terminal_erp.get_default_cost_center(doc.company),
					},
					item=doc,
				)
			)
	...
	def stock_delivered_but_not_billed_gl_entries(self, gl_entries):
		doc = self.doc
		if doc.update_stock or not cint(terminal_erp.is_perpetual_inventory_enabled(doc.company)):
			return
	...
	def _get_sdbnb_booking_for_item(self, item) -> dict | None:
		...
		if not terminal_framework.get_cached_value("Item", item.item_code, "is_stock_item"):
			return None

		dn_expense_account = terminal_framework.get_cached_value("Delivery Note Item", item.dn_detail, "expense_account")
		if not self._is_sdbnb_reversal(dn_expense_account, item):
			return None

		delivery_note = item.delivery_note or terminal_framework.get_cached_value(
			"Delivery Note Item", item.dn_detail, "parent"
		)
		if not delivery_note:
			return None

		item_g = terminal_framework.get_cached_value(
			"Stock Ledger Entry",
			{
	...
	def _is_sdbnb_reversal(self, dn_expense_account, item) -> bool:
		...
		return bool(
			dn_expense_account
			and terminal_framework.get_cached_value("Account", dn_expense_account, "account_type")
	...
	def make_item_gl_entries(self, gl_entries):
		from terminal_erp.accounts.doctype.sales_invoice.sales_invoice import SalesInvoice
		...
		if cint(doc.update_stock) and terminal_erp.is_perpetual_inventory_enabled(doc.company):
	...
	def get_gl_entries_for_fixed_asset(self, item, gl_entries):
		doc = self.doc
		asset = terminal_framework.get_cached_doc("Asset", item.asset)
	...
	def make_pos_gl_entries(self, gl_entries):
		...
		if cint(doc.is_pos):
			skip_change_gl_entries = not cint(
				terminal_framework.get_single_value("POS Settings", "post_change_gl_entries")
			)
	...
	def get_gle_for_change_amount(self) -> list[dict]:
		...
		if not doc.account_for_change_amount:
			terminal_framework.throw(_("Please set Account for Change Amount"), title=_("Mandatory Field"))
	...
	def make_write_off_gl_entry(self, gl_entries):
		...
		if (
			...
			default_cost_center = terminal_framework.get_cached_value("Company", doc.company, "cost_center")
				...
				if not round_off_for_opening:
					terminal_framework.throw(
						_(
							...
							terminal_framework.bold(doc.rounding_adjustment),
							terminal_framework.bold("Round Off for Opening"),
							get_link_to_form("Company", doc.company),
							terminal_framework.bold("Disable Rounded Total"),
						)
					)
				else:
