...
from terminal_framework.utils import add_days, cint, flt, getdate
...
class LoyaltyService:
	...
	def make_loyalty_point_entry(self) -> None:
		...
		if (
			...
			entry = terminal_framework.get_doc(
				{
	...
	def delete_loyalty_point_entry(self) -> None:
		doc = self.doc
		lp_entry = terminal_framework.db.get_all(
		...
		against_lp_entry = terminal_framework.db.get_all(
			"Loyalty Point Entry",
		...
		if against_lp_entry:
			...
			terminal_framework.db.delete("Loyalty Point Entry", filters={"invoice": doc.name})
			self._set_loyalty_program_tier()

	def apply_loyalty_points(self) -> None:
		from terminal_erp.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
	...
	def _get_returned_amount(self) -> float:
		...
		doc = terminal_framework.qb.DocType(self.doc.doctype)
		returned_amount = (
...
def get_loyalty_programs(customer: str) -> list:
	...
	from terminal_erp.selling.doctype.customer.customer import get_loyalty_programs as _get
