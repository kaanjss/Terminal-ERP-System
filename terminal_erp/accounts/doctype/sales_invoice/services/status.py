...
class StatusService:
	...
	def _get_payment_status(self, outstanding_amount: float, total: float) -> str:
		...
		if outstanding_amount > 0 and getdate(doc.due_date) >= getdate():
			return "Unpaid"
		if doc.is_return == 0 and terminal_framework.db.get_value(
...
def get_discounting_status(sales_invoice: str) -> str | None:
	...
	InvoiceDiscounting = terminal_framework.qb.DocType("Invoice Discounting")
	DiscountedInvoice = terminal_framework.qb.DocType("Discounted Invoice")

	query = (
		terminal_framework.qb.from_(InvoiceDiscounting)
