...
from terminal_erp.accounts.party import CROSS_PARTY_FIELD_NO_MAP, _get_party_details
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
		...
		company = terminal_framework.get_cached_value("Supplier", doc.supplier, "represents_company")
...
def get_internal_party(parties, link_doctype, doc):
	if len(parties) == 1:
		...
		if doc.get("company_address") or doc.get("shipping_address"):
			party = terminal_framework.db.get_value(
				"Dynamic Link",
				{
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = terminal_framework.db.get_value("Price List", {"name": price_list, "buying": 1, "selling": 1})
	if not valid_price_list and not doc.is_internal_transfer():
		terminal_framework.throw(_("Selected Price List should have buying and selling fields checked."))

	party = details.get("party")
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
		terminal_framework.throw(_("No {0} found for Inter Company Transactions.").format(partytype))

	company = details.get("company")
	default_currency = terminal_framework.get_cached_value("Company", company, "default_currency")
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		source_doc = terminal_framework.get_doc(doctype, source_name)
...
def _apply_purchase_party_details(target_doc, source_doc, details):
	currency = terminal_framework.db.get_value("Supplier", details.get("party"), "default_currency")
...
def _apply_sales_party_details(target_doc, source_doc, details):
	currency = terminal_framework.db.get_value("Customer", details.get("party"), "default_currency")
	target_doc.company = details.get("company")
...
def get_received_items(reference_name: str, doctype: str, reference_fieldname: str):
	...
	target_doctypes = terminal_framework.get_all(
		doctype,
		filters=filters,
		pluck="name",
	)
	received_items_map = {}
	if target_doctypes:
		received_items_data = terminal_framework.get_all(
			doctype + " Item",
	...
	for item in doc.get("items"):
		...
		if doc.update_stock:
			...
			if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
				item.warehouse = terminal_framework.db.get_value(
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
	for item in doc.get("items"):
		...
		if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
			item.warehouse = terminal_framework.db.get_value("Purchase Order Item", item.purchase_order_item, "warehouse")


def get_delivery_note_details(internal_reference):
	si_item_details = terminal_framework.get_all(
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_item_details = terminal_framework.get_all(
		doctype,
...
def create_invoice_discounting(source_name: str, target_doc: str | Document | None = None):
	invoice = terminal_framework.get_doc("Sales Invoice", source_name)
	invoice_discounting = terminal_framework.new_doc("Invoice Discounting")
...
def create_dunning(
	...
	def postprocess_dunning(source, target):
		from terminal_erp.accounts.doctype.dunning.dunning import get_dunning_letter_text

		dunning_type = terminal_framework.db.exists("Dunning Type", {"is_default": 1, "company": source.company})
		if dunning_type:
			dunning_type = terminal_framework.get_doc("Dunning Type", dunning_type)
