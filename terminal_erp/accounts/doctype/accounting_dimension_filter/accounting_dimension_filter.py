...
class AccountingDimensionFilter(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.allowed_dimension.allowed_dimension import AllowedDimension
		from terminal_erp.accounts.doctype.applicable_on_account.applicable_on_account import ApplicableOnAccount
	...
	def validate(self):
		self.fieldname = terminal_framework.db.get_value(
			"Accounting Dimension", {"document_type": self.accounting_dimension}, "fieldname"
		) or terminal_framework.scrub(self.accounting_dimension)  # scrub to handle default accounting dimension

		self.validate_applicable_accounts()

	def validate_applicable_accounts(self):
		ApplicableOnAccount = terminal_framework.qb.DocType("Applicable On Account")
		AccountingDimensionFilter = terminal_framework.qb.DocType("Accounting Dimension Filter")

		query = (
			terminal_framework.qb.from_(ApplicableOnAccount)
			...
			if account.applicable_on_account in account_list:
				terminal_framework.throw(
					_("Row {0}: {1} account already applied for Accounting Dimension {2}").format(
						account.idx,
						terminal_framework.bold(account.applicable_on_account),
						terminal_framework.bold(self.accounting_dimension),
					)
				)
...
def get_dimension_filter_map():
	ApplicableOnAccount = terminal_framework.qb.DocType("Applicable On Account")
	AccountingDimensionFilter = terminal_framework.qb.DocType("Accounting Dimension Filter")
	AllowedDimension = terminal_framework.qb.DocType("Allowed Dimension")

	query = (
		terminal_framework.qb.from_(AccountingDimensionFilter)
