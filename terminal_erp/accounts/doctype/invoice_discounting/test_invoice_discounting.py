...
from terminal_erp.accounts.doctype.journal_entry.mapper import get_payment_entry_against_invoice
...
class TestInvoiceDiscounting(Terminal ERPTestSuite):
	def setUp(self):
		...
		terminal_framework.db.set_value("Company", "_Test Company", "default_bank_account", self.bank_account)
	...
	def test_on_close_after_loan_period_after_inv_payment(self):
		...
		je_on_payment = terminal_framework.get_doc(get_payment_entry_against_invoice("Sales Invoice", inv.name))
...
def create_invoice_discounting(invoices, **args):
	...
	inv_disc = terminal_framework.new_doc("Invoice Discounting")
