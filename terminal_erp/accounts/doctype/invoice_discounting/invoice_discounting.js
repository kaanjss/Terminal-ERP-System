...
terminal_framework.ui.form.on("Invoice Discounting", {
	setup: (frm) => {
	...
	set_end_date: (frm) => {
		if (frm.doc.loan_start_date && frm.doc.loan_period) {
			let end_date = terminal_framework.datetime.add_days(frm.doc.loan_start_date, frm.doc.loan_period);
		...
		var d = new terminal_framework.ui.Dialog({
			...
			primary_action: function () {
				...
				terminal_framework.call({
					method: "terminal_erp.accounts.doctype.invoice_discounting.invoice_discounting.get_invoices",
					args: {
						filters: data,
					},
					callback: function (r) {
...
terminal_framework.ui.form.on("Discounted Invoice", {
	sales_invoice: (frm) => {
		frm.events.calculate_total_amount(frm);
		frm.events.refresh_filters(frm);
	},
