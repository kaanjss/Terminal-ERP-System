...
from terminal_framework.utils import cint, flt, getdate, today

from terminal_erp.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_details,
	get_loyalty_program_details_with_points,
)
from terminal_erp.accounts.party import get_dashboard_info
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestLoyaltyProgram(Terminal ERPTestSuite):
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = terminal_framework.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		terminal_framework.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = terminal_framework.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = terminal_framework.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = terminal_framework.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		earned_points = get_points_earned(si_original)
		lpe_original = terminal_framework.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		si_original = terminal_framework.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = terminal_framework.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		for d in [si_return, si_original]:
			try:
				d.cancel()
			except terminal_framework.TimestampMismatchError:
				terminal_framework.get_doc("Sales Invoice", d.name).cancel()

	def test_loyalty_points_for_dashboard(self):
		doc = terminal_framework.get_doc("Customer", "Test Loyalty Customer")
		company_wise_info = get_dashboard_info("Customer", doc.name, doc.loyalty_program)

		for d in company_wise_info:
			self.assertTrue(d.get("loyalty_points"))

	@unittest.mock.patch("terminal_erp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_details")
	def test_tier_selection(self, mock_get_loyalty_details):
		# Create a new loyalty program with multiple tiers
		loyalty_program = terminal_framework.get_doc(
			{
	...
	return terminal_framework.get_doc(
		{
			...
			"customer": terminal_framework.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			"company": "_Test Company",
			"due_date": today(),
			"posting_date": today(),
			"currency": "INR",
			...
			"items": [
				{
					...
					"item_code": terminal_framework.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
