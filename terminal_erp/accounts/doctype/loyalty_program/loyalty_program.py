...
class LoyaltyProgram(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.loyalty_program_collection.loyalty_program_collection import (
...
def get_loyalty_details(
	...
	LoyaltyPointEntry = terminal_framework.qb.DocType("Loyalty Point Entry")

	query = (
		terminal_framework.qb.from_(LoyaltyPointEntry)
		.select(
...
def get_loyalty_program_details_with_points(
	...
	loyalty_program = terminal_framework.get_doc("Loyalty Program", loyalty_program)
...
def get_loyalty_program_details(
	...
	lp_details = terminal_framework._dict()

	if not loyalty_program:
		loyalty_program = terminal_framework.db.get_value("Customer", customer, "loyalty_program")

		if not loyalty_program and not silent:
			terminal_framework.throw(_("Customer isn't enrolled in any Loyalty Program"))
		elif silent and not loyalty_program:
			return terminal_framework._dict({"loyalty_programs": None})

	if not company:
		company = terminal_framework.db.get_default("company") or terminal_framework.get_all("Company")[0].name
...
def get_redeemption_factor(loyalty_program: str | None = None, customer: str | None = None):
	...
	if not loyalty_program:
		customer_loyalty_program = terminal_framework.db.get_value("Customer", customer, "loyalty_program")
		loyalty_program = customer_loyalty_program
	if loyalty_program:
		return terminal_framework.db.get_value("Loyalty Program", loyalty_program, "conversion_factor")
	else:
		terminal_framework.throw(_("Customer isn't enrolled in any Loyalty Program"))
...
def validate_loyalty_points(ref_doc, points_to_redeem):
	...
	if hasattr(ref_doc, "loyalty_program") and ref_doc.loyalty_program:
		...
		loyalty_program = terminal_framework.db.get_value("Customer", ref_doc.customer, ["loyalty_program"])

	if (
		loyalty_program
		and terminal_framework.db.get_value("Loyalty Program", loyalty_program, ["company"]) != ref_doc.company
	):
		terminal_framework.throw(_("The Loyalty Program isn't valid for the selected company"))
	...
	if loyalty_program and points_to_redeem:
		...
		if points_to_redeem > loyalty_program_details.loyalty_points:
			terminal_framework.throw(_("You don't have enough Loyalty Points to redeem"))
		...
		if loyalty_amount > total_amount:
			terminal_framework.throw(_("You can't redeem Loyalty Points having more value than the Total Amount."))
