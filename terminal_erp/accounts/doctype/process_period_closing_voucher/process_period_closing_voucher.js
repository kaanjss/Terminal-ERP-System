...
terminal_framework.ui.form.on("Process Period Closing Voucher", {
	refresh(frm) {
		if (frm.doc.docstatus == 1 && ["Queued"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "terminal_erp.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.start_pcv_processing",
					args: {
		...
		if (frm.doc.docstatus == 1 && ["Running"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "terminal_erp.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.pause_pcv_processing",
					args: {
						docname: frm.doc.name,
					},
				}).then((r) => {
					if (!r.exc) {
						terminal_framework.show_alert(__("PCV Paused"));
						frm.reload_doc();
					}
				});
			});
		...
		if (frm.doc.docstatus == 1 && ["Paused"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "terminal_erp.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.resume_pcv_processing",
					args: {
						docname: frm.doc.name,
					},
				}).then((r) => {
					if (!r.exc) {
						terminal_framework.show_alert(__("PCV Resumed"));
						frm.reload_doc();
					}
				});
			});
