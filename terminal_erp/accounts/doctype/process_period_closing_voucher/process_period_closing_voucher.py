...
from terminal_framework.query_builder.functions import Count, Max, Min, Sum
...
class ProcessPeriodClosingVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.process_period_closing_voucher_detail.process_period_closing_voucher_detail import (
	...
	def generate_pcv_dates(self):
		...
		pcv = terminal_framework.get_doc("Period Closing Voucher", self.parent_pcv)
...
def start_pcv_processing(docname: str):
	if terminal_framework.db.get_value("Process Period Closing Voucher", docname, "status") in ["Queued", "Running"]:
		terminal_framework.has_permission("Process Period Closing Voucher", "write", doc=docname, throw=True)
		terminal_framework.db.set_value("Process Period Closing Voucher", docname, "status", "Running")

		timeout = terminal_framework.db.get_single_value("Accounts Settings", "pcv_job_timeout") or 3600
		...
		if normal_balances := (
			...
			if not is_scheduler_inactive():
				for x in normal_balances:
					...
					terminal_framework.enqueue(
						method="terminal_erp.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.process_individual_date",
						queue="long",
						timeout=timeout,
						is_async=True,
			...
			terminal_framework.db.set_value("Process Period Closing Voucher", docname, "status", "Completed")
...
def pause_pcv_processing(docname: str):
	...
	if queued_dates := terminal_framework.db.get_all(
		"Process Period Closing Voucher Detail",
...
def resume_pcv_processing(docname: str):
	...
	if paused_dates := terminal_framework.db.get_all(
		"Process Period Closing Voucher Detail",
	...
	gl_entry = terminal_framework._dict(
		{
			...
			"account_currency": terminal_framework.db.get_value("Account", pcv.closing_account_head, "account_currency"),
	...
	if to_process := (
		...
		if total_no_of_dates == completed:
			...
			if not is_job_running(job_name):
				terminal_framework.enqueue(
					method="terminal_erp.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.summarize_and_post_ledger_entries",
					queue="long",
					timeout=timeout,
					is_async=True,
					job_name=job_name,
...
def get_consolidated_gles(balances, report_type) -> list:
	...
	for x in balances:
		if x.report_type == report_type:
			closing_balances = [terminal_framework._dict(gle) for gle in terminal_framework.json.loads(x.closing_balance)]
...
def get_gl_entries(docname):
	...
	"""
	ppcv = terminal_framework.get_doc("Process Period Closing Voucher", docname)
	...
	terminal_framework.db.set_value(
		"Process Period Closing Voucher", docname, "p_l_closing_balance", terminal_framework.json.dumps(json_dict)
	)

	# build gl map
	pcv = terminal_framework.get_doc("Period Closing Voucher", ppcv.parent_pcv)
...
def calculate_balance_sheet_balance(docname):
	...
	terminal_framework.db.set_value(
		"Process Period Closing Voucher", docname, "bs_closing_balance", terminal_framework.json.dumps(json_dict)
	)
...
def summarize_and_post_ledger_entries(docname):
	...
	pcv_name = terminal_framework.db.get_value("Process Period Closing Voucher", docname, "parent_pcv")
	pcv = terminal_framework.get_doc("Period Closing Voucher", pcv_name)
	...
	terminal_framework.db.set_value("Period Closing Voucher", pcv.name, "gle_processing_status", "Completed")
	terminal_framework.db.set_value("Process Period Closing Voucher", docname, "status", "Completed")
...
def build_dimension_wise_balance_dict(gl_entries):
	dimension_balances = terminal_framework._dict()
	for x in gl_entries:
		dimension_key = get_dimension_key(x)
		dimension_balances.setdefault(dimension_key, terminal_framework._dict()).setdefault(
			x.account,
...
def process_individual_date(docname: str, date, report_type, parentfield):
	current_date_status = terminal_framework.db.get_value(
		"Process Period Closing Voucher Detail",
	...
	pcv_name = terminal_framework.db.get_value("Process Period Closing Voucher", docname, "parent_pcv")
	company = terminal_framework.db.get_value("Period Closing Voucher", pcv_name, "company")
	...
	terminal_framework.db.set_value(
		...
		"closing_balance",
		terminal_framework.json.dumps(res),
	)
