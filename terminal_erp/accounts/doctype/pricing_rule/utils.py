...
from terminal_framework.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from terminal_erp.setup.doctype.item_group.item_group import get_child_item_groups
from terminal_erp.stock.doctype.warehouse.warehouse import get_child_warehouses
from terminal_erp.stock.get_item_details import get_conversion_factor


class MultiplePricingRuleConflict(terminal_framework.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not terminal_framework.db.count("Pricing Rule", cache=True):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if terminal_framework.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = terminal_framework.scrub(apply_on)
			...
			if args.get("uom", None):
				item_conditions += (
					" and ({child_doc}.uom={item_uom} or COALESCE({child_doc}.uom, '')='')".format(
						child_doc=child_doc, item_uom=terminal_framework.db.escape(args.get("uom"))
					)
				)
			if "variant_of" not in args:
				args.variant_of = terminal_framework.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = terminal_framework.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not terminal_framework.flags.tree_conditions:
			terminal_framework.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in terminal_framework.flags.tree_conditions:
			return terminal_framework.flags.tree_conditions[key]

		try:
			lft, rgt = terminal_framework.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			terminal_framework.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = terminal_framework.get_all(
			parenttype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"
		)

		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = f"parent_{terminal_framework.scrub(parenttype)}"
			root_name = terminal_framework.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "coalesce({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(terminal_framework.db.escape(d) for d in parent_groups)
			)

			terminal_framework.flags.tree_conditions[key] = condition
...
def get_other_conditions(conditions, values, args):
	...
	date = (
		...
		or terminal_framework.get_value(args.get("doctype"), args.get("name"), "posting_date", ignore=True)
	)
	if date:
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		pr_doc = terminal_framework.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(terminal_framework.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = terminal_framework.scrub(pr_doc.get("apply_on"))
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if terminal_framework.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
	...
	data_set = terminal_framework.db.sql(
...
def apply_pricing_rule_on_transaction(doc):
	...
	pricing_rules = terminal_framework.db.sql(
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						terminal_framework.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = terminal_framework.db.get_value(
				...
				item_details = terminal_framework._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def get_applied_pricing_rules(pricing_rules):
	if pricing_rules:
		if pricing_rules.startswith("["):
			return terminal_framework.parse_json(pricing_rules)
		else:
...
def apply_pricing_rule_for_free_items(doc, pricing_rule_args):
	if pricing_rule_args:
		...
		for free_item in args.values():
			if doc.is_new() or not terminal_framework.get_value(
...
def get_pricing_rule_items(pr_doc, other_items=False) -> list:
	...
	if pr_doc.apply_rule_on_other and other_items:
		apply_on = terminal_framework.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = terminal_framework.get_doc("Coupon Code", coupon_name)
	if coupon.valid_from and coupon.valid_from > getdate(today()):
		terminal_framework.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto and coupon.valid_upto < getdate(today()):
		terminal_framework.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.maximum_use and coupon.used >= coupon.maximum_use:
		terminal_framework.throw(_("Sorry, this coupon code is no longer valid"))
