...
class PricingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.pricing_rule_brand.pricing_rule_brand import PricingRuleBrand
		from terminal_erp.accounts.doctype.pricing_rule_item_code.pricing_rule_item_code import PricingRuleItemCode
		from terminal_erp.accounts.doctype.pricing_rule_item_group.pricing_rule_item_group import (
	...
	def validate_duplicate_apply_on(self):
		if self.apply_on != "Transaction":
			...
			apply_on_field = terminal_framework.scrub(self.apply_on)
			values = [d.get(apply_on_field) for d in self.get(apply_on_table) if d.get(apply_on_field)]
			if len(values) != len(set(values)):
				terminal_framework.throw(_("Duplicate {0} found in the table").format(self.apply_on))

	def validate_mandatory(self):
		if self.has_priority and not self.priority:
			throw(_("Priority is mandatory"), terminal_framework.MandatoryError, _("Please Set Priority"))
		...
		for apply_on, field in apply_on_dict.items():
			if self.apply_on == apply_on and len(self.get(field) or []) < 1:
				throw(_("{0} is not added in the table").format(apply_on), terminal_framework.MandatoryError)

		tocheck = terminal_framework.scrub(self.get("applicable_for", ""))
		if tocheck and not self.get(tocheck):
			throw(_("{0} is required").format(_(self.meta.get_label(tocheck))), terminal_framework.MandatoryError)

		if self.apply_rule_on_other:
			o_field = "other_" + terminal_framework.scrub(self.apply_rule_on_other)
			if not self.get(o_field) and o_field in other_fields:
				terminal_framework.throw(
					_("For the 'Apply Rule On Other' condition the field {0} is mandatory").format(
						terminal_framework.bold(self.apply_rule_on_other)
					)
				)

		if self.price_or_product_discount == "Price" and not self.rate_or_discount:
			throw(_("Rate or Discount is required for the price discount."), terminal_framework.MandatoryError)

		if self.apply_discount_on_rate:
			if not self.priority:
				throw(
					_("As the field {0} is enabled, the field {1} is mandatory.").format(
						terminal_framework.bold(_("Apply Discount on Discounted Rate")),
						terminal_framework.bold(_("Priority")),
					)
				)
			...
			if self.priority and cint(self.priority) == 1:
				throw(
					...
					).format(terminal_framework.bold(_("Apply Discount on Discounted Rate")), terminal_framework.bold(_("Priority")))
				)
	...
	def cleanup_fields_value(self):
		for logic_field in ["apply_on", "applicable_for", "rate_or_discount"]:
			fieldname = terminal_framework.scrub(self.get(logic_field) or "")
			...
			for f in options:
				...
				scrubbed_f = terminal_framework.scrub(f)
		...
		apply_rule_on_other = terminal_framework.scrub(self.apply_rule_on_other or "")
	...
	def validate_rate_or_discount(self):
		for field in ["Rate"]:
			if flt(self.get(terminal_framework.scrub(field))) < 0:
				throw(_("{0} can not be negative").format(field))

		if self.price_or_product_discount == "Product" and not self.free_item:
			if self.mixed_conditions:
				terminal_framework.throw(_("Free item code is not selected"))
			else:
				self.same_item = 1
	...
	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.get("items"):
			for d in self.items:
				max_discount = terminal_framework.get_cached_value("Item", d.item_code, "max_discount")
				if max_discount and flt(self.discount_percentage) > flt(max_discount):
					throw(_("Max discount allowed for item: {0} is {1}%").format(d.item_code, max_discount))

	def validate_price_list_with_currency(self):
		if self.currency and self.for_price_list:
			price_list_currency = terminal_framework.db.get_value("Price List", self.for_price_list, "currency", True)
	...
	def validate_dates(self):
		if self.is_cumulative and not (self.valid_from and self.valid_upto):
			terminal_framework.throw(_("Valid from and valid upto fields are mandatory for the cumulative"))
	...
	def validate_condition(self):
		if (
			...
			terminal_framework.throw(_("Invalid condition expression"))

	def validate_mixed_with_recursion(self):
		if self.mixed_conditions and self.is_recursive:
			terminal_framework.throw(_("Recursive Discounts with Mixed condition is not supported by the system"))
...
def apply_pricing_rule(args: str | dict, doc: str | dict | Document | None = None):
	...
	query_items = terminal_framework.get_all(
		"Item",
...
def update_pricing_rule_uom(pricing_rule, args):
	...
	apply_on_field = terminal_framework.scrub(pricing_rule.apply_on)
...
def get_pricing_rule_for_item(args, doc=None, for_validate=False):
	...
	if doc:
		doc = terminal_framework.get_doc(doc)

	if args.get("is_free_item") or args.get("parenttype") == "Material Request":
		return {}

	item_details = terminal_framework._dict(
		{
	...
	if args.ignore_pricing_rule or not args.item_code:
		if terminal_framework.db.exists(args.doctype, args.name) and args.get("pricing_rules"):
	...
	if pricing_rules:
		...
		for pricing_rule in pricing_rules:
			...
			if isinstance(pricing_rule, str):
				pricing_rule = terminal_framework.get_cached_doc("Pricing Rule", pricing_rule)
			...
			if pricing_rule.get("coupon_code_based") == 1:
				if not args.coupon_code:
					continue
				coupon_code = terminal_framework.db.get_value(
				...
				item_details.update(
					{
						...
						"apply_rule_on": (
							terminal_framework.scrub(pricing_rule.apply_rule_on_other)
							if pricing_rule.apply_rule_on_other
							else terminal_framework.scrub(pricing_rule.get("apply_on"))
						),
					}
				)
		...
		item_details.pricing_rules = terminal_framework.as_json([d.pricing_rule for d in rules])
...
def update_args_for_pricing_rule(args):
	if not (args.item_group and args.brand):
		item = terminal_framework.get_cached_value("Item", args.item_code, ("item_group", "brand"))
		if not item:
			return
		...
		if not args.item_group:
			terminal_framework.throw(_("Item Group not mentioned in item master for item {0}").format(args.item_code))

	if args.transaction_type == "selling":
		if args.customer and not (args.customer_group and args.territory):
			if args.quotation_to and args.quotation_to != "Customer":
				customer = terminal_framework._dict()
			else:
				customer = terminal_framework.get_cached_value("Customer", args.customer, ["customer_group", "territory"])
		...
		args.supplier_group = terminal_framework.get_cached_value("Supplier", args.supplier, "supplier_group")
...
def apply_price_discount_rule(pricing_rule, item_details, args):
	...
	if pricing_rule.rate_or_discount == "Rate":
		...
		# TODO https://github.com/terminal_framework/terminal_erp/pull/23636 solve this in some other way.
...
def remove_pricing_rule_for_item(
	...
	item_details: str | terminal_framework._dict,
	...
	item_details = terminal_framework._dict(terminal_framework.parse_json(item_details))

	for d in get_applied_pricing_rules(pricing_rules):
		if not d or not terminal_framework.db.exists("Pricing Rule", d):
			continue
		pricing_rule = terminal_framework.get_cached_doc("Pricing Rule", d)
...
def remove_pricing_rules(item_list: str | list):
	item_list = terminal_framework.parse_json(item_list)
...
def set_transaction_type(pricing_ctx: terminal_framework._dict) -> None:
...
def get_item_uoms(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	if filters.get("apply_on") != "Item Code":
		field = terminal_framework.scrub(filters.get("apply_on"))
		items = [d.name for d in terminal_framework.db.get_all("Item", filters={field: filters.get("value")})]
