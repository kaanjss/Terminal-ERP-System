...
terminal_framework.ui.form.on("Exchange Rate Revaluation", {
	setup: function (frm) {
	...
	validate_rounding_loss: function (frm) {
		...
		if (!(allowance >= 0 && allowance < 1)) {
			terminal_framework.throw(__("Rounding Loss Allowance should be between 0 and 1"));
		}
	},
			...
			callback: function (r) {
				if (r.message) {
					...
					if (response["revaluation_jv"] || response["zero_balance_jv"]) {
						terminal_framework.msgprint(__("Journal entries have been created"));
					}
				}
			},
		});
...
terminal_framework.ui.form.on("Exchange Rate Revaluation Account", {
...
var get_account_details = function (frm, cdt, cdn) {
	...
	if (!frm.doc.company || !frm.doc.posting_date) {
		terminal_framework.throw(__("Please select Company and Posting Date to get entries"));
	}
	terminal_framework.call({
		method: "terminal_erp.accounts.doctype.exchange_rate_revaluation.exchange_rate_revaluation.get_account_details",
		args: {
			account: row.account,
