...
from terminal_framework.query_builder import Criterion, Order
from terminal_framework.query_builder.functions import Max, NullIf, Sum
from terminal_framework.utils import flt, get_link_to_form

import terminal_erp
from terminal_erp.accounts.doctype.journal_entry.journal_entry import get_balance_on
from terminal_erp.accounts.utils import get_currency_precision
...
class ExchangeRateRevaluation(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.exchange_rate_revaluation_account.exchange_rate_revaluation_account import (
	...
	def validate_rounding_loss_allowance(self):
		if self.rounding_loss_allowance < 0 or self.rounding_loss_allowance >= 1:
			terminal_framework.throw(_("Rounding Loss Allowance should be between 0 and 1"))
	...
	def validate_mandatory(self):
		if not (self.company and self.posting_date):
			terminal_framework.throw(_("Please select Company and Posting Date to get entries"))
	...
	def remove_accounts_without_gain_loss(self):
		...
		if not self.accounts:
			terminal_framework.throw(_("At least one account with exchange gain or loss is required"))
	...
	def calculate_new_account_balance(company, posting_date, account_details):
		...
		precision = get_field_precision(
			terminal_framework.get_meta("Exchange Rate Revaluation Account").get_field("new_balance_in_base_currency"),
	...
	def throw_invalid_response_message(self, account_details):
		...
		terminal_framework.msgprint(message)

	def get_for_unrealized_gain_loss_account(self):
		unrealized_exchange_gain_loss_account = terminal_framework.get_cached_value(
...
def get_account_details(
	...
	terminal_framework.has_permission("Account", doc=account, throw=True)

	if not (company and posting_date):
		terminal_framework.throw(_("Company and Posting Date is mandatory"))

	account_currency, account_type = terminal_framework.get_cached_value(
		"Account", account, ["account_currency", "account_type"]
	)

	if account_type in ["Receivable", "Payable"] and not (party_type and party):
		terminal_framework.throw(_("Party Type and Party is mandatory for {0} account").format(account_type))

	account_details = {}
	terminal_erp.get_company_currency(company)
