...
terminal_framework.ui.form.on("POS Closing Entry", {
	onload: async function (frm) {
		...
		if (frm.doc.docstatus === 0 && !frm.doc.amended_from)
			frm.set_value("period_end_date", terminal_framework.datetime.now_datetime());

		terminal_framework.realtime.on("closing_process_complete", async function (data) {
		...
		if (frm.doc.docstatus == 1) {
			if (!frm.doc.posting_date) {
				frm.set_value("posting_date", terminal_framework.datetime.nowdate());
			}
	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 1 && frm.doc.status == "Failed") {
			...
			$("#jump_to_error").on("click", (e) => {
				e.preventDefault();
				terminal_framework.utils.scroll_to(cur_frm.get_field("error_message").$wrapper, true, 30);
			});
	...
	pos_opening_entry(frm) {
		if (
			...
			terminal_framework.run_serially([
				() => terminal_framework.dom.freeze(__("Loading Invoices! Please Wait...")),
	...
	get_invoices(frm) {
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.pos_closing_entry.pos_closing_entry.get_invoices",
			args: {
				start: terminal_framework.datetime.get_datetime_as_string(frm.doc.period_start_date),
				end: terminal_framework.datetime.get_datetime_as_string(frm.doc.period_end_date),
...
terminal_framework.ui.form.on("POS Closing Entry Detail", {
	closing_amount: (frm, cdt, cdn) => {
		const row = locals[cdt][cdn];
		terminal_framework.model.set_value(cdt, cdn, "difference", flt(row.closing_amount - row.expected_amount));
	},
});
