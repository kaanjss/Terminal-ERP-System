...
terminal_framework.ui.form.on("Bank Statement Import", {
	onload(frm) {
	...
	setup(frm) {
		terminal_framework.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			terminal_framework.model.clear_doc("Bank Statement Import", frm.doc.name);
			terminal_framework.model.with_doc("Bank Statement Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		terminal_framework.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", terminal_framework.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				terminal_framework.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = terminal_framework.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		...
		terminal_framework.call({
			method: "terminal_erp.accounts.doctype.bank_statement_import.bank_statement_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button(__("Report Error"), () => {
							...
							terminal_framework.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template() {
		let method = "/api/method/terminal_framework.core.doctype.data_import.data_import.download_template";
	...
	toggle_submit_after_import(frm) {
		...
		if (doctype) {
			terminal_framework.model.with_doctype(doctype, () => {
				let meta = terminal_framework.get_meta(doctype);
	...
	// method: 'terminal_framework.core.doctype.data_import.data_import.get_preview_from_template',
	...
	show_import_preview(frm, preview_data) {
		...
		terminal_framework.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new terminal_framework.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/terminal_erp.accounts.doctype.bank_statement_import.bank_statement_import.download_errored_template",
			{
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/terminal_erp.accounts.doctype.bank_statement_import.bank_statement_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		terminal_framework.call({
			method: "terminal_erp.accounts.doctype.bank_statement_import.bank_statement_import.get_import_logs",
			args: {
				docname: frm.doc.name,
			},
			callback: function (r) {
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${terminal_framework.utils.get_form_link(
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = terminal_framework.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		terminal_framework.call({
			method: "terminal_framework.client.get_count",
			args: {
