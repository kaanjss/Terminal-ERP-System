...
terminal_framework.ui.form.on("Shareholder", {
	refresh: function (frm) {
		...
		if (frm.doc.folio_no != undefined) {
			frm.add_custom_button(__("Share Balance"), function () {
				...
				terminal_framework.set_route("query-report", "Share Balance");
			});
			frm.add_custom_button(__("Share Ledger"), function () {
				terminal_framework.route_options = {
					shareholder: frm.doc.name,
				};
				terminal_framework.set_route("query-report", "Share Ledger");
			});
