...
from terminal_erp.accounts.doctype.pos_profile.pos_profile import (
	get_child_nodes,
)
...
class TestPOSProfile(Terminal ERPTestSuite):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = terminal_framework.get_doc("POS Profile", pos_profile.get("name"))
			...
			products_count = terminal_framework.db.count("Item", {"item_group": "_Test Item Group"})
			customers_count = terminal_framework.db.count("Customer", {"customer_group": "_Test Customer Group"})
	...
	def test_disabled_pos_profile_creation(self):
		...
		pos_profile = terminal_framework.get_doc("POS Profile", "_Test POS Profile 001")
	...
	def test_disabled_pos_profile_after_opening(self):
		...
		if pos_profile:
			...
			self.assertRaises(terminal_framework.ValidationError, pos_profile.save)
	...
	def test_disabled_pos_profile_after_completing_session(self):
		...
		from terminal_erp.accounts.doctype.pos_closing_entry.test_pos_closing_entry import init_user_and_profile
		from terminal_erp.accounts.doctype.pos_opening_entry.test_pos_opening_entry import (
...
def make_pos_profile(**args):
	terminal_framework.db.delete("POS Payment Method")
	terminal_framework.db.delete("POS Profile")

	args = terminal_framework._dict(args)

	pos_profile = terminal_framework.get_doc(
		{
			...
			"customer_group": terminal_framework.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	mode_of_payment = terminal_framework.get_doc("Mode of Payment", "Cash")
	company = args.company or "_Test Company"
	default_account = args.income_account or "Sales - _TC"

	if not terminal_framework.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not terminal_framework.db.exists("POS Profile", args.name or "_Test POS Profile"):
