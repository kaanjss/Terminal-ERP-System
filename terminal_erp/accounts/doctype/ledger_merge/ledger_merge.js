...
terminal_framework.ui.form.on("Ledger Merge", {
	setup: function (frm) {
		terminal_framework.realtime.on("ledger_merge_refresh", ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			terminal_framework.model.clear_doc(frm.doc.doctype, frm.doc.name);
			terminal_framework.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		terminal_framework.realtime.on("ledger_merge_progress", (data) => {
		...
		frm.set_query("account", function (doc) {
			if (!doc.company) terminal_framework.throw(__("Please set Company"));
			if (!doc.root_type) terminal_framework.throw(__("Please set Root Type"));
			return {
				filters: {
		...
		frm.set_query("account", "merge_accounts", function (doc) {
			if (!doc.company) terminal_framework.throw(__("Please set Company"));
			if (!doc.root_type) terminal_framework.throw(__("Please set Root Type"));
			if (!doc.account) terminal_framework.throw(__("Please set Account"));
			let acc = [doc.account];
...
terminal_framework.ui.form.on("Ledger Merge Accounts", {
