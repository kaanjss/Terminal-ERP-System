...
terminal_erp.accounts.taxes.setup_tax_validations("Payment Entry");
terminal_erp.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");

terminal_framework.ui.form.on("Payment Entry", {
	onload: function (frm) {
	...
	setup: function (frm) {
		frm.set_query("paid_from", function () {
			...
			var account_types = ["Pay", "Internal Transfer"].includes(frm.doc.payment_type)
				? ["Bank", "Cash"]
				: [terminal_framework.boot.party_account_types[frm.doc.party_type]];
		...
		frm.set_query("contact_person", function () {
			if (frm.doc.party) {
				return {
					query: "terminal_framework.contacts.doctype.contact.contact.contact_query",
					filters: {
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "terminal_erp.controllers.queries.get_payment_terms_for_references",
					filters: {
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			...
			return {
				query: "terminal_erp.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	refresh: function (frm) {
		terminal_erp.hide_company(frm);
		frm.events.hide_unhide_fields(frm);
		frm.events.set_dynamic_labels(frm);
		frm.events.show_general_ledger(frm);
		terminal_erp.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.set_route("List", "Journal Entry", {
		...
		terminal_framework.flags.allocate_payment_amount = true;
	},
	...
	hide_unhide_fields: function (frm) {
		var company_currency = frm.doc.company
			? terminal_framework.get_doc(":Company", frm.doc.company)?.default_currency
			: "";
	...
	mode_of_payment: function (frm) {
		terminal_erp.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
	...
	party_type: function (frm) {
		let party_types = Object.keys(terminal_framework.boot.party_account_types);
		if (frm.doc.party_type && !party_types.includes(frm.doc.party_type)) {
			frm.set_value("party_type", "");
			terminal_framework.throw(__("Party can only be one of {0}", [party_types.join(", ")]));
		}

		frm.set_query("party", function () {
			if (frm.doc.party_type == "Employee") {
				return {
					query: "terminal_erp.controllers.queries.employee_query",
				};
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			if (!frm.doc.posting_date) {
				terminal_framework.msgprint(__("Please select Posting Date before selecting Party"));
				frm.set_value("party", "");
				return;
			}

			terminal_erp.utils.get_employee_contact_details(frm);
			...
			return terminal_framework.call({
				method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	apply_tds: function (frm) {
		if (!frm.doc.apply_tds) {
			...
			terminal_framework.db.get_value(frm.doc.party_type, frm.doc.party, "tax_withholding_category", (values) => {
	...
	set_account_currency_and_balance: function (frm, account, currency_field, callback_function) {
		var company_currency = terminal_framework.get_doc(":Company", frm.doc.company).default_currency;
		if (frm.doc.posting_date && account) {
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", terminal_erp.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", terminal_erp.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_invoices_or_orders: function (frm, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		let fields = [
			{ fieldtype: "Section Break", label: __("Posting Date") },
			{
				...
				default: terminal_framework.datetime.add_days(today, -30),
			},
	...
	validate_filters_data: function (frm, filters) {
		...
		for (let key in fields) {
			...
			if (filters[from_field] && !filters[to_field]) {
				terminal_framework.throw(__("Error: {0} is a mandatory field", [to_field.replace(/_/g, " ")]));
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		terminal_framework.flags.allocate_payment_amount = filters["allocate_payment_amount"];

		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	allocate_party_amount_against_ref_docs: async function (frm, paid_amount, paid_amount_change) {
		await frm.call("allocate_amount_to_references", {
			...
			allocate_payment_amount: terminal_framework.flags.allocate_payment_amount ?? false,
		});
	...
	set_difference_amount: function (frm) {
		...
		var total_deductions = terminal_framework.utils.sum(
	...
	check_mandatory_to_fetch: function (frm) {
		$.each(["Company", "Party Type", "Party", "payment_type"], function (i, field) {
			if (!frm.doc[terminal_framework.model.scrub(field)]) {
				terminal_framework.msgprint(__("Please select {0} first", [field]));
				return false;
			}
		});
	},
	...
	validate_reference_document: function (frm, row) {
		var _validate = function (i, row) {
			...
			if (
				...
				terminal_framework.model.set_value(row.doctype, row.name, "reference_doctype", null);
			...
			if (
				...
				terminal_framework.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				terminal_framework.msgprint(
					__(
	...
	set_exchange_gain_loss_deduction: async function (frm) {
		...
		await terminal_framework.after_ajax();
		...
		if (!row) {
			const company_defaults = terminal_framework.get_doc(":Company", frm.doc.company);
			const account =
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
	...
	validate_taxes_and_charges: function (d) {
		...
		if (msg) {
			terminal_framework.validated = false;
			refresh_field("taxes");
	...
	after_save: function (frm) {
		const { matched_payment_requests } = terminal_framework.last_response;
		...
		terminal_framework.msgprint({
			...
			primary_action: {
				...
				action() {
					terminal_framework.hide_msgprint();
	...
	before_cancel: function (frm) {
		return new Promise((resolve, reject) => {
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_linked_bank_transactions",
				...
				callback: function (r) {
					...
					const bt_links = linked
						.map((name) => terminal_framework.utils.get_form_link("Bank Transaction", name, true))
						.join(", ");
...
terminal_framework.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return terminal_framework.call({
				method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
					reference_doctype: row.reference_doctype,
					reference_name: row.reference_name,
					party_account_currency:
				...
				callback: function (r, rt) {
					if (r.message) {
						$.each(r.message, function (field, value) {
							terminal_framework.model.set_value(cdt, cdn, field, value);
						});
						...
						terminal_framework.model.set_value(cdt, cdn, "allocated_amount", allocated_amount);
						frm.refresh_fields();
					}
				},
			});
...
terminal_framework.ui.form.on("Advance Taxes and Charges", {
	rate: function (frm) {
		frm.events.apply_taxes(frm);
		frm.events.set_unallocated_amount(frm);
	},
...
terminal_framework.ui.form.on("Payment Entry Deduction", {
	before_deductions_remove: function (doc, cdt, cdn) {
		const row = terminal_framework.get_doc(cdt, cdn);
		if (row.is_exchange_gain_loss && row.amount) {
			terminal_framework.throw(__("Cannot delete Exchange Gain/Loss row"));
		}
	},
...
function get_company_defaults(company) {
	return terminal_framework.call({
		method: "terminal_erp.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
		args: {
			company: company,
		},
	});
...
function prompt_for_missing_account(frm, account) {
	return new Promise((resolve) => {
		const dialog = terminal_framework.prompt(
			{
				label: __(terminal_framework.unscrub(account)),
				fieldname: account,
				fieldtype: "Link",
				options: "Account",
				get_query: () => ({
...
function get_deduction_amount_precision() {
	return terminal_framework.meta.get_field_precision(terminal_framework.meta.get_field("Payment Entry Deduction", "amount"));
}
