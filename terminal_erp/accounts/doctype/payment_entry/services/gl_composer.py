...
class PaymentEntryGLComposer(BaseGLComposer):
	...
	def compose(self):
		from terminal_erp.accounts.doctype.payment_entry.payment_entry import add_regional_gl_entries
	...
	def add_party_gl_entries(self, gl_entries):
		...
		party_account_type = terminal_framework.db.get_value("Party Type", doc.party_type, "account_type")
		...
		for d in doc.get("references"):
			...
			if d.reference_doctype == "Sales Invoice" and not cost_center:
				cost_center = terminal_framework.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
	...
	def add_tax_gl_entries(self, gl_entries):
		...
		for d in doc.get("taxes"):
			...
			if account_currency != doc.company_currency:
				terminal_framework.throw(_("Currency for {0} must be {1}").format(d.account_head, doc.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		...
		for d in doc.get("deductions"):
			...
			if account_currency != doc.company_currency:
				terminal_framework.throw(_("Currency for {0} must be {1}").format(d.account, doc.company_currency))
