...
from terminal_framework.utils import add_days, flt, nowdate
...
class TestPaymentEntry(Terminal ERPTestSuite):
	...
	def test_payment_entry_against_order(self):
		...
		so_advance_paid = terminal_framework.db.get_value("Sales Order", so.name, "advance_paid")
	...
	def test_payment_entry_against_si_usd_to_usd(self):
		...
		outstanding_amount = flt(terminal_framework.db.get_value("Sales Invoice", si.name, "outstanding_amount"))
	...
	def test_payment_entry_against_pi(self):
		...
		outstanding_amount = flt(terminal_framework.db.get_value("Sales Invoice", pi.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_payment_against_sales_invoice_to_check_status(self):
		...
		outstanding_amount, status = terminal_framework.db.get_value(
	...
	def test_payment_entry_against_payment_terms_with_discount_on_pi(self):
		...
		terminal_framework.db.set_value("Company", pi.company, "default_discount_account", "Write Off - _TC")
		...
		terminal_framework.db.set_single_value("Accounts Settings", "book_tax_discount_loss", 1)
		...
		terminal_framework.db.set_single_value("Accounts Settings", "book_tax_discount_loss", 0)
	...
	def test_payment_entry_against_payment_terms_with_discount(self):
		...
		terminal_framework.db.set_value("Company", si.company, "default_discount_account", "Write Off - _TC")
	...
	def test_payment_entry_against_payment_terms_with_discount_amount(self):
		...
		pe_1 = get_payment_entry(
			...
			reference_date=terminal_framework.utils.add_days(si.posting_date, 2),
		)
	...
	def test_payment_against_negative_sales_invoice(self):
		...
		si1_outstanding = terminal_framework.db.get_value("Sales Invoice", si1.name, "outstanding_amount")

		# create JE(credit note) manually against si1 and cr_note
		je = terminal_framework.get_doc(
			{
		...
		outstanding_amount = flt(terminal_framework.db.get_value("Sales Invoice", si1.name, "outstanding_amount"))
		self.assertEqual(outstanding_amount, 0)
	...
	def test_duplicate_payment_entry_allocate_amount(self):
		...
		self.assertRaises(terminal_framework.ValidationError, pe_draft.submit)
	...
	def test_overallocation_validation_shouldnt_misfire(self):
		...
		template = terminal_framework.get_doc("Payment Terms Template", "Test Receivable Template")
	...
	def test_payment_entry_merges_gl_entries_with_same_account_head(self):
		...
		gl_entries = terminal_framework.db.get_all(
			"GL Entry",
	...
	def test_payment_entry_does_not_merge_gl_entries_when_setting_disabled(self):
		...
		terminal_framework.db.set_single_value("Accounts Settings", "merge_similar_account_heads", 0)
	...
	def check_gl_entries(self):
		...
		gl_entries = (
			...
			.orderby(gle.account, gle.debit, gle.credit, order=terminal_framework.qb.desc)
		).run(as_dict=True)
	...
	def test_reverse_payment_reconciliation(self):
		customer = create_customer(terminal_framework.generate_hash(length=10), "INR")
		pe = create_payment_entry(
			party_type="Customer",
			party=customer,
			payment_type="Receive",
	...
	def test_opening_flag_for_advance_as_liability(self):
		...
		gl_with_opening_set = terminal_framework.db.get_all(
	...
	def test_delete_linked_exchange_gain_loss_journal(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Journal Entry", {"name": jv[0]}, "docstatus"), 2)

		# check deletion of payment entry and journal entry
		pe.delete()
		self.assertRaises(terminal_framework.DoesNotExistError, terminal_framework.get_doc, pe.doctype, pe.name)
		self.assertRaises(terminal_framework.DoesNotExistError, terminal_framework.get_doc, "Journal Entry", jv[0])
	...
	def test_project_name_in_exchange_gain_loss_entry(self):
		...
		rows = terminal_framework.get_all(
			"Journal Entry Account",
...
def create_payment_entry(**args):
	payment_entry = terminal_framework.new_doc("Payment Entry")
...
def create_payment_terms_template():
	...
	if not terminal_framework.db.exists("Payment Terms Template", "Test Receivable Template"):
		terminal_framework.get_doc(
			{
...
def create_payment_terms_template_with_discount(
	...
	if not terminal_framework.db.exists("Payment Terms Template", template_name):
		terminal_framework.get_doc(
			{
				"doctype": "Payment Terms Template",
...
def create_payment_term(name):
	if not terminal_framework.db.exists("Payment Term", name):
		terminal_framework.get_doc({"doctype": "Payment Term", "payment_term_name": name}).insert()


def create_customer(name="_Test Customer 2 USD", currency="USD"):
	customer = None
	if terminal_framework.db.exists("Customer", name):
		customer = name
	else:
		customer = terminal_framework.new_doc("Customer")
