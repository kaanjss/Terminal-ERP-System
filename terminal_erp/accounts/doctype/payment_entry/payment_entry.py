...
from terminal_framework import ValidationError, _, qb, scrub, throw
from terminal_framework.model.document import Document
from terminal_framework.model.meta import get_field_precision
from terminal_framework.query_builder import Case, Tuple
from terminal_framework.query_builder.functions import Abs, Count, Max
from terminal_framework.utils import cint, comma_or, flt, getdate, nowdate
from terminal_framework.utils.data import comma_and, fmt_money, get_link_to_form
from pypika.functions import Coalesce, Sum

import terminal_erp
from terminal_erp.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from terminal_erp.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_default_company_bank_account,
	get_party_bank_account,
)
from terminal_erp.accounts.doctype.invoice_discounting.invoice_discounting import (
	get_party_account_based_on_invoice_discounting,
)
from terminal_erp.accounts.doctype.journal_entry.journal_entry import get_default_bank_cash_account
from terminal_erp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import (
	validate_docs_for_deferred_accounting,
	validate_docs_for_voucher_types,
)
from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import PaymentTaxWithholding
from terminal_erp.accounts.general_ledger import (
	make_gl_entries,
...
from terminal_erp.controllers.accounts_controller import (
	AccountsController,
...
class PaymentEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.advance_taxes_and_charges.advance_taxes_and_charges import (
			AdvanceTaxesandCharges,
		)
		from terminal_erp.accounts.doctype.payment_entry_deduction.payment_entry_deduction import (
			PaymentEntryDeduction,
		)
		from terminal_erp.accounts.doctype.payment_entry_reference.payment_entry_reference import (
			PaymentEntryReference,
		)
		from terminal_erp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import TaxWithholdingEntry
	...
	def on_submit(self):
		if self.difference_amount:
			terminal_framework.throw(_("Difference Amount must be zero"))
	...
	def set_liability_account(self):
		...
		account_type = terminal_framework.get_value(
		...
		if not liability_account:
			throw(
				_("Please set default {0} in Company {1}").format(
					terminal_framework.bold(terminal_framework.get_meta("Company").get_label(fieldname)), terminal_framework.bold(self.company)
				)
			)
		...
		terminal_framework.msgprint(
			_(
				...
				terminal_framework.bold(self.party_account),
				terminal_framework.bold(liability_account),
			),
			alert=True,
		)
	...
	def validate_allocated_amount(self):
		...
		for d in self.get("references"):
			if (flt(d.allocated_amount)) > 0 and flt(d.allocated_amount) > flt(d.outstanding_amount):
				terminal_framework.throw(fail_message.format(d.idx))
	...
	def term_based_allocation_enabled_for_reference(
		...
		if (
			...
			if template := terminal_framework.db.get_value(reference_doctype, reference_name, "payment_terms_template"):
	...
	def validate_allocated_amount_with_latest_data(self):
		...
		vouchers = [terminal_framework._dict({"voucher_type": x[0], "voucher_no": x[1]}) for x in uniq_vouchers]
		...
		for d in latest_references:
			d = terminal_framework._dict(d)
			latest_lookup.setdefault((d.voucher_type, d.voucher_no), terminal_framework._dict())[d.payment_term] = d

		for idx, d in enumerate(self.get("references"), start=1):
			latest = latest_lookup.get((d.reference_doctype, d.reference_name)) or terminal_framework._dict()
			...
			if (
				...
				terminal_framework.throw(
					...
					).format(terminal_framework.bold(d.reference_name), terminal_framework.bold(idx))
				)
	...
	def delink_advance_entry_references(self):
		for reference in self.references:
			if reference.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				doc = terminal_framework.get_lazy_doc(reference.reference_doctype, reference.reference_name)
	...
	def trigger_invoice_update_for_subscriptions(self):
		...
		for doctype, name in invoice_names:
			terminal_framework.db.savepoint("subscription_update")
			try:
				doc = terminal_framework.get_doc(doctype, name)
				doc.refresh_subscription_status()
			except Exception:
				terminal_framework.db.rollback(save_point="subscription_update")
				terminal_framework.log_error(_("Failed to update subscription status for {0} {1}").format(doctype, name))
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				terminal_framework.throw(_("Party Type is mandatory"))

			if not self.party:
				terminal_framework.throw(_("Party is mandatory"))

			_party_name = "title" if self.party_type == "Shareholder" else self.party_type.lower() + "_name"

			if terminal_framework.db.has_column(self.party_type, _party_name):
				self.party_name = terminal_framework.db.get_value(self.party_type, self.party, _party_name)
			else:
				self.party_name = terminal_framework.db.get_value(self.party_type, self.party, "name")
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			terminal_framework.throw(_("Payment Type must be one of Receive, Pay, or Internal Transfer"))

	def validate_party_details(self):
		if self.party and not terminal_framework.db.exists(self.party_type, self.party):
			terminal_framework.throw(_("{0} {1} does not exist").format(_(self.party_type), self.party))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				terminal_framework.throw(_("{0} is mandatory").format(_(self.meta.get_label(field))))
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not terminal_framework.db.exists(d.reference_doctype, d.reference_name):
					terminal_framework.throw(_("{0} {1} does not exist").format(_(d.reference_doctype), d.reference_name))

				ref_doc = terminal_framework.get_lazy_doc(d.reference_doctype, d.reference_name)
				...
				if d.reference_doctype in terminal_framework.get_hooks("invoice_doctypes"):
					if self.party_type == "Customer":
						ref_party_account = (
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				outstanding_amount, is_return = terminal_framework.get_cached_value(
		...
		for reference_doctype, references in no_oustanding_refs.items():
			terminal_framework.msgprint(
				_(
					...
					terminal_framework.bold(comma_and([d.reference_name for d in references])),
					_(reference_doctype),
				)
				+ "<br><br>"
	...
	def validate_journal_entry(self):
		for d in self.get("references"):
			if d.allocated_amount and d.reference_doctype == "Journal Entry":
				je_accounts = terminal_framework.get_all(
	...
	def update_payment_schedule(self, cancel=0):
		...
		for ref in self.get("references"):
			...
			if not invoice_paid_amount_map.get(key):
				payment_schedule = terminal_framework.get_all(
					"Payment Schedule",
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				terminal_framework.throw(_("Payment term {0} not used in {1}").format(key[0], key[1]))
			...
			conversion_rate = terminal_framework.db.get_value(key[2], {"name": key[1]}, "conversion_rate")
			base_paid_amount_precision = get_field_precision(
				terminal_framework.get_meta("Payment Schedule").get_field("base_paid_amount")
			)
			base_outstanding_precision = get_field_precision(
				terminal_framework.get_meta("Payment Schedule").get_field("base_outstanding")
			)
			...
			ps = terminal_framework.qb.DocType("Payment Schedule")
			if cancel:
				(
					terminal_framework.qb.update(ps)
	...
	def get_allocated_amount_in_transaction_currency(
		...
		* PE in INR (accounting done in base currency)
		"""
		ref_currency, ref_exchange_rate = terminal_framework.db.get_value(
	...
	def set_total_in_words(self):
		from terminal_framework.utils import money_in_words
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				terminal_framework.throw(_("Received Amount cannot be greater than Paid Amount"))
	...
	def set_exchange_gain_loss(self):
		...
		if not exchange_gain_loss_row:
			values = terminal_framework.get_cached_value(
			...
			for fieldname, value in values.items():
				...
				label = _(terminal_framework.get_meta("Company").get_label(fieldname))
				return terminal_framework.msgprint(
	...
	def clear_unallocated_reference_document_rows(self):
		...
		terminal_framework.db.delete("Payment Entry Reference", {"parent": self.name, "allocated_amount": 0})

	def set_title(self):
		if terminal_framework.flags.in_import and self.title:
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = terminal_framework.get_cached_value("Account", bank_account, "account_type")

		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				terminal_framework.throw(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def set_transaction_currency_and_rate(self):
		company_currency = terminal_erp.get_company_currency(self.company)
	...
	def build_gl_map(self):
		from terminal_erp.accounts.doctype.payment_entry.services.gl_composer import PaymentEntryGLComposer
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		merge_entries = terminal_framework.get_single_value("Accounts Settings", "merge_similar_account_heads")
		...
		if cancel:
			cancel_exchange_gain_loss_journal(terminal_framework._dict(doctype=self.doctype, name=self.name))
		else:
	...
	def add_advance_gl_for_reference(self, gl_entries, invoice):
		...
		if invoice.reconcile_effect_on:
			...
			terminal_framework.db.set_value("Payment Entry Reference", invoice.name, "reconcile_effect_on", posting_date)
	...
	def calculate_deductions(self, tax_details):
		return {
			...
			"cost_center": terminal_framework.get_cached_value("Company", self.company, "cost_center"),
	...
	def set_matched_unset_payment_requests_to_response(self):
		...
		And set to `terminal_framework.response` to show in the frontend for allocation.
		"""
		if not self.references:
			return
		...
		terminal_framework.response["matched_payment_requests"] = matched_payment_requests
	...
	def set_matched_payment_requests(self, matched_payment_requests: str | list | None):
		...
		matched_payment_requests = terminal_framework.parse_json(matched_payment_requests)
...
def get_matched_payment_request_of_references(references=None):
	...
	matched_prs = (
		terminal_framework.qb.from_(subquery)
		.select(
...
def get_outstanding_of_references_with_payment_term(references=None):
	...
	response = (
		terminal_framework.qb.from_(PS)
...
def validate_inclusive_tax(tax, doc):
	...
	if cint(getattr(tax, "included_in_paid_amount", None)):
		if tax.charge_type == "Actual":
			...
			terminal_framework.throw(_("Valuation type charges can not be marked as Inclusive"))
...
def get_outstanding_reference_documents(args: str | dict, validate: bool = False):
	...
	if args.get("party_type") and args.get("party"):
		terminal_framework.has_permission(args["party_type"], "read", args["party"], throw=True)
	...
	company_currency = terminal_framework.get_cached_value("Company", args.get("company"), "default_currency")

	# Get positive outstanding sales /purchase invoices
	condition = ""
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = f" and voucher_type={terminal_framework.db.escape(args['voucher_type'])} and voucher_no={terminal_framework.db.escape(args['voucher_no'])}"
		common_filter.append(ple.voucher_type == args["voucher_type"])
		common_filter.append(ple.voucher_no == args["voucher_no"])

	# Add cost center condition
	if args.get("cost_center"):
		condition += f" and cost_center={terminal_framework.db.escape(args.get('cost_center'))}"
	...
	for dim in active_dimensions:
		if args.get(dim.fieldname):
			condition += f" and {dim.fieldname}={terminal_framework.db.escape(args.get(dim.fieldname))}"
	...
	for fieldname, date_fields in date_fields_dict.items():
		from_date = terminal_framework.db.escape(str(args.get(date_fields[0]))) if args.get(date_fields[0]) else None
		to_date = terminal_framework.db.escape(str(args.get(date_fields[1]))) if args.get(date_fields[1]) else None
	...
	if args.get("company"):
		condition += " and company = {}".format(terminal_framework.db.escape(args.get("company")))
	...
	if args.get("get_outstanding_invoices"):
		...
		for d in outstanding_invoices:
			...
			if party_account_currency != company_currency:
				if d.voucher_type in terminal_framework.get_hooks("invoice_doctypes"):
					d["exchange_rate"] = terminal_framework.db.get_value(d.voucher_type, d.voucher_no, "conversion_rate")
				elif d.voucher_type == "Journal Entry":
					d["exchange_rate"] = get_exchange_rate(
						party_account_currency, company_currency, d.posting_date
					)
			if d.voucher_type in ("Purchase Invoice"):
				d["bill_no"] = terminal_framework.db.get_value(d.voucher_type, d.voucher_no, "bill_no")
		...
		if not validate:
			terminal_framework.msgprint(
				_(
					...
					_(ref_document_type), _(args.get("party_type")).lower(), terminal_framework.bold(args.get("party"))
				)
			)
...
def split_refdocs_based_on_payment_terms(refdocs, company) -> list:
	...
	for entry in refdocs:
		if entry.voucher_type in ["Sales Invoice", "Purchase Invoice", "Sales Order", "Purchase Order"]:
			if payment_term_template := terminal_framework.db.get_value(
				...
				if len(split_rows) > 1:
					terminal_framework.msgprint(
						_("Splitting {0} {1} into {2} rows as per Payment Terms").format(
							_(entry.voucher_type), terminal_framework.bold(entry.voucher_no), len(split_rows)
						),
						alert=True,
					)
...
def get_currency_data(outstanding_refdocs: list, company: str | None = None) -> dict:
	...
	exc_rates = terminal_framework._dict()
	company_currency = terminal_framework.db.get_value("Company", company, "default_currency") if company else None
...
def get_split_invoice_rows(invoice: dict, payment_term_template: str, exc_rates: dict) -> list:
	...
	split_rows = []
	allocate_payment_based_on_payment_terms = terminal_framework.db.get_value(
...
def get_orders_to_be_billed(
	...
	voucher = terminal_framework.qb.DocType(voucher_type)
	invoice_amount = (
		Case()
	...
	query = (
		terminal_framework.qb.from_(voucher)
		.select(
...
def get_negative_outstanding_invoices(
	...
	return terminal_framework.db.sql(
		"""
		select
...
def get_party_details(company: str, party_type: str, party: str, date: str, cost_center: str | None = None):
	...
	if not terminal_framework.db.exists(party_type, party):
		terminal_framework.throw(_("{0} {1} does not exist").format(_(party_type), party))

	party_account = get_party_account(party_type, party, company)
	account_currency = get_account_currency(party_account)
	_party_name = "title" if party_type == "Shareholder" else party_type.lower() + "_name"
	party_name = terminal_framework.db.get_value(party_type, party, _party_name)
...
def get_account_details(account: str, date: str | date, cost_center: str | None = None):
	terminal_framework.has_permission("Payment Entry", throw=True)

	# to check if the passed account is accessible under reference doctype Payment Entry
	account_list = terminal_framework.get_list("Account", {"name": account}, reference_doctype="Payment Entry", limit=1)

	# There might be some user permissions which will allow account under certain doctypes
	# except for Payment Entry, only in such case we should throw permission error
	if not account_list:
		terminal_framework.throw(_("Account: {0} is not permitted under Payment Entry").format(account))

	return terminal_framework._dict(
		{
			"account_currency": get_account_currency(account),
			"account_type": terminal_framework.get_cached_value("Account", account, "account_type"),
		}
	)
...
def get_company_defaults(company: str):
	...
	return terminal_framework.get_cached_value("Company", company, fields, as_dict=1)
...
def get_outstanding_on_journal_entry(voucher_no, party_type, party):
	...
	outstanding = (
		terminal_framework.qb.from_(ple)
...
def get_reference_details(
	...
	terminal_framework.has_permission(reference_doctype, "read", reference_name, throw=True)
	ref_doc = terminal_framework.get_lazy_doc(reference_doctype, reference_name)
	company_currency = ref_doc.get("company_currency") or terminal_erp.get_company_currency(ref_doc.company)
...
def get_payment_entry(
	...
	doc = terminal_framework.get_doc(dt, dn)
	over_billing_allowance = terminal_framework.get_single_value("Accounts Settings", "over_billing_allowance")
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) >= (100.0 + over_billing_allowance):
		terminal_framework.throw(_("Can only make payment against unbilled {0}").format(_(dt)))
	...
	if party_type in ["Customer", "Supplier"] and not bank:
		...
		if party_bank_account:
			account = terminal_framework.db.get_value("Bank Account", party_bank_account, "account")
	...
	pe.paid_from_account_type = terminal_framework.db.get_value("Account", pe.paid_from, "account_type")
	pe.paid_to_account_type = terminal_framework.db.get_value("Account", pe.paid_to, "account_type")
	pe.paid_amount = paid_amount
	pe.received_amount = received_amount
	pe.letter_head = doc.get("letter_head")
	pe.bank_account = terminal_framework.db.get_value(
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		terminal_framework.msgprint(_("{0} is on hold until {1}").format(doc.name, doc.release_date))
	else:
		if doc.doctype in (
			"Sales Invoice",
			"Purchase Invoice",
			"Purchase Order",
			"Sales Order",
		) and terminal_framework.get_cached_value(
	...
	if party_account and bank:
		if discount_amount:
			...
			if terminal_framework.get_single_value("Accounts Settings", "book_tax_discount_loss"):
...
def get_open_payment_requests_for_references(references=None):
	...
	response = (
		...
		.orderby(Coalesce(PR.transaction_date, PR.creation), order=terminal_framework.qb.asc)
	).run(as_dict=True)
	...
	while row_number <= len(references):
		...
		if pr_outstanding_amount == allocated_amount:
			...
			while allocated_amount:
				...
				new_row = terminal_framework.copy_doc(row)
...
def set_paid_amount_and_received_amount(
	...
	if party_account_currency == bank.account_currency:
		...
		if bank_amount:
			...
			company_currency = terminal_framework.get_cached_value("Company", doc.get("company"), "default_currency")
...
def apply_early_payment_discount(paid_amount, received_amount, doc, party_account_currency, reference_date):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			...
			money = terminal_framework.utils.fmt_money(total_discount, currency=currency)
			terminal_framework.msgprint(_("Discount of {0} applied as per Payment Term").format(money), alert=1)
...
def set_pending_discount_loss(pe, doc, discount_amount, base_total_discount_loss, party_account_currency):
	...
	if discount_amount > 0.0:
		...
		book_tax_loss = terminal_framework.get_single_value("Accounts Settings", "book_tax_discount_loss")
		account_type = "round_off_account" if book_tax_loss else "default_discount_account"

		pe.append(
			"deductions",
			{
				"account": terminal_framework.get_cached_value("Company", pe.company, account_type),
				"cost_center": pe.cost_center
				or terminal_framework.get_cached_value("Company", pe.company, "cost_center"),
...
def add_income_discount_loss(pe, doc, total_discount_percent) -> float:
	...
	pe.append(
		"deductions",
		{
			"account": terminal_framework.get_cached_value("Company", pe.company, "default_discount_account"),
			"cost_center": pe.cost_center or terminal_framework.get_cached_value("Company", pe.company, "cost_center"),
...
def get_linked_bank_transactions(payment_entry: str) -> list:
	terminal_framework.has_permission("Payment Entry", ptype="read", doc=payment_entry, throw=True)
