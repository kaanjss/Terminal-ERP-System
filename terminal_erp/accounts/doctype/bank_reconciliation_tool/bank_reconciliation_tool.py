...
from terminal_framework.query_builder.custom import ConstantColumn
from terminal_framework.query_builder.functions import Max, Sum
from terminal_framework.utils import cint, create_batch, flt

from terminal_erp import get_default_cost_center
from terminal_erp.accounts.doctype.bank_transaction.bank_transaction import get_total_allocated_amount
...
from terminal_erp.accounts.utils import get_account_currency, get_balance_on
...
def get_bank_transactions(
	...
	transactions = terminal_framework.get_list(
		"Bank Transaction",
		fields=[
			"date",
			"deposit",
...
def get_account_balance(bank_account: str, till_date: str | date, company: str):
	...
	terminal_framework.has_permission("Bank Account", "read", bank_account, throw=True)
	account = terminal_framework.db.get_value("Bank Account", bank_account, "account")
...
def update_bank_transaction(
	...
	bank_transaction = terminal_framework.get_doc("Bank Transaction", bank_transaction_name)
...
def create_journal_entry_bts(
	...
	bank_transaction = terminal_framework.db.get_values(
		"Bank Transaction",
		bank_transaction_name,
		fieldname=["name", "deposit", "withdrawal", "bank_account", "currency"],
		as_dict=True,
	)[0]
	company_account = terminal_framework.get_value("Bank Account", bank_transaction.bank_account, "account")
	account_type = terminal_framework.db.get_value("Account", second_account, "account_type")
	...
	company = terminal_framework.get_value("Account", company_account, "company")
	company_default_currency = terminal_framework.get_cached_value("Company", company, "default_currency")
	company_account_currency = terminal_framework.get_cached_value("Account", company_account, "account_currency")
	second_account_currency = terminal_framework.get_cached_value("Account", second_account, "account_currency")
...
def create_payment_entry_bts(
	...
	bank_account = terminal_framework.get_cached_value("Bank Account", bank_transaction.bank_account, "account")
	company = terminal_framework.get_cached_value("Account", bank_account, "company")
	party_account = get_party_account(party_type, party, company)

	bank_currency = bank_transaction.currency
	party_currency = terminal_framework.get_cached_value("Account", party_account, "account_currency")
...
def get_older_unreconciled_transactions(bank_account: str, from_date: str):
	...
	"""
	count = terminal_framework.db.count(
		"Bank Transaction",
		filters={
	...
	if count > 0:
		oldest_transaction = terminal_framework.db.get_list(
			"Bank Transaction",
			filters={
				"bank_account": bank_account,
				"date": ["<", from_date],
...
def update_clearance_date(
	...
	terminal_framework.has_permission("Bank Clearance", ptype="write", throw=True)
	...
	if payment_document == "Sales Invoice":
		...
		terminal_framework.db.set_value(payment_document, payment_entry, "clearance_date", clearance_date)
...
def clear_clearing_date(voucher_type: str, voucher_name: str):
	...
	payment_entry = terminal_framework.get_doc(voucher_type, voucher_name)
...
def create_bulk_internal_transfer(bank_transaction_names: list[str | int], bank_account: str):
	...
	for bank_transaction_name in bank_transaction_names:
		bank_transaction = terminal_framework.db.get_value(
			"Bank Transaction",
			bank_transaction_name,
		...
		transaction_account = terminal_framework.get_cached_value(
...
def create_bulk_bank_entry_and_reconcile(bank_transactions: list[str | int], account: str):
	...
	for bank_transaction in bank_transactions:
		transactions_details = terminal_framework.db.get_value(
			"Bank Transaction",
			bank_transaction,
			[
				"name",
		...
		is_credit_card = terminal_framework.get_cached_value(
		...
		gl_account = terminal_framework.get_cached_value("Bank Account", transactions_details.bank_account, "account")
...
def create_bank_entry_and_reconcile(
	...
	bank_entry = terminal_framework.get_doc(
		{
	...
	for entry in entries:
		...
		if not cost_center:
			report_type = terminal_framework.get_cached_value("Account", entry["account"], "report_type")
...
def create_bulk_payment_entry_and_reconcile(
	...
	for bank_transaction_name in bank_transaction_names:
		...
		payment_entry_doc = terminal_framework.get_doc(
			{
...
def create_payment_entry_and_reconcile(bank_transaction_name: str | int, payment_entry_doc: dict):
	...
	Create a payment entry and reconcile it with the bank transaction
	"""
	payment_entry = terminal_framework.get_doc(
		{
			**payment_entry_doc,
			"doctype": "Payment Entry",
		}
...
def search_for_transfer_transaction(transaction_id: str | int):
	...
	"""
	company, withdrawal, deposit, date, bank_account = terminal_framework.db.get_value(
		"Bank Transaction", transaction_id, ["company", "withdrawal", "deposit", "date", "bank_account"]
	)

	days = terminal_framework.db.get_single_value("Accounts Settings", "transfer_match_days")

	if not days:
		days = 3

	min_date = terminal_framework.utils.add_days(date, -days)
	max_date = terminal_framework.utils.add_days(date, days)
	mirror_tx = terminal_framework.db.get_list(
		"Bank Transaction",
		filters={
			"company": company,
	...
	if len(mirror_tx) == 1:
		return {
			...
			"account": terminal_framework.get_cached_value("Bank Account", mirror_tx[0].bank_account, "account"),
		}
	...
	if len(bank_transactions) > 10:
		for bank_transaction_batch in create_batch(bank_transactions, 1000):
			terminal_framework.enqueue(
				method="terminal_erp.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.start_auto_reconcile",
				queue="long",
		...
		terminal_framework.msgprint(_("Auto Reconciliation has started in the background"))
	else:
...
def start_auto_reconcile(
	...
	terminal_framework.flags.auto_reconcile_vouchers = True
	...
	terminal_framework.msgprint(title=_("Auto Reconciliation"), msg=alert_message, indicator=indicator)

	terminal_framework.flags.auto_reconcile_vouchers = False
...
def reconcile_vouchers(bank_transaction_name: str | int, vouchers: str | list, is_new_voucher: bool = False):
	...
	vouchers = terminal_framework.parse_json(vouchers)
	transaction = terminal_framework.get_doc("Bank Transaction", bank_transaction_name)
...
def get_linked_payments(
	...
	transaction = terminal_framework.get_doc("Bank Transaction", bank_transaction_name)
	bank_account = terminal_framework.db.get_values(
...
def get_queries(
	...
	for method_name in terminal_framework.get_hooks("get_matching_queries"):
		queries.extend(
			terminal_framework.get_attr(method_name)(
				bank_account,
				company,
				transaction,
				document_types,
...
def get_bt_matching_query(exact_match, transaction):
	...
	bt = terminal_framework.qb.DocType("Bank Transaction")

	field = "deposit" if transaction.withdrawal > 0.0 else "withdrawal"
	amount_equality = getattr(bt, field) == transaction.unallocated_amount
	amount_rank = terminal_framework.qb.terms.Case().when(amount_equality, 1).else_(0)
	amount_condition = amount_equality if exact_match else getattr(bt, field) > 0.0

	ref_rank = terminal_framework.qb.terms.Case().when(bt.reference_number == transaction.reference_number, 1).else_(0)
	unallocated_rank = (
		terminal_framework.qb.terms.Case().when(bt.unallocated_amount == transaction.unallocated_amount, 1).else_(0)
	)

	party_condition = (
		(bt.party_type == transaction.party_type) & (bt.party == transaction.party) & bt.party.isnotnull()
	)
	party_rank = terminal_framework.qb.terms.Case().when(party_condition, 1).else_(0)

	query = (
		terminal_framework.qb.from_(bt)
		.select(
...
def get_pe_matching_query(
	...
	ref_rank = terminal_framework.qb.terms.Case().when(ref_condition, 1).else_(0)
	...
	if terminal_framework.flags.auto_reconcile_vouchers is True:
...
def get_je_matching_query(
	...
	ref_rank = terminal_framework.qb.terms.Case().when(subquery.reference_no == transaction.reference_number, 1).else_(0)
...
def get_pi_matching_query(exact_match, currency, common_filters):
	...
	purchase_invoice = terminal_framework.qb.DocType("Purchase Invoice")
	...
	query = (
		terminal_framework.qb.from_(purchase_invoice)
		.select(
