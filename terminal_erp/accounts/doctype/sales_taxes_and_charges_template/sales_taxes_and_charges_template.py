	...
	def set_missing_values(self):
		for data in self.taxes:
			if data.charge_type == "On Net Total" and flt(data.rate) == 0.0:
				data.rate = terminal_framework.get_cached_value("Account", data.account_head, "tax_rate")


def valdiate_taxes_and_charges_template(doc):
	# default should not be disabled
	# if not doc.is_default and not terminal_framework.get_all(doc.doctype, filters={"is_default": 1}):
	# 	doc.is_default = 1

	if doc.is_default == 1:
		template = terminal_framework.qb.DocType(doc.doctype)
		(
			terminal_framework.qb.update(template)
...
def validate_disabled(doc):
	if doc.is_default and doc.disabled:
		terminal_framework.throw(_("Disabled template must not be default template"))
...
def validate_for_tax_category(doc):
	...
	if terminal_framework.db.exists(
		...
		terminal_framework.throw(
			...
			).format(terminal_framework.bold(doc.tax_category))
		)
