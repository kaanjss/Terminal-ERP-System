...
from terminal_erp.accounts.doctype.subscription.subscription import Subscription, get_prorata_factor, process_all
from terminal_erp.accounts.utils import update_subscription_on_invoice_update
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSubscription(Terminal ERPTestSuite):
	def setUp(self):
		make_plans()
		create_parties()
		reset_settings()
	...
	def test_create_subscription_trial_with_wrong_dates(self):
		...
		self.assertRaises(terminal_framework.ValidationError, subscription.save)
	...
	def test_subscription_cancel_after_grace_period(self):
		settings = terminal_framework.get_single("Subscription Settings")
	...
	def test_restart_active_subscription(self):
		subscription = create_subscription()
		self.assertRaises(terminal_framework.ValidationError, subscription.restart_subscription)
	...
	def test_subscription_with_follow_calendar_months(self):
		subscription = terminal_framework.new_doc("Subscription")
		...
		self.assertEqual(len(subscription.invoices), 1)
		self.assertEqual(
			getdate(terminal_framework.db.get_value("Purchase Invoice", subscription.invoices[0].name, "to_date")),
			getdate("2018-03-31"),
		)
	...
	def test_multi_currency_subscription(self):
		...
		terminal_framework.db.set_value("Customer", party, "default_currency", "USD")
		subscription = create_subscription(
			start_date="2018-01-01",
			generate_invoice_at="Prepaid (bill at period start)",
		...
		currency = terminal_framework.db.get_value("Sales Invoice", subscription.invoices[0].name, "currency")
	...
	def test_subscription_recovery(self):
		...
		self.assertEqual(
			getdate(terminal_framework.db.get_value("Sales Invoice", subscription.invoices[0].name, "from_date")),
			getdate("2021-01-01"),
		)
		self.assertEqual(
			getdate(terminal_framework.db.get_value("Sales Invoice", subscription.invoices[1].name, "from_date")),
			getdate("2021-02-01"),
		)
	...
	def test_subscription_auto_cancellation_uneven_cycle(self):
		...
		self.assertRaises(terminal_framework.ValidationError, subscription.process, posting_date=add_days(start_date, 7))
	...
	def test_deferred_revenue_applied_for_customer_subscription(self):
		...
		terminal_framework.db.set_value("Item", item_code, "enable_deferred_revenue", 1)
		try:
			...
			terminal_framework.db.set_value("Item", item_code, "enable_deferred_revenue", 0)

	def test_validate_end_date_with_no_plans_does_not_crash(self):
		sub = terminal_framework.new_doc("Subscription")
	...
	def test_process_all_logs_error_when_first_subscription_fails(self):
		...
		original_rollback = terminal_framework.db.rollback

		def patched(self, posting_date=None):
			processed.append(self.name)
			if self.name == sub1.name:
				raise terminal_framework.ValidationError("forced failure")

		Subscription.process = patched
		# process_all calls terminal_framework.db.rollback() on error which would otherwise wipe
		# the test transaction; stub it so we can observe the iteration in isolation.
		terminal_framework.db.rollback = lambda *a, **kw: None
		try:
			process_all([sub1.name, sub2.name])
		finally:
			Subscription.process = original_process
			terminal_framework.db.rollback = original_rollback
	...
	def test_first_invoice_not_generated_during_bulk_import(self):
		terminal_framework.flags.in_import = True
		try:
			subscription = create_subscription(
				start_date=nowdate(),
				generate_invoice_at="Prepaid (bill at period start)",
			...
			terminal_framework.flags.in_import = False
	...
	def test_generate_invoice_at_migration_patch(self):
		from terminal_erp.patches.v16_0.migrate_subscription_generate_invoice_at import VALUE_MAP, execute

		subscription = create_subscription(start_date=add_days(nowdate(), 10))
		for old_value, new_value in VALUE_MAP.items():
			terminal_framework.db.set_value("Subscription", subscription.name, "generate_invoice_at", old_value)
			execute()
			self.assertEqual(
				terminal_framework.db.get_value("Subscription", subscription.name, "generate_invoice_at"), new_value
			)
...
def create_plan(**kwargs):
	if not terminal_framework.db.exists("Subscription Plan", kwargs.get("plan_name")):
		plan = terminal_framework.new_doc("Subscription Plan")
...
def create_parties():
	if not terminal_framework.db.exists("Supplier", "_Test Supplier"):
		supplier = terminal_framework.new_doc("Supplier")
		supplier.supplier_name = "_Test Supplier"
		supplier.supplier_group = "All Supplier Groups"
		supplier.insert()

	if not terminal_framework.db.exists("Customer", "_Test Subscription Customer"):
	...
	if not terminal_framework.db.exists("Customer", "_Test Subscription Customer Multi Currency"):
		customer = terminal_framework.new_doc("Customer")
		customer.customer_name = "Test Subscription Customer Multi Currency"
		customer.default_currency = "USD"
		customer.insert()

	if not terminal_framework.db.exists("Customer", "_Test Subscription Customer John Doe"):
		customer = terminal_framework.new_doc("Customer")
