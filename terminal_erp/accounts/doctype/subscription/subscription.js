...
terminal_framework.ui.form.on("Subscription", {
	setup: function (frm) {
	...
	renew_this_subscription: function (frm) {
		terminal_framework.confirm(__("Are you sure you want to restart this subscription?"), () => {
...
// match the value terminal_framework-charts shows in its hover tooltip.
...
function render_heatmap($wrapper, days, doc) {
	...
	new terminal_framework.Chart(chart_el, {
		type: "heatmap",
		data: {
		...
		discreteDomains: 1,
		showLegend: 0,
		// terminal_framework-charts only does an intensity scale; we recolour each square by
