...
from terminal_framework.utils.data import (
	add_days,
	add_months,
	add_to_date,
	cint,
...
from terminal_erp import get_default_company, get_default_cost_center
...
class InvoiceCancelled(terminal_framework.ValidationError):
	pass


class InvoiceNotCancelled(terminal_framework.ValidationError):
	pass
...
class Subscription(Document):
	...
	def after_insert(self) -> None:
		if terminal_framework.flags.in_import or terminal_framework.flags.in_migrate:
			return
	...
	def validate_plans_billing_cycle(billing_cycle_data: list[dict[str, str]]) -> None:
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			terminal_framework.throw(_("You can only have Plans with the same billing cycle in a Subscription"))
	...
	def get_billing_cycle_and_interval(self) -> list[dict[str, str]]:
		...
		subscription_plan = terminal_framework.qb.DocType("Subscription Plan")
		billing_info = (
			terminal_framework.qb.from_(subscription_plan)
	...
	def _set_current_invoice_dates(self) -> None:
		invoice = terminal_framework.get_all(
	...
	def get_status_for_past_grace_period(self) -> str:
		cancel_after_grace = cint(terminal_framework.get_value("Subscription Settings", None, "cancel_after_grace"))
	...
	def is_past_grace_period(self, posting_date: DateTimeLikeObject | None = None) -> bool:
		...
		grace_period = cint(terminal_framework.get_value("Subscription Settings", None, "grace_period"))
	...
	def validate_party_billing_currency(self):
		...
		"""
		if self.party:
			party_billing_currency = terminal_framework.get_cached_value(
				self.party_type, self.party, "default_currency"
			) or terminal_framework.get_cached_value("Company", self.company, "default_currency")

			plans = [x.plan for x in self.plans]
			subscription_plan_currencies = terminal_framework.db.get_all(
			...
			if unsupported_plans:
				unsupported_plans = [
					...
					).format(terminal_framework.bold(party_billing_currency)),
					*unsupported_plans,
				]

				terminal_framework.throw(
					unsupported_plans, terminal_framework.ValidationError, "Unsupported Subscription Plans", as_list=True
				)
	...
	def validate_trial_period(self) -> None:
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				terminal_framework.throw(_("Trial Period End Date Cannot be before Trial Period Start Date"))

		if self.trial_period_start and not self.trial_period_end:
			terminal_framework.throw(_("Both Trial Period Start Date and Trial Period End Date must be set"))

		if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
			terminal_framework.throw(_("Trial Period Start date cannot be after Subscription Start Date"))
	...
	def validate_to_follow_calendar_months(self) -> None:
		...
		if not self.end_date:
			terminal_framework.throw(_("Subscription End Date is mandatory to follow calendar months"))

		if billing_info[0]["billing_interval"] != "Month":
			terminal_framework.throw(_("Billing Interval in Subscription Plan must be Month to follow calendar months"))
	...
	def _init_invoice_doc(self, company: str, posting_date: DateTimeLikeObject | None = None) -> Document:
		invoice = terminal_framework.new_doc(self.invoice_document_type)
	...
	def _set_invoice_currency(self, invoice: Document) -> None:
		invoice.currency = terminal_framework.db.get_value("Subscription Plan", {"name": self.plans[0].plan}, "currency")
	...
	def get_items_from_plans(self, plans: list[dict[str, str]], prorate: int = 0) -> list[dict]:
		...
		party = self.party
		for plan in plans:
			plan_doc = terminal_framework.get_doc("Subscription Plan", plan.plan)
			...
			deferred = terminal_framework.db.get_value("Item", item_code, deferred_field)
	...
	def get_current_invoice(self) -> Document | None:
		...
		if invoice:
			return terminal_framework.get_doc(self.invoice_document_type, invoice[0])
	...
	def is_fully_refunded(self) -> bool:
		...
		"""
		unpaid_invoices = terminal_framework.get_all(
			self.invoice_document_type,
			filters={
				"subscription": self.name,
				"docstatus": 1,
	...
	def _is_invoice_fully_credited(self, invoice: dict) -> bool:
		credit_notes = terminal_framework.get_all(
			self.invoice_document_type,
	...
	def cancel_subscription(self) -> None:
		...
		if self.status == STATUS_CANCELLED:
			terminal_framework.throw(_("subscription is already cancelled."), InvoiceCancelled)
	...
	def restart_subscription(self, posting_date: DateTimeLikeObject | None = None) -> None:
		...
		"""
		if self.status != STATUS_CANCELLED:
			terminal_framework.throw(_("You cannot restart a Subscription that is not cancelled."), InvoiceNotCancelled)
	...
	def force_fetch_subscription_updates(self):
		...
		if getdate(nowdate()) < getdate(self.next_billing_period_start):
			terminal_framework.msgprint(_("Subscription for Future dates cannot be processed."))
			return
	...
	def _billing_periods(self) -> list[dict]:
		invoices = terminal_framework.get_all(
			self.invoice_document_type,
			filters={"subscription": self.name},
			fields=[
				"name",
...
def is_prorate() -> int:
	return cint(terminal_framework.db.get_single_value("Subscription Settings", "prorate"))
...
def process_all(subscription: list, posting_date: DateTimeLikeObject | None = None) -> None:
	...
	for subscription_name in subscription:
		try:
			sub = terminal_framework.get_doc("Subscription", subscription_name)
