...
from terminal_erp.assets.doctype.asset.depreciation import get_disposal_account_and_cost_center
...
class SalesInvoiceItem(Document):
	...
	def validate_cost_center(self, company: str):
		cost_center_company = terminal_framework.get_cached_value("Cost Center", self.cost_center, "company")
		if cost_center_company != company:
			terminal_framework.throw(
				_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
					terminal_framework.bold(self.idx), terminal_framework.bold(self.cost_center), terminal_framework.bold(company)
				)
			)
