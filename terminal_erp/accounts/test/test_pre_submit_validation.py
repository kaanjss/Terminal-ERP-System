...
from terminal_erp.selling.doctype.customer.test_customer import set_credit_limit
from terminal_erp.tests.utils import Terminal ERPTestSuite


def _get_orange_warnings():
	return [m for m in terminal_framework.message_log if m.get("indicator") == "orange"]


class _CreditLimitBase(Terminal ERPTestSuite):
	...
	def setUp(self):
		...
		terminal_framework.message_log.clear()
...
class TestCreditLimitWarnSalesInvoice(_CreditLimitBase):
	...
	def test_no_warning_when_customer_has_no_credit_limit(self):
		...
		terminal_framework.db.delete("Customer Credit Limit", {"parent": self.CUSTOMER})
		si = self._make_si(self.OVER)
		_check_credit_limit_warn(si)
		self.assertFalse(_get_orange_warnings())
...
class TestPackedQtyWarn(Terminal ERPTestSuite):
	COMPANY = "_Test Company"
	CUSTOMER = "_Test Customer"
	...
	def test_warns_when_packed_qty_mismatches(self):
		...
		with patch.object(
			dn,
			"validate_packed_qty",
			side_effect=terminal_framework.ValidationError("Packed Qty must be equal to qty"),
		):
			_check_packed_qty_warn(dn)
		self.assertTrue(_get_orange_warnings())
