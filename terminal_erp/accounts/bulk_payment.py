...
from terminal_erp.utilities.bulk_transaction import transaction_processing
...
def create_payment_entries(
	...
	terminal_framework.has_permission("Payment Entry", "create", throw=True)

	grouped_invoices = [d for d in terminal_framework.parse_json(grouped_invoices or "[]") if d.get("voucher_no")]
	ungrouped_invoices = [d for d in terminal_framework.parse_json(ungrouped_invoices or "[]") if d.get("voucher_no")]

	if not grouped_invoices and not ungrouped_invoices:
		terminal_framework.throw(_("No Purchase Invoices selected"))
...
def make_grouped_payment_entries(groups):
	...
	for group in groups:
		...
		try:
			terminal_framework.db.savepoint("bulk_pe")
			pe = _build_grouped_payment_entry(supplier, group["party_account"], group["vouchers"])
			if not pe:
				terminal_framework.db.rollback(save_point="bulk_pe")
				failed += 1
			...
			terminal_framework.db.rollback(save_point="bulk_pe")
			failed += 1
			terminal_framework.log_error(title=_("Bulk Payment Entry creation failed for {0}").format(supplier))
	...
	refs = get_outstanding_reference_documents(
		{
			...
			"company": pe.company,
			"vouchers": [terminal_framework._dict(voucher_type="Purchase Invoice", voucher_no=n) for n in names],
		}
	)
