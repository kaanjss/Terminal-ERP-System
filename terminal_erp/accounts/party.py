...
from terminal_framework import _, msgprint, qb, scrub
from terminal_framework.contacts.doctype.address.address import get_company_address, get_default_address
...
from terminal_framework.query_builder.functions import Abs, Date, Sum
...
from terminal_erp.exceptions import InvalidAccountCurrency, PartyDisabled, PartyFrozen
from terminal_erp.utilities.regional import temporary_flag

try:
	from terminal_framework.contacts.doctype.address.address import render_address as _render_address
except ImportError:
	# Older terminal_framework versions where this function is not available
	from terminal_framework.contacts.doctype.address.address import get_address_display as _render_address
...
class DuplicatePartyAccountError(terminal_framework.ValidationError):
	pass
...
def _get_party_details(
	...
	party_details = terminal_framework._dict(
		set_account_and_due_date(party, account, party_type, company, posting_date, bill_date, doctype)
	)
	party = party_details[party_type.lower()]
	party = terminal_framework.get_doc(party_type, party)

	if not ignore_permissions:
		ptype = "select" if terminal_framework.only_has_select_perm(party_type) else "read"
		terminal_framework.has_permission(party_type, ptype, party, throw=True)
	...
	if not party_details.get("tax_category") and pos_profile:
		party_details["tax_category"] = terminal_framework.get_value("POS Profile", pos_profile, "tax_category")
...
def complete_contact_details(party_details):
	contact_details = terminal_framework._dict()

	if party_details.party_type == "Employee":
		from terminal_erp.setup.doctype.employee.employee import _get_contact_details as get_employee_contact
		...
		contact_details = terminal_framework.db.get_value(
			"Contact",
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		return terminal_framework.get_cached_value("Customer Group", party.customer_group, "default_price_list")
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = terminal_framework.get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = terminal_framework.get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = terminal_framework.db.get_value(
...
def get_party_account(
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return terminal_framework.get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		group = terminal_framework.get_cached_value(party_type, party, scrub(party_group_doctype))
		account = terminal_framework.db.get_value(
			"Party Account",
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = terminal_framework.get_cached_value("Company", company, default_account_name)
	...
	if not account:
		account_type = terminal_framework.get_cached_value("Party Type", party_type, "account_type")
...
def get_party_advance_account(party_type, party, company):
	...
	if not account:
		...
		account = terminal_framework.get_cached_value("Company", company, account_name)
...
def get_party_account_currency(party_type, party, company):
	def generator():
		...
		return terminal_framework.get_cached_value("Account", party_account, "account_currency")

	return terminal_framework.local_cache("party_account_currency", (party_type, party, company), generator)
...
def get_party_gle_currency(party_type, party, company):
	...
	return terminal_framework.local_cache(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		terminal_framework.throw(
			_(
				...
				terminal_framework.bold(party_type),
				terminal_framework.bold(party),
				terminal_framework.bold(existing_gle_currency),
				terminal_framework.bold(company),
			),
...
def validate_party_accounts(doc):
	from terminal_erp.controllers.accounts_controller import validate_account_head
	...
	for account in doc.get("accounts"):
		...
		party_account_currency = terminal_framework.get_cached_value("Account", account.account, "account_currency")
		if terminal_framework.db.get_default("Company"):
			company_default_currency = terminal_framework.get_cached_value(
				"Company", terminal_framework.db.get_default("Company"), "default_currency"
			)
		else:
			company_default_currency = terminal_framework.get_cached_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = terminal_framework.get_cached_value(party_type, party, "supplier_group")
				template_name = terminal_framework.get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = terminal_framework.get_doc("Payment Terms Template", template_name)
...
def validate_due_date(posting_date, due_date, bill_date=None, template_name=None, doctype=None):
	if getdate(due_date) < getdate(posting_date):
		...
		terminal_framework.throw(_("Due Date cannot be before {0}").format(doctype_date))
	else:
...
def validate_due_date_with_template(posting_date, due_date, bill_date, template_name, doctype=None):
	...
	if getdate(default_due_date) != getdate(posting_date) and getdate(due_date) > getdate(default_due_date):
		if terminal_framework.get_single_value("Accounts Settings", "credit_controller") in terminal_framework.get_roles():
			...
			terminal_framework.throw(_("Due Date cannot be after {0}").format(formatdate(default_due_date)))
...
def get_address_tax_category(
	...
	addr_tax_category_from = terminal_framework.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = terminal_framework.db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = terminal_framework.db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from terminal_erp.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template

	args = {terminal_framework.scrub(party_type): party, "company": company}
	...
	if party_type in ("Customer", "Lead", "Prospect", "CRM Deal"):
		...
		if party_type in ["Lead", "Prospect", "CRM Deal"]:
			args["customer"] = None
			del args[terminal_framework.scrub(party_type)]
	else:
...
def get_payment_terms_template(party_name: str, party_type: str, company: str | None = None):
	...
	if party_type == "Customer":
		customer = terminal_framework.get_cached_value(
			"Customer", party_name, fieldname=["payment_terms", "customer_group"], as_dict=1
		)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = terminal_framework.get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = terminal_framework.get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = terminal_framework.get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = terminal_framework.get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(company, party_type, party_name):
	if terminal_framework.flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = terminal_framework.get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				terminal_framework.throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				role_allowed_for_frozen_entries = terminal_framework.get_cached_value(
					"Company", company, "role_allowed_for_frozen_entries"
				)
				if role_allowed_for_frozen_entries not in terminal_framework.get_roles():
					terminal_framework.throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if terminal_framework.db.get_value("Employee", party_name, "status") != "Active":
				terminal_framework.msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)
...
def validate_account_party_type(self):
	...
	if self.party_type and self.party:
		account_type = terminal_framework.get_cached_value("Account", self.account, "account_type")
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = terminal_framework.get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = terminal_framework._dict(
	...
	company_wise_billing_this_year = terminal_framework._dict()
	...
	company_wise_total_unpaid = terminal_framework._dict(
	...
	for d in companies:
		company_default_currency = terminal_framework.get_cached_value("Company", d.company, "default_currency")
...
def get_party_shipping_address(doctype: str, name: str) -> str | None:
	...
	:return: String
	"""
	shipping_addresses = terminal_framework.get_all(
		"Address",
		filters=[
...
def get_partywise_advanced_payment_amount(
	...
	acc = terminal_framework.qb.DocType("Account")
	...
	if invoice_doctypes := terminal_framework.get_hooks("invoice_doctypes"):
	...
	data = terminal_framework._dict(data or {})
...
def get_default_contact(doctype: str, name: str) -> str | None:
	...
	:return: String
	"""
	contacts = terminal_framework.get_all(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
			["Dynamic Link", "link_name", "=", name],
...
def add_party_account(party_type, party, company, account):
	doc = terminal_framework.get_doc(party_type, party)
...
def render_address(address, check_permissions=True):
	return terminal_framework.call(_render_address, address, check_permissions=check_permissions)


def validate_party_currency_before_merging(party_type, old_party, new_party):
	for company in terminal_framework.get_all("Company"):
