...
from terminal_framework.utils import cint, flt, get_link_to_form, getdate, now
...
from terminal_erp.accounts.services.gl_validator import (
	check_freezing_date,
...
from terminal_erp.accounts.utils import create_payment_ledger_entry, is_immutable_ledger_enabled
...
def make_gl_entries(
	...
	if gl_map:
		if (
			not cancel
			and not cint(terminal_framework.get_single_value("Accounts Settings", "use_legacy_budget_controller"))
...
def get_accounting_dimensions_for_offsetting_entry(gl_map, company):
	acc_dimension = terminal_framework.qb.DocType("Accounting Dimension")
	dimension_detail = terminal_framework.qb.DocType("Accounting Dimension Detail")

	acc_dimensions = (
		terminal_framework.qb.from_(acc_dimension)
	...
	for acc_dimension in acc_dimensions:
		...
		acc_dimension.account_currency = terminal_framework.get_cached_value(
...
def distribute_gl_based_on_cost_center_allocation(gl_map, precision=None, from_repost=False):
	round_off_account, default_currency = terminal_framework.get_cached_value(
		"Company", gl_map[0].company, ["round_off_account", "default_currency"]
	)
	if not precision:
		precision = get_field_precision(
			terminal_framework.get_meta("GL Entry").get_field("debit"),
...
def get_cost_center_allocation_data(company, posting_date, cost_center):
	cost_center_allocation = terminal_framework.db.get_value(
	...
	records = terminal_framework.db.get_all(
...
def merge_similar_entries(gl_map, precision=None):
	...
	company = gl_map[0].company if gl_map else terminal_erp.get_default_company()
	company_currency = terminal_erp.get_company_currency(company)

	if not precision:
		precision = get_field_precision(
			terminal_framework.get_meta("GL Entry").get_field("debit"), currency=company_currency
		)
	...
	merged_gl_map = filter(
		...
		or (
			...
			and terminal_framework.get_cached_value("Journal Entry", x.voucher_no, "voucher_type")
...
def make_entry(args, adv_adj, update_outstanding, from_repost=False):
	gle = terminal_framework.new_doc("GL Entry")
	gle.update(args)
...
def process_debit_credit_difference(gl_map):
	precision = get_field_precision(
		terminal_framework.get_meta("GL Entry").get_field("debit"),
		currency=terminal_framework.get_cached_value("Company", gl_map[0].company, "default_currency"),
	)
	...
	if abs(debit_credit_diff) > allowance:
		if not (
			...
			and terminal_framework.get_cached_value("Journal Entry", voucher_no, "voucher_type")
			== "Exchange Gain Or Loss"
		):
...
def make_round_off_gle(gl_map, debit_credit_diff, trx_cur_debit_credit_diff, precision):
	...
	round_off_gle = terminal_framework._dict()
		...
		if not round_off_for_opening:
			terminal_framework.throw(
				_("Please set '{0}' in Company: {1}").format(
					terminal_framework.bold("Round Off for Opening"), get_link_to_form("Company", gl_map[0].company)
				)
			)
...
def update_accounting_dimensions(round_off_gle):
	...
	meta = terminal_framework.get_meta(round_off_gle["voucher_type"])
	...
	if dimensions and has_all_dimensions:
		dimension_values = terminal_framework.db.get_value(
		...
		report_type = terminal_framework.get_cached_value("Account", round_off_gle.account, "report_type")
...
def get_round_off_account_and_cost_center(company, voucher_type, voucher_no, use_company_default=False):
	round_off_account, round_off_cost_center, round_off_for_opening = terminal_framework.get_cached_value(
		"Company", company, ["round_off_account", "round_off_cost_center", "round_off_for_opening"]
	) or [None, None, None]

	# Use expense account as fallback
	if not round_off_account:
		round_off_account = terminal_framework.get_cached_value("Company", company, "default_expense_account")

	meta = terminal_framework.get_meta(voucher_type)

	# Give first preference to parent cost center for round off GLE
	if not use_company_default and meta.has_field("cost_center"):
		parent_cost_center = terminal_framework.db.get_value(voucher_type, voucher_no, "cost_center")
	...
	if not round_off_account:
		terminal_framework.throw(
			_("Please mention '{0}' in Company: {1}").format(
				terminal_framework.bold("Round Off Account"), get_link_to_form("Company", company)
			)
		)
	...
	if not round_off_cost_center:
		terminal_framework.throw(
			_("Please mention '{0}' in Company: {1}").format(
				terminal_framework.bold("Round Off Cost Center"), get_link_to_form("Company", company)
			)
		)
...
def make_reverse_gl_entries(
	...
	if gl_entries:
		...
		if partial_cancel:
			...
			for x in gl_entries:
				query = (
					terminal_framework.qb.update(gle)
		...
		for entry in gl_entries:
			...
			if immutable_ledger_enabled:
				...
				new_gle["posting_date"] = terminal_framework.form_dict.get("posting_date") or getdate()
			elif posting_date:
