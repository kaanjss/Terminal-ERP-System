...
from terminal_framework import _, qb, throw
...
from terminal_framework.model.naming import determine_consecutive_week_number
from terminal_framework.query_builder import AliasedQuery, Case, Criterion, Field, Table
from terminal_framework.query_builder.functions import Count, IfNull, Max, Min, Round, Sum
from terminal_framework.query_builder.utils import DocType
...
from terminal_framework.utils.caching import site_cache
...
# imported to enable terminal_erp.accounts.utils.get_account_currency
from terminal_erp.accounts.doctype.account.account import get_account_currency
from terminal_erp.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from terminal_erp.stock import get_warehouse_account_map
from terminal_erp.stock.utils import get_combine_datetime, get_stock_value_on

if TYPE_CHECKING:
	from terminal_framework.model.document import Document

	from terminal_erp.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation


class FiscalYearError(terminal_framework.ValidationError):
	pass


class PaymentEntryUnlinkError(terminal_framework.ValidationError):
	pass
...
def get_fiscal_years(
	...
	if raise_on_missing:
		...
		if company:
			error_msg = _("""{0} for {1}""").format(error_msg, terminal_framework.bold(company))

		if verbose == 1:
			terminal_framework.msgprint(error_msg)
...
def _get_fiscal_years(company=None):
	fiscal_years = terminal_framework.cache().hget("fiscal_years", company) or []
	...
	if not fiscal_years:
		...
		query = (
			terminal_framework.qb.from_(FY).select(FY.name, FY.year_start_date, FY.year_end_date).where(FY.disabled == 0)
		)

		if company:
			FYC = DocType("Fiscal Year Company")
			query = query.where(
				ExistsCriterion(terminal_framework.qb.from_(FYC).select(FYC.name).where(FYC.parent == FY.name)).negate()
				| ExistsCriterion(
					terminal_framework.qb.from_(FYC)
					.select(FYC.company)
		...
		terminal_framework.cache().hset("fiscal_years", company, fiscal_years)
	return fiscal_years
...
def get_balance_on(
	...
	if not account and terminal_framework.form_dict.get("account"):
		account = terminal_framework.form_dict.get("account")
	if not date and terminal_framework.form_dict.get("date"):
		date = terminal_framework.form_dict.get("date")
	if not party_type and terminal_framework.form_dict.get("party_type"):
		party_type = terminal_framework.form_dict.get("party_type")
	if not party and terminal_framework.form_dict.get("party"):
		party = terminal_framework.form_dict.get("party")
	if not cost_center and terminal_framework.form_dict.get("cost_center"):
		cost_center = terminal_framework.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if start_date:
		cond.append("posting_date >= %s" % terminal_framework.db.escape(cstr(start_date)))
	if date:
		cond.append("posting_date <= %s" % terminal_framework.db.escape(cstr(date)))
	else:
	...
	if cost_center and report_type == "Profit and Loss":
		cc = terminal_framework.get_lazy_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				f""" exists (
			...
			cond.append(f"""gle.cost_center = {terminal_framework.db.escape(cost_center)} """)

	if account:
		if not (terminal_framework.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == terminal_framework.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append(f"""gle.account = {terminal_framework.db.escape(account)} """)

	if account_type:
		accounts = terminal_framework.db.get_all(
			"Account",
		...
		cond.append(
			...
			gle.account in (%s)
		"""
			% (", ".join([terminal_framework.db.escape(account) for account in accounts]))
		)

	if party_type and party:
		terminal_framework.has_permission(party_type, "read", party, throw=True)
		cond.append(
			f"""gle.party_type = {terminal_framework.db.escape(party_type)} and gle.party = {terminal_framework.db.escape(party)} """
		)

	default_finance_book = None
	if company:
		cond.append("""gle.company = %s """ % (terminal_framework.db.escape(company)))
		default_finance_book = terminal_framework.get_cached_value("Company", company, "default_finance_book")

	if finance_book:
		if default_finance_book and include_default_fb_balances:
			cond.append(
				f"""(gle.finance_book IN (
						{terminal_framework.db.escape(finance_book)},
						{terminal_framework.db.escape(default_finance_book)}
					) OR gle.finance_book IS NULL
				)"""
			)
		else:
			cond.append(f"(gle.finance_book = {terminal_framework.db.escape(finance_book)} OR gle.finance_book IS NULL)")

	elif default_finance_book and include_default_fb_balances:
		# No finance book passed → fall back to default
		cond.append(
			f"""(
				gle.finance_book = {terminal_framework.db.escape(default_finance_book)}
				OR gle.finance_book IS NULL
			)"""
		)
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not terminal_framework.flags.ignore_account_permission:
			acc.check_permission("read")
		...
		entries = terminal_framework.db.sql(
			"""
...
@terminal_framework.whitelist()
def add_ac(args: terminal_framework._dict | None = None):
	from terminal_framework.desk.treeview import make_tree_args

	if not args:
		args = terminal_framework.local.form_dict

	args.pop("ignore_permissions", None)
	terminal_framework.has_permission("Account", "create", throw=True)

	args.doctype = "Account"
	args = make_tree_args(**args)

	ac = terminal_framework.new_doc("Account")
	ac.update(args)
...
@terminal_framework.whitelist()
def add_cc(args: terminal_framework._dict | None = None):
	from terminal_framework.desk.treeview import make_tree_args
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{} - {}".format(
			args.parent_cost_center, terminal_framework.get_cached_value("Company", args.company, "abbr")
		)
...
def _build_dimensions_dict_for_exc_gain_loss(
	...
	dimensions_dict = terminal_framework._dict()
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		doc = terminal_framework.get_doc(voucher_type, voucher_no)
		terminal_framework.flags.ignore_party_validation = True
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from terminal_erp.accounts.general_ledger import process_debit_credit_difference
		...
		terminal_framework.flags.ignore_party_validation = False
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		journal_entry = terminal_framework.qb.DocType("Journal Entry")
		journal_acc = terminal_framework.qb.DocType("Journal Entry Account")

		q = (
			terminal_framework.qb.from_(journal_entry)
		...
		precision = terminal_framework.get_precision("Payment Entry", "unallocated_amount")

		payment_entry = terminal_framework.qb.DocType("Payment Entry")
		payment_ref = terminal_framework.qb.DocType("Payment Entry Reference")

		q = (
			terminal_framework.qb.from_(payment_entry)
...
def validate_allocated_amount(args):
	precision = args.get("precision") or terminal_framework.db.get_single_value("System Settings", "currency_precision")
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	[
		...
		for field in terminal_framework.get_meta("Journal Entry Account").get_fieldnames_with_value()
	]
...
def update_reference_in_payment_entry(
	...
	if not skip_ref_details_update_for_pe:
		reference_exchange_details = terminal_framework._dict()
	...
	payment_entry.make_exchange_gain_loss_journal(
		terminal_framework._dict({"difference_posting_date": d.difference_posting_date}), dimensions_dict
	)
...
def get_reconciliation_effect_date(against_voucher_type, against_voucher, company, posting_date):
	reconciliation_takes_effect_on = terminal_framework.get_cached_value(
	...
	if reconciliation_takes_effect_on == "Advance Payment Date":
		...
		reconcile_on = terminal_framework.db.get_value(against_voucher_type, against_voucher, date_field)
...
def cancel_exchange_gain_loss_journal(
	...
	if parent_doc.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
		...
		for doc in gain_loss_journals:
			gain_loss_je = terminal_framework.get_doc("Journal Entry", doc)
...
def get_linked_exchange_gain_loss_journal(referenced_dt: str, referenced_dn: str, je_docstatus: int) -> list:
	...
	"""
	gain_loss_journals = []
	if journals := terminal_framework.db.get_all(
		...
		gain_loss_journals = terminal_framework.db.get_all(
			"Journal Entry",
			{
				"name": ["in", journals],
...
def cancel_common_party_journal(self):
	...
	if not terminal_framework.get_single_value("Accounts Settings", "enable_common_party_accounting"):
		return
	...
	journal_entry = terminal_framework.db.get_value(
		"Journal Entry Account",
		filters={
	...
	common_party_journal = terminal_framework.db.get_value(
		"Journal Entry",
		filters={
			"name": journal_entry,
	...
	common_party_je = terminal_framework.get_doc("Journal Entry", common_party_journal)
...
def update_accounting_ledgers_after_reference_removal(
	...
	gle_update_query = (
		...
		.set(gle.modified_by, terminal_framework.session.user)
	...
	ple_update_query = (
		...
		.set(ple.modified_by, terminal_framework.session.user)
		.where(
	...
	adv_ple = (
		...
		.set(adv.modified_by, terminal_framework.session.user)
...
def remove_ref_from_advance_section(ref_doc: object = None, payment_name: str | None = None):
	...
	if ref_doc.doctype in ("Sales Invoice", "Purchase Invoice"):
		...
		terminal_framework.db.delete(child_table, {"name": ("in", row_names)})
...
def remove_ref_doc_link_from_jv(
	...
	if linked_jv:
		update_query = (
			...
			.set(jea.modified_by, terminal_framework.session.user)
		...
		terminal_framework.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))
...
def remove_ref_doc_link_from_pe(
	...
	(
		...
		.set(per.modified_by, terminal_framework.session.user)
	...
	for pe in linked_pe:
		try:
			pe_doc = terminal_framework.get_doc("Payment Entry", pe)
			pe_doc.set_amounts()
			...
			terminal_framework.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
		...
		(
			...
			.set(pay.modified_by, terminal_framework.session.user)
			.where(pay.name == pe)
			.run()
		)
	terminal_framework.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@terminal_framework.whitelist()
def get_company_default(company: str, fieldname: str, ignore_validation: bool = False):
	value = terminal_framework.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				_(terminal_framework.get_meta("Company").get_label(fieldname)), company
			)
		)
...
def fix_total_debit_credit():
	...
	for d in vouchers:
		if abs(d.diff) > 0:
			...
			gle = terminal_framework.qb.DocType("GL Entry")
			name = terminal_framework.db.get_value(
				"GL Entry",
				{"voucher_type": d.voucher_type, "voucher_no": d.voucher_no, dr_or_cr: [">", 0]},
				"name",
			)
			if name:
				terminal_framework.qb.update(gle).set(gle[dr_or_cr], gle[dr_or_cr] + d.diff).where(gle.name == name).run()


def get_currency_precision():
	precision = cint(terminal_framework.db.get_default("currency_precision"))
	if not precision:
		number_format = terminal_framework.db.get_default("number_format") or "#,###.##"
...
def get_fraction_units(currency: str) -> int:
	...
	fraction_units = terminal_framework.db.get_value("Currency", currency, "fraction_units")
...
def get_held_invoices(party_type, party):
	...
	if party_type == "Supplier":
		held_invoices = terminal_framework.get_all(
			"Purchase Invoice",
...
def get_outstanding_invoices(
	...
	precision = terminal_framework.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = terminal_framework.get_cached_value(
			"Account", account[0], ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = terminal_erp.get_party_account_type(party_type)
...
def get_account_name(account_type=None, root_type=None, is_group=None, account_currency=None, company=None):
	...
	return terminal_framework.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or terminal_framework.defaults.get_defaults().currency,
			"company": company or terminal_framework.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in terminal_framework.get_list("Company", fields=["name"], order_by="name")]
...
def get_children(
	...
	from terminal_erp.accounts.report.financial_statements import sort_accounts
	...
	acc = terminal_framework.get_list(doctype, fields=fields, filters=filters)
...
def get_account_balances(
	...
	company_currency = terminal_framework.get_cached_value("Company", company, "default_currency")
...
def get_account_balances_coa(company: str, include_default_fb_balances: bool = False):
	...
	account_list = (
		terminal_framework.qb.from_(Account)
	...
	get_ledger_balances_query = (
		terminal_framework.qb.from_(GLEntry)
		.select(
			GLEntry.account,
...
def create_payment_gateway_account(gateway, payment_channel="Email", company=None):
	from terminal_erp.setup.setup_wizard.operations.install_fixtures import create_bank_account

	if not company:
		company = terminal_framework.get_cached_value("Global Defaults", "Global Defaults", "default_company")
		if not company:
			return

	# NOTE: we translate Payment Gateway account name because that is going to be used by the end user
	bank_account = terminal_framework.db.get_value(
		"Account",
	...
	if not bank_account:
		terminal_framework.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(
	...
	"""
	terminal_framework.has_permission("Cost Center", "write", doc=docname, throw=True)
	validate_field_number("Cost Center", docname, cost_center_number, company, "cost_center_number")

	if cost_center_number:
		terminal_framework.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		terminal_framework.db.set_value("Cost Center", docname, "cost_center_number", "")

	terminal_framework.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, company)
	if docname != new_name:
		terminal_framework.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = terminal_framework.db.get_value(doctype_name, filters)
...
def parse_naming_series_variable(doc, variable):
	if variable in ["FY", "TFY"]:
		...
		if doc:
			company = doc.get("company") or terminal_framework.db.get_default("company")
		else:
			company = terminal_framework.db.get_default("company")

		return terminal_framework.db.get_value("Company", company, "abbr") if company else ""
		...
		if doc and doc.doctype in ["Batch", "Serial No"] and doc.reference_doctype and doc.reference_name:
			doc = terminal_framework.get_doc(doc.reference_doctype, doc.reference_name)
		...
		date = (
			...
			if doc and terminal_framework.get_single_value("Global Defaults", "use_posting_datetime_for_naming_documents")
			else now_datetime()
		)
...
def get_coa(doctype: str, parent: str, is_root: bool | None = None, chart: str | None = None):
	...
	chart = chart if chart else terminal_framework.flags.chart
	terminal_framework.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	from terminal_erp.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	precision = get_field_precision(terminal_framework.get_meta("GL Entry").get_field("debit")) or 2
	...
	for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
		...
		for voucher_type, voucher_no in stock_vouchers_chunk:
			...
			voucher_obj = terminal_framework.get_lazy_doc(voucher_type, voucher_no)
...
def get_future_stock_vouchers(posting_date, posting_time, for_warehouses=None, for_items=None, company=None):
	...
	if terminal_framework.db.db_type == "postgres":
		terminal_framework.qb.from_(SLE).select(SLE.name).where(conditions).for_update().run()
...
def get_voucherwise_gl_entries(future_stock_vouchers, posting_date):
	...
	gles = terminal_framework.get_all(
		"GL Entry",
...
def get_stock_accounts(company, voucher_type=None, voucher_no=None, accounts=None):
	stock_accounts = [
		d.name
		for d in terminal_framework.db.get_all("Account", {"account_type": "Stock", "company": company, "is_group": 0})
	]
	...
	if accounts:
		...
		if voucher_type == "Journal Entry":
			stock_accounts = [
				d.account
				for d in terminal_framework.db.get_all(
					"Journal Entry Account",
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	account_table = terminal_framework.qb.DocType("Account")
	query = (
		terminal_framework.qb.from_(account_table)
	...
	precision = terminal_framework.get_precision("Journal Entry Account", "debit_in_account_currency")
...
def check_and_delete_linked_reports(report):
	...
	icons = terminal_framework.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
			terminal_framework.delete_doc("Desktop Icon", icon)


def create_err_and_its_journals(company: dict) -> None:
	err = terminal_framework.new_doc("Exchange Rate Revaluation")
	...
	if err.accounts:
		...
		if company.submit_err_jv:
			...
			jv and terminal_framework.get_doc("Journal Entry", jv).submit()
...
def _auto_create_exchange_rate_revaluation_for(frequency: str) -> None:
	...
	"""
	companies = terminal_framework.db.get_all(
		"Company",
	...
	if companies:
		for company in companies:
			terminal_framework.enqueue(
				"terminal_erp.accounts.utils.create_err_and_its_journals",
				company=company,
				queue="long",
			)
...
def create_payment_ledger_entry(
	...
	if gl_entries:
		...
		for entry in ple_map:
			ple = terminal_framework.get_doc(entry)
			...
			if cancel:
				if not is_immutable_ledger_enabled():
					...
					ple.delinked = 0
					ple.posting_date = terminal_framework.form_dict.get("posting_date") or getdate()
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	from terminal_erp.accounts.doctype.dunning.dunning import update_linked_dunnings

	if not voucher_type or not voucher_no:
		return

	if voucher_type in get_advance_payment_doctypes():
		ref_doc = terminal_framework.get_lazy_doc(voucher_type, voucher_no)
	...
	ple = terminal_framework.qb.DocType("Payment Ledger Entry")
	vouchers = [terminal_framework._dict({"voucher_type": voucher_type, "voucher_no": voucher_no})]
	common_filter = []
	common_filter.append(ple.party_type == party_type)
	common_filter.append(ple.party == party)
...
def create_gain_loss_journal(
	...
	journal_entry = terminal_framework.new_doc("Journal Entry")
	...
	party_account_currency = terminal_framework.get_cached_value("Account", party_account, "account_currency")

	if not gain_loss_account:
		terminal_framework.throw(_("Please set default Exchange Gain/Loss Account in Company {0}").format(company))
	gain_loss_account_currency = get_account_currency(gain_loss_account)
	company_currency = terminal_framework.get_cached_value("Company", company, "default_currency")

	if gain_loss_account_currency != company_currency:
		terminal_framework.throw(_("Currency for {0} must be {1}").format(gain_loss_account, company_currency))

	journal_account = terminal_framework._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or terminal_erp.get_default_cost_center(company),
			"project": project,
...
def get_party_types_from_account_type(account_type):
	return terminal_framework.db.get_all("Party Type", {"account_type": account_type}, pluck="name")
...
def get_advance_payment_doctypes(payment_type=None):
	...
	"""
	if payment_type:
		return terminal_framework.get_hooks(f"advance_payment_{payment_type}_doctypes") or []

	return (terminal_framework.get_hooks("advance_payment_receivable_doctypes") or []) + (
		terminal_framework.get_hooks("advance_payment_payable_doctypes") or []
	)


def run_ledger_health_checks():
	health_monitor_settings = terminal_framework.get_doc("Ledger Health Monitor")
	if health_monitor_settings.enable_health_monitor:
		...
		if health_monitor_settings.debit_credit_mismatch:
			for x in health_monitor_settings.companies:
				...
				voucher_wise = terminal_framework.get_doc("Report", "Voucher-wise Balance")
				res = voucher_wise.execute_script_report(filters=filters)
				for x in res[1]:
					doc = terminal_framework.new_doc("Ledger Health")
		...
		if health_monitor_settings.general_and_payment_ledger_mismatch:
			for x in health_monitor_settings.companies:
				...
				gl_pl_comparison = terminal_framework.get_doc("Report", "General and Payment Ledger Comparison")
...
def sync_auto_reconcile_config(auto_reconciliation_job_trigger: int = 15):
	auto_reconciliation_job_trigger = auto_reconciliation_job_trigger or terminal_framework.get_single_value(
	...
	sch_event = terminal_framework.get_doc(
		"Scheduler Event", {"scheduled_against": "Process Payment Reconciliation", "method": method}
	)
	if terminal_framework.db.get_value("Scheduled Job Type", {"method": method}):
		terminal_framework.get_doc(
			"Scheduled Job Type",
			{
				"method": method,
...
def build_qb_match_conditions(doctype, user=None) -> list:
	...
	criterion = []
	apply_strict_user_permissions = terminal_framework.get_system_settings("apply_strict_user_permissions")
...
def is_immutable_ledger_enabled():
	return terminal_framework.get_single_value("Accounts Settings", "enable_immutable_ledger")
...
def pre_submit_validation(doc, method=None):
	...
	if (
		doc.docstatus != 0
		or not terminal_framework.get_cached_value("Accounts Settings", None, "preview_mode")
		or not cfg
		or not doc.company
	):
		return
...
def _check_prev_docstatus(doc):
	try:
		...
		terminal_framework.msgprint(str(e), title=_("Pre-Submit Warning"), indicator="orange")
...
def _check_credit_limit_warn(doc):
	...
	except terminal_framework.ValidationError as e:
...
def refresh_subscription_status(name: str) -> None:
	subscription = terminal_framework.get_doc("Subscription", name)
