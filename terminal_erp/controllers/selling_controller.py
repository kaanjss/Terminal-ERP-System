...
from terminal_framework import _, bold, throw
from terminal_framework.query_builder.functions import Sum
from terminal_framework.utils import cint, flt, get_link_to_form, nowtime

from terminal_erp.accounts.party import render_address
from terminal_erp.controllers.accounts_controller import get_taxes_and_charges
from terminal_erp.controllers.sales_and_purchase_return import get_rate_for_return, is_batch_expired
from terminal_erp.controllers.stock_controller import StockController
from terminal_erp.stock.doctype.item.item import set_item_default
from terminal_erp.stock.get_item_details import get_bin_details, get_conversion_factor
from terminal_erp.stock.utils import get_combine_datetime, get_incoming_rate, get_valuation_method
...
class SellingController(StockController):
	...
	def onload(self):
		...
		if (
			self.get("company")
			and (
				default_selling_terms := terminal_framework.get_value(
	...
	def validate_standalone_serial_nos_customer(self):
		...
		if self.doctype in ["Sales Invoice", "Delivery Note"]:
			...
			if serial_nos := terminal_framework.get_all(
				"Serial No",
				...
				for sn in serial_nos:
					if sn.customer and sn.customer != self.customer:
						terminal_framework.throw(
							...
							).format(terminal_framework.bold(sn.name), terminal_framework.bold(sn.customer)),
	...
	def set_missing_lead_customer_details(self, for_validate=False):
		...
		if customer:
			...
			from terminal_erp.crm.doctype.lead.lead import get_lead_details
	...
	def set_company_contact_person(self):
		...
		if self.company and self.meta.has_field("company_contact_person") and not self.company_contact_person:
			self.company_contact_person = terminal_framework.get_cached_value(
				"Company", self.company, "default_sales_contact"
			)

	def remove_shipping_charge(self):
		if self.shipping_rule:
			shipping_rule = terminal_framework.get_last_doc("Shipping Rule", self.shipping_rule)
	...
	def validate_sales_team(self, sales_team):
		...
		sales_person_status = terminal_framework.db.get_all(
			"Sales Person", filters={"name": ["in", sales_persons]}, fields=["name", "enabled"]
		)

		for row in sales_person_status:
			if not row.enabled:
				terminal_framework.throw(_("Sales Person <b>{0}</b> is disabled.").format(row.name))

	def validate_max_discount(self):
		for d in self.get("items"):
			if d.item_code:
				discount = flt(terminal_framework.get_cached_value("Item", d.item_code, "max_discount"))

				if discount and flt(d.discount_percentage) > discount:
					terminal_framework.throw(_("Maximum discount for Item {0} is {1}%").format(d.item_code, discount))
		...
		for d in self.get("items"):
			if d.meta.get_field("stock_qty"):
				if not d.conversion_factor:
					terminal_framework.throw(_("Row {0}: Conversion Factor is mandatory").format(d.idx))
	...
	def validate_selling_price(self):
		def throw_message(idx, item_name, rate, ref_rate_field):
			throw(
				_(
					...
					bold("net rate"),
					bold(rate),
					bold(terminal_framework.get_meta("Selling Settings").get_label("validate_selling_price")),
					get_link_to_form("Selling Settings"),
				),
				title=_("Invalid Selling Price"),
			)

		if self.get("is_return") or not terminal_framework.get_single_value("Selling Settings", "validate_selling_price"):
			return
		...
		for item in self.items:
			...
			last_purchase_rate, is_stock_item = terminal_framework.get_cached_value(
	...
	def get_already_delivered_qty(self, current_docname, so, so_detail):
		...
		delivered_via_dn = (
			terminal_framework.qb.from_(dn_item)
	...
	def get_so_qty_and_warehouse(self, so_detail):
		so_item = terminal_framework.db.get_value(
	...
	def update_reserved_qty(self):
		...
		for so, so_item_rows in so_map.items():
			if so and so_item_rows:
				sales_order = terminal_framework.get_lazy_doc("Sales Order", so)
	...
	def set_incoming_rate(self):
		...
		for d in items:
			if not terminal_framework.get_cached_value("Item", d.item_code, "is_stock_item"):
				continue
	...
	def update_stock_ledger(self, allow_negative_stock=False):
		...
		for d in self.get_item_list():
			if terminal_framework.get_cached_value("Item", d.item_code, "is_stock_item") == 1 and flt(d.qty):
	...
	def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
		...
		if doc_list:
			po_nos += [
				d.po_no
				for d in terminal_framework.get_all(ref_doctype, "po_no", filters={"name": ("in", doc_list)})
				if d.get("po_no")
			]
	...
	def validate_for_duplicate_items(self):
		...
		if cint(terminal_framework.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		...
		item_stock_map = terminal_framework._dict(
		...
		for d in self.get("items"):
			...
			duplicate_items_msg = _("Item {0} entered multiple times.").format(terminal_framework.bold(d.item_code))
			duplicate_items_msg += "<br><br>"
			duplicate_items_msg += _("Please enable {0} in {1} to allow same item in multiple rows").format(
				terminal_framework.bold(_("Allow Item to Be Added Multiple Times in a Transaction")),
				get_link_to_form("Selling Settings", "Selling Settings"),
			)
			if item_stock_map.get(d.item_code):
				if stock_items in check_list:
					terminal_framework.throw(duplicate_items_msg)
				else:
	...
	def validate_target_warehouse(self):
		...
		for d in items:
			if d.get("target_warehouse") and d.get("warehouse") == d.get("target_warehouse"):
				warehouse = terminal_framework.bold(d.get("target_warehouse"))
				terminal_framework.throw(
					_(
		...
		if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
			...
			terminal_framework.msgprint(msg, title="Internal Transfer", alert=True)

	def validate_items(self):
		# validate items to see if they have is_sales_item enabled
		from terminal_erp.controllers.buying_controller import validate_item_type
	...
	def validate_sample_retention_warehouse(self):
		...
		sample_retention_warehouse = terminal_framework.db.get_single_value(
			...
			if item.get("warehouse") == sample_retention_warehouse:
				terminal_framework.throw(
					_("Row {0}: Cannot sell item {1} from Sample Retention Warehouse {2}").format(
						item.idx, terminal_framework.bold(item.item_code), terminal_framework.bold(sample_retention_warehouse)
					),
					title=_("Not Allowed"),
				)
	...
	def update_stock_reservation_entries(self) -> None:
		...
		for item in self.items:
			...
			if not packed_items:
				...
				for d in packed_items:
					...
					d.so_detail = terminal_framework.get_value(
						"Packed Item",
						{
		...
		if self._action == "cancel":
			for item in items:
				...
				sre_list = terminal_framework.db.get_all(
	...
	def set_serial_and_batch_bundle_from_pick_list(self):
		...
		for item in self.items:
			...
			if item.pick_list_item and not item.serial_and_batch_bundle:
				...
				bundle_id = terminal_framework.db.get_value("Serial and Batch Bundle", filters, "name")
...
def set_default_income_account_for_item(obj):
	...
	"""
	company_default = terminal_framework.get_cached_value("Company", obj.company, "default_income_account")
...
def get_serial_and_batch_bundle(child, parent, delivery_note_child=None):
	...
	if not terminal_framework.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"):
		return

	item_details = terminal_framework.db.get_value("Item", child.item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
...
def get_delivered_serial_batch_for_reservation(item):
	...
	if item.get("serial_and_batch_bundle"):
		bundle = terminal_framework.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
