...
from terminal_erp.stock.serial_batch_bundle import SerialBatchCreation, get_serial_nos_from_bundle
...
class SubcontractingController(StockController):
	...
	def set_valuation_rate_for_rm(self):
		...
		if self.doctype == "Subcontracting Receipt":
			for row in self.supplied_items:
				...
				precision = terminal_framework.get_precision("Subcontracting Receipt Supplied Item", "rate")
	...
	def validate_items(self):
		for item in self.items:
			is_stock_item, is_sub_contracted_item = terminal_framework.get_value(
				"Item", item.item_code, ["is_stock_item", "is_sub_contracted_item"]
			)

			if not is_stock_item:
				terminal_framework.throw(_("Row {0}: Item {1} must be a stock item.").format(item.idx, item.item_name))
			...
			if not item.get("secondary_item_type") and not item.get("is_legacy_scrap_item"):
				...
				if self.doctype != "Subcontracting Receipt":
					...
					order_item_field = terminal_framework.scrub(order_item_doctype)
					...
					pending_qty = flt(
						...
						terminal_framework.get_precision(
							order_item_doctype,
							"qty",
						),
					)
				...
				if item.bom:
					is_active, bom_item = terminal_framework.get_value("BOM", item.bom, ["is_active", "item"])
	...
	def initialized_fields(self):
		self.available_materials = terminal_framework._dict()
		self.__transferred_items = terminal_framework._dict()
		self.alternative_item_details = terminal_framework._dict()
	...
	def __get_transferred_items(self):
		...
		se_detail = terminal_framework.qb.DocType("Stock Entry Detail")
	...
	def __get_received_items(self, doctype):
		...
		return terminal_framework.get_all(f"{doctype}", fields=fields, filters=filters)
	...
	def __update_consumed_materials(self, doctype, return_consumed_items=False):
		...
		for row in consumed_materials:
			...
			consumed_bundles = voucher_bundle_data.get(bundle_key, terminal_framework._dict())
			...
			if row.serial_no and not consumed_bundles.serial_nos:
				from terminal_erp.deprecation_dumpster import deprecation_warning
	...
	def get_available_materials(self):
		...
		for row in transferred_items:
			...
			if not row.serial_no and not row.batch_no and voucher_bundle_data:
				...
				bundle_data = voucher_bundle_data.get(bundle_key, terminal_framework._dict())
	...
	def __remove_serial_and_batch_bundle(self, item):
		if item.get("serial_and_batch_bundle"):
			terminal_framework.delete_doc("Serial and Batch Bundle", item.serial_and_batch_bundle, force=True)
	...
	def _get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		data = terminal_framework.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
		to_remove = []
		for item in data:
	...
	def __update_reserve_warehouse(self, row, item):
		...
		elif terminal_framework.get_cached_value("Item", row.rm_item_code, "is_customer_provided_item") and self.get(
			"customer_warehouse"
		):
	...
	def __get_batch_nos_for_bundle(self, qty, key):
		...
		precision = terminal_framework.get_precision("Subcontracting Receipt Supplied Item", "consumed_qty")
	...
	def set_batch_for_supplied_items(self):
		from terminal_erp.stock.get_item_details import get_filtered_serial_nos
				...
				if batches:
					...
					for index, d in enumerate(batches):
						...
						if index == 0:
							...
							new_row.update(terminal_framework.copy_doc(row).as_dict())
							new_row.update(
								{
									"consumed_qty": d.get("qty"),
	...
	def __get_qty_based_on_material_transfer(self, item_row, transfer_item):
		...
		if self.qty_to_be_received.get(key):
			...
			if transfer_item.serial_no or terminal_framework.get_cached_value(
				"UOM", transfer_item.item_details.stock_uom, "must_be_whole_number"
			):
				return terminal_framework.utils.ceil(qty)
	...
	def __set_rate_for_serial_and_batch_bundle(self):
		...
		for row in self.get(self.raw_material_table):
			...
			row.rate = terminal_framework.get_cached_value(
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = _("The Batch No {0} has not been supplied against the {1} {2}").format(
				terminal_framework.bold(row.get("batch_no")), self.subcontract_data.order_doctype, link
			)
			terminal_framework.throw(msg, title=_("Incorrect Batch Consumed"))
	...
	def __validate_serial_no(self, row, key):
		if row.get("serial_and_batch_bundle") and self.__transferred_items.get(key).get("serial_no"):
			...
			if incorrect_sn:
				...
				terminal_framework.throw(msg, title=_("Incorrect Serial Number Consumed"))
	...
	def update_ordered_and_reserved_qty(self):
		...
		for sco, sco_item_rows in sco_map.items():
			if sco and sco_item_rows:
				sco_doc = terminal_framework.get_doc("Subcontracting Order", sco)
	...
	def sub_contracted_items(self):
		if not hasattr(self, "_sub_contracted_items"):
			...
			if item_codes:
				items = terminal_framework.get_all(
	...
	def update_requested_qty(self):
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				mr_obj = terminal_framework.get_doc("Material Request", mr)
...
def make_rm_stock_entry(
	...
	if subcontract_order:
		subcontract_order = terminal_framework.get_doc(order_doctype, subcontract_order)

		if not rm_items:
			if not subcontract_order.supplied_items:
				terminal_framework.throw(_("No item available for transfer."))
		...
		if fg_item_code_list:
			...
			def post_process(source_doc, target_doc):
				...
				over_transfer_allowance = terminal_framework.get_single_value(
							...
							if qty <= 0 and rm_item.get("total_supplied_qty"):
								per_transferred = (
									flt(
										...
										terminal_framework.db.get_default("float_precision"),
									)
									* 100
								)
			...
			terminal_framework.throw(_("No Items selected for transfer."))
...
def get_materials_from_supplier(source_name: str, target_doc: Document | str | None = None):
	args = terminal_framework.flags.args or {}
	...
	doc = terminal_framework.get_cached_doc(order_doctype, subcontract_order)
