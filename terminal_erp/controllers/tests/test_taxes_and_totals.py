...
class TestTaxesAndTotals(Terminal ERPTestSuite):
	def test_regional_round_off_accounts(self):
		"""
		Regional overrides cannot extend the list in-place — the return
		value must be assigned back to terminal_framework.flags.round_off_applicable_accounts.
		"""
		...
		with patch(
			"terminal_erp.controllers.taxes_and_totals.get_regional_round_off_accounts",
			mock_regional,
		):
			calculate_taxes_and_totals(so)

		self.assertIn(test_account, terminal_framework.flags.round_off_applicable_accounts)
