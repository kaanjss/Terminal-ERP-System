...
class TestItemWiseInventoryAccount(Terminal ERPTestSuite):
	def setUp(self):
		self.company = "_Test Company with perpetual inventory"
		self.company_abbr = "TCP1"
		self.default_warehouse = terminal_framework.db.get_value(
			"Warehouse",
			{"company": self.company, "is_group": 0, "warehouse_name": ("like", "%Stores%")},
		)
		terminal_framework.db.set_value("Company", self.company, "enable_item_wise_inventory_account", 1)
	...
	def test_item_account_for_purchase_receipt_entry(self):
		...
		for row in items:
			...
			sle_value = terminal_framework.db.get_value(
				"Stock Ledger Entry",
				{"voucher_type": "Purchase Receipt", "voucher_no": pr.name, "item_code": item_code},
				"stock_value_difference",
			)

			gl_value = terminal_framework.db.get_value(
				"GL Entry",
				{
	...
	def test_item_account_for_delivery_note_entry(self):
		...
		dn = create_delivery_note(
			...
			warehouse=self.default_warehouse,
			company=self.company,
			cost_center=terminal_framework.db.get_value("Company", self.company, "cost_center"),
			expense_account=terminal_framework.db.get_value("Company", self.company, "default_expense_account"),
			do_not_submit=True,
		)
	...
	def test_item_group_account_for_purchase_receipt_entry(self):
		...
		inventory_account_dict = terminal_framework._dict()
		for item_name, item_data in items.items():
			item_data = terminal_framework._dict(item_data)
			make_item(
				item_name,
				properties=item_data,
			)

			item_group = terminal_framework.get_doc("Item Group", item_data.item_group)
		...
		for item_group, account in inventory_account_dict.items():
			...
			sle_value = terminal_framework.get_all(
				"Stock Ledger Entry",
				filters={
					"voucher_type": "Purchase Receipt",
					"voucher_no": pr.name,
	...
	def _make_item_group(self, item_name):
		if not terminal_framework.db.exists("Item Group", item_name):
			item_group = terminal_framework.get_doc(
				{
					"doctype": "Item Group",
		...
		return terminal_framework.get_doc("Item Group", item_name)
	...
	def add_inventory_account(self, item, table_name=None):
		...
		if not terminal_framework.db.exists("Account", account):
			account_doc = terminal_framework.get_doc(
				{
					"doctype": "Account",
		...
		if not terminal_framework.db.get_value("Item Default", {"parent": item.name, "company": self.company}):
			item.append(
				table_name,
				{
					"company": self.company,
	...
	def test_item_account_for_manufacture_entry(self):
		...
		wip_warehouse = terminal_framework.db.get_value(
			"Warehouse",
			{"company": self.company, "is_group": 0, "warehouse_name": ("like", "%Work In Progress%")},
		)

		fg_warehouse = terminal_framework.db.get_value(
			"Warehouse",
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 5))
		stock_entry.submit()

		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		stock_entry.submit()
