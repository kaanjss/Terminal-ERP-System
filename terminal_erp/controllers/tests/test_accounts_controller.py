...
from terminal_framework.utils.data import getdate as convert_to_date
...
class TestAccountsController(Terminal ERPTestSuite):
	"""
	...
	def create_account(self):
		...
		for x in accounts:
			if not terminal_framework.db.get_value("Account", filters={"account_name": x.name, "company": self.company}):
				...
				acc = terminal_framework.get_doc("Account", name)
			setattr(self, x.attribute_name, acc.name)

	def setup_advance_accounts_in_party_master(self):
		company = terminal_framework.get_doc("Company", self.company)
		company.book_advance_payments_in_separate_party_account = 1
		company.save()

		customer = terminal_framework.get_doc("Customer", self.customer)
		customer.accounts = []
		customer.append(
			"accounts",
			{
		...
		supplier = terminal_framework.get_doc("Supplier", self.supplier)
		supplier.accounts = []
		supplier.append(
			"accounts",
			{
	...
	def create_payment_reconciliation(self):
		pr = terminal_framework.new_doc("Payment Reconciliation")
	...
	def test_15_gain_loss_on_different_posting_date(self):
		...
		self.assertEqual(
			terminal_framework.db.get_value("Journal Entry", exc_je_for_si[0].parent, "posting_date"),
	...
	@Terminal ERPTestSuite.change_settings("Stock Settings", {"allow_internal_transfer_at_arms_length_price": 1})
	def test_16_internal_transfer_at_arms_length_price(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 0)
		si.items[0].rate = arms_length_price
		si.save()
		...
		terminal_framework.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 1)
		pi = make_inter_company_purchase_invoice(si.name)
		pi.update_stock = 1
		pi.items[0].rate = arms_length_price
		pi.items[0].warehouse = target_warehouse
	...
	def test_17_gain_loss_posting_date_for_normal_payment(self):
		...
		self.assertEqual(
			getdate(nowdate()), terminal_framework.db.get_value("Journal Entry", exc_je_for_pe[0].parent, "posting_date")
		)
		# Cancel Payment
		pe.reload()
		pe.cancel()
	...
	def test_18_fetch_taxes_based_on_taxes_and_charges_template(self):
		...
		if not terminal_framework.db.exists("Sales Taxes and Charges Template", "_Test Tax - _TC"):
			doc = terminal_framework.new_doc("Sales Taxes and Charges Template")
		...
		sinv = terminal_framework.new_doc("Sales Invoice")
	...
	def test_19b_fetch_taxes_from_item_tax_template_purchase_invoice(self):
		pinv = terminal_framework.new_doc("Purchase Invoice")
	...
	def test_40_cost_center_from_payment_entry(self):
		...
		cc = terminal_framework.db.get_value("Company", self.company, "cost_center")
		terminal_framework.db.set_value("Company", self.company, "cost_center", None)
		...
		terminal_framework.db.set_value("Company", self.company, "cost_center", cc)
	...
	def test_difference_posting_date_in_pi_and_si(self):
		...
		jv = terminal_framework.get_doc("Journal Entry", exc_je_for_si[0].parent)
		sales_invoice = terminal_framework.get_doc("Sales Invoice", si.name)
		...
		journal_voucher = terminal_framework.get_doc("Journal Entry", exc_je_for_pi[0].parent)
		purchase_invoice = terminal_framework.get_doc("Purchase Invoice", pi.name)
	...
	def test_company_validation_in_dimension(self):
		...
		self.assertRaises(terminal_framework.ValidationError, si_1.save)
	...
	def test_party_contact(self):
		from terminal_framework.contacts.doctype.contact.test_contact import create_contact
	...
	def test_discount_amount_for_multiple_returns(self):
		...
		# Create first return - Terminal Framework will copy full discount by default, we need to adjust it
	...
	@Terminal ERPTestSuite.change_settings("Global Defaults", {"use_posting_datetime_for_naming_documents": 1})
