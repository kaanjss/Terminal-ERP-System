...
from terminal_framework.utils import comma_or, flt, get_link_to_form, getdate, now, nowdate, safe_div


class OverAllowanceError(terminal_framework.ValidationError):
	pass


def validate_status(status, options):
	if status not in options:
		terminal_framework.throw(_("Status must be one of {0}").format(comma_or(options)))
...
class StatusUpdater(Document):
	...
	def get_status(self):
		...
		for s in sl:
			if not s[1]:
				...
				if terminal_framework.safe_eval(
					s[1][5:],
					None,
					{
						"self": self.as_dict(),
						"getdate": getdate,
						"nowdate": nowdate,
						"get_value": terminal_framework.db.get_value,
					},
				):
					return {"status": s[0]}
	...
	def validate_qty(self):
		...
		for args in self.status_updater:
			...
			items_to_validate = []
			selling_negative_rate_allowed = terminal_framework.get_single_value(
				"Selling Settings", "allow_negative_rates_for_items"
			)
			buying_negative_rate_allowed = terminal_framework.get_single_value(
			...
			for d in self.get_all_children():
				if hasattr(d, "qty") and flt(d.qty) < 0 and not self.get("is_return"):
					terminal_framework.throw(_("For an item {0}, quantity must be a positive number").format(d.item_code))

				if hasattr(d, "qty") and flt(d.qty) > 0 and self.get("is_return"):
					terminal_framework.throw(_("For an item {0}, quantity must be a negative number").format(d.item_code))
					...
					if hasattr(d, "item_code") and hasattr(d, "rate") and flt(d.rate) < 0:
						terminal_framework.throw(
							_(
								...
								terminal_framework.bold(_("`Allow Negative rates for Items`")),
								get_link_to_form(
									"Selling Settings"
			...
			if items_to_validate:
				...
				if pp_sub_assembly_items:
					pp_subcontract_items = terminal_framework.db.get_all(
	...
	def fetch_items_with_pending_qty(self, args, item_field, items):
		doctype = terminal_framework.qb.DocType(args["target_dt"])
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = terminal_framework.get_single_value(
			"Stock Settings", "role_allowed_to_over_deliver_receive"
		)
		role_allowed_to_over_bill = terminal_framework.get_single_value("Accounts Settings", "role_allowed_to_over_bill")
		...
		if overflow_percent - allowance > 0.01:
			...
			if role not in terminal_framework.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		terminal_framework.throw(
			_(
				...
				terminal_framework.bold(_(item["target_ref_field"].title())),
				terminal_framework.bold(item["reduce_by"]),
				terminal_framework.bold(_(args.get("target_dt"))),
				terminal_framework.bold(_(self.doctype)),
				terminal_framework.bold(item.get("item_code")),
			)
	...
	def update_qty(self, update_modified=True):
		...
		for args in self.status_updater:
			...
			if self.docstatus == 1:
				args["cond"] = " or parent=%s" % terminal_framework.db.escape(self.name)
			else:
				args["cond"] = " and parent!=%s" % terminal_framework.db.escape(self.name)
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if (
				...
				args["second_source_condition"] = terminal_framework.db.sql(
	...
	def _calculate_target_parent_percentage(
		...
		child_records = terminal_framework.get_all(
			target_dt,
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if update_data:
			target = terminal_framework.get_lazy_doc(args["target_parent_dt"], args["name"])
	...
	def _update_modified(self, args, update_modified):
		...
		args["update_modified"] = ", modified = {}, modified_by = {}".format(
			terminal_framework.db.escape(now()), terminal_framework.db.escape(terminal_framework.session.user)
		)

	def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
		ref_fieldname = terminal_framework.scrub(ref_dt)

		ref_docs = [item.get(ref_fieldname) for item in (self.get("items") or []) if item.get(ref_fieldname)]
		if not ref_docs:
			return

		zero_amount_refdocs = terminal_framework.get_all(
			ref_dt,
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			ref_item = terminal_framework.qb.DocType(f"{ref_dt} Item")
			ref_doc_qty = flt(
				terminal_framework.qb.from_(ref_item)
				.select(Sum(ref_item.qty))
				.where(ref_item.parent == ref_dn)
				.run()[0][0]
			)

			doc_item = terminal_framework.qb.DocType(f"{self.doctype} Item")
			billed_qty = flt(
				terminal_framework.qb.from_(doc_item)
			...
			ref_doc = terminal_framework.get_lazy_doc(ref_dt, ref_dn)
...
def get_allowance_for(
	...
	if item_allowance is None:
		item_allowance = {}
	if qty_or_amount == "qty":
		if item_allowance.get(item_code, terminal_framework._dict()).get("qty"):
			return (
		...
		if item_allowance.get(item_code, terminal_framework._dict()).get("amount"):
			return (
	...
	qty_allowance, over_billing_allowance = terminal_framework.get_cached_value(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance is None:
			global_qty_allowance = flt(
				terminal_framework.get_single_value(global_qty_allowance_doctype, global_qty_allowance_field)
			)
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance is None:
			global_amount_allowance = flt(
				terminal_framework.get_cached_value("Accounts Settings", None, "over_billing_allowance")
			)
		over_billing_allowance = global_amount_allowance

	if qty_or_amount == "qty":
		allowance = qty_allowance
		item_allowance.setdefault(item_code, terminal_framework._dict()).setdefault("qty", qty_allowance)
	else:
		allowance = over_billing_allowance
		item_allowance.setdefault(item_code, terminal_framework._dict()).setdefault("amount", over_billing_allowance)
