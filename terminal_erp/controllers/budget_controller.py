...
from terminal_framework.utils import fmt_money

from terminal_erp.accounts.doctype.budget.budget import BudgetError, get_accumulated_monthly_budget
...
class BudgetValidation:
	def __init__(self, doc: object | None = None, gl_map: list | None = None):
		...
		self.get_dimensions()
		self.exception_approver_role = terminal_framework.get_cached_value(
	...
	def build_to_validate_map(self):
		self.to_validate = terminal_framework._dict()
	...
	def get_child_nodes(self, budget_against, dimension):
		lft, rgt = terminal_framework.db.get_all(
			budget_against, filters={"name": dimension}, fields=["lft", "rgt"], as_list=1
		)[0]
		return terminal_framework.db.get_all(budget_against, filters={"lft": [">=", lft], "rgt": ["<=", rgt]}, as_list=1)
	...
	def build_budget_keys(self):
		...
		for _bud in self.get_budget_records():
			budget_against = terminal_framework.scrub(_bud.budget_against)
			dimension = _bud.get(budget_against)

			if _bud.is_tree and terminal_framework.get_cached_value(_bud.budget_against, dimension, "is_group"):
	...
	def get_dimensions(self):
		self.dimensions = []
		for _x in terminal_framework.db.get_all("Accounting Dimension"):
			self.dimensions.append(terminal_framework.get_lazy_doc("Accounting Dimension", _x.name))
	...
	def get_budget_records(self) -> list:
		...
		for x in _budgets:
			x.is_tree = terminal_framework.get_meta(x.budget_against).is_tree
	...
	def warn(self, msg):
		terminal_framework.msgprint(msg, _("Budget Exceeded"))

	def execute_action(self, action, msg):
		if self.exception_approver_role and self.exception_approver_role in terminal_framework.get_roles(
		...
		if config.applies:
			...
			if annual_diff > 0:
				_msg = _(
					...
					terminal_framework.bold(key[2]),
					terminal_framework.bold(terminal_framework.unscrub(key[0])),
					terminal_framework.bold(key[1]),
					terminal_framework.bold(fmt_money(budget_amt, currency=currency)),
					terminal_framework.bold(fmt_money(annual_diff, currency=currency)),
				)
			...
			if monthly_diff > 0:
				_msg = _(
					...
					terminal_framework.bold(terminal_framework.unscrub(key[0])),
					terminal_framework.bold(key[1]),
					terminal_framework.bold(fmt_money(acc_monthly_budget, currency=currency)),
					terminal_framework.bold(fmt_money(monthly_diff, currency=currency)),
				)
	...
	def handle_cumulative_overlimit_for_monthly(self, key, v_map):
		...
		if monthly_diff > 0:
			...
			_msg = _(
				...
				terminal_framework.bold(terminal_framework.unscrub(key[0])),
				terminal_framework.bold(key[1]),
				terminal_framework.bold(fmt_money(v_map.accumulated_monthly_budget, currency=currency)),
	...
	def handle_cumulative_overlimit_for_annual(self, key, v_map):
		...
		if total_diff > 0:
			...
			_msg = _(
				...
				terminal_framework.bold(terminal_framework.unscrub(key[0])),
				terminal_framework.bold(key[1]),
				terminal_framework.bold(fmt_money(v_map.budget_amount, currency=currency)),
				self.budget_applicable_for(v_map, current_amt),
				terminal_framework.bold(fmt_money(total_diff, currency=currency)),
			)
