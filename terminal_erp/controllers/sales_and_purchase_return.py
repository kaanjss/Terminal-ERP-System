...
from terminal_framework.query_builder.functions import Abs, NullIf, Sum
from terminal_framework.utils import cint, flt, format_datetime, get_datetime

import terminal_erp
from terminal_erp.stock.serial_batch_bundle import get_batches_from_bundle
from terminal_erp.stock.utils import get_combine_datetime, get_incoming_rate, get_valuation_method, getdate


class StockOverReturnError(terminal_framework.ValidationError):
	pass
...
def validate_return_against(doc):
	if not terminal_framework.db.exists(doc.doctype, doc.return_against):
		terminal_framework.throw(_("Invalid {0}: {1}").format(doc.meta.get_label("return_against"), doc.return_against))
	else:
		ref_doc = terminal_framework.get_doc(doc.doctype, doc.return_against)
...
def validate_returned_items(doc):
	valid_items = terminal_framework._dict()
	...
	for d in doc.get("items"):
		...
		if doc.doctype in ["Purchase Receipt", "Purchase Invoice", "Sales Invoice", "POS Invoice"]:
			field = terminal_framework.scrub(doc.doctype) + "_item"
			if d.get(field):
		...
		if d.item_code and (flt(d.qty) <= 0 or flt(d.get("received_qty")) <= 0):
			if key not in valid_items:
				...
				ref = valid_items.get(key, terminal_framework._dict())
				...
				if (
					...
					and terminal_framework.db.get_value("Item", d.item_code, "is_stock_item")
				):
					terminal_framework.throw(_("Warehouse is mandatory"))
	...
	if not items_returned:
		terminal_framework.throw(_("At least one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, key, args, ref, valid_items, already_returned_items):
	...
	company_currency = terminal_erp.get_company_currency(doc.company)
	field_precision = get_field_precision(
		terminal_framework.get_meta(doc.doctype + " Item").get_field(
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				terminal_framework.throw(_("{0} must be negative in return document").format(label))
...
def get_already_returned_items(doc):
	...
	field = (
		terminal_framework.scrub(doc.doctype) + "_item"
...
def get_returned_qty_map_for_purchase_flow(return_against, supplier, row_name, doctype):
	...
	_return_map = terminal_framework._dict({})

	for row in data:
		if row.warehouse and row.warehouse not in _return_map:
			_return_map[row.warehouse] = terminal_framework._dict({"qty": 0, "stock_qty": 0})

		if row.rejected_warehouse and row.rejected_warehouse not in _return_map:
			_return_map[row.rejected_warehouse] = terminal_framework._dict({"qty": 0, "stock_qty": 0})
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
	...
	reference_field = "dn_detail" if doctype == "Delivery Note" else terminal_framework.scrub(child_doctype)
...
def make_return_doc(doctype: str, source_name: str, target_doc=None, return_against_rejected_qty=False):
	...
	company = terminal_framework.db.get_value(doctype, source_name, "company")
	default_warehouse_for_sales_return = terminal_framework.get_cached_value(
		"Company", company, "default_warehouse_for_sales_return"
	)

	if doctype == "Sales Invoice":
		inv_is_consolidated, inv_is_pos = terminal_framework.db.get_value(
	...
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice" or doctype == "POS Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = terminal_framework.get_cached_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = terminal_framework.get_cached_value("Print Heading", _("Debit Note"))
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if doctype in ["Purchase Receipt", "Subcontracting Receipt"]:
			...
			wh_map = returned_qty_map.get(source_doc.warehouse) or terminal_framework._dict()
			rejected_wh_map = returned_qty_map.get(source_doc.rejected_warehouse) or terminal_framework._dict()
	...
	def get_returned_non_bundled_serial_nos(child_doc, parent_doc, serial_no_field="serial_no"):
		...
		return_ref_field = terminal_framework.scrub(child_doc.doctype)
		...
		for row in terminal_framework.get_all(parent_doc.doctype, fields=fields, filters=filters):
...
def get_rate_for_return(
	...
	if not return_against:
		return_against = terminal_framework.get_cached_value(voucher_type, voucher_no, "return_against")
	...
	if voucher_type in ("Purchase Receipt", "Purchase Invoice", "Subcontracting Receipt"):
		...
		StockLedgerEntry = terminal_framework.qb.DocType("Stock Ledger Entry")
		select_field = Abs(StockLedgerEntry.stock_value_difference / NullIf(StockLedgerEntry.actual_qty, 0))

	item_details = terminal_framework.get_cached_value("Item", item_code, ["has_batch_no", "has_expiry_date"], as_dict=1)
	set_zero_rate_for_expired_batch = terminal_framework.db.get_single_value(
	...
	if (
		...
		batch_no = terminal_framework.db.get_value(f"{voucher_type} Item", voucher_detail_no, "batch_no")
	...
	rate = flt(terminal_framework.db.get_value("Stock Ledger Entry", filters, select_field))
	if not (rate and return_against) and voucher_type in ["Sales Invoice", "Delivery Note"]:
		rate = terminal_framework.db.get_value(f"{voucher_type} Item", voucher_detail_no, "incoming_rate")
	...
	if not rate and voucher_type in ["Sales Invoice", "Delivery Note"]:
		details = terminal_framework.db.get_value(
...
def get_filters(
	...
	if item_row:
		...
		reference_voucher_detail_no = terminal_framework.db.get_value(
...
def get_warehouses_for_return(voucher_type, name):
	warehouses = []
	warehouse_details = terminal_framework.get_all(
		voucher_type + " Item",
...
def get_returned_serial_nos(child_doc, parent_doc, serial_no_field=None, ignore_voucher_detail_no=None):
	...
	from terminal_erp.stock.serial_batch_bundle import get_serial_nos
...
def available_serial_batch_for_return(field, doctype, reference_ids, is_rejected=False):
	...
	if not available_dict:
		terminal_framework.throw(_("No Serial / Batches are available for return"))
...
def get_serial_batches_based_on_bundle(doctype, field, _bundle_ids):
	available_dict = terminal_framework._dict({})
	batch_serial_nos = terminal_framework.get_all(
	...
	for row in batch_serial_nos:
		...
		if terminal_framework.get_cached_value(row.voucher_type, row.voucher_no, "is_return"):
			key = terminal_framework.get_cached_value(row.voucher_type + " Item", row.voucher_detail_no, field)

		if doctype == "Packed Item":
			if key is None:
				key = terminal_framework.get_cached_value(
					"Packed Item",
					{"parent_detail_docname": row.voucher_detail_no, "item_code": row.item_code},
					field,
				)
				if key is None:
					key = terminal_framework.get_cached_value("Packed Item", row.voucher_detail_no, field)

				if row.voucher_type == "Delivery Note":
					key = terminal_framework.get_cached_value("Delivery Note Item", key, "dn_detail")
				elif row.voucher_type == "Sales Invoice":
					key = terminal_framework.get_cached_value("Sales Invoice Item", key, "sales_invoice_item")
		...
		if key not in available_dict:
			available_dict[key] = terminal_framework._dict(
				{
					"qty": 0.0,
...
def get_serial_and_batch_bundle(field, doctype, reference_ids, is_rejected=False):
	...
	_bundle_ids = terminal_framework.get_all(
		doctype,
		filters=filters,
		pluck=pluck_field,
	)
...
def get_filters_for_packed_item(field, reference_ids):
	...
	if dns := terminal_framework.get_all("Delivery Note Item", filters=filters, pluck="name"):
		names.extend(dns)

	filters = {"docstatus": 1, "sales_invoice_item": ("in", reference_ids)}
	if sis := terminal_framework.get_all("Sales Invoice Item", filters=filters, pluck="name"):
		names.extend(sis)
...
def filter_serial_batches(parent_doc, data, row, warehouse_field=None, qty_field=None):
	...
	filterd_serial_batch = terminal_framework._dict(
		{
			"serial_nos": [],
			"batches": defaultdict(float),
	...
	if not child_doc.get(qty_field):
		terminal_framework.throw(
			_("For the {0}, the quantity is required to make the return entry").format(
				terminal_framework.bold(child_doc.item_code)
			)
		)
...
def get_payment_data(invoice: str):
	payment = terminal_framework.db.get_all("Sales Invoice Payment", {"parent": invoice}, ["mode_of_payment", "amount"])
	return payment


@terminal_framework.whitelist()
def get_invoice_item_returned_qty(doctype: str, invoice: str, customer: str, item_row_name: str):
	is_return, docstatus = terminal_framework.db.get_value(doctype, invoice, ["is_return", "docstatus"])
...
def is_invoice_returnable(doctype: str, invoice: str):
	is_return, docstatus, customer = terminal_framework.db.get_value(
		doctype, invoice, ["is_return", "docstatus", "customer"]
	)
	if is_return or docstatus == 0:
		return False

	invoice_item_qty = terminal_framework.db.get_all(f"{doctype} Item", {"parent": invoice}, ["name", "qty"])
...
def get_sales_invoice_item_from_consolidated_invoice(return_against_pos_invoice, pos_invoice_item):
	try:
		...
		query = (
			terminal_framework.qb.from_(SalesInvoice)
...
def is_batch_expired(batch_no, posting_date):
	...
	"""
	expiry_date = terminal_framework.db.get_value("Batch", batch_no, "expiry_date")
	if not expiry_date:
		return
