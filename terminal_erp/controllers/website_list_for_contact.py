...
from terminal_framework.modules.utils import get_module_app
from terminal_framework.utils import cint, flt, has_common
from terminal_framework.utils.user import is_website_user


def get_list_context(context=None):
	return {
		"global_number_format": terminal_framework.db.get_default("number_format") or "#,###.##",
...
def get_webform_list_context(module):
	if get_module_app(module) != "terminal_erp":
		return
	...
	if txt:
		if meta.get_field("items"):
			if meta.get_field("items").options:
				...
				for item in terminal_framework.get_all(child_doctype, {"item_name": ["like", "%" + txt + "%"]}):
					child = terminal_framework.get_doc(child_doctype, item.name)
					or_filters.append([doctype, "name", "=", child.parent])

	if or_filters:
		for r in terminal_framework.get_list(
			doctype,
			fields=fields,
			filters=filters,
...
def rfq_transaction_list(parties_doctype, doctype, parties, limit_start, limit_page_length):
	party = terminal_framework.qb.DocType(parties_doctype)
	data = (
		terminal_framework.qb.from_(party)
		...
		.orderby(party.creation, order=terminal_framework.qb.desc)
...
def get_customers_suppliers(doctype, user):
	...
	if has_common(["Supplier", "Customer"], terminal_framework.get_roles(user)):
		suppliers = get_parents_for_user("Supplier")
		customers = get_parents_for_user("Customer")
	elif terminal_framework.has_permission(doctype, "read", user=user):
		customer_list = terminal_framework.get_list("Customer")
...
def get_parents_for_user(parenttype: str) -> list[str]:
	portal_user = terminal_framework.qb.DocType("Portal User")

	return (
		terminal_framework.qb.from_(portal_user)
		.select(portal_user.parent)
		.where(portal_user.user == terminal_framework.session.user)
...
def has_website_permission(doc, ptype, user, verbose=False):
	...
	if customers:
		return terminal_framework.db.exists(doctype, get_customer_filter(doc, customers))
	elif suppliers:
		fieldname = "suppliers" if doctype == "Request for Quotation" else "supplier"
		return terminal_framework.db.exists(doctype, {"name": doc.name, fieldname: ["in", suppliers]})
	else:
		return False
...
def add_role_for_portal_user(portal_user, role):
	...
	terminal_framework.msgprint(_("Added {1} role to user {0}.").format(terminal_framework.bold(user_doc.name), role), alert=True)
