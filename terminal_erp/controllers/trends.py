...
from terminal_framework.utils import DateTimeLikeObject, getdate, today
...
def validate_filters(filters):
	...
	if not filters.get("company"):
		filters["company"] = terminal_framework.defaults.get_user_default("Company")
	for f in ["Fiscal Year", "Based On", "Period", "Company"]:
		if not filters.get(f.lower().replace(" ", "_")):
			terminal_framework.throw(_("{0} is mandatory").format(_(f)))

	if not terminal_framework.db.exists("Fiscal Year", filters.get("fiscal_year")):
		terminal_framework.throw(_("Fiscal Year {0} does not exist").format(filters.get("fiscal_year")))

	if filters.get("based_on") == filters.get("group_by"):
		terminal_framework.throw(_("'Based On' and 'Group By' can not be the same"))
	...
	if filters.get("period_based_on") and filters.period_based_on not in ["bill_date", "posting_date"]:
		terminal_framework.throw(
			msg=_("{0} can be either {1} or {2}.").format(
				terminal_framework.bold("Period based On"), terminal_framework.bold("Posting Date"), terminal_framework.bold("Billing Date")
			),
...
def get_data(filters, conditions):
	...
	year_start_date, year_end_date = terminal_framework.get_cached_value(
	...
	if filters.get("group_by"):
		...
		data1 = terminal_framework.db.sql(
		...
		for d in range(len(data1)):
			...
			for i in range(len(row)):
				...
				row1 = terminal_framework.db.sql(
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		...
		if trans in ["Sales Invoice", "Delivery Note", "Sales Order"]:
			...
			terminal_framework.throw(_("Project-wise data is not available for Quotation"))
