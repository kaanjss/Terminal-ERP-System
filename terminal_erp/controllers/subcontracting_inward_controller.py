...
class SubcontractingInwardController:
	...
	def validate_material_receipt(self):
		...
		for item in self.items:
			if not terminal_framework.get_cached_value("Item", item.item_code, "is_customer_provided_item"):
	...
	def validate_returns(self):
		for item in self.items:
			if not item.scio_detail:
				...
				or terminal_framework.get_cached_value("Subcontracting Inward Order Item", item.scio_detail, "item_code")
			):
				terminal_framework.throw(
	...
	def validate_manufacture(self):
		if next(item for item in self.items if item.is_finished_item).t_warehouse != (
			fg_warehouse := terminal_framework.get_cached_value("Work Order", self.work_order, "fg_warehouse")
		):
			terminal_framework.throw(
				_(
		...
		items = [
			...
			and terminal_framework.get_cached_value("Item", item.item_code, "is_customer_provided_item")
		]

		if terminal_framework.get_cached_value("Work Order", self.work_order, "skip_transfer"):
	...
	def _validate_manufacture_consumption_against_scio(self, items):
		...
		lookup = {
			d.rm_item_code: terminal_framework._dict(consumed_qty=d.consumed_qty, available_qty=d.available_qty)
	...
	def validate_warehouse_(self):
		if self.subcontracting_inward_order and self.purpose in [
			...
			for item in self.items:
				if self.purpose == "Material Transfer for Manufacture" and not terminal_framework.get_cached_value(
	...
	def get_serial_nos_and_batches_from_sres(self, scio_detail, only_pending=True):
		serial_nos, batch_nos = [], terminal_framework._dict()
			...
			if self._action in ["save", "submit"]:
				...
				for item in self.items:
					if not item.secondary_item_type and not item.is_legacy_scrap_item:
						delivered_qty, returned_qty = terminal_framework.get_value(
							"Subcontracting Inward Order Item",
							item.scio_detail,
	...
	def validate_delivery_on_save(self):
		allow_delivery_of_overproduced_qty = terminal_framework.get_single_value(
		...
		for item in self.items:
			...
			table = terminal_framework.qb.DocType("Subcontracting Inward Order Item")
			query = (
				terminal_framework.qb.from_(table)
				.select(
					(
			...
			if max_allowed_qty:
				...
				table = terminal_framework.qb.DocType("Subcontracting Inward Order Secondary Item")
				query = (
					terminal_framework.qb.from_(table)
	...
	def validate_receive_from_customer_cancel(self):
		if self.purpose == "Receive from Customer":
			for item in self.items:
				scio_rm_item = terminal_framework.get_value(
					"Subcontracting Inward Order Received Item",
					item.scio_detail,
					["received_qty", "returned_qty", "work_order_qty"],
					as_dict=True,
	...
	def validate_manufacture_entry_cancel(self):
		if self.subcontracting_inward_order and self.purpose == "Manufacture":
			fg_item_name = terminal_framework.get_cached_value(
				"Work Order", self.work_order, "subcontracting_inward_order_item"
			)
			produced_qty, delivered_qty = terminal_framework.get_value(
			...
			for item in [item for item in self.items if not item.is_finished_item]:
				if item.secondary_item_type or item.is_legacy_scrap_item:
					scio_secondary_item = terminal_framework.get_value(
	...
	def update_inward_order_item(self):
		if self.purpose == "Manufacture" and (
			scio_item_name := terminal_framework.get_cached_value(
				"Work Order", self.work_order, "subcontracting_inward_order_item"
			)
		):
			...
			for doctype, item_qty_map in qty_map.items():
				table = terminal_framework.qb.DocType(doctype)
				field = table[fieldname]
				doc_updates = {
					scio_detail: {fieldname: field + qty} for scio_detail, qty in item_qty_map.items()
				}
				terminal_framework.db.bulk_update(doctype, doc_updates, chunk_size=len(doc_updates))
	...
	def update_inward_order_received_items_for_raw_materials_receipt(self):
		...
		for item in self.items:
			if item.scio_detail:
				data[item.scio_detail] = terminal_framework._dict(
					{"transfer_qty": item.transfer_qty, "rate": item.customer_provided_item_cost}
				)
			else:
				scio_rm = terminal_framework.new_doc(
					"Subcontracting Inward Order Received Item",
					parent=self.subcontracting_inward_order,
					...
					main_item_code=terminal_framework.get_cached_value(
						"Subcontracting Inward Order Item", item.against_fg, "item_code"
					),
				)
		...
		if data:
			...
			result = terminal_framework.get_all(
				"Subcontracting Inward Order Received Item",
				filters={
					"parent": self.subcontracting_inward_order,
			...
			for d in result:
				...
				if not d.required_qty and not d.received_qty:
					terminal_framework.delete_doc("Subcontracting Inward Order Received Item", d.name)
				else:
	...
	def update_inward_order_received_items_for_manufacture(self):
		...
		item_code_wh = terminal_framework._dict(
			{
				(
					item.item_code,
					customer_warehouse
					if terminal_framework.get_cached_value("Item", item.item_code, "is_customer_provided_item")
		...
		if data := data.run(as_dict=True):
			...
			for extra_item in [
				...
				doc = terminal_framework.new_doc(
					"Subcontracting Inward Order Received Item",
					parent=self.subcontracting_inward_order,
					parenttype="Subcontracting Inward Order",
					parentfield="received_items",
					...
					reference_name=terminal_framework.get_cached_value(
						"Work Order", self.work_order, "subcontracting_inward_order_item"
					),
					required_qty=0,
	...
	def update_inward_order_secondary_items(self):
		if (scio := self.subcontracting_inward_order) and self.purpose == "Manufacture":
			...
			secondary_items = terminal_framework._dict(secondary_items)
			...
			if secondary_items:
				...
				if result:
					secondary_items_dict = terminal_framework._dict(
						{
							(d.item_code, d.warehouse): terminal_framework._dict(
					...
					for key, value in secondary_items_dict.items():
						if (
							...
							terminal_framework.delete_doc("Subcontracting Inward Order Secondary Item", value.name)
						else:
				...
				next_secondary_idx = (
					terminal_framework.db.count("Subcontracting Inward Order Secondary Item", {"parent": scio}) + 1
				)
				for secondary_item in [
					...
					doc = terminal_framework.new_doc(
						...
						delivered_qty=0,
						reference_name=terminal_framework.get_value(
							"Work Order", self.work_order, "subcontracting_inward_order_item"
						),
					)
					doc.flags.skip_docstatus_validation = True
	...
	def remove_reference_for_additional_items(self):
		if self.subcontracting_inward_order:
			items = [
				...
				if item.scio_detail
				and (
					not terminal_framework.db.exists("Subcontracting Inward Order Received Item", item.scio_detail)
					and not terminal_framework.db.exists("Subcontracting Inward Order Item", item.scio_detail)
					and not terminal_framework.db.exists("Subcontracting Inward Order Secondary Item", item.scio_detail)
				)
			]
			for item in items:
	...
	def create_stock_reservation_entries_for_inward(self):
		if self.purpose == "Receive from Customer":
			for item in self.items:
				...
				sre.has_serial_no = terminal_framework.get_cached_value("Item", item.item_code, "has_serial_no")
				sre.has_batch_no = terminal_framework.get_cached_value("Item", item.item_code, "has_batch_no")
				sre.reservation_based_on = "Qty" if not item.serial_and_batch_bundle else "Serial and Batch"
				if item.serial_and_batch_bundle:
					sabb = terminal_framework.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
		...
		if self.purpose == "Return Raw Material to Customer":
			for item in self.items:
				...
				if serial_list or batch_list:
					...
					for row in result:
						if serial_list:
							terminal_framework.get_doc("Serial and Batch Entry", row.sbe_name).db_set(
							...
							sbe_doc = terminal_framework.get_doc("Serial and Batch Entry", row.sbe_name)
							sbe_doc.db_set(
								"delivered_qty",
					...
					for sre_name, qty in qty_to_deliver.items():
						sre_doc = terminal_framework.get_doc("Stock Reservation Entry", sre_name)
						sre_doc.db_set(
							"delivered_qty",
					...
					for sre in sre_list:
						...
						sre_doc = terminal_framework.get_doc("Stock Reservation Entry", sre.name)
						sre_doc.db_set(
							"delivered_qty",
							sre_doc.delivered_qty + (qty if self._action == "submit" else -qty),
						)
