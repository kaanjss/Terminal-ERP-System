...
from terminal_framework import _, bold, qb, throw
from terminal_framework.contacts.doctype.address.address import get_address_display
...
from terminal_erp.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from terminal_erp.accounts.general_ledger import get_round_off_account_and_cost_center
from terminal_erp.accounts.party import (
	PURCHASE_TRANSACTION_TYPES,
	SALES_TRANSACTION_TYPES,
)
from terminal_erp.accounts.utils import (
	get_advance_payment_doctypes as _get_advance_payment_doctypes,
)
from terminal_erp.accounts.utils import get_fiscal_year, validate_fiscal_year
from terminal_erp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from terminal_erp.controllers.sales_and_purchase_return import validate_return
from terminal_erp.setup.utils import get_exchange_rate
from terminal_erp.stock.doctype.item.item import get_uom_conv_factor
from terminal_erp.stock.get_item_details import (
	ItemDetailsCtx,
	get_item_details,
)
from terminal_erp.utilities.regional import temporary_flag
from terminal_erp.utilities.transaction_base import TransactionBase


class AccountMissingError(terminal_framework.ValidationError):
	pass


class InvalidQtyError(terminal_framework.ValidationError):
	pass
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = terminal_erp.get_company_currency(self.company)
	...
	def onload(self):
		self.set_onload(
			...
			terminal_framework.client_cache.get_doc("Accounts Settings").make_payment_via_journal_entry,
		)
	...
	def on_update(self):
		from terminal_erp.controllers.taxes_and_totals import process_item_wise_tax_details
	...
	def ensure_supplier_is_not_blocked(self):
		...
		if supplier_name:
			supplier = terminal_framework.get_lazy_doc("Supplier", supplier_name)
	...
	def validate_against_voucher_outstanding(self):
		from terminal_framework.model.meta import get_meta
		...
		if self.get("is_return") and self.return_against and not (self.get("is_pos") or self.get("is_paid")):
			against_voucher_outstanding = terminal_framework.get_value(
			...
			if self.get("update_outstanding_for_self"):
				msg = _(
					...
					terminal_framework.bold(document_type),
					get_link_to_form(self.doctype, self.get("return_against")),
					terminal_framework.bold(_("Update Outstanding for Self")),
				)
			...
			if msg:
				...
				terminal_framework.msgprint(msg)
	...
	def validate(self):
		...
		from terminal_erp.accounts.services.internal_transfer import InternalTransferService
		...
		from terminal_erp.accounts.services.taxes import TaxService
		...
		from terminal_erp.accounts.services.party_validation import PartyValidator
			...
			if invalid_advances := [x for x in self.advances if not x.reference_type or not x.reference_name]:
				terminal_framework.throw(
					_(
						...
						terminal_framework.bold(comma_and([x.idx for x in invalid_advances])),
						terminal_framework.bold(_("Advance Payments")),
					)
				)
	...
	def set_default_letter_head(self):
		if hasattr(self, "letter_head") and not self.letter_head:
			self.letter_head = terminal_framework.db.get_value("Company", self.company, "default_letter_head")
	...
	def _remove_references_in_unreconcile(self):
		upe = terminal_framework.qb.DocType("Unreconcile Payment Entries")
		rows = (
			terminal_framework.qb.from_(upe)
		...
		if rows:
			references_map = terminal_framework._dict()
			for x in rows:
				references_map.setdefault(x.parent, []).append(x.name)

			for doc, rows in references_map.items():
				unreconcile_doc = terminal_framework.get_doc("Unreconcile Payment", doc)
				for row in rows:
		...
		unreconcile_docs = terminal_framework.db.get_all("Unreconcile Payment", filters={"voucher_no": self.name})
		for x in unreconcile_docs:
			_doc = terminal_framework.get_doc("Unreconcile Payment", x.name)
			if _doc.docstatus == 1:
				_doc.cancel()
			_doc.delete()
	...
	def _remove_references_in_repost_doctypes(self):
		...
		for _doctype in repost_doctypes:
			dt = terminal_framework.qb.DocType(_doctype)

			cancelled_entries = (
				terminal_framework.qb.from_(dt)
			...
			if rows:
				...
				for doc, rows in references_map.items():
					repost_doc = terminal_framework.get_doc(doc[0], doc[1])
	...
	def on_trash(self):
		from terminal_erp.accounts.utils import delete_exchange_gain_loss_journal
		...
		if terminal_framework.get_single_value("Accounts Settings", "delete_linked_ledger_entries"):
			...
			terminal_framework.qb.from_(ple).delete().where(
	...
	def validate_company_in_accounting_dimension(self):
		...
		dimension_list = (
			terminal_framework.qb.from_(accounting_dimension)
	...
	def validate_company(self, dimension_list, child=None):
		for dimension in dimension_list:
			if not child:
				dimension_value = self.get(terminal_framework.scrub(dimension))
			else:
				dimension_value = child.get(terminal_framework.scrub(dimension))

			if dimension_value:
				company = terminal_framework.get_cached_value(dimension, dimension_value, "company")
				if company and company != self.company:
					terminal_framework.throw(
						_("{0}: {1} does not belong to the Company: {2}").format(
							dimension, terminal_framework.bold(dimension_value), self.company
						)
					)
	...
	def validate_return_against_account(self):
		if self.doctype in ["Sales Invoice", "Purchase Invoice"] and self.is_return and self.return_against:
			...
			original_account = terminal_framework.get_value(self.doctype, self.return_against, cr_dr_account_field)
			if original_account != self.get(cr_dr_account_field):
				terminal_framework.throw(
					_(
						...
						terminal_framework.bold(_(self.meta.get_label(cr_dr_account_field), context=self.doctype)),
						terminal_framework.bold(original_account),
						terminal_framework.bold(self.return_against),
					)
				)
		...
		for item in self.get("items"):
			if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
				if not item.get(field_map.get(self.doctype)):
					default_deferred_account = terminal_framework.get_cached_value(
	...
	def validate_auto_repeat_subscription_dates(self):
		if self.get("from_date") and self.get("to_date") and getdate(self.from_date) > getdate(self.to_date):
			terminal_framework.throw(_("To Date cannot be before From Date"), title=_("Invalid Auto Repeat Date"))
	...
	def calculate_taxes_and_totals(self):
		from terminal_erp.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from terminal_erp.accounts.party import validate_due_date
		...
		if terminal_framework.flags.in_import and getdate(self.due_date) < getdate(posting_date):
	...
	def set_price_list_currency(self, buying_or_selling):
		...
		if self.meta.get_field("currency"):
			...
			if self.meta.get_field(fieldname) and self.get(fieldname):
				self.price_list_currency = terminal_framework.db.get_value("Price List", self.get(fieldname), "currency")
			...
			if (
				...
				and terminal_framework.db.get_single_value("Buying Settings", "use_transaction_date_exchange_rate")
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or terminal_erp.get_default_cost_center(self.company),
						)
	...
	def apply_pricing_rule_on_items(self, item, pricing_rule_args):
		if not pricing_rule_args.get("validate_applied_rule", 0):
			...
			for pricing_rule in get_applied_pricing_rules(item.get("pricing_rules")):
				pricing_rule_doc = terminal_framework.get_cached_doc("Pricing Rule", pricing_rule)
					...
					if item.get(field) < pricing_rule_doc.get(field):
						...
						terminal_framework.msgprint(
							_("Row {0}: user has not applied the rule {1} on the item {2}").format(
								item.idx, terminal_framework.bold(title), terminal_framework.bold(item.item_code)
							)
						)
	...
	def get_gl_dict(self, args, account_currency=None, item=None):
		from terminal_erp.accounts.services.base_gl_composer import get_gl_dict

		return get_gl_dict(self, args, account_currency, item)

	def get_voucher_subtype(self):
		from terminal_erp.accounts.services.base_gl_composer import get_voucher_subtype

		return get_voucher_subtype(self)

	def get_value_in_transaction_currency(self, account_currency, gl_dict, field):
		from terminal_erp.accounts.services.base_gl_composer import get_value_in_transaction_currency
	...
	def validate_zero_qty_for_return_invoices_with_stock(self):
		...
		if rows:
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(comma_and(["#" + str(x.idx) for x in rows])))
			)
			...
			if not flt(item.qty):
				terminal_framework.throw(
					msg=_("Row #{0}: Quantity for Item {1} cannot be zero.").format(
						item.idx, terminal_framework.bold(item.item_code)
					),
	...
	def validate_account_currency(self, account, account_currency=None):
		from terminal_erp.accounts.services.base_gl_composer import validate_account_currency
	...
	def clear_unallocated_advances(self, childtype, parentfield):
		...
		doctype = terminal_framework.qb.DocType(childtype)
	...
	def apply_shipping_rule(self):
		if self.shipping_rule:
			shipping_rule = terminal_framework.get_doc("Shipping Rule", self.shipping_rule)
		...
		for fieldname in ("shipping_address_name", "shipping_address"):
			...
			if shipping_field and shipping_field.fieldtype == "Link":
				if self.get(fieldname):
					return terminal_framework.get_doc("Address", self.get(fieldname))

		return {}

	@terminal_framework.whitelist()
	def set_advances(self):
		from terminal_erp.accounts.services.advances import set_advances

		set_advances(self)

	def get_advance_entries(self, include_unallocated=True):
		from terminal_erp.accounts.services.advances import get_advance_entries

		return get_advance_entries(self, include_unallocated)

	def is_inclusive_tax(self):
		is_inclusive = cint(terminal_framework.get_single_value("Accounts Settings", "show_inclusive_tax_in_print"))
	...
	def should_show_taxes_as_table_in_print(self):
		return cint(terminal_framework.get_single_value("Accounts Settings", "show_taxes_as_table_in_print"))

	def validate_advance_entries(self):
		from terminal_erp.accounts.services.advances import validate_advance_entries

		validate_advance_entries(self)

	def set_advance_gain_or_loss(self):
		from terminal_erp.accounts.services.advances import set_advance_gain_or_loss
	...
	def gain_loss_journal_already_booked(
		...
		from terminal_erp.accounts.services.exchange_gain_loss import gain_loss_journal_already_booked
	...
	def make_exchange_gain_loss_journal(
		...
		from terminal_erp.accounts.services.exchange_gain_loss import make_exchange_gain_loss_journal

		make_exchange_gain_loss_journal(self, args, dimensions_dict)

	def is_payable_account(self, reference_doctype, account):
		from terminal_erp.accounts.services.exchange_gain_loss import is_payable_account
	...
	def update_against_document_in_jv(self):
				...
				args = terminal_framework._dict(
					{
						...
						"difference_account": terminal_framework.get_cached_value(
		...
		if lst:
			from terminal_erp.accounts.utils import reconcile_against_document
	...
	def cancel_system_generated_credit_debit_notes(self):
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			for x in journals:
				terminal_framework.get_doc("Journal Entry", x).cancel()

	def on_cancel(self):
		from terminal_erp.accounts.doctype.bank_transaction.bank_transaction import (
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
			...
			if terminal_framework.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
				unlink_ref_doc_from_payment_entries(self)

		elif self.doctype in ["Sales Order", "Purchase Order"]:
			if terminal_framework.get_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order"):
				unlink_ref_doc_from_payment_entries(self)
	...
	def unlink_ref_doc_from_po(self):
		...
		if linked_po:
			...
			terminal_framework.msgprint(_("Purchase Orders {0} are unlinked").format("\n".join(linked_po)))

	def get_company_default(self, fieldname, ignore_validation=False):
		from terminal_erp.accounts.utils import get_company_default
	...
	def get_stock_items(self):
		...
		if item_codes:
			stock_items = terminal_framework.db.get_values(
	...
	def get_asset_items(self):
		...
		item_codes = list(set(item.item_code for item in self.get("items")))
		if item_codes:
			asset_items = terminal_framework.db.get_values(
	...
	def calculate_total_advance_from_ledger(self):
		from terminal_erp.accounts.services.advances import calculate_total_advance_from_ledger

		return calculate_total_advance_from_ledger(self)

	def set_total_advance_paid(self):
		from terminal_erp.accounts.services.advances import set_total_advance_paid

		set_total_advance_paid(self)

	def set_advance_payment_status(self):
		from terminal_erp.accounts.services.advances import set_advance_payment_status
	...
	def company_abbr(self):
		if not hasattr(self, "_abbr"):
			self._abbr = terminal_framework.get_cached_value("Company", self.company, "abbr")
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			terminal_framework.bold(_("Debit To")) if self.doctype == "Sales Invoice" else terminal_framework.bold(_("Credit To"))
		)
		rec_or_pay = "Receivable" if self.doctype == "Sales Invoice" else "Payable"

		link_to_party = terminal_framework.utils.get_link_to_form(party_type, party)
		link_to_company = terminal_framework.utils.get_link_to_form("Company", self.company)

		message = _("{0} Account not found against Customer {1}.").format(db_or_cr, terminal_framework.bold(party) or "")
		...
		terminal_framework.throw(message, title=_("Account Missing"), exc=AccountMissingError)
	...
	def delink_advance_entries(self, linked_doc_name):
		from terminal_erp.accounts.services.advances import delink_advance_entries
	...
	def group_similar_items(self):
		...
		fields_to_group = terminal_framework.get_hooks("fields_for_group_similar_items")
	...
	def is_rounded_total_disabled(self):
		if self.meta.get_field("disable_rounded_total"):
			...
			return terminal_framework.db.get_single_value("Global Defaults", "disable_rounded_total")
	...
	def get_common_party_link(self) -> terminal_framework._dict | None:
		from terminal_erp.accounts.services.internal_transfer import InternalTransferService

		return InternalTransferService(self).get_common_party_link()

	def create_advance_and_reconcile(self, party_link):
		from terminal_erp.accounts.services.advances import create_advance_and_reconcile

		create_advance_and_reconcile(self, party_link)

	def check_conversion_rate(self):
		default_currency = terminal_erp.get_company_currency(self.company)
	...
	def check_if_fields_updated(self, fields_to_check, child_tables):
		from terminal_erp.accounts.services.child_item_update import check_if_child_table_updated
	...
	def repost_accounting_entries(self):
		repost_ledger = terminal_framework.new_doc("Repost Accounting Ledger")
	...
	def set_transaction_currency_and_rate_in_gl_map(self, gl_entries: list) -> None:
		from terminal_erp.accounts.services.exchange_gain_loss import set_transaction_currency_and_rate_in_gl_map
	...
	def set_discount_amount_after_mapping(self, source_doc):
		...
		doctype_table = terminal_framework.qb.DocType(self.doctype)
		item_table = terminal_framework.qb.DocType(item_doctype)
		...
		query = (
			terminal_framework.qb.from_(doctype_table)
		...
		if is_return:
			...
			item_meta = terminal_framework.get_meta(item_doctype)
...
from terminal_erp.accounts.services.advances import (
	get_advance_journal_entries,
	get_advance_payment_entries,
	get_advance_payment_entries_for_regional,
	get_common_query,
)
from terminal_erp.accounts.services.taxes import (
...
def update_invoice_status():
	...
	today = getdate()
	payment_schedule = terminal_framework.qb.DocType("Payment Schedule")
	for doctype in ("Sales Invoice", "Purchase Invoice"):
		invoice = terminal_framework.qb.DocType(doctype)
		...
		payable_amount = (
			terminal_framework.qb.from_(payment_schedule)
		...
		total_amount = terminal_framework.qb.terms.Case().when(consider_base_amount, base_total).else_(total)
		...
		terminal_framework.qb.update(invoice).set("status", status).where(conditions).run()


from terminal_erp.accounts.services.payment_schedule import (
	get_discount_date,
	get_due_date,
...
def get_supplier_block_status(party_name):
	...
	a `Supplier`
	"""
	supplier = terminal_framework.get_doc("Supplier", party_name)
	info = {
...
from terminal_erp.accounts.services.child_item_update import update_child_qty_rate
...
from terminal_erp.accounts.services.base_gl_composer import (
...
def get_missing_company_details(doctype: str, docname: str):
	from terminal_framework.contacts.doctype.address.address import get_address_display_list

	company = terminal_framework.db.get_value(doctype, docname, "company")
	if doctype in ["Purchase Order", "Purchase Invoice"]:
		company_address = terminal_framework.db.get_value(doctype, docname, "billing_address")
	elif doctype in ["Request for Quotation"]:
		company_address = terminal_framework.db.get_value(doctype, docname, "shipping_address")
	else:
		company_address = terminal_framework.db.get_value(doctype, docname, "company_address")

	company_details = terminal_framework.get_value(
	...
	if not all(required_fields) and not terminal_framework.has_permission("Company", "write", throw=False):
		terminal_framework.msgprint(
			_(
	...
	if not company_address and not terminal_framework.has_permission(doctype, "write", throw=False):
		terminal_framework.msgprint(
			_(
	...
	if needs_new_company_address and not terminal_framework.has_permission("Address", "create", throw=False):
		terminal_framework.msgprint(
			_(
...
def update_company_master_and_address(current_doctype: str, name: str, company: str, details: dict | str):
	from terminal_framework.utils import validate_email_address

	if not terminal_framework.has_permission(current_doctype, "write", doc=name, throw=False):
		terminal_framework.throw(
			_("You don't have permission to update this document. Please contact your System Manager."),
			title=_("Insufficient Permissions"),
		)

	if not terminal_framework.has_permission("Company", "write", doc=company, throw=False):
		terminal_framework.throw(
	...
	if isinstance(details, str):
		details = terminal_framework.parse_json(details)
	...
	if company_fields_to_update:
		terminal_framework.db.set_value("Company", company, company_fields_to_update)

	company_address = details.get("company_address")
	if details.get("address_line1"):
		if not terminal_framework.has_permission("Address", "create", throw=False):
			terminal_framework.throw(
				_(
...
def update_doc_company_address(current_doctype, docname, company_address, details):
	...
	current_display = terminal_framework.db.get_value(current_doctype, docname, display_field)
	...
	(
		terminal_framework.qb.update(DocType)
