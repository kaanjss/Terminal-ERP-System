...
def get_accounting_ledger_preview(doc, filters):
	...
	terminal_framework.db.savepoint("ledger_preview")
	try:
		doc.docstatus = 1
		...
		terminal_framework.db.rollback(save_point="ledger_preview")

	return gl_columns, gl_data


def get_stock_ledger_preview(doc, filters):
	from terminal_erp.stock.report.stock_ledger.stock_ledger import get_columns as get_sl_columns
...
def get_gl_entries_for_preview(doctype, docname, fields):
	return terminal_framework.get_all("GL Entry", filters={"voucher_type": doctype, "voucher_no": docname}, fields=fields)
