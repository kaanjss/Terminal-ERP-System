...
from terminal_framework.utils import cint, cstr, flt, get_link_to_form, getdate
...
from terminal_erp.accounts.utils import cancel_exchange_gain_loss_journal
...
# Re-exported for backward compatibility; canonical home is terminal_erp.exceptions.
from terminal_erp.exceptions import (
	BatchExpiredError,
...
from terminal_erp.stock.services.internal_transfer import StockInternalTransferService
from terminal_erp.stock.stock_ledger import get_items_to_be_repost
...
class StockController(AccountsController):
		...
		if self.doctype in [
			...
			for item in self.get("items"):
				if (
					...
					and terminal_framework.get_cached_value("Item", item.item_code, "is_stock_item")
				):
					terminal_framework.toast(
	...
	def validate_items_exist(self):
		...
		exists_items = terminal_framework.get_all("Item", filters={"name": ("in", items)}, pluck="name")
		non_exists_items = set(items) - set(exists_items)

		if non_exists_items:
			terminal_framework.throw(_("Items {0} do not exist in the Item master.").format(", ".join(non_exists_items)))

	def get_item_wise_inventory_account_map(self):
		inventory_account_map = terminal_framework._dict()
	...
	def use_item_inventory_account(self):
		return terminal_framework.get_cached_value("Company", self.company, "enable_item_wise_inventory_account")

	def get_inventory_account_dict(self, row, inventory_account_map, warehouse_field=None):
		account_dict = terminal_framework._dict()
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False, via_landed_cost_voucher=False):
		...
		need_inventory_map = (self.get_stock_items() or self.get("packed_items")) and (
			cint(terminal_erp.is_perpetual_inventory_enabled(self.company))
		)
	...
	def make_gl_entries_on_cancel(self, from_repost=False):
		...
		if terminal_framework.db.exists("GL Entry", {"voucher_type": self.doctype, "voucher_no": self.name}):
	...
	def validate_inspection(self):
		from terminal_erp.stock.services.quality_inspection_service import QualityInspectionService
	...
	def update_blanket_order(self):
		...
		for blanket_order in blanket_orders:
			terminal_framework.get_doc("Blanket Order", blanket_order).update_ordered_qty()

	def validate_customer_provided_item(self):
		for d in self.get("items"):
			# Customer Provided parts will have zero valuation rate
			if terminal_framework.get_cached_value("Item", d.item_code, "is_customer_provided_item"):
	...
	def add_gl_entry(
		...
		from terminal_erp.accounts.services.base_gl_composer import add_gl_entry
	...
	def update_stock_reservation_entries(self):
		...
		if (
			...
			or (
				...
				and terminal_framework.get_value("Subcontracting Order", self.subcontracting_order, "reserve_stock")
			)
			...
			for item in self.get(data_map[purpose or self.doctype]["table_name"]):
				...
				for sre in sre_list:
					...
					sre_doc = terminal_framework.get_doc("Stock Reservation Entry", sre)
			...
			if (status := status_map.get(ref_name)) in ("Closed", "On Hold"):
				errors.append(
					_("{ref_doctype} {ref_name} status is {status}.").format(
						ref_doctype=terminal_framework.bold(_(ref_doctype)),
						ref_name=terminal_framework.bold(ref_name),
						status=terminal_framework.bold(_(status)),
					)
				)

		if errors:
			terminal_framework.throw("<br>".join(errors), terminal_framework.InvalidStatusError)


@terminal_framework.whitelist()
def show_accounting_ledger_preview(company: str, doctype: str, docname: str):
	from terminal_erp.controllers.ledger_preview import get_accounting_ledger_preview

	filters = terminal_framework._dict(company=company, include_dimensions=1)
	doc = terminal_framework.get_lazy_doc(doctype, docname)
...
def show_stock_ledger_preview(company: str, doctype: str, docname: str):
	from terminal_erp.controllers.ledger_preview import get_stock_ledger_preview

	filters = terminal_framework._dict(company=company)
	doc = terminal_framework.get_lazy_doc(doctype, docname)
...
def repost_required_for_queue(doc: StockController) -> bool:
	...
	consuming_sles = terminal_framework.db.get_all(
		"Stock Ledger Entry",
		filters={
...
def check_item_quality_inspection(doctype: str, docstatus: str | int, items: str | list[dict]):
	from terminal_erp.stock.services.quality_inspection_service import INSPECTION_FIELDNAME_MAP

	items = terminal_framework.parse_json(items)

	inspection_fieldname = INSPECTION_FIELDNAME_MAP.get(doctype)
	if inspection_fieldname is None:
		return items if doctype == "Stock Entry" else []

	allow_after_transaction = cint(docstatus) == 1 and terminal_framework.get_single_value(
...
def make_quality_inspections(
	...
	for item in items:
		...
		quality_inspection = terminal_framework.get_doc(
			{
				"company": company,
				"doctype": "Quality Inspection",
				"inspection_type": inspection_type,
				"inspected_by": terminal_framework.session.user,
...
def future_sle_exists(args, sl_entries=None):
	...
	if not hasattr(terminal_framework.local, "future_sle"):
	...
	for d in data:
		terminal_framework.local.future_sle[key][(d.item_code, d.warehouse)] = d.total_row
...
def validate_future_sle_not_exists(args, key, sl_entries=None):
	...
	if not sl_entries and hasattr(terminal_framework.local, "future_sle"):
		if key not in terminal_framework.local.future_sle:
			return False

		if not terminal_framework.local.future_sle.get(key) or (
			item_key and item_key not in terminal_framework.local.future_sle.get(key)
		):
			return True
...
def get_cached_data(args, key):
	if key not in terminal_framework.local.future_sle:
		terminal_framework.local.future_sle[key] = terminal_framework._dict({})

	if args.get("item_code"):
		item_key = (args.get("item_code"), args.get("warehouse"))
		count = terminal_framework.local.future_sle[key].get(item_key)

		return True if (count or count == 0) else False
	else:
		return terminal_framework.local.future_sle[key]
...
def create_repost_item_valuation_entry(args):
	...
	repost_entry = terminal_framework.new_doc("Repost Item Valuation")
...
def make_bundle_for_material_transfer(**kwargs):
	...
	bundle_doc = terminal_framework.get_doc("Serial and Batch Bundle", kwargs.serial_and_batch_bundle)

	if not kwargs.type_of_transaction:
		kwargs.type_of_transaction = "Inward"

	bundle_doc = terminal_framework.copy_doc(bundle_doc)
...
def get_item_wise_inventory_account_map(rows, company):
	...
	inventory_map = terminal_framework._dict()
	...
	for row in rows:
		...
		if item_defaults.default_inventory_account:
			inventory_map[item_code] = terminal_framework._dict(
				{
