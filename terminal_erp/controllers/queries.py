...
from terminal_framework import qb, scrub
from terminal_framework.permissions import has_permission
from terminal_framework.query_builder import Case, Criterion, DocType
from terminal_framework.query_builder.functions import (
	Cast_,
	Concat,
	IfNull,
	Length,
...
from terminal_framework.utils import nowdate, today, unique
from pypika import Order

import terminal_erp
from terminal_erp.accounts.utils import build_qb_match_conditions
from terminal_erp.stock.get_item_details import ItemDetailsCtx, _get_item_tax_template
from terminal_erp.stock.utils import get_combine_datetime
from terminal_erp.utilities.query import get_filter_conditions_qb
...
def employee_query(
	...
	if reference_doctype and ignore_user_permissions:
		ignore_permissions = has_ignored_field(reference_doctype, doctype) and has_permission(
			doctype,
			ptype="select" if terminal_framework.only_has_select_perm(doctype) else "read",
		)
...
def has_ignored_field(reference_doctype, doctype):
	meta = terminal_framework.get_meta(reference_doctype)
...
def lead_query(
	...
	searchfields = terminal_framework.get_meta(doctype).get_search_fields()
	...
	query = terminal_framework.qb.get_query("Lead", fields=fields, filters=filters, ignore_permissions=False)
...
def tax_account_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	company_currency = terminal_erp.get_company_currency(filters.get("company"))

	Account = terminal_framework.qb.DocType("Account")

	def get_accounts(with_account_type_filter):
		query = terminal_framework.qb.get_query("Account", fields=["name", "parent_account"], ignore_permissions=False)
		query = (
...
def item_query(
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			item_rules_list = terminal_framework.get_all(
				"Party Specific Item",
				filters={"party_type": party_type},
				fields=["party", "restrict_based_on", "based_on_value"],
			)

			party_group_rules_list = terminal_framework.get_all(
				"Party Specific Item",
				filters={"party_type": group},
				fields=["party as party_group", "restrict_based_on", "based_on_value"],
			)
			current_party_group = terminal_framework.get_value(party_type, party, terminal_framework.scrub(group))
	...
	barcode_subquery = (
		terminal_framework.qb.from_(barcode_tbl).select(barcode_tbl.parent).where(barcode_tbl.barcode.like(search_str))
	)
	search_conditions.append(item.item_code.isin(barcode_subquery))

	# Condition for the description
	if terminal_framework.db.estimate_count("Item") < 50000 and "description" not in fields_to_process:
	...
	# Building the query
	query = (
		terminal_framework.get_query(doctype, filters=filters, ignore_permissions=False)
		.select(*query_select)
...
def bom(
	...
	query = terminal_framework.qb.get_query("BOM", fields=fields, filters=filters, ignore_permissions=False)
...
def get_project_name(
	...
	searchfields = [
		x for x in terminal_framework.get_meta(doctype).get_search_fields() if x not in ["customer", "status"]
	]
...
def get_batch_no(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	doctype = "Batch"
	meta = terminal_framework.get_meta(doctype, cached=True)
	searchfields = meta.get_search_fields()
	page_len = 300
...
def get_batches_from_serial_and_batch_bundle(searchfields, txt, filters, start=0, page_len=100):
	bundle = terminal_framework.qb.DocType("Serial and Batch Entry")
	stock_ledger_entry = terminal_framework.qb.DocType("Stock Ledger Entry")
	batch_table = terminal_framework.qb.DocType("Batch")
	...
	bundle_query = (
		terminal_framework.qb.from_(bundle)
...
def get_filtered_dimensions(
	...
	from terminal_erp.accounts.doctype.accounting_dimension_filter.accounting_dimension_filter import (
	...
	output = terminal_framework.get_list(
		doctype,
		fields=fields,
...
def warehouse_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: list):
	...
	meta = terminal_framework.get_meta("Warehouse")
	...
	query = (
		terminal_framework.qb.get_query("Warehouse", fields=[warehouse_field], ignore_permissions=False)
		.left_join(bin_dt)
		.on(join_condition)
		.select(
...
def item_manufacturer_query(
	...
	item_manufacturers = terminal_framework.get_all(
		"Item Manufacturer",
...
def get_purchase_invoices(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	pi = terminal_framework.qb.DocType("Purchase Invoice")
...
def get_tax_template(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	item_doc = terminal_framework.get_cached_doc("Item", filters.get("item_code"))
	item_group = filters.get("item_group")
	company = filters.get("company")
	taxes = item_doc.taxes or []

	while item_group:
		item_group_doc = terminal_framework.get_cached_doc("Item Group", item_group)
	...
	if not taxes:
		...
		if txt:
			...
			tax_template_doc = terminal_framework.get_meta("Item Tax Template")
...
def get_payment_terms_for_references(
	...
	terms = []
	if filters:
		terms = terminal_framework.db.get_all(
			"Payment Schedule",
...
def get_item_uom_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	if terminal_framework.get_single_value("Stock Settings", "allow_uom_with_conversion_rate_defined_in_item"):
...
def get_warehouse_address(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	table = terminal_framework.qb.DocType(doctype)
	child_table = terminal_framework.qb.DocType("Dynamic Link")
