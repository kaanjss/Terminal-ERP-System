...
from terminal_framework.query_builder import functions
from terminal_framework.utils import cint, flt, round_based_on_smallest_currency_fraction

import terminal_erp
from terminal_erp.accounts.doctype.journal_entry.journal_entry import get_exchange_rate
from terminal_erp.accounts.doctype.pricing_rule.utils import get_applied_pricing_rules
from terminal_erp.controllers.accounts_controller import (
	validate_conversion_rate,
	validate_inclusive_tax,
	validate_taxes_and_charges,
)
from terminal_erp.deprecation_dumpster import deprecated
...
class calculate_taxes_and_totals:
	def __init__(self, doc: Document):
		...
		terminal_framework.flags.round_row_wise_tax = terminal_framework.get_single_value("Accounts Settings", "round_row_wise_tax")
				...
				if taxes:
					if item.item_tax_template not in taxes:
						...
						terminal_framework.msgprint(
							...
							).format(item.idx, terminal_framework.bold(item.item_code))
						)
	...
	def validate_conversion_rate(self):
		...
		company_currency = terminal_erp.get_company_currency(self.doc.company)
	...
	def _load_item_tax_rate(self, item_tax_rate):
		return terminal_framework.parse_json(item_tax_rate) if item_tax_rate else {}
	...
	def calculate_shipping_charges(self):
		...
		if hasattr(self.doc, "shipping_rule") and self.doc.shipping_rule:
			shipping_rule = terminal_framework.get_doc("Shipping Rule", self.doc.shipping_rule)
	...
	def round_off_totals(self, tax):
		if tax.account_head in terminal_framework.flags.round_off_applicable_accounts:
	...
	def set_discount_amount(self):
		...
		if self.doc.get("is_return") and self.doc.get("return_against"):
			doctype = terminal_framework.qb.DocType(self.doc.doctype)
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				terminal_framework.throw(_("Please select Apply Discount On"))
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			if (
				...
				write_off_limit = flt(
					terminal_framework.db.get_value("POS Profile", self.doc.pos_profile, "write_off_limit")
				)
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		...
		if pending_amount > 0:
			default_mode_of_payment = terminal_framework.db.get_value(
				"POS Payment Method",
...
def get_itemised_tax_breakup_data(doc):
	...
	for item_code, taxes in itemised_tax.items():
		...
		itemised_tax_data.append(terminal_framework._dict({"item": item_code, "taxable_amount": taxable_amount, **taxes}))
...
def get_itemised_tax(doc, with_tax_account=False):
	...
	for row in doc.get("_item_wise_tax_details"):
		...
		tax_info = itemised_tax.setdefault(item_code, terminal_framework._dict()).setdefault(
			tax.description,
			terminal_framework._dict(
				{
					"tax_amount": 0.0,
...
def get_rounding_tax_settings():
	return terminal_framework.get_single_value("Accounts Settings", "round_row_wise_tax")
	...
	def set_account_currency(self):
		...
		for d in self.doc.get(self.tax_field):
			if not d.account_currency:
				account_currency = terminal_framework.get_cached_value("Account", d.expense_account, "account_currency")
	...
	def set_exchange_rate(self):
		...
		for d in self.doc.get(self.tax_field):
			...
			if not d.exchange_rate:
				terminal_framework.throw(_("Row {0}: Exchange Rate is mandatory").format(d.idx))
