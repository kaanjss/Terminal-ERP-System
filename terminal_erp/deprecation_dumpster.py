...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Terminal ERP app. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Terminal ERP version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
from terminal_framework.deprecation_dumpster import Color, _deprecated, colorize


# we use Warning because DeprecationWarning has python default filters which would exclude them from showing
# see also terminal_framework.__init__ enabling them when a dev_server
class Terminal ERPDeprecationError(Warning):
...
class Terminal ERPDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingTerminal ERPDeprecationWarning(Terminal ERPDeprecationWarning):
...
warnings.simplefilter("error", Terminal ERPDeprecationError)
warnings.simplefilter("ignore", PendingTerminal ERPDeprecationWarning)


class V15Terminal ERPDeprecationWarning(Terminal ERPDeprecationError):
	pass


class V16Terminal ERPDeprecationWarning(Terminal ERPDeprecationWarning):
	pass


class V17Terminal ERPDeprecationWarning(PendingTerminal ERPDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}Terminal ERPDeprecationWarning"
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: terminal_framework.utils.make_esc  (fully qualified)
...
### Party starts here
@deprecated(
	"terminal_erp.controllers.taxes_and_totals.get_itemised_taxable_amount",
	"2024-11-07",
	"v17",
...
def taxes_and_totals_get_itemised_taxable_amount(items):
	...
	itemised_taxable_amount = terminal_framework._dict()
	for item in items:
...
@deprecated(
	"terminal_erp.stock.get_pos_profile_item_details",
	"2024-11-19",
	"v16",
	"Use terminal_erp.stock.get_pos_profile_item_details_ with a flipped signature",
)
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from terminal_erp.stock.get_item_details import get_pos_profile_item_details_

	return get_pos_profile_item_details_(ctx, company, pos_profile=pos_profile, update_data=update_data)


@deprecated(
	"terminal_erp.stock.get_item_warehouse",
	"2024-11-19",
	"v16",
	"Use terminal_erp.stock.get_item_warehouse_ with a flipped signature",
)
