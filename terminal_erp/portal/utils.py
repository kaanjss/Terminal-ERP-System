...
def set_default_role(doc, method):
	...
	if terminal_framework.flags.setting_role or terminal_framework.flags.in_migrate:
		return

	roles = terminal_framework.get_roles(doc.name)

	contact_name = terminal_framework.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = terminal_framework.get_doc("Contact", contact_name)
		for link in contact.links:
			terminal_framework.flags.setting_role = True
...
def create_customer_or_supplier():
	...
	if terminal_framework.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = terminal_framework.get_roles()
	portal_settings = terminal_framework.get_single("Portal Settings")
	...
	fullname = terminal_framework.utils.get_fullname(user)
...
def create_party(doctype, fullname):
	party = terminal_framework.new_doc(doctype)
...
def party_exists(doctype, user):
	...
	contact_name = terminal_framework.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = terminal_framework.get_doc("Contact", contact_name)
