...
These are Terminal ERP-local because they are only consumed by Terminal ERP. They return
``pypika`` criteria (rather than the raw SQL strings produced by
``terminal_framework.desk.reportview.get_match_cond`` / ``get_filters_cond``) so the conditions
can be applied to any ``terminal_framework.qb`` query via ``.where(...)`` — including joins and
aliased queries where the permission-checked doctype is not the single base of
``terminal_framework.qb.get_query``.

They are thin wrappers over ``terminal_framework.database.query.Engine`` (``get_permission_conditions``
/ ``apply_filters``), which pre-date this code. Where the permission-checked doctype *is*
the base of the query, prefer ``terminal_framework.qb.get_query(doctype, ignore_permissions=False)``
...
def get_match_conditions_qb(doctype, table=None, user=None):
	...
	Query-builder equivalent of ``terminal_framework.desk.reportview.get_match_cond`` /
	...
	Args:
	        ...
	        table: pypika table the conditions should reference. Defaults to
	                ``terminal_framework.qb.DocType(doctype)``.
	        user: user to evaluate permissions for. Defaults to the session user.
	"""
	from terminal_framework.database.query import Engine
...
def get_filter_conditions_qb(doctype, filters, ignore_permissions=None):
	...
	Query-builder equivalent of ``terminal_framework.desk.reportview.get_filters_cond`` (which returns a
	raw SQL string). Accepts the standard terminal_framework filter forms (dict, or list of
	``[doctype, field, op, value]`` rows) and returns pypika criteria that can be applied to
	any ``terminal_framework.qb`` query via ``.where(...)``.
	"""
	if not filters:
		return []
...
def get_event_conditions_qb(doctype, filters=None):
	...
	Query-builder equivalent of ``terminal_framework.desk.calendar.get_event_conditions(..., as_qb=True)``:
	a list of pypika criteria suitable for applying to a ``terminal_framework.qb`` query via ``.where(...)``
	(e.g. calendar feeds that join across multiple doctypes).
	"""
	if not terminal_framework.has_permission(doctype):
