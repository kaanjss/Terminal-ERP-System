...
from terminal_erp.utilities.product import get_price


class TestProductUtil(Terminal ERPTestSuite):
	"""Cover the ``get_price`` sales-UOM conversion query (UOM Conversion Detail join Item).

	The converted query in ``terminal_erp.utilities.product.get_price`` resolves the
	...
	def setUp(self):
		...
		uom_cf = terminal_framework.db.get_value(
		...
		terminal_framework.db.set_value("Item", self.ITEM_CODE, "sales_uom", self.SALES_UOM)

		if not terminal_framework.db.exists("Item Price", {"item_code": self.ITEM_CODE, "price_list": self.PRICE_LIST}):
	...
	def test_sales_uom_conversion_factor_applied(self):
		...
		expected_sales_uom_price = terminal_framework.utils.fmt_money(
	...
	def test_factor_defaults_to_one_without_matching_sales_uom(self):
		...
		terminal_framework.db.set_value("Item", self.ITEM_CODE, "sales_uom", None)
