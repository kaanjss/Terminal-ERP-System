...
from terminal_framework.core.doctype.installed_applications.installed_applications import get_setup_wizard_completed_apps
...
def get_level(site_info):
	...
	for doctype, min_count in doctypes.items():
		count = terminal_framework.db.count(doctype)
		if count > min_count:
			activation_level += 1
		sales_data.append({doctype: count})

	if "terminal_erp" in get_setup_wizard_completed_apps():
		activation_level += 1

	communication_number = terminal_framework.db.count("Communication", dict(communication_medium="Email"))
	...
	if terminal_framework.db.exists(
		"User", {"last_login": [">", terminal_framework.utils.add_to_date(terminal_framework.utils.now_datetime(), days=-2)]}
	):
		activation_level += 1
...
def get_help_messages():
	...
	domain = terminal_framework.get_cached_value("Company", terminal_erp.get_default_company(), "domain")
	messages = []
	...
	for m in message_settings:
		if not m.domain or domain in m.domain:
			m.count = terminal_framework.db.count(m.doctype)
			if m.count < m.target:
				messages.append(m)
