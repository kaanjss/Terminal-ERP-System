...
from terminal_framework.utils import now_datetime, random_string

from terminal_erp.tests.utils import Terminal ERPTestSuite
from terminal_erp.utilities.transaction_base import delete_events


class TestDeleteEvents(Terminal ERPTestSuite):
	def _make_event(self, reference_doctype, reference_docname):
		...
		participant = terminal_framework.new_doc("Event Participants")
		participant.name = terminal_framework.generate_hash(length=10)
	...
	def test_delete_events_removes_matching_and_keeps_others(self):
		...
		self.assertTrue(terminal_framework.db.exists("Event", event_match))
		self.assertTrue(terminal_framework.db.exists("Event", event_other))
		...
		self.assertFalse(terminal_framework.db.exists("Event", event_match))
		self.assertTrue(terminal_framework.db.exists("Event", event_other))
	...
	def test_delete_events_no_match_is_noop(self):
		...
		self.assertTrue(terminal_framework.db.exists("Event", event))
	...
	def test_delete_events_distinguishes_reference_doctype(self):
		...
		self.assertFalse(terminal_framework.db.exists("Event", event_customer))
		self.assertTrue(terminal_framework.db.exists("Event", event_supplier))
