...
import terminal_framework.share
from terminal_framework import _
from terminal_framework.utils import cint, flt, get_time, now_datetime
...
from terminal_erp.stock.get_item_details import NOT_APPLICABLE_TAX, get_item_details
from terminal_erp.stock.utils import get_incoming_rate


class UOMMustBeIntegerError(terminal_framework.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def on_change(self):
		...
		is_real_save = self.flags.in_insert or (self.doctype, self.name) in terminal_framework.flags.currently_saving
		if not is_real_save:
			return
	...
	def validate_posting_time(self):
		...
		if (terminal_framework.flags.in_import or self.flags.from_restore) and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				terminal_framework.throw(_("Invalid Posting Time"))
	...
	def copy_terms_and_conditions_attachments(self):
		...
		copy_attachments_to_transaction = terminal_framework.db.get_value(
			"Terms and Conditions", self.tc_name, "copy_attachments_to_transaction"
		)
		if not cint(copy_attachments_to_transaction):
			return

		source_attachments = terminal_framework.get_all(
			"File",
			filters={
		...
		existing_file_urls = {
			attachment.file_url
			for attachment in terminal_framework.get_all(
				"File",
				filters={
		...
		for source_attachment in source_attachments:
			...
			new_attachment = terminal_framework.get_doc("File", source_attachment.name).create_attachment_copy(
				attached_to_doctype=self.doctype,
				attached_to_name=self.name,
			)
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							terminal_framework.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
		...
		for reference_doctype, ref_dn_list in ref_doc.items():
			...
			for reference_name in ref_dn_list:
				...
				if not prevdoc_values:
					terminal_framework.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
	...
	def get_prev_doc_reference_details(self, reference_names, reference_doctype, fields):
		prev_doc_detail_map = {}
		details = terminal_framework.get_all(
			reference_doctype,
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		if self.doctype in buying_doctypes:
			action, role_allowed_to_override = terminal_framework.get_cached_value(
			...
			for d in self.get("items"):
				if d.get(ref_link_field):
					...
					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							if role_allowed_to_override not in terminal_framework.get_roles():
								stop_actions.append(
		...
		if stop_actions:
			terminal_framework.throw(stop_actions, as_list=True)
	...
	def validate_currency_for_receivable_payable_and_advance_account(self):
		if self.doctype in ["Customer", "Supplier"]:
			...
			for x in self.accounts:
				company_default_currency = terminal_framework.get_cached_value("Company", x.company, "default_currency")
				receivable_payable_account_currency = None
				advance_account_currency = None

				if x.account:
					receivable_payable_account_currency = terminal_framework.get_cached_value(
				...
				if receivable_payable_account_currency and (
					...
					terminal_framework.throw(
						_(
							...
							terminal_framework.bold(receivable_payable_account_currency),
							terminal_framework.bold(self.default_currency),
							terminal_framework.bold(company_default_currency),
						)
					)
	...
	def handle_internal_parties(self, item_obj: object, item_details: dict) -> None:
		fetch_valuation_rate_for_internal_transaction = cint(
			terminal_framework.get_single_value("Accounts Settings", "fetch_valuation_rate_for_internal_transaction")
		)
		if (
	...
	def add_taxes_from_item_template(self, item_obj: object, item_details: dict) -> None:
		if item_details.item_tax_rate and terminal_framework.get_single_value(
			"Accounts Settings", "add_taxes_from_item_tax_template"
		):
			item_tax_template = terminal_framework.json.loads(item_details.item_tax_rate)
			for tax_head, _rate in item_tax_template.items():
				...
				if not found:
					...
					child = terminal_framework.new_doc(child_doctype, parent_doc=self, parentfield="taxes")
	...
	def conversion_factor(self, item_obj: object, item_details: dict) -> None:
		if terminal_framework.get_meta(item_obj.doctype).has_field("stock_qty"):
			...
			if not terminal_framework.flags.dont_fetch_price_list_rate and terminal_framework.get_meta(self.doctype).has_field(
				"price_list_currency"
			):
	...
	def _apply_price_list(self, item_obj: object, reset_plc_conversion: bool) -> None:
		...
		from terminal_erp.stock.get_item_details import apply_price_list
...
def delete_events(ref_type, ref_name):
	...
	participant = terminal_framework.qb.DocType("Event Participants")
	events = (
	...
	if events:
		terminal_framework.delete_doc("Event", events, for_reload=True)
					...
					if abs(cint(qty) - flt(qty, precision)) > 0.0000001:
						terminal_framework.throw(
							_(
								...
								terminal_framework.bold(d.get(uom_field)),
							),
