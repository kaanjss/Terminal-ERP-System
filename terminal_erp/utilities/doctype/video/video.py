...
class Video(Document):
	...
	def set_youtube_statistics(self):
		api_key = terminal_framework.db.get_single_value("Video Settings", "api_key")
...
def is_tracking_enabled():
	return terminal_framework.db.get_single_value("Video Settings", "enable_youtube_tracking")
...
def update_youtube_data():
	...
	video_settings = terminal_framework.get_cached_doc("Video Settings")
...
def get_id_from_url(url: str):
	...
	if not isinstance(url, str):
		terminal_framework.throw(_("URL can only be a string"), title=_("Invalid URL"))
...
def batch_update_youtube_data():
	def get_youtube_statistics(video_ids):
		...
		try:
			...
			terminal_framework.log_error("Unable to update YouTube statistics")
	...
	def set_youtube_data(entries):
		for entry in entries:
			...
			terminal_framework.db.set_value("Video", video_id, stats)

	video_list = terminal_framework.get_all("Video", fields=["youtube_video_id"])
