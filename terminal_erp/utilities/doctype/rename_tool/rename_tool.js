...
terminal_framework.ui.form.on("Rename Tool", {
	refresh: function (frm) {
		...
		frm.page.set_primary_action(__("Rename"), function () {
			terminal_framework.call({
				method: "terminal_erp.utilities.doctype.rename_tool.rename_tool.upload",
				args: {
		...
		terminal_framework.db
			...
			.then((jobs) => {
				...
				for (const job of jobs) {
					if (job.job_name !== "terminal_framework.model.rename_doc.bulk_rename") {
						continue;
					}
