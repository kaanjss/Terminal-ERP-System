...
class IncomingCallSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.telephony.doctype.incoming_call_handling_schedule.incoming_call_handling_schedule import (
	...
	def validate_call_schedule_timeslot(self, schedule: list):
		...
		if errors:
			terminal_framework.throw("<br/>".join(errors))
		...
		for day in week_days:
			...
			for i in range(1, len(timeslots)):
				if self.check_timeslots_overlap(timeslots[i - 1], timeslots[i]):
					terminal_framework.throw(_("Please fix overlapping time slots for {0}.").format(day))
