...
from terminal_framework.contacts.doctype.contact.contact import get_contact_with_phone_number
from terminal_framework.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from terminal_framework.model.document import Document
from terminal_framework.query_builder import Case
from terminal_framework.query_builder.functions import Sum

from terminal_erp.crm.doctype.lead.lead import get_lead_with_phone_number
from terminal_erp.crm.doctype.utils import get_scheduled_employees_for_popup, strip_number
...
class CallLog(Document):
	...
	if TYPE_CHECKING:
		from terminal_framework.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def on_update(self):
		...
		if _is_call_missed(doc_before_save, self):
			terminal_framework.publish_realtime(f"call_{self.id}_missed", self)
			self.trigger_call_popup()

		if _is_call_ended(doc_before_save, self):
			terminal_framework.publish_realtime(f"call_{self.id}_ended", self)
	...
	def trigger_call_popup(self):
		...
		if terminal_framework.conf.developer_mode:
			self.add_comment(
				text=f"""
		...
		for email in emails:
			terminal_framework.publish_realtime("show_call_popup", self, user=email)
...
def add_call_summary_and_call_type(call_log: str, summary: str, call_type: str):
	doc = terminal_framework.get_doc("Call Log", call_log)
	doc.type_of_call = call_type
	doc.save()
	doc.add_comment("Comment", terminal_framework.bold(_("Call Summary")) + "<br><br>" + summary)
...
def get_employees_with_number(number):
	...
	employee_doc_name_and_emails = terminal_framework.cache().hget("employees_with_number", number)
	if employee_doc_name_and_emails:
		return employee_doc_name_and_emails

	employee_doc_name_and_emails = terminal_framework.get_all(
		"Employee",
		filters={"cell_number": ["like", f"%{number}%"], "user_id": ["!=", ""]},
		fields=["name", "user_id"],
	)

	terminal_framework.cache().hset("employees_with_number", number, employee_doc_name_and_emails)
...
def link_existing_conversations(doc, state):
	...
	if doc.doctype != "Contact":
		return
	terminal_framework.db.savepoint("link_call_logs")
	try:
		...
		for number in numbers:
			...
			if not number:
				continue
			cl = terminal_framework.qb.DocType("Call Log")
			dl = terminal_framework.qb.DocType("Dynamic Link")
			logs = (
				terminal_framework.qb.from_(cl)
				.left_join(dl)
			...
			if logs:
				for log in logs:
					call_log = terminal_framework.get_doc("Call Log", log)
		...
		terminal_framework.db.rollback(save_point="link_call_logs")
		terminal_framework.log_error(title=_("Error during caller information update"))
...
def get_linked_call_logs(doctype, docname):
	...
	logs = terminal_framework.get_all("Call Log", fields=["*"], filters={"name": ["in", logs]})
