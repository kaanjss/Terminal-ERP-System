...
from terminal_erp.telephony.doctype.call_log.call_log import link_existing_conversations
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestCallLog(Terminal ERPTestSuite):
	def setUp(self):
		...
		self.contact = terminal_framework.get_doc(
			{
				"doctype": "Contact",
	...
	def _make_call_log(self, **kwargs):
		doc = terminal_framework.get_doc({"doctype": "Call Log", "id": terminal_framework.generate_hash(length=10), **kwargs})
	...
	def test_already_linked_log_is_not_relinked(self):
		...
		modified_before = terminal_framework.db.get_value("Call Log", self.linked_log, "modified")
		...
		self.assertEqual(
			terminal_framework.db.get_value("Call Log", self.linked_log, "modified"),
			modified_before,
