...
terminal_erp.PointOfSale.Payment = class {
	...
	make_invoice_field_dialog() {
		...
		me.addl_dlg = new terminal_framework.ui.Dialog({
	...
	on_numpad_clicked($btn, from_numpad = true) {
		...
		const number_format_details = get_number_format_info(terminal_framework.sys_defaults.number_format);
		const precision = terminal_framework.sys_defaults.currency_precision || number_format_details.precision;
		this.numpad_value = "0";
		...
		valid_input && terminal_framework.utils.play_sound("numpad-touch");
	...
	bind_events() {
		...
		terminal_framework.ui.form.on("POS Invoice", "contact_mobile", (frm) => {
		...
		terminal_framework.ui.form.on("POS Invoice", "coupon_code", (frm) => {
			this.bind_coupon_code_event(frm);
		});

		terminal_framework.ui.form.on("Sales Invoice", "coupon_code", (frm) => {
			this.bind_coupon_code_event(frm);
		});
		...
		this.$component.on("click", ".submit-order-btn", () => {
			...
			if (
				...
				terminal_framework.show_alert({ message, indicator: "orange" });
		...
		terminal_framework.ui.form.on("POS Invoice", "loyalty_amount", (frm) => {
		...
		terminal_framework.ui.form.on("Sales Invoice", "loyalty_amount", (frm) => {
			this.bind_loyalty_amount_event(frm);
		});

		terminal_framework.ui.form.on("Sales Invoice Payment", "amount", (frm, cdt, cdn) => {
	...
	setup_listener_for_payments() {
		terminal_framework.realtime.on("process_phone_payment", (data) => {
			const doc = this.events.get_frm().doc;
			...
			terminal_framework.msgprint({ message: message, title: title });
		});
	}
	...
	attach_shortcuts() {
		...
		terminal_framework.ui.keys.on("ctrl+enter", () => {
	...
	toggle_remarks_control() {
		if (this.$remarks.find(".terminal_framework-control").length) {
			this.$remarks.html("+ Add Remark");
		} else {
			this.$remarks.html("");
			this[`remark_control`] = terminal_framework.ui.form.make_control({
				df: {
					label: __("Remark"),
					fieldtype: "Data",
					onchange: function () {},
	...
	render_payment_mode_dom() {
				...
				.map((p, i) => {
					...
					<div class="payment-mode-wrapper">
						<div class="mode-of-payment" data-mode="${mode}" data-payment-type="${payment_type}">
							${terminal_framework.utils.escape_html(p.mode_of_payment)}
		...
		payments.forEach((p) => {
			const mode = this.sanitize_mode_of_payment(p.mode_of_payment);
			const me = this;
			this[`${mode}_control`] = terminal_framework.ui.form.make_control({
				df: {
					label: p.mode_of_payment,
					fieldtype: "Currency",
					placeholder: __("Enter {0} amount.", [__(p.mode_of_payment)]),
					onchange: function () {
						const current_value = terminal_framework.model.get_value(p.doctype, p.name, "amount");
						if (current_value != this.value) {
							terminal_framework.model
		...
		this.$payment_modes.append(
			`<div class="payment-mode-wrapper">
				<div class="mode-of-payment loyalty-card" data-mode="loyalty-amount" data-payment-type="loyalty-amount">
					...
					<div class="loyalty-amount-name">${terminal_framework.utils.escape_html(loyalty_program)}</div>
					<div class="loyalty-amount mode-of-payment-control"></div>
				</div>
			</div>`
		);

		this["loyalty-amount_control"] = terminal_framework.ui.form.make_control({
			df: {
