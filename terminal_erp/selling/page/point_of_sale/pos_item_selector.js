...
terminal_erp.PointOfSale.ItemSelector = class {
	...
	async get_parent_item_group() {
		const r = await terminal_framework.call({
			method: "terminal_erp.selling.page.point_of_sale.point_of_sale.get_parent_item_group",
			args: {
	...
	async load_items_data() {
		...
		if (!this.price_list) {
			const res = await terminal_framework.db.get_value("POS Profile", this.pos_profile, "selling_price_list");
	...
	get_items({ start = 0, page_length = 40, search_term = "" }) {
		...
		return terminal_framework.call({
			method: "terminal_erp.selling.page.point_of_sale.point_of_sale.get_items",
			freeze: true,
	...
	get_item_html(item) {
		...
		function sanitize_item_data(item) {
			return Object.fromEntries(
				Object.entries(item).map(([key, value]) => [
					key,
					typeof value === "string" ? terminal_framework.utils.escape_html(value) : value,
				])
			);
		}
		...
		function get_item_image_html() {
			...
			if (item_image) {
				...
				return `<div class="item-qty-pill">
						...
						<div class="item-display abbr">${terminal_framework.get_abbr(item_name)}</div>`;
			}
		}
		...
		return `<div class="item-wrapper"
				...
				<div class="item-detail">
					<div class="item-name">
						${!me.hide_images ? terminal_framework.ellipsis(item_name, 18) : item_name}
					</div>
					${
						!me.hide_images
							? `<div class="item-rate">
								${terminal_framework.utils.escape_html(format_currency(price_list_rate, item.currency, precision)) || 0} / ${uom}
							</div>`
							: `
							<div class="item-price">${
								terminal_framework.utils.escape_html(
	...
	make_search_bar() {
		...
		this.search_field = terminal_framework.ui.form.make_control({
			df: {
				label: __("Search"),
				fieldtype: "Data",
		...
		this.item_group_field = terminal_framework.ui.form.make_control({
			df: {
				label: __("Item Group"),
				fieldtype: "Link",
				options: "Item Group",
				...
				get_query: function () {
					return {
						query: "terminal_erp.selling.page.point_of_sale.point_of_sale.item_group_query",
						filters: {
	...
	set_item_selector_filter_label(value) {
		...
		$filter_label.html(value ? terminal_framework.utils.escape_html(__(value)) : __("All Items"));
	}
		...
		this.search_field.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Clear")}">
					${terminal_framework.utils.icon("close", "sm")}
				</a>
			</span>`
		);
	...
	attach_shortcuts() {
		...
		terminal_framework.ui.keys.on("enter", () => {
