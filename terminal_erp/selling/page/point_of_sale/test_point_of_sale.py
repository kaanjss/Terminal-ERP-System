...
class TestPointOfSaleGetItems(Terminal ERPTestSuite):
	"""Covers the raw-SQL -> terminal_framework.qb conversion of point_of_sale.get_items."""
	...
	def test_disabled_item_is_excluded(self):
		...
		terminal_framework.db.set_value("Item", self.item_code, "disabled", 1)
		item_codes = self._get_item_codes(self.item_code)
		self.assertNotIn(self.item_code, item_codes)
	...
	def test_pos_profile_item_group_restriction_returns_items(self):
		...
		# item.item_group.isin(...) / terminal_framework.get_all, which escapes them once. The names must be RAW;
		...
		from terminal_erp.accounts.doctype.pos_profile.pos_profile import get_item_groups
	...
	def test_non_sales_item_is_excluded(self):
		...
		terminal_framework.db.set_value("Item", self.item_code, "is_sales_item", 0)
		item_codes = self._get_item_codes(self.item_code)
		self.assertNotIn(self.item_code, item_codes)
	...
	def test_hide_unavailable_items_filters_on_bin_actual_qty(self):
		...
		warehouse = terminal_framework.db.get_value("POS Profile", self.pos_profile, "warehouse")
		terminal_framework.db.set_value("POS Profile", self.pos_profile, "hide_unavailable_items", 1)
		...
		self.assertGreater(
			terminal_framework.db.get_value("Bin", {"item_code": in_stock_item, "warehouse": warehouse}, "actual_qty")
			or 0,
			0,
		)
		self.assertFalse(terminal_framework.db.exists("Bin", {"item_code": out_of_stock_item}))
