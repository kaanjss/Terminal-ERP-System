terminal_erp.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new terminal_erp.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		terminal_framework.ui.form.on("POS Invoice", "paid_amount", (frm) => {
			// called when discount is applied
			this.update_totals_section(frm);
		});

		terminal_framework.ui.form.on("Sales Invoice", "paid_amount", (frm) => {
			// called when discount is applied
			this.update_totals_section(frm);
		});
	}
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${terminal_framework.scrub(String(btn))[0]}`;
				...
				const fieldname = this.number_pad.fieldnames[btn]
					...
					: typeof btn === "string"
					? terminal_framework.scrub(btn)
					: btn;

				let shortcut_label = shortcut_key.split("+").map(terminal_framework.utils.to_title_case).join("+");
				shortcut_label = terminal_framework.utils.is_mac() ? shortcut_label.replace("Ctrl", "⌘") : shortcut_label;
				this.$numpad_section
					.find(`.numpad-btn[data-button-value="${fieldname}"]`)
					.attr("title", shortcut_label);

				terminal_framework.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = terminal_framework.utils.is_mac() ? "⌘" : "Ctrl";
		this.$component.find(".checkout-btn").attr("title", `${ctrl_label}+Enter`);
		terminal_framework.ui.keys.add_shortcut({
			shortcut: "ctrl+enter",
		...
		terminal_framework.ui.keys.on("ctrl+e", () => {
		...
		terminal_framework.ui.keys.on("escape", () => {
			const item_cart_visible = this.$component.is(":visible");
	...
	make_customer_selector() {
		...
		this.customer_field = terminal_framework.ui.form.make_control({
			df: {
				label: __("Customer"),
				fieldtype: "Link",
				options: "Customer",
				...
				onchange: function () {
					if (this.value) {
						...
						terminal_framework.model.set_value(frm.doc.doctype, frm.doc.name, "customer", this.value);
					...
					.then(({ message }) => {
						...
						if (loyalty_program) {
							terminal_framework.call({
								method: "terminal_erp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = terminal_framework.ui.form.make_control({
			df: {
				label: __("Discount"),
				fieldtype: "Data",
	...
	update_customer_section() {
				...
				`<div class="customer-details">
					<div class="customer-display">
						...
						<div class="customer-name-desc">
							<div class="customer-name">${terminal_framework.utils.escape_html(customer_name)}</div>
							${get_customer_description()}
						</div>
						<div class="reset-customer-btn" data-customer="${terminal_framework.utils.escape_html(customer)}">
		...
		function get_customer_description() {
			if (!email_id && !mobile_no) {
				...
				return `<div class="customer-desc">${terminal_framework.utils.escape_html(email_id)}</div>`;
			} else if (mobile_no && !email_id) {
				return `<div class="customer-desc">${terminal_framework.utils.escape_html(mobile_no)}</div>`;
			} else {
				return `<div class="customer-desc">${terminal_framework.utils.escape_html(
					email_id
				)} - ${terminal_framework.utils.escape_html(mobile_no)}</div>`;
			}
		}
	}
	...
	get_customer_image() {
		...
		if (image) {
			return `<div class="customer-image"><img src="${terminal_framework.utils.escape_html(
				image
			)}" alt="${terminal_framework.utils.escape_html(image)}"></div>`;
		} else {
			return `<div class="customer-image customer-abbr">${terminal_framework.utils.escape_html(
				terminal_framework.get_abbr(customer)
			)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(terminal_framework.sys_defaults.disable_rounded_total)
			? frm.doc.grand_total
		...
		if (taxes && taxes.length) {
			...
			const taxes_html = taxes
				.map((t) => {
					...
					<div class="tax-label">${terminal_framework.utils.escape_html(t.description)}</div>
	...
	render_cart_item(item_data, $item_to_update) {
		...
		if (!$item_to_update.length) {
			this.$cart_items_wrapper.append(
				`<div class="cart-item-wrapper" data-row-name="${terminal_framework.utils.escape_html(
					item_data.name
				)}"></div>
		...
		$item_to_update.html(
			...
			<div class="item-name-desc">
				<div class="item-name">
					${terminal_framework.utils.escape_html(item_data.item_name)}
				</div>
			...
			if (item_data.rate && item_data.amount && item_data.rate !== item_data.amount) {
				return `
					<div class="item-qty-rate">
						<div class="item-qty"><span>${item_data.qty || 0} ${terminal_framework.utils.escape_html(item_data.uom)}</span></div>
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = terminal_framework.ellipsis(item_data.description, 45);
				return `<div class="item-desc">${terminal_framework.utils.escape_html(item_data.description)}</div>`;
			}
			return ``;
		}
				...
				return `
					<div class="item-image">
						<img
							...
							src="${terminal_framework.utils.escape_html(image)}" alt="${terminal_framework.utils.escape_html(terminal_framework.get_abbr(item_name))}">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${terminal_framework.utils.escape_html(
					terminal_framework.get_abbr(item_name)
				)}</div>`;
			}
		}
	}
	...
	handle_broken_image($img) {
		const item_abbr = terminal_framework.utils.escape_html($($img).attr("alt"));
		...
		if (show) {
			...
			this.$customer_section.find(".customer-details").html(
				...
				<div class="customer-display">
					...
					<div class="customer-name-desc">
						<div class="customer-name">${terminal_framework.utils.escape_html(customer_name)}</div>
						<div class="customer-desc">${terminal_framework.utils.escape_html(customer)}</div>
					</div>
				</div>
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach((df) => {
			this[`customer_${df.fieldname}_field`] = terminal_framework.ui.form.make_control({
				df: df,
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != "loyalty_points") {
				terminal_framework.call({
					method: "terminal_erp.selling.page.point_of_sale.point_of_sale.set_customer_info",
					args: {
					...
					callback: (r) => {
						if (!r.exc) {
							...
							terminal_framework.utils.play_sound("submit");
						}
					},
				});
			}
	...
	fetch_customer_transactions() {
		terminal_framework
			.call({
				method: "terminal_erp.selling.page.point_of_sale.point_of_sale.get_customer_recent_transactions",
			...
			.then((res) => {
				...
				res.forEach((invoice) => {
					const posting_datetime = terminal_framework.datetime.str_to_user(
					...
					transaction_container.append(
						`<div class="invoice-wrapper" data-invoice-name="${terminal_framework.utils.escape_html(
							invoice.name
						)}">
						<div class="invoice-name-date">
							<div class="invoice-name">${terminal_framework.utils.escape_html(invoice.name)}</div>
							<div class="invoice-date">${posting_datetime}</div>
						</div>
						<div class="invoice-total-status">
							<div class="invoice-total">
								${format_currency(invoice.grand_total, invoice.currency, terminal_framework.sys_defaults.currency_precision) || 0}
							</div>
