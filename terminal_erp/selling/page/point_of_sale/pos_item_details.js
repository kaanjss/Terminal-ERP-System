terminal_erp.PointOfSale.ItemDetails = class {
	...
	async toggle_item_details_section(item) {
		...
		if (item && current_item_changed) {
			...
			this.item_meta = terminal_framework.get_meta(this.doctype);
			this.name = item.name;
			this.item_row = item;
	...
	validate_serial_batch_item() {
		...
		if ((serialized && no_bundle_selected) || (batched && no_bundle_selected)) {
			...
			terminal_framework.utils.play_sound("cancel");
	...
	render_dom(item) {
		...
		this.$item_name.html(terminal_framework.utils.escape_html(item_name));
		...
		if (!this.hide_images && image) {
			this.$item_image.html(
				`<img
					...
					class="h-full" src="${terminal_framework.utils.escape_html(image)}"
					alt="${terminal_framework.utils.escape_html(terminal_framework.get_abbr(item_name))}"
					style="object-fit: cover;">`
			);
		} else {
			this.$item_image.html(
				`<div class="item-abbr">${terminal_framework.utils.escape_html(terminal_framework.get_abbr(item_name))}</div>`
			);
		}
	}
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			this[`${fieldname}_control`] = terminal_framework.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function () {
	...
	bind_custom_control_change_event() {
			...
			this.rate_control.df.onchange = function () {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, "rate", this.value).then(() => {
						const item_row = terminal_framework.get_doc(me.doctype, me.name);
		...
		terminal_framework.model.on(`${frm_doctype} Item`, "*", (fieldname, value, item_row) => {
	...
	async auto_update_batch_no() {
		if (this.serial_no_control && this.batch_no_control) {
			...
			const serials_with_batch_no = await terminal_framework.db.get_list("Serial No", {
	...
	bind_auto_serial_fetch_event() {
		this.$form_container.on("click", ".auto-fetch-btn", () => {
			...
			let numbers = terminal_framework.call({
				method: "terminal_erp.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
				args: {
			...
			numbers.then((data) => {
				...
				if (!records_length) {
					...
					terminal_framework.msgprint(__("Fetched only {0} available serial numbers.", [records_length]));
