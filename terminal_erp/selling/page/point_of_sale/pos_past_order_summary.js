terminal_erp.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new terminal_framework.ui.Dialog({
		...
		const print_dialog = new terminal_framework.ui.Dialog({
	...
	get_upper_section_html(doc) {
		const { status } = doc;
		let indicator_color = "";
		const is_customer_naming_by_customer_name = terminal_framework.sys_defaults.cust_master_name !== "Customer Name";
		...
		return `<div class="left-section">
					<div class="customer-section">
						<div class="customer-name">${terminal_framework.utils.escape_html(doc.customer_name)}</div>
						${
							is_customer_naming_by_customer_name
								? `<div class="customer-code">${terminal_framework.utils.escape_html(doc.customer)}</div>`
								: ""
						}
						<div class="customer-email">${terminal_framework.utils.escape_html(this.customer_email)}</div>
					</div>
					<div class="cashier">${__("Sold by")}: ${terminal_framework.utils.escape_html(doc.owner)}</div>
				</div>
				<div class="right-section">
					<div class="paid-amount">${format_currency(doc.paid_amount, doc.currency)}</div>
					<div class="invoice-name">${terminal_framework.utils.escape_html(doc.name)}</div>
	...
	async get_item_html(doc, item_data) {
		...
		return `<div class="item-row-wrapper">
				<div class="item-row-data">
					<div class="item-name">${terminal_framework.utils.escape_html(item_data.item_name)}</div>
					<div class="item-qty">${item_data.qty || 0} ${terminal_framework.utils.escape_html(item_data.uom)}</div>
		...
		async function get_returned_qty() {
			const r = await terminal_framework.call({
				method: "terminal_erp.controllers.sales_and_purchase_return.get_invoice_item_returned_qty",
				args: {
					doctype: doc.doctype,
					invoice: doc.name,
	...
	get_payment_html(doc, payment) {
		return `<div class="summary-row-wrapper payments">
					<div>${terminal_framework.utils.escape_html(__(payment.mode_of_payment))}</div>
	...
	print_receipt() {
		...
		terminal_framework.utils.print(
			this.doc.doctype,
			this.doc.name,
			frm.pos_print_format,
			this.doc.letter_head,
			this.doc.language || terminal_framework.boot.lang
		);
	}
	...
	send_email() {
		...
		terminal_framework.call({
			method: "terminal_framework.core.doctype.communication.email.make",
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: terminal_framework.user.full_name(),
				_lang: doc.language,
			},
			callback: (r) => {
				if (!r.exc) {
					terminal_framework.utils.play_sound("email");
					if (r.message["emails_not_sent_to"]) {
						terminal_framework.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								terminal_framework.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					} else {
					...
					terminal_framework.msgprint(__("There were errors while sending email. Please try again."));
				}
			},
		});
	}
	...
	attach_document_info(doc) {
		terminal_framework.db.get_value("Customer", this.doc.customer, "email_id").then(({ message }) => {
	...
	async is_invoice_returnable(doctype, invoice) {
		const r = await terminal_framework.call({
			method: "terminal_erp.controllers.sales_and_purchase_return.is_invoice_returnable",
			args: {
				doctype: doctype,
				invoice: invoice,
			},
