...
from terminal_framework.query_builder import Criterion, DocType, Order
from terminal_framework.utils import cint, get_datetime
from terminal_framework.utils.nestedset import get_root_of

from terminal_erp.accounts.doctype.pos_invoice.pos_invoice import get_item_group, get_stock_availability
from terminal_erp.accounts.doctype.pos_profile.pos_profile import get_child_nodes, get_item_groups
from terminal_erp.stock.get_item_details import get_conversion_factor
from terminal_erp.stock.utils import scan_barcode
...
def search_by_term(search_term, warehouse, price_list):
	...
	price = terminal_framework.get_list(
		doctype="Item Price",
...
def filter_result_items(result, pos_profile):
	if result and result.get("items"):
		pos_profile_doc = terminal_framework.get_cached_doc("POS Profile", pos_profile)
...
def get_parent_item_group(pos_profile: str):
	...
	if not item_groups:
		item_groups = terminal_framework.get_all("Item Group", {"lft": 1, "is_group": 1}, pluck="name")
...
def get_items(
	...
	warehouse, hide_unavailable_items = terminal_framework.db.get_value(
	...
	if not terminal_framework.db.exists("Item Group", item_group):
	...
	current_date = terminal_framework.utils.today()
...
def add_search_fields_condition(search_term, item=None):
	...
	pattern = f"%{search_term}%"
	conditions = []
	search_fields = terminal_framework.get_all("POS Search Fields", fields=["fieldname"])
...
def check_opening_entry(user: str):
	open_vouchers = terminal_framework.db.get_all(
		"POS Opening Entry",
...
def create_opening_voucher(pos_profile: str, company: str, balance_details: str | list):
	balance_details = terminal_framework.parse_json(balance_details)

	new_pos_opening = terminal_framework.get_doc(
		{
			"doctype": "POS Opening Entry",
			"period_start_date": terminal_framework.utils.get_datetime(),
			"posting_date": terminal_framework.utils.getdate(),
...
def get_past_order_list(search_term: str, status: str, limit: int = 20):
	...
	if search_term and status:
		pos_invoices_by_customer = terminal_framework.db.get_list(
			"POS Invoice",
		...
		pos_invoices_by_name = terminal_framework.db.get_list(
			"POS Invoice",
		...
		sales_invoices_by_customer = terminal_framework.db.get_list(
			"Sales Invoice",
		...
		sales_invoices_by_name = terminal_framework.db.get_list(
			"Sales Invoice",
		...
		pos_invoice_list = terminal_framework.db.get_list(
			"POS Invoice",
			filters=get_invoice_filters("POS Invoice", status),
			fields=fields,
			page_length=limit,
		...
		sales_invoice_list = terminal_framework.db.get_list(
			"Sales Invoice",
			filters=get_invoice_filters("Sales Invoice", status),
			fields=fields,
			page_length=limit,
...
def set_customer_info(fieldname: str, customer: str, value: str = ""):
	customer_doc = terminal_framework.get_doc("Customer", customer)
	...
	if fieldname == "loyalty_program":
		...
		if not contact:
			...
			query = (
				terminal_framework.qb.from_(DynamicLink)
				.join(Contact)
		...
		if not contact:
			new_contact = terminal_framework.new_doc("Contact")
...
def get_pos_profile_data(pos_profile: str):
	pos_profile = terminal_framework.get_doc("POS Profile", pos_profile)
...
def get_customer_recent_transactions(customer: str):
	sales_invoices = terminal_framework.db.get_list(
		"Sales Invoice",
		filters={
			"customer": customer,
			"docstatus": 1,
	...
	pos_invoices = terminal_framework.db.get_list(
		"POS Invoice",
