...
terminal_framework.pages["sales-funnel"].on_page_load = function (wrapper) {
	...
	wrapper.sales_funnel = new terminal_erp.SalesFunnel(wrapper);

	terminal_framework.breadcrumbs.add("Selling");
};

terminal_erp.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		(this.company_field = wrapper.page.add_field({
			...
			change: function () {
				me.company = this.value || terminal_framework.defaults.get_user_default("company");
				me.get_data();
			},
		})),
			(this.elements = {
		...
		this.company = terminal_framework.defaults.get_user_default("company");
		this.options = {
			from_date: terminal_framework.datetime.add_months(terminal_framework.datetime.get_today(), -1),
			to_date: terminal_framework.datetime.get_today(),
			chart: "sales_funnel",
		};
		...
		$.each(this.options, function (k, v) {
			if (["from_date", "to_date"].includes(k)) {
				me.elements[k].val(terminal_framework.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function () {
				if (["from_date", "to_date"].includes(k)) {
					me.options[k] =
						terminal_framework.datetime.user_to_str($(this).val()) != "Invalid date"
							? terminal_framework.datetime.user_to_str($(this).val())
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			terminal_framework.throw(__("Please select a company."));
		}

		const method_map = {
			sales_funnel: "terminal_erp.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			opp_by_utm_source: "terminal_erp.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_source",
			opp_by_utm_campaign: "terminal_erp.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_campaign",
			opp_by_utm_medium: "terminal_erp.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_medium",
			sales_pipeline: "terminal_erp.selling.page.sales_funnel.sales_funnel.get_pipeline_data",
		};
	...
	render_chart(title) {
		let me = this;
		let currency = terminal_framework.defaults.get_default("currency");

		let chart_data = me.options.data ? me.options.data : null;

		const parent = me.elements.funnel_wrapper[0];
		this.chart = new terminal_framework.Chart(parent, {
			title: title,
			height: 400,
			data: chart_data,
			type: "bar",
