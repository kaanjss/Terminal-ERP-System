...
from terminal_framework.query_builder import DocType, Field, Order
from terminal_framework.query_builder.custom import ConstantColumn
from terminal_framework.query_builder.utils import QueryBuilder
...
class SalesPartnerSummaryReport:
	...
	def validate_filters(self):
		if not self.filters.get("doctype"):
			terminal_framework.throw(_("Please select the document type first."))

		if self.filters.get("doctype") not in SALES_TRANSACTION_DOCTYPES:
			terminal_framework.throw(_("DocType can be one of {0}").format(comma_or(SALES_TRANSACTION_DOCTYPES)))
	...
	def _build_report_base_query(self):
		...
		self.query = (
			terminal_framework.qb.from_(self.dt)
			.select(
				self.dt.name,
				self.dt.customer,
				self.dt.territory,
