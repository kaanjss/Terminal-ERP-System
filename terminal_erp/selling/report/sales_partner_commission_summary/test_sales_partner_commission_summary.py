...
from terminal_framework.desk.query_report import run
from terminal_framework.utils.data import comma_or

from terminal_erp.selling.report.sales_partner_commission_summary.sales_partner_commission_summary import (
...
class SalesPartnerSummaryReportTestMixin(Terminal ERPTestSuite):
	...
	def create_transactions(self, doctype):
		from terminal_erp.accounts.doctype.pos_invoice.test_pos_invoice import (
			POSInvoiceTestMixin,
			create_pos_invoice,
		)
	...
	def transaction_doc_with_revoked_commission(self):
		try:
			terminal_framework.db.set_value("Item", "_Test Item", "grant_commission", 0)
			args = {
				"item_code": "_Test Item",
				"rate": 80,
				"qty": 10,
