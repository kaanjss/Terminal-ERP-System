...
from terminal_framework.query_builder.functions import Coalesce, Count, NullIf, Round, Sum
from terminal_framework.utils.data import get_timespan_date_range
...
def get_data(company: str, from_date: str, to_date: str, group_by: Literal["Lost Reason", "Competitor"]):
	...
	if group_by == "Lost Reason":
		...
		dimension = terminal_framework.qb.DocType("Quotation Lost Reason Detail")
	elif group_by == "Competitor":
		fieldname = "competitor"
		dimension = terminal_framework.qb.DocType("Competitor Detail")
	else:
		terminal_framework.throw(_("Invalid Group By"))
	...
	from_lost_quotations = terminal_framework.qb.from_(q).where(lost_quotation_condition)
