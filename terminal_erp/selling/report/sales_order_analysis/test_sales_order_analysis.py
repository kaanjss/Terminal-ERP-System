...
from terminal_erp.selling.report.sales_order_analysis.sales_order_analysis import execute
from terminal_erp.stock.doctype.item.test_item import create_item
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSalesOrderAnalysis(Terminal ERPTestSuite):
	...
	def test_01_so_to_deliver_and_bill(self):
		...
		expected_value = {
			"status": "To Deliver and Bill",
			"sales_order": so.name,
			"delay_days": terminal_framework.utils.date_diff(terminal_framework.utils.datetime.date.today(), so.delivery_date),
			"qty": 10,
			"delivered_qty": 0,
			"pending_qty": 10,
			"qty_to_bill": 10,
