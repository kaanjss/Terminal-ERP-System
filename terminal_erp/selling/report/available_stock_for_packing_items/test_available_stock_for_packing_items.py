...
from terminal_erp.selling.report.available_stock_for_packing_items.available_stock_for_packing_items import (
	execute,
)
...
class TestAvailableStockForPackingItems(Terminal ERPTestSuite):
	...
	def set_bin_projected_qty(self, item_code, warehouse, projected_qty):
		...
		"""
		name = terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse})
		if not name:
			bin_doc = terminal_framework.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
			bin_doc.flags.ignore_permissions = True
			bin_doc.insert()
			name = bin_doc.name
		terminal_framework.db.set_value("Bin", name, "projected_qty", projected_qty)
		return name

	def make_active_bundle(self, parent, components):
		"""components: list of (item_code, qty_per_bundle). Submitted => is_active, docstatus 1."""
		bundle = terminal_framework.get_doc({"doctype": "Product Bundle", "new_item_code": parent})
	...
	def make_secondary_warehouse(self):
		...
		if terminal_framework.db.exists("Warehouse", full_name):
			return full_name
		wh = terminal_framework.get_doc(
			{
				"doctype": "Warehouse",
				"warehouse_name": name,
