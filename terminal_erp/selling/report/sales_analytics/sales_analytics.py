...
from terminal_framework.utils import add_days, add_to_date, flt, getdate
...
class Analytics:
	def __init__(self, filters=None):
		...
		if self.filters.doc_type == "Payment Entry" and self.filters.value_quantity == "Quantity":
			terminal_framework.throw(_("Only Value available for Payment Entry"))
		self.date_field = (
			"transaction_date"
	...
	def update_company_list_for_parent_company(self):
		...
		if (
			...
			and terminal_framework.db.get_value("Company", selected_company, "is_group")
		):
			lft, rgt = terminal_framework.db.get_value("Company", selected_company, ["lft", "rgt"])
			child_companies = terminal_framework.db.get_list(
	...
	def _get_permitted_parent_names(self):
		return terminal_framework.qb.get_query(
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		self.entries = terminal_framework.qb.get_query(
			table=self.filters.doc_type,
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = (
			terminal_framework.qb.from_(doctype_item)
			.join(doctype)
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, terminal_framework._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = terminal_framework._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, terminal_framework._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		if self.filters.tree_type not in parent_field_map:
			terminal_framework.throw(_("Invalid Tree Type {0}").format(self.filters.tree_type))

		parent = parent_field_map[self.filters.tree_type]

		self.depth_map = terminal_framework._dict()

		self.group_entries = terminal_framework.get_all(
	...
	def get_teams(self):
		...
		if not terminal_framework.db.exists("DocType", self.filters.doc_type):
			terminal_framework.throw(_("Invalid Document Type {0}").format(self.filters.doc_type))

		# terminal_framework drops ORDER BY for distinct queries on postgres (db_query), so a SQL order_by="order_type"
		...
		self.group_entries = [terminal_framework._dict(name="Order Types", lft=0, rgt=2, parent="")]
		self.group_entries += [
			terminal_framework._dict(name=order_type, lft=1, rgt=1, parent="Order Types") for order_type in order_types
		]
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = terminal_framework._dict(
			terminal_framework.get_all("Supplier", fields=["name", "supplier_group"], as_list=True)
		)
