...
from terminal_erp.selling.report.sales_analytics.sales_analytics import execute
from terminal_erp.tests.utils import Terminal ERPTestSuite

# Bootstrap masters reused as-is (see terminal_erp/tests/utils.py):
...
class TestSalesAnalytics(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_order_type_leaf_rows_in_sorted_order(self):
		"""get_teams fetches distinct order_types; terminal_framework drops the SQL ORDER BY for distinct queries on
	...
	def test_supplier_group_tree_maps_supplier_to_group(self):
		...
		built by terminal_framework.get_all('Supplier', ['name', 'supplier_group'], as_list=True).
