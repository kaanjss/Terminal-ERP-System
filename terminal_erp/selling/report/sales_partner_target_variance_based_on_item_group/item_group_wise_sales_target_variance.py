...
from terminal_erp.accounts.doctype.monthly_distribution.monthly_distribution import (
...
def get_data_column(filters, partner_doctype):
	...
	for key, value in rows.items():
		value.update({terminal_framework.scrub(partner_doctype): key[0], "item_group": key[1]})
...
def get_data(filters, period_list, partner_doctype):
	sales_field = terminal_framework.scrub(partner_doctype)
...
def get_columns(filters, period_list, partner_doctype):
	...
	columns = [
		{
			"fieldname": terminal_framework.scrub(partner_doctype),
...
def get_item_group_parent_child_map():
	...
	item_groups = terminal_framework.get_all(
...
def get_actual_data(filters, sales_users_or_territory_data, date_field, sales_field):
	...
	parent_doc = terminal_framework.qb.DocType(filters.get("doctype"))
	child_doc = terminal_framework.qb.DocType(filters.get("doctype") + " Item")

	query = terminal_framework.qb.from_(parent_doc).inner_join(child_doc).on(child_doc.parent == parent_doc.name)
