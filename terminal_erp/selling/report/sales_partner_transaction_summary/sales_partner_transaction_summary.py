...
class SalesPartnerTransactionSummaryReport(SalesPartnerSummaryReport):
	...
	def extend_report_query(self):
		self.dt_item = terminal_framework.qb.DocType(f"{self.filters['doctype']} Item")
	...
	def apply_filters(self):
		...
		if self.filters.get("item_group"):
			lft, rgt = terminal_framework.get_cached_value("Item Group", self.filters.get("item_group"), ["lft", "rgt"])
			if item_groups := terminal_framework.get_all(
