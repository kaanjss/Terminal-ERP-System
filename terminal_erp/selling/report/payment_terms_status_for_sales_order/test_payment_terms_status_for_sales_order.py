...
from terminal_framework.utils import add_days, add_months, flt, nowdate
...
from terminal_erp.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order import (
	execute,
)
...
class TestPaymentTermsStatusForSalesOrder(Terminal ERPTestSuite):
	def create_payment_terms_template(self):
		# create template for 50-50 payments
		template = None
		if terminal_framework.db.exists("Payment Terms Template", "_Test 50-50"):
			template = terminal_framework.get_doc("Payment Terms Template", "_Test 50-50")
		else:
			...
			doc = terminal_framework.get_doc(
				{
					...
					"date": date,
					"from_currency": "USD",
					"to_currency": terminal_framework.get_cached_value("Company", "_Test Company", "default_currency"),
					"exchange_rate": 70,
					"for_buying": True,
					"for_selling": True,
				}
	...
	def test_03_group_filters(self):
		...
		for idx, g in enumerate(group_filters, 0):
			# build filter
			filters = terminal_framework._dict({}).update(base_filters).update(g)
