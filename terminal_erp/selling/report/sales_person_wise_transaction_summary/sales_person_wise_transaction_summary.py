...
def validate_filters(filters):
	...
	if filters.get("doc_type") not in ALLOWED_DOCTYPES:
		terminal_framework.throw(_("{0}, {1} or {2} are the only allowed options.").format(*ALLOWED_DOCTYPES))
	...
	columns = [
		{
			...
			"fieldname": terminal_framework.scrub(filters["doc_type"]),
			"fieldtype": "Link",
			"width": 140,
		},
		{
...
def get_entries(filters):
	...
	dt = terminal_framework.qb.DocType(doc_type)
	dt_item = terminal_framework.qb.DocType(f"{doc_type} Item")
	st = terminal_framework.qb.DocType("Sales Team")
	...
	query = (
		terminal_framework.get_query(dt, filters=filters, ignore_permissions=False)
		.join(dt_item)
	...
	query = query.orderby(st.sales_person).orderby(dt.name, order=terminal_framework.qb.desc)
...
def get_conditions(filters, date_field):
	...
	if filters.get("sales_person"):
		lft, rgt = terminal_framework.get_value("Sales Person", filters.get("sales_person"), ["lft", "rgt"])
		conditions.append(
...
def get_item_details():
	items = terminal_framework.get_all("Item", fields=["name", "item_group", "brand"])
