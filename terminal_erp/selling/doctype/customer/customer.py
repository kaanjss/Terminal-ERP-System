...
from terminal_framework import _, msgprint, qb
from terminal_framework.contacts.address_and_contact import (
	delete_contact_and_address,
	load_address_and_contact,
)
from terminal_framework.model.document import Document
from terminal_framework.model.naming import set_name_by_naming_series, set_name_from_naming_options
from terminal_framework.model.utils.rename_doc import update_linked_doctypes
from terminal_framework.query_builder import CustomFunction, Field, functions
from terminal_framework.query_builder.functions import Cast, Coalesce, Max
from terminal_framework.utils import cint, cstr, flt, get_formatted_email, today
...
from terminal_erp.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from terminal_erp.accounts.doctype.party_account.party_account import PartyAccount
		from terminal_erp.selling.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from terminal_erp.selling.doctype.sales_team.sales_team import SalesTeam
		from terminal_erp.selling.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from terminal_erp.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def autoname(self):
		cust_master_name = terminal_framework.defaults.get_global_default("cust_master_name")
		if cust_master_name == "Customer Name":
			self.name = self.get_customer_name()
		elif cust_master_name == "Naming Series":
			set_name_by_naming_series(self)
		else:
			set_name_from_naming_options(terminal_framework.get_meta(self.doctype).autoname, self)

	def get_customer_name(self):
		self.customer_name = self.customer_name.strip()
		if terminal_framework.db.get_value("Customer", self.customer_name) and not terminal_framework.flags.in_import:
	...
	def validate(self):
		...
		if self.sales_team:
			if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
				terminal_framework.throw(_("Total contribution percentage should be equal to 100"))

	@terminal_framework.whitelist()
	def get_customer_group_details(self):
		doc = terminal_framework.get_doc("Customer Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		terminal_framework.flags.customer_group_changed = False

		if not self.get("__islocal"):
			if self.customer_group != terminal_framework.db.get_value("Customer", self.name, "customer_group"):
				terminal_framework.flags.customer_group_changed = True

	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = terminal_framework.db.get_value(
				"Bank Account", self.default_bank_account, "is_company_account"
			)
			if not is_company_account:
				terminal_framework.throw(
					_("{0} is not a company bank account").format(terminal_framework.bold(self.default_bank_account))
				)
	...
	def validate_internal_customer(self):
		...
		internal_customer = terminal_framework.db.get_value(
			"Customer",
			{
		...
		if internal_customer:
			terminal_framework.throw(
				_("Internal Customer for company {0} already exists").format(
					terminal_framework.bold(self.represents_company)
				)
			)
	...
	def update_customer_groups(self):
		...
		if terminal_framework.flags.customer_group_changed:
	...
	def create_primary_contact(self):
		if not self.customer_primary_contact and not self.lead_name:
			...
			terminal_framework.set_value("Contact", self.customer_primary_contact, "is_primary_contact", 1)  # ensure
	...
	def create_primary_address(self):
		...
		if self.flags.is_new_doc and self.get("address_line1"):
			...
			terminal_framework.set_value("Address", self.customer_primary_address, "is_primary_address", 1)  # ensure

	def update_lead_status(self):
		"""If Customer created from Lead, update lead status to "Converted"
		update Customer link in Quotation, Opportunity"""
		if self.lead_name:
			terminal_framework.db.set_value("Lead", self.lead_name, "status", "Converted")
	...
	def link_address_and_contact(self):
		...
		for doctype, docname in linked_documents.items():
			...
			linked_contacts_and_addresses = terminal_framework.get_all(
				"Dynamic Link",
				filters=[
			...
			for row in linked_contacts_and_addresses:
				linked_doc = terminal_framework.get_doc(row.doctype, row.name)
	...
	def copy_communication(self):
		if not self.lead_name or not terminal_framework.db.get_single_value(
			"CRM Settings", "carry_forward_communication_and_comments"
		):
			return

		from terminal_erp.crm.utils import copy_comments, link_communications

		copy_comments("Lead", self.lead_name, self)
		link_communications("Lead", self.lead_name, self)

	def validate_name_with_customer_group(self):
		if terminal_framework.db.exists("Customer Group", self.name):
			terminal_framework.throw(
				_(
	...
	def validate_customer_group(self):
		...
		is_group = terminal_framework.db.get_value("Customer Group", self.customer_group, "is_group")
		if is_group:
			terminal_framework.throw(
	...
	def on_trash(self):
		...
		if self.lead_name:
			terminal_framework.db.set_value("Lead", self.lead_name, "status", "Interested")
	...
	def after_rename(self, olddn, newdn, merge=False):
		if terminal_framework.defaults.get_global_default("cust_master_name") == "Customer Name":
		...
		if len(loyalty_program) == 1:
			...
			terminal_framework.msgprint(
				_("Multiple Loyalty Programs found for Customer {0}. Please select manually.").format(
					terminal_framework.bold(self.customer_name)
				)
			)
	...
	def get_notification_email(self):
		...
		if self.account_manager:
			return terminal_framework.db.get_value("User", self.account_manager, "email")
...
def get_loyalty_programs(doc: Document):
	...
	lp_details = []
	loyalty_programs = terminal_framework.get_all(
		"Loyalty Program",
...
def get_nested_links(link_doctype, link_name, ignore_permissions=False):
	from terminal_framework.desk.treeview import _get_children
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller_role = terminal_framework.get_single_value("Accounts Settings", "credit_controller")
		if not credit_controller_role or credit_controller_role not in terminal_framework.get_roles():
			...
			terminal_framework.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "terminal_erp.selling.doctype.customer.customer.send_emails",
...
def send_emails(
	...
	credit_controller_users_list = terminal_framework.parse_json(credit_controller_users_list)
	subject = _("Credit limit reached for customer {0}").format(customer)
	message = _("Credit limit has been crossed for customer {0} ({1}/{2})").format(
		customer, customer_outstanding, credit_limit
	)
	terminal_framework.sendmail(recipients=credit_controller_users_list, subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	GLEntry = terminal_framework.qb.DocType("GL Entry")
	gle_query = (
	...
	if cost_center:
		lft, rgt = terminal_framework.get_cached_value("Cost Center", cost_center, ["lft", "rgt"])
		CostCenter = terminal_framework.qb.DocType("Cost Center")
		cost_center_subquery = (
			terminal_framework.qb.from_(CostCenter)
	...
	DeliveryNote = terminal_framework.qb.DocType("Delivery Note")
	DeliveryNoteItem = terminal_framework.qb.DocType("Delivery Note Item")
	...
	dn_query = (
		terminal_framework.qb.from_(DeliveryNote)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = terminal_framework.db.get_value(
			"Customer Credit Limit",
		...
		if not credit_limit:
			customer_group = terminal_framework.get_cached_value("Customer", customer, "customer_group")

			result = terminal_framework.db.get_values(
				"Customer Credit Limit",
	...
	if not credit_limit:
		credit_limit = terminal_framework.get_cached_value("Company", company, "credit_limit")
