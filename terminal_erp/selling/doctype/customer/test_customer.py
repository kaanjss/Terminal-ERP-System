...
from terminal_erp.accounts.party import get_due_date
from terminal_erp.exceptions import PartyDisabled, PartyFrozen
from terminal_erp.selling.doctype.customer.customer import (
	get_credit_limit,
	get_customer_outstanding,
)
from terminal_erp.selling.doctype.customer.mapper import (
	parse_full_name,
)
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestCustomer(Terminal ERPTestSuite):
	def test_get_customer_name_dedupes_with_numeric_suffix(self):
		...
		for nm in (base, f"{base} - 3"):
			if not terminal_framework.db.exists("Customer", nm):
				terminal_framework.get_doc(
					{"doctype": "Customer", "customer_name": nm, "customer_type": "Individual"}
				).insert()
			self.addCleanup(terminal_framework.delete_doc, "Customer", nm, force=1)

		doc = terminal_framework.get_doc({"doctype": "Customer", "customer_name": base, "customer_type": "Individual"})
	...
	def test_get_customer_group_details(self):
		doc = terminal_framework.new_doc("Customer Group")
		...
		c_doc = terminal_framework.new_doc("Customer")
	...
	def test_party_details_tax_category(self):
		...
		settings = terminal_framework.get_single("Accounts Settings")
	...
	def test_rename(self):
		...
		# add comments
		comment = terminal_framework.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		terminal_framework.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(terminal_framework.db.exists("Customer", new_name))
		self.assertFalse(terminal_framework.db.exists("Customer", "_Test Customer 1"))
		...
		terminal_framework.rename_doc("Customer", new_name, "_Test Customer 1")

	def test_freezed_customer(self):
		terminal_framework.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		terminal_framework.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = terminal_framework.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		terminal_framework.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		terminal_framework.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		terminal_framework.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		if not terminal_framework.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = terminal_framework.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = terminal_framework.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = terminal_framework.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(terminal_framework.ValidationError, so.submit)
	...
	def test_customer_credit_limit_after_submit(self):
		from terminal_erp.controllers.accounts_controller import update_child_qty_rate
		from terminal_erp.selling.doctype.sales_order.test_sales_order import make_sales_order
		...
		modified_item = terminal_framework._dict()
		for x in fields:
	...
	def test_customer_credit_limit_on_change(self):
		...
		self.assertRaises(terminal_framework.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		terminal_framework.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(customer_name=None, represents_company=None, allowed_to_interact_with=None):
	...
	existing_representative = terminal_framework.db.get_value("Customer", {"represents_company": represents_company})
	...
	if not terminal_framework.db.exists("Customer", customer_name):
		...
		customer_name = terminal_framework.db.get_value("Customer", customer_name)
