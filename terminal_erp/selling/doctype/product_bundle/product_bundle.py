...
class ProductBundle(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.selling.doctype.product_bundle_item.product_bundle_item import ProductBundleItem
	...
	def make_active(self):
		...
		others = terminal_framework.get_all(
			"Product Bundle",
			filters={
		...
		for name in others:
			terminal_framework.db.set_value("Product Bundle", name, "is_active", 0)
	...
	def on_trash(self):
		...
		for doctype in linked_doctypes:
			...
			invoices = terminal_framework.db.get_all(
	...
	def validate_main_item(self):
		...
		if terminal_framework.db.get_value("Item", self.new_item_code, "is_stock_item"):
			terminal_framework.throw(_("Parent Item {0} must not be a Stock Item").format(self.new_item_code))
		if terminal_framework.db.get_value("Item", self.new_item_code, "is_fixed_asset"):
			terminal_framework.throw(_("Parent Item {0} must not be a Fixed Asset").format(self.new_item_code))
...
def get_new_item_code(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	if not searchfield or searchfield == "name":
		searchfield = terminal_framework.get_meta("Item").get("search_fields")
