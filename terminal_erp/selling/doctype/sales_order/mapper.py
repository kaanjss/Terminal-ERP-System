...
from terminal_framework.contacts.doctype.address.address import get_company_address
...
from terminal_framework.utils import add_days, cint, flt, nowdate, strip_html

from terminal_erp.accounts.party import CROSS_PARTY_FIELD_NO_MAP, get_party_account
...
from terminal_erp.stock.doctype.item.item import get_item_defaults
from terminal_erp.stock.doctype.packed_item.packed_item import is_product_bundle, make_packing_list
from terminal_erp.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
	get_sre_details_for_voucher,
	get_sre_reserved_qty_details_for_voucher,
	get_ssb_bundle_for_voucher,
)
from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_bin_details, get_price_list_rate
		...
		if is_product_bundle(item.item_code):
			for packed_item in so.get("packed_items"):
				if (
					...
					result[packed_item.name] = terminal_framework._dict({"qty": packed_item.requested_qty})
		else:
			result[item.name] = terminal_framework._dict({"qty": item.requested_qty})
...
def make_material_request(source_name: str, target_doc: str | Document | None = None):
	...
	def postprocess(source, target):
		if source.tc_name and terminal_framework.db.get_value("Terms and Conditions", source.tc_name, "buying") != 1:
			target.tc_name = None
			target.terms = None
	...
	def get_remaining_packed_item_qty(so_item):
		delivered_qty = terminal_framework.db.get_value(
	...
	def update_item(source, target, source_parent):
		...
		ctx.update(
			{
				...
				"price_list": terminal_framework.db.get_single_value("Buying Settings", "buying_price_list"),
		...
		target.rate = flt(
			get_price_list_rate(ctx, item_doc=terminal_framework.get_cached_doc("Item", target.item_code)).get(
				"price_list_rate"
			)
		)
	...
	if doc and doc.items:
		...
		terminal_framework.throw(_("Material Request already created for the ordered quantity"))
...
def make_delivery_note(
	...
	if not kwargs:
		kwargs = {
			"for_reserved_stock": terminal_framework.flags.args and terminal_framework.flags.args.for_reserved_stock,
			"skip_item_mapping": terminal_framework.flags.args and terminal_framework.flags.args.skip_item_mapping,
		}
	...
	has_unit_price_items = terminal_framework.db.get_value("Sales Order", source_name, "has_unit_price_items")
	use_serial_batch_fields = terminal_framework.get_single_value("Stock Settings", "use_serial_batch_fields")
	...
	def set_missing_values(source, target):
		...
		if terminal_framework.flags.bulk_transaction:
	...
	def condition(doc):
		...
		# make_mapped_doc sets js `args` into `terminal_framework.flags.args`
		if terminal_framework.flags.args and terminal_framework.flags.args.delivery_dates:
			if terminal_framework.utils.cstr(doc.delivery_date) not in terminal_framework.flags.args.delivery_dates:
				return False
		if terminal_framework.flags.args and terminal_framework.flags.args.until_delivery_date:
			if terminal_framework.utils.cstr(doc.delivery_date) > terminal_framework.flags.args.until_delivery_date:
				return False
	...
	def update_item(source, target, source_parent):
		...
		if item:
			target.cost_center = (
				terminal_framework.db.get_value("Project", source_parent.project, "cost_center")
	...
	so = terminal_framework.get_doc("Sales Order", source_name)
			...
			for sre in sre_list:
				...
				if sre.reservation_based_on == "Serial and Batch" and (sre.has_serial_no or sre.has_batch_no):
					if use_serial_batch_fields:
						...
						sb_entries = terminal_framework.get_all(
							"Serial and Batch Entry",
	...
	if not kwargs.skip_item_mapping and terminal_framework.flags.bulk_transaction and not target_doc.items:
...
def make_sales_invoice(
	...
	def postprocess(source, target):
		...
		set_serial_batch_for_bundle_reservation(
			source,
			target,
			terminal_framework.get_single_value("Stock Settings", "use_serial_batch_fields"),
	...
	def update_item(source, target, source_parent):
		def get_billed_qty(so_item_name):
			table = terminal_framework.qb.DocType("Sales Invoice Item")
			query = (
		...
		if source_parent.project:
			target.cost_center = terminal_framework.db.get_value("Project", source_parent.project, "cost_center")
		if target.item_code:
			item = get_item_defaults(target.item_code, source_parent.company)
			item_group = get_item_group_defaults(target.item_code, source_parent.company)
	...
	def add_self_rm(doclist):
		parent = terminal_framework.qb.DocType("Subcontracting Inward Order")
		child = terminal_framework.qb.DocType("Subcontracting Inward Order Received Item")
		query = (
	...
	if terminal_framework.get_cached_value("Sales Order", source_name, "is_subcontracted"):
		add_self_rm(doclist)

	automatically_fetch_payment_terms = cint(
		terminal_framework.get_single_value("Accounts Settings", "automatically_fetch_payment_terms")
	)
...
def make_maintenance_schedule(source_name: str, target_doc: str | Document | None = None):
	maint_schedule = terminal_framework.db.exists(
...
def make_maintenance_visit(source_name: str, target_doc: str | Document | None = None):
	MaintenanceVisit = terminal_framework.qb.DocType("Maintenance Visit")
	MaintenanceVisitPurpose = terminal_framework.qb.DocType("Maintenance Visit Purpose")

	query = (
		terminal_framework.qb.from_(MaintenanceVisit)
...
def make_purchase_order(
	...
	selected_items = terminal_framework.parse_json(selected_items)

	def set_missing_values(source, target):
		target.supplier = supplier
		company_currency = terminal_framework.db.get_value(
			"Company", filters={"name": target.company}, fieldname=["default_currency"]
		)
		supplier_currency = terminal_framework.db.get_value(
		...
		default_price_list = terminal_framework.get_value("Supplier", supplier, "default_price_list")
		if default_price_list:
			target.buying_price_list = default_price_list

		default_payment_terms = terminal_framework.get_value("Supplier", supplier, "payment_terms")
...
def set_delivery_date(items: list, sales_order: str) -> None:
	...
	delivery_dates = terminal_framework.get_all(
		"Sales Order Item", filters={"parent": sales_order}, fields=["delivery_date", "product_bundle"]
	)

	delivery_by_bundle = terminal_framework._dict()
...
def make_work_orders(items: str | dict, sales_order: str, company: str, project: str | None = None):
	...
	items = terminal_framework.parse_json(items).get("items")
	out = []

	for i in items:
		if not i.get("bom"):
			terminal_framework.throw(_("Please select BOM against item {0}").format(i.get("item_code")))
		if not i.get("pending_qty"):
			terminal_framework.throw(_("Please select Qty against item {0}").format(i.get("item_code")))

		work_order = terminal_framework.get_doc(
			doctype="Work Order",
...
def make_production_plan(source_name: str, target_doc: str | Document | None = None):
	sales_order = terminal_framework.get_doc("Sales Order", source_name)

	production_plan = terminal_framework.new_doc(
		"Production Plan",
	...
	if sales_order.name not in open_so:
		terminal_framework.throw(_("Sales Order {0} is not available for production").format(sales_order.name))
...
def make_raw_material_request(
	items: str | terminal_framework._dict, company: str, sales_order: str, project: str | None = None
):
	...
	items = terminal_framework._dict(terminal_framework.parse_json(items))
	...
	if not raw_materials:
		terminal_framework.msgprint(_("Material Request not created, as quantity for Raw Materials already available."))
		return

	material_request = terminal_framework.new_doc("Material Request")
	...
	for item in raw_materials:
		item_doc = terminal_framework.get_cached_doc("Item", item.get("item_code"))
		...
		for item in so.items:
			if item.stock_reserved_qty > 0:
				terminal_framework.throw(
					...
					).format(terminal_framework.bold(source_name))
				)
...
def make_subcontracting_inward_order(source_name: str, target_doc: str | Document | None = None):
	if not is_so_fully_subcontracted(source_name):
		...
		terminal_framework.throw(_("This Sales Order has been fully subcontracted."))


def is_so_fully_subcontracted(so_name: str) -> bool:
	table = terminal_framework.qb.DocType("Sales Order Item")
	query = (
		terminal_framework.qb.from_(table)
		.select(table.name)
...
def get_mapped_subcontracting_inward_order(
	...
	def post_process(source_doc, target_doc):
		if (
			...
			target_doc.customer_warehouse = terminal_framework.get_cached_value(
				"Warehouse",
	...
	if target_doc and isinstance(target_doc, str):
		target_doc = terminal_framework.parse_json(target_doc)
