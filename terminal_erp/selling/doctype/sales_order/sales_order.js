...
terminal_erp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
terminal_erp.accounts.taxes.setup_tax_validations("Sales Order");
terminal_erp.sales_common.setup_selling_controller();

terminal_framework.ui.form.on("Sales Order", {
	setup: function (frm) {
	...
	delivery_date(frm) {
		if (frm.doc.delivery_date) {
			frm.doc.items.forEach((d) => {
				terminal_framework.model.set_value(d.doctype, d.name, "delivery_date", frm.doc.delivery_date);
			});
		}
	},
	transaction_date(frm) {
		...
		frm.doc.items.forEach((d) => {
			terminal_framework.model.set_value(d.doctype, d.name, "delivery_date", "");
		});
	},
		...
		if (frm.doc.docstatus === 1) {
			if (
				...
				frm.add_custom_button(__("Update Items"), () => {
					terminal_erp.utils.update_child_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				terminal_framework.model.can_cancel("Stock Reservation Entry") &&
				!frm.doc.is_subcontracted
			) {
				frm.add_custom_button(
					__("Unreserve"),
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0 && !frm.doc.is_subcontracted) {
				terminal_framework.call({
					method: "terminal_erp.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
	...
	get_items_from_internal_purchase_order(frm) {
		if (!terminal_framework.model.can_read("Purchase Order")) {
			return;
		}
		...
		frm.add_custom_button(
			...
			() => {
				terminal_erp.utils.map_current_doc({
					method: "terminal_erp.buying.doctype.purchase_order.mapper.make_inter_company_sales_order",
	...
	onload: function (frm) {
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "terminal_erp.controllers.queries.warehouse_query";
		...
		const dialog = new terminal_framework.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "terminal_erp.controllers.queries.get_filtered_child_rows",
							filters: {
			...
			primary_action: () => {
				...
				if (data.items && data.items.length > 0) {
					...
					terminal_framework.msgprint(__("Please select items to reserve."));
				}
			},
		});
	...
	cancel_stock_reservation_entries(frm) {
		const dialog = new terminal_framework.ui.Dialog({
			...
			primary_action: () => {
				...
				if (data.sr_entries && data.sr_entries.length > 0) {
					...
					terminal_framework.msgprint(__("Please select items to unreserve."));
				}
			},
		});
		...
		terminal_framework
			.call({
				method: "terminal_erp.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
	...
	show_reserved_stock(frm) {
		...
		terminal_framework.set_route("query-report", "Reserved Stock");
	},
	...
	prepare_delivery_schedule(frm, row, data) {
		let fields = [
			{
				...
				reqd: 1,
				default: row.delivery_date || frm.doc.delivery_date || terminal_framework.datetime.get_today(),
			},
			{
				fieldtype: "Float",
				fieldname: "qty",
			...
			},
			{
				...
				click: () => {
					terminal_framework.db.get_value("UOM", row.uom, "must_be_whole_number", (r) => {
		...
		frm.schedule_dialog = new terminal_framework.ui.Dialog({
			...
			primary_action: (data) => {
				if (!data.delivery_schedule || !data.delivery_schedule.length) {
					terminal_framework.throw(__("Please enter at least one delivery date and quantity"));
				}

				let total_qty = 0;
				data.delivery_schedule.forEach((d) => {
					if (!d.qty) {
						terminal_framework.throw(__("Please enter a valid quantity"));
					}
					total_qty += flt(d.qty);
				});
	...
	add_delivery_schedule(frm, row, must_be_whole_number) {
		...
		if (!frequency) {
			terminal_framework.throw(__("Please select a frequency for delivery schedule"));
		}
		if (!first_delivery_date) {
			terminal_framework.throw(__("Please enter the first delivery date"));
		}
		if (no_of_deliveries <= 0) {
			terminal_framework.throw(__("Please enter a valid number of deliveries"));
		}
		...
		for (let i = 0; i < no_of_deliveries; i++) {
			...
			if (!is_last) {
				if (frequency === "Weekly") {
					first_delivery_date = terminal_framework.datetime.add_days(first_delivery_date, 7);
				} else {
					first_delivery_date = terminal_framework.datetime.add_months(
						first_delivery_date,
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return terminal_framework.call({
			method: "terminal_erp.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return terminal_framework.call({
			method: "terminal_erp.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
	...
	reserve_stock(frm) {
		["items", "packed_items"].forEach((table) => {
			(frm.doc[table] || []).forEach((row) => {
				terminal_framework.model.set_value(row.doctype, row.name, "reserve_stock", frm.doc.reserve_stock);
			});
		});
	},
});

terminal_framework.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			terminal_erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
	...
	add_schedule(frm, cdt, cdn) {
		...
		if (row.__islocal) {
			terminal_framework.throw(__("Please save the Sales Order before adding a delivery schedule."));
		}
	...
	fg_item: async function (frm, cdt, cdn) {
		if (frm.doc.is_subcontracted) {
			...
			if (row.fg_item) {
				...
				if (result.message && Object.keys(result.message).length) {
					terminal_framework.model.set_value(cdt, cdn, "item_code", result.message.service_item);
					...
					terminal_framework.model.set_value(cdt, cdn, "uom", result.message.service_item_uom);
				}
			}
		}
	},
...
terminal_erp.selling.SalesOrderController = class SalesOrderController extends terminal_erp.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
			...
			if (doc.status !== "Closed") {
				if (doc.status !== "On Hold") {
					...
					if (
						...
						flt(doc.per_delivered) < 100 &&
						terminal_framework.model.can_create("Pick List") &&
					...
					if (
						...
						if (terminal_framework.model.can_create("Delivery Note")) {
							this.frm.add_custom_button(
								__("Delivery Note"),
						...
						if (terminal_framework.model.can_create("Work Order") && !doc.is_subcontracted) {
							this.frm.add_custom_button(
								__("Work Order"),
						...
						if (terminal_framework.model.can_create("Production Plan") && !doc.is_subcontracted) {
							this.frm.add_custom_button(
								__("Production Plan"),
					...
					// sales invoice
					if (
						(flt(doc.per_billed) < 100 && terminal_framework.model.can_create("Sales Invoice")) ||
						doc.is_subcontracted
					) {
						this.frm.add_custom_button(
							__("Sales Invoice"),
					...
					if (
						...
						terminal_framework.model.can_create("Material Request") &&
						!doc.is_subcontracted
					) {
						if (!doc.is_subcontracted) {
							this.frm.add_custom_button(
					...
					if (
						...
						terminal_framework.model.can_create("Purchase Order") &&
						!doc.is_subcontracted
					) {
						this.frm.add_custom_button(
							__("Purchase Order"),
					...
					if (flt(doc.per_delivered) < 100 && (order_is_maintenance || order_is_a_custom_sale)) {
						if (terminal_framework.model.can_create("Maintenance Visit")) {
							this.frm.add_custom_button(
						...
						if (terminal_framework.model.can_create("Maintenance Schedule")) {
							this.frm.add_custom_button(
					...
					// project
					if (flt(doc.per_delivered) < 100 && terminal_framework.model.can_create("Project")) {
				...
				// payment request
				if (flt(doc.per_billed) < 100 + terminal_framework.boot.sysdefaults.over_billing_allowance) {
					if (terminal_framework.boot.user.in_create.includes("Payment Request")) {
						this.frm.add_custom_button(
							__("Payment Request"),
					...
					if (terminal_framework.model.can_create("Payment Entry")) {
						this.frm.add_custom_button(
							__("Payment"),
		...
		if (
			...
			terminal_framework.model.can_read("Quotation") &&
			!this.frm.doc.is_subcontracted
		) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = terminal_erp.utils.map_current_doc({
						method: "terminal_erp.selling.doctype.quotation.mapper.make_sales_order",
	...
	items_add(doc, cdt, cdn) {
		...
		if (doc.delivery_date) {
			terminal_framework.model.set_value(cdt, cdn, "delivery_date", doc.delivery_date);
		} else {
	...
	create_pick_list() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "terminal_erp.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
					...
					var d = new terminal_framework.ui.Dialog({
						...
						primary_action: function () {
							...
							if (!data.items.length) {
								terminal_framework.throw(__("Please select at least one item to continue"));
							}
							me.frm.call({
	...
	make_production_plan() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_production_plan",
			frm: this.frm,
		});
	}
	...
	make_material_request() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new terminal_framework.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "terminal_erp.selling.doctype.sales_order.mapper.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
						...
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								<input
									type="checkbox"
									data-date="${date}"
									${terminal_framework.datetime.get_day_diff(new Date(date), today) > 0 ? "" : 'checked="checked"'}
								/>
								${terminal_framework.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_sales_invoice() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_sales_invoice",
			frm: this.frm,
		});
	}
	...
	make_project() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_purchase_order() {
		...
		var dialog = new terminal_framework.ui.Dialog({
			...
			primary_action(args) {
				...
				dialog.hide();
				return terminal_framework.call({
					method: "terminal_erp.selling.doctype.sales_order.mapper.make_purchase_order",
					...
					callback: function (r) {
						if (!r.exc) {
							if (r.message.length == 1) {
								terminal_framework.model.sync(r.message[0]);
								terminal_framework.set_route("Form", r.message[0].doctype, r.message[0].name);
							} else {
								terminal_framework.route_options = {
									sales_order: me.frm.doc.name,
								};
								terminal_framework.set_route("List", "Purchase Order");
							}
						}
					},
				});
	...
	hold_sales_order() {
		...
		var d = new terminal_framework.ui.Dialog({
			...
			primary_action: function () {
				var data = d.get_values();
				terminal_framework.call({
					method: "terminal_framework.desk.form.utils.add_comment",
					args: {
						reference_doctype: me.frm.doctype,
						reference_name: me.frm.docname,
						content: __("Reason for hold:") + " " + data.reason_for_hold,
						comment_email: terminal_framework.session.user,
						comment_by: terminal_framework.session.user_fullname,
					},
					callback: function (r) {
						if (!r.exc) {
	...
	update_status(label, status) {
		...
		terminal_framework.call({
			method: "terminal_erp.selling.doctype.sales_order.sales_order.update_status",
	...
	make_subcontracting_inward_order() {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.selling.doctype.sales_order.mapper.make_subcontracting_inward_order",
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new terminal_erp.selling.SalesOrderController({ frm: cur_frm }));
