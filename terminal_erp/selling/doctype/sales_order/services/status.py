...
class StatusService:
	...
	def check_modified_date(self) -> None:
		doc = self.doc
		mod_db = terminal_framework.db.get_value("Sales Order", doc.name, "modified")
		if mod_db and cstr(mod_db) != cstr(doc.modified):
			terminal_framework.throw(_("{0} {1} has been modified. Please refresh.").format(doc.doctype, doc.name))
	...
	def update_delivery_status(self) -> None:
		...
		for item in doc.items:
			if item.delivered_by_supplier:
				item_delivered_qty = terminal_framework.get_all(
					"Purchase Order Item",
	...
	def update_picking_status(self) -> None:
		...
		for so_item in doc.items:
			if cint(
				terminal_framework.get_cached_value("Item", so_item.item_code, "is_stock_item")
		...
		if total_picked_qty and total_qty:
			...
			pick_percentage = terminal_framework.get_single_value("Stock Settings", "over_picking_allowance")
			if pick_percentage:
