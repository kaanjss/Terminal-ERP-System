...
import terminal_framework.permissions
...
from terminal_framework.utils import add_days, flt, getdate, nowdate, today

from terminal_erp.controllers.accounts_controller import InvalidQtyError, get_due_date, update_child_qty_rate
from terminal_erp.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from terminal_erp.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from terminal_erp.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from terminal_erp.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from terminal_erp.selling.doctype.sales_order.mapper import (
	create_pick_list,
	make_delivery_note,
...
from terminal_erp.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
)
...
from terminal_erp.stock.get_item_details import get_bin_details
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSalesOrder(Terminal ERPTestSuite):
	...
	def test_sales_order_expired_item_price(self):
		...
		terminal_framework.db.delete("Item Price", {"item_code": item_1.item_code})
		...
		self.assertEqual(terminal_framework.db.get_value("Item Price", item_price.name, "price_list_rate"), 100)
		self.assertEqual(
			terminal_framework.db.count("Item Price", {"item_code": item_1.item_code, "price_list": price_list}),
			2,
		)
		all_item_prices = terminal_framework.get_all(
			"Item Price", filters={"item_code": item_1.item_code}, order_by="valid_from desc"
		)
		self.assertEqual(terminal_framework.db.get_value("Item Price", all_item_prices[0].name, "price_list_rate"), 1000)
	...
	def test_sales_order_with_full_material_request(self):
		...
		mr.submit()
		mr.reload()
		self.assertRaises(terminal_framework.ValidationError, make_material_request, so.name)
	...
	def test_sales_order_qty(self):
		...
		self.assertRaises(terminal_framework.NonNegativeError, so.save)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_delivery_note, so.name)
	...
	def test_make_production_plan(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_production_plan, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_sales_invoice, so.name)
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		terminal_framework.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		terminal_framework.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		terminal_framework.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(terminal_framework.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		terminal_framework.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(terminal_framework.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(terminal_framework.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_perm(self):
		...
		with self.set_user(test_user.name):
			...
			self.assertRaises(
				terminal_framework.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name
			)
	...
	def test_update_child_qty_rate_with_workflow(self):
		from terminal_framework.model.workflow import apply_workflow
		...
		test_user = terminal_framework.get_doc("User", user)
		...
		test_user2 = terminal_framework.get_doc("User", user2)
	...
	def test_update_child_qty_rate_follows_allow_edit(self):
		...
		mover_user = terminal_framework.get_doc("User", mover)
		...
		editor_user = terminal_framework.get_doc("User", editor)
	...
	def test_material_request_for_product_bundle(self):
		...
		if not terminal_framework.db.exists("Item", "_Test Product Bundle Item New 1"):
	...
	def test_update_child_product_bundle(self):
		# test Update Items with product bundle
		if not terminal_framework.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not terminal_framework.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not terminal_framework.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
		...
		new_item_with_tax = terminal_framework.get_doc("Item", "Test Item with Tax")
		...
		if not terminal_framework.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
		...
		old_stock_settings_value = terminal_framework.db.get_single_value("Stock Settings", "default_warehouse")
		terminal_framework.db.set_single_value("Stock Settings", "default_warehouse", "_Test Warehouse - _TC")
		...
		terminal_framework.get_doc("Item Tax Template", "Test Update Items Template - _TC").delete()
		terminal_framework.db.set_single_value("Stock Settings", "default_warehouse", old_stock_settings_value)

	def test_warehouse_user(self):
		test_user = create_user("test_so_warehouse_user@example.com", "Sales User", "Stock User")

		test_user_2 = terminal_framework.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		terminal_framework.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		terminal_framework.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		terminal_framework.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		with self.set_user(test_user.name):
			...
			self.assertRaises(terminal_framework.PermissionError, so.insert)

		with self.set_user(test_user_2.name):
			so.insert()

		terminal_framework.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		terminal_framework.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		terminal_framework.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(terminal_framework.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		item_price = terminal_framework.db.get_value(
			"Item Price", {"price_list": "_Test Price List", "item_code": "_Test Item for Auto Price List"}
		)
		if item_price:
			terminal_framework.delete_doc("Item Price", item_price)
		...
		terminal_framework.delete_doc("Item Price", item_price.name)

		terminal_framework.db.set_single_value("Stock Settings", "update_price_list_based_on", "Rate")
		...
		terminal_framework.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)
		...
		terminal_framework.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 1)
	...
	def test_update_existing_item_price(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 0)

	def test_drop_shipping(self):
		from terminal_erp.buying.doctype.purchase_order.purchase_order import update_status
		from terminal_erp.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if terminal_framework.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		bin_po_item = terminal_framework.get_all(
			"Bin",
		...
		self.assertEqual(flt(terminal_framework.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
		so.reload()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not terminal_framework.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not terminal_framework.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_reserved_qty_for_closing_so(self):
		bin = terminal_framework.get_all(
			"Bin",
	...
	def test_create_so_with_margin(self):
		...
		new_so = terminal_framework.copy_doc(so)
	...
	def test_make_work_order(self):
		from terminal_erp.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		terminal_framework.db.set_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order", 0)
		...
		so_doc = terminal_framework.get_doc("Sales Order", so.name)

		self.assertRaises(terminal_framework.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = terminal_framework.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_request_for_raw_materials(self):
		...
		so.submit()
		mr_dict = terminal_framework._dict()
		...
		mr = terminal_framework.get_all("Material Request", fields=["name"], order_by="creation desc", limit=1)[0]
		mr_doc = terminal_framework.get_doc("Material Request", mr.get("name"))
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = terminal_framework.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(terminal_framework.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(terminal_framework.LinkExistsError, so.cancel)
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from terminal_erp.stock.doctype.material_request.material_request import raise_work_orders
		...
		wo = terminal_framework.get_doc("Work Order", wo_name)
		...
		se = terminal_framework.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
	...
	def test_sales_order_partial_advance_payment(self):
		...
		terminal_framework.get_doc(customer).insert()
	...
	def test_packed_items_for_partial_sales_order(self):
		...
		for product_bundle in [
			...
			if not terminal_framework.db.exists("Item", product_bundle):
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"editable_bundle_item_rates": 1})
	def test_expired_rate_for_packed_item(self):
		...
		for scenario in [
			...
			with self.subTest(scenario=scenario):
				...
				so = terminal_framework.new_doc("Sales Order")
				so.transaction_date = nowdate()
	...
	@patch(
		...
		"terminal_erp.accounts.doctype.payment_request.payment_request.PaymentRequest.get_payment_url",
		return_value=None,
	)
	def test_sales_order_advance_payment_status(self, mocked_get_payment_url):
		...
		so.submit()
		self.assertEqual(terminal_framework.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested")
		...
		self.assertEqual(terminal_framework.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested")
		...
		self.assertEqual(terminal_framework.db.get_value(so.doctype, so.name, "advance_payment_status"), "Fully Paid")
		...
		pr.cancel()
		self.assertEqual(
			terminal_framework.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested"
		...
		pe.cancel()
		self.assertEqual(
			terminal_framework.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested"
		)  # here: reset
	...
	def test_pick_list_for_batch(self):
		from terminal_erp.stock.doctype.pick_list.mapper import create_delivery_note
		...
		pick_list.save()
		batch_no = terminal_framework.get_all(
	...
	def test_auto_update_price_list(self):
		...
		item_price = terminal_framework.db.get_value("Item Price", {"item_code": item.name}, "price_list_rate")
		...
		terminal_framework.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 1)
		so = make_sales_order(
	...
	def test_credit_limit_on_so_reopning(self):
		# set credit limit
		company = "_Test Company"
		customer = terminal_framework.get_doc("Customer", "_Test Customer")
		...
		self.assertRaises(terminal_framework.ValidationError, so1.update_status, "Draft")

	@Terminal ERPTestSuite.change_settings("Stock Settings", {"enable_stock_reservation": True})
	def test_warehouse_mapping_based_on_stock_reservation(self):
		...
		sres = terminal_framework.get_all(
		...
		sre_doc = terminal_framework.get_doc("Stock Reservation Entry", sres[0].name)
	...
	@Terminal ERPTestSuite.change_settings("Selling Settings", {"allow_zero_qty_in_sales_order": 1})
	...
	def test_item_tax_transfer_from_sales_to_purchase(self):
		item_tax = terminal_framework.new_doc("Item Tax Template")
		...
		item_group = terminal_framework.get_doc("Item Group", "_Test Item Group")
	...
	def test_make_purchase_order_does_not_inherit_party_fields(self):
		...
		supplier = terminal_framework.get_doc("Supplier", "_Test Supplier")
	...
	def test_selling_price_validation_for_manufactured_item(self):
		...
		self.assertRaises(terminal_framework.ValidationError, so.save)
	...
	def test_product_bundle_reservation(self):
		...
		from terminal_erp.stock.serial_batch_bundle import get_batches_from_bundle, get_serial_nos
		...
		dn.items[0].qty = 5
		dn.save()
		sabb_doc = terminal_framework.get_doc("Serial and Batch Bundle", dn.packed_items[1].serial_and_batch_bundle)
		...
		si.items[0].qty = 5
		si.save()
		sabb_doc = terminal_framework.get_doc("Serial and Batch Bundle", si.packed_items[1].serial_and_batch_bundle)
		sabb_doc.entries = sabb_doc.entries[:5]
	...
	def test_sales_team_contribution_follows_grant_commission(self):
		...
		"""
		terminal_framework.db.set_value("Item", "_Test Item", "grant_commission", 1)
		terminal_framework.db.set_value("Item", "_Test FG Item", "grant_commission", 0)
		try:
			so = make_sales_order(
				do_not_save=True,
				item_list=[
			...
			terminal_framework.db.set_value("Item", "_Test Item", "grant_commission", 1)
			terminal_framework.db.set_value("Item", "_Test FG Item", "grant_commission", 1)
	...
	def test_sales_team_disabled_sales_person_rejected(self):
		terminal_framework.db.set_value("Sales Person", "_Test Sales Person 2", "enabled", 0)
		try:
			so = make_sales_order(do_not_save=True)
			so.append("sales_team", {"sales_person": "_Test Sales Person 2", "allocated_percentage": 100})
			self.assertRaises(terminal_framework.ValidationError, so.save)
		finally:
			terminal_framework.db.set_value("Sales Person", "_Test Sales Person 2", "enabled", 1)
	...
	def test_commission_fields_not_copied_on_duplicate(self):
		...
		try:
			...
			duplicate = terminal_framework.copy_doc(so, ignore_no_copy=False)
	...
	def test_commission_rate_carried_through_mapper(self):
		...
		from terminal_erp.selling.doctype.sales_order.mapper import make_delivery_note, make_sales_invoice

		original = terminal_framework.db.get_value("Item", "_Test Item", "grant_commission")
		terminal_framework.db.set_value("Item", "_Test Item", "grant_commission", 1)
		try:
			so = make_sales_order(qty=10, rate=100, do_not_save=True)
			so.sales_partner = "_Test Sales Partner India - 1"
			...
			terminal_framework.db.set_value("Item", "_Test Item", "grant_commission", original)
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty"))


def make_sales_order_workflow():
	if terminal_framework.db.exists("Workflow", "SO Test Workflow"):
		doc = terminal_framework.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	terminal_framework.get_doc(doctype="Role", role_name="Test Junior Approver").insert(ignore_if_duplicate=True)
	terminal_framework.get_doc(doctype="Role", role_name="Test Approver").insert(ignore_if_duplicate=True)
	terminal_framework.cache().hdel("roles", terminal_framework.session.user)

	workflow = terminal_framework.get_doc(
		{
			"doctype": "Workflow",
...
def make_sales_order_edit_perm_workflow():
	if terminal_framework.db.exists("Workflow", "SO Edit Perm Workflow"):
		doc = terminal_framework.get_doc("Workflow", "SO Edit Perm Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc
