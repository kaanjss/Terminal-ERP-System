terminal_framework.listview_settings["Sales Order"] = {
	add_fields: [
		"base_grand_total",
		"customer_name",
		"currency",
	...
	get_indicator: function (doc) {
		if (doc.status === "Closed") {
			...
			if (terminal_framework.datetime.get_diff(doc.delivery_date) < 0) {
	...
	onload: function (listview) {
		var method = "terminal_erp.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		if (terminal_framework.model.can_create("Sales Invoice")) {
			listview.page.add_action_item(__("Sales Invoice"), () => {
				terminal_erp.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
			});
		}
		...
		if (terminal_framework.model.can_create("Delivery Note")) {
			listview.page.add_action_item(__("Delivery Note"), () => {
				terminal_framework.call({
					method: "terminal_erp.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
					callback: (r) => {
						if (r.message) {
								...
								fields: [
									{
										...
										default: terminal_framework.datetime.add_days(terminal_framework.datetime.nowdate(), 1),
									},
								],
							});
		...
		if (terminal_framework.model.can_create("Payment Entry")) {
			listview.page.add_action_item(__("Advance Payment"), () => {
				terminal_erp.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
			});
		}
	},
};
