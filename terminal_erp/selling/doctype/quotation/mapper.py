...
def make_sales_order(
	...
	if not terminal_framework.db.get_singles_value(
		"Selling Settings", "allow_sales_order_creation_for_expired_quotation"
	):
		quotation = terminal_framework.db.get_value(
			"Quotation", source_name, ["transaction_date", "valid_till"], as_dict=1
		)
		if quotation.valid_till and (
			quotation.valid_till < quotation.transaction_date or quotation.valid_till < getdate(nowdate())
		):
			terminal_framework.throw(_("Validity period of this quotation has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False, args=None):
	...
	selected_rows = [x.get("name") for x in terminal_framework.flags.get("args", {}).get("selected_items", [])]

	# 0 qty is accepted, as the qty uncertain for some items
	has_unit_price_items = terminal_framework.db.get_value("Quotation", source_name, "has_unit_price_items")
	...
	def set_missing_values(source, target):
		...
		if source.referral_sales_partner:
			...
			target.commission_rate = terminal_framework.get_value(
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation.quotation_to == "Customer":
		return terminal_framework.get_doc("Customer", quotation.party_name)
	elif quotation.quotation_to == "CRM Deal":
		customer_name = terminal_framework.get_value("Customer", {"crm_deal": quotation.party_name})
		if customer_name:
			return terminal_framework.get_doc("Customer", customer_name)

	# Check if a Customer already exists for the Lead or Prospect.
	existing_customer = None
	if quotation.quotation_to == "Lead":
		existing_customer = terminal_framework.db.get_value("Customer", {"lead_name": quotation.party_name})
	elif quotation.quotation_to == "Prospect":
		existing_customer = terminal_framework.db.get_value("Customer", {"prospect_name": quotation.party_name})

	if existing_customer:
		return terminal_framework.get_doc("Customer", existing_customer)
...
def create_customer_from_lead(lead_name, ignore_permissions=False):
	from terminal_erp.crm.doctype.lead.lead import _make_customer
	...
	try:
		customer.insert()
		return customer
	except terminal_framework.MandatoryError as e:
...
def handle_mandatory_error(e, customer, lead_name):
	...
	terminal_framework.throw(message, title=_("Mandatory Missing"))


def get_ordered_items(quotation: str) -> terminal_framework._dict:
