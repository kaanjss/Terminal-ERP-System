...
class Quotation(SellingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.selling.doctype.quotation_item.quotation_item import QuotationItem
		from terminal_erp.setup.doctype.quotation_lost_reason_detail.quotation_lost_reason_detail import (
	...
	def validate_valid_till(self):
		if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
			terminal_framework.throw(_("Valid till date cannot be before transaction date"))
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not terminal_framework.get_single_value("Selling Settings", "allow_zero_qty_in_quotation"):
			return
	...
	def set_customer_name(self):
		if self.party_name and self.quotation_to == "Customer":
			...
			self.customer_name = terminal_framework.db.get_value("CRM Deal", self.party_name, "organization")
	...
	def update_opportunity_status(self, status, opportunity=None):
		...
		opp = terminal_framework.get_doc("Opportunity", opportunity)
	...
	def declare_enquiry_lost(
		...
		if not (self.is_fully_ordered() or self.is_partially_ordered()):
			get_lost_reasons = terminal_framework.get_list("Quotation Lost Reason", fields=["name"])
				...
				if reason.get("lost_reason") in lost_reasons_lst:
					...
					terminal_framework.throw(
						_("Invalid lost reason {0}, please create a new lost reason").format(
							terminal_framework.bold(reason.get("lost_reason"))
						)
					)
			...
			terminal_framework.throw(_("Cannot set as Lost as Sales Order is made."))
...
def set_expired_status():
	...
	(
		terminal_framework.qb.update(quotation)
