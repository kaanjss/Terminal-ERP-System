...
terminal_erp.accounts.taxes.setup_tax_validations("Sales Taxes and Charges Template");
terminal_erp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
terminal_erp.pre_sales.set_as_lost("Quotation");
terminal_erp.sales_common.setup_selling_controller();

terminal_framework.ui.form.on("Quotation", {
	setup: function (frm) {
...
terminal_erp.selling.QuotationController = class QuotationController extends terminal_erp.selling.SellingController {
	onload(doc, dt, dn) {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.__islocal && !doc.valid_till) {
			if (terminal_framework.boot.sysdefaults.quotation_valid_till) {
				this.frm.set_value(
					"valid_till",
					terminal_framework.datetime.add_days(
						doc.transaction_date,
						terminal_framework.boot.sysdefaults.quotation_valid_till
					)
				);
			} else {
				this.frm.set_value("valid_till", terminal_framework.datetime.add_months(doc.transaction_date, 1));
			}
		}

		if (doc.docstatus == 1 && !["Lost", "Ordered"].includes(doc.status)) {
			if (
				terminal_framework.model.can_create("Sales Order") &&
				(terminal_framework.boot.sysdefaults.allow_sales_order_creation_for_expired_quotation ||
					!doc.valid_till ||
					terminal_framework.datetime.get_diff(doc.valid_till, terminal_framework.datetime.get_today()) >= 0)
			) {
		...
		if (this.frm.doc.docstatus === 0 && terminal_framework.model.can_read("Opportunity")) {
	...
	set_dynamic_field_label() {
		if (this.frm.doc.quotation_to == "Customer") {
			...
			this.frm.fields_dict.party_name.get_query = function () {
				return { query: "terminal_erp.controllers.queries.lead_query" };
			};
	...
	address_query(doc) {
		return {
			...
			filters: {
				link_doctype: terminal_framework.dynamic_link.doctype,
	...
	get_lead_details() {
		...
		terminal_framework.call({
			method: "terminal_erp.crm.doctype.lead.lead.get_lead_details",
			args: {
...
cur_frm.script_manager.make(terminal_erp.selling.QuotationController);

terminal_framework.ui.form.on(
	"Quotation Item",
...
terminal_framework.ui.form.on("Quotation Item", "stock_balance", function (frm, cdt, cdn) {
	var d = terminal_framework.model.get_doc(cdt, cdn);
	terminal_framework.route_options = { item_code: d.item_code };
