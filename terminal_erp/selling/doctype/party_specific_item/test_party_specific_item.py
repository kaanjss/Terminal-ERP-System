...
def create_supplier(supplier_name):
	if terminal_framework.db.exists("Supplier", supplier_name):
		return terminal_framework.get_doc("Supplier", supplier_name)
...
class TestPartySpecificItem(Terminal ERPTestSuite):
	...
	def test_item_query_for_supplier_with_item_restricted_to_multiple_suppliers(self):
		item = f"Party Specific Item {terminal_framework.generate_hash(length=8)}"
		supplier1 = f"Party Specific Supplier {terminal_framework.generate_hash(length=8)}"
		supplier2 = f"Party Specific Supplier {terminal_framework.generate_hash(length=8)}"
	...
	def test_party_group(self):
		customer = "_Test Customer With Template"
		item = "_Test Item"
		terminal_framework.set_value("Customer", customer, "customer_group", "Government")
