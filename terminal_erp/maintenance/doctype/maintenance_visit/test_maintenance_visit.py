...
from terminal_framework.utils.data import add_days, getdate, today

from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestMaintenanceVisit(Terminal ERPTestSuite):
	def setUp(self):
	...
	def make_warranty_claim(self):
		...
		claim = terminal_framework.new_doc("Warranty Claim")
		claim.status = "Open"
	...
	def test_cancel_blocked_when_later_visit_exists(self):
		...
		self.assertRaisesRegex(terminal_framework.ValidationError, later.name, earlier.cancel)
	...
	def test_cancel_allowed_for_latest_visit(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Maintenance Visit", later.name, "docstatus"), 2)
		self.assertEqual(terminal_framework.db.get_value("Maintenance Visit", later.name, "status"), "Cancelled")
		# The earlier one is untouched and still submitted.
		self.assertEqual(terminal_framework.db.get_value("Maintenance Visit", earlier.name, "docstatus"), 1)
	...
	def test_cancel_reopens_claim_to_work_in_progress_from_prior_partial(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Warranty Claim", claim.name, "status"), "Closed")
...
def make_sales_person(name):
	sales_person = terminal_framework.get_doc({"doctype": "Sales Person", "sales_person_name": name})
	sales_person.insert(ignore_if_duplicate=True)
	if not sales_person.name:
		sales_person = terminal_framework.get_doc("Sales Person", {"sales_person_name": name})
	return sales_person


def make_maintenance_visit():
	mv = terminal_framework.new_doc("Maintenance Visit")
