...
from terminal_framework.utils import add_days, cint, cstr, date_diff, formatdate, getdate

from terminal_erp.setup.doctype.employee.employee import get_holiday_list_for_employee
from terminal_erp.stock.doctype.serial_no.serial_no import get_serial_nos
from terminal_erp.utilities.transaction_base import TransactionBase, delete_events
...
class MaintenanceSchedule(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.maintenance.doctype.maintenance_schedule_detail.maintenance_schedule_detail import (
			MaintenanceScheduleDetail,
		)
		from terminal_erp.maintenance.doctype.maintenance_schedule_item.maintenance_schedule_item import (
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			if d.sales_person and d.sales_person not in email_map:
				sp = terminal_framework.get_doc("Sales Person", d.sales_person)
				try:
			...
			scheduled_date = terminal_framework.db.get_all(
			...
			for key in scheduled_date:
				description = terminal_framework._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
					self.name, d.item_code, self.customer
				)
				event = terminal_framework.get_doc(
					{
						"doctype": "Event",
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = terminal_framework.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = terminal_framework.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = terminal_framework.get_all("Holiday", filters={"parent": holiday_list}, pluck="holiday_date")
	...
	def validate_sales_order(self):
		ms = terminal_framework.qb.DocType("Maintenance Schedule")
		msi = terminal_framework.qb.DocType("Maintenance Schedule Item")
		for d in self.get("items"):
			if d.sales_order:
				# filter the parent schedule's docstatus (matches the original ms.docstatus = 1)
				chk = (
					terminal_framework.qb.from_(ms)
					.inner_join(msi)
	...
	def validate_serial_no_bundle(self):
		...
		voucher_nos = terminal_framework.get_all(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				terminal_framework.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				terminal_framework.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						terminal_framework.bold(serial_no), terminal_framework.bold(item_code)
					),
					title=_("Invalid"),
				)
	...
	def get_pending_data(self, data_type: str, s_date: str | None = None, item_name: str | None = None):
		if data_type == "date":
			dates = ""
			if not item_name:
				terminal_framework.throw(_("Item Name is required."))
			...
			if not s_date:
				terminal_framework.throw(_("Scheduled Date is required."))
			for schedule in self.schedules:
...
def get_serial_nos_from_schedule(item_code: str, schedule: str):
	serial_nos = terminal_framework.db.get_value(
...
def make_maintenance_visit(
	...
	def update_serial(source, target, parent):
		if source.item_reference:
			if sbb := terminal_framework.db.get_value(
				"Maintenance Schedule Item", source.item_reference, "serial_and_batch_bundle"
			):
				serial_nos = terminal_framework.get_doc("Serial and Batch Bundle", sbb).get_serial_nos()
