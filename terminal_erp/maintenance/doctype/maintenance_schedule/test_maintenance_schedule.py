...
from terminal_framework.utils import format_date
from terminal_framework.utils.data import add_days, formatdate, today

from terminal_erp.maintenance.doctype.maintenance_schedule.maintenance_schedule import (
...
class TestMaintenanceSchedule(Terminal ERPTestSuite):
	def setUp(self):
	...
	def make_sales_person(cls):
		...
		cls.sales_person = []
		for x in records:
			if not terminal_framework.db.exists("Sales Person", {"sales_person_name": x.get("sales_person_name")}):
				cls.sales_person.append(terminal_framework.get_doc(x).insert())
			else:
				cls.sales_person.append(
					terminal_framework.get_doc("Sales Person", {"sales_person_name": x.get("sales_person_name")})
				)
	...
	def test_make_schedule(self):
		...
		visit = terminal_framework.new_doc("Maintenance Visit")
		...
		visit.save()
		visit.submit()
		ms = terminal_framework.get_doc("Maintenance Schedule", ms.name)
	...
	def test_validate_sales_order_duplicate_throws(self):
		...
		self.assertRaises(terminal_framework.ValidationError, make_maintenance_schedule, sales_order=so.name)
	...
	def test_validate_schedule_date_skips_holiday(self):
		...
		from terminal_erp.setup.doctype.holiday_list.test_holiday_list import make_holiday_list

		holiday = add_days(today(), 5)
		hl = make_holiday_list(
			"_Test MS Holidays " + terminal_framework.generate_hash("", 6),
			from_date=today(),
		...
		sp = terminal_framework.get_doc(
			{"doctype": "Sales Person", "sales_person_name": "_Test MS SP " + terminal_framework.generate_hash("", 5)}
		).insert(ignore_permissions=True)
		terminal_framework.db.set_value("Company", ms.company, "default_holiday_list", hl.name)
...
def make_serial_item_with_serial(self, item_code):
	...
	active_serials = terminal_framework.db.get_all("Serial No", {"status": "Active", "item_code": item_code})
...
def make_maintenance_schedule(**args):
	ms = terminal_framework.new_doc("Maintenance Schedule")
