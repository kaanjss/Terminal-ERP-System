...
from terminal_framework.utils import DateTimeLikeObject, cint, flt, get_link_to_form, getdate, time_diff_in_hours
...
class AssetRepair(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.assets.doctype.asset_repair_consumed_item.asset_repair_consumed_item import (
			AssetRepairConsumedItem,
		)
		from terminal_erp.assets.doctype.asset_repair_purchase_invoice.asset_repair_purchase_invoice import (
	...
	def validate(self):
		self.asset_doc = terminal_framework.get_lazy_doc("Asset", self.asset)
		self.validate_asset()
		self.validate_dates()
	...
	def validate_duplicate_purchase_invoices(self):
		...
		if duplicates:
			duplicate_links = "".join(
				[
					f"<li>{_('Row #{0}:').format(idx)} {get_link_to_form('Purchase Invoice', pi)} - {terminal_framework.bold(account)}</li>"
	...
	def validate_purchase_invoice_status(self):
		...
		docstatus = terminal_framework._dict(
		...
		if row.expense_account not in valid_accounts:
			terminal_framework.throw(
				_(
					...
					row.idx,
					terminal_framework.bold(row.expense_account),
		...
		if flt(row.repair_cost) > available_amount:
			terminal_framework.throw(
				_(
					...
					row.idx,
					terminal_framework.bold(terminal_framework.format_value(row.repair_cost, {"fieldtype": "Currency"})),
					terminal_framework.bold(terminal_framework.format_value(available_amount, {"fieldtype": "Currency"})),
					get_link_to_form("Purchase Invoice", row.purchase_invoice),
					terminal_framework.bold(row.expense_account),
				)
			)
	...
	def update_status(self):
		if self.repair_status == "Pending" and self.asset_doc.status != "Out of Order":
			terminal_framework.db.set_value("Asset", self.asset, "status", "Out of Order")
	...
	def after_delete(self):
		terminal_framework.get_lazy_doc("Asset", self.asset).set_status()

	def check_repair_status(self):
		if self.repair_status == "Pending" and self.docstatus == 1:
			terminal_framework.throw(_("Please update Repair Status."))
	...
	def validate_serial_no(self, stock_item):
		if not stock_item.serial_and_batch_bundle and terminal_framework.get_cached_value(
			"Item", stock_item.item_code, "has_serial_no"
		):
			msg = _("Serial No Bundle is mandatory for Item {0}").format(stock_item.item_code)
			terminal_framework.throw(msg, title=_("Missing Serial No Bundle"))
	...
	def get_gl_entries(self):
		from terminal_erp.assets.doctype.asset_repair.services.gl_composer import AssetRepairGLComposer
...
def _get_expense_accounts_for_purchase_invoice(purchase_invoice: str) -> list[str]:
	...
	"""
	pi_items = terminal_framework.db.get_all(
...
def get_unallocated_repair_cost(
	...
	terminal_framework.has_permission("Purchase Invoice", "read", purchase_invoice, throw=True)
...
def get_allocated_repair_cost(
	...
	query = (
		terminal_framework.qb.from_(asset_repair_pi)
