...
from terminal_erp.assets.doctype.asset.asset import get_asset_account
...
class AssetRepairGLComposer(BaseGLComposer):
	...
	def _get_gl_entries_for_consumed_items(self, gl_entries: list, fixed_asset_account: str) -> None:
		...
		stock_entry_name = terminal_framework.db.get_value("Stock Entry", {"asset_repair": doc.name}, "name")
		stock_entry_items = terminal_framework.get_all(
			"Stock Entry Detail", filters={"parent": stock_entry_name}, fields=["expense_account", "amount"]
		)

		default_expense_account = None
		if not terminal_erp.is_perpetual_inventory_enabled(doc.company):
			default_expense_account = terminal_framework.get_cached_value(
				"Company", doc.company, "default_expense_account"
			)
			if not default_expense_account:
				terminal_framework.throw(_("Please set default Expense Account in Company {0}").format(doc.company))
