# Copyright (c) 2015, Terminal Framework Technologies Pvt. Ltd. and Contributors
# See license.txt

import terminal_framework
from terminal_framework.utils import add_days, now

from terminal_erp.assets.doctype.asset.test_asset import create_asset
from terminal_erp.setup.doctype.employee.test_employee import make_employee
from terminal_erp.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestAssetMovement(Terminal ERPTestSuite):
	def setUp(self):
		terminal_framework.db.set_value(
			"Company", "_Test Company", "capital_work_in_progress_account", "CWIP Account - _TC"
		)

	def test_movement(self):
		pr = make_purchase_receipt(item_code="Macbook Pro", qty=1, rate=100000.0, location="Test Location")

		asset_name = terminal_framework.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
		asset = terminal_framework.get_doc("Asset", asset_name)
		asset.calculate_depreciation = 1
		asset.available_for_use_date = "2020-06-06"
		asset.purchase_date = "2020-06-06"
		asset.append(
			"finance_books",
			{
				"expected_value_after_useful_life": 10000,
				"next_depreciation_date": "2020-12-31",
				"depreciation_method": "Straight Line",
				"total_number_of_depreciations": 3,
				"frequency_of_depreciation": 10,
			},
		)

		if asset.docstatus == 0:
			asset.submit()

		create_asset_movement(
			purpose="Transfer",
			company=asset.company,
			assets=[
				{
					"asset": asset.name,
					"source_location": "Test Location",
					"target_location": "Test Location 2",
				}
			],
			reference_doctype="Purchase Receipt",
			reference_name=pr.name,
		)
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location 2")

		movement1 = create_asset_movement(
			purpose="Transfer",
			company=asset.company,
			assets=[
				{
					"asset": asset.name,
					"source_location": "Test Location 2",
					"target_location": "Test Location",
				}
			],
			reference_doctype="Purchase Receipt",
			reference_name=pr.name,
		)
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location")

		movement1.cancel()
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location 2")

		employee = make_employee("testassetmovemp@example.com", company="_Test Company")
		create_asset_movement(
			purpose="Issue",
			company=asset.company,
			assets=[{"asset": asset.name, "source_location": "Test Location 2", "to_employee": employee}],
			reference_doctype="Purchase Receipt",
			reference_name=pr.name,
		)

		# after issuing, asset should belong to an employee not at a location
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location 2")
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "custodian"), employee)

		create_asset_movement(
			purpose="Receipt",
			company=asset.company,
			assets=[{"asset": asset.name, "from_employee": employee, "target_location": "Test Location"}],
			reference_doctype="Purchase Receipt",
			reference_name=pr.name,
		)

		# after receiving, asset should belong to a location not at an employee
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location")

	def test_last_movement_cancellation(self):
		pr = make_purchase_receipt(item_code="Macbook Pro", qty=1, rate=100000.0, location="Test Location")

		asset_name = terminal_framework.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
		asset = terminal_framework.get_doc("Asset", asset_name)
		asset.calculate_depreciation = 1
		asset.available_for_use_date = "2020-06-06"
		asset.purchase_date = "2020-06-06"
		asset.append(
			"finance_books",
			{
				"expected_value_after_useful_life": 10000,
				"next_depreciation_date": "2020-12-31",
				"depreciation_method": "Straight Line",
				"total_number_of_depreciations": 3,
				"frequency_of_depreciation": 10,
			},
		)
		if asset.docstatus == 0:
			asset.submit()

		movement = terminal_framework.get_doc({"doctype": "Asset Movement", "reference_name": pr.name})
		self.assertRaises(terminal_framework.ValidationError, movement.cancel)

		movement1 = create_asset_movement(
			purpose="Transfer",
			company=asset.company,
			assets=[
				{
					"asset": asset.name,
					"source_location": "Test Location",
					"target_location": "Test Location 2",
				}
			],
			reference_doctype="Purchase Receipt",
			reference_name=pr.name,
		)
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location 2")

		movement1.cancel()
		self.assertEqual(terminal_framework.db.get_value("Asset", asset.name, "location"), "Test Location")

	def test_movement_transaction_date(self):
		asset = create_asset(item_code="Macbook Pro", do_not_save=1)
		asset.save().submit()

		asset_creation_date = terminal_framework.db.get_value(
			"Asset Movement",
			[["Asset Movement Item", "asset", "=", asset.name], ["docstatus", "=", 1]],
			"transaction_date",
		)
		asset_movement = create_asset_movement(
			purpose="Transfer",
			company=asset.company,
			assets=[
				{
					"asset": asset.name,
					"source_location": "Test Location",
					"target_location": "Test Location 2",
				}
			],
			transaction_date=add_days(asset_creation_date, -1),
			do_not_save=True,
		)
		self.assertRaises(terminal_framework.ValidationError, asset_movement.save)


def create_asset_movement(**args):
	args = terminal_framework._dict(args)

	if not args.transaction_date:
		args.transaction_date = now()

	movement = terminal_framework.new_doc("Asset Movement")
	movement.update(
		{
			"assets": args.assets,
			"transaction_date": args.transaction_date,
			"company": args.company,
			"purpose": args.purpose or "Receipt",
			"reference_doctype": args.reference_doctype,
			"reference_name": args.reference_name,
		}
	)
	if not args.do_not_save:
		movement.insert(ignore_if_duplicate=True)
		if not args.do_not_submit:
			movement.submit()

	return movement
