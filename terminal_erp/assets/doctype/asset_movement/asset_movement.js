...
terminal_framework.ui.form.on("Asset Movement", {
	setup: (frm) => {
...
terminal_framework.ui.form.on("Asset Movement Item", {
			...
			terminal_framework.db
				...
				.then((asset_doc) => {
					if (asset_doc.location)
						terminal_framework.model.set_value(cdt, cdn, "source_location", asset_doc.location);
					if (asset_doc.custodian)
						terminal_framework.model.set_value(cdt, cdn, "from_employee", asset_doc.custodian);
				})
				.catch((err) => {
