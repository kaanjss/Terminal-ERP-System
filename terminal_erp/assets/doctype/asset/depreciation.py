...
from terminal_framework.query_builder.functions import Max, Min
...
from terminal_erp.accounts.doctype.journal_entry.mapper import make_reverse_journal_entry
...
def post_depreciation_entries(date=None):
	...
	if not cint(terminal_framework.get_single_value("Accounts Settings", "book_asset_depreciation_entry_automatically")):
		return
...
def book_depreciation_entries(date):
	...
	for data in depreciable_assets_data:
		...
		try:
			...
			error_log = terminal_framework.log_error(e)
...
def get_depreciable_assets_data(date):
	a = terminal_framework.qb.DocType("Asset")
	ads = terminal_framework.qb.DocType("Asset Depreciation Schedule")
	ds = terminal_framework.qb.DocType("Depreciation Schedule")

	res = (
		terminal_framework.qb.from_(ads)
		.join(a)
		.on(ads.asset == a.name)
		.join(ds)
...
def get_companies_with_frozen_limits():
	...
	for d in terminal_framework.get_all(
		...
		if (
			d.role_allowed_for_frozen_entries not in terminal_framework.get_roles()
			and terminal_framework.session.user != "Administrator"
		):
...
def get_depreciation_cost_center_and_series(asset):
	depreciation_cost_center, depreciation_series = terminal_framework.get_cached_value(
...
def make_depreciation_entry(
	...
	terminal_framework.has_permission("Journal Entry", throw=True)
	date = date or today()

	depr_schedule_doc = terminal_framework.get_doc("Asset Depreciation Schedule", depr_schedule_name)
	asset = terminal_framework.get_doc("Asset", depr_schedule_doc.asset)
	...
	for d in depr_schedule_doc.get("depreciation_schedule")[
		...
		terminal_framework.db.savepoint("depr_entry")
		try:
			...
			terminal_framework.db.rollback(save_point="depr_entry")
...
def _make_journal_entry_for_depreciation(
	...
	je = terminal_framework.new_doc("Journal Entry")
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = terminal_framework.get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		terminal_framework.throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		terminal_framework.db.set_value("Asset", asset_name, "depr_entry_posting_status", "Failed")


def notify_depr_entry_posting_error(failed_asset_names, error_log_names):
	user_role = terminal_framework.db.get_single_value("Accounts Settings", "role_to_notify_on_depreciation_failure")
...
def scrap_asset(asset_name: str, scrap_date: DateTimeLikeObject | None = None):
	terminal_framework.has_permission("Asset", "write", asset_name, throw=True)
...
def validate_asset_for_scrap(asset, scrap_date):
	if asset.docstatus != 1:
		terminal_framework.throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized"):
		terminal_framework.throw(_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
...
def validate_scrap_date(asset, scrap_date):
	if scrap_date > getdate():
		terminal_framework.throw(_("Future date is not allowed"))
	elif scrap_date < getdate(asset.purchase_date):
		terminal_framework.throw(_("Scrap date cannot be before purchase date"))
	...
	if asset.calculate_depreciation:
		...
		if (
			...
			terminal_framework.throw(_("Asset cannot be scrapped before the last depreciation entry."))


def get_last_depreciation_date(asset_name):
	depreciation = terminal_framework.qb.DocType("Asset Depreciation Schedule")
	depreciation_schedule = terminal_framework.qb.DocType("Depreciation Schedule")

	last_depreciation_date = (
		terminal_framework.qb.from_(depreciation)
...
def create_journal_entry_for_scrap(asset, scrap_date):
	depreciation_series = terminal_framework.get_cached_value("Company", asset.company, "series_for_depreciation_entry")
...
def cancel_journal_entry_for_scrap(asset):
	if asset.journal_entry_for_scrap:
		...
		terminal_framework.get_doc("Journal Entry", je).cancel()
...
def create_reverse_depreciation_entry(asset_name, journal_entry):
	...
	terminal_framework.flags.is_reverse_depr_entry = True
...
def update_value_after_depreciation_on_asset_restore(schedule, row, journal_entry):
	terminal_framework.db.set_value("Depreciation Schedule", schedule.name, "journal_entry", None, update_modified=False)
...
def get_depreciation_accounts(asset_category, company):
	...
	accounts = terminal_framework.db.get_value(
	...
	if not fixed_asset_account:
		terminal_framework.throw(_("Please set Fixed Asset Account in Asset Category {0}").format(asset_category))

	if not non_depreciable_category:
		accounts = terminal_framework.get_cached_value(
...
def get_disposal_account_and_cost_center(company: str):
	disposal_account, depreciation_cost_center = terminal_framework.get_cached_value(
		"Company", company, ["disposal_account", "depreciation_cost_center"]
	)

	if not disposal_account:
		terminal_framework.throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		terminal_framework.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
