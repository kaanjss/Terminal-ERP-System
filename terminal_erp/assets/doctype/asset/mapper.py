...
def make_sales_invoice(asset: str, item_code: str, company: str, sell_qty: int, serial_no: str | None = None):
	...
	si.company = company
	si.currency = terminal_framework.get_cached_value("Company", company, "default_currency")
...
def create_asset_maintenance(
	...
	asset_maintenance = terminal_framework.new_doc("Asset Maintenance")
...
def create_asset_repair(
	...
	asset_repair = terminal_framework.new_doc("Asset Repair")
...
def create_asset_capitalization(
	...
	asset_capitalization = terminal_framework.new_doc("Asset Capitalization")
...
def create_asset_value_adjustment(
	...
	asset_value_adjustment = terminal_framework.new_doc("Asset Value Adjustment")
...
def make_asset_movement(
	...
	assets = terminal_framework.parse_json(assets)

	if len(assets) == 0:
		terminal_framework.throw(_("At least one asset has to be selected."))

	asset_movement = terminal_framework.new_doc("Asset Movement")
	asset_movement.purpose = purpose
	for asset in assets:
		asset = terminal_framework.get_doc("Asset", asset.get("name"))
...
def split_asset(asset_name: str, split_qty: int):
	...
	existing_asset = terminal_framework.get_doc("Asset", asset_name)
...
def validate_split_quantity(existing_asset, split_qty):
	if split_qty >= existing_asset.asset_quantity:
		terminal_framework.throw(_("Split Quantity must be less than Asset Quantity"))
...
def process_asset_split(existing_asset, split_qty, splitted_asset=None, is_new_asset=False):
	...
	new_asset = terminal_framework.copy_doc(existing_asset) if is_new_asset else splitted_asset
...
def create_new_depr_schedule(current_depr_schedule_doc, existing_asset, new_asset, is_new_asset, fb_row):
	...
	new_depr_schedule_doc = terminal_framework.copy_doc(current_depr_schedule_doc)
...
def add_reference_in_jv_on_split(entry_name, new_asset_name, old_asset_name, depreciation_amount):
	...
	journal_entry = terminal_framework.get_doc("Journal Entry", entry_name)
	entries_to_add = []
...
def adjust_existing_accounts(journal_entry, old_asset_name, depreciation_amount, entries_to_add):
	...
	for account in journal_entry.get("accounts"):
		if account.reference_name == old_asset_name:
			entries_to_add.append(terminal_framework.copy_doc(account).as_dict())
