...
import terminal_erp
from terminal_erp.accounts.general_ledger import make_reverse_gl_entries
...
class Asset(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.assets.doctype.asset_finance_book.asset_finance_book import AssetFinanceBook
	...
	def create_asset_depreciation_schedule(self):
		...
		for fb_row in self.get("finance_books"):
			...
			if not existing_schedule:
				new_schedule = terminal_framework.new_doc("Asset Depreciation Schedule")
	...
	def before_submit(self):
		if self.asset_type == "Composite Asset" and not has_active_capitalization(self.name):
			if self.split_from and has_active_capitalization(self.split_from):
				return
			terminal_framework.throw(_("Please capitalize this asset before submitting."))
	...
	def get_linked_item(self, purchase_doc_type, purchase_doc):
		purchase_doc = terminal_framework.get_doc(purchase_doc_type, purchase_doc)
	...
	def validate_asset_and_reference(self):
		if self.purchase_invoice or self.purchase_receipt:
			...
			reference_doc = terminal_framework.get_doc(reference_doc, reference_name)
		...
		if self.asset_type == "Existing Asset" and self.purchase_invoice:
			terminal_framework.throw(_("Purchase Invoice cannot be made against an existing asset {0}").format(self.name))
	...
	def validate_item(self):
		...
		if not item:
			terminal_framework.throw(_("Item {0} does not exist").format(self.item_code))
		elif item.disabled:
			terminal_framework.throw(_("Item {0} has been disabled").format(self.item_code))
		elif not item.is_fixed_asset:
			terminal_framework.throw(_("Item {0} must be a Fixed Asset Item").format(self.item_code))
		elif item.is_stock_item:
			terminal_framework.throw(_("Item {0} must be a non-stock item").format(self.item_code))

	def validate_cost_center(self):
		if self.cost_center:
			cost_center_company, cost_center_is_group = terminal_framework.db.get_value(
				"Cost Center", self.cost_center, ["company", "is_group"]
			)
			if cost_center_company != self.company:
				terminal_framework.throw(
					_("Cost Center {0} does not belong to Company {1}").format(
						terminal_framework.bold(self.cost_center), terminal_framework.bold(self.company)
					),
			...
			if cost_center_is_group:
				terminal_framework.throw(
					...
					).format(terminal_framework.bold(self.cost_center)),
					title=_("Invalid Cost Center"),
				)

		else:
			if not terminal_framework.get_cached_value("Company", self.company, "depreciation_cost_center"):
				terminal_framework.throw(
					_(
						"Please set a Cost Center for the Asset or set an Asset Depreciation Cost Center for the Company {0}"
					).format(terminal_framework.bold(self.company)),
					title=_("Missing Cost Center"),
				)

	def validate_in_use_date(self):
		if not self.available_for_use_date and self.asset_type != "Composite Component":
			terminal_framework.throw(_("Available for use date is required"))
	...
	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = terminal_framework.get_cached_value("Item", self.item_code, "asset_category")
	...
	def validate_category(self):
		non_depreciable_category = terminal_framework.db.get_value(
	...
	def validate_asset_values(self):
		...
		if not flt(self.net_purchase_amount) and self.asset_type != "Composite Asset":
			terminal_framework.throw(_("Net Purchase Amount is mandatory"), terminal_framework.MandatoryError)
		...
		if is_cwip_accounting_enabled(self.asset_category):
			...
			if (
				...
				and not terminal_framework.db.get_value("Purchase Invoice", self.purchase_invoice, "update_stock")
			):
				terminal_framework.throw(
		...
		if not self.calculate_depreciation:
			...
			if not self.finance_books:
				terminal_framework.throw(_("Enter depreciation details"))
			if self.is_fully_depreciated:
				terminal_framework.throw(_("Depreciation cannot be calculated for fully depreciated assets"))

		if self.asset_type == "Existing Asset":
			return

		if self.available_for_use_date and getdate(self.available_for_use_date) < getdate(self.purchase_date):
			terminal_framework.throw(_("Available-for-use Date should be after purchase date"))
	...
	def validate_linked_purchase_documents(self):
		...
		for fieldname, doctype in [
			...
			if terminal_framework.db.get_value(doctype, purchase_doc, "docstatus") == 0:
				terminal_framework.throw(
	...
	def validate_asset_qty_with_purchase_doc(self, doctype, purchase_doc):
		...
		existing_asset_qty = (
			terminal_framework.qb.from_(Asset)
			.select(IfNull(Sum(Asset.asset_quantity), 0))
			.where((Asset.item_code == self.item_code) & (Asset.name != self.name) & (Asset.docstatus != 2))
			.where(asset_filter)
		).run()[0][0]

		PurchaseDoc = terminal_framework.qb.DocType(doctype)
		PurchaseDocItems = terminal_framework.qb.DocType(f"{doctype} Item")

		purchased_qty = (
			terminal_framework.qb.from_(PurchaseDoc)
	...
	def validate_gross_and_purchase_amount(self):
		...
		if self.net_purchase_amount and self.net_purchase_amount != self.purchase_amount:
			...
			terminal_framework.throw(error_message, title=_("Invalid Net Purchase Amount"))
	...
	def make_asset_movement(self):
		...
		if reference_docname:
			posting_date, posting_time = terminal_framework.db.get_value(
		...
		asset_movement = terminal_framework.get_doc(
			{
	...
	def validate_asset_finance_books(self, row):
		...
		if flt(row.expected_value_after_useful_life) < 0:
			terminal_framework.throw(_("Row {0}: Expected Value After Useful Life cannot be negative").format(row.idx))
	...
	def validate_opening_depreciation_values(self, row):
		...
		if self.opening_accumulated_depreciation:
			if not self.opening_number_of_booked_depreciations:
				terminal_framework.throw(_("Please set opening number of booked depreciations"))
		else:
			self.opening_number_of_booked_depreciations = 0
	...
	def validate_total_number_of_depreciations_and_frequency(self, row):
		...
		if row.frequency_of_depreciation <= 0:
			terminal_framework.throw(_("Row #{0}: Frequency of Depreciation must be greater than zero").format(row.idx))
	...
	def validate_cancellation(self):
		...
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			terminal_framework.throw(_("Asset cannot be cancelled, as it is already {0}").format(self.status))
	...
	def cancel_movement_entries(self):
		...
		for movement in movements:
			movement = terminal_framework.get_doc("Asset Movement", movement.get("name"))
			movement.cancel()
		...
		if self.calculate_depreciation:
			for row in self.get("finance_books"):
				...
				for d in depr_schedule or []:
					if d.journal_entry:
						terminal_framework.get_doc("Journal Entry", d.journal_entry).cancel()
		else:
			depr_entries = self.get_manual_depreciation_entries()

			for depr_entry in depr_entries or []:
				terminal_framework.get_doc("Journal Entry", depr_entry.name).cancel()
	...
	def get_default_finance_book_idx(self):
		if not self.get("default_finance_book") and self.company:
			self.default_finance_book = terminal_erp.get_default_finance_book(self.company)
	...
	def validate_make_gl_entry(self):
		...
		if asset_bought_with_invoice:
			...
			expense_booked = terminal_framework.db.exists(
			...
			cwip_booked = terminal_framework.db.exists(
			...
			return bool(
				terminal_framework.db.exists("GL Entry", {"voucher_no": purchase_document, "account": cwip_account})
			)

	def get_purchase_document(self):
		asset_bought_with_invoice = self.purchase_invoice and terminal_framework.db.get_value(
		...
		if not fixed_asset_account:
			terminal_framework.throw(
				_("Set {0} in asset category {1} for company {2}").format(
					terminal_framework.bold(_("Fixed Asset Account")),
					terminal_framework.bold(self.asset_category),
	...
	def check_asset_capitalization_gl_entries(self):
		if self.asset_type == "Composite Asset":
			result = terminal_framework.db.get_value(
	...
	def get_depreciation_rate(self, args: str | dict | Document, on_validate: bool = False):
		...
		rate_field_precision = terminal_framework.get_single_value("System Settings", "float_precision") or 2
...
def has_gl_entries(doctype, docname, target_account):
	gl_entry = terminal_framework.qb.DocType("GL Entry")
	gl_entries = (
		terminal_framework.qb.from_(gl_entry)
...
def update_maintenance_status():
	...
	for asset in assets:
		asset = terminal_framework.get_doc("Asset", asset.name)
		if terminal_framework.db.exists("Asset Repair", {"asset_name": asset.name, "repair_status": "Pending"}):
			asset.set_status("Out of Order")
		elif terminal_framework.db.exists("Asset Maintenance Task", {"parent": asset.name, "next_due_date": today()}):
...
def make_post_gl_entry():
	asset_categories = terminal_framework.db.get_all("Asset Category", fields=["name", "enable_cwip_accounting"])
	...
	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			...
			for asset in assets:
				doc = terminal_framework.get_doc("Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = terminal_framework.get_meta("Asset")
...
def get_item_details(
	...
	asset_category_doc = terminal_framework.get_cached_doc("Asset Category", asset_category)
	books = []
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
	...
	if not account:
		if not asset_category:
			terminal_framework.throw(_("Set {0} in company {1}").format(account_name.replace("_", " ").title(), company))
		else:
			terminal_framework.throw(
...
def is_cwip_accounting_enabled(asset_category):
	return cint(terminal_framework.db.get_value("Asset Category", asset_category, "enable_cwip_accounting"))
...
def get_asset_value_after_depreciation(
	...
	asset = terminal_framework.get_doc("Asset", asset_name)
...
def has_active_capitalization(asset: str):
	active_capitalizations = terminal_framework.db.count(
...
def get_values_from_purchase_doc(
	...
	purchase_doc = terminal_framework.get_doc(doctype, purchase_doc_name)
	matching_items = [item for item in purchase_doc.items if item.item_code == item_code]

	if not matching_items:
		terminal_framework.throw(_("Selected {0} does not contain the Item Code {1}").format(doctype, item_code))
