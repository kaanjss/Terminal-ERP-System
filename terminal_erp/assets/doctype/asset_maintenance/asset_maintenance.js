...
terminal_framework.ui.form.on("Asset Maintenance", {
	setup: (frm) => {
		...
		frm.set_query("assign_to", "asset_maintenance_tasks", function (doc) {
			return {
				query: "terminal_erp.assets.doctype.asset_maintenance.asset_maintenance.get_team_members",
				filters: {
	...
	make_dashboard: (frm) => {
		if (!frm.is_new()) {
			terminal_framework.call({
				method: "terminal_erp.assets.doctype.asset_maintenance.asset_maintenance.get_maintenance_log",
					...
					(r.message || []).forEach(function (d) {
						$(`<div class='row' style='margin-bottom: 10px;'>
							<div class='col-sm-3 small'>
								<a onclick="terminal_framework.set_route('List', 'Asset Maintenance Log',
...
terminal_framework.ui.form.on("Asset Maintenance Task", {
...
var get_next_due_date = function (frm, cdt, cdn) {
	...
	if (d.start_date && d.periodicity) {
		return terminal_framework.call({
			method: "terminal_erp.assets.doctype.asset_maintenance.asset_maintenance.calculate_next_due_date",
			args: {
			...
			callback: function (r) {
				if (r.message) {
					terminal_framework.model.set_value(cdt, cdn, "next_due_date", r.message);
				} else {
					terminal_framework.model.set_value(cdt, cdn, "next_due_date", "");
				}
			},
		});
	}
