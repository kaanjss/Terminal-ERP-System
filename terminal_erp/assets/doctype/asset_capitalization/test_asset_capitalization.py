...
from terminal_framework.utils import cint, flt, now_datetime
...
class TestAssetCapitalization(Terminal ERPTestSuite):
	def setUp(self):
		set_depreciation_settings_in_company()
	...
	def test_capitalization_with_periodical_inventory(self):
		...
		# Test General Ledger Entries
		default_expense_account = terminal_framework.db.get_value("Company", company, "default_expense_account")
		expected_gle = {
	...
	def test_capitalize_composite_component(self):
		...
		consumed_asset_name = terminal_framework.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
		consumed_asset_doc = terminal_framework.get_doc("Asset", consumed_asset_name)
...
def create_asset_capitalization(**args):
	...
	now = now_datetime()
	target_asset = terminal_framework.get_doc("Asset", args.target_asset) if args.target_asset else terminal_framework._dict()
				...
				terminal_framework._dict(
					{
						...
						"company": terminal_framework.get_cached_value("Warehouse", source_warehouse, "company"),
...
def create_depreciation_asset(**args):
	...
	asset = terminal_framework.new_doc("Asset")
