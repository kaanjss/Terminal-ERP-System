...
from terminal_framework.utils import cint, flt, get_link_to_form

import terminal_erp
from terminal_erp.assets.doctype.asset.asset import get_asset_value_after_depreciation
...
class AssetCapitalization(StockController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.assets.doctype.asset_capitalization_asset_item.asset_capitalization_asset_item import (
			AssetCapitalizationAssetItem,
		)
		from terminal_erp.assets.doctype.asset_capitalization_service_item.asset_capitalization_service_item import (
			AssetCapitalizationServiceItem,
		)
		from terminal_erp.assets.doctype.asset_capitalization_stock_item.asset_capitalization_stock_item import (
	...
	def validate_target_item(self):
		target_item = terminal_framework.get_cached_doc("Item", self.target_item_code)

		if not target_item.is_fixed_asset:
			terminal_framework.throw(_("Target Item {0} must be a Fixed Asset item").format(target_item.name))
	...
	def validate_target_asset(self):
		if self.target_asset:
			...
			if not target_asset.asset_type == "Composite Asset":
				terminal_framework.throw(_("Target Asset {0} needs to be a composite asset").format(target_asset.name))
			...
			if target_asset.docstatus == 1:
				terminal_framework.throw(_("Target Asset {0} cannot be submitted").format(target_asset.name))
			elif target_asset.docstatus == 2:
				terminal_framework.throw(_("Target Asset {0} cannot be cancelled").format(target_asset.name))
	...
	def validate_consumed_stock_item(self):
		for d in self.stock_items:
			if d.item_code:
				item = terminal_framework.get_cached_doc("Item", d.item_code)

				if not item.is_stock_item:
					terminal_framework.throw(_("Row #{0}: Item {1} is not a stock item").format(d.idx, d.item_code))

				if flt(d.stock_qty) <= 0:
					terminal_framework.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))
		...
		for d in self.asset_items:
			if d.asset:
				...
				if asset.docstatus == 0:
					terminal_framework.throw(_("Row #{0}: Consumed Asset {1} cannot be Draft").format(d.idx, asset.name))
		...
		for d in self.service_items:
			if d.item_code:
				...
				if item.is_stock_item or item.is_fixed_asset:
					terminal_framework.throw(_("Row #{0}: Item {1} is not a service item").format(d.idx, d.item_code))

				if flt(d.qty) <= 0:
					terminal_framework.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))

				if flt(d.rate) <= 0:
					terminal_framework.throw(_("Row #{0}: Amount must be a positive number").format(d.idx))

				self.validate_item(item)

			if not d.cost_center:
				d.cost_center = terminal_framework.get_cached_value("Company", self.company, "cost_center")
	...
	def get_gl_entries(
		...
		from terminal_erp.assets.doctype.asset_capitalization.services.gl_composer import (
	...
	def get_target_account(self):
		...
		asset_category = terminal_framework.get_cached_value("Asset", self.target_asset, "asset_category")
	...
	def get_composite_component_value(self):
		...
		for item in self.asset_items:
			asset = terminal_framework.db.get_value("Asset", item.asset, ["asset_type"], as_dict=True)
	...
	def update_target_asset(self):
		...
		asset_doc = terminal_framework.get_doc("Asset", self.target_asset)
	...
	def restore_consumed_asset_items(self):
		for item in self.asset_items:
			asset = terminal_framework.get_doc("Asset", item.asset)
...
def get_target_asset_details(asset: str | None = None, company: str | None = None):
	...
	# Get Asset Details
	asset_details = terminal_framework._dict()
	if asset:
		asset_details = terminal_framework.db.get_value("Asset", asset, ["asset_name", "item_code"], as_dict=1)
		if not asset_details:
			terminal_framework.throw(_("Asset {0} does not exist").format(asset))
...
def get_consumed_stock_item_details(ctx: ItemDetailsCtx):
	...
	if ctx.item_code and out.warehouse:
		incoming_rate_args = terminal_framework._dict(
			{
...
@terminal_framework.whitelist()
@terminal_erp.normalize_ctx_input(ItemDetailsCtx)
def get_warehouse_details(ctx: ItemDetailsCtx) -> terminal_framework._dict:
	out = terminal_framework._dict()
...
@terminal_framework.whitelist()
@terminal_erp.normalize_ctx_input(ItemDetailsCtx)
def get_consumed_asset_details(ctx: ItemDetailsCtx) -> terminal_framework._dict:
	out = terminal_framework._dict()

	asset_details = terminal_framework._dict()
	if ctx.asset:
		asset_details = terminal_framework.db.get_value(
			"Asset", ctx.asset, ["asset_name", "item_code", "item_name"], as_dict=1
		)
		if not asset_details:
			terminal_framework.throw(_("Asset {0} does not exist").format(ctx.asset))
	...
	# Cost Center
	if asset_details.item_code:
		item = terminal_framework.get_cached_doc("Item", asset_details.item_code)
		item_defaults = get_item_defaults(item.name, ctx.company)
		item_group_defaults = get_item_group_defaults(item.name, ctx.company)
		brand_defaults = get_brand_defaults(item.name, ctx.company)
		out.cost_center = get_default_cost_center(ctx, item_defaults, item_group_defaults, brand_defaults)
...
@terminal_framework.whitelist()
@terminal_erp.normalize_ctx_input(ItemDetailsCtx)
def get_service_item_details(ctx: ItemDetailsCtx) -> terminal_framework._dict:
	out = terminal_framework._dict()
...
def get_items_tagged_to_wip_composite_asset(params: dict | str):
	params = terminal_framework.parse_json(params)
	...
	pr_items = terminal_framework.get_all(
...
def process_stock_item(d):
	stock_capitalized = terminal_framework.db.exists(
	...
	stock_item_data = terminal_framework._dict(d)
...
def process_fixed_asset(d):
	...
	if asset_details:
		...
		return terminal_framework._dict(d)
	return None
