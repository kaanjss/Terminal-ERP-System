...
from terminal_framework.utils import cstr, flt, getdate
...
from terminal_erp.assets.doctype.asset_repair.test_asset_repair import create_asset_repair
from terminal_erp.assets.doctype.asset_value_adjustment.test_asset_value_adjustment import (
	make_asset_value_adjustment,
)
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestAssetDepreciationSchedule(Terminal ERPTestSuite):
	def test_throw_error_if_another_asset_depr_schedule_exist(self):
		...
		second_asset_depr_schedule = terminal_framework.get_doc(
			{"doctype": "Asset Depreciation Schedule", "asset": asset.name, "finance_book": None}
		)

		self.assertRaises(terminal_framework.ValidationError, second_asset_depr_schedule.insert)

	def test_daily_prorata_based_depr_on_sl_method(self):
		terminal_framework.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 0)
		asset = create_asset(
	...
	def test_schedule_for_slm_for_existing_asset_daily_pro_rata_enabled(self):
		terminal_framework.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 1)
		asset = create_asset(
			calculate_depreciation=1,
			depreciation_method="Straight Line",
