...
from terminal_framework.utils import add_months, cstr, flt, formatdate, getdate, nowdate, today
...
def get_data(filters):
	...
	group_by = terminal_framework.scrub(filters.get("group_by"))
	...
	assets_record = terminal_framework.db.get_all("Asset", filters=conditions, fields=fields)
...
def get_conditions(filters):
	...
	date_field = terminal_framework.scrub(filters.date_based_on or "Purchase Date")
...
def prepare_chart_data(data, filters):
	...
	if filters.filter_based_on not in ("Date Range", "Fiscal Year"):
		...
		date_field = terminal_framework.scrub(filters.date_based_on)
	...
	for d in period_list:
		labels_values_map.setdefault(
			d.get("label"), terminal_framework._dict({"asset_value": 0, "depreciated_amount": 0})
		)
...
def get_assets_linked_to_fb(filters):
	...
	query = terminal_framework.qb.from_(afb).select(
		afb.parent,
	)
	...
	if filters.include_default_book_assets:
		...
		if filters.finance_book and company_fb and cstr(filters.finance_book) != cstr(company_fb):
			terminal_framework.throw(_("To use a different finance book, please uncheck 'Include Default FB Assets'"))
...
def get_group_by_data(
	...
	assets = terminal_framework.db.get_all("Asset", filters=conditions, fields=fields)
...
def get_purchase_receipt_supplier_map():
	...
	pri = terminal_framework.qb.DocType("Purchase Receipt Item")
...
def get_purchase_invoice_supplier_map():
	...
	pii = terminal_framework.qb.DocType("Purchase Invoice Item")
	return terminal_framework._dict(
		...
		return [
			{
				...
				"fieldtype": "Link",
				"fieldname": terminal_framework.scrub(filters.get("group_by")),
