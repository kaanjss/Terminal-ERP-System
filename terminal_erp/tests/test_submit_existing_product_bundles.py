...
from terminal_erp.patches.v16_0.submit_existing_product_bundles import execute
...
class TestSubmitExistingProductBundles(Terminal ERPTestSuite):
	def _make_legacy_bundle(self, item_code, child, disabled=0):
		...
		bundle = terminal_framework.get_doc(
			{
		...
		terminal_framework.rename_doc("Product Bundle", bundle.name, item_code, force=True, show_alert=False)
	...
	def test_patch_renames_and_submits_legacy_bundle(self):
		...
		self.assertFalse(terminal_framework.db.exists("Product Bundle", legacy))
		migrated = get_active_product_bundle(parent)
		self.assertTrue(migrated and migrated.startswith("PB-"))
		self.assertEqual(terminal_framework.db.get_value("Product Bundle", migrated, "docstatus"), 1)
		self.assertEqual(terminal_framework.db.get_value("Product Bundle", migrated, "is_active"), 1)
	...
	def test_patch_seeds_is_active_from_disabled(self):
		...
		migrated = terminal_framework.db.get_value("Product Bundle", {"new_item_code": parent, "docstatus": 1}, "name")
		self.assertTrue(migrated and migrated.startswith("PB-"))
		self.assertEqual(terminal_framework.db.get_value("Product Bundle", migrated, "is_active"), 0)
	...
	def test_patch_submits_partially_migrated_bundle(self):
		...
		self.assertEqual(terminal_framework.db.get_value("Product Bundle", bundle.name, "docstatus"), 1)
