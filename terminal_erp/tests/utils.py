...
from terminal_framework.core.doctype.report.report import get_report_module_dotted_path
from terminal_framework.custom.doctype.custom_field.custom_field import create_custom_fields
from terminal_framework.tests.utils import load_test_records_for
from terminal_framework.utils import now_datetime, today
...
def execute_script_report(
	...
	test_filters = []
	report_execute_fn = terminal_framework.get_attr(get_report_module_dotted_path(module, report_name) + ".execute")
	report_filters = terminal_framework._dict(default_filters).copy().update(filters)
...
def if_lending_app_not_installed(function):
	...
	def wrapper(*args, **kwargs):
		if "lending" not in terminal_framework.get_installed_apps():
			return function(*args, **kwargs)
		return
...
class BootStrapTestData:
	...
	def make_presets(self):
		from terminal_framework.desk.page.setup_wizard.install_fixtures import update_genders, update_salutations

		from terminal_erp.setup.setup_wizard.operations.install_fixtures import (
			add_uom_data,
			get_preset_records,
			get_sale_stages,
		)
		...
		for x in records:
			...
			if not terminal_framework.db.exists(dt, dn):
				doc = terminal_framework.get_doc(x)
				doc.insert()

		add_uom_data()
		# add sale stages
		for sales_stage in get_sale_stages():
			if not terminal_framework.db.exists("Sales Stage", {"stage_name": sales_stage.get("stage_name")}):
				terminal_framework.get_doc(sales_stage).insert()

		from terminal_erp.buying.doctype.supplier_scorecard.supplier_scorecard import (
		...
		for x in get_default_scorecard_variables():
			...
			if not terminal_framework.db.exists("Supplier Scorecard Variable", {"name": x.get("variable_label")}):
				terminal_framework.get_doc(x).insert()
		for x in get_default_scorecard_standing():
			x["doctype"] = "Supplier Scorecard Standing"
			if not terminal_framework.db.exists("Supplier Scorecard Standing", {"name": x.get("standing_name")}):
				terminal_framework.get_doc(x).insert()
	...
	def update_system_settings(self):
		system_settings = terminal_framework.get_doc("System Settings")
	...
	def update_support_settings(self):
		support_settings = terminal_framework.get_doc("Support Settings")
	...
	def update_stock_settings(self):
		...
		stock_settings.default_warehouse = terminal_framework.db.get_value("Warehouse", {"warehouse_name": _("Stores")})
	...
	def make_records(self, key, records):
		...
		for x in records:
			...
			if not terminal_framework.db.exists(doctype, filters):
				terminal_framework.get_doc(x).insert()
	...
	def make_test_account(self):
		...
		for company, abbr in [
			...
			for account_name, parent_account, is_group, account_type, currency in records:
				if not terminal_framework.db.exists("Account", {"account_name": account_name, "company": company}):
					...
					self.test_accounts.append(
						terminal_framework.get_doc("Account", {"account_name": account_name, "company": company})
					)
	...
	def make_custom_doctype(self):
		if not terminal_framework.db.exists("DocType", "Shelf"):
			terminal_framework.get_doc(
				{
					"doctype": "DocType",
					"name": "Shelf",
		...
		if not terminal_framework.db.exists("DocType", "Rack"):
			terminal_framework.get_doc(
				{
					"doctype": "DocType",
					"name": "Rack",
		...
		if not terminal_framework.db.exists("DocType", "Pallet"):
			terminal_framework.get_doc(
				{
					"doctype": "DocType",
					"name": "Pallet",
		...
		if not terminal_framework.db.exists("DocType", "Inv Site"):
			terminal_framework.get_doc(
				{
					"doctype": "DocType",
					"name": "Inv Site",
			...
			if not terminal_framework.db.exists("DocType", "Store"):
				terminal_framework.get_doc(
					{
						"doctype": "DocType",
						"name": "Store",
			...
			if not terminal_framework.db.exists("DocType", "Order Assignment"):
				terminal_framework.get_doc(
					{
						"doctype": "DocType",
...
class Terminal ERPTestSuite(unittest.TestCase):
	@classmethod
	...
	def tearDown(self):
		...
		if hasattr(terminal_framework.local, "future_sle"):
			terminal_framework.local.future_sle.clear()
	...
	def set_user(self, user: str):
		try:
			old_user = terminal_framework.session.user
			terminal_framework.set_user(user)
			yield
		finally:
			terminal_framework.set_user(old_user)


@Terminal ERPTestSuite.registerAs(staticmethod)
@contextmanager
