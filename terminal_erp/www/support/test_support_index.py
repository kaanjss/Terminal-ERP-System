...
from terminal_erp.www.support.index import get_favorite_articles_by_page_view


class TestSupportIndex(Terminal ERPTestSuite):
	def make_help_category(self):
		category_name = "_Test Support Category " + random_string(8)
		category = terminal_framework.get_doc(
			{
	...
	def make_help_article(self, category, route, title, content, published=1):
		article = terminal_framework.get_doc(
			{
	...
	def seed_page_views(self, path, count):
		...
		for _ in range(count):
			view = terminal_framework.new_doc("Web Page View")
			view.path = path
			view.is_unique = "1"
			view.flags.name_set = True
			view.name = terminal_framework.generate_hash("wpv", 12)
			view.db_insert()
