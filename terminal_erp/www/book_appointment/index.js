terminal_framework.ready(async () => {
...
async function get_global_variables() {
	...
	window.appointment_settings = (
		await terminal_framework.call({
			method: "terminal_erp.www.book_appointment.index.get_appointment_settings",
		})
	).message;
	window.timezones = (
		await terminal_framework.call({
			method: "terminal_erp.www.book_appointment.index.get_timezones",
		})
	).message;
}
...
function hide_next_button() {
	...
	next_button.onclick = () => terminal_framework.msgprint(__("Please select a date and time"));
}
...
function on_date_or_timezone_select() {
	...
	if (date_picker.value === "") {
		clear_time_slots();
		hide_next_button();
		terminal_framework.throw(__("Please select a date"));
	}
...
async function get_time_slots(date, timezone) {
	let slots = (
		await terminal_framework.call({
			method: "terminal_erp.www.book_appointment.index.get_appointment_slots",
			args: {
				date: date,
				timezone: timezone,
			},
...
async function submit() {
	...
	let appointment = terminal_framework.call({
		method: "terminal_erp.www.book_appointment.index.create_appointment",
		args: {
		...
		callback: (response) => {
			if (response.message.status == "Unverified") {
				terminal_framework.show_alert(__("Please check your email to confirm the appointment"));
			} else {
				terminal_framework.show_alert(__("Appointment created successfully"));
			}
			setTimeout(() => {
				let redirect_url = "/";
		...
		error: (err) => {
			terminal_framework.show_alert(__("Something went wrong, please try again"));
			button.disabled = false;
		},
	});
}
