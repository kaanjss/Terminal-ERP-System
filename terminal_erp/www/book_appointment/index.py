...
from terminal_framework.utils.data import get_system_timezone
...
def handle_appointment_booking_disabled():
	if not terminal_framework.get_single_value("Appointment Booking Settings", "enable_scheduling"):
		terminal_framework.redirect_to_message(
			_("Appointment Scheduling Disabled"),
			_("Appointment Scheduling has been disabled for this site"),
			http_status_code=302,
			indicator_color="red",
		)
		raise terminal_framework.Redirect


@terminal_framework.whitelist(allow_guest=True)
def get_appointment_settings():
	handle_appointment_booking_disabled()
	settings = terminal_framework.get_single_value(
...
def get_appointment_slots(date: str, timezone: str):
	...
	holiday_list = terminal_framework.get_doc("Holiday List", settings.holiday_list)
...
def create_appointment(date: str, time: str, tz: str, contact: str | dict):
	...
	appointment = terminal_framework.new_doc("Appointment")
	appointment.scheduled_time = scheduled_time
	contact = terminal_framework.parse_json(contact)
...
def check_availabilty(timeslot, settings):
	return terminal_framework.db.count("Appointment", {"scheduled_time": timeslot}) < settings.number_of_agents
