...
from terminal_erp.stock.get_item_details import get_default_cost_center, get_default_expense_account
...
class BOMQuantityError(terminal_framework.ValidationError):
	pass
...
class SubcontractingReceipt(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.subcontracting.doctype.subcontracting_receipt_item.subcontracting_receipt_item import (
			SubcontractingReceiptItem,
		)
		from terminal_erp.subcontracting.doctype.subcontracting_receipt_supplied_item.subcontracting_receipt_supplied_item import (
	...
	def validate(self):
		...
		if getdate(self.posting_date) > getdate(nowdate()):
			terminal_framework.throw(_("Posting Date cannot be a future date"))

		super().validate()

		if self.is_new() and self.get("_action") == "save" and not terminal_framework.in_test:
	...
	def update_job_card(self):
		for row in self.get("items"):
			if row.job_card:
				doc = terminal_framework.get_doc("Job Card", row.job_card)
	...
	def set_service_expense_account(self, default_expense_account):
		for row in self.get("items"):
			if not row.service_expense_account and row.purchase_order_item:
				service_item = terminal_framework.db.get_value(
	...
	def get_manufactured_qty(self, job_card):
		table = terminal_framework.qb.DocType("Subcontracting Receipt Item")
		query = (
	...
	def set_items_bom(self):
		if self.is_return:
			for item in self.items:
				if not item.bom:
					item.bom = terminal_framework.db.get_value(
	...
	def get_secondary_items(self, recalculate_rate: bool | None = False):
		...
		for item in list(self.items):
			if item.bom:
				bom = terminal_framework.get_doc("BOM", item.bom)
	...
	def set_available_qty_for_consumption(self):
		...
		sco_supplied_item = terminal_framework.qb.DocType("Subcontracting Order Supplied Item")
		for item in self.get("items"):
			supplied_items = (
				terminal_framework.qb.from_(sco_supplied_item)
				.select(
		...
		for item in self.get("items") or []:
			...
			if item.bom:
				...
				item.amount = (
					flt(item.received_qty)
					* flt(item.rate)
					* (terminal_framework.get_value("BOM", item.bom, "cost_allocation_per") / 100)
				)
	...
	def validate_bom_required_qty(self):
		...
		) and not (terminal_framework.db.get_single_value("Buying Settings", "validate_consumed_qty")):
			return
		...
		for row in self.items:
			...
			is_allow_alternative_item = terminal_framework.db.get_value("BOM", row.bom, "allow_alternative_item")
			...
			for bom_item in self._get_materials_from_bom(
				...
				if diff < 0:
					msg = _(
						...
						terminal_framework.bold(abs(diff)),
						terminal_framework.bold(bom_item.stock_uom),
						terminal_framework.bold(bom_item.rm_item_code),
					)
	...
	def update_status(self, status=None, update_modified=False):
		if not status:
			...
			if self.is_return:
				terminal_framework.get_doc("Subcontracting Receipt", self.return_against).update_status(
	...
	def get_gl_entries(self, inventory_account_map=None):
		from terminal_erp.subcontracting.doctype.subcontracting_receipt.services.gl_composer import (
	...
	def auto_create_purchase_receipt(self):
		if terminal_framework.db.get_single_value("Buying Settings", "auto_create_purchase_receipt"):
