...
def make_purchase_receipt(
	...
	if isinstance(source_name, str):
		source_doc = terminal_framework.get_doc("Subcontracting Receipt", source_name)
	else:
		...
		target.update(
			{
				...
				"is_subcontracted": 1,
				"currency": terminal_framework.get_cached_value("Company", target.company, "default_currency"),
			}
		)
	...
	if (save or submit) and terminal_framework.has_permission(target_doc.doctype, "create"):
		...
		if submit and terminal_framework.has_permission(target_doc.doctype, "submit", target_doc):
			terminal_framework.db.savepoint("submit_subcontracting_receipt")
			try:
				target_doc.submit()
			except Exception as e:
				terminal_framework.db.rollback(save_point="submit_subcontracting_receipt")
...
def add_po_items_to_pr(scr_doc, target_doc):
	...
	for (item_code, po_name), fg_qty in fg_items.items():
		po_doc = terminal_framework.get_doc("Purchase Order", po_name)
