...
terminal_erp.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Receipt");

terminal_framework.ui.form.on("Subcontracting Receipt", {
	setup: (frm) => {
				...
				frm.add_custom_button(
					...
					() => {
						terminal_framework.model.open_mapped_doc({
							method: "terminal_erp.subcontracting.doctype.subcontracting_receipt.mapper.make_purchase_receipt",
							frm: frm,
							freeze: true,
			...
			frm.add_custom_button(
				...
				() => {
					const make_standard_return = () => {
						terminal_framework.model.open_mapped_doc({
							method: "terminal_erp.subcontracting.doctype.subcontracting_receipt.mapper.make_subcontract_return",
							frm: frm,
						});
					};
							...
							function (values) {
								if (values.return_for_rejected_warehouse) {
									terminal_framework.call({
										method: "terminal_erp.subcontracting.doctype.subcontracting_receipt.mapper.make_subcontract_return_against_rejected_warehouse",
										args: {
	...
	reset_raw_materials_table: (frm) => {
		...
		if (!frm.doc.set_posting_time) {
			frm.set_value("posting_time", terminal_framework.datetime.now_time());
		}
...
terminal_framework.ui.form.on("Subcontracting Receipt Item", {
	item_code(frm) {
		set_missing_values(frm);
	},
...
terminal_framework.ui.form.on("Subcontracting Receipt Supplied Item", {
	consumed_qty(frm) {
		set_missing_values(frm);
	},
