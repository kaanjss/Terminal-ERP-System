...
class SubcontractingBOM(Document):
	...
	def validate_finished_good(self):
		disabled, is_stock_item, default_bom, is_sub_contracted_item = terminal_framework.db.get_value(
			"Item",
			self.finished_good,
		...
		if disabled:
			terminal_framework.throw(_("Finished Good {0} is disabled.").format(terminal_framework.bold(self.finished_good)))
		if not is_stock_item:
			terminal_framework.throw(_("Finished Good {0} must be a stock item.").format(terminal_framework.bold(self.finished_good)))
		if not default_bom:
			terminal_framework.throw(
				_("Finished Good {0} does not have a default BOM.").format(terminal_framework.bold(self.finished_good))
			)
		if not is_sub_contracted_item:
			terminal_framework.throw(
				_("Finished Good {0} must be a sub-contracted item.").format(terminal_framework.bold(self.finished_good))
			)

	def validate_service_item(self):
		disabled, is_stock_item = terminal_framework.db.get_value(
			"Item", self.service_item, ["disabled", "is_stock_item"]
		)

		if disabled:
			terminal_framework.throw(_("Service Item {0} is disabled.").format(terminal_framework.bold(self.service_item)))
		if is_stock_item:
			terminal_framework.throw(
				_("Service Item {0} must be a non-stock item.").format(terminal_framework.bold(self.service_item))
			)

	def validate_is_active(self):
		if self.is_active:
			if sb := terminal_framework.db.exists(
				"Subcontracting BOM",
				...
				terminal_framework.throw(
					_("There is already an active Subcontracting BOM {0} for the Finished Good {1}.").format(
						terminal_framework.bold(sb), terminal_framework.bold(self.finished_good)
					)
				)
...
def get_subcontracting_boms_for_finished_goods(fg_items: str | list):
	if fg_items:
		...
		if subcontracting_boms := terminal_framework.get_all("Subcontracting BOM", filters=filters, fields=["*"]):
			if isinstance(fg_items, list):
...
def get_subcontracting_boms_for_service_item(service_item: str) -> dict:
	if service_item:
		...
		Subcontracting_boms = terminal_framework.db.get_all("Subcontracting BOM", filters=filters, fields=["*"])
