...
terminal_erp.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
terminal_framework.ui.form.on("Subcontracting Order Item", {
	qty(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		terminal_framework.model.set_value(cdt, cdn, "amount", row.qty * row.rate);
		...
		terminal_framework.model.set_value(service_item.doctype, service_item.name, "fg_item_qty", row.qty);
		terminal_framework.model.set_value(
			service_item.doctype,
			service_item.name,
			"amount",
...
terminal_framework.ui.form.on("Subcontracting Order", {
	setup: (frm) => {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", terminal_erp.queries.address_query);

		frm.set_query("billing_address", terminal_erp.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return terminal_erp.queries.company_address_query(frm.doc);
		});
	},
	...
	refresh: function (frm) {
		terminal_framework.dynamic_link = { doc: frm.doc, fieldname: "supplier", doctype: "Supplier" };
			...
			if (frm.doc.reserve_stock) {
				...
				frm.doc.supplied_items.forEach((item) => {
					if (
						...
						terminal_framework.model.can_read("Stock Reservation Entry")
					) {
						frm.add_custom_button(
							__("Reserved Stock"),
	...
	update_subcontracting_order_status(frm, status) {
		terminal_framework.call({
			method: "terminal_erp.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
			...
			const dialog = new terminal_framework.ui.Dialog({
				...
				primary_action: () => {
					...
					if (values.some((i) => !i.qty || i.qty == 0)) {
						terminal_framework.throw(__("Quantity is mandatory for the selected items."));
					} else {
...
terminal_erp.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt(items) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
			args: { items: items || [] },
			freeze: true,
...
extend_cscript(cur_frm.cscript, new terminal_erp.buying.SubcontractingOrderController({ frm: cur_frm }));
