...
from terminal_erp.buying.doctype.purchase_order.mapper import get_mapped_subcontracting_order
...
class TestSubcontractingOrder(Terminal ERPTestSuite):
	def setUp(self):
		make_subcontracted_items()
		make_raw_materials()
		make_service_items()
	...
	def test_update_status(self):
		...
		scr = make_subcontracting_receipt(sco.name)
		scr.save()
		self.assertRaises(terminal_framework.exceptions.ValidationError, scr.submit)
		sco.update_status()
		self.assertEqual(sco.status, "Partially Received")
		scr.cancel()
	...
	def test_sco_requires_a_subcontracting_purchase_order(self):
		...
		self.assertRaises(terminal_framework.ValidationError, sco.validate_purchase_order_for_subcontracting)

	def test_service_item_must_be_non_stock(self):
		sco = get_subcontracting_order(do_not_submit=1)
		sco.service_items[0].item_code = "_Test Item"  # a stock item
		self.assertRaises(terminal_framework.ValidationError, sco.validate_service_items)
	...
	def test_reserve_warehouse_must_differ_from_supplier_warehouse(self):
		...
		self.assertRaises(terminal_framework.ValidationError, sco.validate_supplied_items)

	def test_subcontracting_receipt_applies_bom_process_loss(self):
		sco = get_subcontracting_order()
		terminal_framework.db.set_value("BOM", sco.items[0].bom, "process_loss_percentage", 10)
	...
	def test_update_reserved_qty_for_subcontracting(self):
		...
		bin_before_sco = terminal_framework.db.get_value(
			"Bin",
		...
		bin_after_sco = terminal_framework.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname=["reserved_qty_for_sub_contract", "projected_qty", "modified"],
			as_dict=1,
		...
		ste = terminal_framework.get_doc(make_rm_stock_entry(sco.name, rm_items))
		ste.to_warehouse = "_Test Warehouse 1 - _TC"
		ste.save()
		ste.submit()

		bin_after_rm_transfer = terminal_framework.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
		...
		ste.cancel()
		bin_after_cancel_ste = terminal_framework.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract",
			as_dict=1,
		...
		sco.reload()
		sco.cancel()
		bin_after_cancel_sco = terminal_framework.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "_Test Item"},
			fieldname="reserved_qty_for_sub_contract",
			as_dict=1,
	...
	def test_send_to_subcontractor_ste_submit_without_sco_write_permission(self):
		...
		self.assertFalse(
			terminal_framework.has_permission("Subcontracting Order", "write", user=stock_user.name),
			"Precondition: the Stock-only user must not have Subcontracting Order write permission.",
		)

		terminal_framework.set_user(stock_user.name)
		try:
			ste.reload()
	...
	def test_exploded_items(self):
		...
		item_name = terminal_framework.db.get_value("BOM", {"item": item_code}, "name")
		bom = terminal_framework.get_doc("BOM", item_name)
	...
	def test_backflush_based_on_stock_entry(self):
		...
		transferred_rm_map = terminal_framework._dict()
		for item in rm_items:
	...
	def test_ordered_qty_for_subcontracting_order(self):
		...
		ordered_qty = terminal_framework.db.get_value(
			"Bin",
		...
		new_ordered_qty = terminal_framework.db.get_value(
			"Bin",
			filters={"warehouse": "_Test Warehouse - _TC", "item_code": "Subcontracted Item SA8"},
			fieldname="ordered_qty",
		)
	...
	@Terminal ERPTestSuite.change_settings("System Settings", {"float_precision": 3})
	...
	def test_stock_reservation(self):
		...
		se = terminal_framework.get_doc(se_dict)
		...
		for sre in sre_list:
			self.assertEqual(terminal_framework.get_value("Stock Reservation Entry", sre.name, "status"), "Closed")

		make_subcontracting_receipt(sco.name).submit()
		for status in terminal_framework.get_all(
	...
	def test_stock_reservation_transfer(self):
		...
		from terminal_erp.manufacturing.doctype.production_plan.test_production_plan import create_production_plan
		from terminal_erp.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
		...
		po = terminal_framework.get_doc(
			"Purchase Order",
			terminal_framework.get_value("Purchase Order Item", {"production_plan": plan.name}, "parent"),
		)
...
def create_subcontracting_order(**args):
	...
	if args.warehouse:
		...
		warehouse = terminal_framework.get_value("Purchase Order", args.po_name, "set_warehouse")
		if warehouse:
			for item in sco.items:
				item.warehouse = warehouse
		else:
			po = terminal_framework.get_doc("Purchase Order", args.po_name)
			warehouses = []
