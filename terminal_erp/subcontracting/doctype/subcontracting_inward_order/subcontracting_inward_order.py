...
from terminal_framework.utils import comma_and, flt, get_link_to_form
...
class SubcontractingInwardOrder(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.subcontracting.doctype.subcontracting_inward_order_item.subcontracting_inward_order_item import (
			SubcontractingInwardOrderItem,
		)
		from terminal_erp.subcontracting.doctype.subcontracting_inward_order_received_item.subcontracting_inward_order_received_item import (
			SubcontractingInwardOrderReceivedItem,
		)
		from terminal_erp.subcontracting.doctype.subcontracting_inward_order_secondary_item.subcontracting_inward_order_secondary_item import (
			SubcontractingInwardOrderSecondaryItem,
		)
		from terminal_erp.subcontracting.doctype.subcontracting_inward_order_service_item.subcontracting_inward_order_service_item import (
	...
	def update_subcontracted_quantity_in_so(self):
		for service_item in self.service_items:
			doc = terminal_framework.get_doc("Sales Order Item", service_item.sales_order_item)
	...
	def validate_customer_warehouse(self):
		if terminal_framework.get_cached_value("Warehouse", self.customer_warehouse, "customer") != self.customer:
			terminal_framework.throw(
				_("Customer Warehouse {0} does not belong to Customer {1}.").format(
					terminal_framework.bold(self.customer_warehouse), terminal_framework.bold(self.customer)
				)
			)
	...
	def populate_items_table(self):
		...
		for si in self.service_items:
			if si.fg_item:
				...
				so_item = terminal_framework.get_doc("Sales Order Item", si.sales_order_item)
				...
				si.fg_item_qty = flt(
					available_qty / conversion_factor, terminal_framework.get_precision("Sales Order Item", "qty")
				)
				...
				items.append(
					{
						...
						"expected_delivery_date": terminal_framework.get_cached_value(
	...
	def set_is_customer_provided_item(self):
		for item in self.get("received_items"):
			item.is_customer_provided_item = terminal_framework.get_cached_value(
	...
	def make_work_order(self):
		...
		if not wo_list:
			terminal_framework.msgprint(_("No Work Orders were created"))
	...
	def get_production_items(self):
		...
		for d in self.items:
			...
			item_details = {
				...
				"project": terminal_framework.get_cached_value("Sales Order", self.sales_order, "project"),
			...
			qty = min(
				int(qty) if terminal_framework.get_cached_value("UOM", d.stock_uom, "must_be_whole_number") else qty,
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		if doc_list:
			...
			terminal_framework.msgprint(_("{0} created").format(comma_and(doc_list)))
	...
	def make_rm_stock_entry_inward(self, target_doc: Document | str | None = None):
		def calculate_qty_as_per_bom(rm_item):
			...
			stock_qty = terminal_framework.get_value(
			...
			qty = flt(
				...
				terminal_framework.get_precision("Subcontracting Inward Order Received Item", "required_qty"),
			)
	...
	def make_subcontracting_delivery(self, target_doc: Document | str | None = None):
		...
		allow_over = terminal_framework.get_single_value("Selling Settings", "allow_delivery_of_overproduced_qty")
		...
		if (
			terminal_framework.get_single_value("Selling Settings", "deliver_secondary_items")
...
def set_subcontracting_inward_order_status(scio: str | Document, status: str | None = None):
	if isinstance(scio, str):
		scio = terminal_framework.get_doc("Subcontracting Inward Order", scio)
