terminal_framework.ui.form.on("Communication", {
	refresh: (frm) => {
	...
	setup_custom_buttons: (frm) => {
		...
		if (frm.doc.reference_doctype !== "Issue") {
			frm.add_custom_button(
				__("Issue"),
				() => {
					terminal_framework.confirm(__(confirm_msg, [__("Issue")]), () => {
		...
		if (!["Lead", "Opportunity"].includes(frm.doc.reference_doctype)) {
			frm.add_custom_button(
				__("Lead"),
				() => {
					terminal_framework.confirm(__(confirm_msg, [__("Lead")]), () => {
			...
			frm.add_custom_button(
				__("Opportunity"),
				() => {
					terminal_framework.confirm(__(confirm_msg, [__("Opportunity")]), () => {
	...
	make_lead_from_communication: (frm) => {
		return terminal_framework.call({
			method: "terminal_erp.crm.doctype.lead.mapper.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return terminal_framework.call({
			method: "terminal_erp.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
		...
		terminal_framework.prompt(
			...
			(data) => {
				terminal_framework.call({
					method: "terminal_erp.crm.doctype.opportunity.mapper.make_opportunity_from_communication",
					args: {
						communication: frm.doc.name,
