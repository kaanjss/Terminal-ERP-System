class BOMConfigurator {
	...
	make() {
		...
		terminal_framework.views.trees["BOM Configurator"] = new terminal_framework.views.TreeView(options);
		let node = terminal_framework.views.trees["BOM Configurator"].tree.root_node;
		terminal_framework.views.trees["BOM Configurator"].tree.show_toolbar(node);
		terminal_framework.views.trees["BOM Configurator"].tree.load_children(node, true);
		this.tree_view = terminal_framework.views.trees["BOM Configurator"];
	}

	bind_events() {
		terminal_framework.views.trees["BOM Configurator"].events = {
			frm: this.frm,
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "terminal_erp.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	tree_methods() {
		let frm_obj = this;
		let view = terminal_framework.views.trees["BOM Configurator"];
		...
		return {
			...
			onrender(node) {
				...
				amount = terminal_framework.format(amount, { fieldtype: "Currency", currency: frm_obj.frm.doc.currency });
			...
			toolbar:
				this.frm?.doc.docstatus === 0
					? [
							{
								label: __(terminal_framework.utils.icon("edit", "sm") + " BOM"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("add", "sm") + " Raw Material"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("add", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("add", "sm") + " Phantom Item"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("move", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("move", "sm") + " Phantom Item"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
							...
							{
								label: __(terminal_framework.utils.icon("delete", "sm") + " Item"),
								click: function (node) {
									let view = terminal_framework.views.trees["BOM Configurator"];
	...
	add_sub_assembly(node, view, phantom = false) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			if (dialog.operation && !dialog.workstation_type && !dialog.workstation) {
				terminal_framework.throw(__("Either Workstation or Workstation Type is mandatory"));
			}
				...
				...[
					{ fieldtype: "Section Break" },
					{
						...
						get_query() {
							...
							if (doc.routing) {
								return {
									query: "terminal_erp.manufacturing.doctype.routing.routing.get_operations",
									filters: {
										routing: doc.routing,
									},
								};
	...
	convert_to_sub_assembly(node, view, phantom = false) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			bom_item.items.forEach((d) => {
				if (!d.item_code) {
					terminal_framework.throw(__("Item is mandatory in Raw Materials table."));
				}
			});
	...
	delete_node(node, view) {
		terminal_framework.confirm(__("Are you sure you want to delete this Item?"), () => {
			terminal_framework.call({
				method: "delete_node",
				doc: this.frm.doc,
				args: {
	...
	edit_bom(node, view) {
		...
		this.frm.edit_bom_dialog = terminal_framework.prompt(
			fields,
			(data) => {
	...
	load_tree(response, node) {
		...
		if (!response?.message?.items) {
			terminal_framework.views.trees["BOM Configurator"].tree.load_children(node);
			return;
		}
		...
		while (node) {
			...
			total_amount = terminal_framework.format(total_amount, {
				fieldtype: "Currency",
...
terminal_framework.ui.BOMConfigurator = BOMConfigurator;
