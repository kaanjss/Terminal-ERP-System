...
function handle_route_event() {
	const route = terminal_framework.get_route();
...
function fetch_company_details(current_doctype, current_docname) {
	terminal_framework.call({
		method: "terminal_erp.controllers.accounts_controller.get_missing_company_details",
...
function save_company_details(dialog, data, values, current_doctype) {
	terminal_framework.call({
		method: "terminal_erp.controllers.accounts_controller.update_company_master_and_address",
		args: {
			name: data.name,
		...
		callback() {
			dialog.hide();
			terminal_framework.msgprint(__("Updating details."));
