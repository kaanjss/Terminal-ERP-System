terminal_framework.provide("terminal_erp.stock_reservation");

$.extend(terminal_erp.stock_reservation, {
	make_entries(frm, table_name) {
		terminal_erp.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = terminal_erp.stock_reservation.get_parms(frm, table_name);

		terminal_erp.stock_reservation.dialog = new terminal_framework.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: terminal_erp.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				terminal_erp.stock_reservation.reserve_stock(frm, table_name, parms);
			},
		});

		terminal_erp.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"terminal_erp.manufacturing.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"terminal_erp.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = terminal_erp.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = terminal_erp.stock_reservation.dialog;
		...
		let table_fields = [
			{ fieldtype: "Section Break" },
			{
				...
				fields: [
					{
						fieldname: terminal_framework.scrub(parms.child_doctype),
						fieldtype: "Link",
	...
	render_items(frm, parms) {
		...
		let dialog = terminal_erp.stock_reservation.dialog;
		let field = terminal_framework.scrub(parms.child_doctype);
	...
	unreserve_stock(frm) {
		terminal_erp.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					terminal_erp.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
					terminal_framework.msgprint(__("No reserved stock to unreserve."));
				}
			}
		});
	},
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: terminal_erp.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				terminal_erp.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "terminal_erp.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"terminal_erp.manufacturing.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
