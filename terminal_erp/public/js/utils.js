...
terminal_framework.provide("terminal_erp.utils");
terminal_framework.provide("terminal_erp.stock.utils");

$.extend(terminal_erp, {
	get_currency: function (company) {
		if (!company && cur_frm) company = cur_frm.doc.company;
		if (company)
			return terminal_framework.get_doc(":Company", company)?.default_currency || terminal_framework.boot.sysdefaults.currency;
		else return terminal_framework.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = terminal_framework.boot.docs;
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (terminal_erp.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", terminal_erp.last_selected_company);
			}
		}
	},
	...
	stale_rate_allowed: () => {
		return cint(terminal_framework.boot.sysdefaults.allow_stale);
	},
	...
	route_to_adjustment_jv: (args) => {
		terminal_framework.model.with_doctype("Journal Entry", () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = terminal_framework.model.get_new_doc("Journal Entry");

			args.accounts.forEach((je_account) => {
				let child_row = terminal_framework.model.add_child(journal_entry, "accounts");
			...
			terminal_framework.set_route("Form", "Journal Entry", journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		terminal_framework.set_route("List", "Repost Item Valuation", args);
	},
});

$.extend(terminal_erp.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					terminal_erp.utils.add_indicator_for_multicompany(frm, info);
				});
		...
		if (bundle_ids?.length) {
			frm.add_custom_button(
				...
				() => {
					...
					terminal_framework.set_route("query-report", "Serial and Batch Summary");
				},
				__("View")
			);
		}
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return terminal_framework.call({
				method: "terminal_erp.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		terminal_framework.new_doc("Bank Account", {
			party_type: doctype,
			party: docname,
		});
	},
	...
	add_dimensions: function (report_name, index) {
		let filters = terminal_framework.query_reports[report_name].filters;
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function (txt) {
								return terminal_framework.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		terminal_framework.call({
			method: "terminal_erp.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
							...
							filters.splice(index, 0, {
								...
								depends_on:
									report_name === "Stock Balance"
										...
										: "",
								get_data: function (txt) {
									return terminal_framework.db.get_link_options(dimension["doctype"], txt);
								},
							});
						} else {
	...
	make_subscription: function (doctype, docname) {
		terminal_framework.call({
			method: "terminal_framework.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_tree_options: function (option) {
		...
		let unscrub_option = terminal_framework.model.unscrub(option);
		let user_permission = terminal_framework.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function (option) {
		...
		if (options.includes(terminal_framework.defaults.get_default(option))) {
			return terminal_framework.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function (doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			terminal_framework.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		terminal_framework.model.with_doctype(doctype, function () {
			var new_doc = terminal_framework.model.get_new_doc(doctype);
	...
	check_payments_app: () => {
		if (terminal_framework.boot.versions && !terminal_framework.boot.versions.payments) {
			const marketplace_link =
				'<a href="https://terminal_frameworkcloud.com/marketplace/apps/payments">Marketplace</a>';
			const github_link = '<a href="https://github.com/terminal_framework/payments/">GitHub</a>';
	...
	pick_serial_and_batch_bundle(frm, cdt, cdn, type_of_transaction, warehouse_field) {
		let item_row = terminal_framework.get_doc(cdt, cdn);
		item_row.type_of_transaction = type_of_transaction;

		terminal_framework.db.get_value("Item", item_row.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		if (!terminal_framework.boot.setup_complete) {
			return;
		}
		const today = terminal_framework.datetime.get_today();
		if (!date) {
			date = today;
		}
		...
		let fiscal_year = "";
		if (
			terminal_framework.boot.current_fiscal_year &&
			date >= terminal_framework.boot.current_fiscal_year[1] &&
			date <= terminal_framework.boot.current_fiscal_year[2]
		) {
			if (with_dates) fiscal_year = terminal_framework.boot.current_fiscal_year;
			else fiscal_year = terminal_framework.boot.current_fiscal_year[0];
		} else if (today != date) {
			terminal_framework.call({
				method: "terminal_erp.accounts.utils.get_fiscal_year",
	...
	set_letter_head: function (frm) {
		if (frm.fields_dict.letter_head) {
			terminal_framework.db.get_value("Company", frm.doc.company, "default_letter_head").then((res) => {
...
terminal_erp.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
	...
	const dialog = new terminal_framework.ui.Dialog({
					...
					},
					{
						...
						onchange: function () {
							...
							if (item_code && warehouse) {
								terminal_framework.call({
									method: "terminal_erp.stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse,
									},
						...
						get_query: (e) => {
							return {
								query: "terminal_erp.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
		...
		primary_action: function () {
			...
			alternative_items.forEach((d) => {
				let row = terminal_framework.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				terminal_framework.model.set_value(row.doctype, row.name, "qty", qty);
				terminal_framework.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
terminal_erp.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	const cannot_add_row = typeof opts.cannot_add_row === "undefined" ? true : opts.cannot_add_row;
	const child_docname = typeof opts.cannot_add_row === "undefined" ? "items" : opts.child_docname;
	const child_meta = terminal_framework.get_meta(`${frm.doc.doctype} Item`);
	...
	let dialog = new terminal_framework.ui.Dialog({
		...
		update_items: function () {
			...
			terminal_framework.call({
				method: "terminal_erp.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
terminal_erp.utils.map_current_doc = function (opts) {
	function _map() {
		if ($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = terminal_framework.meta.get_docfield(cur_frm.doctype, "items").options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			terminal_framework.get_meta(items_doctype).fields.forEach(function (d) {
			...
			if (already_set) {
				opts.source_name.forEach(function (src) {
					terminal_framework.model.with_doc(opts.source_doctype, src, function (r) {
						var source_doc = terminal_framework.model.get_doc(opts.source_doctype, src);
		...
		return terminal_framework.call({
			...
			type: "POST",
			method: "terminal_framework.model.mapper.map_docs",
			args: {
				method: opts.method,
	...
	if (opts.source_doctype) {
		...
		if (["Purchase Receipt", "Delivery Note", "Purchase Invoice"].includes(opts.source_doctype)) {
			let target_meta = terminal_framework.get_meta(cur_frm.doc.doctype);
		...
		const d = new terminal_framework.ui.form.MultiSelectDialog({
			...
			action: function (selections, args) {
				...
				if (values.length === 0) {
					terminal_framework.msgprint(__("Please select {0}", [opts.source_doctype]));
					return;
				}
...
terminal_framework.form.link_formatters["Item"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "item_name");
};

terminal_framework.form.link_formatters["Employee"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "employee_name");
};

terminal_framework.form.link_formatters["Project"] = function (value, doc, df) {
...
$(document).on("app_ready", function () {
	if (!terminal_framework.datetime.is_timezone_same()) {
		$.each(
			[
				"Stock Reconciliation",
				"Stock Entry",
			...
			function (i, d) {
				terminal_framework.ui.form.on(d, "onload", function (frm) {
					cur_frm.set_df_property("posting_time", "description", terminal_framework.sys_defaults.time_zone);
				});
			}
		);
	}
...
// Show SLA dashboard
$(document).on("app_ready", function () {
	$.each(terminal_framework.boot.service_level_agreement_doctypes, function (_i, d) {
		terminal_framework.ui.form.on(d, {
			onload: function (frm) {
				if (!frm.doc.service_level_agreement) return;

				terminal_framework.call({
					method: "terminal_erp.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
function get_time_left(timestamp, agreement_status) {
	const diff = moment(timestamp).diff(terminal_framework.datetime.system_datetime(true));
...
$.extend(terminal_erp.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new terminal_erp.utils.BarcodeScanner({ frm: frm });
