terminal_framework.provide("terminal_erp.financial_statements");

function get_filter_value(filter_name) {
	// not warn when the filter is missing
	return terminal_framework.query_report.get_filter_value(filter_name, false);
}

terminal_erp.financial_statements = {
	filters: get_filters(),
	baseData: null,
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		if (terminal_erp.financial_statements.is_blank_row(data)) return "";

		const report_params = [value, row, column, data, default_formatter, filter];
		// Growth/Margin
		if (terminal_erp.financial_statements._is_special_view(column, data))
			return terminal_erp.financial_statements._format_special_view(...report_params);

		if (get_filter_value("report_template"))
			return terminal_erp.financial_statements._format_custom_report(...report_params);
		else return terminal_erp.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = terminal_erp.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = terminal_erp.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return terminal_erp.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return terminal_erp.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"terminal_erp.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return terminal_erp.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return terminal_erp.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == terminal_erp.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"terminal_erp.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	open_general_ledger: function (data) {
		...
		let filters = terminal_framework.query_report.filters;
		...
		filters.forEach((f) => {
			if (f.df.fieldtype == "MultiSelectList") {
				if (f.df.fieldname in terminal_framework.route_options) return;
				let value = f.get_value();
				if (value && value.length > 0) {
					terminal_framework.route_options[f.df.fieldname] = value;
				}
			}
		});
		...
		if (["Payable", "Receivable"].includes(data.account_type)) {
			...
			terminal_framework.route_options["party_account"] = data.account;
			terminal_framework.route_options["report_date"] = data.year_end_date;
		}

		terminal_framework.set_route("query-report", report);
	},
	tree: true,
	name_field: "account",
	...
	onload: function (report) {
		...
		terminal_erp.financial_statements.filters = get_filters();

		let fiscal_year = terminal_erp.utils.get_fiscal_year(terminal_framework.datetime.get_today());
		...
		if (report.page) {
			...
			report.page.add_custom_menu_item(views_menu, __("Balance Sheet"), function () {
				var filters = report.get_values();
				terminal_framework.set_route("query-report", "Balance Sheet", {
			...
			report.page.add_custom_menu_item(views_menu, __("Profit and Loss"), function () {
				var filters = report.get_values();
				terminal_framework.set_route("query-report", "Profit and Loss Statement", {
					company: filters.company,
					filter_based_on: filters.filter_based_on,
					period_start_date: filters.period_start_date,
					period_end_date: filters.period_end_date,
			...
			report.page.add_custom_menu_item(views_menu, __("Cash Flow Statement"), function () {
				var filters = report.get_values();
				terminal_framework.set_route("query-report", "Cash Flow", {
					company: filters.company,
					filter_based_on: filters.filter_based_on,
					period_start_date: filters.period_start_date,
					period_end_date: filters.period_end_date,
...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			on_change: function () {
				...
				terminal_framework.query_report.toggle_filter_display(
					"from_fiscal_year",
					filter_based_on === "Date Range"
				);
				terminal_framework.query_report.toggle_filter_display("to_fiscal_year", filter_based_on === "Date Range");
				terminal_framework.query_report.toggle_filter_display(
					"period_start_date",
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: terminal_erp.get_presentation_currency_list(),
	...
	let fiscal_year = terminal_erp.utils.get_fiscal_year(terminal_framework.datetime.get_today(), false, false);
	if (fiscal_year) {
