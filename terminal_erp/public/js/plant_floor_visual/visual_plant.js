class VisualPlantFloor {
	...
	prepare_data() {
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.workstation.workstation.get_workstations",
			args: {
	...
	add_filter() {
		this.plant_floor = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Plant Floor",
		...
		this.workstation_type = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Link",
		...
		this.workstation = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Workstation",
		...
		this.workstation_status = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Select",
				options: "\nProduction\nOff\nIdle\nProblem\nMaintenance\nSetup",
	...
	render_workstations() {
		...
		let template = terminal_framework.render_template("visual_plant_floor_template", {
	...
	update_status(data) {
		...
		if (data.status_image) {
			...
			workstation_card$
				.find(".workstation-image-container")
				.append(
					`<div class="workstation-image-cls workstation-abbr" style="margin:6px; height:82px">${terminal_framework.get_abbr(
						data.name,
						2
					)}</div>`
				);
...
terminal_framework.ui.VisualPlantFloor = VisualPlantFloor;
