...
// get tax rate
terminal_framework.provide("terminal_erp.taxes");

terminal_erp.accounts.taxes = {
			...
			onload: function (frm) {
				if (frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function (doc) {
						...
						return {
							query: "terminal_erp.controllers.queries.tax_account_query",
							filters: {
		...
		terminal_framework.ui.form.on(doctype, {
			account_head: function (frm, cdt, cdn) {
				...
				if (!d.charge_type && d.account_head) {
					terminal_framework.msgprint(__("Please select Charge Type first"));
					terminal_framework.model.set_value(cdt, cdn, "account_head", "");
				} else if (d.account_head) {
					terminal_framework.call({
						type: "GET",
						method: "terminal_erp.controllers.accounts_controller.get_tax_rate",
						args: { account_head: d.account_head },
						callback: function (r) {
							if (d.charge_type !== "Actual") {
								terminal_framework.model.set_value(cdt, cdn, "rate", r.message.tax_rate || 0);
							}
							terminal_framework.model.set_value(cdt, cdn, "description", r.message.account_name);
						},
					});
				}
			},
			...
			included_in_print_rate: function (frm, cdt, cdn) {
				let tax = terminal_framework.get_doc(cdt, cdn);
				try {
					me.validate_taxes_and_charges(cdt, cdn);
	...
	validate_inclusive_tax: function (tax, frm) {
		...
		if (cint(tax.included_in_print_rate)) {
			if (tax.charge_type == "Actual") {
				...
				terminal_framework.throw(__("Valuation type charges cannot be marked as Inclusive"));
			}
		}
	},
};

terminal_erp.accounts.payment_triggers = {
...
terminal_erp.accounts.pos = {
	setup: function (doctype) {
		terminal_framework.ui.form.on(doctype, {
			mode_of_payment: function (frm, cdt, cdn) {
				var d = locals[cdt][cdn];
				terminal_erp.accounts.pos.get_payment_mode_account(frm, d.mode_of_payment, function (account) {
					terminal_framework.model.set_value(cdt, cdn, "account", account);
				});
			},
		});
	},
	...
	get_payment_mode_account: function (frm, mode_of_payment, callback) {
		...
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
