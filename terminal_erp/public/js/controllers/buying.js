...
terminal_erp.buying = {
	setup_buying_controller: function () {
		terminal_erp.buying.BuyingController = class BuyingController extends terminal_erp.TransactionController {
			setup() {
				super.setup();
			...
			onload(doc, cdt, cdn) {
				...
				if (
					...
					terminal_framework.meta.has_field(this.frm.doc.doctype, "disable_rounded_total")
				) {
					var df = terminal_framework.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total");
					var disable = cint(df.default) || cint(terminal_framework.sys_defaults.disable_rounded_total);
				...
				if (this.frm.get_field("shipping_address")) {
					this.frm.set_query("shipping_address", () => {
						if (this.frm.doc.customer) {
							return {
								query: "terminal_framework.contacts.doctype.address.address.address_query",
								filters: { link_doctype: "Customer", link_name: this.frm.doc.customer },
							};
						} else return terminal_erp.queries.company_address_query(this.frm.doc);
					});
				}

				if (this.frm.get_field("dispatch_address")) {
					this.frm.set_query("dispatch_address", () => {
						return terminal_erp.queries.address_query(this.frm.doc);
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query("supplier", terminal_erp.queries.supplier);
				me.frm.set_query("contact_person", terminal_erp.queries.contact_query);
				me.frm.set_query("supplier_address", terminal_erp.queries.address_query);

				me.frm.set_query("billing_address", terminal_erp.queries.company_address_query);
				terminal_erp.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function (doc, cdt, cdn) {
					...
					return {
						query: "terminal_erp.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				terminal_erp.utils.get_party_details(this.frm, null, null, function () {
					me.apply_price_list();
				});
			}
			...
			company() {
				super.company();
				if (!terminal_framework.meta.has_field(this.frm.doc.doctype, "billing_address")) return;

				terminal_framework.call({
					method: "terminal_erp.setup.doctype.company.company.get_billing_shipping_address",
					args: {
					...
					callback: (r) => {
						...
						if (terminal_framework.meta.has_field(this.frm.doc.doctype, "shipping_address")) {
							this.frm.set_value("shipping_address", r.message.shipping_address || "");
						}
					},
				});
				terminal_erp.utils.set_letter_head(this.frm);
			}

			supplier_address() {
				terminal_erp.utils.get_address_display(this.frm);
				terminal_erp.utils.set_taxes_from_address(
					this.frm,
					"supplier_address",
					"supplier_address",
					"supplier_address"
			...
			calculate_received_qty(doc, cdt, cdn) {
				var item = terminal_framework.get_doc(cdt, cdn);
				terminal_framework.model.round_floats_in(item, ["qty", "rejected_qty"]);
				...
				terminal_framework.model.set_value(cdt, cdn, "received_qty", received_qty);
				terminal_framework.model.set_value(cdt, cdn, "received_stock_qty", received_stock_qty);
			}
					...
					if (item[fieldnames[i]] < 0) {
						terminal_framework.msgprint(
							__("Row #{0}: {1} can not be negative for item {2}", [
								item.idx,
								__(terminal_framework.meta.get_label(cdt, fieldnames[i], cdn)),
								item.item_code,
							])
						);
			...
			warehouse(doc, cdt, cdn) {
				...
				if (item.item_code && item.warehouse) {
					return this.frm.call({
						method: "terminal_erp.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(
					__("Product Bundle"),
					function () {
						terminal_erp.buying.get_items_from_product_bundle(me.frm);
					},
					__("Get Items From")
				);
			}
			...
			shipping_address() {
				var me = this;
				terminal_erp.utils.get_address_display(
					this.frm,
					"shipping_address",
			...
			update_auto_repeat_reference(doc) {
				if (doc.auto_repeat) {
					terminal_framework.call({
						method: "terminal_framework.automation.doctype.auto_repeat.auto_repeat.update_reference",
						args: {
			...
			manufacturer(doc, cdt, cdn) {
				...
				if (row.manufacturer) {
					terminal_framework.call({
						method: "terminal_erp.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
						...
						callback: function (r) {
							if (r.message) {
								terminal_framework.model.set_value(cdt, cdn, "manufacturer_part_no", r.message);
							}
						},
					});
				}
			...
			manufacturer_part_no(doc, cdt, cdn) {
				...
				if (row.manufacturer_part_no) {
					terminal_framework.model.get_value(
						"Item Manufacturer",
						{
							item_code: row.item_code,
							manufacturer: row.manufacturer,
			...
			add_serial_batch_bundle(doc, cdt, cdn) {
				...
				terminal_framework.db.get_value("Item", item.item_code, fields).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new terminal_erp.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
								...
								terminal_framework.model.set_value(item.doctype, item.name, update_values);
							}
						});
					}
				});
...
terminal_erp.buying.link_to_mrs = function (frm) {
	terminal_framework.call({
		method: "terminal_erp.buying.utils.get_linked_material_requests",
		args: {
			...
			for (let item of frm.doc.items) {
				...
				(r.message[0] || []).forEach(function (d) {
					if (
						...
						if (qty > 0) {
							terminal_framework.msgprint(__("Splitting {0} units of {1}", [qty, d.item_code]));
							var newrow = terminal_framework.model.add_child(frm.doc, item.doctype, "items");
							item_length++;
...
terminal_erp.buying.get_default_bom = function (frm) {
	$.each(frm.doc["items"] || [], function (i, d) {
		if (d.item_code && d.bom === "") {
			return terminal_framework.call({
				type: "GET",
				method: "terminal_erp.stock.get_item_details.get_default_bom",
				args: {
					item_code: d.item_code,
				},
				callback: function (r) {
					if (r) {
						terminal_framework.model.set_value(d.doctype, d.name, "bom", r.message);
					}
				},
			});
		}
...
terminal_erp.buying.get_items_from_product_bundle = function (frm) {
	var dialog = new terminal_framework.ui.Dialog({
		...
		primary_action(args) {
			...
			return terminal_framework.call({
				type: "GET",
				method: "terminal_erp.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
				...
				callback: function (r) {
					...
					if (!r.exc && r.message) {
						...
						for (var i = 0; i < r.message.length; i++) {
							...
							if (terminal_framework.meta.get_docfield(d.doctype, "price_list_rate", d.name)) {
...
terminal_erp.buying.prevent_past_schedule_dates = function (frm) {
