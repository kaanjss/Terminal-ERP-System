...
terminal_erp.taxes_and_totals = class TaxesAndTotals extends terminal_erp.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		terminal_framework.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (terminal_framework.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = terminal_framework.meta.get_label(
		...
		if (!this.frm.doc.conversion_rate) {
			if (this.frm.doc.currency == company_currency) {
				...
				terminal_framework.throw(err_message);
			}
		}
	}
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function (i, tax) {
			...
			if (!this.discount_amount_applied) {
				terminal_erp.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				terminal_erp.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
			terminal_framework.model.round_floats_in(tax);
		});
	}

	fetch_round_off_accounts() {
		let me = this;
		terminal_framework.flags.round_off_applicable_accounts = [];

		if (me.frm.doc.company) {
			terminal_framework.call({
				method: "terminal_erp.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				args: {
					company: me.frm.doc.company,
					account_list: terminal_framework.flags.round_off_applicable_accounts,
					// pass the doc so regional overrides can inspect it
					doc: me.frm.doc,
				},
				callback(r) {
					if (r.message) {
						terminal_framework.flags.round_off_applicable_accounts.push(...r.message);
					}
				},
			});
		}
		...
		terminal_framework.call({
			method: "terminal_erp.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function (r) {
				terminal_framework.flags.round_off_settings = r.message;
			},
		});
	}
	...
	calculate_net_total() {
		...
		terminal_framework.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
	}
	...
	calculate_shipping_charges() {
		...
		terminal_framework.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (terminal_framework.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	add_taxes_from_item_tax_template(item_tax_map) {
		...
		if (item_tax_map && cint(terminal_framework.defaults.get_default("add_taxes_from_item_tax_template"))) {
			...
			$.each(item_tax_map, function (tax, rate) {
				...
				if (!found) {
					let child = terminal_framework.model.add_child(me.frm.doc, "taxes");
	...
	calculate_taxes() {
		...
		$.each(this.frm._items || [], function (n, item) {
			...
			$.each(doc.taxes, function (i, tax) {
				...
				if (terminal_framework.flags.round_row_wise_tax) {
	...
	round_off_totals(tax) {
		if (terminal_framework.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	adjust_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = terminal_framework.utils.sum(
	...
	calculate_totals() {
		...
		if (
			...
			if (tax_count) {
				...
				terminal_framework.model.round_floats_in(this.frm.doc, [
		...
		terminal_framework.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if (terminal_framework.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (terminal_framework.sys_defaults.disable_rounded_total) {
			disable_rounded_total = terminal_framework.sys_defaults.disable_rounded_total;
		}

		if (terminal_framework.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if (items && items.length) {
			if (!terminal_framework.meta.get_docfield(items[0].doctype, "item_tax_amount", this.frm.doctype)) {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (
				!terminal_framework.meta.get_docfield(
	...
	set_discount_amount() {
		if (this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(
				(flt(this.frm.doc[terminal_framework.scrub(this.frm.doc.apply_discount_on)]) *
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if (!this.frm.doc.apply_discount_on) terminal_framework.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = terminal_framework.utils.sum(
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		terminal_framework.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if (["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
			...
			terminal_framework.model.round_floats_in(this.frm.doc, ["paid_amount"]);
	...
	async set_total_amount_to_default_mop() {
		...
		*/
		...
		if (this.frm.doc.return_against) {
			let { message: return_against_mop } = await terminal_framework.call({
				method: "terminal_erp.controllers.sales_and_purchase_return.get_payment_data",
				args: {
		...
		if (
			...
			$.each(this.frm.doc["payments"] || [], function (index, data) {
				if (data.default && payment_status && total_amount_to_pay > 0) {
					...
					terminal_framework.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					terminal_framework.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;
				} else if (me.frm.doc.paid_amount) {
					terminal_framework.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
