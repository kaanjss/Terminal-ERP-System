terminal_framework.provide("terminal_erp.setup");

terminal_framework.pages["setup-wizard"].on_page_load = function (wrapper) {
	if (terminal_framework.sys_defaults.company) {
		terminal_framework.set_route("desk");
		return;
	}
};

terminal_framework.setup.on("before_load", function () {
	if (
		terminal_framework.boot.setup_wizard_completed_apps?.length &&
		terminal_framework.boot.setup_wizard_completed_apps.includes("terminal_erp")
	) {
		return;
	}

	terminal_erp.setup.slides_settings.map(terminal_framework.setup.add_slide);
});

terminal_erp.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		validate: function () {
			...
			if ((this.values.company_name || "").toLowerCase() == "company") {
				terminal_framework.msgprint(__("Company Name cannot be Company"));
				return false;
			}
		...
		validate_fy_dates: function () {
			...
			if (invalid || start_greater_than_end) {
				terminal_framework.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = terminal_framework.wizard.values.country || terminal_framework.defaults.get_default("country");

			if (country) {
				let fy = terminal_erp.setup.fiscal_years[country];
				...
				if (year_start_date > terminal_framework.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		load_chart_of_accounts: function (slide) {
			let country = terminal_framework.wizard.values.country || terminal_framework.defaults.get_default("country");
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date = terminal_framework.datetime.add_days(terminal_framework.datetime.add_months(start_date, 12), -1);
			...
			slide
				...
				.val(terminal_framework.boot.sysdefaults.company_name || "")
				.trigger("change");
			...
			slide
				...
				.on("change", function () {
					...
					if (abbr.length > 10) {
						terminal_framework.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
						abbr = abbr.slice(0, 10);
					}
					slide.get_field("company_abbr").set_value(abbr);
				})
				.val(terminal_framework.boot.sysdefaults.company_abbr || "")
				.trigger("change");
		},
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new terminal_framework.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: "terminal_erp.accounts.utils.get_coa",
				args: {
			...
			const buttons = $(form_container).find(".terminal_framework-control");
...
terminal_erp.setup.fiscal_years = {
