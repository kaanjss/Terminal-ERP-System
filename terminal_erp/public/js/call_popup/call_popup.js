class CallPopup {
	...
	make() {
		terminal_framework.utils.play_sound("incoming-call");
		...
		terminal_framework.utils.bind_actions_with_object(this.dialog.$body, this);
	...
	close_modal() {
		this.dialog.hide();
		delete terminal_erp.call_popup;
	}

	call_ended(call_log, missed) {
		terminal_framework.utils.play_sound("call-disconnect");
	...
	setup_listener() {
		terminal_framework.realtime.on(`call_${this.call_log.id}_ended`, (call_log) => {
			this.call_ended(call_log);
		});

		terminal_framework.realtime.on(`call_${this.call_log.id}_missed`, (call_log) => {
			this.call_ended(call_log, true);
		});
	}

	clear_listeners() {
		terminal_framework.realtime.off(`call_${this.call_log.id}_ended`);
		terminal_framework.realtime.off(`call_${this.call_log.id}_missed`);
	}

	setup_call_details() {
		this.caller_info = $(`<div></div>`);
		this.call_details = new terminal_framework.ui.FieldGroup({
			fields: [
				{
					fieldname: "name",
					label: "Name",
				...
				},
				{
					...
					click: () => terminal_framework.set_route("Form", "Contact", this.get_contact_link().link_name),
				...
				},
				{
					...
					click: () => terminal_framework.new_doc("Lead", { mobile_no: this.caller_number }),
					depends_on: () => !this.get_caller_name(),
				},
				{
				...
				},
				{
					...
					click: () => {
						...
						terminal_framework
							.xcall(
								"terminal_erp.telephony.doctype.call_log.call_log.add_call_summary_and_call_type",
								{
	...
	create_new_customer() {
		...
		const new_customer = terminal_framework.model.get_new_doc("Customer");
		new_customer.mobile_no = this.caller_number;
		terminal_framework.set_route("Form", new_customer.doctype, new_customer.name);
	}

	create_new_contact() {
		// TODO: fix new_doc, it should accept child table values
		const new_contact = terminal_framework.model.get_new_doc("Contact");
		const phone_no = terminal_framework.model.add_child(new_contact, "Contact Phone", "phone_nos");
		phone_no.phone = this.caller_number;
		phone_no.is_primary_mobile_no = 1;
		terminal_framework.set_route("Form", new_contact.doctype, new_contact.name);
	}
}

$(document).on("app_ready", function () {
	terminal_framework.realtime.on("show_call_popup", (call_log) => {
		let call_popup = terminal_erp.call_popup;
		if (call_popup && call_log.name === call_popup.call_log.name) {
			call_popup.update_call_log(call_log);
			call_popup.dialog.show();
		} else {
			terminal_erp.call_popup = new CallPopup(call_log);
		}
	});
});
