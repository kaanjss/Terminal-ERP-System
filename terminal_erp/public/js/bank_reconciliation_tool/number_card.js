...
terminal_erp.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new terminal_erp.accounts.NumberCard(summary);
...
terminal_erp.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = terminal_framework.utils.build_summary_item(options);
	}
