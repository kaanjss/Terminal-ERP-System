terminal_erp.utils.CRMActivities = class CRMActivities {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		if (!$(this.all_activities_wrapper).find(".form-footer").length) {
			...
			// remove terminal_framework-control class to avoid absolute position for action-btn
			$(this.all_activities_wrapper).removeClass("terminal_framework-control");
		...
		terminal_framework.call({
			method: "terminal_erp.crm.utils.get_open_activities",
			args: {
			...
			callback: (r) => {
				if (!r.exc) {
					var activities_html = terminal_framework.render_template("crm_activities", {
	...
	create_task() {
		...
		let _create_task = () => {
			...
			let composer = new terminal_framework.views.InteractionComposer(args);
	...
	async update_status(input_field, doctype) {
		...
		if (completed) {
			await terminal_framework.db.set_value(doctype, docname, "status", "Closed");
			this.refresh();
		}
	}
};

terminal_erp.utils.CRMNotes = class CRMNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		let notes_html = terminal_framework.render_template("crm_notes", {
			notes: notes,
		});
