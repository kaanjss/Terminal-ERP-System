...
terminal_erp.sales_common = {
	setup_selling_controller: function () {
		terminal_erp.selling.SellingController = class SellingController extends terminal_erp.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], terminal_erp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", terminal_erp.queries.contact_query);
				me.frm.set_query("company_contact_person", terminal_erp.queries.company_contact_query);
				me.frm.set_query("customer_address", terminal_erp.queries.address_query);
				me.frm.set_query("shipping_address_name", terminal_erp.queries.address_query);
				me.frm.set_query("dispatch_address_name", terminal_erp.queries.dispatch_address_query);
				me.frm.set_query("company_address", terminal_erp.queries.company_address_query);
			...
			set_default_company_address() {
				if (!terminal_framework.meta.has_field(this.frm.doc.doctype, "company_address")) return;
				var me = this;
				if (this.frm.doc.company) {
					terminal_framework.call({
						method: "terminal_erp.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			set_default_company_contact_person() {
				if (!terminal_framework.meta.has_field(this.frm.doc.doctype, "company_contact_person")) {
					return;
				}
			...
			customer_address() {
				terminal_erp.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				terminal_erp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				terminal_erp.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				terminal_erp.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			allocated_percentage(doc, cdt, cdn) {
				var sales_person = terminal_framework.get_doc(cdt, cdn);
			...
			warehouse(doc, cdt, cdn) {
				if (doc.docstatus === 0 && doc.is_return && !doc.return_against) {
					terminal_framework.model.set_value(cdt, cdn, "incoming_rate", 0.0);
				}
			...
			set_actual_qty(doc, cdt, cdn) {
				...
				if (row.item_code && row.warehouse && sales_doctypes.includes(doc.doctype)) {
					terminal_framework.call({
						...
						callback(r) {
							if (r.message) {
								terminal_framework.model.set_value(cdt, cdn, "actual_qty", r.message.actual_qty);
							}
						},
					});
				}
			...
			toggle_editable_price_list_rate() {
				var df = terminal_framework.meta.get_docfield(
					this.frm.doc.doctype + " Item",
					"price_list_rate",
					this.frm.doc.name
				);
				var editable_price_list_rate = cint(terminal_framework.defaults.get_default("editable_price_list_rate"));

				if (df && editable_price_list_rate) {
					const parent_field = terminal_framework.meta.get_parentfield(
				...
				if (this.frm.doc.commission_rate > 100) {
					...
					terminal_framework.throw(
						__("{0} cannot be greater than 100", [
							__(
								terminal_framework.meta.get_label(
									this.frm.doc.doctype,
									"commission_rate",
									this.frm.doc.name
								)
			...
			calculate_contribution() {
				...
				$.each(this.frm.doc.doctype.sales_team || [], function (i, sales_person) {
					terminal_framework.model.round_floats_in(sales_person);
				...
				if ((doc.packed_items || []).length) {
					...
					if (["Delivery Note", "Sales Invoice"].includes(doc.doctype)) {
						var help_msg =
							...
							"</div>";
						terminal_framework.meta.get_docfield(doc.doctype, "product_bundle_help", doc.name).options =
							help_msg;
					}
				} else {
						...
						callback: function (r) {
							if (r.message) {
								me.frm.set_value(
									...
									terminal_framework.utils.html2text(r.message)
								);
							}
						},
					});
			...
			project(doc, cdt, cdn) {
				if (!cdt || !cdn) {
					if (this.frm.doc.project) {
						$.each(this.frm.doc["items"] || [], function (i, item) {
							if (!item.project) {
								terminal_framework.model.set_value(item.doctype, item.name, "project", doc.project);
							}
						});
					}
				} else {
					const item = terminal_framework.get_doc(cdt, cdn);
					if (item.project) {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						terminal_framework.call({
							method: "terminal_erp.projects.doctype.project.project.get_cost_center_name",
