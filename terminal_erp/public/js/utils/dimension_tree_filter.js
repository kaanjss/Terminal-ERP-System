...
terminal_erp.accounts.dimensions = {
	...
	setup_filters(frm, doctype) {
		...
		if (this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
				terminal_framework.model.with_doctype(dimension["document_type"], () => {
					let parent_fields = [];
					terminal_framework.meta.get_docfields(doctype).forEach((df) => {
						...
						if (terminal_framework.meta.has_field(doctype, dimension["fieldname"])) {
	...
	setup_child_filters(frm, doctype, parentfield, dimension) {
		...
		if (terminal_framework.meta.has_field(doctype, dimension)) {
			terminal_framework.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function (doc, cdt, cdn) {
					...
					return terminal_erp.queries.get_filtered_dimensions(row, fields, dimension, doc.company);
				});
			});
		}
	},
	...
	setup_account_filters(frm, dimension, fields) {
		frm.set_query(dimension, function (doc) {
			return terminal_erp.queries.get_filtered_dimensions(doc, fields, dimension, doc.company);
		});
	},
	...
	update_dimension(frm, doctype) {
		...
		this.accounting_dimensions.forEach((dimension) => {
			...
			(frm.doc.items || frm.doc.accounts || []).forEach((row) => {
				terminal_framework.model.set_value(row.doctype, row.name, dimension["fieldname"], default_dimension);
			});
		});
	},

	copy_dimension_from_first_row(frm, cdt, cdn, fieldname) {
		if (terminal_framework.meta.has_field(frm.doctype, fieldname) && this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
