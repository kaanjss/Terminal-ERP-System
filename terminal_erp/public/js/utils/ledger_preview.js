...
terminal_erp.accounts.ledger_preview = {
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.call({
						type: "GET",
						method: "terminal_erp.controllers.stock_controller.show_accounting_ledger_preview",
						args: {
		...
		if (!frm.is_new() && frm.doc.docstatus == 0) {
			frm.add_custom_button(
				...
				function () {
					terminal_framework.call({
						type: "GET",
						method: "terminal_erp.controllers.stock_controller.show_stock_ledger_preview",
						args: {
							company: frm.doc.company,
							doctype: frm.doc.doctype,
							docname: frm.doc.name,
	...
	make_dialog(label, fieldname, columns, data) {
		if (data.length === 0 && fieldname === "accounting_ledger_preview_html") {
			terminal_framework.msgprint("<strong>" + __("No Impact on Accounting Ledger") + "</strong>");
		} else {
			let me = this;
	...
	get_datatable(columns, data, wrapper) {
		...
		new terminal_framework.DataTable(wrapper, datatable_options);
	},
};
