...
terminal_erp.accounts.unreconcile_payment = {
	add_unreconcile_btn(frm) {
		if (frm.doc.docstatus == 1) {
			...
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.unreconcile_payment.unreconcile_payment.doc_has_references",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						frm.add_custom_button(
							__("UnReconcile"),
							function () {
								terminal_erp.accounts.unreconcile_payment.build_unreconcile_dialog(frm);
							},
							__("Actions")
						);
					}
	...
	build_unreconcile_dialog(frm) {
		if (
			...
			// get linked payments
			terminal_framework.call({
				method: "terminal_erp.accounts.doctype.unreconcile_payment.unreconcile_payment.get_linked_payments_for_doc",
				args: {
						...
						let d = new terminal_framework.ui.Dialog({
							...
							primary_action(values) {
								...
								if (selected_allocations.length > 0) {
									let selection_map =
										terminal_erp.accounts.unreconcile_payment.build_selection_map(
											frm,
											selected_allocations
										);
									terminal_erp.accounts.unreconcile_payment.create_unreconcile_docs(
										selection_map
									);
									d.hide();
								} else {
									terminal_framework.msgprint(__("No Selection"));
								}
							},
						});
	...
	create_unreconcile_docs(selection_map) {
		terminal_framework.call({
			method: "terminal_erp.accounts.doctype.unreconcile_payment.unreconcile_payment.create_unreconcile_doc_for_selection",
			args: {
