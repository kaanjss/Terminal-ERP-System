terminal_erp.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		this.frm = opts.frm;
		// terminal_framework.flags.trigger_from_barcode_scanner is used for custom scripts
		...
		// see https://terminal_frameworkframework.com/docs/v14/user/en/python-api/hooks#sounds
		...
		// }
		this.scan_api = opts.scan_api || "terminal_erp.stock.utils.scan_barcode";
		this.has_last_scanned_warehouse = terminal_framework.meta.has_field(this.frm.doctype, "last_scanned_warehouse");
	}
	...
	update_table(data) {
		return new Promise((resolve, reject) => {
			...
			terminal_framework.flags.trigger_from_barcode_scanner = true;
			...
			if (!row) {
				...
				row = terminal_framework.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
	...
	revert_selector_flag() {
		...
		terminal_framework.flags.trigger_from_barcode_scanner = false;
	}
	...
	set_item(row, item_code, barcode, batch_no, serial_no) {
		return new Promise((resolve) => {
			const increment = async (value = 1) => {
				...
				await terminal_framework.model.set_value(row.doctype, row.name, item_data);
				return value;
			};

			if (this.prompt_qty) {
				terminal_framework.prompt(__("Please enter quantity for item {0}", [item_code]), ({ value }) => {
	...
	prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no) {
		var me = this;
		this.dialog = new terminal_framework.ui.Dialog({
		...
		this.dialog.set_primary_action(__("Update"), () => {
			...
			terminal_framework.model.set_value(row.doctype, row.name, item_data);
	...
	validate_duplicate_serial_no(serial_no) {
		...
		if (serial_nos.includes(serial_no)) {
			terminal_framework.throw(__("Serial No {0} already scanned", [serial_no]));
		}
	}
	...
	add_child_for_remaining_qty(prev_row) {
		if (this.remaining_qty && this.remaining_qty > 0) {
			let cur_grid = this.frm.fields_dict[this.items_table_name].grid;
			let row = terminal_framework.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
			...
			if (this.qty_field == "qty" && terminal_framework.meta.has_field(row.doctype, "stock_qty")) {
	...
	async set_serial_no(row, serial_no) {
		if (serial_no && terminal_framework.meta.has_field(row.doctype, this.serial_no_field)) {
			...
			await terminal_framework.model.set_value(row.doctype, row.name, this.serial_no_field, new_serial_nos);
		}
	}

	async set_barcode_uom(row, uom) {
		if (uom && terminal_framework.meta.has_field(row.doctype, this.uom_field)) {
			await terminal_framework.model.set_value(row.doctype, row.name, this.uom_field, uom);
		}
	}

	async set_batch_no(row, batch_no) {
		if (batch_no && terminal_framework.meta.has_field(row.doctype, this.batch_no_field)) {
			await terminal_framework.model.set_value(row.doctype, row.name, this.batch_no_field, batch_no);
		}
	}

	async set_barcode(row, barcode) {
		if (barcode && terminal_framework.meta.has_field(row.doctype, this.barcode_field)) {
			await terminal_framework.model.set_value(row.doctype, row.name, this.barcode_field, barcode);
		}
	}
	...
	async set_warehouse(row) {
		...
		if (!warehouse_field || !terminal_framework.meta.has_field(row.doctype, warehouse_field)) return;

		await terminal_framework.model.set_value(row.doctype, row.name, warehouse_field, last_scanned_warehouse);
	}
	...
	get_row_to_modify_on_scan(item_code, batch_no, uom, barcode, default_warehouse) {
		...
		let is_batch_no_scan = batch_no && terminal_framework.meta.has_field(cur_grid.doctype, this.batch_no_field);
		let check_max_qty = this.max_qty_field && terminal_framework.meta.has_field(cur_grid.doctype, this.max_qty_field);

		const warehouse_field = this.has_last_scanned_warehouse && this.get_warehouse_field();
		const has_warehouse_field =
			warehouse_field && terminal_framework.meta.has_field(cur_grid.doctype, warehouse_field);
	...
	setup_last_scanned_warehouse() {
		...
		this.frm.set_df_property("last_scanned_warehouse", "formatter", function (value, df, options, doc) {
			const link_formatter = terminal_framework.form.get_formatter(df.fieldtype);
			...
			const clear_btn = `
				<a class="btn-clear-last-scanned-warehouse" title="${__("Clear Last Scanned Warehouse")}">
					${terminal_framework.utils.icon("close", "xs", "es-icon")}
				</a>
			`;
	...
	handle_warehouse_scan(data) {
		...
		const warehouse_field_label = terminal_framework.meta.get_label(cur_grid.doctype, warehouse_field);
	...
	play_success_sound() {
		this.success_sound && terminal_framework.utils.play_sound(this.success_sound);
	}

	play_fail_sound() {
		this.fail_sound && terminal_framework.utils.play_sound(this.fail_sound);
	}
	...
	show_alert(msg, indicator, duration = 3) {
		terminal_framework.show_alert({ message: msg, indicator: indicator }, duration);
	}
};
