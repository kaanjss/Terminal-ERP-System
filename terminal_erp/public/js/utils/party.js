...
terminal_erp.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (terminal_framework.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!terminal_erp.utils.validate_mandatory(
				frm,
...
terminal_erp.utils.add_item = function (frm) {
	if (frm.is_new()) {
		var prev_route = terminal_framework.get_prev_route();
		if (prev_route[1] === "Item" && !(frm.doc.items && frm.doc.items.length)) {
			...
			// set item
			terminal_framework.model.set_value(item.doctype, item.name, "item_code", prev_route[2]);
		}
	}
};

terminal_erp.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
terminal_erp.utils.set_taxes_from_address = function (
	frm,
	...
	terminal_framework.call({
		method: "terminal_erp.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					terminal_erp.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
terminal_erp.utils.set_taxes = function (frm, triggered_from_field) {
	if (terminal_framework.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!terminal_erp.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	if (!frm.doc.company) {
		terminal_framework.throw(__("Kindly select the company first"));
	}

	terminal_framework.call({
		method: "terminal_erp.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
terminal_erp.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
terminal_erp.utils.get_employee_contact_details = function (frm) {
	...
	terminal_framework.call({
		method: "terminal_erp.setup.doctype.employee.employee.get_contact_details",
...
terminal_erp.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
		refresh_field(trigger_on);
		terminal_framework.throw({ message: __("Please enter {0} first", [label]), title: __("Mandatory") });
		return false;
	}
	return true;
};

terminal_erp.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		terminal_framework.call({
			method: "terminal_erp.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
		...
		terminal_framework.msgprint(__("Select company first"));
	}
};
