...
def validate_company_region(filters):
	if filters.get("company") and get_region(filters.get("company")) != "United Arab Emirates":
		terminal_framework.throw(
			...
			).format(terminal_framework.bold(filters.get("company")))
		)
...
def append_vat_on_sales(data, filters):
	...
	append_data(
		...
		terminal_framework.format((-1) * get_tourist_tax_return_total(filters), "Currency"),
		terminal_framework.format((-1) * get_tourist_tax_return_tax(filters), "Currency"),
	)
	...
	append_data(
		...
		terminal_framework.format(get_reverse_charge_total(filters), "Currency"),
		terminal_framework.format(get_reverse_charge_tax(filters), "Currency"),
	)

	append_data(data, "4", _("Zero Rated"), terminal_framework.format(get_zero_rated_total(filters), "Currency"), "-")

	append_data(data, "5", _("Exempt Supplies"), terminal_framework.format(get_exempt_total(filters), "Currency"), "-")
...
def standard_rated_expenses_emiratewise(data, filters):
	...
	for emirate, amount, vat in total_emiratewise:
		amounts_by_emirate[emirate] = {
			...
			"raw_amount": amount,
			"raw_vat_amount": vat,
			"amount": terminal_framework.format(amount, "Currency"),
			"vat_amount": terminal_framework.format(vat, "Currency"),
		}
	...
	for no, emirate in enumerate(emirates, 97):
		if emirate in amounts_by_emirate:
			...
			append_data(
				...
				terminal_framework.format(0, "Currency"),
				terminal_framework.format(0, "Currency"),
			)
...
def append_vat_on_expenses(data, filters):
	...
	append_data(
		...
		terminal_framework.format(get_standard_rated_expenses_total(filters), "Currency"),
		terminal_framework.format(get_standard_rated_expenses_tax(filters), "Currency"),
	)
	append_data(
		data,
		"10",
		_("Supplies subject to the reverse charge provision"),
		terminal_framework.format(get_reverse_charge_recoverable_total(filters), "Currency"),
		terminal_framework.format(get_reverse_charge_recoverable_tax(filters), "Currency"),
	)
...
def get_total_emiratewise(filters):
	...
	i = terminal_framework.qb.DocType("Sales Invoice Item")
	s = terminal_framework.qb.DocType("Sales Invoice")
	query = (
		terminal_framework.qb.from_(i)
		.inner_join(s)
		.on(i.parent == s.name)
...
def get_reverse_charge_tax(filters):
	...
	p = terminal_framework.qb.DocType("Purchase Invoice")
	gl = terminal_framework.qb.DocType("GL Entry")
	uae_vat = terminal_framework.qb.DocType("UAE VAT Account")
	query = (
		...
		.where(
			...
			& (gl.docstatus == 1)
			& gl.account.isin(
				terminal_framework.qb.from_(uae_vat)
