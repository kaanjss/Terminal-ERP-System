...
from terminal_erp.regional.report.uae_vat_201.uae_vat_201 import (
	execute,
	get_exempt_total,
...
from terminal_erp.stock.doctype.warehouse.test_warehouse import get_warehouse_account
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestUaeVat201(Terminal ERPTestSuite):
	def setUp(self):
		set_vat_accounts()
...
def set_vat_accounts():
	if not terminal_framework.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
...
def make_customer():
	if not terminal_framework.db.exists("Customer", "_Test UAE Customer"):
...
def make_supplier():
	if not terminal_framework.db.exists("Supplier", "_Test UAE Supplier"):
		terminal_framework.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not terminal_framework.db.exists("Warehouse", warehouse_id):
		warehouse = terminal_framework.new_doc("Warehouse")
