...
terminal_framework.query_reports["Electronic Invoice Register"] = {
	filters: [
		{
			fieldname: "from_date",
			label: __("From Date"),
	...
	onload: function (reportview) {
		reportview.page.add_inner_button(__("Export E-Invoices"), function () {
			...
			if (!reportview.data.length) {
				terminal_framework.msgprint(__("No data to export"));
				return;
			}

			var w = window.open(
				terminal_framework.urllib.get_full_url(
					"/api/method/terminal_erp.regional.italy.utils.export_invoices?" +
						"filters=" +
