...
from terminal_erp.regional.report.vat_audit_report.vat_audit_report import execute
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestVATAuditReport(Terminal ERPTestSuite):
	def setUp(self):
...
def make_company(company_name, abbr):
	if not terminal_framework.db.exists("Company", company_name):
		...
		company = terminal_framework.get_doc("Company", company_name)

	company.create_default_warehouses()

	if not terminal_framework.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_sa_vat_accounts():
	if not terminal_framework.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
		vat_accounts = terminal_framework.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not terminal_framework.db.exists("Customer", "_Test SA Customer"):
		terminal_framework.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not terminal_framework.db.exists("Supplier", "_Test SA Supplier"):
		terminal_framework.get_doc(
			{
				"doctype": "Supplier",
