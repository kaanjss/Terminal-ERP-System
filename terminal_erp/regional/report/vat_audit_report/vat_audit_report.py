...
from terminal_framework.utils import formatdate, get_link_to_form

from terminal_erp import get_region
from terminal_erp.accounts.report.item_wise_sales_register.item_wise_sales_register import get_tax_details_query
...
class VATAuditReport:
	...
	def validate_company_region(self):
		if self.filters.company and get_region(self.filters.company) != "South Africa":
			terminal_framework.throw(
				...
				).format(terminal_framework.bold(self.filters.company))
			)

	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = terminal_framework.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not terminal_framework.in_test and not terminal_framework.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			terminal_framework.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))

	def get_invoice_data(self, doctype):
		self.invoices = terminal_framework._dict()
		invoice_doctype = terminal_framework.qb.DocType(doctype)
		...
		query = (
			terminal_framework.qb.from_(invoice_doctype)
			.select(
			...
			.orderby(invoice_doctype.posting_date, order=terminal_framework.qb.desc)
		)
	...
	def get_invoice_items(self, doctype):
		self.invoice_items = terminal_framework._dict()
		item_doctype = terminal_framework.qb.DocType(doctype + " Item")
		self.invoice_items = terminal_framework._dict(
			terminal_framework.qb.from_(item_doctype)
			.select(
				item_doctype.name,
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = terminal_framework._dict()
		self.tax_doctype = (
			"Purchase Taxes and Charges" if doctype == "Purchase Invoice" else "Sales Taxes and Charges"
		)

		taxes_and_charges = terminal_framework.qb.DocType(self.tax_doctype)
		item_wise_tax = terminal_framework.qb.DocType("Item Wise Tax Detail")
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = terminal_framework.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": terminal_framework.bold(_("Total")),
