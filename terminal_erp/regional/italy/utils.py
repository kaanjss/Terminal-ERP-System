...
from terminal_framework.utils.file_manager import remove_file

from terminal_erp.controllers.taxes_and_totals import get_itemised_tax
from terminal_erp.regional.italy import state_codes
from terminal_erp.stock.utils import get_default_stock_uom
...
def export_invoices(filters: str | None = None):
	terminal_framework.has_permission("Sales Invoice", throw=True)
	...
	zip_filename = "{}-einvoices.zip".format(terminal_framework.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = terminal_framework.get_doc("Company", invoice.company)
	...
	company_address = terminal_framework.get_doc("Address", invoice.company_address)
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended = get_unamended_name(
				terminal_framework.get_doc("Sales Invoice", invoice.return_against)
			)
		else:
			invoice.type_of_document = "TD01"  # Sales Invoice (Fattura)

	# set customer information
	invoice.customer_data = terminal_framework.get_doc("Customer", invoice.customer)
	customer_address = terminal_framework.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = terminal_framework.get_doc("Address", invoice.shipping_address_name)
...
def download_zip(files, output_filename):
	...
	with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zip_file:
		for file in files:
			file_path = terminal_framework.utils.get_files_path(file.file_name, is_private=file.is_private)

			zip_file.write(file_path, arcname=file.file_name)

	terminal_framework.local.response.filename = output_filename
	terminal_framework.local.response.filecontent = zip_stream.getvalue()
	terminal_framework.local.response.type = "download"
	zip_stream.close()


def get_invoice_summary(items, taxes, item_wise_tax_details):
	summary_data = terminal_framework._dict()
...
def sales_invoice_validate(doc):
	...
	company_fiscal_regime = terminal_framework.get_cached_value("Company", doc.company, "fiscal_regime")
	...
	doc.company_tax_id = terminal_framework.get_cached_value("Company", doc.company, "tax_id")
	doc.company_fiscal_code = terminal_framework.get_cached_value("Company", doc.company, "fiscal_code")
	...
	customer = terminal_framework.get_doc("Customer", doc.customer)
	...
	if not doc.customer_address:
		terminal_framework.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
	...
	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = terminal_framework.get_cached_value(
...
def sales_invoice_on_submit(doc, method):
	...
	for schedule in doc.payment_schedule:
		...
		elif not terminal_framework.db.get_value("Mode of Payment", schedule.mode_of_payment, "mode_of_payment_code"):
			terminal_framework.throw(
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	item_meta = terminal_framework.get_meta("Sales Invoice Item")

	invoice_xml = terminal_framework.render_template(
		"terminal_erp/regional/italy/e-invoice.xml",
...
def generate_single_invoice(docname: str):
	doc = terminal_framework.get_doc("Sales Invoice", docname)
	terminal_framework.has_permission("Sales Invoice", doc=doc, throw=True)
...
def get_e_invoice_attachments(invoices):
	...
	attachments = terminal_framework.get_all(
		"File",
...
def validate_address(address_name):
	...
	data = terminal_framework.get_cached_value("Address", address_name, fields, as_dict=1) or {}
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = terminal_framework.model.naming.make_autoname(company_tax_id + "_.#####")
