...
from terminal_erp.regional.address_template.setup import get_address_templates, update_address_template
from terminal_erp.tests.utils import Terminal ERPTestSuite


def ensure_country(country):
	if terminal_framework.db.exists("Country", country):
		return terminal_framework.get_doc("Country", country)
	else:
		c = terminal_framework.get_doc({"doctype": "Country", "country_name": country})
		c.insert()
		return c


class TestRegionalAddressTemplate(Terminal ERPTestSuite):
	...
	def test_create_address_template(self):
		...
		doc = terminal_framework.get_doc("Address Template", country.name)
		self.assertEqual(doc.template, "TEST")

	def test_update_address_template(self):
		"""Update an existing Address Template."""
		country = ensure_country("Germany")
		if not terminal_framework.db.exists("Address Template", country.name):
