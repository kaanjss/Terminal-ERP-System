...
def update_address_template(country, html, is_default=0):
	...
	if not terminal_framework.db.exists("Country", country):
		terminal_framework.log_error(f"Country {country} for regional Address Template does not exist.")
		return

	if terminal_framework.db.exists("Address Template", country):
		terminal_framework.db.set_value("Address Template", country, "template", html)
		terminal_framework.db.set_value("Address Template", country, "is_default", is_default)
	else:
