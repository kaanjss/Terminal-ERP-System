...
from terminal_framework.utils import flt, get_datetime_str, today
from terminal_framework.utils.data import format_datetime
...
class ImportSupplierInvoice(Document):
	...
	def validate(self):
		if not terminal_framework.db.get_value("Stock Settings", fieldname="stock_uom"):
			terminal_framework.throw(_("Please set default UOM in Stock Settings"))
	...
	def import_xml_data(self):
		zip_file = terminal_framework.get_doc(
			"File",
		...
		self.default_uom = terminal_framework.db.get_value("Stock Settings", fieldname="stock_uom")
	...
	def prepare_data_for_import(self, file_content, file_name, encoded_content):
		for line in file_content.find_all("DatiGeneraliDocumento"):
			...
			if not invoices_args.get("bill_no", ""):
				terminal_framework.throw(_("Numero has not been set in the XML file"))
			...
			terminal_framework.db.savepoint("import_invoice")
			try:
				pi_name = create_purchase_invoice(supplier_name, file_name, invoices_args, self.name)
			except Exception:
				terminal_framework.db.rollback(save_point="import_invoice")
			...
			file_doc = terminal_framework.new_doc("File")
	...
	def process_file_data(self):
		...
		terminal_framework.enqueue_doc(self.doctype, self.name, "import_xml_data", queue="long", timeout=3600)
...
def get_file_content(file_name, zip_file_object):
	...
	try:
		...
		try:
			...
			terminal_framework.log_error("UTF-16 encoding error for File Name: " + file_name)
...
def get_payment_terms_from_file(file_content):
	...
	mop_options = terminal_framework.get_meta("Mode of Payment").fields[4].options
	mop_str = re.sub("\n", ",", mop_options)
...
def create_supplier(supplier_group, args):
	...
	existing_supplier_name = terminal_framework.db.get_value(
	...
	if existing_supplier_name:
		...
		if not terminal_framework.get_list("Contact", filters):
		...
		new_supplier = terminal_framework.new_doc("Supplier")
...
def create_address(supplier_name, args):
	...
	existing_address = terminal_framework.get_list("Address", filters)
	...
	if args.address_line1:
		...
		for address in existing_address:
			address_doc = terminal_framework.get_doc("Address", address["name"])
			if (
	...
	pi = terminal_framework.get_doc(
		{
			...
			"currency": terminal_erp.get_company_currency(args.company),
...
def get_country(code):
	existing_country_name = terminal_framework.db.get_value("Country", filters={"code": code}, fieldname="name")
	if existing_country_name:
		return existing_country_name
	else:
		terminal_framework.throw(_("Country Code in File does not match with country code set up in the system"))


def create_uom(uom):
	existing_uom = terminal_framework.db.get_value("UOM", filters={"uom_name": uom}, fieldname="uom_name")
	if existing_uom:
		return existing_uom
	else:
		new_uom = terminal_framework.new_doc("UOM")
