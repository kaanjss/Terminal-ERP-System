...
from terminal_framework.utils import get_link_to_form, getdate
...
class LowerDeductionCertificate(Document):
	...
	def validate_dates(self):
		if getdate(self.valid_upto) < getdate(self.valid_from):
			terminal_framework.throw(_("Valid Up To date cannot be before Valid From date"))

		fiscal_year = get_fiscal_year(fiscal_year=self.fiscal_year, as_dict=True)

		if not (fiscal_year.year_start_date <= getdate(self.valid_from) <= fiscal_year.year_end_date):
			terminal_framework.throw(_("Valid From date not in Fiscal Year {0}").format(terminal_framework.bold(self.fiscal_year)))

		if not (fiscal_year.year_start_date <= getdate(self.valid_upto) <= fiscal_year.year_end_date):
			terminal_framework.throw(_("Valid Up To date not in Fiscal Year {0}").format(terminal_framework.bold(self.fiscal_year)))

	def validate_supplier_against_tax_category(self):
		duplicate_certificate = terminal_framework.db.get_value(
		...
		if duplicate_certificate and self.are_dates_overlapping(duplicate_certificate):
			...
			terminal_framework.throw(
				_(
					...
					certificate_link, terminal_framework.bold(self.supplier), terminal_framework.bold(self.tax_withholding_category)
				)
			)
