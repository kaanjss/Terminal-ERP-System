...
class TestWorkOrderStockReport(Terminal ERPTestSuite):
	def test_report_executes_and_lists_work_order(self):
		...
		from terminal_erp.manufacturing.report.work_order_stock_report.work_order_stock_report import execute

		wo = make_wo_order_test_record(
			production_item="_Test FG Item", qty=1, source_warehouse="_Test Warehouse - _TC"
		)

		columns, data = execute(terminal_framework._dict(warehouse="_Test Warehouse - _TC"))
