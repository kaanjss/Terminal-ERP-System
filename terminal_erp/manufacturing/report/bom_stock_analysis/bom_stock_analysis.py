...
from terminal_framework.query_builder.functions import Floor, IfNull, Max, Min, Sum
...
def fmt_qty(value):
	...
	return terminal_framework.utils.fmt_money(value, precision=2, currency=None)


def fmt_rate(value):
	"""Format a currency rate for display as a string."""
	currency = terminal_framework.defaults.get_global_default("currency")
	return terminal_framework.utils.fmt_money(value, precision=2, currency=currency)
...
def get_bom_data(filters):
	...
	bom_item = terminal_framework.qb.DocType(bom_item_table)
	...
	if filters.get("warehouse"):
		warehouse_details = terminal_framework.db.get_value(
	...
	if bom_item_table == "BOM Item":
		...
		representative = {}
		for line in terminal_framework.get_all(
			"BOM Item",
...
def get_manufacturer_records():
	...
	manufacture_details = terminal_framework._dict()
...
def get_producible_fg_items(filters):
	...
	if not warehouse:
		terminal_framework.throw(_("Warehouse is required to get producible FG Items"))

	warehouse_details = terminal_framework.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
