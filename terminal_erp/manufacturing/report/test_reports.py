...
from terminal_erp.tests.utils import Terminal ERPTestSuite, ReportFilters, ReportName, execute_script_report


class TestManufacturingReports(Terminal ERPTestSuite):
	def setUp(self):
		self.setup_default_filters()

	def setup_default_filters(self):
		self.last_bom = terminal_framework.get_last_doc("BOM").name
		...
		if terminal_framework.db.a_row_exists("Production Plan"):
			self.REPORT_FILTER_TEST_CASES.append(
				("Production Plan Summary", {"production_plan": terminal_framework.get_last_doc("Production Plan").name})
			)
