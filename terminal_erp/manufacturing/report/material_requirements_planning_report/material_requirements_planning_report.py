...
class MaterialRequirementsPlanningReport:
	...
	def add_non_planned_orders(self, items):
		_adhoc_so_details = terminal_framework._dict({})

		so = terminal_framework.qb.DocType("Sales Order")
		so_item = terminal_framework.qb.DocType("Sales Order Item")
		...
		if self.filters.get("warehouse"):
			...
			if terminal_framework.db.get_value("Warehouse", self.filters.get("warehouse"), "is_group"):
	...
	def get_item_wise_bin_details(self):
		...
		_bin_details = terminal_framework._dict({})
	...
	def update_mps_data_with_bin_details(self, bin_details):
		...
		sales_orders = terminal_framework.get_all(
			"Production Plan Sales Order",
			filters={"parent": self.filters.mps},
			pluck="sales_order",
		)
		...
		sales_order_items = terminal_framework.get_all(
			"Sales Order Item",
	...
	def get_detailed_view_chart_data(self, data):
		chart_data = terminal_framework._dict({})
		i = 0
		...
		for row in sorted_data:
			...
			if delivery_date not in chart_data:
				i += 1
				chart_data[delivery_date] = terminal_framework._dict(
					{
						"demand": 0.0,
						"supply": 0.0,
					}
			...
			for d in data:
				if getdate(d.delivery_date) >= getdate(row.from_date) and getdate(d.delivery_date) <= getdate(
					...
					if row.from_date not in chart_data:
						...
						labels.append(label)
						chart_data[row.from_date] = terminal_framework._dict(
							{
								"demand": 0.0,
								"supply": 0.0,
							}
	...
	def get_bucket_view_data(self, data):
		...
		item_wise_data = terminal_framework._dict({})
		for item in data:
			if item.item_code not in item_wise_data:
				item_wise_data[item.item_code] = terminal_framework._dict(
					{
			...
			for date in self.dates:
				date = terminal_framework._dict(date)
		...
		for row in item_wise_data:
			new_data.append(terminal_framework._dict(item_wise_data[row]))
	...
	def get_detailed_view_data(self):
		...
		for row in sorted_data:
			rm_details = self.item_rm_details.get(row.item_code) or terminal_framework._dict({})
			...
			if i != 0:
				data.append(terminal_framework._dict({}))
	...
	def get_work_order_data(self):
		wo_details = terminal_framework._dict({})

		doctype = terminal_framework.qb.DocType("Work Order")
		query = (
			terminal_framework.qb.from_(doctype)
			.select(
	...
	def get_purchase_order_data(self):
		po_details = terminal_framework._dict({})

		parent_doctype = terminal_framework.qb.DocType("Purchase Order")
		doctype = terminal_framework.qb.DocType("Purchase Order Item")

		query = (
			terminal_framework.qb.from_(parent_doctype)
			.inner_join(doctype)
	...
	def get_sales_order_data(self):
		...
		so_details = terminal_framework._dict({})

		parent_doctype = terminal_framework.qb.DocType("Sales Order")
		doctype = terminal_framework.qb.DocType("Sales Order Item")
	...
	def get_packed_items_sales_order(self):
		parent_doctype = terminal_framework.qb.DocType("Sales Order")
		so_item = terminal_framework.qb.DocType("Sales Order Item")
		doctype = terminal_framework.qb.DocType("Packed Item")
	...
	def get_subcontracted_data(self):
		parent_doctype = terminal_framework.qb.DocType("Subcontracting Order")
		doctype = terminal_framework.qb.DocType("Subcontracting Order Item")
	...
	def get_mps_data(self):
		doctype = terminal_framework.qb.DocType("Master Production Schedule")
		child_doctype = terminal_framework.qb.DocType("Master Production Schedule Item")
	...
	def get_items_from_mps(self, mps_data):
		...
		if self.filters.mps:
			sales_forecasts = terminal_framework.get_all(
			...
			if sales_forecasts:
				sales_forecast_items = terminal_framework.get_all(
					"Sales Forecast Item",
	...
	def get_raw_materials_data(self, items):
		item_wise_rm_details = terminal_framework._dict()
		for item_code in items:
			if item_code not in item_wise_rm_details:
				item_wise_rm_details[item_code] = terminal_framework.db.get_value(
					"Item",
					item_code,
	...
	def get_sales_forecast_data(self):
		forecast_doc = terminal_framework.qb.DocType("Sales Forecast")
		doctype = terminal_framework.qb.DocType("Sales Forecast Item")

		query = (
			terminal_framework.qb.from_(forecast_doc)
			.inner_join(doctype)
		...
		if self.filters.mps:
			forecast = terminal_framework.db.get_value("Master Production Schedule", self.filters.mps, "sales_forecast")
...
def get_item_details(item_code, company):
	...
	) or terminal_framework._dict({"safety_stock": 0})

	default_data = terminal_framework.db.get_value(
		"Item Default",
...
def get_item_lead_time(item_code, type_of_material):
	doctype = terminal_framework.qb.DocType("Item Lead Time")

	query = terminal_framework.qb.from_(doctype).where(doctype.item_code == item_code)
...
def get_item_capacity(item_code, bucket_size):
	capacity = terminal_framework.db.get_value(
		"Item Lead Time",
		item_code,
		"capacity_per_day",
	)
...
def make_order(selected_rows: str | list, company: str, warehouse: str | None = None, mps: str | None = None):
	if not terminal_framework.has_permission("Purchase Order", "create"):
		terminal_framework.throw(_("Not permitted to make Purchase Orders"), terminal_framework.PermissionError)
	...
	for (supplier, release_date), items in purchase_orders.items():
		...
		if len(po.items) > 0:
			...
			terminal_framework.msgprint(
				_("Purchase Order {0} created").format(terminal_framework.bold(po.name)),
				alert=True,
			)
...
def make_work_orders(work_orders, company, warehouse=None, mps=None):
	for item in work_orders:
		...
		wo.insert()
		terminal_framework.msgprint(
			_("Work Order {0} created").format(terminal_framework.bold(wo.name)),
			alert=True,
		)
...
def get_item_uom(item_code):
	return terminal_framework.get_cached_value("Item", item_code, ["stock_uom", "purchase_uom"], as_dict=True)


@terminal_framework.request_cache
def is_whole_number(uom):
	return terminal_framework.get_cached_value("UOM", uom, "must_be_whole_number")
