...
class ProductionPlanReport:
	...
	def get_open_orders(self):
		...
		parent = terminal_framework.qb.DocType(doctype)
		query = None
		...
		if doctype == "Work Order":
			...
			child = terminal_framework.qb.DocType(f"{doctype} Item")
			query = (
				terminal_framework.qb.from_(parent)
				.from_(child)
				.select(
	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			for d in self.orders:
				bom_no = d.bom_no or terminal_framework.get_cached_value("Item", d.production_item, "default_bom")
			...
			bom_item = terminal_framework.qb.DocType(bom_item_doctype)
	...
	def get_purchase_details(self):
		...
		purchased_items = terminal_framework.get_all(
			"Purchase Order Item",
			fields=[
				"item_code",
