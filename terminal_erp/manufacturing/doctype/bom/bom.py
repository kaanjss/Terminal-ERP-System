...
from terminal_framework.query_builder import Field
from terminal_framework.query_builder.functions import Count, IfNull, Max, Min, NullIf, Sum
from terminal_framework.utils import cint, cstr, flt, get_link_to_form, parse_json
from terminal_framework.website.website_generator import WebsiteGenerator

import terminal_erp
from terminal_erp.setup.utils import get_exchange_rate
from terminal_erp.stock.doctype.item.item import get_item_details
from terminal_erp.stock.get_item_details import ItemDetailsCtx, get_conversion_factor, get_price_list_rate
...
from terminal_erp.manufacturing.doctype.bom.mapper import (
	get_bom_diff,
	get_children,
	item_query,
	make_variant_bom,
)
from terminal_erp.manufacturing.doctype.bom.services.costing import (
	BOMCostingService,
)
from terminal_erp.manufacturing.doctype.bom.services.exploded_items import (
	BOMExplodedItemsService,
)
from terminal_erp.manufacturing.doctype.bom.services.operations_cost import (
	add_additional_cost,
...
class BOMRecursionError(terminal_framework.ValidationError):
	pass
...
class BOMTree:
	...
	def __create_tree(self):
		bom = terminal_framework.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.bom_explosion_item.bom_explosion_item import BOMExplosionItem
		from terminal_erp.manufacturing.doctype.bom_item.bom_item import BOMItem
		from terminal_erp.manufacturing.doctype.bom_operation.bom_operation import BOMOperation
		from terminal_erp.manufacturing.doctype.bom_secondary_item.bom_secondary_item import BOMSecondaryItem
	...
	website = terminal_framework._dict(
	...
	def autoname(self):
		...
		existing_boms = terminal_framework.get_all(
		...
		if terminal_framework.db.exists("BOM", name):
			existing_boms = terminal_framework.get_all(
	...
	def set_onload_for_multi_level_bom(self):
		use_multi_level_bom = terminal_framework.db.get_value(
			"Property Setter",
	...
	def validate(self):
		self.route = terminal_framework.scrub(self.name).replace("_", "-")

		if not self.company:
			terminal_framework.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def set_default_uom(self):
		...
		item_wise_uom = terminal_framework._dict(
	...
	def on_update(self):
		terminal_framework.cache().hdel("bom_children", self.name)
	...
	def update_bom_creator_status(self):
		...
		doc = terminal_framework.get_doc("BOM Creator", self.bom_creator)
	...
	def validate_total_cost_allocation(self):
		...
		if total_cost_allocation_per != 100:
			terminal_framework.throw(_("Cost allocation between finished goods and secondary items should equal 100%"))
	...
	def get_item_det(self, item_code):
		...
		if not item:
			terminal_framework.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def _normalize_material_args(kwargs):
		if not kwargs:
			kwargs = terminal_framework.form_dict.get("args")
	...
	def _build_rm_detail(self, args, item, rate):
		ret_item = {
			...
			"bom_no": args["bom_no"],
			"is_phantom_item": terminal_framework.get_value("BOM", args["bom_no"], "is_phantom_bom")
			if args["bom_no"]
			else 0,
			"rate": rate,
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			...
			not terminal_framework.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			self.db_set("is_default", 1)
			terminal_framework.db.set_value("Item", self.item, "default_bom", self.name)
		else:
			self._unset_default_bom()

	def _set_as_default_bom(self):
		from terminal_framework.model.utils import set_default

		set_default(self, "item")
		item = terminal_framework.get_doc("Item", self.item)
	...
	def _unset_default_bom(self):
		...
		item = terminal_framework.get_doc("Item", self.item)
		if item.default_bom == self.name:
			terminal_framework.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			terminal_framework.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = terminal_framework.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			terminal_framework.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = terminal_framework.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			terminal_framework.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				terminal_framework.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
			items.append(m.item_code)

		if fixed_asset_items := terminal_framework.db.get_all(
	...
	def check_recursion(self, bom_list=None):
		...
		child_items = terminal_framework.get_all(
			"BOM Item",
	...
	def add_materials_from_bom(
		...
		if not terminal_framework.db.exists("BOM", {"item": finished_good, "name": bom_no, "docstatus": 1}):
			terminal_framework.throw(_("BOM {0} not found for the item {1}").format(bom_no, finished_good))
	...
	def company_currency(self):
		return terminal_erp.get_company_currency(self.company)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			act_pbom = (
				terminal_framework.qb.from_(bom_item)
				.join(bom)
			...
			if act_pbom and act_pbom[0][0]:
				terminal_framework.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			terminal_framework.throw(_("Operations cannot be left blank"))
	...
	def _validate_operation_row(self, d):
		if not d.description:
			d.description = terminal_framework.db.get_value("Operation", d.operation, "description")
	...
	def validate_uom(self, item_code, uom, process_loss_per, process_loss_qty):
		must_be_whole_number = terminal_framework.get_value("UOM", uom, "must_be_whole_number")

		if process_loss_per and process_loss_per > 100:
			terminal_framework.throw(_("Process Loss Percentage cannot be greater than 100"))
		...
		if process_loss_qty and must_be_whole_number and process_loss_qty % 1 != 0:
			...
			).format(terminal_framework.bold(item_code), terminal_framework.bold(uom), terminal_framework.bold(uom))
			terminal_framework.throw(msg, title=_("Invalid Process Loss Configuration"))
...
def _get_bom_children(bom_no):
	children = terminal_framework.cache().hget("bom_children", bom_no)
	if children is None:
		bom_item = terminal_framework.qb.DocType("BOM Item")
		children = (
			terminal_framework.qb.from_(bom_item)
		...
		terminal_framework.cache().hset("bom_children", bom_no, children)
	return children
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(terminal_framework.db.get_value("Item", args["item_code"], "last_purchase_rate"))
...
def _get_price_list_item_rate(args, bom_doc):
	if not bom_doc.buying_price_list:
		terminal_framework.throw(_("Please select Price List"))
	...
	item_doc = terminal_framework.get_cached_doc("Item", args.get("item_code"))
...
def get_valuation_rate(data):
	...
	if not valuation_rate:
		valuation_rate = terminal_framework.db.get_value("Item", item_code, "valuation_rate")
...
def _get_avg_valuation_rate_from_bins(item_code, company, data):
	...
	wh_table = terminal_framework.qb.DocType("Warehouse")
	item_valuation = (
...
def _get_last_valuation_rate_from_sle(item_code):
	...
	last_val_rate = (
		...
		.orderby(sle.posting_datetime, order=terminal_framework.qb.desc)
		.orderby(sle.creation, order=terminal_framework.qb.desc)
		.limit(1)
	).run(as_dict=True)
...
def _query_bom_items(bom, company, opts):
	track_semi_finished_goods = terminal_framework.get_cached_value("BOM", bom, "track_semi_finished_goods")
...
def _get_bom_item_tables(opts):
	if cint(opts.fetch_exploded):
		bom_item = terminal_framework.qb.DocType("BOM Explosion Item")
		qty_field_col = bom_item.stock_qty
	elif opts.fetch_secondary_items:
		bom_item = terminal_framework.qb.DocType("BOM Secondary Item")
		qty_field_col = bom_item.stock_qty
	else:
		bom_item = terminal_framework.qb.DocType("BOM Item")
	...
	return terminal_framework._dict(
		...
		bom_doc=terminal_framework.qb.DocType("BOM"),
		item_doc=terminal_framework.qb.DocType("Item"),
...
def _build_base_bom_items_query(bom, company, qty, t):
	return (
		terminal_framework.qb.from_(t.bom_item)
		.join(t.bom_doc)
...
def _add_exploded_item_columns(query, t, bom, amount_col, stock_item_condition):
	bom_item_table = terminal_framework.qb.DocType("BOM Item")
	idx_subquery = (
		terminal_framework.qb.from_(bom_item_table)
...
def _set_default_accounts_for_items(item_dict, company):
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = terminal_framework.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = terminal_framework.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	bom = terminal_framework.get_doc("BOM", bom_no)
	if not bom.is_active:
		terminal_framework.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not terminal_framework.in_test:
			terminal_framework.throw(_("BOM {0} must be submitted").format(bom_no))
	if item and not _bom_contains_item(bom, item):
		terminal_framework.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))
...
def _bom_contains_item(bom, item):
	...
	return (
		...
		or bom.item.lower() == cstr(terminal_framework.db.get_value("Item", item, "variant_of")).lower()
	)
...
def get_backflush_based_on(bom_no=None):
	...
	if bom_no:
		backflush_based_on = terminal_framework.db.get_value("BOM", bom_no, "backflush_based_on")
