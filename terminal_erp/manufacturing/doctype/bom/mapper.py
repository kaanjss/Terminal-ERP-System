...
from terminal_framework.core.doctype.version.version import get_diff
...
def get_children(parent: str | None = None, is_root: bool = False, **filters):
	...
	if not parent or parent == "BOM":
		terminal_framework.msgprint(_("Please select a BOM"))
		return

	terminal_framework.form_dict.parent = parent
	bom_doc = terminal_framework.get_cached_doc("BOM", parent)
	terminal_framework.has_permission("BOM", doc=bom_doc, throw=True)
...
def _enrich_bom_items(bom_items, bom_doc):
	...
	items = terminal_framework.get_list(
		"Item",
	...
	for bom_item in bom_items:
		...
		bom_item.image = terminal_framework.db.escape(bom_item.image)
...
def get_bom_diff(bom1: str, bom2: str):
	...
	if bom1 == bom2:
		terminal_framework.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be the same").format(terminal_framework.bold(bom1), terminal_framework.bold(bom2))
		)

	doc1 = terminal_framework.get_doc("BOM", bom1)
	doc2 = terminal_framework.get_doc("BOM", bom2)
...
def _diff_table_field(df, doc1, doc2, out):
	from terminal_framework.model import table_fields
...
def item_query(
	...
	searchfields = terminal_framework.get_meta("Item", cached=True).get_search_fields()
...
def _item_query_filters(filters):
	...
	if filters and filters.get("item_code"):
		if not terminal_framework.get_cached_value("Item", filters.get("item_code"), "has_variants"):
...
def make_variant_bom(
	...
	terminal_framework.has_permission("BOM", "write", throw=True)
...
def _postprocess_variant_bom(source, doc, item, variant_items, source_name):
	from terminal_erp.manufacturing.doctype.work_order.work_order import add_variant_item
