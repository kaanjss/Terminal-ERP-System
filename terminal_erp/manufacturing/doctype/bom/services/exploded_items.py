...
class BOMExplodedItemsService:
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			explosion_item = terminal_framework.qb.DocType("BOM Explosion Item")
			terminal_framework.qb.from_(explosion_item).delete().where(explosion_item.parent == self.doc.name).run()
