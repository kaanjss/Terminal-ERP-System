...
class BOMCostingService:
	...
	def validate_bom_currency(self, item):
		if (
			item.get("bom_no")
			and terminal_framework.db.get_value("BOM", item.get("bom_no"), "currency") != self.doc.currency
		):
	...
	def _raw_material_rate(self, arg, notify):
		...
		if terminal_framework.db.get_value("Item", arg["item_code"], "is_customer_provided_item") or arg.get(
			"sourced_by_supplier"
		):
			return 0
	...
	def _warn_rate_not_found(self, arg, notify):
		if self.doc.rm_cost_as_per == "Price List":
			...
			return
		terminal_framework.msgprint(msg, alert=True)
	...
	def update_cost(
		...
		if not from_child_bom:
			...
			terminal_framework.msgprint(_(msg), alert=True)
	...
	def _update_parent_boms(self):
		...
		for bom in parent_boms:
			terminal_framework.get_doc("BOM", bom).update_cost(from_child_bom=True)
	...
	def get_bom_unitcost(self, bom_no):
		bom_table = terminal_framework.qb.DocType("BOM")
		bom = (
			terminal_framework.qb.from_(bom_table)
			.select(
				bom_table.name,
	...
	def _set_row_hour_rate(self, row):
		hour_rate = 0
		if row.workstation:
			hour_rate = flt(terminal_framework.get_cached_value("Workstation", row.workstation, "hour_rate"))
		elif row.workstation_type:
			hour_rate = flt(terminal_framework.get_cached_value("Workstation Type", row.workstation_type, "hour_rate"))
	...
	def get_rm_rate_map(self) -> dict[str, float]:
		...
		for item in self.doc.get("items"):
			if item.bom_no:
				...
				explosion_items = terminal_framework.get_all(
					"BOM Explosion Item",
