...
def add_additional_cost(stock_entry, work_order, job_card=None):
	...
	expense_account = terminal_framework.get_value(
		"Company",
		work_order.company,
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account, job_card=None):
	bom = terminal_framework.get_doc("BOM", work_order.bom_no)
...
def _non_stock_items_cost(item_amounts, stock_entry, bom):
	non_stock_items = terminal_framework.get_all(
		"Item",
		fields="name",
...
def _add_operation_workstation_costs(stock_entry, work_order, row, op_expense_account):
	from terminal_erp.stock.doctype.stock_entry.stock_entry import get_consumed_operating_cost

	workstation_cost = terminal_framework.get_all(
		"Workstation Cost",
...
def _workstation_cost_row(expense_account, row, wc, actual, operating_cost, qty):
	precision = terminal_framework.get_precision("Landed Cost Taxes and Charges", "amount")
	return {
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None, job_card=None):
	from terminal_erp.stock.doctype.stock_entry.stock_entry import get_remaining_operating_cost
...
def _max_operation_quantity(work_order):
	table = terminal_framework.qb.DocType("Job Card")
	query = (
...
def _utilised_corrective_cost(work_order):
	charges = terminal_framework.qb.DocType("Landed Cost Taxes and Charges")
	query = (
		terminal_framework.qb.from_(charges)
...
def get_op_cost_from_sub_assemblies(bom_no, op_cost=0):
	...
	bom_items = terminal_framework.get_all(
	...
	for row in bom_items:
		...
		if cost := terminal_framework.get_cached_value("BOM", row.bom_no, "operating_cost_per_bom_quantity"):
			op_cost += flt(cost)
