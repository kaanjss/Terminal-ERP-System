...
from terminal_erp.manufacturing.doctype.bom_update_log.bom_update_log import (
	BOMMissingError,
	resume_bom_cost_update_jobs,
)
from terminal_erp.manufacturing.doctype.bom_update_tool.bom_update_tool import (
	enqueue_replace_bom,
	enqueue_update_cost,
)
...
class TestBOMUpdateLog(Terminal ERPTestSuite):
	...
	def setUp(self):
		...
		self.boms = terminal_framework._dict(
	...
	def test_bom_update_log_validate(self):
		...
		with self.assertRaises(terminal_framework.ValidationError):
			enqueue_replace_bom(boms=terminal_framework._dict(current_bom=self.boms.new_bom, new_bom=self.boms.new_bom))

		with self.assertRaises(terminal_framework.ValidationError):
			enqueue_replace_bom(boms=terminal_framework._dict(current_bom=self.boms.new_bom, new_bom="Dummy BOM"))
	...
	def test_bom_replace_for_root_bom(self):
		...
		exploded_items = terminal_framework.get_all(
		...
		old_bom = terminal_framework.db.get_value("BOM", {"item": "B-Item E"}, "name")
		bom_tree = {"B-Item E": {"B-Item G": {}}}

		new_bom = create_nested_bom(bom_tree, prefix="")
		enqueue_replace_bom(boms=terminal_framework._dict(current_bom=old_bom, new_bom=new_bom.name))
...
def remove_bom(item_code):
	boms = terminal_framework.get_all("BOM", fields=["docstatus", "name"], filters={"item": item_code})

	for row in boms:
		if row.docstatus == 1:
			terminal_framework.get_doc("BOM", row.name).cancel()

		terminal_framework.delete_doc("BOM", row.name)
