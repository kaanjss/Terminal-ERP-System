...
from terminal_framework.query_builder.functions import IfNull, Max, Min, Sum
...
from terminal_erp.controllers.stock_controller import (
	QualityInspectionNotSubmittedError,
	QualityInspectionRejectedError,
)
from terminal_erp.manufacturing.doctype.bom.bom import add_additional_cost, get_bom_items_as_dict
from terminal_erp.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
	get_mins_between_operations,
)
from terminal_erp.manufacturing.doctype.workstation_type.workstation_type import get_workstations
from terminal_erp.subcontracting.doctype.subcontracting_bom.subcontracting_bom import (
...
class OperationMismatchError(terminal_framework.ValidationError):
	pass


class OperationSequenceError(terminal_framework.ValidationError):
	pass


class JobCardCancelError(terminal_framework.ValidationError):
	pass


class JobCardOverTransferError(terminal_framework.ValidationError):
	pass
...
class JobCard(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.job_card_item.job_card_item import JobCardItem
		from terminal_erp.manufacturing.doctype.job_card_operation.job_card_operation import JobCardOperation
		from terminal_erp.manufacturing.doctype.job_card_scheduled_time.job_card_scheduled_time import (
			JobCardScheduledTime,
		)
		from terminal_erp.manufacturing.doctype.job_card_secondary_item.job_card_secondary_item import (
			JobCardSecondaryItem,
		)
		from terminal_erp.manufacturing.doctype.job_card_time_log.job_card_time_log import JobCardTimeLog
	...
	def onload(self):
		excess_transfer = terminal_framework.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
	...
	def has_stock_entry(self):
		return terminal_framework.db.exists("Stock Entry", {"job_card": self.name, "docstatus": ["!=", 2]})
	...
	def get_manufactured_qty(self):
		...
		table = terminal_framework.qb.DocType(table_name)
		query = terminal_framework.qb.from_(table).where((table.job_card == self.name) & (table.docstatus == 1))

		if self.is_subcontracted:
			query = query.select(Sum(table.qty))
		else:
			child = terminal_framework.qb.DocType("Stock Entry Detail")
			query = (
				query.join(child)
	...
	def validate_job_card_qty(self):
		...
		completed_qty = flt(terminal_framework.db.get_value("Work Order Operation", self.operation_id, "completed_qty"))
	...
	def get_allowed_wo_qty(self):
		wo_qty = flt(terminal_framework.get_cached_value("Work Order", self.work_order, "qty"))
		over_production_percentage = flt(
			terminal_framework.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
		return wo_qty + (wo_qty * over_production_percentage / 100)

	def get_total_job_card_qty(self):
		job_card_qty = terminal_framework.get_all(
			"Job Card",
	...
	def set_secondary_items(self):
		...
		for item_code, values in items_dict.items():
			self.append_secondary_item(item_code, terminal_framework._dict(values))
	...
	def validate_time_log_row(self, d):
		if d.to_time and get_datetime(d.from_time) > get_datetime(d.to_time):
			terminal_framework.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_production_capacity(self):
		if self.workstation:
			return terminal_framework.get_cached_value("Workstation", self.workstation, "production_capacity") or 1
		return 1
	...
	def get_time_logs(self, args, doctype, open_job_cards=None):
		...
		jc = terminal_framework.qb.DocType("Job Card")
		jctl = terminal_framework.qb.DocType(doctype)
	...
	def get_base_overlap_query(self, jc, jctl, args):
		...
		return (
			terminal_framework.qb.from_(jctl)
			.from_(jc)
			.select(
				jc.name.as_("name"),
	...
	def get_open_job_cards(self, employee, workstation=None):
		jc = terminal_framework.qb.DocType("Job Card")
		jctl = terminal_framework.qb.DocType("Job Card Time Log")

		query = (
			terminal_framework.qb.from_(jc)
			.left_join(jctl)
	...
	def check_workstation_time(self, row):
		workstation_doc = terminal_framework.get_cached_doc("Workstation", self.workstation)
		if not workstation_doc.working_hours or cint(
			terminal_framework.db.get_single_value("Manufacturing Settings", "allow_overtime")
		):
	...
	def add_start_time_logs(self, args, employees):
		new_args = terminal_framework._dict(
			{
	...
	def get_required_items(self):
		terminal_framework.has_permission("Job Card", "write", doc=self, throw=True)

		if not self.get("work_order"):
			return

		doc = terminal_framework.get_doc("Work Order", self.get("work_order"))
	...
	def append_required_item(self, doc, d):
		...
		self.append(
			"items",
			{
				...
				"uom": terminal_framework.db.get_value("Item", d.item_code, "stock_uom"),
	...
	def validate_inspection(self):
		bom_inspection_required = terminal_framework.get_value("BOM", self.bom_no, "inspection_required")
		operation_inspection_required = terminal_framework.get_value(
		...
		action_submit, action_reject = terminal_framework.get_single_value(
			"Stock Settings",
			["action_if_quality_inspection_is_not_submitted", "action_if_quality_inspection_is_rejected"],
		)

		qa_status, docstatus = terminal_framework.get_value(
	...
	def handle_unsubmitted_inspection(self, action_submit):
		...
		if action_submit == "Stop":
			terminal_framework.throw(message, title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
		else:
			terminal_framework.msgprint(message, alert=True, indicator="orange")
	...
	def handle_rejected_inspection(self, action_reject):
		...
		if action_reject == "Stop":
			terminal_framework.throw(message, title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
		else:
			terminal_framework.msgprint(message, alert=True, indicator="orange")
	...
	def validate_job_card(self):
		if self.work_order and terminal_framework.get_cached_value("Work Order", self.work_order, "status") == "Stopped":
			terminal_framework.throw(
	...
	def validate_time_logs_present(self):
		...
		elif terminal_framework.db.get_single_value("Manufacturing Settings", "enforce_time_logs"):
			for row in self.time_logs:
	...
	def update_work_order(self):
		...
		if self.is_corrective_job_card and not cint(
			terminal_framework.db.get_single_value(
		...
		wo = terminal_framework.get_doc("Work Order", self.work_order)
	...
	def update_semi_finished_good_details(self):
		if self.operation_id:
			...
			terminal_framework.db.set_value("Work Order Operation", self.operation_id, "completed_qty", qty)
			if (
				self.finished_good
				and terminal_framework.get_cached_value("Work Order", self.work_order, "production_item")
				== self.finished_good
			):
				_wo_doc = terminal_framework.get_doc("Work Order", self.work_order)
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, pending_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty + pending_qty:
			...
			).format(terminal_framework.bold(_("Job Card")), terminal_framework.bold(self.name), terminal_framework.bold(self.production_item))

			second_part_msg = _(
				"Kindly cancel the Manufacturing Entries first against the work order {0}."
			).format(terminal_framework.bold(get_link_to_form("Work Order", self.work_order)))
	...
	def update_wo_operation_row(
		...
		if data.get("workstation") != self.workstation:
			...
			data.hour_rate = flt(terminal_framework.get_value("Workstation", self.workstation, "hour_rate"))
	...
	def set_consumed_qty_in_job_card_item(self, ste_doc):
		...
		sed = terminal_framework.qb.DocType("Stock Entry Detail")

		query = (
			terminal_framework.qb.from_(sed)
			.join(se)
		...
		itemwise_consumed_qty = terminal_framework._dict(query.run(as_list=True))
		...
		for row in ste_doc.items:
			...
			terminal_framework.db.set_value("Job Card Item", row.job_card_item, "consumed_qty", consumed_qty)
	...
	def set_transferred_qty_in_job_card_item(self, ste_doc):
		...
		allow_excess = terminal_framework.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		...
		for row in ste_doc.items:
			...
			terminal_framework.db.set_value("Job Card Item", row.job_card_item, "transferred_qty", flt(transferred_qty))
	...
	def get_job_card_items_transferred_qty(self, ste_doc):
		...
		return terminal_framework._dict(query.run(as_list=True))

	def validate_over_transfer(self, ste_doc, row, transferred_qty):
		"Block over transfer of items if not allowed in settings."
		required_qty = terminal_framework.db.get_value("Job Card Item", row.job_card_item, "required_qty")
		if flt(transferred_qty) > flt(required_qty):
			terminal_framework.throw(
				_(
					"Row #{0}: Cannot transfer more than Required Qty {1} for Item {2} against Job Card {3}"
				).format(row.idx, terminal_framework.bold(required_qty), terminal_framework.bold(row.item_code), ste_doc.job_card),
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.work_order and not terminal_framework.get_cached_value(
	...
	def get_transferred_qty_from_stock_entry(self):
		...
		stock_entry = terminal_framework.qb.DocType("Stock Entry")

		result = (
			terminal_framework.qb.from_(stock_entry)
	...
	def set_transferred_qty_in_work_order(self):
		doc = terminal_framework.get_doc("Work Order", self.work_order)
	...
	def set_non_semi_fg_status(self):
		if self.items:
			item_data = terminal_framework.get_all(
				"Job Card Item",
	...
	def set_wip_warehouse(self):
		if not self.wip_warehouse:
			self.wip_warehouse = terminal_framework.get_cached_value("Company", self.company, "default_wip_warehouse")
	...
	def validate_operation_id(self):
		if (
			...
			and self.operation
			and self.work_order
			and terminal_framework.get_cached_value("Work Order Operation", self.operation_row_number, "name")
			!= self.operation_id
		):
	...
	def validate_sequence_id(self):
		...
		previous_operations = terminal_framework.get_all(
	...
	def validate_work_order(self):
		if self.is_work_order_closed():
			terminal_framework.throw(_("You cannot make any changes to Job Card since Work Order is closed."))
	...
	def is_work_order_closed(self):
		if self.work_order:
			status = terminal_framework.get_value("Work Order", self.work_order, "status")
	...
	def update_status_in_workstation(self, status):
		...
		terminal_framework.db.set_value("Workstation", self.workstation, "status", status)
	...
	def update_workstation_status(self):
		...
		if not job_cards:
			terminal_framework.db.set_value("Workstation", self.workstation, "status", "Off")

		for row in job_cards:
			terminal_framework.db.set_value("Workstation", self.workstation, "status", status_map.get(row.status))
			return
	...
	def validate_docstatus(self):
		if self.docstatus == 2:
			terminal_framework.throw(_("Cancelled Job Card cannot be processed."))

		if self.docstatus == 1:
			terminal_framework.throw(_("Submitted Job Card cannot be processed."))

	def validate_complete_job_card_qty(self, kwargs):
		if flt(kwargs.pending_qty) and flt(kwargs.pending_qty) < 0:
			terminal_framework.throw(_("Pending quantity cannot be negative."))

		if flt(kwargs.process_loss_qty) and flt(kwargs.process_loss_qty) < 0:
			terminal_framework.throw(_("Process loss quantity cannot be negative."))

		if flt(kwargs.pending_qty) and flt(kwargs.pending_qty) > self.for_quantity:
			terminal_framework.throw(_("Pending quantity cannot be greater than the for quantity."))
	...
	def auto_submit_job_card(self, auto_submit):
		...
		terminal_framework.msgprint(_("Job Card {0} has been completed").format(get_link_to_form("Job Card", self.name)))

	@terminal_framework.whitelist()
	def make_stock_entry_for_semi_fg_item(self, auto_submit: bool = False):
		terminal_framework.has_permission("Job Card", "write", doc=self, throw=True)
		terminal_framework.has_permission("Stock Entry", "create", throw=True)
	...
	def get_consumed_process_loss(self):
		table = terminal_framework.qb.DocType("Stock Entry")
		query = (
	...
	def build_manufacture_stock_entry(self):
		from terminal_erp.stock.doctype.stock_entry_type.stock_entry_type import ManufactureEntry
		...
		return ManufactureEntry(
			{
				...
				"project": terminal_framework.db.get_value("Work Order", self.work_order, "project"),
			}
		)

	def populate_manufacture_stock_entry(self, ste):
		from terminal_erp.stock.doctype.stock_entry.services.manufacturing import ManufactureStockEntry

		ste.make_stock_entry()
		ste.stock_entry.flags.ignore_mandatory = True
		wo_doc = terminal_framework.get_doc("Work Order", self.work_order)
...
def make_time_log(kwargs: str | dict):
	kwargs = terminal_framework.parse_json(kwargs)

	kwargs = terminal_framework._dict(kwargs)
	doc = terminal_framework.get_doc("Job Card", kwargs.job_card_id)
	terminal_framework.has_permission("Job Card", "write", doc=doc, throw=True)
...
def get_operation_details(work_order: str, operation: str):
	terminal_framework.has_permission("Work Order", "read", throw=True)
...
def get_operations(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	if not filters.get("work_order"):
		terminal_framework.msgprint(_("Please select a Work Order first."))
		return []
...
def get_job_details(start: Any, end: Any, filters: str | dict | None = None):
	if not terminal_framework.has_permission("Job Card", "read"):
		terminal_framework.throw(_("Not permitted to read Job Card"), terminal_framework.PermissionError)
