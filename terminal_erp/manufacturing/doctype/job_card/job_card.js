...
terminal_framework.ui.form.on("Job Card", {
	setup(frm) {
		frm.set_query("operation", () => ({
			query: "terminal_erp.manufacturing.doctype.job_card.job_card.get_operations",
	...
	pending_qty(frm) {
		if (frm.doc.total_completed_qty <= 0.0) {
			...
			terminal_framework.throw(__("Please complete the job first before entering Pending Quantity"));
		}

		if (frm.doc.pending_qty < 0) {
			terminal_framework.throw(__("Pending Quantity cannot be less than 0"));
		}
		...
		if (remaining_qty < frm.doc.pending_qty) {
			frm.doc.pending_qty = 0.0;
			refresh_field("pending_qty");
			terminal_framework.throw(__("Pending Quantity cannot be greater than {0}", [remaining_qty]));
		}
	...
	refresh(frm) {
		...
		if (doc.work_order) {
			terminal_framework.db.get_value("Work Order", doc.work_order, "transfer_material_against").then((r) => {
	...
	complete_job_card(frm) {
		...
		frm.job_completion_dialog = terminal_framework.prompt(
			fields,
			(data) => {
				if (data.qty <= 0) {
					terminal_framework.throw(__("Quantity should be greater than 0"));
				}
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				terminal_framework.model.open_mapped_doc({
					method: "terminal_erp.manufacturing.doctype.job_card.mapper.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card(frm, operation, for_operation) {
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.job_card.mapper.make_corrective_job_card",
			args: {
	...
	operation(frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			terminal_framework.call({
				method: "terminal_erp.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
	...
	make_time_log(frm, args) {
		...
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: { args },
			freeze: true,
			callback() {
				frm.reload_doc();
	...
	make_dashboard(frm, has_items) {
		...
		const btn = (cls, icon_path, label, icon_color) => `
			<button class="btn btn-sm ${cls}" style="display:inline-flex;align-items:center;gap:5px;font-weight:600;padding:6px 14px;">
				${terminal_framework.utils.icon(icon_path, "sm", "", "", "", "", icon_color)}
				${label}
			</button>`;
				...
				<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
					<div>
						...
						<div style="display:flex;align-items:center;gap:8px;">
							${terminal_framework.utils.icon("clock-4", "md", "", "", "", "", timer_color)}
			...
			wrapper.find(".jcd-btn-start").on("click", () => {
				...
				if (has_no_employee) {
					...
					terminal_framework.prompt(
						...
						(d) => frm.events.start_timer(frm, terminal_framework.datetime.now_datetime(), d.employees),
						__("Assign Job to Employee")
					);
				} else {
					frm.events.start_timer(frm, terminal_framework.datetime.now_datetime(), frm.doc.employee);
				}
			});
		}
		...
		if (show_resume) {
			wrapper.find(".jcd-btn-resume").on("click", () => {
				frm.call({
					method: "resume_job",
					doc: frm.doc,
					args: { start_time: terminal_framework.datetime.now_datetime() },
					callback() {
						frm.reload_doc();
					},
				});
		...
		if (show_pause) {
			wrapper.find(".jcd-btn-pause").on("click", () => {
				frm.call({
					method: "pause_job",
					doc: frm.doc,
					args: { end_time: terminal_framework.datetime.now_datetime() },
					callback() {
						frm.reload_doc();
					},
				});
	...
	get_current_time(frm) {
		...
		frm.doc.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += get_seconds_diff(terminal_framework.datetime.now_datetime(), d.from_time);
			}
		});
	...
	make_material_request(frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.manufacturing.doctype.job_card.mapper.make_material_request",
			frm: frm,
	...
	make_stock_entry(frm) {
		terminal_framework.model.open_mapped_doc({
			method: "terminal_erp.manufacturing.doctype.job_card.mapper.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	source_warehouse(frm) {
		if (frm.doc.source_warehouse) {
			frm.doc.items.forEach((d) => {
				terminal_framework.model.set_value(d.doctype, d.name, "source_warehouse", frm.doc.source_warehouse);
			});
		}
	},
});

terminal_framework.ui.form.on("Job Card Time Log", {
	completed_qty(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!row.completed_qty) {
			terminal_framework.model.set_value(row.doctype, row.name, {
				time_in_mins: 0,
				to_time: "",
			});
		}
