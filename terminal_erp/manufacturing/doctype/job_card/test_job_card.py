...
from terminal_framework.utils import flt, random_string
from terminal_framework.utils.data import add_to_date, now, today

from terminal_erp.manufacturing.doctype.job_card.job_card import (
	JobCardOverTransferError,
	OperationMismatchError,
	OverlapError,
)
from terminal_erp.manufacturing.doctype.job_card.mapper import (
...
from terminal_erp.manufacturing.doctype.work_order.work_order import WorkOrder, make_work_order
...
class TestJobCard(Terminal ERPTestSuite):
	def setUp(self):
	...
	def make_bom_for_jc_tests(self):
		bom = terminal_framework.copy_doc(self.globalTestRecords["BOM"][2])
	...
	def test_quality_inspection_mandatory_check(self):
		...
		final_bom = terminal_framework.new_doc(
			"BOM",
			item=final.name,
			quantity=1,
			with_operations=1,
		...
		work_order.submit()
		job_card = terminal_framework.get_all("Job Card", filters={"work_order": work_order.name, "operation": "Cutting"})
		job_card_doc = terminal_framework.get_doc("Job Card", job_card[0].name)
		job_card_doc.append(
			"time_logs",
			{
		...
		self.assertRaises(terminal_framework.ValidationError, job_card_doc.submit)
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			terminal_framework.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = terminal_framework.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = terminal_framework.db.get_value(
			...
			completed_qty = terminal_framework.db.get_value(
	...
	def test_job_card_cannot_be_submitted_while_on_hold(self):
		...
		job_card = terminal_framework.get_all(
			"Job Card",
			filters={"work_order": self.work_order.name},
		...
		doc.is_paused = 1
		self.assertRaises(terminal_framework.ValidationError, doc.submit)

	def test_job_card_overlap(self):
		wo2 = make_wo_order_test_record(item="_Test FG Item 2", qty=2)

		jc1 = terminal_framework.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = terminal_framework.get_last_doc("Job Card", {"work_order": wo2.name})
	...
	def test_job_card_overlap_with_capacity(self):
		...
		workstation = make_workstation(workstation_name=random_string(5)).name
		terminal_framework.db.set_value("Workstation", workstation, "production_capacity", 1)
		...
		terminal_framework.db.set_value("Workstation", workstation, "production_capacity", 2)
		jc2.load_from_db()
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = terminal_framework.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = terminal_framework.get_doc("Job Card", job_card_name)
	...
	@Terminal ERPTestSuite.change_settings("Manufacturing Settings", {"job_card_excess_transfer": 1})
	def test_job_card_excess_material_transfer(self):
		...
		job_card = terminal_framework.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	def test_job_card_excess_material_transfer_with_no_reference(self):
		...
		self.assertRaises(terminal_framework.ValidationError, transfer_entry_1.insert)
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = terminal_framework.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = terminal_framework.get_doc(
			doctype="Operation", is_corrective_operation=1, name=terminal_framework.generate_hash()
		).insert()
	...
	def test_if_corrective_jc_ops_cost_is_added_to_manufacture_stock_entry(self):
		...
		job_card = terminal_framework.get_last_doc("Job Card", {"work_order": wo.name})
		...
		terminal_framework.get_doc(stock_entry).submit()

		from terminal_erp.manufacturing.doctype.work_order.work_order import make_job_card
	...
	def test_job_card_statuses(self):
		...
		jc = terminal_framework.new_doc("Job Card")
	...
	def test_job_card_material_request_and_bom_details(self):
		from terminal_erp.stock.doctype.material_request.mapper import make_stock_entry

		create_bom_with_multiple_operations()
		work_order = make_wo_with_transfer_against_jc()

		job_card_name = terminal_framework.db.get_value("Job Card", {"work_order": work_order.name}, "name")
	...
	def test_job_card_proccess_qty_and_completed_qty(self):
		from terminal_erp.manufacturing.doctype.routing.test_routing import (
			create_routing,
			setup_bom,
			setup_operations,
		)
		...
		first_job_card = terminal_framework.get_all(
			"Job Card",
		...
		jc = terminal_framework.get_doc("Job Card", first_job_card)
		...
		second_job_card = terminal_framework.get_all(
			"Job Card",
		...
		jc2 = terminal_framework.get_doc("Job Card", second_job_card)
		...
		self.assertRaises(terminal_framework.ValidationError, jc2.save)
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 10))
		s.submit()
	...
	def test_op_cost_calculation(self):
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 4))
		s.submit()
		...
		job_card = terminal_framework.get_last_doc("Job Card", {"work_order": wo_doc.name})
		job_card.append(
			"time_logs",
			{
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 6))
		self.assertEqual(s.additional_costs[0].amount, 8)

	def test_co_by_product_for_sfg_flow(self):
		from terminal_erp.manufacturing.doctype.operation.test_operation import make_operation

		terminal_framework.db.set_value("UOM", "Nos", "must_be_whole_number", 0)
		...
		job_card = terminal_framework.get_doc(
			"Job Card",
		...
		manufacturing_entry = terminal_framework.get_doc(job_card.make_stock_entry_for_semi_fg_item())
	...
	def test_secondary_items_without_sfg(self):
		for row in terminal_framework.get_doc("BOM", self.work_order.bom_no).items:
			make_stock_entry(
				item_code=row.item_code,
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(self.work_order.name, "Manufacture"))
		s.submit()
	...
	def test_operating_cost_with_overproduction(self):
		...
		first_operation = terminal_framework.get_all(
			"Job Card",
			filters={"work_order": wo_doc.name, "sequence_id": 1},
			fields=["name"],
			order_by="sequence_id",
		...
		jc = terminal_framework.get_doc("Job Card", first_operation)
		...
		second_operation = terminal_framework.get_all(
			"Job Card",
			filters={"work_order": wo_doc.name, "sequence_id": 2},
			fields=["name"],
			order_by="sequence_id",
		...
		jc = terminal_framework.get_doc("Job Card", second_operation)
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 6))  # overproduction
		s.submit()
		...
		s2 = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 1))
		s2.submit()
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 2))
		s.submit()
		...
		s = terminal_framework.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 3))
		s.submit()
...
def create_bom_with_multiple_operations():
	...
	test_record = terminal_framework.get_test_records("BOM")[2]
	bom_doc = terminal_framework.get_doc(test_record)
