...
from terminal_framework.utils import cint, flt, get_link_to_form, nowdate

from terminal_erp.manufacturing.doctype.bom.bom import get_bom_item_rate
...
def _default_bom_for_item(item, project):
	...
	bom_no = terminal_framework.db.get_value("BOM", filters=filters)
	if bom_no:
		return bom_no

	variant_of = terminal_framework.db.get_value("Item", item, "variant_of")
	return terminal_framework.db.get_value("BOM", {"item": variant_of, "is_default": 1}) if variant_of else None
...
def _handle_missing_default_bom(res, item, project, throw):
	...
	terminal_framework.msgprint(msg, raise_exception=throw, indicator="yellow", alert=(not throw))
	return res


def _merge_bom_details(res, project):
	bom_data = terminal_framework.db.get_value(
		"BOM",
		res["bom_no"],
...
def _variant_default_bom(item):
	if not terminal_framework.db.get_value("Item", item, "variant_of"):
		return None
	return terminal_framework.db.get_value("BOM", {"item": item, "is_default": 1, "docstatus": 1})


def _new_work_order(item, bom_no, company, item_details, use_multi_level_bom):
	from terminal_erp import get_default_company

	wo_doc = terminal_framework.new_doc("Work Order")
	wo_doc.track_semi_finished_goods = terminal_framework.db.get_value("BOM", bom_no, "track_semi_finished_goods")
...
def add_variant_item(variant_items, wo_doc, bom_no, table_name="items"):
	variant_items = terminal_framework.parse_json(variant_items)
...
def _add_variant_row(item, wo_doc, bom_no, table_name):
	bom_doc = terminal_framework.get_cached_doc("BOM", bom_no)
...
def check_if_scrap_warehouse_mandatory(bom_no: str):
	terminal_framework.has_permission("BOM", "read", throw=True)
...
def make_stock_entry(
	...
	work_order = terminal_framework.get_doc("Work Order", work_order_id)
...
def _set_stock_entry_warehouses(stock_entry, work_order, purpose, target_warehouse, source_stock_entry):
	is_group = terminal_framework.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
	...
	if purpose == "Material Transfer for Manufacture":
		...
		if work_order.bom_no:
			stock_entry.inspection_required = terminal_framework.db.get_value(
...
def make_job_card(work_order: str, operations: str | list, parent_bom: str | None = None):
	terminal_framework.has_permission("Job Card", "create", throw=True)

	operations = terminal_framework.parse_json(operations)
...
def split_qty_based_on_batch_size(wo_doc, row, qty):
	if not cint(terminal_framework.db.get_value("Operation", row.operation, "create_job_card_based_on_batch_size")):
	...
	if flt(row.get("qty")) <= 0:
		terminal_framework.throw(
			_("Quantity to Manufacture can not be zero for the operation {0}").format(
				terminal_framework.bold(row.get("operation"))
			)
		)
	...
	if flt(row.get("qty")) > flt(row.get("pending_qty")):
		terminal_framework.throw(
			_("For operation {0}: Quantity ({1}) can not be greater than pending quantity ({2})").format(
				terminal_framework.bold(row.get("operation")),
				terminal_framework.bold(row.get("qty")),
				terminal_framework.bold(row.get("pending_qty")),
			)
		)


def create_job_card(work_order, row, enable_capacity_planning=False, auto_create=False):
	doc = terminal_framework.new_doc("Job Card")
...
def _auto_create_job_card(doc, row, enable_capacity_planning):
	...
	doc.insert()
	terminal_framework.msgprint(_("Job card {0} created").format(get_link_to_form("Job Card", doc.name)), alert=True)
...
def create_pick_list(source_name: str, target_doc: str | dict | None = None, for_qty: float | None = None):
	terminal_framework.has_permission("Pick List", "create", throw=True)

	for_qty = for_qty or terminal_framework.parse_json(target_doc).get("for_qty")
	max_finished_goods_qty = terminal_framework.db.get_value("Work Order", source_name, "qty")
	postprocess = partial(
...
def _set_pick_list_item_qty(source, target, source_parent, for_qty, max_finished_goods_qty):
	...
	target.uom = terminal_framework.get_value("Item", source.item_code, "stock_uom")
...
def make_stock_return_entry(work_order: str):
	...
	wo_doc = terminal_framework.get_cached_doc("Work Order", work_order)
