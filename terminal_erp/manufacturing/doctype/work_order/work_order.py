...
from terminal_framework.query_builder.functions import Coalesce, IfNull, Sum
...
from terminal_erp.manufacturing.doctype.bom.bom import validate_bom_no
...
from terminal_erp.manufacturing.doctype.work_order.services.operations import (
	OperationsService,
)
from terminal_erp.manufacturing.doctype.work_order.services.required_items import (
...
from terminal_erp.manufacturing.doctype.work_order.services.status import (
	StatusService,
)
...
from terminal_erp.stock.doctype.serial_no.serial_no import get_available_serial_nos
from terminal_erp.stock.utils import validate_warehouse_company
from terminal_erp.utilities.transaction_base import validate_uom_is_integer


class OverProductionError(terminal_framework.ValidationError):
	pass


class CapacityError(terminal_framework.ValidationError):
	pass


class StockOverProductionError(terminal_framework.ValidationError):
	pass


class OperationTooLongError(terminal_framework.ValidationError):
	pass


class ItemHasVariantError(terminal_framework.ValidationError):
	pass


class SerialNoQtyError(terminal_framework.ValidationError):
	pass
...
class WorkOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.work_order_additional_item.work_order_additional_item import (
			WorkOrderAdditionalItem,
		)
		from terminal_erp.manufacturing.doctype.work_order_item.work_order_item import WorkOrderItem
		from terminal_erp.manufacturing.doctype.work_order_operation.work_order_operation import WorkOrderOperation
	...
	def onload(self):
		ms = terminal_framework.get_doc("Manufacturing Settings")
		...
		self.set_onload(
			...
			terminal_framework.db.get_single_value("Stock Settings", "enable_stock_reservation"),
		)

		if self.bom_no:
			if based_on := terminal_framework.get_cached_value("BOM", self.bom_no, "backflush_based_on"):
				self.set_onload("backflush_raw_materials_based_on", based_on)

	@property
	def secondary_items(self):
		parent = terminal_framework.qb.DocType("Stock Entry")
		...
		if secondary_items_generated:
			...
			secondary_items = terminal_framework.get_query(
				"BOM",
	...
	def non_stock_items(self):
		non_stock_items = terminal_framework.get_query(
			"BOM",
	...
	def show_create_job_card_button(self):
		jc_doctype = terminal_framework.qb.DocType("Job Card")
		query = (
			terminal_framework.qb.from_(jc_doctype)
		...
		operation_details = terminal_framework._dict(operation_details)
	...
	def validate(self):
		...
		if not len(self.get("required_items")) or not terminal_framework.db.get_single_value(
	...
	def validate_dates(self):
		if self.actual_start_date and self.actual_end_date:
			if self.actual_end_date < self.actual_start_date:
				terminal_framework.throw(_("Actual End Date cannot be before Actual Start Date"))
				...
				if op.idx == 1 and op.sequence_id != 1:
					terminal_framework.throw(
						_("Row #1: Sequence ID must be 1 for Operation {0}.").format(
							terminal_framework.bold(op.operation)
						)
					)
					...
					terminal_framework.throw(
						_("Row #{0}: Sequence ID must be {1} or {2} for Operation {3}.").format(
							op.idx,
							terminal_framework.bold(sequence_id),
							terminal_framework.bold(sequence_id + 1),
							terminal_framework.bold(op.operation),
						)
					)
	...
	def validate_subcontracting_inward_order(self):
		if scio := self.subcontracting_inward_order:
			if self.source_warehouse != (
				rm_receipt_warehouse := terminal_framework.get_cached_value(
			...
			if self.fg_warehouse != (
				delivery_warehouse := terminal_framework.get_cached_value(
			...
			possible_customer_provided_items = terminal_framework.get_all(
			...
			for item in self.required_items:
							...
							if item.required_qty > row.received_qty - row.returned_qty - row.work_order_qty:
								terminal_framework.throw(
									_(
										...
										item.idx,
										get_link_to_form("Item", item.item_code),
										terminal_framework.bold(row.received_qty - row.returned_qty - row.work_order_qty),
									)
								)
							else:
				...
				elif terminal_framework.get_cached_value("Warehouse", item.source_warehouse, "customer"):
					terminal_framework.throw(
						_(
	...
	def validate_sales_order(self):
		...
		if self.material_request_item and (
			mr_plan_item := terminal_framework.get_value(
				"Material Request Item", self.material_request_item, "material_request_plan_item"
			)
		):
			if main_item_code := terminal_framework.get_value(
		...
		if self.sales_order:
			...
			PackedItem = terminal_framework.qb.DocType("Packed Item")
			ProductBundleItem = terminal_framework.qb.DocType("Product Bundle Item")

			so = (
				terminal_framework.qb.from_(SalesOrder)
			...
			if len(so):
				...
				terminal_framework.throw(_("Sales Order {0} is not valid").format(self.sales_order))

	def set_default_warehouse(self):
		if not self.wip_warehouse and not self.skip_transfer:
			self.wip_warehouse = terminal_framework.get_cached_value("Company", self.company, "default_wip_warehouse")
		if not self.fg_warehouse:
			self.fg_warehouse = terminal_framework.get_cached_value("Company", self.company, "default_fg_warehouse")
	...
	def validate_additional_transferred_qty(self):
		transfer_extra_materials_percentage = terminal_framework.db.get_single_value(
		...
		precision = terminal_framework.get_precision("Work Order", "qty")
	...
	def validate_warehouse(self):
		...
		if not self.wip_warehouse and not self.skip_transfer:
			terminal_framework.throw(_("Work-in-Progress Warehouse is required before Submit"))
		if not self.fg_warehouse:
			terminal_framework.throw(_("Target Warehouse is required before Submit"))
	...
	def on_submit(self):
		...
		if self.production_plan and terminal_framework.db.exists(
	...
	def set_qty_change(self):
		if scio_item_name := self.get("subcontracting_inward_order_item"):
			self.qty_change = terminal_framework._dict()
	...
	def update_subcontracting_inward_order_received_items(self):
		if scio_item_name := self.get("subcontracting_inward_order_item"):
			scio_rm_data = terminal_framework.get_all(
				"Subcontracting Inward Order Received Item",
				filters={
			...
			table = terminal_framework.qb.DocType("Subcontracting Inward Order Received Item")
			case_expr = Case()
			...
			terminal_framework.qb.update(table).set(table.work_order_qty, case_expr).where(
	...
	def create_serial_no_batch_no(self):
		...
		if not cint(
			terminal_framework.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = terminal_framework.get_cached_value("Item", self.production_item, "create_new_batch")
		if not batch_auto_creation:
			terminal_framework.msgprint(
				_("Batch not created for item {0} since it does not have a batch series.").format(
					terminal_framework.bold(self.production_item)
				),
				alert=True,
				indicator="orange",
			)
	...
	def validate_cancel(self):
		if self.status == "Stopped":
			terminal_framework.throw(_("Stopped Work Order cannot be cancelled, Unstop it first to cancel"))

		# Check whether any stock entry exists against this Work Order
		stock_entry = terminal_framework.get_all(
			"Stock Entry",
		...
		if stock_entry:
			terminal_framework.throw(
				_("Cannot cancel because submitted Stock Entry {0} exists").format(
					terminal_framework.utils.get_link_to_form("Stock Entry", stock_entry[0])
				)
			)

	def validate_production_item(self):
		if terminal_framework.get_cached_value("Item", self.production_item, "has_variants"):
			terminal_framework.throw(_("Work Order cannot be raised against an Item Template"), ItemHasVariantError)
	...
	def validate_qty(self):
		if self.qty <= 0:
			terminal_framework.throw(_("Quantity to Manufacture must be greater than 0."))

		if (
			self.stock_uom
			and terminal_framework.get_cached_value("UOM", self.stock_uom, "must_be_whole_number")
		...
		if self.production_plan and self.production_plan_item and not self.production_plan_sub_assembly_item:
			qty_dict = terminal_framework.db.get_value(
	...
	def validate_operations(self):
		for d in self.operations:
			...
			if d.time_in_mins <= 0:
				terminal_framework.throw(_("Operation Time must be greater than 0 for Operation {0}").format(d.operation))
	...
	def make_bom(self):
		...
		bom = terminal_framework.new_doc("BOM")
...
def get_bom_operations(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	return terminal_framework.get_all("BOM Operation", filters=filters, fields=["operation"], as_list=1)


@terminal_framework.whitelist()
def set_work_order_ops(name: str):
	po = terminal_framework.get_doc("Work Order", name)
...
def get_disassembly_available_qty(stock_entry_name: str, current_se_name: str | None = None) -> float:
	se = terminal_framework.db.get_value("Stock Entry", stock_entry_name, ["fg_completed_qty"], as_dict=True)
	if not se:
		return 0.0
	...
	already_disassembled = flt(terminal_framework.db.get_value("Stock Entry", filters, [{"SUM": "fg_completed_qty"}]))
...
def get_default_warehouse(company: str):
	wip, fg, scrap = terminal_framework.get_cached_value(
...
def stop_unstop(work_order: str, status: str):
	...
	if not terminal_framework.has_permission("Work Order", "write"):
		terminal_framework.throw(_("Not permitted"), terminal_framework.PermissionError)

	pro_order = terminal_framework.get_doc("Work Order", work_order)

	if pro_order.status == "Closed":
		terminal_framework.throw(_("Closed Work Order can not be stopped or Re-opened"))

	pro_order.update_status(status)
	pro_order.update_planned_qty()
	terminal_framework.msgprint(_("Work Order has been {0}").format(status))
...
def close_work_order(work_order: str, status: str):
	...
	if work_order.get("operations"):
		job_cards = terminal_framework.get_list(
			"Job Card",
