...
class OperationsService:
	...
	def create_job_card(self):
		manufacturing_settings_doc = terminal_framework.get_doc("Manufacturing Settings")
	...
	def _validate_capacity_window(self, row, plan_days):
		from terminal_erp.manufacturing.doctype.work_order.work_order import CapacityError

		if date_diff(row.planned_end_time, self.doc.planned_start_date) <= plan_days:
			return

		terminal_framework.message_log.pop()
		msg = _(
		...
		terminal_framework.throw(msg, CapacityError)
	...
	def set_operation_start_end_time(self, row, idx):
		...
		if row.planned_start_time == row.planned_end_time:
			terminal_framework.throw(_("Capacity Planning Error, planned start time can not be same as end time"))
	...
	def set_work_order_operations(self):
		...
		if not self.doc.bom_no or not terminal_framework.get_cached_value("BOM", self.doc.bom_no, "with_operations"):
			return
	...
	def _collect_bom_operations(self):
		...
		if self.doc.use_multi_level_bom:
			bom_tree = terminal_framework.get_doc("BOM", self.doc.bom_no).get_tree_representation()
		...
		bom_qty = terminal_framework.get_cached_value("BOM", self.doc.bom_no, "quantity")
	...
	def _adjust_operation_row(self, d, qty, exploded):
		if not d.fixed_time:
			if terminal_framework.get_value("Operation", d.operation, "create_job_card_based_on_batch_size"):
				qty = d.batch_size
	...
	def _operation_status(self, d, max_allowed_qty_for_wo):
		...
		if qty <= flt(max_allowed_qty_for_wo, precision):
			return "Completed"
		terminal_framework.throw(_("Completed Qty cannot be greater than 'Qty to Manufacture'"))
...
def get_hour_rate(workstation):
	return terminal_framework.get_cached_value("Workstation", workstation, "hour_rate") or 0.0
