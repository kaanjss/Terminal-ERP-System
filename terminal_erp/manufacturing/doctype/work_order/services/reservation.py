...
class WorkOrderStockReservation:
	...
	def validate_fg_warehouse_for_reservation(self):
		...
		warehouses = terminal_framework.get_all(
			"Sales Order Item",
	...
	def enable_auto_reserve_stock(self):
		if self.doc.is_new() and terminal_framework.db.get_single_value("Stock Settings", "auto_reserve_stock"):
	...
	def update_qty_in_stock_reservation(self, row, transferred_qty, row_wise_serial_batch):
		...
		names = terminal_framework.get_all(
	...
	def _apply_transferred_qty(self, name, transferred_qty, row_wise_serial_batch):
		...
		doc = terminal_framework.get_doc("Stock Reservation Entry", name)
	...
	def update_consumed_qty_in_stock_reservation(self, item, consumed_qty, wip_warehouse):
		...
		names = terminal_framework.get_all("Stock Reservation Entry", filters=filters, pluck="name", order_by="creation")
		for name in names:
	...
	def get_list_of_materials_for_reservation(self, stock_entry):
		items = terminal_framework._dict()
	...
	def _finished_goods_item_details(self, stock_entry):
		...
		if stock_entry.job_card:
			...
			finished_good = terminal_framework.db.get_value("Job Card", stock_entry.job_card, "finished_good")
	...
	def get_wo_details(self):
		...
		item = terminal_framework.qb.DocType("Work Order Item")
		query = (
	...
	def get_voucher_details(self, stock_entry):
		if stock_entry.purpose == "Manufacture" and self.doc.sales_order:
			return terminal_framework._dict({self.doc.production_item: self._so_voucher_detail()})
		return terminal_framework._dict({d.item_code: d.name for d in self.doc.required_items})
	...
	def release_reserved_qty_for_subcontract_transfer(self):
		...
		for entry in entries:
			...
			sre = terminal_framework.get_doc("Stock Reservation Entry", entry.name)
	...
	def _subcontract_transferred_qty_by_item(self):
		...
		sco_names = terminal_framework.get_all(
			"Subcontracting Order Item", filters={"job_card": ["in", job_cards]}, pluck="parent"
		)
		po_names = terminal_framework.get_all(
		...
		ste_child = terminal_framework.qb.DocType("Stock Entry Detail")
...
def make_stock_reservation_entries(
	...
	terminal_framework.has_permission("Work Order", "write", doc=doc, throw=True)
...
def _reserve_or_transfer(sre, doc, is_transfer):
	if doc.production_plan and is_transfer:
		...
		terminal_framework.msgprint(_("Stock Reservation Entries Created"), alert=True)
...
def get_sre_details(work_order):
	sre_details = terminal_framework._dict()
...
def get_row_wise_serial_batch(work_order, purpose=None):
	...
	stock_entries = terminal_framework.get_all(
		"Stock Entry",
...
def _accumulate_serial_batch(row_wise_serial_batch, entry):
	...
	details = row_wise_serial_batch.setdefault(
		key, terminal_framework._dict({"serial_nos": [], "batch_nos": defaultdict(float)})
	)
	if entry.serial_no:
