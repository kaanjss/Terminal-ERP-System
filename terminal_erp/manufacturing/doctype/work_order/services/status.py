...
from terminal_erp.stock.stock_balance import get_planned_qty, update_bin_qty
...
class StatusService:
	...
	def validate_work_order_against_so(self):
		...
		allowance_percentage = flt(
			terminal_framework.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_sales_order")
		)
		...
		terminal_framework.throw(
			_("Cannot produce more Item {0} than Sales Order quantity {1} {2}").format(
				...
				terminal_framework.bold(so_qty),
				terminal_framework.bold(terminal_framework.get_value("Item", self.doc.production_item, "stock_uom")),
			),
			OverProductionError,
		)
	...
	def _so_item_qty(self):
		...
		return (
			terminal_framework.qb.from_(so_item)
	...
	def _packed_item_qty(self):
		packed_item = terminal_framework.qb.DocType("Packed Item")
		return (
			terminal_framework.qb.from_(packed_item)
	...
	def _submitted_status(self, status):
		...
		precision = terminal_framework.get_precision("Work Order", "produced_qty")
	...
	def _update_qty_for_purpose(self, purpose, fieldname):
		from terminal_erp.manufacturing.doctype.work_order.work_order import StockOverProductionError
	...
	def _qty_allowance(self, purpose):
		...
		if not allowance and purpose == "Material Transfer for Manufacture":
			allowance = flt(
				terminal_framework.db.get_single_value("Manufacturing Settings", "transfer_extra_materials_percentage")
			)
		return allowance
	...
	def update_disassembled_qty(self, qty, is_cancel=False):
		...
		if not is_cancel and self.doc.disassembled_qty > self.doc.produced_qty:
			terminal_framework.throw(_("Cannot disassemble more than produced quantity."))
	...
	def get_transferred_or_manufactured_qty(self, purpose, fieldname):
		...
		query = terminal_framework.qb.from_(parent).where(self._stock_entry_filter(parent, purpose, is_additional))
	...
	def update_production_plan_status(self):
		production_plan = terminal_framework.get_doc("Production Plan", self.doc.production_plan)
		produced_qty = 0
		if self.doc.production_plan_item:
			total_qty = terminal_framework.get_all(
				"Work Order",
	...
	def update_planned_qty(self):
		...
		if self.doc.material_request:
			mr_obj = terminal_framework.get_doc("Material Request", self.doc.material_request)
	...
	def update_ordered_qty(self):
		...
		if self.doc.production_plan_item:
			terminal_framework.db.set_value("Production Plan Item", self.doc.production_plan_item, "ordered_qty", qty)
		elif self.doc.production_plan_sub_assembly_item:
			field = self.doc.production_plan_sub_assembly_item
			terminal_framework.db.set_value("Production Plan Sub Assembly Item", field, "ordered_qty", qty)

		doc = terminal_framework.get_doc("Production Plan", self.doc.production_plan)
		doc.set_status()
	...
	def update_work_order_qty_in_combined_so(self):
		total_bundle_qty = self._total_bundle_qty()
		prod_plan = terminal_framework.get_doc("Production Plan", self.doc.production_plan)
		item_reference = terminal_framework.get_value(
		...
		for plan_reference in prod_plan.prod_plan_references:
			...
			terminal_framework.db.set_value("Sales Order Item", plan_reference.sales_order_item, "work_order_qty", qty)

	def _total_bundle_qty(self):
		if not self.doc.product_bundle_item:
			return 1

		pbi = terminal_framework.qb.DocType("Product Bundle Item")
		total_bundle_qty = (
			terminal_framework.qb.from_(pbi).select(Sum(pbi.qty)).where(pbi.parent == self.doc.product_bundle_item)
		).run()[0][0]
	...
	def update_completed_qty_in_material_request(self):
		if self.doc.material_request and self.doc.material_request_item:
			terminal_framework.get_doc("Material Request", self.doc.material_request).update_completed_qty(
