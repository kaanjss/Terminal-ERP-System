...
from terminal_erp.manufacturing.doctype.work_order.mapper import check_if_scrap_warehouse_mandatory
...
from terminal_erp.manufacturing.doctype.work_order.services.status import StatusService
from terminal_erp.stock.utils import get_bin, get_latest_stock_qty
...
class RequiredItemsService:
	...
	def _append_required_items(self, item_dict, operation, reset_source_warehouse):
		for item in sorted(item_dict.values(), key=lambda d: d["idx"] or float("inf")):
			...
			if self.doc.subcontracting_inward_order and not terminal_framework.get_cached_value(
	...
	def update_transferred_qty_for_required_items(self):
		...
		row_wise_serial_batch = terminal_framework._dict({})
	...
	def _material_transfer_qty_by_item(self, is_return):
		...
		qty_by_item = terminal_framework._dict()
	...
	def _additional_items_by_code(self, stock_entry):
		...
		additional_items = terminal_framework._dict()
		for row in stock_entry.items:
