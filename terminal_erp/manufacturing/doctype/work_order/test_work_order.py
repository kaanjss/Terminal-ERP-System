...
from terminal_framework.utils import add_days, add_months, add_to_date, cint, flt, now, nowdate, nowtime, today

from terminal_erp.manufacturing.doctype.job_card.job_card import JobCardCancelError
from terminal_erp.manufacturing.doctype.job_card.mapper import make_stock_entry as make_stock_entry_from_jc
from terminal_erp.manufacturing.doctype.production_plan.test_production_plan import make_bom
from terminal_erp.manufacturing.doctype.work_order.mapper import (
	make_stock_entry,
	make_stock_return_entry,
)
from terminal_erp.manufacturing.doctype.work_order.work_order import (
	CapacityError,
	ItemHasVariantError,
	OverProductionError,
...
from terminal_erp.stock.doctype.item.test_item import create_item, make_item
from terminal_erp.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
	get_batch_from_bundle,
	get_serial_nos_from_bundle,
	make_serial_batch_bundle,
)
from terminal_erp.stock.doctype.serial_no.serial_no import get_serial_nos
from terminal_erp.stock.doctype.stock_entry import test_stock_entry
...
class TestWorkOrder(Terminal ERPTestSuite):
	def setUp(self):
	...
	def check_planned_qty(self):
		...
		planned1 = terminal_framework.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
		...
		# from stores to wip
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(terminal_framework.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = terminal_framework.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
	...
	def test_over_production(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_reserved_qty_for_partial_completion(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		s.submit()
	...
	def test_production_item(self):
		...
		terminal_framework.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(terminal_framework.ValidationError, wo_order.save)

		terminal_framework.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		terminal_framework.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(terminal_framework.ValidationError, wo_order.save)

		terminal_framework.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 2))
		...
		s = terminal_framework.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_backflush_qty_for_overpduction_manufacture(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)

		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)
		...
		s1 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 50))
		s1.submit()
		...
		for ste in cancel_stock_entry:
			doc = terminal_framework.get_doc("Stock Entry", ste)
			doc.cancel()
	...
	def test_reserved_qty_for_stopped_production(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_secondary_material_qty(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()

		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		s.insert()
		s.submit()

		wo_order_details = terminal_framework.db.get_value(
	...
	def test_allow_overproduction(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not terminal_framework.db.get_value("Item Price", {"item_code": "_Test FG Non Stock Item"}):
		...
		if not terminal_framework.db.get_value("BOM", {"item": fg_item, "docstatus": 1}):
			bom = make_bom(
				item=fg_item,
				rate=1000,
		...
		se = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()

		ste = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		ste.insert()
	...
	def test_job_card(self):
		stock_entries = []
		bom = terminal_framework.get_doc(
		...
		ste = terminal_framework.get_doc(make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		ste.submit()
		...
		ste1 = terminal_framework.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		ste1.submit()
	...
	def test_work_order_material_transferred_qty_with_process_loss(self):
		...
		if not terminal_framework.db.exists("Workstation", workstation_name):
			doc = terminal_framework.new_doc("Workstation")
			doc.workstation_name = workstation_name
			doc.save()

		if not terminal_framework.db.exists("Operation", operation_name):
			doc = terminal_framework.new_doc("Operation")
		...
		for row in job_cards:
			...
			doc = terminal_framework.get_doc("Job Card", row.name)
			for row in doc.scheduled_time_logs:
				doc.append(
					"time_logs",
					{
	...
	def test_capcity_planning(self):
		...
		data = terminal_framework.get_cached_value(
			"BOM",
		...
		if data:
			...
			terminal_framework.db.set_single_value("Manufacturing Settings", {"capacity_planning_for_days": 30})
	...
	def test_planned_qty_updates_after_closing_work_order(self):
		...
		planned_before = (
			terminal_framework.db.get_value("Bin", {"item_code": item_code, "warehouse": fg_warehouse}, "planned_qty")
			or 0
		)
		...
		self.assertEqual(terminal_framework.db.get_value("Work Order", wo.name, "status"), "Closed")
	...
	def test_work_order_with_non_transfer_item(self):
		terminal_framework.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		...
		if not terminal_framework.db.get_value("BOM", {"item": fg_item}):
			make_bom(item=fg_item, raw_materials=["_Test FG Item", "_Test FG Item 1"])

		wo = make_wo_order_test_record(production_item=fg_item)
		ste = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
		self.assertEqual(len(ste.items), 1)
		ste1 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		self.assertEqual(len(ste1.items), 3)

	def test_cost_center_for_manufacture(self):
		wo_order = make_wo_order_test_record()
		ste = terminal_framework.get_doc(
	...
	def test_operation_time_with_batch_size(self):
		...
		bom_name = terminal_framework.db.get_value("BOM", {"item": fg_item, "is_active": 1, "with_operations": 1}, "name")
	...
	@Terminal ERPTestSuite.change_settings("Global Defaults", {"default_company": "_Test Company"})
	def test_batch_size_for_fg_item(self):
		fg_item = "Test Batch Size Item For BOM 3"
		rm1 = "Test Batch Size Item RM 1 For BOM 3"

		terminal_framework.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 0)
		...
		terminal_framework.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 1)
		ste1 = terminal_framework.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
		...
		batches = terminal_framework.get_all("Batch", filters={"reference_name": work_order.name})
		self.assertEqual(len(batches), 3)
		batches = [batch.name for batch in batches]

		ste1 = terminal_framework.get_doc(make_stock_entry(work_order.name, "Manufacture", 10))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
				self.assertEqual(row.qty, 10)

				bundle_id = terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		remaining_batches = []
		ste1 = terminal_framework.get_doc(make_stock_entry(work_order.name, "Manufacture", 20))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
	...
	def test_partial_material_consumption(self):
		terminal_framework.db.set_single_value("Manufacturing Settings", "material_consumption", 1)
		...
		ste1 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		ste1.submit()
		ste_cancel_list.append(ste1)

		ste3 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Consumption for Manufacture", 2))
		...
		terminal_framework.db.set_single_value("Manufacturing Settings", "material_consumption", 0)
	...
	def test_extra_material_transfer(self):
		...
		ste3 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		ste2 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
	...
	def test_make_stock_entry_for_customer_provided_item(self):
		...
		if not terminal_framework.db.exists("BOM", {"item": finished_item}):
		...
		ste = terminal_framework.get_doc(make_stock_entry(wo.name, purpose="Material Transfer for Manufacture"))
		ste.insert()
	...
	def test_valuation_rate_missing_on_make_stock_entry(self):
		...
		if not terminal_framework.db.get_value("BOM", {"item": item_name}):
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture"))
		self.assertRaises(terminal_framework.ValidationError, stock_entry.save)

	def test_wo_completion_with_pl_bom(self):
		from terminal_erp.manufacturing.doctype.bom.test_bom import (
		...
		if not terminal_framework.db.exists("BOM", bom_no):
		...
		se = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", qty))
		se.get("items")[0].s_warehouse = "Stores - _TC"
		se.get("items")[0].t_warehouse = wip_warehouse
		se.insert()
		se.submit()

		se = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", qty))
		se.insert()
		se.submit()
	...
	def test_job_card_secondary_item(self):
		...
		for job_card in job_cards:
			if job_card.docstatus == 1:
				terminal_framework.get_doc("Job Card", job_card.name).cancel()

			terminal_framework.delete_doc("Job Card Time Log", job_card.name)
		...
		if not terminal_framework.db.get_value("BOM", {"item": item}):
			bom = make_bom(
		...
		job_card = terminal_framework.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		update_job_card(job_card)

		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		...
		job_card = terminal_framework.db.get_value("Job Card", {"work_order": wo_order.name, "docstatus": 0}, "name")
	...
	def test_close_work_order(self):
		...
		job_cards = terminal_framework.db.get_value("Job Card", {"work_order": wo_order.name}, "name")

		if len(job_cards) == len(bom.operations):
			for jc in job_cards:
				job_card_doc = terminal_framework.get_doc("Job Card", jc)
				job_card_doc.append(
					"time_logs",
	...
	def test_partial_manufacture_entries(self):
		...
		sm = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 100))
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 90))
		...
		s1 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		...
		s2 = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
	...
	@Terminal ERPTestSuite.change_settings("Manufacturing Settings", {"make_serial_no_batch_from_work_order": 1})
	def test_auto_batch_creation(self):
		from terminal_erp.manufacturing.doctype.bom.test_bom import create_nested_bom

		fg_item = terminal_framework.generate_hash(length=20)
		child_item = terminal_framework.generate_hash(length=20)

		bom_tree = {fg_item: {child_item: {}}}

		create_nested_bom(bom_tree, prefix="")

		item = terminal_framework.get_doc("Item", fg_item)
		item.has_batch_no = 1
		...
		except terminal_framework.MandatoryError:
		...
		try:
			...
			for row in stock_entry.items:
				if row.item_code == fg_item:
					...
					sn_doc = terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def get_serial_nos_for_fg(self, work_order):
		serial_nos = []
		for row in terminal_framework.get_all("Serial No", filters={"work_order": work_order}):
	...
	def test_manufacture_entry_mapped_idx_with_exploded_bom(self):
		...
		ste_manu = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
	...
	def test_work_order_multiple_material_transfer(self):
		...
		transfer_entry = terminal_framework.get_doc(
		...
		final_transfer_entry = terminal_framework.get_doc(  # transfer last RM with For Quantity = 0
	...
	def test_backflushed_batch_raw_materials_based_on_transferred(self):
		...
		transferred_ste_doc = terminal_framework.get_doc(
			...
			terminal_framework._dict(
				{
					...
					"batches": terminal_framework._dict({batch_no: 4}),
		...
		manufacture_ste_doc1 = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 1))
		...
		manufacture_ste_doc2 = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 2))
		...
		bundle_doc = terminal_framework.get_doc(
	...
	def test_backflushed_serial_no_raw_materials_based_on_transferred(self):
		...
		# Second Manufacture stock entry
		manufacture_ste_doc2 = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 3))
	...
	def test_backflushed_serial_no_batch_raw_materials_based_on_transferred(self):
		...
		for row in ste_doc.items:
			bundle_doc = terminal_framework.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		bundle_doc = terminal_framework.get_doc("Serial and Batch Bundle", bundle)
		for d in bundle_doc.entries:
			self.assertTrue(d.serial_no)
			self.assertTrue(d.batch_no)
			batch_no = terminal_framework.get_cached_value("Serial No", d.serial_no, "batch_no")
			self.assertEqual(d.batch_no, batch_no)
	...
	def test_backflushed_batch_raw_materials_based_on_transferred_autosabb(self):
		...
		manufacture_ste_doc = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 4))
	...
	def test_non_consumed_material_return_against_work_order(self):
		...
		ste_doc = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Material Transfer for Manufacture", 5))
		for row in ste_doc.items:
			row.qty += 2
			row.transfer_qty += 2
			test_stock_entry.make_stock_entry(
		...
		ste_doc = terminal_framework.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 5))
		for row in ste_doc.items:
	...
	def test_job_card_extra_qty(self):
		...
		job_card = terminal_framework.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		job_card_doc = terminal_framework.get_doc("Job Card", job_card)
		...
		job_card2 = terminal_framework.copy_doc(job_card_doc)
		job_card2.append(
			"time_logs",
			{
				"from_time": row.from_time,
		...
		self.assertRaises(terminal_framework.ValidationError, job_card2.save)

		terminal_framework.db.set_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order", 100)
	...
	def test_operating_cost_account(self):
		...
		company = "_Test Company"
		if not terminal_framework.db.exists("Account", operating_cost_account):
			terminal_framework.get_doc(
				{
					"doctype": "Account",
		...
		terminal_framework.db.set_value("Company", company, "default_operating_cost_account", operating_cost_account)
		...
		if not terminal_framework.db.exists("Workstation", workstation):
		...
		job_cards = terminal_framework.get_all("Job Card", filters={"work_order": wo.name})
		for job_card in job_cards:
			job_card_doc = terminal_framework.get_doc("Job Card", job_card.name)
		...
		se_doc = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		se_doc.save()
	...
	def test_set_op_cost_and_secondary_items_from_sub_assemblies(self):
		...
		se_doc = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		se_doc.save()
	...
	def test_get_rm_cost_from_consumption_entry(self):
		from terminal_erp.stock.doctype.stock_entry.test_stock_entry import (
		...
		mte = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		mte.items[0].s_warehouse = "Stores - _TC"
		mte.insert().submit()

		mce = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Consumption for Manufacture", 10))
		mce.insert().submit()

		me = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		me.insert().submit()
	...
	def test_partial_material_consumption_with_batch(self):
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10))
		stock_entry.submit()
		stock_entry.reload()

		batch_no = get_batch_from_bundle(stock_entry.items[0].serial_and_batch_bundle)

		stock_entry = terminal_framework.get_doc(
	...
	def test_disassemby_order(self):
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		for row in stock_entry.items:
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Disassemble", 1))
		stock_entry.save()
	...
	def test_disassembly_order_with_qty_from_wo_behavior(self):
		...
		se_for_material_tranfer_mfr = terminal_framework.get_doc(
		...
		se_for_manufacture = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", wo.qty))
		se_for_manufacture.submit()

		# Disassembly via WO required_items path (no source_stock_entry)
		disassemble_qty = 4
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Disassemble", disassemble_qty))
		...
		disassemble_qty_2 = 2
		stock_entry_2 = terminal_framework.get_doc(
			make_stock_entry(
	...
	def test_disassembly_with_multiple_manufacture_entries(self):
		...
		se_for_material_transfer = terminal_framework.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", wo.qty)
		)
		...
		se_manufacture1 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		se_manufacture1.submit()

		# Second Manufacture Entry - 7 units
		se_manufacture2 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 7))
		...
		manufacture_entries = terminal_framework.get_all(
			"Stock Entry",
			filters={
				"work_order": wo.name,
				"purpose": "Manufacture",
		...
		bom_se = terminal_framework.get_doc(
			{
				"doctype": "Stock Entry",
	...
	def test_disassembly_auto_sets_source_stock_entry(self):
		...
		se_transfer = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", wo.qty))
		for item in se_transfer.items:
			item.s_warehouse = wo.wip_warehouse
		se_transfer.save()
		se_transfer.submit()

		se_manufacture = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", wo.qty))
		se_manufacture.submit()

		# Disassemble without specifying source_stock_entry
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Disassemble", 3))
		stock_entry.save()
	...
	def test_disassembly_batch_tracked_items(self):
		from terminal_erp.stock.doctype.batch.batch import make_batch
		from terminal_erp.stock.doctype.stock_entry.test_stock_entry import (
			make_stock_entry as make_stock_entry_test_record,
		)
		...
		fg_batch_1 = make_batch(terminal_framework._dict(item=fg_item))
		fg_batch_2 = make_batch(terminal_framework._dict(item=fg_item))

		# Manufacture entry 1 — 3 FG using batch_1 RM/FG
		se_manufacture_1 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		...
		se_manufacture_2 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
	...
	def test_disassembly_serial_tracked_items(self):
		from terminal_framework.model.naming import make_autoname
		...
		series = terminal_framework.db.get_value("Item", fg_item, "serial_no_series")
	...
	def test_components_alternate_item_for_bom_based_manufacture_entry(self):
		terminal_framework.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		terminal_framework.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 1)
		...
		transfer_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()
		...
		manufacture_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		...
		terminal_framework.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 0)
	...
	def test_components_qty_for_bom_based_manufacture_entry(self):
		...
		self.assertRaises(terminal_framework.ValidationError, transfer_entry.save)
		...
		self.assertRaises(terminal_framework.ValidationError, manufacture_entry.save)
	...
	def test_serial_no_status_for_stock_entry(self):
		...
		ste = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 4))
		...
		for row in serial_nos:
			status = terminal_framework.db.get_value("Serial No", row, "status")
			self.assertEqual(status, "Consumed")

	def test_stock_reservation_for_serialized_raw_material(self):
		from terminal_erp.stock.doctype.stock_entry.stock_entry_utils import (
			make_stock_entry as make_stock_entry_test_record,
		)
		...
		self.assertTrue(terminal_framework.get_all("Stock Reservation Entry", filters={"voucher_no": wo.name}))
		...
		self.assertFalse(terminal_framework.get_all("Stock Reservation Entry", filters={"voucher_no": wo1.name}))

		transfer_entry = terminal_framework.get_doc(make_stock_entry(wo1.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()

		self.assertRaises(terminal_framework.ValidationError, transfer_entry.submit)
	...
	def test_stock_reservation_moves_from_store_to_wip_on_transfer(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "enable_stock_reservation", 1)
		try:
			store = "Stores - _TC"
			...
			def reserved_in(warehouse):
				return sum(
					...
					for r in terminal_framework.get_all(
			...
			# Transfer Store -> WIP.
			se = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
			se.submit()
			...
			terminal_framework.db.set_single_value("Stock Settings", "enable_stock_reservation", 0)
	...
	def test_sales_order_linked_work_order_reserves_finished_good(self):
		...
		try:
			...
			transfer = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
			transfer.submit()
			manufacture = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
			manufacture.submit()
	...
	def test_send_to_subcontractor_can_consume_work_order_reserved_stock(self):
		from terminal_erp.buying.doctype.purchase_order.mapper import make_subcontracting_order
		from terminal_erp.controllers.subcontracting_controller import make_rm_stock_entry
		from terminal_erp.manufacturing.doctype.job_card.mapper import make_subcontracting_po
		from terminal_erp.stock.doctype.stock_entry.stock_entry_utils import (
			make_stock_entry as make_stock_entry_test_record,
		)
		from terminal_erp.stock.doctype.warehouse.test_warehouse import create_warehouse
		...
		sfg_bom = terminal_framework.new_doc("BOM")
		...
		tshirt_from_stitched = terminal_framework.new_doc("BOM")
		...
		if not terminal_framework.db.exists("Subcontracting BOM", {"finished_good": tshirt}):
			terminal_framework.get_doc(
				{
		...
		if not terminal_framework.db.exists("Workstation", "Test S2S Workstation"):
			make_workstation(workstation="Test S2S Workstation", production_capacity=1)
		for op in ("Test S2S Stitching", "Test S2S Ironing"):
			if not terminal_framework.db.exists("Operation", op):
		...
		fg_bom = terminal_framework.new_doc("BOM")
		...
		stitching_jc = terminal_framework.get_doc(
			"Job Card",
			terminal_framework.db.get_value("Job Card", {"work_order": wo.name, "operation": "Test S2S Stitching"}),
		)
		stitching_jc.append(
			"time_logs",
			{
		...
		manufacturing_entry = terminal_framework.get_doc(stitching_jc.make_stock_entry_for_semi_fg_item())
		manufacturing_entry.submit()

		sre_name = terminal_framework.db.get_value(
			"Stock Reservation Entry",
		...
		ironing_jc = terminal_framework.db.get_value("Job Card", {"work_order": wo.name, "operation": "Test S2S Ironing"})
		po = terminal_framework.get_doc(make_subcontracting_po(ironing_jc))
		...
		ste = terminal_framework.new_doc("Stock Entry").update(make_rm_stock_entry(sco.name))
		ste.insert()
		ste.submit()

		# The reservation is freed: transferred_qty == sent qty and the SRE is Closed.
		sre = terminal_framework.get_doc("Stock Reservation Entry", sre_name)
	...
	def test_auto_stock_reservation_for_batched_raw_material(self):
		...
		terminal_framework.db.set_single_value("Stock Settings", "auto_reserve_serial_and_batch", 1)
		...
		itemwise_batches = terminal_framework._dict()
		for row in bom.exploded_items:
		...
		for row in terminal_framework.get_all("Stock Reservation Entry", filters={"voucher_no": wo.name}):
			reservation_entry = terminal_framework.get_doc("Stock Reservation Entry", row.name)
	...
	def test_work_order_valuation_auto_pick(self):
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 5))
		...
		original_value = terminal_framework.db.get_single_value(
			"Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"
		)
		original_based_on = terminal_framework.db.get_single_value("Stock Settings", "pick_serial_and_batch_based_on")

		terminal_framework.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)
		terminal_framework.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", "Expiry")

		stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 5))
		stock_entry.items[0].use_serial_batch_fields = 1
		stock_entry.submit()
		stock_entry.reload()
		...
		terminal_framework.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", original_based_on)

	def test_operations_time_planning_calculation(self):
		from terminal_erp.manufacturing.doctype.routing.test_routing import create_routing, setup_operations
		...
		workstation = terminal_framework.get_doc("Workstation", "Test Workstation A")
	...
	def test_allow_additional_material_transfer(self):
		...
		terminal_framework.db.set_single_value("Manufacturing Settings", "transfer_extra_materials_percentage", 50)
		wo_order = make_wo_order_test_record(planned_start_date=now(), qty=2)
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		stock_entry.insert()
		stock_entry.submit()
		...
		stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 1))
		stock_entry.insert()
		stock_entry.submit()

		wo_order.reload()
		self.assertEqual(wo_order.material_transferred_for_manufacturing, 3)
		terminal_framework.db.set_single_value("Manufacturing Settings", "transfer_extra_materials_percentage", 0)
	...
	def test_req_qty_clamping_in_manufacture_entry(self):
		...
		se_transfer_1 = terminal_framework.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", 6, wip_warehouse)
		)
		se_transfer_1.insert()
		se_transfer_1.submit()

		stock_entry_1 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 6, fg_warehouse))
		...
		se_transfer_2 = terminal_framework.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", 20, wip_warehouse)
		)
		se_transfer_2.insert()
		se_transfer_2.submit()

		stock_entry_2 = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 20, fg_warehouse))
	...
	def test_overproduction_allowed_qty(self):
		...
		mt_stock_entry = terminal_framework.get_doc(
			make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10)
		)
		mt_stock_entry.submit()

		fg_stock_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
	...
	def test_reserved_serial_batch(self):
		...
		original_auto_reserve = terminal_framework.db.get_single_value("Stock Settings", "auto_reserve_serial_and_batch")
		original_backflush = terminal_framework.db.get_single_value(
		...
		# Transfer 5 qty
		mt_stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 5))
		mt_stock_entry.submit()
		...
		# Manufacture 2 qty
		fg_stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 2))
		fg_stock_entry.submit()
		...
		# Manufacture 3 qty
		fg_stock_entry = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		fg_stock_entry.submit()
		...
		terminal_framework.db.set_single_value("Stock Settings", "auto_reserve_serial_and_batch", original_auto_reserve)
	...
	def test_phantom_bom_item_not_in_additional_cost(self):
		...
		for item_code, rate in [
			...
			if not terminal_framework.db.get_value("Item Price", {"item_code": item_code, "price_list": price_list}):
				terminal_framework.get_doc(
					{
						"doctype": "Item Price",
						"item_code": item_code,
		...
		phantom_bom = terminal_framework.get_doc(
			{
				"doctype": "BOM",
				"item": phantom_item,
				"is_default": 1,
		...
		fg_bom = terminal_framework.get_doc(
			{
				"doctype": "BOM",
				"item": fg_item,
				"is_default": 1,
		...
		# Transfer materials
		se_transfer = terminal_framework.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se_transfer.insert()
		se_transfer.submit()

		# Manufacture
		se_manufacture = terminal_framework.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_phantom_bom_explosion(self):
		from terminal_erp.manufacturing.doctype.bom.test_bom import create_tree_for_phantom_bom_tests
	...
	def test_reserved_qty_for_pp_with_extra_material_transfer(self):
		...
		s = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_backflush_based_on_in_bom(self):
		...
		backflush_based_on = terminal_framework.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)
		...
		backflush_based_on = terminal_framework.db.get_value("BOM", wo_order.bom_no, "backflush_based_on")
		self.assertEqual(backflush_based_on, "Material Transferred for Manufacture")

		material_transfer_entry = terminal_framework.get_doc(
		...
		material_transfer_entry.append(
			"items",
			{
				...
				"item_group": terminal_framework.get_value("Item", raw_material_2, "item_group"),
				"uom": terminal_framework.get_value("Item", raw_material_2, "stock_uom"),
				"conversion_factor": 1,
		...
		manufacture_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		manufacture_entry.save()
	...
	def test_secondary_items_reflect_manufacture_entry(self):
		...
		manufacture_entry = terminal_framework.get_doc(make_stock_entry(wo_order.name, "Manufacture", 8))
		manufacture_entry.submit()
	...
	def test_transferred_qty_not_misattributed_between_item_and_its_substitute(self):
		...
		"""
		from terminal_erp.manufacturing.doctype.work_order.services.required_items import RequiredItemsService
		...
		if not terminal_framework.db.exists("Item Alternative", {"item_code": item_a, "alternative_item_code": item_b}):
			terminal_framework.get_doc(
				{
					"doctype": "Item Alternative",
					"item_code": item_a,
...
def get_reserved_entries(voucher_no, warehouse=None):
	doctype = terminal_framework.qb.DocType("Stock Reservation Entry")
	sabb = terminal_framework.qb.DocType("Serial and Batch Entry")
...
def make_stock_in_entries_and_get_batches(rm_item, source_warehouse, wip_warehouse):
	...
	for qty, rate in ((5, 100), (5, 200)):
		...
		batch_no = get_batch_from_bundle(stock_entry.items[0].serial_and_batch_bundle)
		batch_doc = terminal_framework.get_doc("Batch", batch_no)
...
def make_operation(**kwargs):
	...
	operation_doc = terminal_framework.get_doc(
		{
			"doctype": "Operation",
...
def make_workstation(**kwargs):
	...
	workstation_doc = terminal_framework.get_doc(
		{
			"doctype": "Workstation",
...
def prepare_boms_for_sub_assembly_test():
	if not terminal_framework.db.exists("BOM", {"item": "Test Final SF Item 1"}):
		bom = make_bom(
	...
	if not terminal_framework.db.exists("BOM", {"item": "Test Final SF Item 2"}):
		bom = make_bom(
	...
	if not terminal_framework.db.exists("BOM", {"item": "Test Final FG Item"}):
		bom = make_bom(
...
def prepare_data_for_workstation_type_check():
	from terminal_erp.manufacturing.doctype.operation.test_operation import make_operation
	from terminal_erp.manufacturing.doctype.workstation.test_workstation import make_workstation
	from terminal_erp.manufacturing.doctype.workstation_type.test_workstation_type import (
	...
	if not terminal_framework.db.exists("BOM", {"item": fg_item.name}):
		bom_doc = make_bom(
			item=fg_item.name,
			source_warehouse="Stores - _TC",
		...
		for index, operation in enumerate(operations):
			if not terminal_framework.db.exists("BOM Operation", {"parent": bom_doc.name, "operation": operation}):
				bom_doc.append(
					"operations",
					{
						"operation": operation,
...
def update_job_card(job_card, jc_qty=None, days=None):
	employee = terminal_framework.db.get_value("Employee", {"status": "Active"}, "name")
	job_card_doc = terminal_framework.get_doc("Job Card", job_card)
	job_card_doc.set(
		"secondary_items",
		[
...
def get_secondary_item_details(bom_no):
	secondary_items = {}
	for item in terminal_framework.get_all(
...
def allow_overproduction(fieldname, percentage):
	doc = terminal_framework.get_doc("Manufacturing Settings")
...
def make_wo_order_test_record(**args):
	...
	wo_order = terminal_framework.new_doc("Work Order")
	wo_order.production_item = args.production_item or args.item or args.item_code or "_Test FG Item"
	wo_order.bom_no = args.bom_no or terminal_framework.db.get_value(
