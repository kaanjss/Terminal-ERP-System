...
terminal_framework.ui.form.on("Workstation", {
	...
	onload(frm) {
		if (frm.is_new()) {
			terminal_framework.call({
				type: "GET",
				method: "terminal_erp.manufacturing.doctype.workstation.workstation.get_default_holiday_list",
				callback: function (r) {
					if (!r.exe && r.message) {
...
terminal_framework.tour["Workstation"] = [
	{
...
class WorkstationDashboard {
	...
	prepapre_dashboard() {
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.workstation.workstation.get_job_cards",
			args: {
	...
	render_job_cards() {
		this.template = terminal_framework.render_template("workstation_job_card", {
			data: this.job_cards,
		});
	...
	setup_qrcode_fields() {
		this.start_job_qrcode = terminal_framework.ui.form.make_control({
			df: {
				label: __("Start Job"),
				fieldtype: "Data",
				options: "Barcode",
		...
		this.complete_job_qrcode = terminal_framework.ui.form.make_control({
			df: {
	...
	validate_job_card(job_card, status, callback) {
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.workstation.workstation.validate_job_card",
			args: {
				job_card: job_card,
				status: status,
			},
	...
	toggle_job_card() {
		this.$wrapper.find(".collapse-indicator-job").on("click", (e) => {
			...
			if (
				...
				$(e.currentTarget).html(terminal_framework.utils.icon("es-line-down", "sm", "mb-1"));
			else $(e.currentTarget).html(terminal_framework.utils.icon("es-line-up", "sm", "mb-1"));
		});
	}
	...
	bind_events() {
		...
		this.$wrapper.find(".btn-pause").on("click", (e) => {
			...
			me.update_job_card(job_card, "pause_job", {
				end_time: terminal_framework.datetime.now_datetime(),
			});
		});
		...
		this.$wrapper.find(".btn-resume").on("click", (e) => {
			...
			me.update_job_card(job_card, "resume_job", {
				start_time: terminal_framework.datetime.now_datetime(),
			});
		});
	...
	start_job(job_card) {
		...
		this.employee_dialog = terminal_framework.prompt(fields, (values) => {
	...
	update_job_card(job_card, method, data) {
		...
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.workstation.workstation.update_job_card",
			args: {
				job_card: job_card,
				method: method,
	...
	make_material_request(job_card) {
		let me = this;
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.workstation.workstation.get_raw_materials",
			args: {
				job_card: job_card,
			},
			callback: (r) => {
	...
	prepare_materials_modal(raw_materials, job_card, callback) {
		...
		this.materials_dialog = new terminal_framework.ui.Dialog({
			title: "Raw Materials",
			fields: fields,
			size: "large",
	...
	get_current_time(data) {
		...
		data.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += this.get_seconds_diff(terminal_framework.datetime.now_datetime(), d.from_time);
			}
		});
