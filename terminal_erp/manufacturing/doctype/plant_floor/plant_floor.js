...
terminal_framework.ui.form.on("Plant Floor", {
	setup(frm) {
	...
	add_workstation(frm) {
		frm.add_custom_button(__("Create Workstation"), () => {
			var doc = terminal_framework.model.get_new_doc("Workstation");
			doc.plant_floor = frm.doc.name;
			doc.status = "Off";
			terminal_framework.ui.form.make_quick_entry(
				"Workstation",
				() => {
	...
	setup_queries(frm) {
		frm.set_query("warehouse", (doc) => {
			if (!doc.company) {
				terminal_framework.throw(__("Please select Company first"));
			}
	...
	update_realtime_status(frm) {
		terminal_framework.realtime.on("update_workstation_status", (data) => {
			terminal_framework.visual_plant_floor.update_status(data);
		});
	},
	...
	prepare_stock_dashboard(frm) {
		...
		terminal_framework.visual_stock = new VisualStock({
			wrapper: wrapper,
			frm: frm,
		});
	},
...
class VisualStock {
	...
	prepare_filters() {
		...
		this.item_filter = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Link",
				fieldname: "item_code",
				placeholder: __("Item"),
		...
		this.item_group_filter = terminal_framework.ui.form.make_control({
			df: {
				fieldtype: "Link",
	...
	async get_stock_summary(start, item_code, item_group) {
		let stock_summary = await terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.plant_floor.plant_floor.get_stock_summary",
			args: {
	...
	render_stock_summary() {
		let template = terminal_framework.render_template("stock_summary_template", {
	...
	update_values() {
		if (!this.values.qty) {
			terminal_framework.throw(__("Quantity is required"));
		}
