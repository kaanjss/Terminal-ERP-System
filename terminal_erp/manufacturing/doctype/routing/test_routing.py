...
from terminal_erp.manufacturing.doctype.job_card.job_card import OperationSequenceError
...
class TestRouting(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_sequence_id(self):
		...
		for data in terminal_framework.get_all(
			...
			job_card_doc = terminal_framework.get_doc("Job Card", data.name)
...
def create_routing(**args):
	...
	doc = terminal_framework.new_doc("Routing")
	doc.update(args)

	if not args.do_not_save:
		terminal_framework.db.savepoint("create_routing")
		try:
			doc.insert()
		except terminal_framework.DuplicateEntryError:
			terminal_framework.db.rollback(save_point="create_routing")
			doc = terminal_framework.get_doc("Routing", args.routing_name)
...
def setup_bom(**args):
	...
	if not args.raw_materials:
		if not terminal_framework.db.exists("Item", "Test Extra Item N-1"):
			make_item(
				"Test Extra Item N-1",
				{
					"is_stock_item": 1,
	...
	name = terminal_framework.db.get_value("BOM", {"item": args.item_code}, "name")
	if not name:
		bom_doc = make_bom(
			item=args.item_code,
		...
		bom_doc = terminal_framework.get_doc("BOM", name)
