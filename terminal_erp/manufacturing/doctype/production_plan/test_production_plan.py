...
from terminal_framework.utils import add_to_date, flt, getdate, now_datetime, nowdate

from terminal_erp.controllers.item_variant import create_variant
from terminal_erp.manufacturing.doctype.production_plan.production_plan import (
	get_items_for_material_requests,
	get_non_completed_production_plans,
	get_sales_orders,
	get_warehouse_list,
)
from terminal_erp.manufacturing.doctype.work_order.mapper import make_stock_entry as make_se_from_wo
from terminal_erp.manufacturing.doctype.work_order.work_order import OverProductionError
...
class TestProductionPlan(Terminal ERPTestSuite):
	def setUp(self):
		for item in [
			...
			sr = terminal_framework.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = terminal_framework.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = terminal_framework.get_all(
		...
		for row in material_requests:
			mr_schedule_date = getdate(terminal_framework.db.get_value("Material Request", row[0], "schedule_date"))
		...
		pln.make_work_order()
		work_orders = terminal_framework.get_all(
			"Work Order", fields=["name"], filters={"production_plan": pln.name}, as_list=1
		)

		pln.make_work_order()
		nwork_orders = terminal_framework.get_all(
			"Work Order", fields=["name"], filters={"production_plan": pln.name}, as_list=1
		)
		...
		for name in material_requests:
			mr = terminal_framework.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = terminal_framework.delete_doc("Work Order", name[0])

		pln = terminal_framework.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			terminal_framework.delete_doc("Work Order", wo.name)
	...
	def test_projected_qty_cascading_across_multiple_sales_orders(self):
		...
		if not terminal_framework.db.exists("BOM", {"item": fg_item_a, "docstatus": 1}):
		...
		# Production plan
		pln = terminal_framework.get_doc(
			{
	...
	def test_production_plan_sales_orders(self):
		...
		pln = terminal_framework.new_doc("Production Plan")
		...
		work_order = terminal_framework.db.get_value(
			"Work Order",
			{"sales_order": sales_order, "production_plan": pln.name, "sales_order_item": sales_order_item},
			"name",
		)

		wo_doc = terminal_framework.get_doc("Work Order", work_order)
		wo_doc.update({"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"})
		wo_doc.submit()

		so_wo_qty = terminal_framework.db.get_value("Sales Order Item", sales_order_item, "work_order_qty")
	...
	def test_donot_allow_to_make_multiple_pp_against_same_so(self):
		...
		pln.get_so_items()
		self.assertRaises(terminal_framework.ValidationError, pln.save)
	...
	def test_so_based_bill_of_material(self):
		...
		pln1 = terminal_framework.new_doc("Production Plan")
		...
		pln2 = terminal_framework.new_doc("Production Plan")
	...
	def test_production_plan_combine_items(self):
		...
		for plan_reference in pln.prod_plan_references:
			...
			so_wo_qty = terminal_framework.db.get_value(
		...
		for so_item in so_items:
			so_wo_qty = terminal_framework.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_subassembly_default_supplier(self):
		...
		item_doc = terminal_framework.get_doc("Item", "Test Motherboard")
		company = "_Test Company"
	...
	def test_production_plan_for_subcontracting_po(self):
		...
		from terminal_erp.manufacturing.doctype.bom.test_bom import create_nested_bom
		from terminal_erp.subcontracting.doctype.subcontracting_bom.test_subcontracting_bom import (
		...
		def make_purchase_receipt_from_po(po_doc):
			...
			from terminal_erp.subcontracting.doctype.subcontracting_order.subcontracting_order import (
				make_subcontracting_receipt,
			)
			from terminal_erp.subcontracting.doctype.subcontracting_receipt.mapper import (
		...
		po = terminal_framework.db.get_value("Purchase Order Item", {"production_plan": plan.name}, "parent")
		po_doc = terminal_framework.get_doc("Purchase Order", po)
		...
		po_doc.cancel()
		po_doc = terminal_framework.copy_doc(po_doc)
		...
		self.assertEqual(
			terminal_framework.db.count("Purchase Order Item", {"production_plan": plan.name, "docstatus": 1}), 2
	...
	def test_sales_order_references_for_sub_assembly_items(self):
		...
		production_plan = terminal_framework.new_doc("Production Plan")
		...
		po_items = terminal_framework.get_all(
			"Purchase Order Item",
			{
	...
	def test_production_plan_for_mr_items(self):
		...
		mr_item = terminal_framework.db.get_value("Material Request Item", {"production_plan": plan.name}, "parent")
		mr_doc = terminal_framework.get_doc("Material Request", mr_item)
		mr_doc.submit()
		plan.reload()
		plan.make_material_request()

		# Test 1 : No more MRs should be created as quantity from Production Plan is fulfilled
		self.assertEqual(terminal_framework.db.count("Material Request Item", {"production_plan": plan.name}), 2)
		...
		self.assertRaises(terminal_framework.ValidationError, mr_doc.submit)
	...
	def test_production_plan_combine_subassembly(self):
		...
		terminal_framework.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				"use_multi_level_bom": 1,
				"item_code": "Green-Car",
				"bom_no": terminal_framework.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = terminal_framework.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = terminal_framework.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not terminal_framework.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not terminal_framework.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not terminal_framework.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": terminal_framework.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not terminal_framework.db.exists("Item", {"item_code": "PIV"}):
		...
		if not terminal_framework.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = terminal_framework.get_doc("BOM", {"item": "PIV"})

		if not terminal_framework.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = terminal_framework.get_doc("Item", "PIV-RED")
		if not terminal_framework.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		so.cancel()
		terminal_framework.delete_doc("Sales Order", so.name)
		variant_bom.cancel()
		terminal_framework.delete_doc("BOM", variant_bom.name)
	...
	def test_get_sales_order_items_for_product_bundle(self):
		...
		from terminal_erp.manufacturing.doctype.work_order.test_work_order import (
		...
		if not terminal_framework.db.exists("Warehouse", fg_warehouse):
		...
		manufacture_entry = terminal_framework.get_doc(create_stock_entry(work_order.name, "Manufacture"))
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for _key, item in items_data.items():
				...
				wo_doc = terminal_framework.get_doc("Work Order", wo_name)
				wo_doc.update(
		...
		wo1 = terminal_framework.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = terminal_framework.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = terminal_framework.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = terminal_framework.new_doc("Production Plan")
		pp.po_items = [terminal_framework._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			terminal_framework._dict(planned_qty=5, produce_qty=10),
			terminal_framework._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())

	def test_production_plan_planned_qty(self):
		# Case 1: When Planned Qty is non-integer and UOM is integer.
		from terminal_erp.utilities.transaction_base import UOMMustBeIntegerError
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					"name": terminal_framework.generate_hash(length=10),
					"temporary_name": terminal_framework.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": terminal_framework.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	def test_produced_qty_for_multi_level_bom_item(self):
		...
		pln.make_work_order()
		work_orders = terminal_framework.get_all("Work Order", filters={"production_plan": pln.name}, pluck="name")
		sa_wo = fg_wo = None
		...
		se = terminal_framework.get_doc(make_se_from_wo(sa_wo, "Manufacture"))
		se.submit()

		se = terminal_framework.get_doc(make_se_from_wo(fg_wo, "Manufacture"))
		se.submit()
	...
	def test_material_request_item_for_purchase_uom(self):
		...
		if not terminal_framework.db.exists("UOM Conversion Detail", {"parent": bom_item, "uom": "Nos"}):
			doc = terminal_framework.get_doc("Item", bom_item)
		...
		for row in terminal_framework.get_all(
			"Material Request Item",
			filters={"production_plan": pln.name},
	...
	def test_reserved_qty_for_production_plan_for_material_requests(self):
		from terminal_erp.stock.utils import get_or_make_bin

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		before_qty = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))

		pln = create_production_plan(item_code="Test Production Item 1")

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		after_qty = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_reserved_qty_for_production_plan_for_work_order(self):
		...
		work_orders = []
		for row in terminal_framework.get_all("Work Order", filters={"production_plan": pln.name}, fields=["name"]):
			wo_doc = terminal_framework.get_doc("Work Order", row.name)
		...
		if rm_work_order:
			s = terminal_framework.get_doc(make_se_from_wo(rm_work_order.name, "Material Transfer for Manufacture", 1))
			s.submit()
			bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
			after_qty = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_reserved_qty_for_production_plan_for_material_requests_with_multi_UOM(self):
		...
		for row in pln.mr_items:
			...
			reserved_qty = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
			self.assertEqual(reserved_qty - before_qty, 100.0)

		pln.submit_material_request = 1
		pln.make_work_order()

		for work_order in terminal_framework.get_all(
			"Work Order",
			fields=["name"],
			filters={"production_plan": pln.name},
		):
			wo_doc = terminal_framework.get_doc("Work Order", work_order.name)
			wo_doc.source_warehouse = "_Test Warehouse - _TC"
			wo_doc.wip_warehouse = "_Test Warehouse 1 - _TC"
			wo_doc.fg_warehouse = "_Test Warehouse - _TC"
			wo_doc.submit()

		reserved_qty_after_mr = flt(terminal_framework.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_transfer_and_purchase_mrp_for_purchase_uom(self):
		...
		parent_bom = create_nested_bom(bom_tree, prefix="")
		if not terminal_framework.db.exists("UOM Conversion Detail", {"parent": "Test RM Item INK", "uom": "Kg"}):
			doc = terminal_framework.get_doc("Item", "Test RM Item INK")
			doc.purchase_uom = "Kg"
	...
	def test_mr_qty_for_complex_bom(self):
		...
		def set_bom_qty(item_code, qtys):
			...
			bom = terminal_framework.get_doc("BOM", {"item": item_code})
	...
	def test_reserve_sub_assembly_items(self):
		...
		for d in work_orders:
			wo_doc = terminal_framework.get_doc("Work Order", d.name)
			...
			if d.production_item == "Frame Assembly":
				...
				se_doc = terminal_framework.get_doc(make_se_from_wo(wo_doc.name, "Manufacture", 2))
				se_doc.submit()
	...
	def test_calculation_of_sub_assembly_items(self):
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": terminal_framework.db.get_value("Item", "_Test FG Item 2", "default_bom"),
				"planned_qty": 50,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": terminal_framework.db.get_value("Item", "_Test FG Item 3", "default_bom"),
				"planned_qty": 10,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": terminal_framework.db.get_value("Item", "_Test FG Item 4", "default_bom"),
				"planned_qty": 10,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_stock_reservation_against_production_plan(self):
		...
		work_orders = terminal_framework.get_all("Work Order", filters={"production_plan": plan.name}, pluck="name")
	...
	def test_stock_reservation_restored_on_work_order_cancel(self):
		...
		try:
			...
			for wo_name in work_orders:
				terminal_framework.get_doc("Work Order", wo_name).cancel()
	...
	def test_stock_reservation_of_serial_nos_against_production_plan(self):
		...
		for item_code in [
			...
			doc = terminal_framework.get_doc("Item", item_code)
			doc.has_serial_no = 1
		...
		serial_nos_res_for_pp = terminal_framework.get_all(
		...
		for wo_name in list(set(work_orders)):
			...
			sre = StockReservation(wo_doc)
			reserved_entries = sre.get_reserved_entries("Work Order", wo_doc.name)
			serial_nos_res_for_wo = terminal_framework.get_all(
				"Serial and Batch Entry",
				filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
				pluck="serial_no",
			)
	...
	def test_stock_reservation_of_batch_nos_against_production_plan(self):
		...
		reserved_entries = sre.get_reserved_entries("Production Plan", plan.name)
		self.assertEqual(len(reserved_entries), 9)
		batches_reserved_for_pp = terminal_framework.get_all(
			"Serial and Batch Entry",
			filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
			pluck="batch_no",
		)
		...
		for wo_name in list(set(work_orders)):
			...
			sre = StockReservation(wo_doc)
			reserved_entries = sre.get_reserved_entries("Work Order", wo_doc.name)
			batches_reserved_for_wo = terminal_framework.get_all(
				"Serial and Batch Entry",
				filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
				pluck="batch_no",
			)
	...
	def test_production_plan_for_partial_sub_assembly_items(self):
		...
		terminal_framework.flags.test_print = False
		...
		work_order = terminal_framework.db.get_value("Work Order", {"production_plan": plan.name, "docstatus": 0}, "name")
		wo_doc = terminal_framework.get_doc("Work Order", work_order)
	...
	def test_phantom_bom_explosion_across_multiple_po_items(self):
		...
		plan = terminal_framework.new_doc("Production Plan")
	...
	def test_sub_assembly_rm_query_keeps_bom_no_phantom_pair_coherent(self):
		...
		"""
		from terminal_erp.manufacturing.doctype.production_plan.services.sub_assembly_queries import (
		...
		component_doc = terminal_framework.get_doc("Item", component)
		sa_bom.append(
			"items",
			{
				"item_code": component,
	...
	def test_subitems_query_keeps_real_rm_listed_alongside_phantom(self):
		...
		"""
		from terminal_erp.manufacturing.doctype.production_plan.services.bom_explosion import _subitems_query
		...
		terminal_framework.db.set_value("Item", component, "default_bom", "")
		terminal_framework.clear_document_cache("Item", component)
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": terminal_framework.db.get_value("Item", args.item_code, "default_bom"),
...
def make_bom(**args):
	...
	if args.set_as_default_bom and not args.do_not_save and not args.do_not_submit:
		terminal_framework.set_value("Item", args.item, "default_bom", bom.name)
