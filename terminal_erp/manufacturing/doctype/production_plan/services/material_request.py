...
from terminal_framework.utils import add_days, ceil, cint, comma_and, flt, get_link_to_form, nowdate
from terminal_framework.utils.csvutils import build_csv_response

from terminal_erp.manufacturing.doctype.production_plan.services.bom_explosion import (
	get_exploded_items,
	get_subitems,
)
from terminal_erp.manufacturing.doctype.production_plan.services.planning_queries import (
	get_bin_details,
	get_item_data,
	get_sales_orders,
...
class MaterialRequestService:
	...
	def validate_mr_subcontracted(self):
		for row in self.doc.mr_items:
			if row.material_request_type != "Subcontracting":
				continue
			if not terminal_framework.db.get_value("Item", row.item_code, "is_sub_contracted_item"):
	...
	def _add_item_to_material_request(self, item, material_request_map, material_request_list):
		item_doc = terminal_framework.get_cached_doc("Item", item.item_code)
	...
	def _material_request_item(self, item, material_request_type, schedule_date):
		...
		project = (
			terminal_framework.db.get_value("Sales Order", item.sales_order, "project") if item.sales_order else None
		)
		return {
	...
	def _submit_material_requests(self, material_request_list):
		...
		terminal_framework.flags.mute_messages = False
...
def get_items_for_material_requests(
	doc: str | terminal_framework._dict | Document,
	warehouses: str | list | None = None,
	get_parent_warehouse_data: bool | int | None = None,
):
	terminal_framework.has_permission("Production Plan", "read", throw=True)
...
def _normalize_mr_doc(doc):
	doc = terminal_framework._dict(terminal_framework.parse_json(doc))
	return doc
...
def _collect_po_items(doc):
	...
	for sa_row in doc.get("sub_assembly_items") or []:
		sa_row = terminal_framework._dict(sa_row)
...
def _collect_item_details(doc, po_items):
	...
	so_item_details = terminal_framework._dict()
	qty_precision = terminal_framework.get_precision("Material Request Plan Item", "quantity")
...
def _bom_item_details(doc, data, company, planned_qty, sub_assembly_items, existing_sub_assembly_items):
	...
	if not planned_qty:
		terminal_framework.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
	if not bom_no:
		return {}
...
def _plain_item_details(doc, data, planned_qty):
	item_master = terminal_framework.get_doc("Item", data["item_code"]).as_dict()
	purchase_uom = item_master.purchase_uom or item_master.stock_uom
	conversion_factor = (
		get_uom_conversion_factor(item_master.name, purchase_uom) if item_master.purchase_uom else 1.0
	)
	return {
		item_master.item_code: terminal_framework._dict(
			{
...
def _accumulate_so_items(so_item_details, sales_order, item_details, qty_precision):
	for key, details in item_details.items():
		...
		so_item_details.setdefault(sales_order, terminal_framework._dict())
...
def _warn_no_mr_items(doc):
	to_enable = terminal_framework.bold(terminal_framework.get_meta("Production Plan").get_field("ignore_existing_ordered_qty").label)
	warehouse = terminal_framework.bold(doc.get("for_warehouse"))
	message = (
		_(
	...
	terminal_framework.msgprint(message, title=_("Note"))
...
def _adjust_required_qty_for_uom(row, required_qty, include_safety_stock):
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or terminal_framework.flags.show_qty_in_stock_uom):
			terminal_framework.throw(
	...
	if terminal_framework.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
...
def _mr_purchase_conversion_factor(row):
	item_details = terminal_framework.get_cached_value("Item", row.item_code, ["purchase_uom", "stock_uom"], as_dict=1)
	if (
...
def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from terminal_erp.stock.doctype.pick_list.pick_list import get_available_item_locations
...
def _add_remaining_purchase_request(item, new_mr_items, required_qty):
	...
	precision = terminal_framework.get_precision("Material Request Plan Item", "quantity")
	if flt(required_qty, precision) <= 0:
		return

	purchase_uom = terminal_framework.db.get_value("Item", item.get("item_code"), "purchase_uom")
	if terminal_framework.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
		required_qty = ceil(required_qty)
...
def download_raw_materials(doc: str | dict | Document, warehouses: str | list | None = None):
	...
	doc.warehouse = None
	terminal_framework.flags.show_qty_in_stock_uom = 1
...
def _build_download_rows(doc, items, item_list):
	duplicate_item_wh_list = terminal_framework._dict()
	for d in items:
