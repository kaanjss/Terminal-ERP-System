...
from terminal_framework.utils import flt, now_datetime
from pypika.terms import ExistsCriterion

from terminal_erp.manufacturing.doctype.production_plan.services.planning_queries import get_sales_orders
...
class SalesOrderSourcingService:
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			terminal_framework.msgprint(_("Sales orders are not available for production"))
	...
	def _pending_mr_base_query(self, mr, mr_item):
		...
		bom_exists = ExistsCriterion(
			terminal_framework.qb.from_(bom)
			.select(bom.name)
	...
	def get_bom_item_condition(self):
		...
		has_bom = terminal_framework.db.exists({"doctype": "BOM", "item": self.doc.item_code, "docstatus": 1})

		if not has_bom:
			bom = terminal_framework.qb.DocType("BOM")
			template_item = terminal_framework.db.get_value("Item", self.doc.item_code, ["variant_of"])
	...
	def get_so_items(self):
		...
		if not self.doc.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			terminal_framework.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))
	...
	def _so_items(self, so_list):
		...
		items_subquery = terminal_framework.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
		items_query = (
		...
		if self.doc.item_code and terminal_framework.db.exists("Item", self.doc.item_code):
	...
	def _so_packed_items(self, so_list):
		bom = terminal_framework.qb.DocType("BOM")
		so_item = terminal_framework.qb.DocType("Sales Order Item")
		pi = terminal_framework.qb.DocType("Packed Item")
		query = (
			terminal_framework.qb.from_(so_item)
			.from_(pi)
	...
	def get_mr_items(self):
		...
		if not self.doc.get("material_requests") or not self.get_so_mr_list(
			...
			terminal_framework.throw(_("Please fill the Material Requests table"), title=_("Material Requests Required"))
...
def _so_packed_filter(bom, so_item, pi, so_list):
	bom_exists = ExistsCriterion(
		terminal_framework.qb.from_(bom).select(bom.name).where((bom.item == pi.item_code) & (bom.is_active == 1))
	)
...
def _mr_items_filter(bom, mr_item, mr_list):
	bom_exists = ExistsCriterion(
		terminal_framework.qb.from_(bom).select(bom.name).where((bom.item == mr_item.item_code) & (bom.is_active == 1))
	)
	return (
