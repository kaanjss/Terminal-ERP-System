...
from terminal_erp.manufacturing.doctype.bom.bom import get_children as get_bom_children
...
def get_sub_assembly_items(
	...
	precision = terminal_framework.get_precision("Production Plan Sub Assembly Item", "qty")
	parent_item_code = terminal_framework.get_cached_value("BOM", bom_no, "item")
...
def _sub_assembly_rm_query(company, bom_no, include_non_stock_items, planned_qty):
	bei = terminal_framework.qb.DocType("BOM Item")
	...
	item_uom = terminal_framework.qb.DocType("UOM Conversion Detail")
	return (
		terminal_framework.qb.from_(bei)
		.join(bom)
