...
class SubAssemblyService:
	...
	def get_sub_assembly_items(self, manufacturing_type: str | None = None):
		...
		bin_details = terminal_framework._dict()
	...
	def _bom_tracks_semi_finished(row):
		if not terminal_framework.db.get_value("BOM", row.bom_no, "track_semi_finished_goods"):
			return False
	...
	def _validate_sub_assembly_row(self, row):
		if self.doc.skip_available_sub_assembly_item and not self.doc.sub_assembly_warehouse:
			terminal_framework.throw(_("Row #{0}: Please select the Sub Assembly Warehouse").format(row.idx))
		if not row.item_code:
			terminal_framework.throw(_("Row #{0}: Please select Item Code in Assembly Items").format(row.idx))
		if not row.bom_no:
			terminal_framework.throw(_("Row #{0}: Please select the BOM No in Assembly Items").format(row.idx))
	...
	def set_sub_assembly_items_based_on_level(self, row, bom_data, manufacturing_type=None):
		...
		is_group_warehouse = terminal_framework.db.get_value("Warehouse", self.doc.sub_assembly_warehouse, "is_group")
	...
	def all_items_completed(self):
		...
		wo_status = terminal_framework.get_all(
			"Work Order",
			filters={
