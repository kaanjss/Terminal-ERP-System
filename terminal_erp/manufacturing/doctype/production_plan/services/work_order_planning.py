...
from terminal_framework.utils import flt, get_filtered_list_link, getdate, nowdate

from terminal_erp.manufacturing.doctype.production_plan.services.planning_queries import set_default_warehouses
...
class WorkOrderCreationService:
	...
	def _production_item_details(self, d):
		details = {
			...
			"source_warehouse": terminal_framework.get_value("BOM", d.bom_no, "default_source_warehouse"),
		}
		if not details["project"] and d.sales_order:
			details["project"] = terminal_framework.get_cached_value("Sales Order", d.sales_order, "project")
		return details
	...
	def make_work_order(self):
		from terminal_erp.manufacturing.doctype.work_order.work_order import get_default_warehouse
		...
		if not po_list:
			terminal_framework.msgprint(_("No Purchase Orders were created"))
	...
	def _sub_assembly_work_order(self, row, default_warehouses):
		...
		work_order_data = {
			"source_warehouse": terminal_framework.get_value("BOM", row.bom_no, "default_source_warehouse"),
