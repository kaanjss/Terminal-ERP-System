...
def get_bin_details(
	...
	row = terminal_framework._dict(terminal_framework.parse_json(row))
...
def _bin_warehouse_subquery(bin, company, row, for_warehouse, all_warehouse):
	...
	subquery = terminal_framework.qb.from_(wh).select(wh.name).where(wh.company == company)
...
def get_warehouse_list(warehouses):
	...
	warehouses = terminal_framework.parse_json(warehouses)

	for row in warehouses:
		child_warehouses = terminal_framework.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_sales_orders(self):
	...
	bom_subquery = terminal_framework.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
	query = _open_so_base_query(self, so, so_item)
	query = _apply_open_so_filters(self, query, so, so_item)

	if self.item_code and terminal_framework.db.exists("Item", self.item_code):
