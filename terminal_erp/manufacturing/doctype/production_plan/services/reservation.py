...
from terminal_framework.utils import flt, parse_json
...
def _production_plan_reserved_qty(item_code, warehouse, non_completed_production_plans):
	table = terminal_framework.qb.DocType("Production Plan")
	child = terminal_framework.qb.DocType("Material Request Plan Item")
	qty = (
...
def get_reserved_qty_for_sub_assembly(item_code, warehouse):
	table = terminal_framework.qb.DocType("Production Plan")
	child = terminal_framework.qb.DocType("Production Plan Sub Assembly Item")
...
def make_stock_reservation_entries(
	...
	terminal_framework.has_permission("Production Plan", "write", doc=doc, throw=True)
...
def _load_production_plan(doc: str | Document) -> Document:
	if isinstance(doc, str):
		doc = parse_json(doc)
		doc = terminal_framework.get_doc("Production Plan", doc.get("name"))
	return doc
