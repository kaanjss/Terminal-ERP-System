...
terminal_framework.ui.form.on("Production Plan", {
	before_save(frm) {
	...
	setup_queries(frm) {
		frm.set_query("sales_order", "sales_orders", () => {
			return {
				query: "terminal_erp.manufacturing.doctype.production_plan.production_plan.sales_order_query",
				filters: {
		...
		frm.set_query("bom_no", "po_items", (doc, cdt, cdn) => {
			...
			} else terminal_framework.msgprint(__("Please enter Item first"));
		});
	...
	refresh(frm) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				() => {
					terminal_framework.set_route("query-report", "Production Plan Summary", {
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://terminal_erp.com/docs/user/manual/en/stock/projected-quantity">
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "sub_assembly_items")) {
				frm.add_custom_button(
					...
					() => terminal_erp.stock_reservation.make_entries(frm, "sub_assembly_items"),
			...
			if (frm.events.has_reserved_stock(frm, "sub_assembly_items")) {
				...
				frm.add_custom_button(
					...
					() => terminal_erp.stock_reservation.show_reserved_stock(frm, "sub_assembly_items"),
					__("Stock Reservation")
				);
			}
		}
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "mr_items", "required_bom_qty")) {
				frm.add_custom_button(
					...
					() => terminal_erp.stock_reservation.make_entries(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
			...
			if (frm.events.has_reserved_stock(frm, "mr_items")) {
				...
				frm.add_custom_button(
					...
					() => terminal_erp.stock_reservation.show_reserved_stock(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	get_items_for_mr(frm) {
		if (!frm.doc.for_warehouse) {
			...
			terminal_framework.throw(__("Select the Warehouse"));
		}
	...
	get_items_for_material_requests(frm, warehouses) {
		terminal_framework.call({
			method: "terminal_erp.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
		...
		terminal_framework.prompt(
			...
			(row) => {
				let get_template_url =
					"terminal_erp.manufacturing.doctype.production_plan.production_plan.download_raw_materials";
				open_url_post(terminal_framework.request.url, {
...
terminal_framework.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.item_code) {
			terminal_framework.call({
				method: "terminal_erp.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
					item_code: row.item_code,
				},
				callback: function (r) {
					for (let key in r.message) {
						terminal_framework.model.set_value(cdt, cdn, key, r.message[key]);
					}
				},
			});
		}
...
terminal_framework.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.item_code && frm.doc.company) {
			terminal_framework.call({
				method: "terminal_erp.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
	...
	material_request_type(frm, cdt, cdn) {
		...
		if (row.from_warehouse && row.material_request_type !== "Material Transfer") {
			terminal_framework.model.set_value(cdt, cdn, "from_warehouse", "");
		}
	},
});

terminal_framework.ui.form.on("Production Plan Sales Order", {
	sales_order(frm, cdt, cdn) {
		...
		if (row.sales_order) {
			frm.call({
				...
				callback(r) {
					terminal_framework.call({
						method: "terminal_erp.manufacturing.doctype.production_plan.production_plan.get_so_details",
						args: { sales_order },
						callback(r) {
							const { transaction_date, customer, grand_total } = r.message;
							terminal_framework.model.set_value(cdt, cdn, "sales_order_date", transaction_date);
							terminal_framework.model.set_value(cdt, cdn, "customer", customer);
							terminal_framework.model.set_value(cdt, cdn, "grand_total", grand_total);
						},
					});
				},
			});
...
terminal_framework.ui.form.on("Production Plan Sub Assembly Item", {
	fg_warehouse(frm, cdt, cdn) {
		terminal_erp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "sub_assembly_items", "fg_warehouse");
			...
			terminal_framework.call({
				...
				callback: function (r) {
					if (r.message && r.message.length) {
						terminal_framework.model.set_value(cdt, cdn, "actual_qty", r.message[0].actual_qty);
						terminal_framework.model.set_value(cdt, cdn, "projected_qty", r.message[0].projected_qty);
					}
				},
			});
		}
...
terminal_framework.tour["Production Plan"] = [
	{
