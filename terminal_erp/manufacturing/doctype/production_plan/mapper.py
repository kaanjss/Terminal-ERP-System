...
def get_so_details(sales_order: str):
	terminal_framework.has_permission("Sales Order", "read", throw=True)
...
def sales_order_query(
	...
	terminal_framework.has_permission("Production Plan", throw=True)

	filters = filters or {}
	so_table = terminal_framework.qb.DocType("Sales Order")
	table = terminal_framework.qb.DocType("Sales Order Item")

	query = (
		terminal_framework.qb.from_(so_table)
		.join(table)
