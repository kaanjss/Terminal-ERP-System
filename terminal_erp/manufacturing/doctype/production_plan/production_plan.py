...
from terminal_erp.manufacturing.doctype.production_plan.mapper import (
	get_so_details,
	sales_order_query,
)
from terminal_erp.manufacturing.doctype.production_plan.services.material_request import (
...
from terminal_erp.manufacturing.doctype.production_plan.services.reservation import (
...
from terminal_erp.manufacturing.doctype.production_plan.services.sales_order_planning import (
	SalesOrderSourcingService,
)
from terminal_erp.manufacturing.doctype.production_plan.services.sub_assembly import (
	SubAssemblyService,
)
from terminal_erp.manufacturing.doctype.production_plan.services.work_order_planning import (
...
class ProductionPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.material_request_plan_item.material_request_plan_item import (
			MaterialRequestPlanItem,
		)
		from terminal_erp.manufacturing.doctype.production_plan_item.production_plan_item import ProductionPlanItem
		from terminal_erp.manufacturing.doctype.production_plan_item_reference.production_plan_item_reference import (
			ProductionPlanItemReference,
		)
		from terminal_erp.manufacturing.doctype.production_plan_material_request.production_plan_material_request import (
			ProductionPlanMaterialRequest,
		)
		from terminal_erp.manufacturing.doctype.production_plan_material_request_warehouse.production_plan_material_request_warehouse import (
			ProductionPlanMaterialRequestWarehouse,
		)
		from terminal_erp.manufacturing.doctype.production_plan_sales_order.production_plan_sales_order import (
			ProductionPlanSalesOrder,
		)
		from terminal_erp.manufacturing.doctype.production_plan_sub_assembly_item.production_plan_sub_assembly_item import (
	...
	def enable_auto_reserve_stock(self):
		if self.is_new() and terminal_framework.db.get_single_value("Stock Settings", "auto_reserve_stock"):
	...
	def validate_sales_orders(self, sales_order: str | None = None):
		...
		if not data and sales_orders:
			...
			terminal_framework.throw(msg, title=title)
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				terminal_framework.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				terminal_framework.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def add_reference_to_raw_materials(self):
		for item in self.mr_items:
			if reference := next(
				...
				and item.from_bom
				and item.main_item_code != terminal_framework.get_cached_value("BOM", item.from_bom, "item")
			):
				terminal_framework.throw(
					_(
	...
	def get_so_wise_planned_qty(sales_orders):
		so_wise_planned_qty = terminal_framework._dict()
	...
	def update_bin_qty(self):
		for d in self.mr_items:
			if d.warehouse:
				...
				bin = terminal_framework.get_doc("Bin", bin_name, for_update=True)
	...
	def delete_draft_work_order(self):
		for d in terminal_framework.get_all(
			...
			terminal_framework.delete_doc("Work Order", d.name)
