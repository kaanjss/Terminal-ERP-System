...
from terminal_framework.utils import add_days, getdate, parse_json
...
class MasterProductionSchedule(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.master_production_schedule_item.master_production_schedule_item import (
	...
	def validate_company(self):
		if self.sales_forecast:
			sales_forecast_company = terminal_framework.db.get_value("Sales Forecast", self.sales_forecast, "company")
	...
	def get_item_details(self, items):
		doctype = terminal_framework.qb.DocType("Item")
		...
		item_wise_details = terminal_framework._dict({})
	...
	def get_cumulative_lead_time(self, item_code, bom_no, time_in_days=0):
		...
		bom_materials = terminal_framework.get_all(
			"BOM Item",
	...
	def get_material_requests_data(self):
		...
		doctype = terminal_framework.qb.DocType("Material Request Item")
	...
	def get_sales_order_schedules(self):
		...
		query = terminal_framework.qb.from_(doctype).select(
			doctype.item_code,
			doctype.warehouse,
			doctype.stock_uom,
			doctype.delivery_date,
	...
	def get_item_wise_mps_data(self, data):
		...
		for item in data:
			...
			if key not in item_wise_data:
				item_wise_data[key] = terminal_framework._dict(
					{
	...
	def add_mps_data(self, data):
		data = terminal_framework._dict(sorted(data.items(), key=lambda x: x[0][1] or ""))
	...
	def get_material_requests(self, data):
		doctype = terminal_framework.qb.DocType("Material Request")
	...
	def get_sales_orders(self, kwargs):
		doctype = terminal_framework.qb.DocType("Sales Order")
		...
		if items := self.get_items_for_mps():
			doctype_item = terminal_framework.qb.DocType("Sales Order Item")
	...
	def enqueue_mrp_creation(self):
		terminal_framework.enqueue_doc("Master Production Schedule", self.name, "make_mrp", queue="long", timeout=1800)
