...
def get_mins_between_operations():
	return relativedelta(
		minutes=cint(terminal_framework.db.get_single_value("Manufacturing Settings", "mins_between_operations")) or 10
	)


@terminal_framework.whitelist()
def is_material_consumption_enabled():
	if not hasattr(terminal_framework.local, "material_consumption"):
		terminal_framework.local.material_consumption = cint(
			terminal_framework.db.get_single_value("Manufacturing Settings", "material_consumption")
		)

	return terminal_framework.local.material_consumption
