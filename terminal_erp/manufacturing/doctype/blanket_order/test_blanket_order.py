...
from terminal_framework.utils import add_months, today
...
class TestBlanketOrder(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_sales_order_creation(self):
		...
		terminal_framework.flags.args.doctype = "Sales Order"
		...
		bo = terminal_framework.get_doc("Blanket Order", bo.name)
	...
	def test_purchase_order_creation(self):
		...
		terminal_framework.flags.args.doctype = "Purchase Order"
	...
	def test_blanket_order_allowance(self):
		...
		terminal_framework.db.set_single_value("Selling Settings", "blanket_order_allowance", 10)
		so.submit()
		...
		po.schedule_date = today()
		po.items[0].qty = 110
		self.assertRaises(terminal_framework.ValidationError, po.submit)

		terminal_framework.db.set_single_value("Buying Settings", "blanket_order_allowance", 10)
		po.submit()
	...
	def test_blanket_order_over_order_aggregated_across_rows(self):
		...
		terminal_framework.db.set_single_value("Selling Settings", "blanket_order_allowance", 0)
		bo = make_blanket_order(blanket_order_type="Selling", quantity=100)
	...
	def test_party_item_code(self):
		...
		if not terminal_framework.db.exists("Item Customer Detail", {"customer_name": customer, "parent": item_code}):
			item_doc.append("customer_items", {"customer_name": customer, "ref_code": "CUST-REF-1"})
			item_doc.save()

		if not terminal_framework.db.exists("Item Supplier", {"supplier": supplier, "parent": item_code}):
...
def make_blanket_order(**args):
	...
	bo = terminal_framework.new_doc("Blanket Order")
