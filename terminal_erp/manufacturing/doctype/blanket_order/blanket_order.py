...
class BlanketOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.blanket_order_item.blanket_order_item import BlanketOrderItem
	...
	def validate_duplicate_items(self):
		...
		for item in self.items:
			if item.item_code in item_list:
				terminal_framework.throw(_("Note: Item {0} added multiple times").format(terminal_framework.bold(item.item_code)))
			item_list.append(item.item_code)

	def update_ordered_qty(self):
		ref_doctype = "Sales Order" if self.blanket_order_type == "Selling" else "Purchase Order"

		trans = terminal_framework.qb.DocType(ref_doctype)
		trans_item = terminal_framework.qb.DocType(f"{ref_doctype} Item")

		item_ordered_qty = terminal_framework._dict(
			(
				terminal_framework.qb.from_(trans_item)
				.from_(trans)
	...
	def validate_item_qty(self):
		for d in self.items:
			if flt(d.qty) < 0:
				terminal_framework.throw(_("Row {0}: Quantity cannot be negative.").format(d.idx))


@terminal_framework.whitelist()
def make_order(source_name: str):
	doctype = terminal_framework.flags.args.doctype
	...
	if order_doc.doctype in ("Sales Order", "Purchase Order"):
		...
		if order_data:
			...
			for bo_name, item_data in order_data.items():
				bo_doc = terminal_framework.get_doc("Blanket Order", bo_name)
