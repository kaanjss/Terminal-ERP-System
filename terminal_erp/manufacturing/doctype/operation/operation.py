...
class Operation(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.manufacturing.doctype.sub_operation.sub_operation import SubOperation
		...
		for row in self.sub_operations:
			if row.operation in operation_list:
				terminal_framework.throw(
					_("The operation {0} cannot be added multiple times").format(terminal_framework.bold(row.operation))
				)

			if self.name == row.operation:
				terminal_framework.throw(
					_("The operation {0} cannot be its own sub-operation").format(terminal_framework.bold(row.operation))
				)
