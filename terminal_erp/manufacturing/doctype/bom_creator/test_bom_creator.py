...
class TestBOMCreator(Terminal ERPTestSuite):
	def setUp(self) -> None:
		create_items()
	...
	def test_make_boms_from_bom_creator(self):
		...
		data = terminal_framework.get_all("BOM", filters={"bom_creator": doc.name, "docstatus": 1})
...
def make_bom_creator(**kwargs):
	...
	doc = terminal_framework.new_doc("BOM Creator")
	doc.update(kwargs)
	doc.save()
