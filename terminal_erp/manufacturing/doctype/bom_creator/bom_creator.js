...
terminal_framework.ui.form.on("BOM Creator", {
	setup(frm) {
	...
	setup_bom_creator(frm) {
		...
		if (!frm.is_new()) {
			if (!terminal_framework.bom_configurator || terminal_framework.bom_configurator.bom_configurator !== frm.doc.name) {
	...
	build_tree(frm) {
		...
		terminal_framework.require("bom_configurator.bundle.js").then(() => {
			terminal_framework.bom_configurator = new terminal_framework.ui.BOMConfigurator({
				wrapper: $parent,
				page: $parent,
				frm: frm,
		...
		let dialog = new terminal_framework.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Section Break", depends_on: "eval:!doc.is_phantom" },
				{
					...
					options: "Currency",
					reqd: 1,
					default: terminal_framework.defaults.get_global_default("currency"),
				},
				{ fieldtype: "Column Break" },
				{
			...
			primary_action: (values) => {
				terminal_framework.db.get_value("Item", values.item_code, "is_stock_item").then((r) => {
					if (r.message) {
						if (r.message.is_stock_item && values.is_phantom) {
							...
							terminal_framework.db.insert(values).then((doc) => {
	...
	set_root_item(frm) {
		if (frm.is_new() && frm.doc.items?.length) {
			terminal_framework.model.set_value(frm.doc.items[0].doctype, frm.doc.items[0].name, "is_root", 1);
		}
	},
...
terminal_framework.ui.form.on("BOM Creator Item", {
	item_code(frm, cdt, cdn) {
		let item = terminal_framework.get_doc(cdt, cdn);
		if (item.item_code && item.is_root) {
			terminal_framework.model.set_value(cdt, cdn, "fg_item", item.item_code);
		}
	},
	...
	do_not_explode(frm, cdt, cdn) {
		...
		if (!item.do_not_explode) {
			frm.call({
				...
				callback(r) {
					if (r.message) {
						terminal_framework.model.set_value(cdt, cdn, "bom_no", r.message);
					}
				},
			});
		} else {
			terminal_framework.model.set_value(cdt, cdn, "bom_no", "");
		}
	},
});

terminal_erp.bom.BomConfigurator = class BomConfigurator extends terminal_erp.TransactionController {
	conversion_rate(doc) {
...
extend_cscript(cur_frm.cscript, new terminal_erp.bom.BomConfigurator({ frm: cur_frm }));
