terminal_framework.pages["bom-comparison-tool"].on_page_load = function (wrapper) {
	...
	new terminal_erp.BOMComparisonTool(page);
};

terminal_erp.BOMComparisonTool = class BOMComparisonTool {
	constructor(page) {
		this.page = page;
		this.make_form();
	}
	...
	make_form() {
		this.form = new terminal_framework.ui.FieldGroup({
			fields: [
				{
					label: __("BOM 1"),
					fieldname: "name1",
	...
	fetch_and_render() {
		...
		terminal_framework
			.call("terminal_erp.manufacturing.doctype.bom.bom.get_bom_diff", {
				bom1: name1,
				bom2: name2,
			})
			.then((r) => {
				let diff = r.message;
				terminal_framework.model.with_doctype("BOM", () => {
	...
	render(doctype, name1, name2, diff) {
		let change_html = (title, doctype, changed) => {
			let values_changed = this.get_changed_values(doctype, changed)
				.map((change) => {
					...
					return `
						<tr>
							<td>${terminal_framework.meta.get_label(doctype, fieldname)}</td>
							<td>${terminal_framework.utils.escape_html(cstr(value1))}</td>
							<td>${terminal_framework.utils.escape_html(cstr(value2))}</td>
						</tr>
					`;
				})
				.join("");
		...
		let table_changes = Object.keys(row_changes_by_fieldname)
			.map((fieldname) => {
				...
				let df = terminal_framework.meta.get_docfield(doctype, fieldname);
					...
					.map((change) => {
						...
						return values_changed
							.map((change, i) => {
								...
								let th =
									i === 0
										? `<th rowspan="${values_changed.length}">${terminal_framework.utils.escape_html(
												cstr(item_code)
										  )}</th>`
										: "";
								return `
						<tr>
							${th}
							<td>${terminal_framework.meta.get_label(child_doctype, fieldname)}</td>
							<td>${terminal_framework.utils.escape_html(cstr(value1))}</td>
							<td>${terminal_framework.utils.escape_html(cstr(value2))}</td>
						</tr>
					`;
		...
		let get_added_removed_html = (title, grouped_items) => {
			return Object.keys(grouped_items)
				.map((fieldname) => {
					...
					let df = terminal_framework.meta.get_docfield(doctype, fieldname);
					let fields = terminal_framework.meta.get_docfields(df.options).filter((df) => df.in_list_view);

					let html = rows
						.map((row) => {
							let [, doc] = row;
							let cells = fields
								.map((df) => `<td>${terminal_framework.utils.escape_html(cstr(doc[df.fieldname]))}</td>`)
								.join("");
							return `<tr>${cells}</tr>`;
						})
						.join("");
