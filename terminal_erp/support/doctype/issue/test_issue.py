...
from terminal_framework.utils import flt, get_datetime

from terminal_erp.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestSetUp(Terminal ERPTestSuite):
	def setUp(self):
		terminal_framework.db.delete("Service Level Agreement")
		terminal_framework.db.delete("Service Level Priority")
		terminal_framework.db.delete("SLA Fulfilled On Status")
		terminal_framework.db.delete("Pause SLA On Status")
		terminal_framework.db.delete("Service Day")
...
class TestIssue(TestSetUp):
	def test_response_time_and_resolution_time_based_on_different_sla(self):
		...
		terminal_framework.flags.current_time = get_datetime("2019-03-04 15:00")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_hold_time_on_replied(self):
		...
		terminal_framework.flags.current_time = get_datetime("2020-03-04 4:15")
		issue.reload()
		issue.status = "Replied"
		issue.save()

		self.assertEqual(issue.on_hold_since, terminal_framework.flags.current_time)
		self.assertFalse(issue.sla_resolution_by)

		creation = get_datetime("2020-03-04 5:00")
		terminal_framework.flags.current_time = get_datetime("2020-03-04 5:00")
		create_communication(issue.name, "test@example.com", "Received", creation)
		...
		terminal_framework.flags.current_time = get_datetime("2020-03-04 5:05")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_close_after_on_hold(self):
		terminal_framework.flags.current_time = get_datetime("2021-11-01 19:00")

		issue = make_issue(terminal_framework.flags.current_time, index=1)
		create_communication(issue.name, "test@example.com", "Received", terminal_framework.flags.current_time)

		# send a reply within SLA
		terminal_framework.flags.current_time = get_datetime("2021-11-02 11:00")
		create_communication(issue.name, "test@admin.com", "Sent", terminal_framework.flags.current_time)
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-22 01:00")
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_open_after_closed(self):
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-01 13:00")
		issue = make_issue(
			terminal_framework.flags.current_time, index=1, issue_type="Critical"
		...
		# Replied on → 2 pm
		terminal_framework.flags.current_time = get_datetime("2021-11-01 14:00")
		create_communication(issue.name, "test@admin.com", "Sent", terminal_framework.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		...
		self.assertEqual(issue.on_hold_since, terminal_framework.flags.current_time)
		self.assertEqual(issue.first_responded_on, terminal_framework.flags.current_time)

		# Customer Replied → 3 pm
		terminal_framework.flags.current_time = get_datetime("2021-11-01 15:00")
		create_communication(issue.name, "test@example.com", "Received", terminal_framework.flags.current_time)
		issue.reload()
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-01 16:00")
		create_communication(issue.name, "test@admin.com", "Sent", terminal_framework.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-01 22:00")
		issue.status = "Closed"
		issue.save()
		# Hold Time + 6 Hrs
		...
		self.assertEqual(issue.agreement_status, "Fulfilled")
		self.assertEqual(issue.sla_resolution_date, terminal_framework.flags.current_time)

		# Customer Open → 3 am i.e after resolution by is crossed
		terminal_framework.flags.current_time = get_datetime("2021-11-02 03:00")
		create_communication(issue.name, "test@example.com", "Received", terminal_framework.flags.current_time)
		issue.reload()
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-02 04:00")
		issue.status = "Closed"
		issue.save()
		self.assertEqual(issue.sla_resolution_by, get_datetime("2021-11-02 07:00"))
		self.assertEqual(issue.agreement_status, "Fulfilled")
	...
	def test_recording_of_assignment_on_first_reponse_failure(self):
		from terminal_framework.desk.form.assign_to import add as add_assignment
		...
		terminal_framework.flags.current_time = get_datetime("2021-11-02 15:00")
		create_communication(issue.name, "test@admin.com", "Sent", terminal_framework.flags.current_time)

		# assert if a new timeline item has been added
		# to record the assignment
		comment = terminal_framework.db.exists(
			"Comment",
			{
...
class TestFirstResponseTime(TestSetUp):
	...
	def _get_no_perm_user(self):
		...
		if not terminal_framework.db.exists("User", email):
	...
	def test_set_status_requires_write_permission(self):
		from terminal_erp.support.doctype.issue.issue import set_status

		issue = terminal_framework.new_doc("Issue")
		issue.subject = "_Test Permission Issue"
		issue.insert(ignore_permissions=True)
		terminal_framework.set_user(self._get_no_perm_user())
		self.assertRaises(terminal_framework.PermissionError, set_status, issue.name, "Closed")
		terminal_framework.set_user("Administrator")

	def test_set_multiple_status_requires_write_permission(self):
		import json

		from terminal_erp.support.doctype.issue.issue import set_multiple_status

		issue = terminal_framework.new_doc("Issue")
		issue.subject = "_Test Permission Issue"
		issue.insert(ignore_permissions=True)
		terminal_framework.set_user(self._get_no_perm_user())
		self.assertRaises(terminal_framework.PermissionError, set_multiple_status, json.dumps([issue.name]), "Closed")
		terminal_framework.set_user("Administrator")
...
def create_issue_and_communication(issue_creation, first_responded_on):
	...
	terminal_framework.flags.current_time = first_responded_on
...
def make_issue(creation=None, customer=None, index=0, priority=None, issue_type=None):
	if issue_type and not terminal_framework.db.exists("Issue Type", issue_type):
		doc = terminal_framework.new_doc("Issue Type")
		doc.name = issue_type
		doc.insert()
...
def create_customer(name, customer_group, territory):
	...
	if not terminal_framework.db.exists("Customer", {"customer_name": name}):
...
def create_customer_group(customer_group):
	if not terminal_framework.db.exists("Customer Group", {"customer_group_name": customer_group}):
		terminal_framework.get_doc({"doctype": "Customer Group", "customer_group_name": customer_group}).insert(
			ignore_permissions=True
		)


def create_territory(territory):
	if not terminal_framework.db.exists("Territory", {"territory_name": territory}):
		terminal_framework.get_doc(
			{
				"doctype": "Territory",
...
def create_communication(reference_name, sender, sent_or_received, creation):
	communication = terminal_framework.get_doc(
		{
