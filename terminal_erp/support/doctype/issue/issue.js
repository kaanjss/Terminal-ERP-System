terminal_framework.ui.form.on("Issue", {
	onload: function (frm) {
			...
			frm.add_custom_button(
				...
				function () {
					terminal_framework.model.open_mapped_doc({
						method: "terminal_erp.support.doctype.issue.issue.make_task",
						frm: frm,
					});
				},
				__("Create")
	...
	reset_service_level_agreement: function (frm) {
		let reset_sla = new terminal_framework.ui.Dialog({
			...
			primary_action: (values) => {
				...
				terminal_framework.call(
					"terminal_erp.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement",
					{
						reason: values.reason,
						user: terminal_framework.session.user_email,
					...
					() => {
						...
						frm.refresh();
						terminal_framework.msgprint(__("Service Level Agreement was reset."));
					}
				);
			},
		});
	...
	timeline_refresh: function (frm) {
		if (!frm.timeline.wrapper.find(".btn-split-issue").length) {
			let split_issue_btn = $(`
				<a class="action-btn btn-split-issue" title="${__("Split Issue")}">
					${terminal_framework.utils.icon("branch", "sm")}
				</a>
			`);
		...
		// if (terminal_framework.model.can_create("Help Article")) {
		...
		// 			${terminal_framework.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = terminal_framework.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			terminal_framework.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
