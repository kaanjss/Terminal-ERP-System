...
from terminal_framework.utils.caching import redis_cache
from terminal_framework.utils.nestedset import get_ancestors_of
from terminal_framework.utils.safe_exec import get_safe_globals

from terminal_erp.support.doctype.issue.issue import calculate_first_response_time, get_holidays
...
class ServiceLevelAgreement(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.support.doctype.pause_sla_on_status.pause_sla_on_status import PauseSLAOnStatus
		from terminal_erp.support.doctype.service_day.service_day import ServiceDay
		from terminal_erp.support.doctype.service_level_priority.service_level_priority import (
			ServiceLevelPriority,
		)
		from terminal_erp.support.doctype.sla_fulfilled_on_status.sla_fulfilled_on_status import (
	...
	def check_priorities(self):
		...
		if len(set(priorities)) != len(priorities):
			...
			terminal_framework.throw(_("Priority {0} has been repeated.").format(repeated_priority))

		# set default priority from priorities
		try:
			self.default_priority = next(d.priority for d in self.priorities if d.default_priority)
		except Exception:
			terminal_framework.throw(_("Select a Default Priority."))
	...
	def check_support_and_resolution(self):
		...
		if len(set(support_days)) != len(support_days):
			...
			terminal_framework.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			self.enabled
			and self.document_type == "Issue"
			and not terminal_framework.db.get_single_value("Support Settings", "track_service_level_agreement")
		):
			terminal_framework.throw(
				_("{0} is not enabled in {1}").format(
					terminal_framework.bold(_("Track Service Level Agreement")),
					get_link_to_form("Support Settings", "Support Settings"),
				)
			)

		if self.default_service_level_agreement and terminal_framework.db.exists(
		...
		if (
			self.entity_type
			and self.entity
			and terminal_framework.db.exists(
				"Service Level Agreement",
			...
			terminal_framework.throw(
				_("Service Level Agreement for {0} {1} already exists.").format(
					terminal_framework.bold(self.entity_type), terminal_framework.bold(self.entity)
				)
			)

	def validate_selected_doctype(self):
		invalid_doctypes = list(terminal_framework.model.core_doctypes_list)
		invalid_doctypes.extend(["Cost Center", "Company"])
		valid_document_types = terminal_framework.get_all(
			"DocType",
			{
				"issingle": 0,
				"istable": 0,
		...
		if self.document_type not in valid_document_types:
			terminal_framework.throw(msg=_("Please select a valid document type."), title=_("Invalid Document Type"))

	def validate_status_field(self):
		meta = terminal_framework.get_meta(self.document_type)
		if not meta.get_field("status"):
			terminal_framework.throw(
				_(
					"The Document Type {0} must have a Status field to configure Service Level Agreement"
				).format(terminal_framework.bold(self.document_type))
			)

	def validate_condition(self):
		temp_doc = terminal_framework.new_doc(self.document_type)
		if self.condition:
			try:
				terminal_framework.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				terminal_framework.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_service_level_agreement_priority(self, priority):
		priority = terminal_framework.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
	...
	def before_insert(self):
		...
		meta = terminal_framework.get_meta(self.document_type, cached=False)
	...
	def create_docfields(self, meta, service_level_agreement_fields):
		...
		terminal_framework.get_doc("DocType", self.document_type).save(ignore_permissions=True)
	...
	def reset_field_properties(self, field, field_dt, sla_field):
		field = terminal_framework.get_doc(field_dt, {"fieldname": field.fieldname})
...
def check_agreement_status():
	service_level_agreements = terminal_framework.get_all(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		...
		if doc.end_date and getdate(doc.end_date) < getdate(terminal_framework.utils.getdate()):
			terminal_framework.db.set_value("Service Level Agreement", service_level_agreement.name, "enabled", 0)


def get_active_service_level_agreement_for(doc):
	if not terminal_framework.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	default_sla = terminal_framework.get_all(
		"Service Level Agreement",
	...
	agreements = terminal_framework.get_all(
		"Service Level Agreement",
		filters=filters,
	...
	for agreement in agreements:
		...
		if not condition or (condition and terminal_framework.safe_eval(condition, None, get_context(doc))):
...
def get_context(doc):
	return {
		"doc": doc.as_dict(),
		"nowdate": nowdate,
		"terminal_framework": terminal_framework._dict(utils=get_safe_globals().get("terminal_framework").get("utils")),
	}


def get_customer_group(customer):
	customer_groups = []
	customer_group = terminal_framework.db.get_value("Customer", customer, "customer_group") if customer else None
	if customer_group:
...
def get_customer_territory(customer):
	...
	customer_territory = terminal_framework.db.get_value("Customer", customer, "territory") if customer else None
...
def get_service_level_agreement_filters(doctype: str, name: str, customer: str | None = None):
	...
	return {
		"priority": [
			priority.priority
			for priority in terminal_framework.get_all(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name for d in terminal_framework.get_all("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_service_level_agreement():
	sla_doctypes = terminal_framework.cache.get_value("doctypes_with_active_sla")
...
def set_documents_with_active_service_level_agreement():
	try:
		...
		terminal_framework.cache.set_value("doctypes_with_active_sla", active)
	except (terminal_framework.DoesNotExistError, terminal_framework.db.TableMissingError):
...
def handle_status_change(doc, apply_sla_for_resolution):
	now_time = terminal_framework.flags.current_time or now_datetime(doc.get("owner"))
	prev_status = terminal_framework.db.get_value(doc.doctype, doc.name, "status")
...
def get_fulfillment_statuses(service_level_agreement):
	return [
		entry.status
		for entry in terminal_framework.db.get_all(
...
def get_allotted_seconds(parameter, service_level):
	...
	if parameter == "response":
		...
		terminal_framework.throw(_("{0} parameter is invalid").format(parameter))
...
def get_support_days(service_level):
	...
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = terminal_framework._dict(
			{
...
def change_service_level_agreement_and_priority(self):
	if (
		...
		and terminal_framework.db.exists("Issue", self.name)
		and terminal_framework.db.get_single_value("Support Settings", "track_service_level_agreement")
	):
		if self.priority != terminal_framework.db.get_value("Issue", self.name, "priority"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
			terminal_framework.msgprint(_("Priority has been changed to {0}.").format(self.priority))

		if self.service_level_agreement != terminal_framework.db.get_value("Issue", self.name, "service_level_agreement"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
...
def get_response_and_resolution_duration(doc):
	sla = terminal_framework.get_doc("Service Level Agreement", doc.service_level_agreement)
...
def reset_service_level_agreement(doctype: str, docname: str, reason: str, user: str):
	if not terminal_framework.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
		terminal_framework.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = terminal_framework.db.get_value(
...
def record_assigned_users_on_failure(doc):
	...
	if assigned_users:
		from terminal_framework.utils import get_fullname
...
def get_tz(user):
	return terminal_framework.db.get_value("User", user, "time_zone") or get_system_timezone()
...
def get_sla_doctypes():
	doctypes = []
	data = terminal_framework.get_all("Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
