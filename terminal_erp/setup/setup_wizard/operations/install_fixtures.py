...
from terminal_framework.desk.doctype.global_search_settings.global_search_settings import (
...
from terminal_erp.accounts.doctype.account.account import RootNotEditable
from terminal_erp.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	response = terminal_framework.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))
	...
	from terminal_erp.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_buying_defaults():
	buying_settings = terminal_framework.get_doc("Buying Settings")
...
def add_uom_data():
	...
	for d in uoms:
		if d.get("category") and not terminal_framework.db.exists("UOM Category", d.get("category")):
			terminal_framework.get_doc({"doctype": "UOM Category", "category_name": d.get("category")}).db_insert()

		if not terminal_framework.db.exists("UOM", d.get("uom_name")):
			doc = terminal_framework.new_doc("UOM")
			doc.update(d)
	...
	uom_conversions = json.loads(
		open(
			terminal_framework.get_app_path("terminal_erp", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
...
def add_sale_stages():
	...
	for sales_stage in records:
		terminal_framework.get_doc(sales_stage).db_insert()
...
def install_defaults(args=None):  # nosemgrep
	...
	terminal_framework.db.set_value("Currency", args.get("currency"), "enabled", 1)
	terminal_framework.db.set_single_value("Stock Settings", "email_footer_address", args.get("company_name"))
...
def set_global_defaults(kwargs):
	global_defaults = terminal_framework.get_doc("Global Defaults", "Global Defaults")
	company = terminal_framework.db.get_value(
		"Company",
...
def create_bank_account(args, demo=False):
	...
	bank_account_group = terminal_framework.db.get_value(
		"Account",
		{"account_type": "Bank", "is_group": 1, "root_type": "Asset", "company": company_name},
	)
	if bank_account_group:
		bank_account = terminal_framework.get_doc(
			{
				"doctype": "Account",
		...
		try:
			terminal_framework.db.savepoint("create_bank_account")
			...
			terminal_framework.throw(terminal_framework._("Bank account cannot be named as {0}").format(args.get("bank_account")))
		except terminal_framework.DuplicateEntryError:
			terminal_framework.db.rollback(save_point="create_bank_account")  # preserve transaction in postgres
