...
def setup_taxes_and_charges(company_name: str, country: str):
	if not terminal_framework.db.exists("Company", company_name):
		terminal_framework.throw(_("Company {0} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	if (
		terminal_framework.db.get_value("Company", company_name, "create_chart_of_accounts_based_on")
		== "Existing Company"
	):
		charts_company_name = terminal_framework.db.get_value("Company", company_name, "existing_company")
	coa_name = terminal_framework.db.get_value("Company", charts_company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	path = terminal_framework.get_app_path("terminal_erp", "regional", terminal_framework.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		terminal_framework.db.savepoint("regional_tax_settings")
		try:
			module_name = f"terminal_erp.regional.{terminal_framework.scrub(country)}.setup.update_regional_tax_settings"
			terminal_framework.get_attr(module_name)(country, company)
		except (ImportError, AttributeError):
			pass
		except Exception:
			# Log error and ignore if failed to setup regional tax settings
			terminal_framework.db.rollback(save_point="regional_tax_settings")
			terminal_framework.log_error("Unable to setup regional tax settings")
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if terminal_framework.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": terminal_framework.db.get_value("Company", company_name, "cost_center"),
		}
	...
	doc = terminal_framework.get_doc(template)
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = terminal_framework.get_all(
		"Account", filters={"company": company_name, "root_type": root_type}, or_filters=or_filters
	)

	if existing_accounts:
		return terminal_framework.get_doc("Account", existing_accounts[0].name)
	...
	doc = terminal_framework.get_doc(account)
	doc.flags.ignore_links = True
	doc.flags.ignore_validate = True
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = terminal_framework.db.get_value(
		"Account",
	...
	root_account = terminal_framework.get_all(
		"Account",
		{
			"is_group": 1,
			"root_type": root_type,
	...
	tax_group_account = terminal_framework.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
...
def make_tax_category(tax_category):
	...
	if not terminal_framework.db.exists(doctype, tax_category["title"]):
		doc = terminal_framework.get_doc(tax_category)
