...
from terminal_framework.utils import add_days, flt, get_datetime_str, nowdate
...
def get_pegged_currencies():
	pegged_currencies = terminal_framework.get_all(
...
def get_exchange_rate(
	...
	currency_settings = terminal_framework.get_cached_doc("Accounts Settings")
	...
	if terminal_framework.get_single_value("Currency Exchange Settings", "disabled"):
		return 0.00
	...
	try:
		...
		if not value:
			...
			settings = terminal_framework.get_cached_doc("Currency Exchange Settings")
			req_params = {
		...
		terminal_framework.log_error("Unable to fetch exchange rate")
...
def _enable_all_roles_for_admin():
	...
	all_roles = set(terminal_framework.get_all("Role", pluck="name"))
	admin_roles = set(
		terminal_framework.db.get_values("Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role")
	)
...
def set_defaults_for_tests():
	...
	terminal_framework.db.set_single_value("Selling Settings", defaults)
	for key, value in defaults.items():
		terminal_framework.db.set_default(key, value)
...
def welcome_email():
	site_name = get_default_company() or "Terminal ERP"
...
def identity(x, *args, **kwargs):
	...
	Use like this: `from terminal_erp.setup.utils import identity as _`
	"""
	return x
