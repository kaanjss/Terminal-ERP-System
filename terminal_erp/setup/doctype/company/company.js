...
terminal_framework.provide("terminal_erp.company");

terminal_framework.ui.form.on("Company", {
	onload: function (frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			terminal_framework.db.get_value("Company", frm.doc.parent_company, "is_group", (r) => {
				if (!r.is_group) {
	...
	date_of_commencement: function (frm) {
		if (frm.doc.date_of_commencement < frm.doc.date_of_incorporation) {
			terminal_framework.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function (frm) {
		terminal_erp.company.setup_queries(frm);
		...
		if (!frm.is_new()) {
			...
			if (terminal_framework.perm.has_perm("Cost Center", 0, "read")) {
				frm.add_custom_button(
					__("Cost Centers"),
					function () {
						terminal_framework.set_route("Tree", "Cost Center", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (terminal_framework.perm.has_perm("Account", 0, "read")) {
				frm.add_custom_button(
					__("Chart of Accounts"),
					function () {
						terminal_framework.set_route("Tree", "Account", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (terminal_framework.perm.has_perm("Sales Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Sales Tax Template"),
					function () {
						terminal_framework.set_route("List", "Sales Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
			...
			if (terminal_framework.perm.has_perm("Purchase Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Purchase Tax Template"),
					function () {
						terminal_framework.set_route("List", "Purchase Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
			...
			if (terminal_framework.user.has_role("System Manager")) {
				if (frm.has_perm("write")) {
					frm.add_custom_button(
		...
		terminal_erp.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function (frm) {
		frm.call({
			...
			freeze: true,
			callback: function () {
				terminal_framework.msgprint(__("Default tax templates for sales, purchase and items are created."));
			},
		});
	},
	...
	delete_company_transactions: function (frm) {
		terminal_framework.call({
			method: "terminal_erp.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				if (!r.exc) {
					terminal_framework.verify_password(function () {
						var d = terminal_framework.prompt(
							{
								fieldtype: "Data",
								...
								description: __(
									...
									[terminal_framework.utils.bold(frm.doc.name)]
								),
							},
							function (data) {
								if (data.company_name !== frm.doc.name) {
									terminal_framework.msgprint(__("Company name does not match"));
									return;
								}
								terminal_framework.call({
									method: "terminal_erp.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
									...
									onerror: function () {
										terminal_framework.msgprint(__("Wrong Password"));
									},
								});
							},
							__("Delete all the Transactions for {0}", [terminal_framework.utils.bold(frm.doc.name)]),
							__("Delete")
						);
...
terminal_erp.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return terminal_framework.call({
			method: "terminal_erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
terminal_erp.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			terminal_erp.company.set_custom_query(frm, v);
		}
	);
...
terminal_erp.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
