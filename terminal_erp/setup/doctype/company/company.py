...
from terminal_framework.cache_manager import clear_defaults_cache
from terminal_framework.contacts.address_and_contact import load_address_and_contact
from terminal_framework.custom.doctype.property_setter.property_setter import make_property_setter
from terminal_framework.desk.page.setup_wizard.setup_wizard import make_records
...
from terminal_framework.utils.nestedset import NestedSet, rebuild_tree

from terminal_erp.accounts.doctype.account.account import get_account_currency
from terminal_erp.accounts.doctype.financial_report_template.financial_report_template import (
	sync_financial_report_templates,
)
from terminal_erp.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
from terminal_erp.stock.utils import check_pending_reposting
...
class Company(NestedSet):
	...
	def check_if_transactions_exist(self):
		...
		for doctype in [
			...
			if terminal_framework.db.exists(doctype, {"company": self.name, "docstatus": 1}):
				exists = True
				break
	...
	def cant_change_valuation_method(self):
		...
		if previous_valuation_method and previous_valuation_method != self.valuation_method:
			...
			sle_dt = terminal_framework.qb.DocType("Stock Ledger Entry")
			item = terminal_framework.qb.DocType("Item")
			sle = (
				terminal_framework.qb.from_(sle_dt)
				.select(sle_dt.name)
				.where(
	...
	def validate_inventory_account_settings(self):
		...
		if (
			...
			and terminal_framework.db.get_value("Stock Ledger Entry", {"is_cancelled": 0, "company": self.name}, "name")
	...
	def validate_abbr(self):
		...
		if not self.abbr.strip():
			terminal_framework.throw(_("Abbreviation is mandatory"))

		if terminal_framework.db.exists("Company", {"name": ["!=", self.name], "abbr": self.abbr}):
			terminal_framework.throw(_("Abbreviation already used for another company"))
	...
	def validate_default_accounts(self):
		...
		for account in accounts:
			if self.get(account[1]):
				for_company, is_group, disabled = terminal_framework.db.get_value(
					"Account", self.get(account[1]), ["company", "is_group", "disabled"]
				)

				if disabled:
					terminal_framework.throw(_("Account {0} is disabled.").format(terminal_framework.bold(self.get(account[1]))))

				if is_group:
					terminal_framework.throw(
						_("{0}: {1} is a group account.").format(
							terminal_framework.bold(account[0]), terminal_framework.bold(self.get(account[1]))
						)
					)
				...
				if get_account_currency(self.get(account[1])) != self.default_currency:
					...
					).format(terminal_framework.bold(account[0]))
					terminal_framework.throw(error_message)

	def validate_advance_account_currency(self):
		if (
			self.default_advance_received_account
			and terminal_framework.get_cached_value("Account", self.default_advance_received_account, "account_currency")
			!= self.default_currency
		):
			terminal_framework.throw(
				_("'{0}' should be in company currency {1}.").format(
					terminal_framework.bold(_("Default Advance Received Account")), terminal_framework.bold(self.default_currency)
				)
			)

		if (
			self.default_advance_paid_account
			and terminal_framework.get_cached_value("Account", self.default_advance_paid_account, "account_currency")
			!= self.default_currency
		):
			terminal_framework.throw(
				_("'{0}' should be in company currency {1}.").format(
					terminal_framework.bold(_("Default Advance Paid Account")), terminal_framework.bold(self.default_currency)
				)
			)
	...
	def validate_currency(self):
		if self.is_new():
			return
		self.previous_default_currency = terminal_framework.get_cached_value("Company", self.name, "default_currency")
		if (
			self.default_currency
	...
	def on_update(self):
		...
		if not terminal_framework.db.exists("Account", {"company": self.name, "docstatus": ["<", 2]}):
			if not terminal_framework.local.flags.ignore_chart_of_accounts:
				terminal_framework.flags.country_change = True
				sync_financial_report_templates(self.chart_of_accounts, self.existing_company)
				self.create_default_accounts()
				self.create_default_warehouses()

		if not terminal_framework.db.get_value("Cost Center", {"is_group": 0, "company": self.name}):
			self.create_default_cost_center()

		if terminal_framework.flags.country_change:
			install_country_fixtures(self.name, self.country)
			self.create_default_tax_template()

		if not terminal_framework.db.get_value("Department", {"company": self.name}):
		...
		if self.default_currency:
			terminal_framework.db.set_value("Currency", self.default_currency, "enabled", 1)

		if (
			hasattr(terminal_framework.local, "enable_perpetual_inventory")
			and self.name in terminal_framework.local.enable_perpetual_inventory
		):
			terminal_framework.local.enable_perpetual_inventory[self.name] = self.enable_perpetual_inventory

		if terminal_framework.flags.parent_company_changed:
			from terminal_framework.utils.nestedset import rebuild_tree

			rebuild_tree("Company")

		terminal_framework.clear_cache()
	...
	def create_default_warehouses(self):
		...
		for wh_detail in [
			...
			warehouse = terminal_framework.get_doc(
				{
					"doctype": "Warehouse",
					"warehouse_name": wh_detail["warehouse_name"],
	...
	def create_default_accounts(self):
		from terminal_erp.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts

		terminal_framework.local.flags.ignore_root_company_validation = True
		...
		self.db_set(
			...
			terminal_framework.db.get_value("Account", {"company": self.name, "account_type": "Payable", "is_group": 0}),
		)
	...
	def create_default_departments(self):
		records = [
			# Department
			{
				...
				"is_group": 1,
				"parent_department": "",
				"__condition": lambda: not terminal_framework.db.exists("Department", _("All Departments")),
			},
			{
				"doctype": "Department",
		...
		terminal_framework.local.flags.ignore_update_nsm = True
		make_records(records)
		terminal_framework.local.flags.ignore_update_nsm = False
	...
	def validate_coa_input(self):
		if self.create_chart_of_accounts_based_on == "Existing Company":
			...
			if not self.existing_company:
				terminal_framework.throw(_("Please select Existing Company for creating Chart of Accounts"))
	...
	def validate_perpetual_inventory(self):
		...
		if (
			...
			if terminal_framework.db.get_value("Stock Ledger Entry", {"is_cancelled": 0, "company": self.name}, "name"):
				terminal_framework.throw(
					_(
			...
			if (
				...
				terminal_framework.throw(
					_("Set default {0} account for non stock items").format(
						terminal_framework.bold(_("Provisional Account"))
					)
				)
	...
	def check_country_change(self):
		terminal_framework.flags.country_change = False

		if not self.is_new() and self.country != terminal_framework.get_cached_value("Company", self.name, "country"):
			terminal_framework.flags.country_change = True
	...
	def validate_parent_company(self):
		if self.parent_company:
			is_group = terminal_framework.get_value("Company", self.parent_company, "is_group")

			if not is_group:
				terminal_framework.throw(_("Parent Company must be a group company"))

	def set_reporting_currency(self):
		self.reporting_currency = self.default_currency
		if self.parent_company:
			parent_reporting_currency = terminal_framework.db.get_value(
	...
	def set_default_accounts(self):
		...
		if not self.default_income_account:
			income_account = terminal_framework.db.get_all(
				"Account",
		...
		if not self.write_off_account:
			write_off_acct = terminal_framework.db.get_value(
		...
		if not self.exchange_gain_loss_account:
			exchange_gain_loss_acct = terminal_framework.db.get_value(
		...
		if not self.disposal_account:
			disposal_acct = terminal_framework.db.get_value(
				"Account",
	...
	def set_mode_of_payment_account(self):
		cash = terminal_framework.db.get_value("Mode of Payment", {"type": "Cash"}, "name")
		if (
			cash
			and self.default_cash_account
			and not terminal_framework.db.get_value("Mode of Payment Account", {"company": self.name, "parent": cash})
		):
			mode_of_payment = terminal_framework.get_doc("Mode of Payment", cash, for_update=True)
	...
	def create_default_cost_center(self):
		...
		for cc in cc_list:
			...
			cc_doc = terminal_framework.get_doc(cc)
	...
	def after_rename(self, olddn, newdn, merge=False):
		...
		default_value = terminal_framework.qb.DocType("DefaultValue")
		(
			terminal_framework.qb.update(default_value)
	...
	def on_trash(self):
		...
		if not terminal_framework.db.exists("GL Entry", {"company": self.name}):
			budgets = terminal_framework.get_all("Budget", filters={"company": self.name}, pluck="name")
			if budgets:
				terminal_framework.db.delete("Budget Account", {"parent": ["in", budgets]})

			for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
				terminal_framework.db.delete(doctype, {"company": self.name})

		if not terminal_framework.db.get_value("Stock Ledger Entry", {"company": self.name}):
			terminal_framework.db.delete("Warehouse", {"company": self.name})

		terminal_framework.defaults.clear_default("company", value=self.name)
		for doctype in ["Mode of Payment Account", "Item Default"]:
			terminal_framework.db.delete(doctype, {"company": self.name})

		# clear default accounts, warehouses from item
		warehouses = terminal_framework.get_all("Warehouse", filters={"company": self.name}, pluck="name")
		if warehouses:
			terminal_framework.db.delete("Item Reorder", {"warehouse": ["in", warehouses]})

		# reset default company
		singles = terminal_framework.qb.DocType("Singles")
		(
			terminal_framework.qb.update(singles)
			.set(singles.value, "")
			.where(
		...
		# delete BOMs
		boms = terminal_framework.get_all("BOM", filters={"company": self.name}, pluck="name")
		if boms:
			terminal_framework.db.delete("BOM", {"company": self.name})
			for dt in ("BOM Operation", "BOM Item", "BOM Secondary Item", "BOM Explosion Item"):
				terminal_framework.db.delete(dt, {"parent": ["in", boms]})

		terminal_framework.db.delete("Employee", {"company": self.name})
		terminal_framework.db.delete("Department", {"company": self.name})
		terminal_framework.db.delete("Tax Withholding Account", {"company": self.name})
		terminal_framework.db.delete("Transaction Deletion Record", {"company": self.name})

		# delete tax templates
		terminal_framework.db.delete("Sales Taxes and Charges Template", {"company": self.name})
		terminal_framework.db.delete("Purchase Taxes and Charges Template", {"company": self.name})
		terminal_framework.db.delete("Item Tax Template", {"company": self.name})

		# delete Process Deferred Accounts if no GL Entry found
		if not terminal_framework.db.get_value("GL Entry", {"company": self.name}):
			terminal_framework.db.delete("Process Deferred Accounting", {"company": self.name})

	def check_parent_changed(self):
		terminal_framework.flags.parent_company_changed = False

		if not self.is_new() and self.parent_company != terminal_framework.db.get_value(
			"Company", self.name, "parent_company"
		):
			terminal_framework.flags.parent_company_changed = True
...
def install_country_fixtures(company, country):
	try:
		module_name = f"terminal_erp.regional.{terminal_framework.scrub(country)}.setup.setup"
		terminal_framework.get_attr(module_name)(company, False)
	except ImportError:
		pass
	except Exception:
		terminal_framework.db.rollback()
		terminal_framework.log_error("Unable to set country fixtures")
		terminal_framework.throw(
			_("Failed to setup defaults for country {0}. Please contact support.").format(
				terminal_framework.bold(country)
			)
		)
...
def update_company_current_month_sales(company):
	...
	# Fieldname in standard Terminal ERP is `total_monthly_sales`
...
def update_company_monthly_sales(company):
	...
	from terminal_framework.utils.goal import get_monthly_results
	...
	terminal_framework.db.set_value("Company", company, "sales_monthly_history", json.dumps(month_to_value_dict))
...
def update_transactions_annual_history(company, commit=False):
	...
	terminal_framework.db.set_value("Company", company, "transactions_annual_history", json.dumps(transactions_history))

	if commit and not terminal_framework.in_test:
		terminal_framework.db.commit()


def cache_companies_monthly_sales_history():
	companies = [d["name"] for d in terminal_framework.get_list("Company")]
...
def get_timeline_data(doctype, name):
	...
	history = terminal_framework.get_cached_value("Company", name, "transactions_annual_history")
...
def get_default_company_address(
	...
	addr = terminal_framework.qb.DocType("Address")
	dl = terminal_framework.qb.DocType("Dynamic Link")
	out = (
		terminal_framework.qb.from_(addr)
		.inner_join(dl)
...
def create_transaction_deletion_request(company: str):
	terminal_framework.only_for("System Manager")

	from terminal_erp.setup.doctype.transaction_deletion_record.transaction_deletion_record import (
		is_deletion_doc_running,
	)

	is_deletion_doc_running(company)

	tdr = terminal_framework.get_doc({"doctype": "Transaction Deletion Record", "company": company})
	tdr.insert()
	...
	terminal_framework.msgprint(
		_("Transaction Deletion Document {0} has been triggered for company {1}").format(
			get_link_to_form("Transaction Deletion Record", tdr.name), terminal_framework.bold(company)
		)
	)
