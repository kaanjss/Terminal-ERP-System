...
class TestEmailDigest(Terminal ERPTestSuite):
	...
	def test_get_todo_list_priority_and_date_ordering(self):
		...
		def mk(desc, priority, date):
			...
			terminal_framework.db.set_value("ToDo", td.name, {"priority": priority, "date": date}, update_modified=False)
			return td.name
		...
		rows = terminal_framework.new_doc("Email Digest").get_todo_list(user_id=user)
...
def create_email_digest(**args):
	...
	doc = terminal_framework.new_doc("Email Digest")
