...
import terminal_framework.desk.notifications
from dateutil.relativedelta import relativedelta
from terminal_framework import _
from terminal_framework.core.doctype.user.user import STANDARD_USERS
from terminal_framework.query_builder import Case
from terminal_framework.query_builder.functions import Count, IfNull, Sum
...
from terminal_erp.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.setup.doctype.email_digest_recipient.email_digest_recipient import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = terminal_framework.db.get_value("Company", self.company, "default_currency")

	@terminal_framework.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = terminal_framework.get_all(
			"User",
		...
		terminal_framework.response["user_list"] = user_list

	@terminal_framework.whitelist()
	def send(self):
		# send email only to enabled users
		valid_users = terminal_framework.get_all("User", filters={"enabled": 1}, pluck="name")
	...
	def get_msg_html(self):
		...
		from terminal_erp.setup.doctype.email_digest.quotes import get_random_quote
		...
		terminal_framework.flags.ignore_account_permission = False

		# style
		return terminal_framework.render_template(
			"terminal_erp/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = terminal_framework.desk.notifications.get_notifications()
	...
	def get_calendar_events(self):
		...
		from terminal_framework.desk.doctype.event.event import get_events
	...
	def get_todo_list(self, user_id=None):
		"""Get to-do list"""
		if not user_id:
			user_id = terminal_framework.session.user

		todo = terminal_framework.qb.DocType("ToDo")
		...
		todo_list = (
			terminal_framework.qb.from_(todo)
			.select(todo.star)
	...
	def get_issue_list(self, user_id=None):
		...
		meta = terminal_framework.get_meta("Issue")
		role_permissions = terminal_framework.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = terminal_framework.get_all(
			"Issue",
	...
	def get_issue_count(self):
		"""Get count of Issue"""
		return terminal_framework.db.count("Issue", {"status": ["in", ["Open", "Replied"]]})
	...
	def get_project_list(self, user_id=None):
		...
		project_list = terminal_framework.get_all(
			"Project",
	...
	def get_project_count(self):
		...
		return terminal_framework.db.count("Project", {"status": "Open", "project_type": "External"})
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = terminal_framework.safe_eval(card)

				else:
					card = terminal_framework._dict(getattr(self, "get_" + key)())
	...
	def get_expenses_booked(self):
		...
		expense_account = terminal_framework.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_purchase_orders_to_receive(self):
		...
		value, count = (
			terminal_framework.qb.from_(po)
			.select(
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		doc = terminal_framework.qb.DocType(doc_type)
		value, count, billed_value, delivered_value = (
			terminal_framework.qb.from_(doc)
			.select(
	...
	def get_purchase_orders_items_overdue_list(self):
		po = terminal_framework.qb.DocType("Purchase Order")
		poi = terminal_framework.qb.DocType("Purchase Order Item")
		...
		query = (
			...
			.orderby(poi.parent, order=terminal_framework.qb.desc)
			.orderby(poi.idx)
		)

		items_by_parent = terminal_framework._dict()
...
def send():
	...
	for ed in terminal_framework.get_all(
		"Email Digest", filters={"enabled": 1, "docstatus": ["<", 2]}, fields=["name"], as_list=True
	):
		ed_obj = terminal_framework.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name: str):
	return terminal_framework.get_doc("Email Digest", name).get_msg_html()
