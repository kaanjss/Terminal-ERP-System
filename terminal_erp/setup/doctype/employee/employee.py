...
from terminal_framework import _, scrub, throw
from terminal_framework.model.naming import set_name_by_naming_series
from terminal_framework.permissions import (
	add_user_permission,
	get_doc_permissions,
	remove_user_permission,
)
from terminal_framework.utils import cint, cstr, getdate, today, validate_email_address
...
class EmployeeUserDisabledError(terminal_framework.ValidationError):
	pass


class InactiveEmployeeStatusError(terminal_framework.ValidationError):
	pass
...
class Employee(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.setup.doctype.employee_education.employee_education import EmployeeEducation
		from terminal_erp.setup.doctype.employee_external_work_history.employee_external_work_history import (
			EmployeeExternalWorkHistory,
		)
		from terminal_erp.setup.doctype.employee_internal_work_history.employee_internal_work_history import (
	...
	def validate(self):
		from terminal_erp.controllers.status_updater import validate_status
		...
		if self.user_id:
			...
			existing_user_id = terminal_framework.db.get_value("Employee", self.name, "user_id")
			if existing_user_id:
				user = terminal_framework.get_doc("User", existing_user_id)
	...
	def validate_auto_user_creation(self):
		if self.create_user_automatically and not (
			...
			terminal_framework.throw(
				...
				terminal_framework.MandatoryError,
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = terminal_framework.db.exists(
	...
	def update_user(self):
		...
		user = terminal_framework.get_doc("User", self.user_id)
	...
	def set_preferred_email(self):
		preferred_email_field = terminal_framework.scrub(self.prefered_contact_email)
		self.prefered_email = self.get(preferred_email_field) if preferred_email_field else None

	def validate_status(self):
		if self.status == "Left":
			reports_to = terminal_framework.db.get_all(
				"Employee",
			...
			if reports_to:
				link_to_employees = [
					terminal_framework.utils.get_link_to_form("Employee", employee.name, label=employee.employee_name)
					for employee in reports_to
				]
				message = _("The following employees are currently still reporting to {0}:").format(
					terminal_framework.bold(self.employee_name)
				)
	...
	def validate_for_enabled_user_id(self):
		if not terminal_framework.db.exists("User", self.user_id):
			terminal_framework.throw(_("User {0} does not exist").format(self.user_id))
	...
	def validate_duplicate_user_id(self):
		Employee = terminal_framework.qb.DocType("Employee")
		employee = (
			terminal_framework.qb.from_(Employee)
		...
		if employee:
			throw(
				...
				terminal_framework.DuplicateEntryError,
			)
	...
	def validate_preferred_email(self):
		if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
			terminal_framework.msgprint(_("Please enter {0}").format(self.prefered_contact_email))
	...
	def reset_employee_emails_cache(self):
		...
		if cell_number != prev_number or self.get("user_id") != prev_doc.get("user_id"):
			terminal_framework.cache().hdel("employees_with_number", cell_number)
			terminal_framework.cache().hdel("employees_with_number", prev_number)


def validate_employee_role(doc, method=None, ignore_emp_check=False):
	# called via User hook
	if not ignore_emp_check:
		if terminal_framework.db.get_value("Employee", {"user_id": doc.name}):
			return

	user_roles = [d.role for d in doc.get("roles")]
	if "Employee" in user_roles:
		terminal_framework.msgprint(_("User {0}: Removed Employee role as there is no mapped employee.").format(doc.name))
...
def get_holiday_list_for_employee(employee, raise_exception=True, as_on=None):
	hrms_override = terminal_framework.get_hooks("employee_holiday_list")

	if hrms_override:
		return terminal_framework.get_attr(hrms_override[-1])(employee, raise_exception, as_on)
	if employee:
		holiday_list, company = terminal_framework.get_cached_value("Employee", employee, ["holiday_list", "company"])
	else:
		holiday_list = ""
...
def is_holiday(employee, date=None, raise_exception=True, only_non_weekly=False, with_description=False):
	...
	holidays = terminal_framework.get_all("Holiday", fields=["description"], filters=filters, pluck="description")
...
def deactivate_sales_person(status: str, employee: str):
	terminal_framework.has_permission("Employee", doc=employee, ptype="write", throw=True)
	if status == "Left":
		sales_person = terminal_framework.db.get_value("Sales Person", {"employee": employee})
		if sales_person:
			terminal_framework.db.set_value("Sales Person", sales_person, "enabled", 0)


@terminal_framework.whitelist()
def create_user(employee: str, email: str | None = None, create_user_permission: int = 0) -> str:
	emp = terminal_framework.get_doc("Employee", employee)
	emp.check_permission("write")
	if emp.user_id:
		terminal_framework.throw(_("Employee {0} already has a linked user").format(emp.name))

	if not email:
		terminal_framework.throw(_("Email is required to create a user"))
	...
	user = terminal_framework.new_doc("User")
	user.update(
		{
			"email": email,
			"enabled": 1,
...
def get_all_employee_emails(company):
	...
	employee_list = terminal_framework.get_all(
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = terminal_framework.db.get_value(
...
def get_children(
	...
	employees = terminal_framework.get_list(doctype, fields=fields, filters=filters, order_by="name")

	for employee in employees:
		is_expandable = terminal_framework.get_all(doctype, filters=[["reports_to", "=", employee.get("value")]])
...
def on_doctype_update():
	terminal_framework.db.add_index("Employee", ["lft", "rgt"])
...
def get_contact_details(employee: str) -> dict:
	...
	"""
	if not employee:
		terminal_framework.throw(msg=_("Employee is required"), title=_("Missing Parameter"))

	terminal_framework.has_permission("Employee", "read", employee, throw=True)

	return _get_contact_details(employee)


def _get_contact_details(employee: str) -> dict:
	contact_data = terminal_framework.db.get_value(
		"Employee",
		employee,
		[
			"employee_name",
	...
	if not contact_data:
		terminal_framework.throw(msg=_("Employee {0} not found").format(employee), title=_("Not Found"))
