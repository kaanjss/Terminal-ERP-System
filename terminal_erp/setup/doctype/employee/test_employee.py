...
from terminal_erp.setup.doctype.employee.employee import InactiveEmployeeStatusError
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestEmployee(Terminal ERPTestSuite):
	def test_employee_status_left(self):
		...
		employee1_doc = terminal_framework.get_doc("Employee", employee1)
		employee2_doc = terminal_framework.get_doc("Employee", employee2)
	...
	def test_user_has_employee(self):
		...
		employee_doc = terminal_framework.get_doc("Employee", employee)
		user = employee_doc.user_id
		self.assertIn("Employee", terminal_framework.get_roles(user))
		employee_doc.user_id = ""
		employee_doc.save()
		self.assertNotIn("Employee", terminal_framework.get_roles(user))
	...
	def test_employee_user_permission(self):
		...
		terminal_framework.set_user(employee1_doc.user_id)
		...
		employee_list = terminal_framework.db.get_list("Employee", pluck="name", order_by="name")
	...
	def test_create_user_automatically(self):
		...
		user = terminal_framework.db.get_value("User", "test_auto_user1@example.com")
		...
		user2 = terminal_framework.db.get_value("User", "test_auto_user2@example.com")
...
def make_employee(user, company=None, **kwargs):
	if not terminal_framework.db.get_value("User", user):
	...
	if not terminal_framework.db.get_value("Employee", {"user_id": user}):
		employee = terminal_framework.get_doc(
			{
				"doctype": "Employee",
				"naming_series": "EMP-",
				"first_name": user,
				...
				"date_of_joining": "2013-01-01",
				"department": terminal_framework.get_all("Department", fields="name")[0].name,
				"gender": "Female",
				"company_email": user,
		...
		employee = terminal_framework.get_doc("Employee", {"employee_name": user})
		employee.update(kwargs)
