...
terminal_erp.setup.EmployeeController = class EmployeeController extends terminal_framework.ui.form.Controller {
	setup() {
		this.frm.fields_dict.user_id.get_query = function (doc, cdt, cdn) {
			return {
				query: "terminal_framework.core.doctype.user.user.user_query",
...
terminal_framework.ui.form.on("Employee", {
	setup: function (frm) {
		frm.make_methods = {
			...
			frm.add_custom_button(__("Create User"), () => {
				const dialog = new terminal_framework.ui.Dialog({
					...
					primary_action: (values) => {
						if (!values.email) {
							terminal_framework.msgprint(__("Email is required to create a user."));
							return;
						}

						terminal_framework
							.call({
								method: "terminal_erp.setup.doctype.employee.employee.create_user",
								args: {
	...
	build_anniversary_content: function (frm) {
			...
			items.push(`
				<div class="form-sidebar-items milestone-item">
					<span class="form-sidebar-label">
						${terminal_framework.utils.icon("cake", "sm")}
		...
		if (frm.events.is_work_anniversary(frm, today)) {
			...
			items.push(`
				<div class="form-sidebar-items milestone-item">
					<span class="form-sidebar-label">
						${terminal_framework.utils.icon("briefcase", "sm")}
	...
	update_contact: function (frm) {
		var prefered_email_fieldname = terminal_framework.model.scrub(frm.doc.prefered_contact_email) || "user_id";
...
cur_frm.cscript = new terminal_erp.setup.EmployeeController({
	frm: cur_frm,
});

terminal_framework.tour["Employee"] = [
	{
