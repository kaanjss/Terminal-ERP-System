...
from terminal_framework.desk.notifications import clear_notifications
from terminal_framework.model.document import Document
from terminal_framework.query_builder.functions import Max
from terminal_framework.utils import cint, comma_and, create_batch, get_link_to_form
from terminal_framework.utils.background_jobs import get_job, is_job_enqueued
...
def get_company_link_fields(doctype_name: str):
	...
	"""
	terminal_framework.only_for("System Manager")
	if not doctype_name or not terminal_framework.db.exists("DocType", doctype_name):
		return []
...
def _get_protected_doctypes_internal():
	...
	for doctype in PROTECTED_CORE_DOCTYPES:
		if terminal_framework.db.exists("DocType", doctype):
			protected.append(doctype)

	singles = terminal_framework.get_all("DocType", filters={"issingle": 1}, pluck="name")
...
class TransactionDeletionRecord(Document):
	...
	if TYPE_CHECKING:
		...
		from terminal_erp.accounts.doctype.transaction_deletion_record_details.transaction_deletion_record_details import (
			TransactionDeletionRecordDetails,
		)
		from terminal_erp.setup.doctype.transaction_deletion_record_item.transaction_deletion_record_item import (
			TransactionDeletionRecordItem,
		)
		from terminal_erp.setup.doctype.transaction_deletion_record_to_delete.transaction_deletion_record_to_delete import (
	...
	def validate_to_delete_list(self):
		...
		for item in self.doctypes_to_delete:
			if not terminal_framework.db.exists("DocType", item.doctype_name):
				terminal_framework.throw(_("DocType {0} does not exist").format(item.doctype_name))
			...
			is_child_table = terminal_framework.db.get_value("DocType", item.doctype_name, "istable")
			if is_child_table:
				terminal_framework.throw(
					_(
			...
			is_virtual = terminal_framework.db.get_value("DocType", item.doctype_name, "is_virtual")
			if is_virtual:
				terminal_framework.throw(
					_(
	...
	def before_submit(self):
		if queued_docs := terminal_framework.db.get_all(
			"Transaction Deletion Record",
		...
		if not self.doctypes_to_delete and not self.doctypes_to_be_ignored:
			terminal_framework.throw(_("Please generate To Delete list before submitting"))
	...
	def _set_deletion_cache(self):
		...
		for item in self.doctypes_to_delete:
			terminal_framework.cache.set_value(
	...
	def _clear_deletion_cache(self):
		...
		for item in self.doctypes_to_delete:
			terminal_framework.cache.delete_value(f"deletion_running_doctype:{item.doctype_name}")
	...
	def _get_child_tables(self, doctype_name):
		...
		"""
		child_tables = terminal_framework.get_all(
			"DocField",
	...
	def _get_to_delete_row_infos(self, doctype_name, company_field=None, company=None):
		...
		if company_field and company:
			doc_count = terminal_framework.db.count(doctype_name, filters={company_field: company})
		else:
			doc_count = terminal_framework.db.count(doctype_name)
	...
	def _has_company_field(self, doctype_name):
		...
		return terminal_framework.db.exists(
			"DocField",
	...
	def _get_company_link_fields(self, doctype_name):
		...
		Returns:
		        list: List of field names that link to Company DocType, ordered by field index
		"""
		company_fields = terminal_framework.get_all(
			"DocField",
			filters={"parent": doctype_name, "fieldtype": "Link", "options": "Company"},
			pluck="fieldname",
			order_by="idx",
	...
	def generate_to_delete_list(self):
		...
		doctypes_with_company_field = terminal_framework.get_all(
			"DocField",
		...
		for doctype_name in doctypes_with_company_field:
			...
			if terminal_framework.db.exists("DocType", doctype_name):
				meta = terminal_framework.get_meta(doctype_name)
		...
		for doctype_name in doctypes_with_company:
			...
			for company_field in company_fields:
				doc_count = terminal_framework.db.count(doctype_name, {company_field: self.company})
	...
	def populate_doctype_details(
		...
		if not terminal_framework.db.exists("DocType", doctype_name):
			terminal_framework.throw(_("DocType {0} does not exist").format(doctype_name))

		is_child_table = terminal_framework.db.get_value("DocType", doctype_name, "istable")
		if is_child_table:
			return {
				"child_doctypes": "",
				"document_count": 0,
	...
	def export_to_delete_template_method(self):
		...
		if not self.doctypes_to_delete:
			terminal_framework.throw(_("Generate To Delete list first"))
		...
		terminal_framework.response["result"] = output.getvalue()
		terminal_framework.response["type"] = "csv"
		terminal_framework.response[
			"doctype"
		] = f"deletion_template_{self.company}_{terminal_framework.utils.now_datetime().strftime('%Y%m%d')}"
	...
	def import_to_delete_template_method(self, csv_content):
		...
		if "doctype_name" not in (reader.fieldnames or []):
			terminal_framework.throw(_("Invalid CSV format. Expected column: doctype_name"))
		...
		for row in reader:
			...
			is_child = terminal_framework.db.get_value("DocType", doctype_name, "istable")
			if is_child:
				skipped.append(_("{0}: Child table (auto-deleted with parent)").format(doctype_name))
				continue

			is_virtual = terminal_framework.db.get_value("DocType", doctype_name, "is_virtual")
			if is_virtual:
		...
		if task and task in self.task_to_internal_method_map:
			...
			if self.process_in_single_transaction:
				...
				terminal_framework.enqueue(
					"terminal_framework.utils.background_jobs.run_doc_method",
		...
		if task_to_execute:
			...
			if task := getattr(self, method, None):
				try:
					...
					traceback = terminal_framework.get_traceback(with_context=True)
					if traceback:
						message = "Traceback: <br>" + traceback
						terminal_framework.db.set_value(self.doctype, self.name, "error_log", message)
					terminal_framework.db.set_value(self.doctype, self.name, "status", "Failed")
					self._clear_deletion_cache()
	...
	def delete_bins(self):
		...
		if self.delete_bin_data_status == "Pending":
			...
			warehouses = terminal_framework.get_all("Warehouse", filters={"company": self.company}, pluck="name")
			if warehouses:
				terminal_framework.db.delete("Bin", {"warehouse": ["in", warehouses]})
	...
	def delete_lead_addresses(self):
		...
		if self.delete_leads_and_addresses_status == "Pending":
			...
			leads = terminal_framework.db.get_all("Lead", filters={"company": self.company}, pluck="name")
			addresses = []
			if leads:
				addresses = terminal_framework.db.get_all(
	...
	def delete_company_transactions(self):
		...
		if self.delete_transactions_status == "Pending":
			...
			for docfield in self.doctypes:
				if docfield.doctype_name != self.doctype and not docfield.done:
					if docfield.doctype_name in protected_doctypes:
						...
						terminal_framework.log_error(error_msg, "Transaction Deletion Security")
						terminal_framework.throw(
					...
					if company_field:
						...
						no_of_docs = terminal_framework.db.count(docfield.doctype_name)

					if no_of_docs > 0:
						if company_field:
							reference_docs = terminal_framework.get_all(
						...
						terminal_framework.db.set_value(docfield.doctype, docfield.name, "no_of_docs", processed)
					else:
						naming_series = terminal_framework.db.get_value("DocType", docfield.doctype_name, "autoname")
						if naming_series:
							if "#" in naming_series:
								self.update_naming_series(naming_series, docfield.doctype_name)
						terminal_framework.db.set_value(docfield.doctype, docfield.name, "done", 1)

						to_delete_row = terminal_framework.db.get_value(
			...
			pending_doctypes = terminal_framework.db.get_all(
	...
	def get_doctypes_to_be_ignored_list(self):
		doctypes_to_be_ignored_list = terminal_framework.get_all(
	...
	def get_doctypes_with_company_field(self, doctypes_to_be_ignored_list):
		docfields = terminal_framework.get_all(
			"DocField",
			filters={
				"fieldtype": "Link",
				"options": "Company",
	...
	def get_all_child_doctypes(self):
		return terminal_framework.get_all("DocType", filters={"istable": 1}, pluck="name")

	def get_number_of_docs_linked_with_specified_company(self, doctype, company_fieldname):
		return terminal_framework.db.count(doctype, {company_fieldname: self.company})
	...
	def delete_child_tables(self, doctype, reference_doc_names):
		...
		for table in child_tables:
			terminal_framework.db.delete(table, {"parent": ["in", reference_doc_names]})

	def delete_docs_linked_with_specified_company(self, doctype, reference_doc_names):
		terminal_framework.db.delete(doctype, {"name": ("in", reference_doc_names)})
	...
	def get_naming_series_prefix(naming_series: str, doctype_name: str) -> str:
		...
		if "." in naming_series:
			...
			# Fallback for unexpected patterns (shouldn't happen with valid Terminal Framework naming series)
			terminal_framework.log_error(
	...
	def update_naming_series(self, naming_series, doctype_name):
		...
		terminal_framework.db.set_value("Series", prefix, "current", last, update_modified=False)
	...
	def delete_communications(self, doctype, reference_doc_names):
		...
		for batch in create_batch(communication_names, self.batch_size):
			terminal_framework.delete_doc("Communication", batch, ignore_permissions=True)
	...
	def unlink_attachments(self, doctype, reference_doc_names):
		files = terminal_framework.get_all(
			"File",
...
def get_doctypes_to_be_ignored():
	...
	doctypes_to_be_ignored.extend(terminal_framework.get_hooks("company_data_to_be_ignored") or [])
...
def export_to_delete_template(name: str):
	...
	terminal_framework.only_for("System Manager")
	doc = terminal_framework.get_doc("Transaction Deletion Record", name)
...
def process_import_template(transaction_deletion_record_name: str, file_url: str):
	...
	doc = terminal_framework.get_doc("Transaction Deletion Record", transaction_deletion_record_name)
	doc.check_permission("write")

	if not file_url or ".." in file_url:
		terminal_framework.throw(_("Invalid file URL"))

	try:
		file_doc = terminal_framework.get_doc("File", {"file_url": file_url})
	except terminal_framework.DoesNotExistError:
		terminal_framework.throw(_("File not found"))

	if (
		file_doc.attached_to_doctype != "Transaction Deletion Record"
		or file_doc.attached_to_name != transaction_deletion_record_name
	):
		terminal_framework.throw(_("File does not belong to this Transaction Deletion Record"))

	if not file_doc.file_name or not file_doc.file_name.lower().endswith(".csv"):
		terminal_framework.throw(_("Only CSV files are allowed"))

	file_path = file_doc.get_full_path()

	if not os.path.isfile(file_path):
		terminal_framework.throw(_("File not found on server"))
...
def is_deletion_doc_running(company: str | None = None, err_msg: str | None = None):
	...
	"""
	running_deletion_job = terminal_framework.db.get_value(
		"Transaction Deletion Record",
...
def check_for_running_deletion_job(doc, method=None):
	...
	deletion_name = terminal_framework.cache.get_value(f"deletion_running_doctype:{doc.doctype}")
	...
	if deletion_name:
		terminal_framework.throw(
			...
			msg=_(
				...
				get_link_to_form("Transaction Deletion Record", deletion_name), terminal_framework.bold(doc.doctype)
			),
		)
