...
terminal_framework.ui.form.on("Item Group", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button(__("Item Group Tree"), function () {
			terminal_framework.set_route("Tree", "Item Group");
		});

		if (!frm.is_new()) {
			frm.add_custom_button(__("Items"), function () {
				terminal_framework.set_route("List", "Item", { item_group: frm.doc.name });
			});
		}
	},
...
terminal_framework.ui.form.on("Item Default", {
	form_render: function (frm, cdt, cdn) {
		...
		const row = locals[cdt][cdn];
		if (!row || !row.company) {
			Object.values(COMPANY_DEFAULTS_TO_VF).forEach((vf) => terminal_framework.model.set_value(cdt, cdn, vf, ""));
			return;
		}
	...
	terminal_framework.call({
		...
		callback: function (r) {
			...
			Object.entries(COMPANY_DEFAULTS_TO_VF).forEach(([key, vf_field]) => {
				terminal_framework.model.set_value(cdt, cdn, vf_field, defaults[key] || "—");
			});
