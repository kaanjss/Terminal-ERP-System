...
class ItemGroup(NestedSet):
	...
	def validate(self):
		if not self.parent_item_group and not terminal_framework.in_test:
			if terminal_framework.db.exists("Item Group", _("All Item Groups")):
				...
				if (d.item_tax_template, d.tax_category) in check_list:
					terminal_framework.throw(
						_("{0} entered twice {1} in Item Taxes").format(
							...
							_("for tax category {0}").format(terminal_framework.bold(d.tax_category))
							if d.tax_category
							else "",
						)
					)
	...
	def delete_child_item_groups_key(self):
		terminal_framework.cache().hdel("child_item_groups", self.name)

	def validate_item_group_defaults(self):
		from terminal_erp.stock.doctype.item.item import validate_item_default_company_links

		validate_item_default_company_links(self.item_group_defaults)


def get_child_item_groups(item_group_name):
	item_group = terminal_framework.get_cached_value("Item Group", item_group_name, ["lft", "rgt"], as_dict=1)
...
def get_item_group_defaults(item, company):
	...
	item_group = terminal_framework.get_cached_doc("Item Group", item.item_group)
...
def get_company_resolved_defaults(company: str) -> dict:
	...
	company_doc = terminal_framework.get_cached_doc("Company", company)
	default_warehouse = terminal_framework.db.get_single_value("Stock Settings", "default_warehouse")
