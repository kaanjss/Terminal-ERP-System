...
from terminal_erp.setup.doctype.holiday_list.holiday_list import local_country_name
from terminal_erp.tests.utils import Terminal ERPTestSuite


class TestHolidayList(Terminal ERPTestSuite):
	def test_holiday_list(self):
		...
		fetched_holiday_list = terminal_framework.get_value("Holiday List", holiday_list.name)
		self.assertEqual(holiday_list.name, fetched_holiday_list)

	def test_weekly_off(self):
		holiday_list = terminal_framework.new_doc("Holiday List")
...
def make_holiday_list(name, from_date=None, to_date=None, holiday_dates=None):
	...
	terminal_framework.delete_doc_if_exists("Holiday List", name, force=1)
