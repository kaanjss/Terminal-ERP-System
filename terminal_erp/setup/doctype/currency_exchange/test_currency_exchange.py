...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			terminal_framework.set_value(**kwargs)
		except terminal_framework.DoesNotExistError:
			curr_exchange = terminal_framework.new_doc(record.get("doctype"))
	...
	class PatchResponse:
		...
		def raise_for_status(self):
			if self.status_code != 200:
				raise terminal_framework.DoesNotExistError
...
class TestCurrencyExchange(Terminal ERPTestSuite):
	def setUp(self):
	...
	def test_exchange_rate(self, mock_get):
		terminal_framework.db.set_single_value("Accounts Settings", "allow_stale", 1)
	...
	def test_exchange_rate_via_exchangerate_host(self, mock_get):
		...
		settings = terminal_framework.get_single("Currency Exchange Settings")
	...
	def test_exchange_rate_strict(self, mock_get):
		...
		terminal_framework.db.set_single_value("Accounts Settings", "allow_stale", 0)
		terminal_framework.db.set_single_value("Accounts Settings", "stale_days", 1)
